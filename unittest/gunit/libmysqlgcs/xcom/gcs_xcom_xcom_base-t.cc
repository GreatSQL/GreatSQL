/* Copyright (c) 2018, 2024, Oracle and/or its affiliates.
   Copyright (c) 2025, 2026, GreatDB Software Co., Ltd.
...
TEST_F(XcomBase, PaxosLearnSameValue) {
  ...
  link_init(&s0_paxos->watchdog, TYPE_HASH("time_queue"));

  auto *s1_paxos =
      static_cast<pax_machine *>(std::calloc(1, sizeof(pax_machine)));
  init_pax_machine(s1_paxos, nullptr, synod);
  link_init(&s1_paxos->watchdog, TYPE_HASH("time_queue"));

  auto *s2_paxos =
      static_cast<pax_machine *>(std::calloc(1, sizeof(pax_machine)));
  init_pax_machine(s2_paxos, nullptr, synod);
  link_init(&s2_paxos->watchdog, TYPE_HASH("time_queue"));
  ...
  pax_msg *s0_ack_accept_tx = handle_simple_accept(
      s0_paxos, s0_accept_tx_s0, s0_accept_tx_s0->synode, false);
  ASSERT_NE(nullptr, s0_ack_accept_tx);
  s0_ack_accept_tx->from = 0;

  pax_msg *s0_accept_tx_s1 = clone_pax_msg(s0_accept_tx);
  pax_msg *s1_ack_accept_tx = handle_simple_accept(
      s1_paxos, s0_accept_tx_s1, s0_accept_tx_s1->synode, false);
  ASSERT_NE(nullptr, s1_ack_accept_tx);
  s1_ack_accept_tx->from = 1;

  pax_msg *s0_accept_tx_s2 = clone_pax_msg(s0_accept_tx);
  pax_msg *s2_ack_accept_tx = handle_simple_accept(
      s2_paxos, s0_accept_tx_s2, s0_accept_tx_s2->synode, false);
  ...
  pax_msg *s1_ack_accept_noop = handle_simple_accept(
      s1_paxos, clone_pax_msg(s1_accept_noop), synod, false);
  s1_ack_accept_noop->from = 1;

  pax_msg *s2_ack_accept_noop = handle_simple_accept(
      s2_paxos, clone_pax_msg(s1_accept_noop), synod, false);
