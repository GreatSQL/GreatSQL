...
/**
   Calc days in one months.
   @note There is already an array 'days_in_month' denotes the days of months
         for common years(365-day year), the leap year(366-day) need to be
         considered as well.

   @return number of days in the month of a specified year
*/
uint calc_days_in_month(uint year, uint month) {
  int ret_days = days_in_month[month - 1];
  if (month == 2 && calc_days_in_year(year) == 366) ret_days++;
  return ret_days;
}
/**
...
static const char *RMs[] = {"XII", "XI", "X",   "IX", "VIII", "VII", "VI",
                            "V",   "IV", "III", "II", "I",    NullS};
static const char *Rms[] = {"Xii", "Xi", "X",   "Ix", "Viii", "Vii", "Vi",
                            "V",   "Iv", "Iii", "Ii", "I",    NullS};
static const char *rms[] = {"xii", "xi", "x",   "ix", "viii", "vii", "vi",
...
const char *month_to_roman_numeral(int month, int flag) {
  if (flag > 0) {
    return RMs[12 - month];
  } else if (flag < 0) {
    return rms[12 - month];
...
unsigned int roman_numeral_to_month(const char **find, const char *end) {
  if (find && end) {
    char firstc;
    const char *start_find = *find;
    if (start_find && start_find < end) {
      firstc = toupper(*start_find);

      for (const char *const *rm = RMs; *rm != NULL; ++rm) {
        if (firstc != **rm) {
          continue;
        }

        const char *rm_next = *rm + 1;
        const char *find_next = start_find + 1;
        for (;; ++rm_next, ++find_next) {
          if (*rm_next == '\0') {
            *find += rm_next - *rm;
            return 12 - (rm - RMs);
          }

          if (find_next == end || toupper(*find_next) != *rm_next) {
            break;
          }
        }
      }
...
/**
 a formula for calculating the Julian n_day number:
   a = (14 - n_month) / 12
   y = n_year+ 4800 - a;
   m = n_month + 12 * a - 3;
  For a date in the Gregorian calendar:
   JD = n_day + (153*m+2)/5 + y*365 + y/4 - y/100 + y/400 - 32045
  For a date in the Julian calendar:
   JD = n_day + (153*m+2)/5 + y*365 + y/4 - 32083

 Note: 1582-10-15 was the first n_day of Gregorian calendar
*/
int date_to_julianday(int n_year, int n_month, int n_day) {
  int a = (14 - n_month) / 12;
  int y = n_year + 4800 - a;
  int m = n_month + 12 * a - 3;
  if (n_year > 1582 || (n_year == 1582 && n_month > 10) ||
      (n_year == 1582 && n_month == 10 &&
       n_day >= 5)) {  // for Gregorian calendar
    n_day = (n_year == 1582 && n_month == 10 && n_day < 15) ? 15 : n_day;
    return (n_day + (153 * m + 2) / 5 + 365 * y + y / 4 - y / 100 + y / 400 -
            32045);
  } else {  // for Julian calendar
    return (n_day + (153 * m + 2) / 5 + 365 * y + y / 4 - 32083);
  }
}

/**
 convert a Julian n_day number to a n_day, n_month, and n_year:
  ...
  Then, for both calendars:
   ...
   n_day   = e - (153*m+2)/5 + 1
   n_month = m + 3 - 12*(m/10)
   n_year  = b*100 + d - 4800 + m/10

 Note: 2299161 was the Julian n_day number of 1582-10-15
*/
void julianday_to_date(int jd, int &n_year, int &n_month, int &n_day) {
  int b;
  int c;
  if (jd >= 2299161) {  // for Gregorian calendar
    int a;
    a = jd + 32044;
    b = (4 * a + 3) / 146097;
    c = a - (b * 146097) / 4;
...
int julianday_to_day_of_week(int jd, bool sunday_first_day_of_week) {
  int n_day_of_week = (jd + 1) % 7;
  if (n_day_of_week < 0) n_day_of_week += 7;

  return sunday_first_day_of_week ? n_day_of_week : (n_day_of_week + 6L) % 7;
}

int date_to_ISO_week(int n_year, int n_month, int n_day) {
  float result;
  int day0, day4, dayn;

  /* current n_day */
  dayn = date_to_julianday(n_year, n_month, n_day);

  /* fourth n_day of current n_year */
  day4 = date_to_julianday(n_year, 1, 4);

  /* day0 == offset to first n_day of week (Monday) */
  day0 = julianday_to_day_of_week(day4 - 1);

  /*
   * if this n_day earlier than the first n_day of the n_year, then it falls
   * into the previous n_year
   */
  if (dayn < day4 - day0) {
    day4 = date_to_julianday(n_year - 1, 1, 4);

    /* day0 == offset to first n_day of week (Monday) */
    day0 = julianday_to_day_of_week(day4 - 1);
  }

  /* calculate the total week between this n_day and first n_day of the n_year
   */
  result = (dayn - (day4 - day0)) / 7 + 1;

  /*
   * Sometimes the last few days in a n_year will fall into the first week of
   * the next n_year, so check for this.
   */
  if (result >= 52) {
    day4 = date_to_julianday(n_year + 1, 1, 4);
    ...
    if (dayn >= day4 - day0) result = (dayn - (day4 - day0)) / 7 + 1;
  }
