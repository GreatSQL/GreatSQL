...
bool dynstr_concat_mem_internal(DYNAMIC_STRING *str, const char *append,
                                size_t length, bool is_prepend = false) {
  char *new_ptr;
  ...
  if (length > 0) {
    if (is_prepend) {
      memmove(str->str + length, str->str, str->length + 1);
      memcpy(str->str, append, length);
    } else
...
bool dynstr_append_mem(DYNAMIC_STRING *str, const char *append, size_t length) {
  return dynstr_concat_mem_internal(str, append, length, false);
}

bool dynstr_prepend(DYNAMIC_STRING *str, const char *prepend) {
  return dynstr_concat_mem_internal(str, prepend, (uint)strlen(prepend), true);
}
...
bool dynstr_append_os_quoted(DYNAMIC_STRING *str, const char *append, ...) {
  ...
  // cppcheck-suppress-begin va_end_missing
  va_start(dirty_text, append);
  return dynstr_append_quoted_inner(str, quote_str, quote_len, append,
                                    dirty_text);
  // cppcheck-suppress-end va_end_missing
}
