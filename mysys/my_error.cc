...
/**
 * The difference between my_strerror and my_sterror_oserror is the latter
 * function only handles linux kernel error number. However, the my_strerror
 * will overwrite the kernel error code information within the range of
 * 120 to 210 as the information defined by mysql itself.
  Get a string describing a system or handler error. thread-safe.
  ...
  @returns buf always buf. for signature compatibility with strerror(3).
*/
char *my_strerror_oserror(char *buf, size_t len, int nr) {
  buf[0] = '\0'; /* failsafe */
