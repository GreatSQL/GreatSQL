...
/**
  Implementation of the get thread id interface
  only for pfs_thread_provider.h not support
  service interface and components
*/
ulonglong pfs_get_thread_id_vc(PSI_thread *thread) {
  PFS_thread *pfs = reinterpret_cast<PFS_thread *>(thread);
  if (unlikely(pfs == nullptr)) {
    return 0;
  }
  return pfs->m_processlist_id;
}

/**
  Implementation of the get thread THD interface
  only for pfs_thread_provider.h not support
  service interface and components
*/
THD *pfs_get_thread_THD_vc(PSI_thread *thread) {
  PFS_thread *pfs = reinterpret_cast<PFS_thread *>(thread);
  if (unlikely(pfs == nullptr)) {
    return nullptr;
  }
