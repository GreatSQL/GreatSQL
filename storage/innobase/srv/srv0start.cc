...
dberr_t srv_start(bool create_new_db) {
  ...
  if (create_new_db) {
    ...
    if (clone_incremental_dir != nullptr) {
      ...
      sprintf(errmsg, "replace redo files from clone_incremental_dir");
      ib::info(ER_CLONE_APPLY_FILE, errmsg);
      try {
        // rm files in full
        if (std::filesystem::remove_all(full_clone_redo) == 0) {
          sprintf(
              errmsg,
              "Error drop old redo files, #innodb_redo directory not exists");
          ib::error(ER_CLONE_APPLY_FILE_ERROR, errmsg);
          exit(EXIT_FAILURE);
        }
        // copy files from inc
        std::filesystem::copy(inc_clone_redo, full_clone_redo,
                              std::filesystem::copy_options::recursive);
      } catch (std::filesystem::filesystem_error &e) {
        sprintf(errmsg,
                "Error replace redo files from clone_incremental_dir: %s",
                e.what());
        ib::error(ER_CLONE_APPLY_FILE_ERROR, errmsg);
        exit(EXIT_FAILURE);
      }
