                                         ...
                                         for reloading FTS table */
{
  ...
  const char *case_stopword_to_use = nullptr;
  byte case_str_buffer[MAX_FULL_NAME_LEN + 1] = {0};
  ...
  if (reload) {
    ...
    if (strlen((char *)str.f_str) > 0) {
      ...
      memcpy(case_str_buffer, str_buffer, sizeof(str_buffer));
      case_stopword_to_use = (const char *)(case_str_buffer);
    }
  } else {
    stopword_to_use = (session_stopword_table) ? session_stopword_table
                                               : global_stopword_table;
    if (stopword_to_use) {
      strncpy(reinterpret_cast<char *>(case_str_buffer), stopword_to_use,
              sizeof(case_str_buffer) - 1);
      case_stopword_to_use = reinterpret_cast<const char *>(case_str_buffer);
    }
  }

  if (lower_case_table_names >= 1 && nullptr != case_stopword_to_use) {
    /** when lower_case_table_names>=1 means:
     * the db/table name compare is case-insensitive,
     * use down-case to compare, so we do case conversion
     */
    my_casedn_str(files_charset_info, const_cast<char *>(case_stopword_to_use));
  }

  if (case_stopword_to_use &&
      fts_load_user_stopword(case_stopword_to_use, &cache->stopword_info)) {
    /* Save the stopword table name to the configure
    table */
    if (!reload) {
      str.f_n_char = 0;
      str.f_str = (byte *)case_stopword_to_use;
      str.f_len = ut_strlen(case_stopword_to_use);
