...
written */
{
  ...
  if (mtr != nullptr) {
    ...
    // when enable binlog, it will set HA_IGNORE_DURABILITY for thd,
    // flush_logs will delay when binlog file rotate.
    // while for detach xa commit, trx->mysql_thd is empty, but not
    // current_thd, HA_IGNORE_DURABILITY is missing, lead double
    // flush_logs operation, which is expensive
    bool ignore_flush_log = false;
    if (trx->mysql_thd) {
      ignore_flush_log =
          (thd_requested_durability(trx->mysql_thd) == HA_IGNORE_DURABILITY);
    } else if (trx->xid) {
      // external xid
      my_xid xid = trx->xid->get_my_xid();
      if (!xid) {
        ignore_flush_log =
            (thd_requested_durability(current_thd) == HA_IGNORE_DURABILITY);
      }
    }
    if (lsn == 0) {
      /* Nothing to be done. */
    ...
    } else if ((srv_flush_log_at_trx_commit == 0 || ignore_flush_log) &&
               (!trx->ddl_operation)) {
      /* Do nothing */
    } else {
