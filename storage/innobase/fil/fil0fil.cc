...
fil_node_t *Fil_shard::create_node(const char *name, page_no_t size,
  ...
  /** if the function call failure, the stat info block size will keep unchange
   */
  os_get_fs_block_size(file.name, &stat_info);
...
dberr_t Tablespace_dirs::scan() {
  ...
  if (err == DB_SUCCESS /*&& populate_fil_cache*/ &&
    ...
    par_for(PFS_NOT_INSTRUMENTED, ibd_files, n_threads, open, std::ref(result));
