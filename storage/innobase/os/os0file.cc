...
static dberr_t os_get_fs_block_size_posix(const char *path,
                                          uint64_t &block_size) {
  struct statvfs stat;
  auto ret = statvfs(path, &stat);
...
[[maybe_unused]] static dberr_t os_get_fs_block_size_posix(
    int fd, uint64_t &block_size) {
  struct statvfs stat;
  auto ret = fstatvfs(fd, &stat);
...
dberr_t os_get_fs_block_size(const char *path, os_file_stat_t *stat_info) {
  dberr_t ret = DB_SUCCESS;

#ifdef _WIN32
  /** on windows platform, for punch hole, the size is correct, so we need not
   * fix it */

#else
  uint64_t block_size = 0;
  ret = os_get_fs_block_size_posix(path, block_size);

  if (ret == DB_SUCCESS) {
    stat_info->block_size = block_size;
  }
