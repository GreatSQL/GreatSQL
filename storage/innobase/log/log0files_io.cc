...
static dberr_t log_check_file(const Log_files_context &ctx, Log_file_id file_id,
  ...
  // adapt to encrypt colne operation
  {
    os_offset_t dst_size_in_bytes = 0;
    auto reset_handle = os_file_create(innodb_log_file_key, file_path.c_str(),
                                       OS_FILE_OPEN, OS_LOG_FILE, false, &ret);
    if (!ret) {
      ib::error(ER_IB_MSG_LOG_FILE_OPEN_FAILED, file_path.c_str(),
                static_cast<int>(DB_ERROR));
      return DB_ERROR;
    }
    size_in_bytes = os_file_get_size(reset_handle);
    if (size_in_bytes < LOG_FILE_MIN_SIZE) {
      dst_size_in_bytes = LOG_FILE_MIN_SIZE;
    } else if (size_in_bytes % UNIV_PAGE_SIZE != 0) {
      dst_size_in_bytes = (size_in_bytes / UNIV_PAGE_SIZE + 1) * UNIV_PAGE_SIZE;
    }
    if (size_in_bytes < dst_size_in_bytes) {
      bool success = os_file_set_size(file_path.c_str(), reset_handle,
                                      size_in_bytes, dst_size_in_bytes, true);
      if (!success) {
        char errbuf[MYSYS_STRERROR_SIZE];
        ib::error(ER_ERROR_ON_WRITE, MYF(0), file_path.c_str(), errno,
                  my_strerror(errbuf, sizeof(errbuf), errno));
        return DB_ERROR;
      }
    }

    ret = os_file_close(reset_handle);
  }
