...
#ifndef WITH_TURBO_MTR_TEST
#define DEFAULT_PARALLEL_READ 4
#else
#define DEFAULT_PARALLEL_READ 1
#endif
    ...
    const dd::Table *dd_table) /*!< in: dd::Table for the table*/
{
  ...
  /* NOTE: index->nulls_equal will be set even for temporary table */
    ...
    *trx->xid = *xid;
    ...
    if (trx_is_prepared_in_tc(trx)) {
      ...
      if (undo_ptr->is_update()) {
        undo_ptr->update_undo->m_gtid_storage =
            trx_undo_t::Gtid_storage::PREPARE_AND_COMMIT;
      }
      return XA_OK;
    }
...
static int innodb_stopword_table_validate(THD *thd, SYS_VAR *, void *save,
  ...
  int len = sizeof(buff);
  int ret = 1;
  uint32_t case_ignore_buffer_len = (MAX_FULL_NAME_LEN + 1);
  char *case_ignore_buffer = nullptr;
  const char *case_ignore_table_name = nullptr;
  ...
  /** we do the conversion here, since we can use the converted name
   * for 1:  internal table name; 2: the variable value.
   */
  case_ignore_table_name = stopword_table_name;
  if (lower_case_table_names >= 1 && nullptr != stopword_table_name) {
    ut_a((static_cast<uint32_t>(len) + 1) <= case_ignore_buffer_len);
    case_ignore_buffer =
        static_cast<char *>(thd_calloc(thd, case_ignore_buffer_len));
    ut_a(case_ignore_buffer);
    memcpy(case_ignore_buffer, stopword_table_name, len);
    my_casedn_str(files_charset_info, const_cast<char *>(case_ignore_buffer));
    case_ignore_table_name = case_ignore_buffer;
  }

  /* Validate the stopword table's (if supplied) existence and
  of the right format */
  if (!case_ignore_table_name ||
      fts_valid_stopword_table(case_ignore_table_name)) {
    /** here, we use the converted name as the variable name since
     * we want to reflect the value that is used actually.
     */
    *static_cast<const char **>(save) = case_ignore_table_name;
    ret = 0;
  }
...
static int innodb_internal_table_validate(THD *thd, SYS_VAR *, void *save,
  ...
  if (lower_case_table_names >= 1) {
    ut_a(((uint32_t)len + 1) < case_ignore_buffer_len);
    case_ignore_buffer =
        static_cast<char *>(thd_calloc(thd, case_ignore_buffer_len));
    memcpy(case_ignore_buffer, table_name, len);
    my_casedn_str(files_charset_info, case_ignore_buffer);
    table_name = case_ignore_buffer;
  }
