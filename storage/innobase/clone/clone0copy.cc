...
int Clone_Snapshot::synchronize_binlog_gtid(Clone_Alert_Func cbk,
  ...
  are completed. */
  auto error = wait_for_binlog_prepared_trx(recovery_mode);
  ...
  error = update_binlog_position(recovery_mode);
  return (error);
}

int Clone_Snapshot::update_binlog_position(long unsigned recovery_mode) {
  ...
  *Check and update in [3] are atomic for trx sys page.
  ...
  if (written) {
    auto err = wait_for_binlog_prepared_trx(recovery_mode);
    return (err);
  }
  return (0);
}
...
int Clone_Snapshot::wait_for_binlog_prepared_trx(long unsigned recovery_mode) {
  /* Return if binary log is not enabled. */
  if (!opt_bin_log) {
    return (0);
  }
  if (recovery_mode == CLONE_DATA_RECOVERY_GTID) {
    return (0);
  }
...
int Clone_Snapshot::init_redo_copy(Snapshot_State new_state,
  ...
  int binlog_error = 0;
  auto thd = thd_get_current_thd();
  auto recovery_mode = get_clone_data_recovery_mode();
  Clone_handler::XA_Block xa_block_guard(thd, recovery_mode);
  ...
  DBUG_SIGNAL_WAIT_FOR(thd, "test_clone_redo_copy_synchronize_binlog",
                       "redo_copy_before_synchronize_binlog",
                       "redo_copy_continue_synchronize_binlog");
  ...
  if (binlog_error == 0) {
    binlog_error = synchronize_binlog_gtid(cbk, recovery_mode);
  }
