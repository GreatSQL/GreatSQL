...
TEST_P(RouterRoutingConnectTimeoutTest, ConnectTimeout) {
  ...
  //   // the test requires a address:port which is not responding to SYN
  //   packets:
  //   //
  //   // - all the TEST-NET-* return "network not reachable" right away.
  //   // - RFC2606 defines example.org and its TCP port 81 is currently
  //   blocking
  ...
  //     EXPECT_EQ(e.code(), 2003) << e.what();
  //     EXPECT_THAT(e.what(),
  //                 ::testing::HasSubstr("Can't connect to remote MySQL
  //                 server"))
  //         << e.what();
  //   } catch (...) {
  //     FAIL() << "expected connect fail with a mysql-error";
...
TEST_F(RouterRoutingTest, ConnectTimeoutShutdownEarly) {
  ...
  // TempDirectory conf_dir("conf");
  // std::string conf_file = create_config_file(conf_dir.name(),
  // routing_section);
...
TEST_F(RouterRoutingTest, ConnectTimeoutShutdownEarlyXProtocol) {
  ...
  //   const auto res =
  //       make_x_connection(x_session, "127.0.0.1", router_port, "user",
  //       "pass",
  ...
  //                   ::testing::HasSubstr("Connection refused connecting
  //                   to")));
  // });
