...
// TYPELIB set_dump_encrypt_mode_typelib = {MY_AES_END + 1, "",
 ...
 */
int Client_Share::read_encrypt_key(const char *encrypt_key_path) {
  int error = read_clone_encrypt_key(encrypt_key_path, &m_encrypt_mode,
                                     &m_encrypt_key, &m_encrypt_iv);
  if (error != 0) {
    char err_buf[MYSYS_ERRMSG_SIZE];
    switch (error) {
      case 1: {
        snprintf(err_buf, sizeof(err_buf),
                 ...
                 encrypt_key_path, errno);
      } break;

      case 2: {
        snprintf(err_buf, sizeof(err_buf), "Error: Cannot malloc memory.\n");
      } break;

      case 3: {
        snprintf(err_buf, sizeof(err_buf),
                 "Error: Invalid dump encrypt key file.\n");
      } break;

      case 4: {
        snprintf(err_buf, sizeof(err_buf),
                 "Error: Fail to find encrypt mode.\n");
      } break;
      default:
        break;
    }
