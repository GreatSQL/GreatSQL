...
int encrypt_and_write(struct encrypt_op_buffer *m_buffer_node,
  ...
  int enc_len = 0;
  int ret = 0;
  auto cipher_buff = (char *)clone_os_align((uchar *)m_cipher_buff);
  cipher_buff[0] = '\0';
  while (length > 0) {
    cp_len = length < m_buffer_node->remain_size ? length
                                                 : m_buffer_node->remain_size;
    memcpy(m_buffer_node->pos, source, cp_len);
    m_buffer_node->pos += cp_len;
    m_buffer_node->size += cp_len;
    m_buffer_node->remain_size -= cp_len;
    length -= cp_len;
    if (length > 0) source += cp_len;
    if (m_buffer_node->remain_size == 0) {
      enc_len = encrypt_buffer(
          reinterpret_cast<unsigned char *>(m_buffer_node->buffer),
          ENCRYPT_BATCH_SIZE, cipher_buff, mode, key, iv);
      if ((size_t)enc_len != ENCRYPT_BATCH_SIZE) {
        ret = -1;
        break;
      }
      ...
      memmove(m_buffer_node->buffer, m_buffer_node->buffer + ENCRYPT_BATCH_SIZE,
              REMAIN_BLOCK_SIZE);
      m_buffer_node->pos = m_buffer_node->buffer + REMAIN_BLOCK_SIZE;
      m_buffer_node->size = REMAIN_BLOCK_SIZE;
      m_buffer_node->remain_size = ENCRYPT_BATCH_SIZE;
    } else if (length == 0) {
      if (m_buffer_node->size < REMAIN_BLOCK_SIZE) {
        ret = -1;
        break;
      }
      ...
      enc_len = encrypt_buffer(
          reinterpret_cast<unsigned char *>(m_buffer_node->buffer),
          m_buffer_node->size, cipher_buff, mode, key, iv);
      if ((size_t)enc_len != m_buffer_node->size) {
        ret = -1;
        break;
      }
      if (comp_file) {
      ...
      m_buffer_node->pos = m_buffer_node->buffer;
      m_buffer_node->size = 0;
      m_buffer_node->remain_size = ENCRYPT_BUFFER_SIZE;
    }
  }
  return ret;
}
