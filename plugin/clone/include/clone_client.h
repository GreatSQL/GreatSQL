...
struct Client_Share {
  ...
  void free_key() {
    ...
    if (m_encrypt_iv) my_free(m_encrypt_iv);
  }
  /** Remote host name */
  const char *m_host;
...
class Client_Cbk : public Ha_clone_cbk {
 public:
  ...
  Client_Cbk(Client *clone) : m_clone_client(clone) {
    char *clear_buff = (char *)clone_os_align((uchar *)m_clear_buff);
    clear_buff[0] = '\0';
    m_buffer_node = {clear_buff, clear_buff, 0, ENCRYPT_BUFFER_SIZE};
  }
 ...
 private:
  ...
  char m_clear_buff[ENCRYPT_BUFFER_SIZE + CLONE_OS_ALIGN];
  char m_cipher_buff[ENCRYPT_BUFFER_SIZE + CLONE_OS_ALIGN];
  struct encrypt_op_buffer m_buffer_node;
};
