...
#ifndef PLUGIN_GDB_SQL_PLAN_BASELINES_H
#define PLUGIN_GDB_SQL_PLAN_BASELINES_H
...
#ifndef FOREACH_PLAN_BASELINE_FIELED
#define FOREACH_PLAN_BASELINE_FIELED(F)         \
  F(id, PSI_Val_ulonglong, "bigint")            \
  F(db_name, PSI_Val_string, "varchar(64)")     \
  F(digest_hash, PSI_Val_string, "varchar(64)") \
  F(digest_text, PSI_Val_blob, "longtext")      \
  F(plan_name, PSI_Val_string, "varchar(64)")   \
  F(cost, PSI_Val_double, "decimal(7,2)")
#endif

class sql_plan_baselines_data : public Gdb_plan_baseline_pfs_data {
  enum DATA_FIELD_E { FOREACH_PLAN_BASELINE_FIELED(GENERATE_ENUM) };
...
class sql_plan_baselines_table : public Gdb_plan_baseline_pfs_table {
  std::vector<std::unique_ptr<PSI_Val>> data;

 public:
  sql_plan_baselines_table() { FOREACH_PLAN_BASELINE_FIELED(GENERATE_VAL) }
...
class sql_plan_baselines : public Gdb_plan_baseline_pfs_base {
  typedef Gdb_plan_baseline_pfs_base super;

 private:
  sql_plan_baselines() = default;
  sql_plan_baselines(const sql_plan_baselines &) = delete;
  sql_plan_baselines(sql_plan_baselines &&) = delete;
  sql_plan_baselines &operator=(const sql_plan_baselines &) = delete;
  sql_plan_baselines &operator=(sql_plan_baselines &&) = delete;

 public:
  static sql_plan_baselines *get_instance() {
    static sql_plan_baselines instance;
    return &instance;
  }

 public:
  const char *tabDef = FOREACH_PLAN_BASELINE_FIELED(GENERATE_STRING);
  bool init() override {
    if (super::_init(std::make_unique<sql_plan_baselines_data>())) {
      return true;
    }
    auto table = get_proxy_share();
    table->m_table_name = "gdb_sql_plan_baselines";
  ...
  static PSI_table_handle *plan_baseline_open_table(PSI_pos **pos) {
    ...
    sql_plan_baselines_table *table = new sql_plan_baselines_table();
