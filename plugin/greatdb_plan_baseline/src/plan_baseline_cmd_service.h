...
#ifndef PLUGIN_GDB_PLAN_BASELINE_SERVICE_INCLUDED
#define PLUGIN_GDB_PLAN_BASELINE_SERVICE_INCLUDED
...
class plan_baseline_cmd_cb_data {
 public:
  plan_baseline_cmd_cb_data();
  ~plan_baseline_cmd_cb_data() = default;
  ...
  inline std::shared_ptr<std::string> get_value(uint row, uint field) {
    auto ptr = m_result.find(row);
    if (ptr == m_result.end()) {
      return nullptr;
    }
    return ptr->second.at(field);
  }

  ulonglong affected_rows() { return m_affected_rows; }
  std::vector<Field_meta> &get_meta() { return m_fields_meta; }
  /**
   *  Returns true if the result is empty.  (Thus begin() would
   *  equal end().)
   */
 ...
 private:
  ...
  std::map<uint, std::vector<std::shared_ptr<std::string>>> m_result;
...
/* NOTE: cmd service always use autocommit=on */
class plan_baseline_cmd_service {
 public:
  plan_baseline_cmd_service() : m_st_session(nullptr) {}
  ~plan_baseline_cmd_service() { close_session(); }
  int execute_sql(const char *sql, size_t len);
  inline int execute_sql(std::string sql) {
    return execute_sql(sql.c_str(), sql.length());
  }
  int execute_sqls(std::vector<std::string> &sqls);

  plan_baseline_cmd_cb_data &get_cb_data() { return cb_data; }
 ...
 private:
  ...
  plan_baseline_cmd_cb_data cb_data;
