...
int Transaction_consistency_info::handle_remote_prepare(
  ...
  if (get_majority_after_mode_var()) {
    ...
    finished = members_that_must_prepare_the_transaction_empty;
  }
...
int Transaction_consistency_manager::handle_remote_prepare(
    ...
    const Gcs_member_identifier &gcs_member_id, int *delayed) {
  DBUG_TRACE;
  rpl_sidno sidno = 0;
...
int Transaction_consistency_manager::transaction_begin_sync_before_execution(
  ...
  */
  /*
   * 1. When do trans_begin here, it will hold read_lock_observer_list
   * 2. Another thread do STOP GROUP_REPLICATION, after stopped, invoke
   *    unregister_transaction_observer, and try write_lock_observer_list
   * 3. So, thread-2 should wait thread-1
   * 4. However, thread-1 still need wait gtid_set, the gtid_set cannot update
   *    when plugin stopped. Now, thread-1 wait forever until timeout
   * 5. And, then, STOP GROUP_REPLICATION cannot return yet
   */
  ulong retry = 0;
  while (true) {
    ...
    current_thd->is_wait_gtid_set_timeout = false;
    bool ret = wait_for_gtid_set_committed(applier_retrieved_gtids.c_str(), 1,
                                           false /* update_thd_status */);
    bool wait_timeout = current_thd->is_wait_gtid_set_timeout;
    current_thd->is_wait_gtid_set_timeout = false;
    if (ret) {
      if (wait_timeout && !thd->killed && (timeout == 0 || ++retry < timeout)) {
        continue;
      }
      /* purecov: begin inspected */
      LogPluginErr(ERROR_LEVEL, ER_GRP_RPL_TRX_WAIT_FOR_GROUP_GTID_EXECUTED,
