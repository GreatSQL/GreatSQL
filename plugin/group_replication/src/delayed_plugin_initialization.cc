...
int Delayed_initialization_thread::initialization_thread_handler() {
  ...
  if (is_server_engine_initialized) {
    ...
    if (channel_is_active("group_replication_recovery", CHANNEL_NO_THD)) {
      set_plugin_is_setting_read_mode(true);
      error = initialize_plugin_and_join(PSESSION_INIT_THREAD, this);
    } else {
      set_plugin_starting(false);
      LogPluginErr(
          INFORMATION_LEVEL, ER_GRP_RPL_ERROR_MSG,
          "Not join to any group yet, no need start Group Replication on boot");
