...
void Autorejoin_thread::execute_rejoin_process() {
  ...
  while (!m_abort && num_attempts++ < m_attempts) {
    ...
    // Set start_wait_time to 1, to indicate doing rejoin attempt.
    gr_autorejoin_vars.set_start_wait_time_and_interval(1);
    ...
    /*
      Wait on m_run_cond up to group_replication_autorejoin_interval seconds.
      This is a simple way to allow the thread to be interrupted and/or canceled
      at will.

      Also, we only wait if we're not already in our last try.
    */
    if (num_attempts < m_attempts) {
      uint rejoin_timeout = get_rejoin_timeout();
      gr_autorejoin_vars.set_start_wait_time_and_interval(
          my_timer_microseconds(), rejoin_timeout);
      DBUG_EXECUTE_IF("group_replication_stop_before_rejoin_wait", {
        const char act[] = "now signal signal.before_rejoin_wait";
        assert(!debug_sync_set_action(current_thd, STRING_WITH_LEN(act)));
      });
      set_timespec(&tm, rejoin_timeout);
      mysql_mutex_lock(&m_run_lock);
  ...
  // Set start_wait_time to 0, to indicate that the thread has terminated.
  gr_autorejoin_vars.set_start_wait_time_and_interval(0);
