...
int group_replication_trans_before_dml(Trans_param *param, int &out) {
  ...
  if ((param->trans_ctx_info.tx_isolation == ISO_SERIALIZABLE) &&
  ...
  for (uint table = 0; out == 0 && table < param->number_of_tables; table++) {
    ...
    if (param->tables_info[table].has_cascade_foreign_key &&
        local_member_info->has_enforces_update_everywhere_checks()) {
...
int group_replication_trans_before_commit(Trans_param *param) {
  ...
  uint32_t thread_id = param->thread_id;
  ...
  */
  ...
  XID_STATE *xid_state = current_thd->get_transaction()->xid_state();
  ...
  if (trx_cache_log_position > 0 && stmt_cache_log_position == 0) {
    ...
    if (xid_state->get_state() == XID_STATE::XA_IDLE) {
      if (!current_thd->is_one_phase_commit()) {
        thread_id = thread_id & 0x3FFFFFFF;
        thread_id = thread_id | 0x80000000;
      }
    }
    ...
    if (xid_state->get_state() == XID_STATE::XA_PREPARED) {
      thread_id = thread_id & 0x3FFFFFFF;
      thread_id = thread_id | 0xC0000000;
    }
  } else {
    /* purecov: begin inspected */
    LogPluginErr(ERROR_LEVEL,
  ...
  tcle = new Transaction_context_log_event(
      param->server_uuid, is_dml || param->is_atomic_ddl, thread_id,
      is_gtid_specified, param->is_empty_transaction);
  if (!tcle->is_valid()) {
    /* purecov: begin inspected */
  ...
  */
  ...
  if (request_time_threshold > 0) {
    time_diff = my_micro_time() - time_diff;
    add_before_commit_request_time(time_diff);
    if (time_diff >= request_time_threshold) {
      LogPluginErrMsg(INFORMATION_LEVEL, ER_LOG_PRINTF_MSG,
                      "MGR request time:%lluus, server id:%lu, thread_id:%u",
