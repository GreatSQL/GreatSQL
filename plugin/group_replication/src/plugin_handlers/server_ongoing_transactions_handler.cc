...
int Server_ongoing_transactions_handler::
  ...
  while (!transactions_to_wait.empty() && !(*abort_flag) && !error) {
    ...
    my_sleep(100);
    // log wait transaction every 10s for analysis
    if (++log_time_period == 100 * 1000 /* about 10s */) {
      if (!transactions_to_wait.empty()) {
        my_thread_id wait_id = *transactions_to_wait.begin();
        LogPluginErrMsg(
            INFORMATION_LEVEL, MYF(0),
            "still waitting ongoing transaction finish for query_id = %u",
            wait_id);
      }
      log_time_period = 0;
    }
