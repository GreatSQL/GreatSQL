...
Group_partition_handling::Group_partition_handling(ulong unreachable_timeout)
  ...
  group_leaving_stage = new Group_leaving_stage_on_failure();
}
...
Group_partition_handling::~Group_partition_handling() {
  ...
  if (group_leaving_stage) {
    delete group_leaving_stage;
    group_leaving_stage = nullptr;
  }
}
...
int Group_partition_handling::terminate_partition_handler_thread() {
  ...
  while (group_partition_thd_state.is_thread_alive()) {
    ...
    if (++wait_period == 10) {
      if (stop_wait_timeout == 0) {
        LogPluginErrMsg(
            INFORMATION_LEVEL, MYF(0),
            "stop group_replication waitting for partition_handler_thread "
            "terminated, the member now leave group with stage %s",
            group_leaving_stage->stage_to_string());
      }
      wait_period = 0;
    }
