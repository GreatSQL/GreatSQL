...
/* status that are collect in applier_module */
struct applier_status_vars gr_applier_vars;
/* status that are collect in autorejoin_module */
struct autorejoin_status_vars gr_autorejoin_vars;

/* status which collect in plugin */
struct plugin_status_vars gr_status_vars;
...
bool get_plugin_is_starting() { return lv.plugin_is_starting; }
void set_plugin_starting(bool b) { lv.plugin_is_starting = b; }
...
ulonglong get_rejoin_timeout() { return ov.autorejoin_interval_var; }
...
void add_before_commit_request_time(uint64_t request_time) {
  if (plugin_is_group_replication_running()) {
    gr_status_vars.total_before_commit_request_time += request_time;
  }
}
  ...
  */
  ...
  if (local_member_info != nullptr) {
    local_member_info->update(
        ...
        get_allow_single_leader(), ov.zone_id_var,
...
int plugin_group_replication_stop(char **error_message) {
  ...
  */
  ...
  bool enable_read_only_error = false;
  if (!lv.server_shutdown_status && !lv.plugin_is_being_uninstalled &&
      server_engine_initialized()) {
    if (enable_server_read_mode()) {
      enable_read_only_error = true;
      /* purecov: begin inspected */
      LogPluginErr(ERROR_LEVEL,
  ...
  if (enable_read_only_error) {
    ...
    if (!error) error = GROUP_REPLICATION_STOP_WITH_SUPER_READ_ONLY_TIMEOUT;
  } else {
...
int initialize_plugin_modules(gr_modules::mask modules_to_init) {
  ...
  DBUG_EXECUTE_IF("group_replication_rejoin_short_retry",
                  { ov.autorejoin_interval_var = 1ULL; };);
  DBUG_EXECUTE_IF("group_replication_rejoin_long_retry",
                  { ov.autorejoin_interval_var = 60ULL; };);
...
int build_gcs_parameters(Gcs_interface_parameters &gcs_module_parameters) {
  ...
  gcs_module_parameters.add_parameter(
      "xcom_expel_timeout", std::to_string(ov.member_expel_timeout_var));
...
static void update_member_expel_timeout(MYSQL_THD, SYS_VAR *, void *var_ptr,
  ...
  *static_cast<ulong *>(var_ptr) = in_val;
  ...
  if (gcs_module != nullptr) {
    ...
    gcs_module->set_xcom_expel_timeout(in_val);
  }
}
...
static int check_autorejoin_interval(MYSQL_THD, SYS_VAR *, void *save,
                                     struct st_mysql_value *value) {
  DBUG_TRACE;
  ...
  if (in_val < 0 || in_val > lv.MAX_AUTOREJOIN_INTERVAL) {
    return 1;
  }
...
static void update_autorejoin_interval(MYSQL_THD, SYS_VAR *, void *var_ptr,
                                       const void *save) {
  DBUG_TRACE;
  ...
  *static_cast<uint *>(var_ptr) = in_val;
  ...
  ov.autorejoin_interval_var = in_val;
}
...
static MYSQL_SYSVAR_UINT(autorejoin_interval,        /* name */
                         ov.autorejoin_interval_var, /* var */
                         PLUGIN_VAR_OPCMDARG |
                             PLUGIN_VAR_PERSIST_AS_READ_ONLY, /* optional var */
                         "The time interval (in seconds) between two attempts "
                         "in the auto-rejoin procedure.",
                         check_autorejoin_interval,  /* check func */
                         update_autorejoin_interval, /* update func */
                         300U,                       /* default */
                         1U,                         /* min */
                         lv.MAX_AUTOREJOIN_INTERVAL, /* max */
                         0);                         /* block */
...
static SYS_VAR *group_replication_system_vars[] = {
    ...
    MYSQL_SYSVAR(autorejoin_interval),
...
static SHOW_VAR applier_status_variables[] = {
    {"apply_queue_size", (char *)(&gr_applier_vars.apply_queue_size), SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"applied_messages", (char *)(&gr_applier_vars.total_messages),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"applied_data_messages", (char *)(&gr_applier_vars.total_data_messages),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"applied_events", (char *)(&gr_applier_vars.apply_events), SHOW_LONGLONG,
     SHOW_SCOPE_GLOBAL},
    {"io_buffered_events", (char *)(&gr_applier_vars.io_buffer_events),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"flow_control_count", (char *)(&gr_applier_vars.flow_control_wait_events),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"flow_control_time", (char *)(&gr_applier_vars.flow_control_wait_time),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {NullS, NullS, SHOW_LONG, SHOW_SCOPE_GLOBAL}};

static int64 show_applier_status_variables(MYSQL_THD, SHOW_VAR *var, char *) {
  var->type = SHOW_ARRAY;
  var->value = (char *)&applier_status_variables;
  var->scope = SHOW_SCOPE_GLOBAL;
  return (0);
}

/**
  Returns the time remaining until the next retry in seconds, for a running
  auto-rejoin process. Return relevant information for the following two
  special situations:
      1. no auto-rejoin process is running
      2. doing rejoin attempt currently
*/
static int get_remaining_time_until_next_autorejoin_retry(MYSQL_THD thd,
                                                          SHOW_VAR *var,
                                                          char *buffer) {
  var->type = SHOW_CHAR;
  var->value = buffer;

  uint64 start_wait_time;
  uint current_interval;
  gr_autorejoin_vars.get_start_wait_time_and_interval(&start_wait_time,
                                                      &current_interval);
  uint64 current_time = my_timer_microseconds();
  if (start_wait_time == 0) {
    sprintf(buffer, "No auto-rejoin process is running!");
  } else if (start_wait_time == 1 || current_time < start_wait_time ||
             (current_time - start_wait_time) > current_interval * 1000000) {
    sprintf(buffer, "Doing rejoin attempt currently!");
  } else {
    sprintf(buffer, "%f",
            current_interval - (current_time - start_wait_time) / 1000000.0);
  }

  return 0;
}

static int get_second_behind_group(MYSQL_THD thd, SHOW_VAR *var, char *buffer) {
  var->type = SHOW_CHAR;
  var->value = buffer;

  if (local_member_info != nullptr) {
    sprintf(buffer, "%llu", local_member_info->get_local_second_behind_group());
  } else {
    sprintf(buffer, " ");
  }
...
static SHOW_VAR group_replication_status_vars[] = {
    {"group_replication_before_commit_request_time",
     (char *)(&gr_status_vars.total_before_commit_request_time), SHOW_LONGLONG,
     SHOW_SCOPE_GLOBAL},
    ...
    {"Gr_last_consensus_end_timestamp",
     ...
     SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"group_replication", (char *)(&show_applier_status_variables), SHOW_FUNC,
     SHOW_SCOPE_GLOBAL},
    {"group_replication_remaining_time_until_next_autorejoin_retry",
     (char *)(&get_remaining_time_until_next_autorejoin_retry), SHOW_FUNC,
     SHOW_SCOPE_GLOBAL},
    {"group_replication_second_behind_group",
     (char *)(&get_second_behind_group), SHOW_FUNC, SHOW_SCOPE_GLOBAL},
