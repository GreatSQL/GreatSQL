...
rpl_gno Certifier::certify(Gtid_set *snapshot_version,
                           ...
                           bool is_empty_transaction, Gtid_log_event *gle,
                           bool local_transaction, uint32_t thread_id) {
  DBUG_TRACE;
  rpl_gno result = 0;
    ...
    if (!local_transaction && !conflict_detection_enable) {
      if (thread_id & 0xC0000000) {
        ...
        if (it != xa_replay_map.end()) {
          has_write_set = true;
          uint32_t item_previous_sequence_number = it->second;
          if (item_previous_sequence_number > transaction_last_committed &&
...
void Certifier::add_xa_prepare_writeset_item_to_map(int32_t thread_id,
                                                    const char *item) {
  DBUG_TRACE;
  std::string key(item);
