  ...
  if ((members.size() - unreachable.size()) <= (members.size() / 2)) {
    ...
    if (!left_members.empty()) {
      ...
      for (it = left_members.begin(); it != left_members.end(); it++) {
        const Gcs_member_identifier *member = *it;
        Group_member_info member_info;
        if (!group_member_mgr->get_group_member_info_by_member_id(
                *member, member_info)) {
          LogPluginErrMsg(INFORMATION_LEVEL, ER_LOG_PRINTF_MSG,
...
void Plugin_gcs_events_handler::on_gcs_join_failed() const {
  LogPluginErrMsg(INFORMATION_LEVEL, ER_LOG_PRINTF_MSG,
                  "on_gcs_join_failed is called");
  ...
  */
  if (election_mode == DEAD_OLD_PRIMARY &&
    ...
    if (is_arbitrator_role()) {
      auto *all_members_info = group_member_mgr->get_all_members();
      bool group_valid = false;
      if (all_members_info->size() > 1) {
        for (auto it = all_members_info->begin(); it != all_members_info->end();
             it++) {
...
#if !defined(NDEBUG)
  DBUG_EXECUTE_IF("group_replication_assume_need_vcle", need_vcle = true;);
#endif
