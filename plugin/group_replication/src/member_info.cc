...
#include "sql/rpl_channel_service_interface.h"  // get_cur_exec_channel_delay
...
Group_member_info::Group_member_info(
    ...
    bool allow_single_leader, int zone_id_arg, bool zone_id_sync_mode_arg,
...
void Group_member_info::update(
    ...
    uint lower_case_table_names_arg, bool default_table_encryption_arg,
    const char *recovery_endpoints_arg, const char *view_change_uuid_arg,
    bool allow_single_leader, int zone_id_arg,
...
int Group_member_info::compare_gtid(Group_member_info *other) {
  int valid = 1;
  Tsid_map member_sid_map(nullptr);
  Tsid_map other_sid_map(nullptr);
...
unsigned long long Group_member_info::get_local_second_behind_group() {
  // server_uuid is defiened by mysqld.h
  if (0 != this->get_uuid().compare(server_uuid)) return 0ULL;
  return get_cur_exec_channel_delay();
}
