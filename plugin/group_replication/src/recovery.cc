...
#include "sql/rpl_channel_service_interface.h"  // set_cur_exec_channel
...
int Recovery_module::recovery_thread_handle() {
  ...
  if (!is_arbitrator_role()) {
    ...
    channel_get_gtid_set_to_apply("group_replication_applier",
                                  gtid_set_to_apply);
    set_cur_exec_channel_as_applier();
  ...
  if (!is_arbitrator_role() &&
      m_until_condition == CHANNEL_UNTIL_APPLIER_AFTER_GTIDS) {
    set_cur_exec_channel_as_recover();
