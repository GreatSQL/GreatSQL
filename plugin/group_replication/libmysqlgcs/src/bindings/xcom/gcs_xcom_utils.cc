...
static const uint64_t DEFAULT_XCOM_EXPEL_TIMEOUT = 5;
...
static const uint64_t MIN_XCOM_EXPEL_TIMEOUT = 0;
static const uint64_t MAX_XCOM_EXPEL_TIMEOUT = 3600;
...
void fix_parameters_syntax(Gcs_interface_parameters &interface_params) {
  ...
  std::string *xcom_expel_timeout_str = const_cast<std::string *>(
      interface_params.get_parameter("xcom_expel_timeout"));
  ...
  // sets the default XCom expel timeout
  if (!xcom_expel_timeout_str) {
    interface_params.add_parameter("xcom_expel_timeout",
                                   std::to_string(DEFAULT_XCOM_EXPEL_TIMEOUT));
  }
}
...
bool is_parameters_syntax_correct(
  ...
  const std::string *xcom_flp_timeout_str =
      interface_params.get_parameter("xcom_flp_timeout");
  const std::string *xcom_expel_timeout_str =
      interface_params.get_parameter("xcom_expel_timeout");
  ...
  // Validate XCom expel timeout
  errno = 0;
  if (xcom_expel_timeout_str != nullptr &&
      // Verify if the input value is a valid number
      (xcom_expel_timeout_str->size() == 0 ||
       !is_number(*xcom_expel_timeout_str) ||
       // Check that it is not lower than the min value allowed for the var
       strtoull(xcom_expel_timeout_str->c_str(), nullptr, 10) <
           MIN_XCOM_EXPEL_TIMEOUT ||
       // Check that it is not higher than the max value allowed
       strtoull(xcom_expel_timeout_str->c_str(), nullptr, 10) >
           MAX_XCOM_EXPEL_TIMEOUT ||
       // Check that it is within the range of values allowed for the var type.
       // This is need in addition to the check above because of overflows.
       errno == ERANGE)) {
    MYSQL_GCS_LOG_ERROR("The xcom_expel_timeout parameter ("
                        << xcom_expel_timeout_str->c_str() << ") is not valid.")
    error = GCS_NOK;
    goto end;
  }
