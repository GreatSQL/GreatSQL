...
#define TCP_KEEPALIVE_INTERVAL 6
#define TCP_KEEPALIVE_PROBES 5
#define TCP_KEEPALIVE_USER_TIMEOUT 60

int set_conn_tcp_keepalive(int fd) {
  int keepalive = 1;
  int idle = TCP_KEEPALIVE_IDLE;
  int interval = TCP_KEEPALIVE_INTERVAL;
  int probes = TCP_KEEPALIVE_PROBES;
  int user_timeout = TCP_KEEPALIVE_USER_TIMEOUT;
  if (setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, &keepalive, sizeof(keepalive)) ||
      ...
      setsockopt(fd, IPPROTO_TCP, TCP_USER_TIMEOUT, &user_timeout,
                 sizeof(user_timeout))) {
    MYSQL_GCS_LOG_INFO("Unable to set socket tcp keepalive options (fd="
                       << fd << ", errno=" << to_errno(GET_OS_ERR) << ")");
    return 1;
  }
  return 0;
}
...
bool Gcs_xcom_proxy_impl::xcom_client_set_expel_timeout(uint64_t timeout) {
  app_data_ptr data = new_app_data();
  data = init_set_expel_timeout_msg(data, timeout);
  /* Takes ownership of data. */
  bool const successful = xcom_input_try_push(data);
  if (!successful) {
    MYSQL_GCS_LOG_DEBUG(
        "xcom_client_set_expel_timeout: Failed to push into XCom.");
  }
  return successful;
}
...
void Gcs_xcom_app_cfg::set_xcom_expel_timeout(uint64_t timeout) {
  if (the_app_xcom_cfg) the_app_xcom_cfg->m_expel_timeout = timeout;
}
...
bool Gcs_xcom_proxy_base::xcom_get_synode_app_data(
  ...
  if (con != nullptr && set_conn_tcp_keepalive(con->fd)) {
...
bool Gcs_xcom_proxy_base::xcom_set_expel_timeout(uint64_t timeout) {
  return xcom_client_set_expel_timeout(timeout);
}
