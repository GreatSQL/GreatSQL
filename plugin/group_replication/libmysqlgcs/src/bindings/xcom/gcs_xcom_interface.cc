...
enum_gcs_error Gcs_xcom_interface::configure(
  ...
  */
  ...
  m_group_interface_lock.lock_shared();
  ...
  m_group_interface_lock.unlock_shared();
...
bool Gcs_xcom_interface::initialize_xcom(
  ...
  */
  ...
  // configure expel timeout
  if (xcom_expel_timeout_str != nullptr) {
    m_gcs_xcom_app_cfg.set_xcom_expel_timeout(
        (uint64_t)atoll(xcom_expel_timeout_str->c_str()));
    MYSQL_GCS_LOG_DEBUG("Configured XCom expel timeout: %s",
                        xcom_expel_timeout_str->c_str());
  }
