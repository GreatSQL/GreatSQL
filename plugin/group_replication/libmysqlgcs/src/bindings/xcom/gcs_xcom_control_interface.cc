...
enum_gcs_error Gcs_xcom_control::set_xcom_expel_timeout(uint64_t timeout) {
  bool const success = m_xcom_proxy->xcom_set_expel_timeout(timeout);
  return success ? GCS_OK : GCS_NOK;
}
...
enum_gcs_error Gcs_xcom_control::do_join(bool retry) {
  ...
  if (ret == GCS_NOK) {
    map<int, const Gcs_control_event_listener &>::const_iterator callback_it;
    for (callback_it = event_listeners.begin();
         callback_it != event_listeners.end(); callback_it++) {
      callback_it->second.on_gcs_join_failed();
    }
  }
...
bool Gcs_xcom_control::try_send_add_node_request_to_seeds(
  ...
  for (auto it = m_initial_peers.begin();
    ...
    if (connected && set_conn_tcp_keepalive(con->fd)) {
...
void Gcs_xcom_control::do_remove_node_from_group() {
  ...
  */
  ...
  if (con != nullptr && con->fd != -1 && !set_conn_tcp_keepalive(con->fd) &&
      !m_leave_view_delivered && m_view_control->belongs_to_group()) {
    MYSQL_GCS_LOG_TRACE(
