...
static std::atomic<bool> input_ongoing = false;
static std::atomic<bool> free_ongoing = false;
bool_t xcom_input_signal() {
  bool_t successful = FALSE;
  input_ongoing = true;
  if (free_ongoing.load()) {
    input_ongoing = false;
    return successful;
  }
...
void xcom_input_free_signal_connection() {
  free_ongoing = true;
  while (input_ongoing.load()) {
    // the connection is busy, wait send/read finish
    usleep(1000);  // 1ms
  }
  if (input_signal_connection != nullptr) {
    if (input_signal_connection_pipe != nullptr) {
...
static node_no remote_synode_allocator(site_def *site, app_data const &a) {
  ...
  for (;;) {
    if (is_active_leader(i, site) &&
        !may_be_dead(
            site->detected, i, task_now(), silent,
            site->servers[i]->unreachable)) {  // Found leader, send request
      pax_msg *p =
...
static bool check_delivery_timeout(site_def *site, double start_propose,
                                   app_data *a, int timeout) {
  bool retval = (start_propose + a->expiry_time) < task_now() &&
                !enough_live_nodes(site, timeout);
  if (retval) {
    ...
    *site = proposer_site = find_site_def_rw(*msgno);
    ...
    while (too_far(*msgno)) { /* Too far ahead of executor */
      ...
      IFDBG(D_NONE, FN; SYCEXP(ep->msgno); TIMECEXP(ep->start_propose);
            ...
            TIMECEXP(task_now());
            NDBG(enough_live_nodes(outer_ep->site, ep->basic_timeout), d));
#ifdef DELIVERY_TIMEOUT
    ...
    for (i = 0; i < nr_nodes_to_add; i++) {
      if (node_exists(&nodes_to_change[i], &new_site_def->nodes) ||
        ...
        */
        ...
        for (uint j = 0; j < new_site_def->nodes.node_list_len; j++) {
          G_INFO("Exists node in current site: %s",
                 (new_site_def->nodes.node_list_val + j)->address);
        }
        return 0;
      }
    }
...
static void process_client_msg(site_def const *site, pax_msg *p,
  ...
  if (p->a && (p->a->body.c_t == set_expel_timeout)) {
    CREATE_REPLY(p);
    if (the_app_xcom_cfg) {
      the_app_xcom_cfg->m_expel_timeout = p->a->body.app_u_u.expel_timeout;
      reply->cli_err = REQUEST_OK;
    } else {
      reply->cli_err = REQUEST_FAIL;
    }
...
#else
  return (long)(get_micro_time() ^ getpid());
#endif
}
...
app_data_ptr init_set_expel_timeout_msg(app_data *a, uint64_t expel_timeout) {
  init_app_data(a);
  a->body.c_t = set_expel_timeout;
  a->body.app_u_u.expel_timeout = expel_timeout;
  return a;
}
