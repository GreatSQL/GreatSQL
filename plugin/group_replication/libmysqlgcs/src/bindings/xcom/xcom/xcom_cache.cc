...
static inline int can_deallocate(lru_machine *link_iter, uint64_t ttl_sec);
static size_t do_shrink_cache(bool force, uint64_t ttl_sec = 0);
...
static inline int can_deallocate(lru_machine *link_iter, uint64_t ttl_sec) {
  ...
  */
  if ((site->install_time + basic_timeout) > now) return 0;
  if (dealloc_site ==
  ...
  if (synode_eq(delivered_msg,
    ...
    return 0;
  if (ttl_sec != 0 && link_iter->pax.link_into_lru > now - ttl_sec) {
    return 0;
  }
...
*/
static size_t do_shrink_cache(bool force, uint64_t ttl_sec) {
  size_t shrunk = 0;
  FWD_ITER(&protected_lru, lru_machine, {
    if ((force || above_cache_limit()) && can_deallocate(link_iter, ttl_sec)) {
...
int cache_manager_task(task_arg arg [[maybe_unused]]) {
  ...
  while (!xcom_shutdown) {
    ...
    if (the_app_xcom_cfg) {
      uint64_t ttl_sec = the_app_xcom_cfg->m_flp_timeout +
                         the_app_xcom_cfg->m_expel_timeout * 2;
      do_shrink_cache(true, ttl_sec);
    }
  }
  FINALLY
