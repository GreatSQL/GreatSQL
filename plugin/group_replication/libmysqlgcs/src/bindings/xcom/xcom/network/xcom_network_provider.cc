...
void xcom_tcp_server_startup(Xcom_network_provider *net_provider) {
  G_DEBUG(
      "Thread xcom_tcp_server startted, listen connection on port "
      "local_address");
  ...
  do {
    ...
    if (accept_fd == -1) {
      ...
      if (accept_fd != -1) {
        ...
        G_DEBUG("Thread xcom_tcp_server accept new connection %d", accept_fd);
  ...
  G_DEBUG("Thread xcom_tcp_server exit");
  return;
}
...
int Xcom_network_provider::close_connection(
  ...
  G_DEBUG("Do close xcom_connection %d", connection.fd);
...
std::pair<bool, int> Xcom_network_provider::stop() {
  G_DEBUG("Do stop xcom_tcp_server thread");
  if (!is_provider_initialized()) {
    return std::make_pair(true, -1);
  }
  ...
  if (m_network_provider_tcp_server.joinable())
    m_network_provider_tcp_server.join();
  /*
   * Do close new incoming connection again.
   * 1. gr exit first
   * 2. xcom_tcp_server_startup thread, accept new connection
   * 3. Xcom_network_provider::stop, set_shutdown_tcp_server
   * 4. Xcom_network_provider::stop, close the latest connection before gr exit
   * 5. xcom_tcp_server_startup thread, set_new_connection from step 2
   * 6. xcom_tcp_server_startup thread check m_shutdown_tcp_server and exit
   * 7. now, close connection from step 2 for cleanup
   */
  this->reset_new_connection();
