...
class Gcs_control_event_listener {
  ...
  */
  ...
  /**
    This method is called when a join operation at the GCS level has failed.

    This notification allows the plugin layer to immediately react to a failure
    in joining the group, rather than waiting for view modification to timeout.
  */
  virtual void on_gcs_join_failed() const = 0;
