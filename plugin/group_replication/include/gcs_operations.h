...
*/
  ...
  /**
   * Requests GCS to change the expel timeout of the XCom communication.
   *
   * @param new_timeout The new expel timeout of the XCom communication.
   *
   * @retval GCS_OK if request successfully scheduled
   * @retval GCS_NOK if GCS is unable to schedule the request
   */
  enum enum_gcs_error set_xcom_expel_timeout(uint64_t new_timeout);
