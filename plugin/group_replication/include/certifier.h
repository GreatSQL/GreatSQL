...
typedef struct Last_xa_prepare_writeset_info {
  std::list<std::string> writeset;
  time_t create_time;
} Last_xa_prepare_writeset_info;
...
class Certifier : public Certifier_interface {
 public:
  typedef std::unordered_map<
      std::string, Cert_basic_info, std::hash<std::string>,
      std::equal_to<std::string>,
      Malloc_allocator<std::pair<const std::string, Cert_basic_info>>>
      Certification_info;
  typedef std::map<uint64, std::string, std::less<uint64>,
                   Malloc_allocator<std::pair<const uint64, std::string>>>
      Certification_index;
  typedef std::map<uint32_t, int64, std::less<uint32_t>,
                   Malloc_allocator<std::pair<const uint32_t, int64>>>
      Last_commit_xa_replay_map;
  typedef std::map<
      uint32_t, Last_xa_prepare_writeset_info *, std::less<uint32_t>,
      Malloc_allocator<
          std::pair<const uint32_t, Last_xa_prepare_writeset_info *>>>
      Last_xa_prepare_writeset_map;
  ...
  */
  ...
  bool is_certfy_gtid_committed(const Gtid &gtid) override;
  /**
  ...
  */
  ...
  Last_commit_xa_replay_map xa_replay_map;
  ...
  double last_update_last_committed_global_usec;
  ...
  */
  ...
  bool add_item_for_xa_commit(uint32_t thread_id, Gtid_set *snapshot_version,
                              int *has_to_flush_last_commited,
                              int64 *transaction_last_committed);
  void add_xa_prepare_writeset_item_to_map(uint32_t thread_id,
                                           const char *item);
