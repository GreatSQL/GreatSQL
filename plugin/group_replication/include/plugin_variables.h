...
struct plugin_local_variables {
  ...
  std::atomic<bool> plugin_is_starting;
  ...
  const uint MAX_AUTOREJOIN_TRIES = UINT_MAX;
  // 5min
  const uint MAX_AUTOREJOIN_INTERVAL = 300;
...
/*
  status which collect in applier_module
*/
struct applier_status_vars {
  uint64_t total_messages;
  uint64_t total_data_messages;
  size_t apply_queue_size;
  uint64_t io_buffer_events;
  uint64_t apply_events;

  uint64_t flow_control_wait_events;
  uint64_t flow_control_wait_time;

  applier_status_vars() { reset(); }
  ...
  void reset() {
    ...
    flow_control_wait_events = 0;
    flow_control_wait_time = 0;
  }
};

/*
  status which collect in autorejoin_module
*/
struct autorejoin_status_vars {
 private:
  /*
    start_wait_time and current_interval as a whole should be atomic
  */
  std::mutex m_mutex;
  uint64 start_wait_time;  // the time for start waiting of next retry
  uint current_interval;   // the waiting time for the next retry

 public:
  autorejoin_status_vars() { reset(); }
  ...
  void set_start_wait_time_and_interval(uint64 time, uint interval = 0) {
    std::lock_guard<std::mutex> lock(m_mutex);
    start_wait_time = time;
    current_interval = interval;
  }

  void get_start_wait_time_and_interval(uint64 *time, uint *interval) {
    std::lock_guard<std::mutex> lock(m_mutex);
    *time = start_wait_time;
...
*/
struct plugin_status_vars {
  std::atomic<uint64_t> total_before_commit_request_time;

  plugin_status_vars() { reset(); }

  void reset() { total_before_commit_request_time = 0; }
};

extern applier_status_vars gr_applier_vars;
extern autorejoin_status_vars gr_autorejoin_vars;
