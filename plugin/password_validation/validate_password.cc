...
static bool is_valid_password_by_changed_user_name(THD *thd, const char *buffer,
                                                   int length) {
  LEX_USER *user;
  if (!(user = thd->get_gdb_current_user()) || !user->user.str ||
      user->user.length != (size_t)length || user->user.length == 0)
    return true;
  return (
      memcmp(buffer, user->user.str, user->user.length) &&
      !my_memcmp_reverse(user->user.str, user->user.length, buffer, length));
}
...
static bool is_valid_password_by_user_name(mysql_string_handle password) {
  ...
  return is_valid_user(ctx, buffer, length, "user", "login user name") &&
         is_valid_user(ctx, buffer, length, "priv_user",
                       "effective user name") &&
         is_valid_password_by_changed_user_name(thd_get_current_thd(), buffer,
                                                length);
}
