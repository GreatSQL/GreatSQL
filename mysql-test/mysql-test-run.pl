...
sub main {
  ...
  $mtr_cases::opt_mtr_hint =$opt_mtr_hint;
...
sub print_global_resfile {
  ...
  resfile_global("mtr-hint",         $opt_mtr_hint         ? 1 : 0);
...
sub client_arguments ($;$) {
  ...
  if ($opt_mtr_hint && ($client_name =~ /^mysqld$/)) {
    mtr_add_arg($args, "--mtr-hint=true");
  }
...
sub mysqldecrypt_arguments () {
  my $exe = mtr_exe_exists("$path_client_bindir/mysqldecrypt");
...
sub environment_setup {
  ...
  $ENV{'MYSQL_DECRYPT'}       = mysqldecrypt_arguments ();
...
# The Thread Sanitizer allocator should return NULL instead of crashing on out-of-memory.
  ...
  mtr_verbose("MYSQL_CLIENT_TEST: $ENV{'MYSQL_CLIENT_TEST'}");
}
...
sub initialize_servers {
  if (using_extern()) {
    ...
    if (!$opt_start_dirty) {
      ...
      mysql_install_db(default_mysqld(), "$opt_vardir/data/", undef, 0);
...
sub mysql_install_db {
  my ($mysqld, $datadir, $bootstrap_opts, $opt_postfix) = @_;
  ...
  my $install_tmpdir = "$opt_vardir/tmp";
  if ($opt_postfix) {
    $install_tmpdir = "$opt_vardir/tmp" . $mysqld->after('mysqld');
    copytree("$opt_vardir/tmp/", "$install_tmpdir");
  }
  mtr_add_arg($args, "--tmpdir=%s", $install_tmpdir);
  ...
  my $bootstrap_sql_file = "$install_tmpdir/bootstrap.sql";
  ...
  if ($opt_postfix) {
    $path_bootstrap_log = "$opt_vardir/log/bootstrap". $mysqld->after('mysqld').".log";
  }
...
sub clean_datadir {
  ...
  foreach my $mysqld (mysqlds()) {
    ...
    my $tmp_dir = "$opt_vardir/tmp" . $mysqld->after('mysqld');
    clean_dir("$tmp_dir") if -d "$tmp_dir";
  }
...
sub _encode_prepare_result_string {
  my $string = shift || "";
  $string =~ s/\\/\\\\/g;
  $string =~ s/\n/\\n/g;
  $string =~ s/\t/\\t/g;
...
sub _decode_prepare_result_string {
  my $string = shift || "";
  $string =~ s/\\t/\t/g;
  $string =~ s/\\n/\n/g;
  $string =~ s/\\\\/\\/g;
...
sub _write_prepare_result {
  my ($fh, $result) = @_;
  my $comment = _encode_prepare_result_string($result->{comment});
  print $fh join("\t", $result->{name}, $result->{failed} ? 1 : 0,
                   $result->{already_started} ? 1 : 0, $comment), "\n";
}

sub _read_prepare_result {
  my ($line) = @_;
  return undef unless defined $line;
  chomp $line;
  my ($name, $failed, $already_started, $comment) = split(/\t/, $line, 4);
  return {
    name          => $name,
    failed        => $failed ? 1 : 0,
    already_started => $already_started ? 1 : 0,
    comment       => _decode_prepare_result_string($comment),
  };
}

sub _prepare_mysqld_start {
  my ($mysqld, $tinfo, $opt_postfix) = @_;
  my %result = (name => $mysqld->name(), failed => 0, already_started => 0, comment => "");
  ...
  my $datadir = $mysqld->value('datadir');
  if ($opt_start_dirty) {
    # Don't delete anything when starting dirty.
  } else {
    ...
    foreach my $option_name (@options) {
      ...
      unlink($file_name) or do {
        $result{failed} = 1;
        $result{comment} = "unable to remove file '$file_name'";
        return \%result;
      };
    }
  ...
  if ($basedir eq $mysqld_basedir) {
    ...
    if (!$opt_start_dirty) {
      ...
      if (!-d $datadir or
        ...
        unless (-d $datadir) {
          $result{failed} = 1;
          $result{comment} = "Failed to copy system db to '$datadir'";
          return \%result;
        }
      }
    ...
    mysql_install_db($mysqld, undef, undef, $opt_postfix);
    unless (-d $datadir) {
      $result{failed} = 1;
      $result{comment} = "Failed to install system db to '$datadir'";
      return \%result;
    }
  }
  ...
  if ($mysqld->{need_reinitialization}) {
    clean_dir($datadir);
    mysql_install_db($mysqld, $datadir, $bootstrap_opts, $opt_postfix);
    ...
    my $bs_file = "$opt_vardir/tmp/bootstrap.sql";
    if ($opt_postfix) {
      $bs_file = "$opt_vardir/tmp" . $mysqld->after('mysqld') . "/bootstrap.sql";
    }
    unlink("$bs_file") if (-f "$bs_file");
  }
  ...
  if ($mysqld->option('#!run-master-sh') and
    ...
    $result{failed} = 1;
    $result{comment} = "Failed to execute '$tinfo->{master_sh}'";
    return \%result;
  }
  ...
  if ($mysqld->option('#!run-slave-sh') and
    ...
    $result{failed} = 1;
    $result{comment} = "Failed to execute '$tinfo->{slave_sh}'";
    return \%result;
  }
...
sub start_servers($) {
  ...
  foreach my $mysqld (mysqlds()) {
    ...
    push(@pending_mysqlds, $mysqld);
  }

  if (@pending_mysqlds > 1 && !IS_WINDOWS) {
    my %children;
    my %child_data;

    foreach my $mysqld (@pending_mysqlds) {
      pipe(my $reader, my $writer) or die("pipe failed: $!");
      my $pid = My::SafeProcess::Base::_safe_fork();
      if ($pid == 0) {
        close($reader);
        my $result = _prepare_mysqld_start($mysqld, $tinfo, @pending_mysqlds > 1);
        _write_prepare_result($writer, $result);
        close($writer);
        exit($result->{failed} ? 1 : 0);
      }
      close($writer);
      $children{$pid} = $reader;
      $child_data{$pid} = $mysqld;
    }

    foreach my $pid (keys %children) {
      waitpid($pid, 0);
      my $reader = $children{$pid};
      my $line = <$reader>;
      close($reader);
      my $result = _read_prepare_result($line);
      unless (defined $result) {
        $tinfo->{comment} = "Failed to prepare server '" . $child_data{$pid}->name() . "'";
        return 1;
      }
      if ($result->{failed}) {
        $tinfo->{comment} = $result->{comment};
        return 1;
      }
    }
  } else {
...
sub start_check_testcase ($$$) {
  ...
  #if ($opt_mtr_hint) {
  #  mtr_add_arg($args, "--mtr-hint=true");
  #  mtr_verbose("start_check_testcase() with --mtr-hint=true write to: '$name.result'");
  #}
...
Options to control what engine/variation to run
  ...
  mtr-hint              Enable the mtr-hint feature between client and server.
