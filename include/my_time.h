...
constexpr const int MAX_DATETIME_FORMAT_MODEL_LEN = 144;
/**
...
unsigned int calc_days_in_month(unsigned int year, unsigned month);
...
/**
  Switch month to roman numeral
  @param month month of year
  @param flag Represents the conversion mode
          >0 Return upper roman numeral
          =0 Return initial upper roman numeral
          <0 Return lower roman numeral
  @retval Roman numeral for the month
*/
const char *month_to_roman_numeral(int month, int flag);

/**
  Switch roman numeral to month
*/
unsigned int roman_numeral_to_month(const char **find, const char *end);

/**
 * Get weekday from a julian day.
 * @param jd
 * @return week from 0 to 6
 */
int julianday_to_day_of_week(int jd, bool sunday_first_day_of_week = 1);

/**
  Switch date to julian day
*/
int date_to_julianday(int n_year, int n_month, int n_day);

/**
  Switch julian day to date
*/
...
/**
  ...
  a. A calendar week starts on Monday.
  b. The first calendar week of the year includes January 4.
  c. The first calendar week of the year may include December 29, 30 and 31.
  d. The last calendar week of the year may include January 1, 2, and 3.
*/
int date_to_ISO_week(int n_year, int n_month, int n_day);

/**
  Get the year containing the calendar week, as defined by the ISO 8601
  standard.
*/
