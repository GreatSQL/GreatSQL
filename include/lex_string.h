...
static inline void string_replace(std::string &str,
                                  const std::string &old_value,
                                  const std::string &new_value) {
  std::string::size_type pos = 0, src_len = old_value.size(),
                         dst_len = new_value.size();
  while ((pos = str.find(old_value, pos)) != std::string::npos) {
    str.replace(pos, src_len, new_value);
    pos += dst_len;
  }
}
