...
struct load_balance_host {
  char *host{nullptr};
  unsigned int port{0};
  // Number of milliseconds since the Epoch, 1970-01-01 00:00:00 +0000
  unsigned long long invalid_time{0};
  unsigned long long connect_cnt{0};
  unsigned long long connect_fail_cnt{0};
  unsigned long long reconnect_cnt{0};
};

struct st_mysql_load_balance_options {
  char *load_balance_hosts{nullptr};
  load_balance_host *hosts{nullptr};
  unsigned int hosts_cnt{0};
  std::vector<load_balance_host *> available_hosts;
  std::list<load_balance_host *> block_hosts;
  unsigned long long blocklist_timeout{0};              // milliseconds
  unsigned long long mark_host_block_used_time{0};      // microseconds
  unsigned long long mark_host_available_used_time{0};  // microseconds
  mysql_load_balance_strategy strategy{LOAD_BALANCE_RANDOM};
  unsigned int random_seed{0};
  // next_conn_host_idx used for LOAD_BALANCE_ROUND_ROBIN
  unsigned long long next_conn_host_idx{0};
  load_balance_host *current_conn_host{nullptr};
  bool reconnecting{false};
  // The number of attempts to create a connection
  unsigned long long external_connect_cnt{0};
  unsigned long long external_connect_nonblocking_cnt{0};
  char *load_balance_status{nullptr};
  size_t load_balance_status_len{0};
  void cleanup_hosts();
  int set_hosts(const char *arg);
};
