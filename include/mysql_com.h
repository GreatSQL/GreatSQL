...
/* Reuse this flag only when server started with param --mtr-hint */

#ifdef WITH_MTR_HINT
#define CLIENT_MTR_HINT CLIENT_RESERVED
#endif
...
*/
#ifdef WITH_MTR_HINT
#define MTR_HINT_OPTIONAL_FLAG | CLIENT_MTR_HINT
#else
#define MTR_HINT_OPTIONAL_FLAG
#endif

#define CLIENT_BASIC_FLAGS                                          \
  (CLIENT_ALL_FLAGS &                                               \
   ~(CLIENT_SSL | CLIENT_COMPRESS | CLIENT_SSL_VERIFY_SERVER_CERT | \
     CLIENT_ZSTD_COMPRESSION_ALGORITHM MTR_HINT_OPTIONAL_FLAG))
...
#define DISABLED_MTR_HINT_MSG_LENGTH                                        \
  0x00FFFFFF /* Send it when server --mtr-hint=false even if client enabled \
                it*/
