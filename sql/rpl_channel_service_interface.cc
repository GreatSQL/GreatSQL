...
int channel_add_executed_gtids_to_received_gtids(const char *channel) {
  ...
  DBUG_EXECUTE_IF("disable_adding_executed_gtids_to_received_gtids", {
    channel_map.unlock();
    return 0;
  });
...
int channel_queue_packet(const char *channel, const char *buf,
                         ...
                         uint64_t &total_io_buffered_events) {
  int result;
  DBUG_TRACE;
  ...
  if (result == QUEUE_EVENT_OK) {
    total_events = mi->total_events;
    total_io_buffered_events = mi->total_buffered_events;
  }
...
/**
 * Dead code for an deleted option:
 *   group_replication_async_auto_failover_channel_read_only_mode
 */
...
void set_cur_exec_channel_as_applier() { cur_exec_channel.set_as_applier(); }

void set_cur_exec_channel_as_recover() { cur_exec_channel.set_as_recover(); }

unsigned long long get_cur_exec_channel_delay() {
  return cur_exec_channel.get_second_behind_group();
}
