...
bool evict_cached_sequences(const std::vector<std::string> *seq_keys = nullptr,
                            bool need_lock = true);
...
class Gdb_sequence_entity {
 ...
 private:
  ...
  friend bool evict_cached_sequences(const std::vector<std::string> *seq_keys,
                                     bool need_lock);
};
...
#define NO_SEQ_DEF 0ull
#define FIND_SEQ_FAIL UINT64_MAX
...
class Sql_cmd_reload_sequences final : public Sql_cmd {
 public:
  Sql_cmd_reload_sequences(const Sql_cmd_reload_sequences &) = delete;
  Sql_cmd_reload_sequences(Sql_cmd_reload_sequences &&) = delete;
  Sql_cmd_reload_sequences &operator=(const Sql_cmd_reload_sequences &) =
      delete;
  Sql_cmd_reload_sequences &operator=(Sql_cmd_reload_sequences &&) = delete;

  explicit Sql_cmd_reload_sequences(const Mem_root_array<Table_ident *> *seqs);
  ~Sql_cmd_reload_sequences() override = default;

  enum_sql_command sql_command_code() const override {
    return enum_sql_command::SQLCOM_RELOAD_SEQUENCES;
  }

  bool execute(THD *thd) override;

 private:
  std::vector<std::string> m_seq_keys;
};
