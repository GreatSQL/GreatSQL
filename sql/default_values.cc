...
bool prepare_default_value(THD *thd, uchar *buf, TABLE *table,
  ...
  if (thd->lex && thd->lex->sql_command == SQLCOM_CREATE_TABLE &&
      thd->lex->query_block && !thd->lex->query_block->field_list_is_empty() &&
      (thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
      (field.flags & NOT_NULL_FLAG) && regfield->is_empty_string()) {
    // In sqlmode `EMPTYSTRING_EQUAL_NULL`, when this field is not nullable and
    // it's default value is empty string (''), should set `has_no_default`
    // true. If not, executing DDL on the table containing this field will be
    // failed in sqlmode `EMPTYSTRING_EQUAL_NULL,STRICT_ALL_TABLES`.
    //
    // NOTE:
    //   Currently this is only for table(s) created by `CREATE TABLE...SELECT`
    //   and field(s) generated by constant string.
    //   Such as:
    //     CREATE TABLE t2 SELECT 'xxx' AS const_c, c1, c2 FROM t1.
    //   field `const_c` of t2 need to set has_no_default true in sqlmode
    //   `EMPTYSTRING_EQUAL_NULL`.
    col_obj->set_has_no_default(true);
  } else {
