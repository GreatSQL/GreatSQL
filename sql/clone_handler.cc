...
#define XA_BLOCKING_TIMEOUT 60
#define XA_OPERATION_TIMEOUT (XA_BLOCKING_TIMEOUT + 10)
...
static long unsigned clone_recovery_mode;
...
Clone_handler::XA_Operation::XA_Operation(THD *thd, bool two_phase_xa)
    : m_thd(thd), m_two_phase_xa(two_phase_xa), m_success(true) {
  if (m_two_phase_xa) {
    m_success = begin_xa_operation(thd);
...
bool Clone_handler::XA_Operation::failed() const { return (!m_success); }

bool Clone_handler::begin_xa_operation(THD *thd) {
  ...
  while (s_xa_block_op.load()) {
    ...
    /* Timeout in 70 seconds */
    int max_wait_count = XA_OPERATION_TIMEOUT * 100;
    if (DBUG_EVALUATE_IF("test_small_clone_block_timeout", true, false)) {
      max_wait_count = 10 * 100;  // 10s
    }
    if (++count > max_wait_count) {
      LogErr(WARNING_LEVEL, ER_CLONE_CLIENT_TRACE,
             "Clone blocked XA operation too long: 70s(Timeout)");
      ret = false;
      break;
    }
...
Clone_handler::XA_Block::XA_Block(THD *thd, long unsigned recovery_mode) {
  m_recovery_mode = recovery_mode;
  if (m_recovery_mode == CLONE_DATA_RECOVERY_BINLOG_POSITION) {
...
bool Clone_handler::block_xa_operation(THD *thd) {
...
#ifndef NDEBUG
  DBUG_SIGNAL_WAIT_FOR(thd, "clone_block_xa_operation", "reach_xa_block",
                       "end_xa_block");
#endif
  int count = 0;
  ...
  while (s_xa_counter.load() != 0) {
    ...
    if (++count > XA_BLOCKING_TIMEOUT * 100) {
...
void set_clone_data_recovery_mode(ulong mode) { clone_recovery_mode = mode; }
long unsigned get_clone_data_recovery_mode() { return clone_recovery_mode; }
