...
Item *create_condition(List<Item> &join_conds) {
  ...
  /**
   * If current_block is a derived_table which can be merged in the future,
   * then the join_cond of inner_table need to update_used_tables after
   * remap_tables().for example:
   * select () t1, (select ..from e, f where e.id=f.id(+)) t2
   * where t2.x = t1.x(+); after derived_table t2 was merged, remap_tables()
   * will remap all tables, so the join_cond of table 'f' need setting fixed to
   * true, then fix_tables_after_pullout() will update tables info for this
   * cond, or not. fix bug#13621.
   */
  cond->fixed = true;
  return cond;
}
  ...
  for (std::vector<uint>::iterator it = joined_tables.begin();
    ...
    if (mit2 != outer_joined_map.end()) {
      ...
      if (i == 0) {
         ...
         */
        Table_ref *outer_prev_join = get_prev_join(nest_joins, outer_table);
        Table_ref *inner_prev_join = get_prev_join(nest_joins, inner_table);
        if (outer_prev_join) outer_table = outer_prev_join;
        if (inner_prev_join) inner_table = inner_prev_join;
        inner_table->outer_join = true;
        add_join_on(inner_table, on);
...
bool Outerjoin_transformer::hasCycle() {
  std::set<uint> visited, stack;

  for (auto &[node, _] : m_outer_info.outer_to_inner_map) {
    if (!visited.count(node) && dfs(node, visited, stack)) return true;
  }
  return false;
}

bool Outerjoin_transformer::dfs(uint node, std::set<uint> &visited,
                                std::set<uint> &stack) {
  visited.insert(node);
  stack.insert(node);

  for (uint neighbor : m_outer_info.outer_to_inner_map[node]) {
    if (stack.count(neighbor)) return true;
    if (!visited.count(neighbor) && dfs(neighbor, visited, stack)) return true;
  }
