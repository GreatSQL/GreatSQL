...
JOIN::JOIN(THD *thd_arg, Query_block *select)
    : query_block(select),
      ...
      order_initial(select->order_list.first),
...
bool JOIN::optimize(bool finalize_access_paths) {
  ...
  */
  ...
  if ((query_expression()->select_limit_ties ||
  ...
  */
  ...
  } else if (tables_list == nullptr && connect_by_cond == nullptr) {
...
static bool test_if_skip_sort_order(JOIN_TAB *tab, ORDER_with_src &order,
  ...
  *order_idx = -1;
  ...
  /*
    NOTE: sort even after index scan if sorting order is not in MySQL style.
    To support nulls_pos == NULLS_POS_ORACLE, it is necessary to create
    INDEX to use the same NULL order as ORACLE which is not avaiable now.
...
void JOIN::adjust_access_methods() {
  ...
  for (uint i = const_tables; i < tables; i++) {
    ...
    Query_block *outer_query_block =
        (tl && tl->query_block && tl->query_block->master_query_expression())
  ...
  for (uint tableno = 0; tableno < tables; tableno++) {
    ...
    */
    ...
    if (tab->type() != JT_CONST && tab->type() != JT_SYSTEM) {
      ...
      } else if (!pos->key || tab->table_ref->foj_inner ||
                 tab->table_ref->foj_outer) {
        if (tab->range_scan())
          tab->set_type(calc_join_type(tab->range_scan()));
        else
          tab->set_type(JT_ALL);
    ...
    if (!tab->const_keys.is_clear_all() ||
      ...
      */
      ...
      range_analysis_done = true;
               ...
               tab->join_cond()->val_int() == 0 && !query_block->merge_target) {
      /* MERGE_PS8404_CHECK_TAG: @fnw, !query_block->merge_target */
      always_false_cond = true;
    }
...
bool evaluate_during_optimization(const Item *item, const Query_block *select) {
  ...
  */
  ...
  // forced rownum execute
  if (item->has_rownum_expr()) return false;
