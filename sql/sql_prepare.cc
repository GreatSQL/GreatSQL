...
bool Prepared_statement::send_statement(THD *thd, uint no_columns,
                                        Query_result *result,
                                        const mem_root_deque<Item *> *types) {
  const Prepared_statement *stmt = this;
  ...
  if (thd->is_classic_protocol()) {
    ...
    uchar *pos;
    if (const auto net = thd->get_protocol_classic()->get_net();
        !rc && net->mtr_hint) {
      // write mtr-hint message string length
      size_t msg_size = strlen(thd->net.mtr_hint_msg) + 1;
      pos = net_store_length((uchar *)&tmp, msg_size);
      my_net_write(net, (uchar *)&tmp, (size_t)(pos - (uchar *)&tmp));
      // write mtr-hint message string
      my_net_write(net, (uchar *)net->mtr_hint_msg, msg_size);
    } else {
      pos =
          net_store_length((uchar *)&tmp, (size_t)DISABLED_MTR_HINT_MSG_LENGTH);
      my_net_write(net, (uchar *)&tmp, (size_t)(pos - (uchar *)&tmp));
    }
  }
#endif
...
bool Sql_cmd_create_table::prepare(THD *thd) {
  ...
  if (!query_block->fields.empty()) {
    ...
    if (lex->create_info->used_fields & HA_CREATE_MATERIALIZED_VIEW) {
      // in fast mode can't reopen mlog in execute

      if (lex->create_info->m_mv_info->flush_mode == 1) {
        my_error(ER_NOT_SUPPORTED_YET, MYF(0),
                 "prepare create materaizlied view fast mode");
        return true;
      }

      result =
          new (thd->mem_root) greatdb::Query_result_materialized_view_create(
              thd, create_table, &query_block->fields, lex->duplicates,
...
bool Prepared_statement::prepare_query(THD *thd) {
  ...
  */
  ...
  switch (sql_command) {
    ...
    case SQLCOM_SELECT:
      /*
         with function is not supported with
         prepared statements
       */
      if (m_lex->m_with_func) {
        my_error(ER_UNSUPPORTED_PS, MYF(0));
        return true;
      } else {
  ...
  return send_statement(thd, no_columns, result, types);
}
...
void Prepared_statement::set_state_for_prepare(THD *thd) {
  m_esn_state = (thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL);
}

bool Prepared_statement::is_state_inconsistent(THD *thd) {
  bool is_emptystr = (thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL);
  return is_emptystr != m_esn_state;
}
...
Prepared_statement::~Prepared_statement() {
  ...
  if (m_dbms_sql_define_column) {
    ...
    ::destroy_at(m_dbms_sql_define_column);
...
// <<<<<<< HEAD
  // #MERGE_PS8404_CHECK_TAG: @tianjun, why reprepare 2 times? can merge those?
...
//>>>>>>> remotes/wenfengz/ps-8.4.4-revert-fast-mode
...
bool Prepared_statement::reprepare(THD *thd) {
  ...
  if (m_is_ref_cursor) {
    assert(thd->get_reprepare_observer());
    return ask_to_reprepare(thd);
  }
  /*
