...
/**
  Oracle compatible:
    like : add 1 = add 1 day
    transform double to Interval
    use interval_type: INTERVAL_DAY_SECOND

   Uses propagate_datetime_overflow() to handle and propagate any warnings from
   date_add_interval() to the THD.

   @param  thd
   @param  ltime  datetype
   @param  val    days value ，transformation into interval day
   @param  neg    true: minus ,  false: plus

   @return False on success, true on error.
 */
bool date_add_double_with_warn(THD *thd, MYSQL_TIME *ltime, double val,
                               bool neg) {
  Interval interval = {};

  interval.neg = val >= 0 ? neg : !neg;
  ...
  decPart = modf(val, &intPart);
  interval.day = lround(intPart);  // static_cast<unsigned long int>(intPart);
  // date_add_interval will check overflow
  interval.second =
      static_cast<unsigned long long int>(std::round(decPart * SECONDS_IN_24H));

  return propagate_datetime_overflow(thd, [&](int *w) {
    return date_add_interval(ltime, INTERVAL_DAY_SECOND, interval, w);
  });
}
