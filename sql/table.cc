...
void TABLE_SHARE::destroy() {
  ...
  */
  ...
  if (mview_info != nullptr) {
    mview_info->~materialized_view_info();
    mview_info = nullptr;
  }
...
bool TABLE_SHARE::export_structure(THD *thd, List<Create_field> *defs,
                                   bool need_hidden) {
  for (Field **src = field; *src; src++) {
    Create_field *def = new (thd->mem_root) Create_field();
    *def = Create_field(*src, NULL);
    if (!need_hidden && def->hidden != dd::Column::enum_hidden_type::HT_VISIBLE)
      continue;
    if ((*src)->real_type() == MYSQL_TYPE_ENUM ||
        (*src)->real_type() == MYSQL_TYPE_SET) {
      Field_enum *enum_field = dynamic_cast<Field_enum *>(*src);
      TYPELIB *type_new = create_typelib_from_field(thd->mem_root, enum_field);
      if (!type_new) return true;
      def->interval = type_new;
    }
    // It must maintain the field_name to show for udt column.
    // @@NOTE@@ zip_dict_name is not used now.
    // If it should be used outside, re-allocate here.
    const char *name = (*src)->field_name;
    def->field_name = thd->strmake(name, strlen(name));
    if (def->udt_name.str) {
      def->udt_name.str =
    ...
    def->m_secondary_engine_attribute =
        thd->strmake((*src)->m_secondary_engine_attribute);
    def->field = nullptr;
...
int open_table_from_share(THD *thd, TABLE_SHARE *share, const char *alias,
  ...
  */
  ...
  if (!(bitmaps = root->ArrayAlloc<uchar>(bitmap_size * 9))) goto err;
#else
  ...
  bitmap_init(&outparam->read_set_internal,
              ...
              share->fields);
#ifdef HAVE_QUERY_PLAN_PLUGIN
  bitmap_init(&outparam->def_select_list_set,
              (my_bitmap_map *)(bitmaps + bitmap_size * 8), share->fields);
  outparam->select_list_set = &outparam->def_select_list_set;
#endif
...
/**
  ...
  @param udt_fld    buffer for udt field index
...
#ifdef HAVE_QUERY_PLAN_PLUGIN
  bitmap_clear_all(&def_select_list_set);
#endif
...
#ifdef HAVE_QUERY_PLAN_PLUGIN
      if (select_list_set)
        bitmap_set_bit(select_list_set, field->field_index());
#endif
      break;
    }
...
void TABLE::set_all_fields_to_null() {
  for (uint i = 0; i < s->fields; i++) field[i]->set_null();
}
bool TABLE::sp_set_all_fields_from_item(THD *thd, Item *value) {
  assert(value->fixed == true);
  assert(value->cols() == s->fields);
  for (uint i = 0; i < value->cols(); i++) {
    if (field[i]->sp_prepare_and_store_item(thd, value->addr(i))) return true;
  }
  return false;
}

bool TABLE::sp_find_field_by_name_or_error(
    uint *idx, const LEX_CSTRING &var_name,
    const LEX_CSTRING &field_name) const {
  if (sp_find_field_by_name(idx, field_name)) {
    my_error(ER_ROW_VARIABLE_DOES_NOT_HAVE_FIELD, MYF(0), var_name.str,
             field_name.str);
    return true;
  }
  return false;
...
bool TABLE::sp_find_field_by_name(uint *idx, const LEX_CSTRING &name) const {
  Field *f;
  for (uint i = 0; (f = field[i]); i++) {
    // Use the same comparison style with sp_context::find_variable()
    if (!my_strnncoll(system_charset_info, (const uchar *)f->field_name,
                      strlen(f->field_name), (const uchar *)name.str,
                      name.length)) {
      *idx = i;
      return false;
    }
