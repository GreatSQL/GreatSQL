...
unique_ptr<
    ...
    proc_priv_hash, func_priv_hash, package_priv_hash, package_body_priv_hash,
    ora_type_priv_hash;
...
static void set_grantorname(char **grantor, const char *grantor_arg,
                            MEM_ROOT *mem) {
  assert(grantor != nullptr);
  *grantor =
      (grantor_arg && *grantor_arg) ? strdup_root(mem, grantor_arg) : nullptr;
}
#endif
/**
...
#ifdef CASCADE_REVOKE
  if (grantor) dst->grantor = safe_strdup_root(root, grantor);
#endif
...
#ifdef CASCADE_REVOKE
void ACL_USER::set_grantor(MEM_ROOT *mem, const char *grantor_arg) {
  set_grantorname(&grantor, grantor_arg, mem);
}
#endif
...
void grant_free(void) {
  ...
  package_priv_hash.reset();
  ora_type_priv_hash.reset();
  package_body_priv_hash.reset();
  memex.Clear();
}
...
static bool grant_reload_procs_priv(Table_ref *table) {
  ...
  unique_ptr<
      malloc_unordered_multimap<string, unique_ptr_destroy_only<GRANT_NAME>>>
      old_package_priv_hash(std::move(package_priv_hash));
  unique_ptr<
      malloc_unordered_multimap<string, unique_ptr_destroy_only<GRANT_NAME>>>
      old_package_body_priv_hash(std::move(package_body_priv_hash));
  unique_ptr<
      malloc_unordered_multimap<string, unique_ptr_destroy_only<GRANT_NAME>>>
      old_ora_type_priv_hash(std::move(ora_type_priv_hash));
  ...
  if ((return_val = grant_load_procs_priv(table->table))) {
    ...
    package_priv_hash = std::move(old_package_priv_hash);
    package_body_priv_hash = std::move(old_package_body_priv_hash);
    ora_type_priv_hash = std::move(old_ora_type_priv_hash);
  }
...
#ifdef CASCADE_REVOKE
  acl_user.set_grantor(&global_acl_memory, grantor);
#endif
  assert(plugin.str);
  if (plugin.str[0]) {
...
Acl_map::Acl_map(Security_context *sctx, uint64 ver)
  ...
  get_privilege_access_maps(
      ...
      &m_db_wild_acls, &m_table_acls, &m_sp_acls, &m_func_acls, &m_type_acls,
      &m_package_acls, &m_package_body_acls, &granted_roles, &m_with_admin_acls,
...
Acl_map &Acl_map::operator=(Acl_map &&map) {
  ...
  m_type_acls = std::move(map.m_type_acls);
  m_package_acls = std::move(map.m_package_acls);
  m_package_body_acls = std::move(map.m_package_body_acls);
...
SP_access_map *Acl_map::type_acls() { return &m_type_acls; }

SP_access_map *Acl_map::package_acls() { return &m_package_acls; }

SP_access_map *Acl_map::package_body_acls() { return &m_package_body_acls; }
