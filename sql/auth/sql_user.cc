...
enum enum_acl_lists {
  ...
  PROXY_USERS_ACL,
  PACKAGE_PRIVILEGES_HASH,
  PACKAGE_BODY_PRIVILEGES_HASH,
  TYPE_PRIVILEGES_HASH
};
  ...
  if (!is_role &&
    ...
    if (auth->generate_authentication_string(outbuf, &buflen, inbuf,
      ...
      */
      ...
      thd->set_gdb_current_user(nullptr);
      return (true);
    }
    thd->set_gdb_current_user(nullptr);
...
/**
  ...
  @note
    ...
    5 ACL_PROXY_USERS
    6 PACKAGE_PRIVILEGES_HASH
    7 PACKAGE_BODY_PRIVILEGES_HASH
    8 TYPE_PRIVILEGES_HASH
...
static int handle_grant_struct(enum enum_acl_lists struct_no, bool drop,
  ...
  switch (struct_no) {
    ...
    case PACKAGE_PRIVILEGES_HASH:
      if (drop)
        remove_matching_grants(package_priv_hash.get(), matches);
      else if (user_to) {
        if (rename_matching_grants(package_priv_hash.get(), matches, user_to))
          return -1;
      } else
        search_for_matching_grant(package_priv_hash.get(), matches);
      break;

    case PACKAGE_BODY_PRIVILEGES_HASH:
      if (drop)
        remove_matching_grants(package_body_priv_hash.get(), matches);
      else if (user_to) {
        if (rename_matching_grants(package_body_priv_hash.get(), matches,
                                   user_to))
          return -1;
      } else
        search_for_matching_grant(package_body_priv_hash.get(), matches);
      break;

    case TYPE_PRIVILEGES_HASH:
      if (drop)
        remove_matching_grants(ora_type_priv_hash.get(), matches);
      else if (user_to) {
        if (rename_matching_grants(ora_type_priv_hash.get(), matches, user_to))
          return -1;
      } else
        search_for_matching_grant(ora_type_priv_hash.get(), matches);
      break;
...
static int handle_grant_data(THD *thd, Table_ref *tables, bool drop,
  ...
  if ((found = handle_grant_table(thd, tables, ACL_TABLES::TABLE_PROCS_PRIV,
    ...
    /* Handle package array. */
    if ((((ret = handle_grant_struct(PACKAGE_PRIVILEGES_HASH, drop, user_from,
                                     user_to, on_drop_role_priv)) > 0 &&
          !result) ||
         found) &&
        !result) {
    ...
    if ((((ret = handle_grant_struct(PACKAGE_BODY_PRIVILEGES_HASH, drop,
                                     user_from, user_to, on_drop_role_priv)) >
              0 &&
          !result) ||
         found) &&
        !result) {
    ...
    /* Handle type array. */
    if ((((ret = handle_grant_struct(TYPE_PRIVILEGES_HASH, drop, user_from,
                                     user_to, on_drop_role_priv)) > 0 &&
          !result) ||
         found) &&
        !result) {
