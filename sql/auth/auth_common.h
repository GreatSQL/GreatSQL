...
#ifdef CASCADE_REVOKE
  virtual uint user_grantor_idx() = 0;
#endif
...
#ifdef CASCADE_REVOKE
  uint user_grantor_idx() override { return MYSQL_USER_FIELD_GRANTOR; }
#endif
};
...
#ifdef CASCADE_REVOKE
  uint user_grantor_idx() override { return MYSQL_USER_FIELD_COUNT_56; }
#endif
};
...
bool mysql_routine_grant(THD *thd, Table_ref *table, enum_sp_type sp_type,
...
bool check_grant_routine(THD *thd, Access_bitmask want_access, Table_ref *procs,
                         enum_sp_type sp_type, bool no_error);
...
bool mysql_show_grants(THD *, LEX_USER *, const List_of_auth_id_refs &, bool,
                       bool, bool, String *func_caller_output = nullptr);
...
#ifdef SSL_GM
#define DEFAULT_GM_SSL_SERVER_CERT "server-sign-cert.pem server-enc-cert.pem"
#define DEFAULT_GM_SSL_SERVER_KEY "server-sign-key.pem server-enc-key.pem"
#define DEFAULT_SSL_SERVER_SIGN_CERT "server-sign-cert.pem"
#define DEFAULT_SSL_SERVER_ENC_CERT "server-enc-cert.pem"
#define DEFAULT_SSL_SERVER_SIGN_KEY "server-sign-key.pem"
#define DEFAULT_SSL_SERVER_ENC_KEY "server-enc-key.pem"
#endif /* SSL GM */
