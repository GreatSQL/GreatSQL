...
void Acl_table_user_reader::add_row_to_acl_users(ACL_USER &user) {
  ...
  acl_users_add_one(user.user, user.host.get_host(),
#ifdef CASCADE_REVOKE
                    user.grantor,
#endif
                    user.ssl_type, user.ssl_cipher, user.x509_issuer,
                    user.x509_subject, &user.user_resource, user.access,
                    user.plugin, auth, second_auth, user.password_last_changed,
                    password_life, false, *m_restrictions,
                    password_life.failed_login_attempts,
                    password_life.password_lock_time, user.m_mfa, m_thd);
}

#ifdef CASCADE_REVOKE
void Acl_table_user_reader::read_user_grantor(ACL_USER &user) {
  if (!m_table->field[m_table_schema->user_grantor_idx()]->is_null()) {
    user.grantor = get_field(
        &m_mem_root, m_table->field[m_table_schema->user_grantor_idx()]);
  } else {
    user.grantor = nullptr;
  }
}
#endif
