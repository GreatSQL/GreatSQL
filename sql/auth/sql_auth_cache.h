...
class ACL_USER : public ACL_ACCESS {
  ...
  */
  ...
  char *grantor;
  void set_grantor(MEM_ROOT *mem, const char *grantor_arg);
#endif
...
inline GRANT_NAME *routine_hash_search(const char *host, const char *ip,
                                       ...
                                       const char *tname, enum_sp_type sp_type,
                                       bool exact) {
  switch (sp_type) {
    case enum_sp_type::PROCEDURE:
      return name_hash_search(*proc_priv_hash, host, ip, db, user, tname, exact,
                              true);
    case enum_sp_type::FUNCTION:
      return name_hash_search(*func_priv_hash, host, ip, db, user, tname, exact,
                              true);
    case enum_sp_type::PACKAGE_SPEC:
      return name_hash_search(*package_priv_hash, host, ip, db, user, tname,
                              exact, true);
    case enum_sp_type::PACKAGE_BODY:
      return name_hash_search(*package_body_priv_hash, host, ip, db, user,
                              tname, exact, true);
    case enum_sp_type::TYPE:
      return name_hash_search(*ora_type_priv_hash, host, ip, db, user, tname,
                              exact, true);
    default:
      assert(false);
      return nullptr;
...
class Acl_map {
 ...
 public:
  ...
  SP_access_map *type_acls();
  SP_access_map *package_acls();
  SP_access_map *package_body_acls();
 ...
 private:
  ...
  SP_access_map m_type_acls;
  SP_access_map m_package_acls;
  SP_access_map m_package_body_acls;
