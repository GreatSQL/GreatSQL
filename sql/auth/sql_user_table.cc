...
static const TABLE_FIELD_TYPE
    mysql_procs_priv_table_fields[MYSQL_PROCS_PRIV_FIELD_COUNT] = {
        ...
        {{STRING_WITH_LEN("Routine_type")},
         {STRING_WITH_LEN(
             "enum('FUNCTION','PROCEDURE','PACKAGE','PACKAGE BODY','TYPE')")},
         {nullptr, 0}},
        {{STRING_WITH_LEN("Grantor")},
         {STRING_WITH_LEN("varchar(288)")},
         {nullptr, 0}},
...
#ifdef CASCADE_REVOKE
static void get_current_grantor(THD *thd, char *grantor) {
  const char *user = thd->security_context()->priv_user().str;
  const char *host = thd->security_context()->priv_host().str;
  if (thd->slave_thread && thd->has_invoker()) {
    user = thd->get_invoker_user().str;
    host = thd->get_invoker_host().str;
  }
...
int replace_routine_table(THD *thd, GRANT_NAME *grant_name, TABLE *table,
                          ...
                          const char *routine_name, enum_sp_type sp_type,
  ...
  */
  ...
  table->field[3]->store(routine_name, strlen(routine_name),
                         &my_charset_latin1);
  table->field[4]->store(to_longlong(sp_type), true);
  store_record(table, record[1]);  // store at pos 1
  key_copy(user_key, table->record[0], table->key_info,
           table->key_info->key_length);
  error = table->file->ha_index_read_idx_map(table->record[0], 0, user_key,
  ...
  if (rights) {
    ...
    if (sp_type == enum_sp_type::PROCEDURE)
      erase_specific_element(proc_priv_hash.get(), grant_name->hash_key,
                             grant_name);
    else if (sp_type == enum_sp_type::FUNCTION)
      erase_specific_element(func_priv_hash.get(), grant_name->hash_key,
                             grant_name);
    else if (sp_type == enum_sp_type::PACKAGE_SPEC)
      erase_specific_element(package_priv_hash.get(), grant_name->hash_key,
                             grant_name);
    else if (sp_type == enum_sp_type::PACKAGE_BODY)
      erase_specific_element(package_body_priv_hash.get(), grant_name->hash_key,
                             grant_name);
    else if (sp_type == enum_sp_type::TYPE)
      erase_specific_element(ora_type_priv_hash.get(), grant_name->hash_key,
                             grant_name);
    else
      return 1;
  }
...
#ifdef CASCADE_REVOKE
bool check_and_insert_grantor(THD *thd, ACL_USER *this_user, LEX_USER *user,
                              TABLE *table, bool revoke_grant) {
  int error;
  if (revoke_grant) return false;

  char grantor[USER_HOST_BUFF_SIZE];
  get_current_grantor(thd, grantor);

  if (this_user->grantor != nullptr) {
    if (!strcmp(this_user->grantor, grantor))
      return false;
    else {
      my_error(ER_MULTIPLE_GRANT_ERROR, MYF(0), user->user.str, user->host.str);
      return true;
    }
  }
  ...
  uchar user_key[MAX_KEY_LENGTH];
  table->use_all_columns();
  table->field[mysql_user_table_field::MYSQL_USER_FIELD_HOST]->store(
      user->host.str, user->host.length, system_charset_info);
  table->field[mysql_user_table_field::MYSQL_USER_FIELD_USER]->store(
      user->user.str, user->user.length, system_charset_info);
  key_copy(user_key, table->record[0], table->key_info,
           table->key_info->key_length);

  error = table->file->ha_index_read_idx_map(table->record[0], 0, user_key,
                                             HA_WHOLE_KEY, HA_READ_KEY_EXACT);
  if (error == HA_ERR_END_OF_FILE || error == HA_ERR_KEY_NOT_FOUND) assert(0);
  store_record(table, record[1]);  // store at pos 1
  table->field[mysql_user_table_field::MYSQL_USER_FIELD_GRANTOR]->store(
      grantor, strlen(grantor), system_charset_info);
  table->field[mysql_user_table_field::MYSQL_USER_FIELD_GRANTOR]->set_notnull();
  error = table->file->ha_update_row(table->record[1], table->record[0]);
  if (error && error != HA_ERR_RECORD_IS_THE_SAME) {
    acl_print_ha_error(error);
    return true;
  }
  this_user->set_grantor(&global_acl_memory, grantor);
  return false;
}
#endif
