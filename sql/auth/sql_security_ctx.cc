...
Access_bitmask Security_context::type_acl(LEX_CSTRING db, LEX_CSTRING type_name) {
  if (m_acl_map == nullptr)
    return 0;
  else {
    SP_access_map::iterator it;
    ...
    append_identifier(&q_name, db.str, db.length);
    q_name.append(".");
    std::string name(type_name.str, type_name.length);
    my_casedn_str(files_charset_info, &name[0]);
    append_identifier(&q_name, name.c_str(), name.length());
    it = m_acl_map->sp_acls()->find(q_name.c_ptr());
    if (it == m_acl_map->type_acls()->end()) return 0;
    return filter_access(it->second, q_name.c_ptr());
  }
}

Access_bitmask Security_context::package_acl(LEX_CSTRING db, LEX_CSTRING package_name) {
  if (m_acl_map == nullptr)
    return 0;
  else {
    SP_access_map::iterator it;
    ...
    append_identifier(&q_name, db.str, db.length);
    q_name.append(".");
    std::string name(package_name.str, package_name.length);
    my_casedn_str(files_charset_info, &name[0]);
    append_identifier(&q_name, name.c_str(), name.length());
    it = m_acl_map->sp_acls()->find(q_name.c_ptr());
    if (it == m_acl_map->package_acls()->end()) return 0;
    return filter_access(it->second, q_name.c_ptr());
  }
}

Access_bitmask Security_context::package_body_acl(LEX_CSTRING db,
                                         LEX_CSTRING package_name) {
  if (m_acl_map == nullptr)
    return 0;
  else {
    SP_access_map::iterator it;
    ...
    append_identifier(&q_name, name.c_str(), name.length());
    it = m_acl_map->sp_acls()->find(q_name.c_ptr());
    if (it == m_acl_map->package_body_acls()->end()) return 0;
    return filter_access(it->second, q_name.c_ptr());
  }
}
