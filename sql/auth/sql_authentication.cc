 ...
 public:
  X509 *operator()(EVP_PKEY *pkey, const Sql_string_t cn, uint32_t serial,
    ...
    if (!enable_gm_ssl) {
      ...
      if (!X509_sign(x509, self_sign ? pkey : ca_pkey, EVP_sm3())) goto err;
    }
#else
    if (!X509_sign(x509, self_sign ? pkey : ca_pkey, EVP_sha256())) goto err;
#endif
...
#ifdef SSL_GM
static EVP_PKEY *generate_ecc_key() {
  EVP_PKEY *pkey = nullptr;
  EVP_PKEY_CTX *sm2_ctx = nullptr;
  sm2_ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_EC, NULL);
  if (sm2_ctx == nullptr) goto end;
  if (!EVP_PKEY_keygen_init(sm2_ctx)) goto end;
  if (!EVP_PKEY_CTX_set_ec_paramgen_curve_nid(sm2_ctx, OBJ_sn2nid("SM2")))
    goto end;
  if (!EVP_PKEY_keygen(sm2_ctx, &pkey)) goto end;
  if (!EVP_PKEY_set_alias_type(pkey, EVP_PKEY_SM2)) goto end;
  return pkey;
end:
  if (sm2_ctx) EVP_PKEY_CTX_free(sm2_ctx);
...
bool create_x509_certificate(RSA_generator_func &rsa_gen, const Sql_string_t cn,
  ...
  if (enable_gm_ssl) {
    ...
    if (!pkey) {
      LogErr(ERROR_LEVEL, ER_X509_NEEDS_ECC_PRIVKEY);
      goto end;
    }
  } else {
    ...
    */
    ...
    if (auto_detection_status == SSL_ARTIFACTS_VIA_OPTIONS) {
      ...
      */
      ...
      bool gen_cert_ret = true;
      gen_cert_ret = create_x509_certificate(
          rsa_gen, ca_name, 1, DEFAULT_SSL_CA_CERT, DEFAULT_SSL_CA_KEY, fcr);
#ifdef SSL_GM
      if (enable_gm_ssl) {
        gen_cert_ret =
            gen_cert_ret &
            create_x509_certificate(rsa_gen, server_name, 2,
                                    DEFAULT_SSL_SERVER_SIGN_CERT,
                                    DEFAULT_SSL_SERVER_SIGN_KEY, fcr,
                                    DEFAULT_SSL_CA_KEY, DEFAULT_SSL_CA_CERT) &
            create_x509_certificate(rsa_gen, server_name + "enc", 3,
                                    DEFAULT_SSL_SERVER_ENC_CERT,
                                    DEFAULT_SSL_SERVER_ENC_KEY, fcr,
                                    DEFAULT_SSL_CA_KEY, DEFAULT_SSL_CA_CERT) &
            create_x509_certificate(rsa_gen, client_name, 4,
                                    "client-sign-cert.pem",
                                    "client-sign-key.pem", fcr,
                                    DEFAULT_SSL_CA_KEY, DEFAULT_SSL_CA_CERT) &
            create_x509_certificate(rsa_gen, client_name + "enc", 5,
                                    "client-enc-cert.pem", "client-enc-key.pem",
                                    fcr, DEFAULT_SSL_CA_KEY,
                                    DEFAULT_SSL_CA_CERT);
      } else {
        gen_cert_ret = gen_cert_ret &
      ...
      gen_cert_ret = gen_cert_ret &
                     ...
                     create_x509_certificate(
                         ...
                         DEFAULT_SSL_CA_CERT);
#endif /* SSL_GM */
      if (gen_cert_ret == false) return false;

      *ssl_ca = DEFAULT_SSL_CA_CERT;
#ifdef SSL_GM
      if (enable_gm_ssl) {
        *ssl_cert = DEFAULT_GM_SSL_SERVER_CERT;
        *ssl_key = DEFAULT_GM_SSL_SERVER_KEY;
      } else {
