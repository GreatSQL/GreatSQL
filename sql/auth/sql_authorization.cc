...
static bool mysql_revoke_cascade(THD *thd, Table_ref *priv_tables,
                                 const char *db, List<LEX_USER> &grantor_list,
                                 ulong rights,
                                 const List<LEX_CSTRING> &dynamic_privilege,
                                 bool grant_all_current_privileges);

static bool mysql_table_revoke_cascade(THD *thd, Table_ref *table_list,
                                       Table_ref *priv_tables,
...
static bool mysql_revoke_all_cascade(THD *thd, Table_ref *tables,
                                     List<LEX_USER> &user_list);
...
inline static void show_grants_output(String output, Protocol *protocol,
                                      String *func_caller_output,
                                      int object_type, bool new_line = true) {
  if (func_caller_output) {
    int caller_object_type = *func_caller_output->ptr();
    if (caller_object_type == object_type) {
      if (new_line) func_caller_output->append(STRING_WITH_LEN("\n"));
      func_caller_output->append(output);
    }
  } else {
    protocol->start_row();
...
void make_database_privilege_statement(THD *thd, ACL_USER *role,
                                       ...
                                       String *func_caller_output = nullptr) {
  assert(assert_acl_cache_read_lock(thd));

  auto make_grant_stmts = [thd, role, protocol,
                           func_caller_output](const Db_access_map &map) {
    for (const Db_access_map::value_type &it : map) {
      ...
      /*output to func_caller_output or protocol */
      show_grants_output(db, protocol, func_caller_output, 9);
    }
  };
...
void make_proxy_privilege_statement(THD *thd [[maybe_unused]], ACL_USER *user,
  ...
  for (ACL_PROXY_USER *proxy = acl_proxy_users->begin();
    ...
    if (proxy->granted_on(user->host.get_host(), user->user)) {
      ...
      show_grants_output(global, protocol, func_caller_output, 9);
    }
  }
}
...
void make_sp_privilege_statement(THD *thd, ACL_USER *role, Protocol *protocol,
                                 SP_access_map &sp_map, enum_sp_type type,
                                 String *func_caller_output = nullptr) {
  assert(assert_acl_cache_read_lock(thd));
  ...
  for (; it != sp_map.end(); ++it) {
    ...
    else if (type == enum_sp_type::FUNCTION)
      db.append(STRING_WITH_LEN("FUNCTION "));
    else if (type == enum_sp_type::TYPE)
      db.append(STRING_WITH_LEN("TYPE "));
    else if (type == enum_sp_type::PACKAGE_SPEC)
      db.append(STRING_WITH_LEN("PACKAGE "));
    else
      db.append(STRING_WITH_LEN("PACKAGE BODY "));
  ...
  */
  for (int grant_option = 0; grant_option < 2; ++grant_option) {
    ...
    if (found) {
      ...
      show_grants_output(global, protocol, func_caller_output, 10, false);
    }
    found = false;
  }  // end for
}
...
class Get_access_maps : public boost::default_bfs_visitor {
 public:
  Get_access_maps(ACL_USER *acl_user, Access_bitmask *access,
                  ...
                  SP_access_map *func_map, SP_access_map *type_map,
                  SP_access_map *package_map, SP_access_map *package_body_map,
      ...
      : m_access(access),
        ...
        m_package_body_map(package_body_map),
  ...
  void discover_vertex(Vertex u, const Graph &) const {
    ...
    *m_access |= implicit_cast<ACL_ACCESS *>(&acl_user)->access;
    ...
    /* Add type access */
    get_sp_access_map(&acl_user, m_type_map, ora_type_priv_hash.get(),
                      m_effective_grants);

    /* Add package access */
    get_sp_access_map(&acl_user, m_package_map, package_priv_hash.get(),
                      m_effective_grants);

    /* Add package body access */
    get_sp_access_map(&acl_user, m_package_body_map,
                      package_body_priv_hash.get(), m_effective_grants);
 ...
 private:
  ...
  SP_access_map *m_type_map;
  SP_access_map *m_package_map;
  SP_access_map *m_package_body_map;
...
bool check_single_table_access(THD *thd, Access_bitmask privilege,
  ...
  */
  ...
  if (check_grant(thd, privilege, all_tables, false, 1, no_errors))
    goto deny;
...
bool check_routine_access(THD *thd, Access_bitmask want_access, const char *db,
                          char *name, enum_sp_type sp_type, bool no_errors) {
  DBUG_TRACE;
  Table_ref tables[1];
  ...
  */
  ...
  DBUG_PRINT("info", ("Checking routine %s.%s for routine level access.", db,
                      tables->table_name));
  return check_grant_routine(thd, want_access, tables, sp_type, no_errors);
}
...
bool has_partial_view_routine_access(THD *thd, const char *db,
  ...
  */
  ...
  return !check_routine_level_acl(thd, db, routine_name, sp_type);
}
  ...
  */
  ...
  { /* Critical Section */
...
#ifdef CASCADE_REVOKE
      if (this_user &&
          check_and_insert_grantor(thd, this_user, Str,
                                   tables[ACL_TABLES::TABLE_USER].table,
                                   revoke_grant)) {
        result = true;
        break;
      }
    ...
    if (revoke_grant && !result &&
        (result = mysql_table_revoke_cascade(thd, table_list, tables, user_list,
                                             columns, rights & TABLE_ACLS)) &&
        !thd->is_error())
...
bool mysql_routine_grant(THD *thd, Table_ref *table_list, enum_sp_type sp_type,
  ...
  if (!revoke_grant) {
    if (sp_exist_routines(thd, table_list, sp_type)) return true;
  }
  ...
  { /* Critical section */
    ...
    while ((tmp_Str = str_list++)) {
      ...
      if (!grant_name) {
        ...
        if (sp_type == enum_sp_type::PROCEDURE)
          ...
          ora_type_priv_hash->emplace(
              grant_name->hash_key,
              unique_ptr_destroy_only<GRANT_NAME>(grant_name));
        else if (sp_type == enum_sp_type::PACKAGE_SPEC)
          package_priv_hash->emplace(
              grant_name->hash_key,
              unique_ptr_destroy_only<GRANT_NAME>(grant_name));
        else  // enum_sp_type::PACKAGE_BODY
          package_body_priv_hash->emplace(
              grant_name->hash_key,
              unique_ptr_destroy_only<GRANT_NAME>(grant_name));
      }
      ...
      if ((error = replace_routine_table(
               ...
               sp_type, rights, revoke_grant, all_current_privileges))) {
    ...
    */
    assert(!result || thd->is_error() ||
           ...
           thd->lex->sql_command == SQLCOM_CREATE_PACKAGE ||
           thd->lex->sql_command == SQLCOM_CREATE_PACKAGE_BODY ||
...
#ifdef CASCADE_REVOKE
      if (this_user &&
          check_and_insert_grantor(thd, this_user, user,
                                   tables[ACL_TABLES::TABLE_USER].table,
                                   revoke_grant)) {
        error = true;
        break;
...
#ifdef CASCADE_REVOKE
    if (revoke_grant && !is_proxy && !error &&
        (error = mysql_revoke_cascade(thd, tables, db, list, rights,
                                      dynamic_privilege,
                                      grant_all_current_privileges)) &&
        !thd->is_error())
      my_error(ER_REVOKE_GRANTS, MYF(0));
...
bool check_grant_db(THD *thd, const char *db,
  ...
  if (error) {
    ...
    error = check_grant_db_routine(thd, db, proc_priv_hash.get()) &&
            ...
            check_grant_db_routine(thd, db, package_priv_hash.get()) &&
            check_grant_db_routine(thd, db, package_body_priv_hash.get()) &&
            check_grant_db_routine(thd, db, ora_type_priv_hash.get());
  }
...
/****************************************************************************
  ...
  SYNPOSIS
   ...
   sp_type      Type of the routine (enum_sp_type::PROCEDURE...)
...
bool check_grant_routine(THD *thd, Access_bitmask want_access, Table_ref *procs,
                         enum_sp_type sp_type, bool no_errors) {
  Table_ref *table;
  Security_context *sctx = thd->security_context();
  ...
  for (table = procs; table; table = table->next_global) {
    if (has_roles) {
      ...
      if (sp_type == enum_sp_type::PROCEDURE) {
        ...
        acl = sctx->type_acl({table->db, table->db_length},
                             {table->table_name, table->table_name_length});
      } else if (sp_type == enum_sp_type::PACKAGE_SPEC) {
        acl = sctx->package_acl({table->db, table->db_length},
                                {table->table_name, table->table_name_length});
      } else {  // enum_sp_type::PACKAGE_BODY
        acl = sctx->package_body_acl(
      ...
      if ((grant_proc =
               routine_hash_search(host, sctx->ip().str, table->db, user,
                                   table->table_name, sp_type, false))) {
...
static bool check_routine_level_acl(THD *thd, const char *db, const char *name,
  ...
  if ((grant_proc =
           routine_hash_search(sctx->priv_host().str, sctx->ip().str, db,
                               sctx->priv_user().str, name, sp_type, false)))
...
void get_privilege_access_maps(
    ...
    SP_access_map *type_map, SP_access_map *package_map,
    SP_access_map *package_body_map, List_of_granted_roles *granted_roles,
  ...
  *access = acl_user->access;
  ...
  // get stored type privileges
  get_sp_access_map(acl_user, type_map, ora_type_priv_hash.get(),
                    effective_grants);
  // get stored package privileges
  get_sp_access_map(acl_user, package_map, package_priv_hash.get(),
                    effective_grants);
  // get stored package body privileges
  get_sp_access_map(acl_user, package_body_map, package_body_priv_hash.get(),
                    effective_grants);
  ...
  */
  ...
  const Get_access_maps vis(acl_user, access, db_map, db_wild_map, table_map,
                            sp_map, func_map, type_map, package_map,
                            package_body_map, with_admin_acl, dynamic_acl,
...
/**
  ...
  @param func_caller_output output for generic function caller, null if not
  specified
...
bool mysql_show_grants(THD *thd, LEX_USER *lex_user,
                       ...
                       bool effective_grants, String *func_caller_output) {
  int error = 0;
  ...
  */
  ...
  SP_access_map type_map;
  SP_access_map package_map;
  SP_access_map package_body_map;
  ...
  get_privilege_access_maps(acl_user, &using_roles, &access, &db_map,
                            ...
                            &type_map, &package_map, &package_body_map,
  ...
  show_grants_output(output, protocol, func_caller_output, 9, false);
  ...
  make_sp_privilege_statement(thd, acl_user, protocol, sp_map,
                              enum_sp_type::PROCEDURE, func_caller_output);
  make_sp_privilege_statement(thd, acl_user, protocol, func_map,
                              enum_sp_type::FUNCTION, func_caller_output);
  make_sp_privilege_statement(thd, acl_user, protocol, type_map,
                              enum_sp_type::TYPE, func_caller_output);
  make_sp_privilege_statement(thd, acl_user, protocol, package_map,
                              enum_sp_type::PACKAGE_SPEC, func_caller_output);
  make_sp_privilege_statement(thd, acl_user, protocol, package_body_map,
                              enum_sp_type::PACKAGE_BODY, func_caller_output);
  make_proxy_privilege_statement(thd, acl_user, protocol, func_caller_output);
  make_roles_privilege_statement(thd, acl_user, protocol, granted_roles,
                                 show_mandatory_roles, func_caller_output);
  make_with_admin_privilege_statement(thd, acl_user, protocol, with_admin_acl,
                                      granted_roles, func_caller_output);
...
#ifdef CASCADE_REVOKE
                                       ,
                                       ulong rights, bool revoke_all = true
#endif
) {
  ACL_DB *acl_db;
...
#ifdef CASCADE_REVOKE
      if (revoke_all || count == acl_dbs->size())
#endif
        ++acl_db;
    }
  } while (revoked
...
#ifdef CASCADE_REVOKE
        ulong cols_rights = grant_table->cols & ~rights;
#endif
...
static int remove_procedure_access_privileges(THD *thd, TABLE *procs_priv_table,
  ...
  // loop for all 4 procs hashes: func/proc/package/package_body.
  const enum_sp_type sp_type_arr[5] = {
      enum_sp_type::FUNCTION, enum_sp_type::PROCEDURE, enum_sp_type::TYPE,
      enum_sp_type::PACKAGE_SPEC, enum_sp_type::PACKAGE_BODY};
  for (int hash_index = 0; hash_index < 5; hash_index++) do {
      enum_sp_type sp_type = sp_type_arr[hash_index];
      malloc_unordered_multimap<std::string,
                                unique_ptr_destroy_only<GRANT_NAME>> *hash;

      if (sp_type == enum_sp_type::FUNCTION)
        hash = func_priv_hash.get();
      else if (sp_type == enum_sp_type::PROCEDURE)
        hash = proc_priv_hash.get();
      else if (sp_type == enum_sp_type::TYPE)
        hash = ora_type_priv_hash.get();
      else if (sp_type == enum_sp_type::PACKAGE_SPEC)
        hash = package_priv_hash.get();
      else
        hash = package_body_priv_hash.get();
      revoked = false;
      for (auto it = hash->begin(), next_it = it; it != hash->end();
        ...
        */
        ...
        if (!strcmp(lex_user.user.str, user) && !strcmp(lex_user.host.str, host)
#ifdef CASCADE_REVOKE
            && (!db || !strcmp(db, grant_proc->db))
#endif
        ) {
          const int ret =
              replace_routine_table(thd, grant_proc, procs_priv_table, lex_user,
                                    grant_proc->db, grant_proc->tname, sp_type,
#ifdef CASCADE_REVOKE
                                    rights,
#else
                                    ~(Access_bitmask)0,
  ...
  { /* Critical section */
    ...
    while ((tmp_lex_user = user_list++)) {
      ...
      if ((ret1 = remove_db_access_privileges(
               thd, tables[ACL_TABLES::TABLE_DB].table, *lex_user
#ifdef CASCADE_REVOKE
               ,
               ~(ulong)0
#endif
          ...
          (ret2 = remove_column_access_privileges(
               ...
               tables[ACL_TABLES::TABLE_COLUMNS_PRIV].table, *lex_user
#ifdef CASCADE_REVOKE
               ,
               nullptr, ~(ulong)0
#endif
...
#ifdef CASCADE_REVOKE
    result = mysql_revoke_all_cascade(thd, tables, list);
#endif
      ...
      */
      ...
      if (!my_strcasecmp(&my_charset_utf8mb3_bin, grant_proc->db, sp_db) &&
        ...
        const int ret = replace_routine_table(
            ...
            grant_proc->tname, sp_type, ~(Access_bitmask)0, true, true);
        if (ret < 0) {
          result = true;
          revoked = false;
          break;
...
/**
  ...
  @param      sp_db                DB of the stored procedure.
  @param      sp_name              Name of the stored procedure
  @param      sp_type              Type of the routine (enum_sp_type::PROC...)
...
bool sp_grant_privileges(THD *thd, const char *sp_db, const char *sp_name,
  ...
  */
  thd->push_internal_handler(&error_handler);
  result = mysql_routine_grant(thd, tables, sp_type, user_list,
...
#ifdef CASCADE_REVOKE
static bool mysql_revoke_table_priv(THD *thd, Table_ref *table_list,
                                    Table_ref *priv_tables,
                                    List<LEX_USER> &user_list,
                                    List<LEX_COLUMN> &columns, ulong rights) {
  ulong column_priv = 0;
  ...
  { /* Critical Section */
    while ((tmp_Str = str_list++)) {
      ...
      grant_table = table_hash_search(Str->host.str, NullS, db_name,
                                      Str->user.str, table_name, true);
      if (!grant_table) continue;

      /* If revoke_grant, calculate the new column privilege for tables_priv */
      class LEX_COLUMN *column;
      List_iterator<LEX_COLUMN> column_iter(columns);
      GRANT_COLUMN *grant_column;
      List<LEX_COLUMN> grant_columns;
      /* Fix old grants */
      while ((column = column_iter++)) {
        grant_column = column_hash_search(grant_table, column->column.ptr(),
                                          column->column.length());
        if (grant_column) {
          ...
          grant_columns.push_back(column);
        }
      }
      ...
      if ((error = replace_table_table(
               thd, grant_table, &deleted_grant_table,
               priv_tables[ACL_TABLES::TABLE_TABLES_PRIV].table, *Str, db_name,
               table_name, rights, column_priv, true))) {
        result = true;
        if (error < 0) break;
      ...
      if (priv_tables[3].table) {
        if ((error = replace_column_table(
                 thd, grant_table,
                 priv_tables[ACL_TABLES::TABLE_COLUMNS_PRIV].table, *Str,
                 grant_columns, db_name, table_name, rights, true))) {
          result = true;
          if (error < 0) break;
  ...
  DEBUG_SYNC(thd, "after_table_cascade_grant_revoke");
  return result;
}
...
static bool mysql_revoke_priv(THD *thd, Table_ref *priv_tables, const char *db,
                              List<LEX_USER> &list, ulong rights,
                              const List<LEX_CSTRING> &dynamic_privilege,
                              bool grant_all_current_privileges) {
  List_iterator<LEX_USER> str_list(list);
  LEX_USER *user, *target_user;
  ...
  { /* Critical section */
    ...
    dynpriv_table = priv_tables[ACL_TABLES::TABLE_DYNAMIC_PRIV].table;
    Update_dynamic_privilege_table update_table(thd, dynpriv_table);
    while ((target_user = str_list++)) {
      if (!(user = get_current_user(thd, target_user))) {
        error = true;
      ...
      if (db) {
        db_access =
            acl_get(thd, this_user->host.get_host(), this_user->host.get_host(),
                    (this_user->user ? this_user->user : ""), db, false);
      }
      if ((ret = replace_user_table(
               thd, priv_tables[ACL_TABLES::TABLE_USER].table, user,
               (!db ? rights : 0), true, false, what_to_set, &restrictions))) {
        error = true;
        if (ret < 0) break;
      ...
      /*
         DB table operation is needed in either of the following cases :
          ...
          - Hybrid operation; Partial revokes filtered some of the access
            for DB table
      */
      else if (db && db_access &&
               ((aggregator == nullptr) ||
                (aggregator && aggregator->find_if_require_next_level_operation(
                                   filtered_rights)))) {
        ulong db_rights = filtered_rights & DB_ACLS;
        if (db_rights == filtered_rights) {
          if ((ret = replace_db_table(thd,
                                      priv_tables[ACL_TABLES::TABLE_DB].table,
                                      db, *user, db_rights, true))) {
            error = true;
            if (ret < 0) break;
        ...
        if ((ret = remove_db_access_privileges(
                 thd, priv_tables[ACL_TABLES::TABLE_DB].table, *user, rights,
                 false))) {
          error = true;
          if (ret < 0) break;
      ...
      if (!db &&
        ...
        while ((priv = priv_it++) && !error) {
          /* We already checked privileges required to perform GRANT/REVOKE */
          revoke_dynamic_privilege(*priv, user->user, user->host, update_table);
        }
      }
    }  // for each user
  }
...
using Grantor_Map = std::unordered_multimap<std::string, ACL_USER *>;
static std::unique_ptr<Grantor_Map> cached_grantor_user_list() {
  std::unique_ptr<Grantor_Map> grantor_map(new Grantor_Map());
  for (ACL_USER *acl_user = acl_users->begin(); acl_user != acl_users->end();
       ++acl_user) {
    if (acl_user->grantor != nullptr)
      grantor_map->emplace(std::string(acl_user->grantor), acl_user);
  }
  return grantor_map;
}

static void get_grantee_user_list(THD *thd, List<LEX_USER> &grantor_list,
                                  List<LEX_USER> &grantee_list,
                                  Grantor_Map *grantor_map) {
  List_iterator<LEX_USER> str_list(grantor_list);
  LEX_USER *target_user, *user;
  char grantor[USER_HOST_BUFF_SIZE];
  while ((target_user = str_list++)) {
    strxmov(grantor, target_user->user.str, "@", target_user->host.str, NullS);
    auto it_range = grantor_map->equal_range(std::string(grantor));
    for (auto it = it_range.first; it != it_range.second; it++) {
      ACL_USER *this_user = it->second;
      LEX_STRING user_name = {this_user->user,
                              this_user->user ? strlen(this_user->user) : 0};
      LEX_STRING host_name = {const_cast<char *>(this_user->host.get_host()),
                              this_user->host.get_host_len()};
      user = LEX_USER::alloc(thd, &user_name, &host_name);
      grantee_list.push_back(user);
    }
  }
}

static void get_all_grantee_user_list(THD *thd, List<LEX_USER> &grantor_list,
                                      List<LEX_USER> &grantee_list) {
  std::unique_ptr<Grantor_Map> grantor_map = cached_grantor_user_list();
  List<LEX_USER> grantors = grantor_list;
  while (true) {
    List<LEX_USER> list;
    get_grantee_user_list(thd, grantors, list, grantor_map.get());
    if (list.is_empty())
      break;
    else {
      List_iterator<LEX_USER> str_list(list);
      LEX_USER *user;
      while ((user = str_list++)) grantee_list.push_back(user);
      grantors = list;
    }
  }
}

static int remove_all_column_and_producer_access_privileges(
    THD *thd, Table_ref *priv_tables, const char *db, List<LEX_USER> &list_user,
    ulong rights) {
  int ret, result = 0;
  List_iterator<LEX_USER> str_list(list_user);
  LEX_USER *user;
  while ((user = str_list++)) {
    if (rights & TABLE_ACLS) {
      if ((ret = remove_column_access_privileges(
               thd, priv_tables[ACL_TABLES::TABLE_TABLES_PRIV].table,
               priv_tables[ACL_TABLES::TABLE_COLUMNS_PRIV].table, *user, db,
               rights, false)) < 0) {
        result = true;
        break;
      } else if (ret)
    ...
    if (rights & PROC_ACLS) {
      if ((ret = remove_procedure_access_privileges(
               thd, priv_tables[ACL_TABLES::TABLE_PROCS_PRIV].table, *user, db,
               rights, false)) < 0) {
        result = true;
        break;
      } else if (ret)
...
static bool mysql_revoke_cascade(THD *thd, Table_ref *priv_tables,
  ...
  List<LEX_USER> grantee_list;
  /*get all grantee*/
  get_all_grantee_user_list(thd, grantor_list, grantee_list);
  if (grantee_list.is_empty()) return 0;
  /*revoke priv on user or db*/
  if (mysql_revoke_priv(thd, priv_tables, db, grantee_list, rights,
                        dynamic_privilege, grant_all_current_privileges))
    return true;

  if (remove_all_column_and_producer_access_privileges(thd, priv_tables, db,
                                                       grantee_list, rights))
    return true;
  return false;
}
...
static bool mysql_table_revoke_cascade(THD *thd, Table_ref *table_list,
  ...
  return mysql_revoke_table_priv(thd, table_list, priv_tables, grantee_list,
                                 columns, rights & TABLE_ACLS);
  return false;
}
...
static bool mysql_revoke_all_cascade(THD *thd, Table_ref *tables,
  ...
  int ret;
  bool result = false;
  LEX_USER *tmp_lex_user, *lex_user;
  List<LEX_USER> grantee_list;
  TABLE *dynpriv_table = tables[ACL_TABLES::TABLE_DYNAMIC_PRIV].table;
  /*get all grantee*/
  get_all_grantee_user_list(thd, user_list, grantee_list);
  List_iterator<LEX_USER> user_str(grantee_list);

  while ((tmp_lex_user = user_str++)) {
    ulong what_to_set = 0;
    if (!(lex_user = get_current_user(thd, tmp_lex_user))) {
      result = true;
      continue;
    ...
    int ret1, ret2, ret3;
    if ((ret1 = remove_db_access_privileges(
             thd, tables[ACL_TABLES::TABLE_DB].table, *lex_user, ~(ulong)0)) <
            0 ||
        (ret2 = remove_column_access_privileges(
             thd, tables[ACL_TABLES::TABLE_TABLES_PRIV].table,
             tables[ACL_TABLES::TABLE_COLUMNS_PRIV].table, *lex_user, nullptr,
             ~(ulong)0)) < 0 ||
        (ret3 = remove_procedure_access_privileges(
             thd, tables[ACL_TABLES::TABLE_PROCS_PRIV].table, *lex_user,
             nullptr, ~(ulong)0)) < 0) {
