  ...
  { /* Critical section */
    ...
    while ((tmp_Str = str_list++)) {
      ...
      if ((error = replace_routine_table(thd, grant_name, tables[4].table, *Str,
                                         db_name, table_name, sp_type, rights,
                                         revoke_grant))) {
...
void get_privilege_access_maps(
  ...
  */
  ...
  Get_access_maps vis(acl_user, access, db_map, db_wild_map, table_map, sp_map,
                      func_map, type_map, package_map, package_body_map,
                      with_admin_acl, dynamic_acl, &restrictions,
                      effective_grants);
      ...
      */
      ...
      if (!my_strcasecmp(&my_charset_utf8mb3_bin, grant_proc->db, sp_db) &&
        ...
        int ret = replace_routine_table(
            ...
            grant_proc->tname, sp_type, ~(ulong)0, true);
        if (ret < 0) {
          result = true;
          revoked = false;
          break;
