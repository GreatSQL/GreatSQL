...
sp_rcontext *Table_function_record::get_rcontext(sp_rcontext *local_ctx) const {
  return m_rcontext_handler ? m_rcontext_handler->get_rcontext(local_ctx)
                            : local_ctx;
}

bool Table_function_record::init() {
  if (m_forall_i)
    return init_udt_table();
  else
    return init_udt_type();
}

bool Table_function_record::init_udt_table() {
  Item_field_row_table *record =
      (Item_field_row_table *)get_rcontext(current_thd->sp_runtime_ctx)
          ->get_item(m_spv->offset);
  if (!record) {
    my_error(ER_UDT_REFFER, MYF(0), "table function init error");
    return true;
  }
  m_vt_list = record->get_row_table_field()->table_record->m_vt_list;
  Field_row_table *field = record->get_row_table_field();
  /*e.g: forall i in 3 .. 1 insert into table not need to fill table.
     but insert into t1 values table(0) needs to fill table.*/
  if (!m_spv_i || !m_spv_i->field_def.ora_record.is_forall_loop_var) {
    if (field->resolve_forall_table_record(current_thd, m_forall_i))
      return true;
  }
  return false;
}

bool Table_function_record::init_udt_type() {
  Item_field_row *record =
      (Item_field_row *)get_rcontext(current_thd->sp_runtime_ctx)
          ->get_item(m_spv->offset);
  if (!record) {
    my_error(ER_UDT_REFFER, MYF(0), "table function init error");
    return true;
  }
  if (record->field->m_udt_table->s->export_structure(current_thd,
                                                      &m_vt_list)) {
    my_error(ER_UDT_REFFER, MYF(0), "table function init error");
    return true;
  }
...
// copy data from table_from to this.e.g:insert into t2 values stu_record_val(i)
bool Table_function_record::fill_result_table() {
  const Sp_udt_stmt_arena_holder sp_arena_holder(
      current_thd, current_thd->sp_runtime_ctx->callers_arena);
  if (m_forall_i)
    return fill_result_table_with_udt_table();
  else
    return fill_result_table_with_udt_type();
}

// set table from table_from to this.e.g:insert into t2 values stu_record_val(i)
bool Table_function_record::fill_result_table_with_udt_table() {
  Item_field_row_table *record =
      (Item_field_row_table *)get_rcontext(current_thd->sp_runtime_ctx)
          ->get_item(m_spv->offset);
  Field_row_table *field = record->get_row_table_field();
  bool rc = false;
  if (field->forall_table_record && field->forall_table_record->table) {
    rc = fill_result_table_with_table(field->forall_table_record->table);
    field->forall_table_record->init_table();
  }
  return rc;
}

// copy data from table_from to this.e.g:insert into t2 values stu_record_val(i)
bool Table_function_record::fill_forall_table_with_udt_table(
    THD *thd, Table_function_record *table_from) {
  if (!m_forall_i->fixed) {
    Item *dummy = m_forall_i;
    if (m_forall_i->fix_fields(thd, &dummy)) return true;
  }
  // init_table();
  mem_root_deque<Item *> items(thd->mem_root);
  if (table_from->select_result_table_one_row(m_forall_i->this_item(), &items))
    return true;
  for (uint col = 0; col < table->s->fields; col++) {
    Item *item = items.at(col);
    if (sp_eval_expr(thd, true, get_field(col), &item)) return true;
  }
  if (write_row_and_cmp_index(items.at(0))) return true;
  return false;
}

// copy data from table_from to this.e.g:insert into t2 values stu_record_val
bool Table_function_record::fill_result_table_with_udt_type() {
  Item_field_row *record =
      (Item_field_row *)get_rcontext(current_thd->sp_runtime_ctx)
          ->get_item(m_spv->offset);
  init_table();
  for (uint col = 0; col < table->s->fields; col++) {
    Item *item = record->element_index(col);
    if (sp_eval_expr(current_thd, true, get_field(col), &item)) return true;
  }
...
bool Table_function_record::fill_result_table_with_table(TABLE *table_from) {
  ...
  init_table();
  bool rc = true;
  const Sp_udt_stmt_arena_holder sp_arena_holder(thd, m_rtx_arena);
  do {
    int res = table_from->file->ha_rnd_next(table_from->record[0]);
    if (res == 0) {
      Item *item_index = new (thd->mem_root) Item_field(table_from->field[0]);
      if (sp_eval_expr(thd, true, table->field[0], &item_index)) goto done;
      for (uint i = 1; i < table_from->s->fields; i++) {
        ...
        if (sp_eval_expr(thd, true, table->field[i], &item)) goto done;
      }
    ...
    if (item->type() == Item::ORACLE_ROWTYPE_ITEM &&
      ...
      for (uint i = 1; i < table->s->fields; i++) {
        if (check_count_and_fields(i - 1 + is_field_table) ||
            sp_eval_expr(thd, true, get_field(i), item->addr(i - 1 + is_field_table)))
          return true;
      }
      ...
      for (uint i = 0; i < udt->argument_count(); i++) {
        if (check_count_and_fields(i + 1) ||
            sp_eval_expr(thd, true, get_field(i + 1), udt->arguments() + i))
          return true;
      }
      ...
      item->val_str(&buf);
      Item *item_real = item->type() == Item::SUBQUERY_ITEM
                            ? dynamic_cast<Item_cache *>(item->element_index(0))
                                  ->get_example()
                            : item;
      for (uint i = 1; i < table->s->fields; i++) {
        Item *item_ele = item_real->element_index(i - 1);
        if (sp_eval_expr(thd, true, get_field(i), &item_ele)) return true;
      }
    } else if (check_count_and_fields(count + 1) ||
               sp_eval_expr(thd, true, get_field(count + 1), &item))
      return true;
    count++;
  }
  if (sp_eval_expr(thd, true, get_field(0), &item_str_index)) return true;
...
bool Table_function_record::update_result_table_one_col(Item **value,
  ...
  if (!field_udt) {
    if (sp_eval_expr(current_thd, true, get_field(field_idx), value)) goto error;
  } else {
  ...
  */
  ...
  if (sp_eval_expr(current_thd, true, get_field(0), &item_str_index)) goto error;
...
bool Table_function_record::update_result_table_all_cols(Item **value,
                                                         Item *item_index) {
  bool rc = true;
  const Sp_udt_stmt_arena_holder sp_arena_holder(current_thd, m_rtx_arena);
  Item *item_str_index = make_item_index(current_thd, item_index);
  if (!item_str_index || table_index_read_map(item_str_index, 1)) {
    my_error(ER_SP_NOT_EXIST_OF_RECORD_TABLE, MYF(0), table->alias);
    return true;
  ...
  if ((*value)->null_value) {
    ...
    for (uint i = 1; i < table->s->fields; i++) {
      ...
      if (sp_eval_expr(current_thd, true, get_field(i), &item_field)) goto error;
    }
  }
...
table_map Table_function_record::used_tables() const { return 0; }
...
bool Table_function_record::create_result_table(THD *thd, ulonglong options,
  ...
  table = create_tmp_table_from_fields(thd, *get_field_list(), false, options,
                                       table_alias, m_rtx_arena);
  if (!table) return true;
  // only if first column is ORA_TYPE_RECORD_INDEX_COLUMN_NAME should make
  // index.
  if (!my_strcasecmp(system_charset_info, table->field[0]->field_name,
                     ORA_TYPE_RECORD_INDEX_COLUMN_NAME) &&
      create_tmp_table_unique_index_info(thd, table, table_alias,
                                         get_field_list()))
    return true;
  return table == nullptr;
}
  ...
  do {
    ...
    if (res == 0) {
      ...
      for (uint i = 0; i < table->s->fields; i++) {
        ...
        if (sp_eval_expr(thd, true, get_field(i), &item)) goto done;
      }
...
table_map Table_function_udt::used_tables() const {
  return m_source->used_tables();
}
