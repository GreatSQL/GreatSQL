...
class sp_rcontext {
 ...
 private:
  sp_rcontext(const sp_pcontext *root_parsing_ctx, Field *return_value_fld,
              bool in_sub_stmt, const sp_pcontext *package_parent_ctx,
              Query_arena *arena);
 ...
 public:
  ...
  bool set_variable_row_field_by_name(THD *thd, uint var_idx,
                                      const Name_string &field_name,
                                      Item **value);
  bool set_variable_row_table_by_name(THD *thd, uint var_idx,
                                      const Name_string &field_name,
                                      List<ora_simple_ident_def> *arg_list,
                                      Item **value);
  Create_field *find_field_def_inner_variable(
      List<ora_simple_ident_def> arg_list);
  ...
  int get_var_items_count() { return m_var_items.size(); }
  ...
  TABLE *get_var_table() { return m_var_table; }
 ...
 private:
  ...
  List<sp_variable> m_vars_list;

  List<sp_variable> m_udt_vars_list;

  const sp_pcontext *m_package_parent_ctx;
...
class sp_cursor {
  ...
  */
  class Query_fetch_into_spvars : public Query_result_interceptor {
    ...
    /**
      - If m_var_ident_lists is empty: means fetch bulk into
   ...
   public:
    ...
    void set_spvar_idents(std::vector<Bipartite_name_ora *> idents) {
      m_ora_var_idents = idents;
    }
    bool is_variable(uint index) const {
      return !(unlikely(index < m_ora_var_idents.size()) &&
               m_ora_var_idents[index]);
    }
    ...
    List<ora_simple_ident_def> *get_full_ident_list(THD *thd,
 ...
 public:
  ...
  bool set_return_table_from_pcursor(THD *thd, sp_pcursor *from_cursor,
                                     sp_rcontext *ctx);
...
class sp_refcursor : public sp_cursor {
 public:
  ...
  bool open_for_ident(THD *thd, Item *item_query);
