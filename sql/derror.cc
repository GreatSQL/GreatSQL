...
const char *error_message_for_error_log(int mysql_errno) {
  if (errorlog_lc_messages && errorlog_lc_messages->errmsgs->is_loaded())
    return errorlog_lc_messages->errmsgs->lookup(mysql_errno);
...
bool init_errmessage() {
  ...
  // add for "error log support chinese", initialize the errmsgs
  (void)my_default_errorlog_lc_messages->errmsgs->read_texts();
  if (!my_default_errorlog_lc_messages->errmsgs->is_loaded())
    return true; /* Fatal error, not able to allocate memory. */
