...
class Item_func_gtid_sub final : public Item_str_ascii_func {
  String buf1, buf2;

 public:
  Item_func_gtid_sub(const POS &pos, Item *a, Item *b)
      : Item_str_ascii_func(pos, a, b) {}
  bool resolve_type(THD *) override;
  const char *func_name() const override { return "gtid_sub"; }
  String *val_str_ascii(String *) override;
};

class Item_func_gtid_count final : public Item_int_func {
  String buf1;

 public:
  Item_func_gtid_count(const POS &pos, Item *a) : Item_int_func(pos, a) {}
  longlong val_int() override;
  const char *func_name() const override { return "gtid_count"; }
  bool resolve_type(THD *thd) override {
    if (param_type_is_default(thd, 0, ~0U)) return true;
    return false;
  }
...
class Item_func_gtid_pos_in_binlog final : public Item_str_ascii_func {
  String file_pos;
  bool find_more_info;

 public:
  Item_func_gtid_pos_in_binlog(Item *a, Item *b) : Item_str_ascii_func(a, b) {}
  Item_func_gtid_pos_in_binlog(const POS &pos, Item *a, Item *b)
      : Item_str_ascii_func(pos, a, b) {}

  String *val_str_ascii(String *) override;
  bool resolve_type(THD *thd) override;
  const char *func_name() const override { return "gtid_position_in_binlog"; }

 private:
  static constexpr const char *const POSITION_NAME = "~position:";
  static constexpr const char *const TRANSACTION_LENGTH =
      "~transaction_length:";
};
