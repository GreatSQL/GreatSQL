...
bool Event_queue_element::compute_next_execution_time(THD *thd) {
  ...
  auto job_next_time = [&](my_time_t &next_exec_time) {
    ...
    auto interval_expr = string_to_selector_expr(thd, m_interval_expr_str.str,
...
int Event_timed::get_create_event(const THD *thd, String *buf) {
  ...
  DBUG_PRINT("ret_info", ("body_len=[%d]body=[%s]", (int)m_definition.length,
                          m_definition.str));
  if (m_expression) {
    if (m_interval_expr_str.length == 0) {
      if (Events::reconstruct_interval_expression(&expr_buf, m_interval,
                                                  m_expression))
        return EVEX_MICROSECOND_UNSUP;
    } else {
      expr_buf.append(m_interval_expr_str.str, m_interval_expr_str.length);
    }
  }
  ...
  // 'END;' <-- need remove to 'END'
  if (m_definition.length != 0 &&
      m_definition.str[m_definition.length - 1] == ';') {
    m_definition.str[m_definition.length - 1] = '\0';
    m_definition.length--;
  }
...
bool create_new_event_job(THD *thd, String *job_name, String *event_body,
  ...
  Item *interval_item =
      string_to_selector_expr(thd, interval_str->ptr(), interval_str->length());
...
bool change_event_job(THD *thd, String *job_name, String *event_body,
  ...
  if (interval_str != nullptr) {
    Item *interval_item = string_to_selector_expr(thd, interval_str->ptr(),
                                                  interval_str->length());
    if (interval_item == nullptr) {
