...
class sp_instr : public sp_printable {
 public:
  sp_instr(uint ip, sp_pcontext *ctx,
           const Sp_rcontext_handler *rcontext_handler = nullptr)
...
/** used for forall loop.
  e.g:
  forall i in first .. last
    INSERT INTO t1 VALUES stu_record_val(i);
  ==>
|  10 | set i@3 round(first,0)                                       |
|  11 | set alias_forto_173571954098307078@4 round(last,0)           |
|  12 | jump_if_not 16(16) (i@3 <= alias_forto_173571954098307078@4) |
|  13 | forall_fill_table stu_record_val@2                           |
|  14 | set i@3 (i@3 + 1)                                            |
|  15 | jump 12                                                      |
|  16 | stmt "INSERT INTO t1 VALUES stu_record_val(i)"               |
the step 13 is sp_instr_forall_fill_table.
              copy                       copy
table_record ------> forall_table_record -----> stu_record_val(i)'s table_record
             step 13                     step 16
*/
class sp_instr_forall_fill_table : public sp_instr {
  uint m_offset;
  Item *m_forall_i;

 public:
  sp_instr_forall_fill_table(uint ip, sp_pcontext *ctx, uint spv_offset,
                             const Sp_rcontext_handler *rh, Item *forall_i)
      : sp_instr(ip, ctx, rh), m_offset(spv_offset), m_forall_i(forall_i) {}
...
class sp_instr_set : public sp_lex_instr {
 public:
  sp_instr_set(uint ip, LEX *lex, uint offset, Item *value_item,
               LEX_CSTRING value_query, bool is_lex_owner, sp_pcontext *ctx,
               const Sp_rcontext_handler *rh)
      : sp_lex_instr(ip, ctx, lex, is_lex_owner, rh),
        m_offset(offset),
...
/*
  ...
  SET stu_record.id= 10;
*/
class sp_instr_set_row_field : public sp_instr_set {
  sp_instr_set_row_field(
      const sp_instr_set_row_field &);  // Prevent use of this
  void operator=(sp_instr_set_row_field &);
  uint m_field_offset;

 public:
  sp_instr_set_row_field(uint ip, LEX *lex, uint offset, Item *value_item,
...
class sp_instr_set_row_field_by_name : public sp_instr_set {
  // Prevent use of this
  sp_instr_set_row_field_by_name(const sp_instr_set_row_field_by_name &);
 ...
 public:
  sp_instr_set_row_field_by_name(uint ip, LEX *lex, uint offset,
...
/*
  This class sets values to a record table field:
  type stu_record is record(
    id    int := 1,
    name  varchar(20) ,
    score float := 0
  ...
  SET stu_record_arr_val(1).id= 10;
*/
class sp_instr_set_row_field_table_by_name
    : public sp_instr_set_row_field_by_name {
  sp_instr_set_row_field_table_by_name(
      const sp_instr_set_row_field_table_by_name &);  // Prevent use of this
  void operator=(sp_instr_set_row_field_table_by_name &);
  List<ora_simple_ident_def> *m_args_list;

 public:
  sp_instr_set_row_field_table_by_name(
      uint ip, LEX *lex, uint offset, Item *value_item, LEX_CSTRING value_query,
      bool is_lex_owner, sp_pcontext *ctx, const Sp_rcontext_handler *rh,
      List<ora_simple_ident_def> *args_list, const LEX_CSTRING &field_name)
...
/**
  sp_instr_preturn represents RETURN statement in stored functions.
*/
class sp_instr_preturn : public sp_instr {
 ...
 public:
  sp_instr_preturn(uint ip, LEX *lex)
      : sp_instr(ip, lex->get_sp_current_parsing_ctx()) {}
...
class sp_instr_jump : public sp_instr, public sp_branch_instr {
 public:
  ...
  uint get_destination() { return m_dest; }
...
class sp_instr_cpush : public sp_lex_instr {
 public:
  sp_instr_cpush(uint ip, sp_pcontext *ctx, LEX *cursor_lex,
                 ...
                 const Sp_rcontext_handler *rh = nullptr)
      : sp_lex_instr(ip, ctx, cursor_lex, true, rh),
...
class sp_instr_cpush_rowtype : public sp_instr_cpush {
 public:
  sp_instr_cpush_rowtype(uint ip, sp_pcontext *ctx, LEX *cursor_lex,
                         LEX_CSTRING cursor_query, int cursor_idx,
                         const Sp_rcontext_handler *rh)
      : sp_instr_cpush(ip, ctx, cursor_lex, cursor_query, cursor_idx, rh),
...
class sp_instr_cpop : public sp_instr {
 public:
  sp_instr_cpop(uint ip, sp_pcontext *ctx, uint count)
      : sp_instr(ip, ctx, nullptr), m_count(count) {}
...
class sp_instr_copen : public sp_instr {
 public:
  sp_instr_copen(uint ip, sp_pcontext *ctx, int cursor_idx,
                 const Sp_rcontext_handler *rh = nullptr)
      : sp_instr(ip, ctx, rh), m_cursor_idx(cursor_idx) {}
...
class sp_instr_copen_for : public sp_lex_instr {
 public:
  sp_instr_copen_for(uint ip, sp_pcontext *ctx, LEX *cursor_lex, int spv_idx,
                     const Sp_rcontext_handler *rh)
      : sp_lex_instr(ip, ctx, cursor_lex, true, rh), m_spv_idx(spv_idx) {}
...
class sp_instr_copen_for_sql : public sp_instr_copen_for {
 public:
  sp_instr_copen_for_sql(uint ip, sp_pcontext *ctx, LEX *cursor_lex,
                         ...
                         const Sp_rcontext_handler *rh)
      : sp_instr_copen_for(ip, ctx, cursor_lex, spv_idx, rh),
        m_cursor_query(cursor_query),
        m_valid(true) {}
...
class sp_instr_copen_for_ident : public sp_instr_copen_for {
 public:
  sp_instr_copen_for_ident(uint ip, sp_pcontext *ctx, LEX *lex,
                           ...
                           const Sp_rcontext_handler *rh,
                           sp_pcontext *pctx_ident,
                           const Sp_rcontext_handler *rh_ident)
      : sp_instr_copen_for(ip, ctx, lex, cursor_spv_idx, rh),
        m_sql_spv_idx(sql_spv_idx),
        m_pctx_ident(pctx_ident),
        m_rh_ident(rh_ident) {}
 ...
 private:
  int m_sql_spv_idx;
  sp_pcontext *m_pctx_ident;
  const Sp_rcontext_handler *m_rh_ident;
};
 ...
 public:
  sp_instr_cursor_copy_struct(uint ip, sp_pcontext *ctx, uint coffs, uint voffs,
      ...
      : sp_instr(ip, ctx, rh),
        ...
        m_ctx_cursor(ctx_cursor),
        m_rh_cursor(rh_cursor) {}
...
class sp_instr_cfetch : public sp_instr {
 public:
  ...
  void add_to_varlist(sp_variable *var, Bipartite_name_ora *name = nullptr) {
    m_ora_var_idents.push_back(name);
 ...
 protected:
  ...
  /**
    - If m_ora_var_idents[index] is null: fetch value into the sp variable
      directly
    - If m_ora_var_idents[index].medium is empty: no identifier between sp
    variable and field name, e.g., spv.field
    - If m_ora_var_idents[index].medium is not empty: there is at least one
    identifier between sp variable and field name, e.g., spv.ident.field,
  */
  std::vector<Bipartite_name_ora *> m_ora_var_idents;
...
class sp_instr_cfetch_bulk : public sp_instr_cfetch {
 public:
  sp_instr_cfetch_bulk(uint ip, sp_pcontext *ctx, int cursor_idx,
                       const Sp_rcontext_handler *rh)
      : sp_instr_cfetch(ip, ctx, cursor_idx, rh) {}
  ...
  bool execute(THD *thd, uint *nextp) override;
  void set_row_count(int i) { m_row_count = i; }
