...
class Window {
 ...
 protected:
   ...
   *------------------------------------------------------------------------*/
 public:
  Mem_root_array<Cached_item *> *get_partition_items() {
    return &m_partition_items;
  }
  /**
  ...
  */
  ...
  /**
    Get the ORDER BY list. Currently, only Item_func_group_concat uses it.
  */
  PT_order_list *get_order_list(void) { return m_order_by; }

  /**
    Set the ORDER BY list. Currently, only Item_func_group_concat uses it.
  */
  ...
  */
  void set_order_list_print(bool enabled) { m_order_by_print = enabled; }
 ...
 public:
  Window *pq_clone(THD *thd, Query_block *select);
};
