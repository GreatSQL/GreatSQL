...
static bool mysql_drop_function_internal(const LEX_STRING *name, TABLE *table);
  ...
  */
  ...
  if (udf_hash->count(to_string(udf->name)) != 0) {
    ...
    if (thd->lex->is_sp_create_or_replace()) {
      if (mysql_drop_function_internal(&udf->name, table)) {
        mysql_rwlock_unlock(&THR_LOCK_udf);
        return error;
      }
    } else {
...
/**
  This operation drops user defined function, and is now only used by
  mysql_create_function(). to support 'create or replace' UDFs.

  @param thd    Thread handler.
  @param udf_name   name of udf which is to be deleted.
  @param table  mysql.func table reference (opened and locked)

  Assumption

  - udf_name is not null.
  - table is already opened and locked
*/
static bool mysql_drop_function_internal(const LEX_STRING *udf_name,
                                         TABLE *table) {
  udf_func *udf;
  bool error = true;
