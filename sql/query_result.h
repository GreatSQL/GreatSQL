...
class Query_dumpvar final : public Query_result_interceptor {
  ha_rows row_count;
  enum enum_dumpvar_type { GLOBAL_VAR, LOCAL_VAR_NORMAL };
 ...
 private:
  void push_param_rela(enum_dumpvar_type var_type, THD *thd, Item *item,
                       std::string var_key);
  void init_query_dumpvar_map(THD *thd, const mem_root_deque<Item *> &list);
};
