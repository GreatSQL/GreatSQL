...
// MERGE_PS8404_CHECK_TAG: @linlj, func param changed
static bool extract_date_time(const Date_time_format *format, const char *val,
                              ...
                              enum_field_types data_type, my_time_flags_t flags,
    ...
    switch (*ptr) {
      ...
      case FMT_DAY: {
        if (l_time->day == 0)
          ...
          char_len = collation.collation->cset->numchars(
              collation.collation, tmpStr.ptr(),
      ...
      case FMT_RM:
        ...
        tmpStr.copy(name_ptr, name_len, collation.collation);
        char_len = collation.collation->cset->numchars(
            collation.collation, tmpStr.ptr(), tmpStr.ptr() + tmpStr.length());
...
String *Item_func_to_char::val_str(String *str) {
  ...
  if (is_date) {  // format datetime
    ...
    Truncated_value_error_handler handler(current_thd);
    // string or num
...
bool Item_func_to_date::val_datetime(MYSQL_TIME *ltime,
                                     my_time_flags_t fuzzy_date) {
...
void Item_func_trunc::print(const THD *thd, String *str,
                            enum_query_type query_type) const {
  str->append(func_name());
  str->append('(');
