...
inline AccessPath *NewLimitOffsetAccessPath(THD *thd, AccessPath *child,
  ...
  path->limit_offset().percent_value = percent_value;
  path->limit_offset().is_with_ties = is_with_ties;
  ...
  if (child->init_cost < 0.0) {
    ...
    if (percent_value > 0) {
      // double cost init
      const double fraction_full_read = std::min(
          1.0,
          ((double(offset) / child->num_output_rows() + percent_value) / 100));
