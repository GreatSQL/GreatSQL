    ...
    switch (path->type) {
      ...
      case AccessPath::HASH_JOIN: {
        ...
        if (param.is_foj) {
          foj_attach_it = NewIterator<HashJoinIterator>(
              ...
              /* MERGE_PS8404_CHECK_TAG: @fangnw,
                1. use above "first_input" is OK?
                2. move -> std::move?
              */
              join_predicate->expr->join_conditions, first_input,
              probe_input_batch_mode, hash_table_generation,
              /* m_foj_attach */ true);
          HashJoinIterator *it =
              down_cast<HashJoinIterator *>(iterator->real_iterator());
      ...
      case AccessPath::LIMIT_OFFSET: {
        ...
        iterator = NewIterator<LimitOffsetIterator>(
            ...
            send_records, param.percent_value, param.is_with_ties, join);
        break;
      }
