      ...
      if ((limit_cnt->percent_value > 0) || limit_cnt->is_with_ties) {
        if (limit_cnt->offset == 0) {
          if (limit_cnt->percent_value > 0) {
            snprintf(buf, sizeof(buf), "Fetch next %g percent row(s) %s",
                     limit_cnt->percent_value,
                     limit_cnt->is_with_ties ? "with ties" : "only");
          } else {
            snprintf(buf, sizeof(buf), "Fetch next %llu row(s) %s",
                     limit_cnt->limit,
                     limit_cnt->is_with_ties ? "with ties" : "only");
          }
        } else {
          if (limit_cnt->percent_value > 0) {
            snprintf(buf, sizeof(buf),
                     "Offset %llu rows Fetch next %g percent row(s) %s",
                     limit_cnt->offset, limit_cnt->percent_value,
                     limit_cnt->is_with_ties ? "with ties" : "only");
          } else {
            snprintf(buf, sizeof(buf),
                     "Offset %llu rows Fetch next %llu row(s) %s",
                     limit_cnt->offset, limit_cnt->limit,
                     limit_cnt->is_with_ties ? "with ties" : "");
          }
        }
      } else {
        if (limit_cnt->offset == 0) {
          snprintf(buf, sizeof(buf), "Limit: %llu row(s)", limit_cnt->limit);
        } else if (limit_cnt->limit == HA_POS_ERROR) {
          snprintf(buf, sizeof(buf), "Offset: %llu row(s)", limit_cnt->offset);
        } else {
          snprintf(buf, sizeof(buf), "Limit/Offset: %llu/%llu row(s)",
                   limit_cnt->limit - limit_cnt->offset, limit_cnt->offset);
        }
      }
      error |=
