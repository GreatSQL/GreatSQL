...
void init_sql_command_flags() {
  ...
  */
  ...
  sql_command_flags[SQLCOM_CREATE_PACKAGE] =
      CF_CHANGES_DATA | CF_AUTO_COMMIT_TRANS;
  sql_command_flags[SQLCOM_DROP_PACKAGE] =
      CF_CHANGES_DATA | CF_AUTO_COMMIT_TRANS;
  sql_command_flags[SQLCOM_CREATE_PACKAGE_BODY] =
      CF_CHANGES_DATA | CF_AUTO_COMMIT_TRANS;
  sql_command_flags[SQLCOM_DROP_PACKAGE_BODY] =
      CF_CHANGES_DATA | CF_AUTO_COMMIT_TRANS;
  ...
  sql_command_flags[SQLCOM_INSERT_SELECT] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED;
  sql_command_flags[SQLCOM_INSERT_ALL_SELECT] =
      CF_CHANGES_DATA | CF_REEXECUTION_FRAGILE | CF_CAN_GENERATE_ROW_EVENTS |
      CF_OPTIMIZER_TRACE | CF_CAN_BE_EXPLAINED;
  ...
  */
  ...
  sql_command_flags[SQLCOM_SHOW_STATUS_PACKAGE] =
      CF_STATUS_COMMAND | CF_REEXECUTION_FRAGILE | CF_HAS_RESULT_SET;
  sql_command_flags[SQLCOM_SHOW_STATUS_PACKAGE_BODY] =
      CF_STATUS_COMMAND | CF_REEXECUTION_FRAGILE | CF_HAS_RESULT_SET;
  sql_command_flags[SQLCOM_SHOW_STATUS_TYPE] =
      CF_STATUS_COMMAND | CF_REEXECUTION_FRAGILE | CF_HAS_RESULT_SET;
  ...
  sql_command_flags[SQLCOM_SHOW_CREATE_TYPE] = CF_STATUS_COMMAND;
  ...
  */
  ...
  sql_command_flags[SQLCOM_INSERT_ALL_SELECT] |= CF_PREOPEN_TMP_TABLES;
  ...
  sql_command_flags[SQLCOM_COMPOUND] |= CF_PREOPEN_TMP_TABLES;
  ...
  */
  ...
  sql_command_flags[SQLCOM_INSERT_ALL_SELECT] |= CF_ALLOW_PROTOCOL_PLUGIN;
  ...
  sql_command_flags[SQLCOM_COMPOUND] |= CF_ALLOW_PROTOCOL_PLUGIN;
  ...
  sql_command_flags[SQLCOM_RELOAD_SEQUENCES] |= CF_ALLOW_PROTOCOL_PLUGIN;
...
int mysql_execute_command(THD *thd, bool first_level) {
  ...
  */
  ...
  if (thd->is_classic_protocol()) {
    ...
    memset((void *)net->mtr_hint_msg, '\0', sizeof(net->mtr_hint_msg));
  }
#endif
  ...
  switch (lex->sql_command) {
    ...
    case SQLCOM_STOP_GROUP_REPLICATION: {
      ...
      */
      ...
      if (res == 50)  // GROUP_REPLICATION_STOP_WITH_SUPER_READ_ONLY_TIMEOUT
      {
        if (error_message == nullptr) {
          my_error(ER_GROUP_REPLICATION_COMMAND_FAILURE, MYF(0),
                   "STOP GROUP_REPLICATION", "Enable super_read_only failed.");
        } else {
          my_error(ER_GROUP_REPLICATION_COMMAND_FAILURE, MYF(0),
                   "STOP GROUP_REPLICATION", error_message);
          my_free(error_message);
    ...
    case SQLCOM_LOAD: {
      ...
      if (lex->m_with_func && lex->sql_command != SQLCOM_SELECT) {
        /*
           only allow the following syntax:
           with function ....select ....
        */
        my_error(
            ER_NOT_SUPPORTED_YET, MYF(0),
            "with function support format :with function ....select .... ");
        break;
      }
      res = lex->m_sql_cmd->execute(thd);
      break;
    ...
    */
      ...
      */
      ...
      if (first_table) {
        ...
        if (lex->type == TYPE_ENUM_PROCEDURE ||
            ...
            lex->type == TYPE_ENUM_PACKAGE_BODY ||
            lex->type == TYPE_ENUM_TYPE) {
          uint grants = lex->all_privileges
                            ? (PROC_OP_ACLS) | (lex->grant & GRANT_ACL)
                            : lex->grant;
          if (check_grant_routine(thd, grants | GRANT_ACL, all_tables,
                                  (enum_sp_type)lex->type, false))
            goto error;
          /* Conditionally writes to binlog */
          res = mysql_routine_grant(thd, all_tables, (enum_sp_type)lex->type,
      ...
      if (!(res = sp_create_routine(
        ...
        if (!sp_already_exists) {
          ...
          */
          ...
          if (sqlcom_to_sp_type(lex->sql_command, &sp_type)) {
            goto error;
          }

          if (sp_automatic_privileges && !opt_noacl &&
              check_routine_access(thd, DEFAULT_CREATE_PROC_ACLS,
                                   lex->sphead->m_db.str, name, sp_type,
                                   true)) {
            if (sp_grant_privileges(thd, lex->sphead->m_db.str, name, sp_type))
    ...
    case SQLCOM_ALTER_FUNCTION: {
      ...
      if (check_routine_access(thd, ALTER_PROC_ACL, lex->spname->m_db.str,
                               lex->spname->m_name.str, sp_type, false))
        goto error;
      /*
    ...
    case SQLCOM_DROP_FUNCTION: {
      ...
      else if (lex->sql_command == SQLCOM_DROP_FUNCTION)
        sp_type = enum_sp_type::FUNCTION;
      else if (lex->sql_command == SQLCOM_DROP_PACKAGE)
        sp_type = enum_sp_type::PACKAGE_SPEC;
      else if (lex->sql_command == SQLCOM_DROP_PACKAGE_BODY)
        sp_type = enum_sp_type::PACKAGE_BODY;
      else if (lex->sql_command == SQLCOM_DROP_TYPE)
        sp_type = enum_sp_type::TYPE;
      else
        goto error;

      if (check_routine_access(thd, ALTER_PROC_ACL, db, name, sp_type, false))
        goto error;
      ...
      */
      ...
      if (sp_result != SP_DOES_NOT_EXISTS && sp_automatic_privileges &&
          !opt_noacl && sp_revoke_privileges(thd, db, name, sp_type)) {
        ...
        case SP_DOES_NOT_EXISTS:
          if (lex->drop_if_exists) {
            ...
            push_warning_printf(thd, Sql_condition::SL_NOTE,
                                ER_SP_DOES_NOT_EXIST,
                                ER_THD(thd, ER_SP_DOES_NOT_EXIST),
                                sp_type_str(sp_type), lex->spname->m_qname.str);
            if (!res) my_ok(thd);
            break;
          }
          ...
          goto error;
        default:
          my_error(ER_SP_DROP_FAILED, MYF(0), sp_type_str(sp_type),
                   lex->spname->m_qname.str);
          goto error;
      }
      break;
...
void dispatch_sql_command(THD *thd, Parser_state *parser_state,
  ...
  DBUG_EXECUTE_IF("parser_debug", turn_parser_debug_on_my_sql_parser_parse(););
  DBUG_EXECUTE_IF("parser_debug", turn_parser_debug_on_ora_sql_parser_parse(););
  ...
  sp_cache_enforce_limit(thd->sp_ora_type_cache, stored_program_cache_size);
  thd->lex->destroy();
  thd->end_statement();
  thd->cleanup_after_query();
...
bool Alter_info::add_field(
    ...
    Qualified_column_ident *udt_name) {
...
bool string_to_expr(THD *thd, const char *expr_str, size_t length,
                    Parser_state *parser_state) {
...
Item *string_to_selector_expr(THD *thd, const char *expr_str, size_t length) {
  Expression_parser_state parser_state;
  return string_to_expr(thd, expr_str, length,
                        static_cast<Parser_state *>(&parser_state))
             ? nullptr
             : parser_state.result;
}

Item *string_to_selector_derived_expr(THD *thd, const char *expr_str,
                                      size_t length) {
