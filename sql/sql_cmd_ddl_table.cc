...
#include "sql/sql_delta.h"  // check view
  ...
  if (!query_block->field_list_is_empty())  // With select
  {
    ...
    */
    ...
    if (create_info.m_transactional_ddl &
        (!(create_info.used_fields & HA_CREATE_MATERIALIZED_VIEW))) {
    ...
    */
    ...
    if (create_info.used_fields & HA_CREATE_MATERIALIZED_VIEW) {
      res = greatdb::populate_materialized_view_and_update_table(thd, lex);
    } else
    ...
    if (create_info.options & HA_LEX_CREATE_TABLE_LIKE) {
      if (create_info.used_fields & HA_CREATE_MATERIALIZED_VIEW_LOG) {
        res = mysql_create_mlog_table(thd, create_table,
                                      query_expression_tables, &create_info);
      } else
        /* CREATE TABLE ... LIKE ... */
        res = mysql_create_like_table(thd, create_table,
                                      query_expression_tables, &create_info);
    } else if (create_info.options & HA_LEX_CREATE_TABLE_OF) {
      /* CREATE TABLE ... OF ... */
      res = mysql_create_of_table(thd, create_table, &create_info);
    } else {
