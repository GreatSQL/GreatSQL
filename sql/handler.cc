...
int ha_commit_trans(THD *thd, bool all, bool ignore_global_read_lock) {
  ...
  */
  ...
  if (DBUG_EVALUATE_IF("xa_commit_one_phase_error", true, false)) {
    error = 1;
  }
...
int ha_prepare_low(THD *thd, bool all) {
  ...
  DBUG_SIGNAL_WAIT_FOR(thd, "test_big_trx_during_clone_redo_copy",
                       "clone_trx_prepared", "clone_trx_commit_continue");
  return error;
}
...
void HA_CREATE_INFO::init_create_options_from_share(const TABLE_SHARE *share,
  ...
  if (m_mv_info == nullptr) {
    m_mv_info = share->mview_info;
  } else if (share->mview_info) {
    auto build = m_mv_info->build_clasue;
    m_mv_info = share->mview_info;
    m_mv_info->build_clasue = build;
  }
}
...
void print_key_contains_error(TABLE *table, KEY *key, const char *msg,
                              myf errflag) {
  /* Write the duplicated key in the error message */
  ...
  my_printf_error(ER_KEY_NOT_FOUND, msg, errflag, str.c_ptr(),
                  key_name.c_str());
}
...
void print_key_contains_error(TABLE *table, KEY *key, myf errflag) {
  print_key_contains_error(table, key, ER_THD(current_thd, ER_TRX_CONTAINS_KEY),
                           errflag);
}
/**
...
void handler::set_end_range(const key_range *range,
  ...
  */
  ...
  /*
    In MODE_EMPTYSTRING_EQUAL_NULL mode, string may contain '' did not reach the
    expected range, so need to compare_key
  */
  if (m_record_buffer != nullptr &&
      (current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
      range_key_part && is_string_type(range_key_part->field->type())) {
    m_record_buffer->set_out_of_range(true);
...
static int write_locked_table_maps(THD *thd) {
  ...
  if (DBUG_EVALUATE_IF("test_binlog_table_maps_not_clean", true, false)) {
    thd->set_binlog_table_maps(1);
  }
  if (thd->get_binlog_table_maps() == 0) {
    int table_maps = 0;
    for (MYSQL_LOCK *lock : {thd->extra_lock, thd->lock}) {
      if (DBUG_EVALUATE_IF("test_thd_lock_no", true, false)) {
        lock = nullptr;
      }
      ...
      for (TABLE **table_ptr = lock->table; table_ptr != end_ptr; ++table_ptr) {
        ...
        if (DBUG_EVALUATE_IF("test_table_wrlock_no", true, false)) {
          continue;
        }
    ...
    if (table_maps == 0) {
      if (DBUG_EVALUATE_IF("test_no_table_map_for_slave", true, false)) {
        return 0;
      }
      char err_buf[1024];
      auto len =
      ...
      err_buf[len] = 0;
      LogErr(ERROR_LEVEL, ER_LOG_PRINTF_MSG, err_buf);
      int i = 0;
      for (MYSQL_LOCK *lock : {thd->extra_lock, thd->lock}) {
