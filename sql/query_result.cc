...
bool Query_dumpvar::prepare(THD *thd, const mem_root_deque<Item *> &list,
                            Query_expression *u) {
  unit = u;

  if (var_list.elements == 1 && var_list.head()->get_my_var_sp()) {
    if (var_list.head()->var_is_rowtype()) {
      // SELECT INTO row_type_sp_variable
      if (var_list.head()
              ->get_rh(thd->sp_runtime_ctx)
              ->get_item(var_list.head()->get_offset())
      ...
      return false;
    } else if (var_list.head()->var_is_record_table_type()) {
      ...
      return false;
    } else if (dynamic_cast<PT_select_sp_var_field *>(
                   var_list.head())) {  // select into variable's field
      if (var_list.elements != CountVisibleFields(list)) {
        auto sel_sp_var_field = (PT_select_sp_var_field *)(var_list.head());
        auto ctx = sel_sp_var_field->get_rh(thd->sp_runtime_ctx);
        auto arg_list = sel_sp_var_field->get_full_ident_list();
        Create_field *def = nullptr;
        if (!(def = ctx->find_field_def_inner_variable(arg_list)) ||
            !def->ora_record.row_field_definitions() ||
            def->ora_record.row_field_definitions()->size() !=
                CountVisibleFields(list)) {
          my_error(ER_WRONG_NUMBER_OF_COLUMNS_IN_SELECT, MYF(0));
          return true;
        }
  ...
  // init query_dumpvar_map
  init_query_dumpvar_map(thd, list);

  return false;
}

void Query_dumpvar::init_query_dumpvar_map(THD *thd,
                                           const mem_root_deque<Item *> &list) {
  List_iterator_fast<PT_select_var> var_li(var_list);
  auto it = VisibleFields(list).begin();
  PT_select_var *mv;
  Item_null *null = new (thd->mem_root) Item_null();
  while ((mv = var_li++) && it != VisibleFields(list).end()) {
    if (mv->is_local()) {
      if (!mv->var_is_rowtype() && !mv->var_is_record_table_type()) {
        Field *field = mv->get_rh(thd->sp_runtime_ctx)
                           ->get_var_table()
                           ->field[mv->get_offset()];

        push_param_rela(Query_dumpvar::LOCAL_VAR_NORMAL, thd, null,
                        std::string(field->field_name));
      }
    } else {
      push_param_rela(Query_dumpvar::GLOBAL_VAR, thd, null,
                      std::string(mv->name.str));
    }
  }
}
...
void Query_dumpvar::push_param_rela(enum_dumpvar_type var_type, THD *thd,
                                    Item *item, std::string var_key) {
  if (!thd->sp_rcontext_layers.size()) return;

  Query_dumpvar_param_rela param;
  param.is_query_dumpvar_param_rela_null = (item->type() == Item::NULL_ITEM);
  if (!param.is_query_dumpvar_param_rela_null) {
    uint counter = 0;
    enum_resolution_type resolution;
    Item **res = nullptr;
    Query_block *block = unit->first_query_block();

    JOIN *join = block->join;
    LEX *lex = block->parent_lex;

    if (!block) return;

    param.is_multi_table =
        (lex->query_tables ? ((lex->query_tables->next_global) ? true : false)
                           : false);

    if (lex->query_tables &&
        (lex->query_tables->is_derived() || lex->query_tables->is_view())) {
      param.is_multi_table = true;
    }

    param.is_updating =
        (block->m_table_list.first
             ? ((block->m_table_list.first->updating) ? true : false)
             : false);

    if (!param.is_multi_table && param.is_updating) {
      Item *current_where =
          (block->where_cond()
               ? block->where_cond()
               : ((join && join->is_optimized() ? join->where_cond : nullptr)));
    ...
    find_item_in_list(thd, item, unit->first_query_block()->get_fields_list(),
                      &res, &counter, &resolution);
    if (nullptr == res) return;

    if ((*res)->type() == Item::FIELD_ITEM) {
      Item_field *origin_field = pointer_cast<Item_field *>(*res);

      param.field.is_lob = origin_field->is_blob_field();
      param.field.item_type = origin_field->type();
      param.field.table_name.assign(
          std::string(origin_field->table_ref->table_name));
      param.field.db_name.assign(std::string(origin_field->db_name));
      param.field.field_name.assign(std::string(origin_field->field_name));
      if (origin_field->fixed && param.field.is_lob &&
          origin_field->field->type() == MYSQL_TYPE_BLOB) {
        Field_blob *blob_field = down_cast<Field_blob *>(origin_field->field);
        size_t length = blob_field->get_length();
        if (blob_field->is_null() || !length) {
          param.origin_lob_content.assign("");
        } else {
          std::string lob_content((char *)blob_field->get_blob_data(), length);
          param.origin_lob_content.assign(lob_content);
        }
      }
    } else {
      param.field.is_lob = false;
      param.field.item_type = (*res)->type();
    }
  }

  switch (var_type) {
    case Query_dumpvar::LOCAL_VAR_NORMAL: {
      Sp_rcontext_layer::push_param(thd, var_key, param);
      break;
    }
    case Query_dumpvar::GLOBAL_VAR: {
      std::string var_name("@");
      Sp_rcontext_layer::push_param(thd, var_name.append(var_key), param);
      break;
    }
    default:
      break;
...
bool Query_dumpvar::send_data(THD *thd, const mem_root_deque<Item *> &items) {
  ...
  while ((mv = var_li++) && it != VisibleFields(items).end()) {
    Item *item = *it++;
    sp_rcontext *ctx = mv->get_rh(thd->sp_runtime_ctx);
    if (mv->is_local()) {
      if (mv->var_is_rowtype()) {
        // mv maybe %rowtype, udt%type, column%type
        Item *item_into = ctx->get_item(mv->get_offset());
        if (item->this_item()->is_ora_type() == item_into->is_ora_type()) {
          if (ctx->set_variable(thd, false, mv->get_offset(), &item))
            return true;
        } else if (item_into->this_item()->is_ora_type()) {
          if (ctx->set_variable_row(thd, mv->get_offset(), items)) return true;
        } else {
          my_error(ER_UDT_INCONS_DATATYPES, myf(0));
          return true;
        }
      } else if (mv->var_is_record_table_type()) {
        if (mv->var_is_bulk_collect()) {
          if (ctx->set_variable_row_table(thd, mv->get_offset(), items,
                                          row_count + 1, true))
            return true;
          row_count++;
        } else {
        ...
        auto sel_sp_var_field = dynamic_cast<PT_select_sp_var_field *>(mv);
        if (sel_sp_var_field) {  // select into variable's field
          assert(sel_sp_var_field->get_var_ident());
          Create_field *def = nullptr;
          if ((def = ctx->find_field_def_inner_variable(
                   sel_sp_var_field->get_full_ident_list())) &&
              def->ora_record.row_field_definitions() &&
              !ctx->search_udt_variable(
                  def->udt_name.str, def->udt_name.length, def->udt_db_name.str,
                  def->udt_db_name.length)) {  // variable's field is row type
            assert(def->ora_record.row_field_definitions()->size() ==
                   CountVisibleFields(items));
            auto fieldval_it = VisibleFields(items).begin();
            for (auto field_def =
                     def->ora_record.row_field_definitions()->begin();
                 field_def != def->ora_record.row_field_definitions()->end() &&
                 fieldval_it != VisibleFields(items).end();
                 ++field_def) {
              Item *fieldval_item = *fieldval_it++;
              Name_string ext_field_name(field_def->field_name,
                                         strlen(field_def->field_name));
              if (ctx->set_variable_row_table_by_name(
                      thd, sel_sp_var_field->get_offset(), ext_field_name,
                      sel_sp_var_field->get_full_ident_list_ptr(),
                      &fieldval_item))
                return true;
            }
          } else {  // variable's field isn't row type
            if (!sel_sp_var_field->get_var_ident()->medium) {
              uint field_idx;
              if (ctx->find_row_field_by_name_or_error(
                      &field_idx, sel_sp_var_field->get_offset(),
                      sel_sp_var_field->get_var_ident()->bipartite_name.name) ||
                  ctx->set_variable_row_field(
                      thd, sel_sp_var_field->get_offset(), field_idx, &item))
                return true;
            } else {
              if (ctx->set_variable_row_table_by_name(
                      thd, sel_sp_var_field->get_offset(),
                      sel_sp_var_field->get_var_ident()->bipartite_name.name,
                      sel_sp_var_field->get_var_ident()->medium, &item))
                return true;
            }
          }
        } else {
          ...
          Field *field = ctx->get_var_table()->field[mv->get_offset()];

          push_param_rela(Query_dumpvar::LOCAL_VAR_NORMAL, thd, item,
                          std::string(field->field_name));
        }
      }
    } else {
      ...
      */
      push_param_rela(Query_dumpvar::GLOBAL_VAR, thd, item,
                      std::string(mv->name.str));
    }
  }
...
bool Query_dumpvar::send_eof(THD *thd) {
  ...
  */
  ...
  if (var_list.head()->var_is_record_table_type() &&
    ...
    if (var_list.head()
            ->get_rh(thd->sp_runtime_ctx)
            ->fill_bulk_table_with_tmp_table(var_list.head()->get_offset()))
      return true;
  }
  ::my_ok(thd, row_count);
  return false;
...
bool Query_dumpvar::start_execution(THD *thd) {
  if (var_list.head()->var_is_record_table_type() &&
    ...
    var_list.head()
        ->get_rh(thd->sp_runtime_ctx)
        ->cleanup_record_variable_tmp_row_table(var_list.head()->get_offset());
  }
  return false;
}
