...
bool Query_dumpvar::prepare(THD *thd, const mem_root_deque<Item *> &list,
  ...
  if (var_list.elements == 1 && var_list.head()->get_my_var_sp()) {
    if (var_list.head()->var_is_rowtype()) {
      if (var_list.head()->var_is_record_type()) {
        my_error(ER_SP_MISMATCH_USE_OF_RECORD_VAR, MYF(0),
                 var_list.head()->name.str);
        return true;
      }
      ...
      if (var_list.head()->var_is_record_table_define_type()) {
        my_error(ER_SP_MISMATCH_USE_OF_RECORD_VAR, MYF(0),
                 var_list.head()->name.str);
        return true;
      }
  ...
  while ((mv = var_li++) && it != VisibleFields(list).end()) {
    if (mv->is_local()) {
      if (!mv->var_is_rowtype() && !mv->var_is_record_table_type()) {
        Field *field =
            thd->sp_runtime_ctx->get_var_table()->field[mv->get_offset()];
...
void Query_dumpvar::push_param_rela(enum_dumpvar_type var_type, THD *thd,
  ...
  if (!param.is_query_dumpvar_param_rela_null) {
    ...
    res = find_item_in_list(thd, item,
                            unit->first_query_block()->get_fields_list(),
                            &counter, REPORT_EXCEPT_NOT_FOUND, &resolution);
    if (not_found_item == res) return;
      ...
      if (mv->var_is_rowtype()) {
        ...
        if (item->this_item()->is_ora_type()) {
          if (thd->sp_runtime_ctx->set_variable(thd, mv->get_offset(), &item))
            return true;
        } else if (thd->sp_runtime_ctx->set_variable_row(thd, mv->get_offset(),
                                                         items))
          return true;
      } else if (mv->var_is_record_table_type()) {
        // This case select bulk collect into
        if (var_list.head()->var_is_record_table_define_type()) {
          my_error(ER_SP_MISMATCH_USE_OF_RECORD_VAR, MYF(0), mv->name.str);
          return true;
        }
