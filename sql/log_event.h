...
struct PRINT_EVENT_INFO {
  ...
  */
  ...
  /* follows are used for split update events */
  std::list<char *> last_table_map_base64;  // last table_map
  uint32 last_table_map_size;               // last table_map size
  uint split_events_when_print;             // count of event
  uint row_events_base64_max_size;          // enable split events locally
  uint row_events_base64_size_calc;  // calc size of events that has print
  bool need_print_end;

  void clear_last_table_map_base64() {
    auto it = last_table_map_base64.begin();
    for (; it != last_table_map_base64.end(); it++) {
      my_free(*it);
      it = last_table_map_base64.erase(it);
    }
    last_table_map_size = 0;
  }
  void print_last_table_map_base64(IO_CACHE *file) {
    auto it = last_table_map_base64.begin();
    for (; it != last_table_map_base64.end(); it++) {
      my_b_printf(file, "%s\n", *it);
    }
  ...
  */
  ...
  /* Indicate if the body cache has only unflushed rows query log event */
  bool only_unflushed_rows_query_log_event;
...
class Log_event {
  ...
  */
  ...
  /**
    Return true if the event is XA COMMIT/ROLLBACL/PREPARE.
  */
  virtual bool is_external_xid_end_event() const { return false; }
  ...
  */
  ...
  /*
    Get header and event length of simple events to be written to binlog cache.
    Complicated events need to override this function.
  */
  virtual size_t binlog_cache_write_len() {
...
class Query_log_event : public virtual mysql::binlog::event::Query_event,
 ...
 public: /* !!! Public in this patch to allow old usage */
  ...
  bool is_external_xid_end_event() const override {
    return !native_strncasecmp(query, STRING_WITH_LEN("XA COMMIT")) ||
           !native_strncasecmp(query, STRING_WITH_LEN("XA ROLLBACK"));
  }
...
class XA_prepare_log_event : public mysql::binlog::event::XA_prepare_event,
 ...
 public:
  ...
  bool is_external_xid_end_event() const override { return true; }
...
class Rows_log_event : public virtual mysql::binlog::event::Rows_event,
 ...
 public:
  ...
  int print_row_data_to_report_file(Relay_log_info const *rli, TABLE *table);
#if defined(MYSQL_SERVER)
  int pack_info(Protocol *protocol) override;
#endif
 ...
 private:
  // Be Used to write diagnostics info when occur HA_ERR_KEY_NOT_FOUND.
  // Only when use ROW_LOOKUP_HASH_SCAN and occur error
  // will write extra info to report_file.
  std::string report_file;
  void init_report_file(const Relay_log_info *rli);
  int print_hash_info_to_cache(IO_CACHE *cache, Relay_log_info const *rli,
                               TABLE *table);
#if defined(MYSQL_SERVER)
...
#ifdef MYSQL_SERVER
  int pack_info(Protocol *) override;
  bool write_zero(Basic_ostream *ostream, size_t event_data_len);
  bool write_with_data(Basic_ostream *ostream, size_t event_data_len) {
    return (write_header(ostream, event_data_len) ||
            write_zero(ostream, event_data_len));
  }
#endif
...
class Transaction_payload_log_event
 ...
 public:
  ...
  size_t get_data_size() override;
  size_t binlog_cache_write_len() override;
...
class Transaction_context_log_event
 ...
 public:
  ...
  my_thread_id get_thread_id() const {
    return static_cast<my_thread_id>(thread_id & 0x3FFFFFFF);
  }
  my_thread_id get_full_thread_id() const {
  ...
  bool is_empty_transaction() const { return gtid_empty_transaction; }
};
