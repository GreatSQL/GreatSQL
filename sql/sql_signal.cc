...
void Sql_cmd_common_signal::eval_defaults(THD *thd, Sql_condition *cond) {
  ...
  if (thd->variables.sql_mode & MODE_ORACLE) {
    if (is_sqlstate_warning(sqlstate)) {
      ...
      assign_defaults(
          thd, cond, set_defaults, Sql_condition::SL_WARNING,
          m_cond && m_cond->mysqlerr ? m_cond->mysqlerr : ER_SIGNAL_WARN);
    } else if (is_sqlstate_not_found(sqlstate)) {
      /* SQLSTATE class "02": not found. */
      assign_defaults(
          thd, cond, set_defaults, Sql_condition::SL_ERROR,
          m_cond && m_cond->mysqlerr ? m_cond->mysqlerr : ER_SIGNAL_NOT_FOUND);
    } else {
      /* other SQLSTATE classes : error. */
      assign_defaults(
          thd, cond, set_defaults, Sql_condition::SL_ERROR,
          m_cond && m_cond->mysqlerr ? m_cond->mysqlerr : ER_SIGNAL_EXCEPTION);
    }
  } else {
    if (is_sqlstate_warning(sqlstate)) {
      /* SQLSTATE class "01": warning. */
...
bool Sql_cmd_signal::execute(THD *thd) {
  ...
  */
  ...
  if (m_cond->m_is_user_defined) {
    assert(thd->variables.sql_mode & MODE_ORACLE);
    // cond.set_user_defined_exception(
    // const_cast<sp_condition_value*>(m_cond));
    raised = thd->raise_condition(cond.mysql_errno(), cond.returned_sqlstate(),
                                  cond.severity(), cond.message_text(), false,
                                  const_cast<sp_condition_value *>(m_cond));
  } else {
    raised = thd->raise_condition(cond.mysql_errno(), cond.returned_sqlstate(),
...
bool Sql_cmd_resignal::execute(THD *thd) {
  ...
  if (m_cond) {
    ...
    if (!eval_signal_informations(thd, &signaled_err)) {
      ...
      // signaled_err.set_user_defined_exception();

      if (thd->variables.sql_mode & MODE_ORACLE) {
        raised = thd->raise_condition(
            signaled_err.mysql_errno(), signaled_err.returned_sqlstate(),
            signaled_err.severity(), signaled_err.message_text(), false,
            frame->sql_condition->get_user_defined());
      } else {
        raised = thd->raise_condition(
            signaled_err.mysql_errno(), signaled_err.returned_sqlstate(),
