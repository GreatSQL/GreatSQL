...
Mts_submode_logical_clock::Mts_submode_logical_clock(int wait_mode) {
...
void Mts_submode_logical_clock::insert_waited_timestamp(
    longlong last_committed_arg) {
  if (m_wait_mode == REPLICA_WAIT_ON_WORKER_THREAD) {
    waited_timestamp_set.insert(last_committed_arg);
    assert(!waited_timestamp_set.empty());
    auto it = waited_timestamp_set.begin();
    min_waited_timestamp.store(*it);
  } else {
...
void Mts_submode_logical_clock::erase_waited_timestamp(
    longlong last_committed_arg) {
  if (m_wait_mode == REPLICA_WAIT_ON_WORKER_THREAD) {
    auto it = waited_timestamp_set.find(last_committed_arg);
    assert(it != waited_timestamp_set.end());
    waited_timestamp_set.erase(it);
    if (waited_timestamp_set.empty()) {
      min_waited_timestamp.store(SEQ_UNINIT);
    } else {
      it = waited_timestamp_set.begin();
      min_waited_timestamp.store(*it);
    }
  } else {
    min_waited_timestamp.store(SEQ_UNINIT);
...
bool Mts_submode_logical_clock::wait_for_last_committed_trx(
    THD *thd, Relay_log_info *rli, longlong last_committed_arg,
    bool need_wait) {
  DBUG_TRACE;
  ...
  if (need_wait) {
    insert_waited_timestamp(last_committed_arg);
  }
  /*
  ...
  */
  if ((!rli->info_thd->killed && !is_error) &&
    ...
    if (!need_wait) {
      rli->m_next_worker_wait_last_committed = last_committed_arg;
    ...
    erase_waited_timestamp(last_committed_arg);
    mysql_mutex_unlock(&rli->mts_gaq_LOCK);
    thd->EXIT_COND(&old_stage);
  ...
  if (!is_new_group) {
    ...
    if (!clock_leq(last_committed, lwm_estimate) &&
      ...
      */
      ...
      if (wait_for_last_committed_trx(rli->info_thd, rli, last_committed,
                                      need_wait)) {
        /*
