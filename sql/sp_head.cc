...
#include "sql/sp.h"
#include "sql/sp_cache.h"  // sp_cache_lookup
...
void init_sp_psi_keys() {
  ...
  mysql_statement_register(category, &sp_instr_forall_fill_table::psi_info, 1);
}
#endif
...
void sp_head::destroy(sp_head *sp) {
  ...
  if (sp_package *pkg = dynamic_cast<sp_package *>(sp)) {
    pkg->~sp_package();
  } else if (sp_ora_type *type = dynamic_cast<sp_ora_type *>(sp)) {
    type->~sp_ora_type();
  } else {
    sp->~sp_head();
  }
...
int64 sp_head::sp_cache_version() const {
  // for package, only package body is inserted into cache.
  // m_sp_cache_version of package body is set at that time.
  // for package routines, use package body's version instead.
...
sp_head::sp_head(MEM_ROOT &&mem_root, enum_sp_type type)
    : m_type(type),
      ...
      m_trg_list(nullptr),
      type(enum_udt_table_of_type::TYPE_INVALID),
      size_limit(0),
      nested_table_udt(NULL_STR),
      udt_table_db(NULL_STR),
...
void sp_head::set_default_items(THD *thd) {
  auto default_list = thd->item_list();

  // default_list is not null, means default parameters is created
  if (default_list != nullptr && thd->variables.sql_mode & MODE_ORACLE) {
    m_default_arena = new (thd->mem_root)
        Query_arena(thd->mem_root, Query_arena::STMT_INITIALIZED_FOR_SP);
    m_default_arena->set_item_list(thd->item_list());
    thd->reset_item_list();
  }
}

void sp_head::set_body_start(THD *thd, const char *begin_ptr) {
  set_default_items(thd);
...
sp_head::~sp_head() {
  ...
  if (m_default_arena != nullptr) {
    m_default_arena->free_items();
    ::destroy_at(m_default_arena);
    m_default_arena = nullptr;
  }
...
Field *sp_head::create_result_field(THD *thd, size_t field_max_length,
  ...
  if (m_return_field_def.ora_record.table_rowtype_ref()) {
    List<Create_field> defs;
    if (m_return_field_def.ora_record.table_rowtype_ref()
            ->resolve_table_rowtype_ref(thd, defs))
      return nullptr;
    if (m_return_field_def.ora_record.row_field_definitions()) {
      // return table%rowtype
      if (m_return_field_def.ora_record.row_field_definitions()->size() > 0)
        m_return_field_def.ora_record.row_field_definitions()->clear();
      if (m_return_field_def.ora_record.row_field_definitions()->make_from_defs(
              thd->mem_root, &defs, true))
        return nullptr;
    } else if (m_return_field_def.ora_record.row_field_table_definitions()) {
      // return table of table%rowtype
      Row_definition_table_list *list =
          m_return_field_def.ora_record.row_field_table_definitions();
      Row_definition_list *row_old = list->find_row_fields_by_offset(0);
      row_old->clear();
      row_old->make_from_defs(thd->mem_root, &defs, true);

      Row_definition_list *row_new = nullptr;
      if (row_old->make_new_create_field_to_store_index(
              thd,
              m_return_field_def.ora_record.index_length > 0
                  ? std::to_string(m_return_field_def.ora_record.index_length)
                        .c_str()
                  : nullptr,
              m_return_field_def.ora_record.is_index_by, &row_new))
        return nullptr;
      row_old->clear();
      List_iterator_fast<Create_field> it(*row_new);
      Create_field *def = nullptr;
      for (uint i = 0; (def = it++); i++) {
        def->offset = i;
        if (row_old->append_uniq(thd->mem_root, def)) return nullptr;
      }
    }
  }
  Field *field =
...
bool sp_head::execute(THD *thd, bool merge_da_on_success) {
  ...
  */
  ...
  m_flags |= IS_INVOKED;
  if (m_parent) m_parent->m_invoked_subroutine_count++;
  ...
  */
  if (m_creation_ctx) saved_creation_ctx = m_creation_ctx->set_n_backup(thd);
  ...
  */
  ...
  if (m_creation_ctx) m_creation_ctx->restore_env(thd, saved_creation_ctx);
...
bool sp_head::execute_trigger(THD *thd, const LEX_CSTRING &db_name,
  ...
  */
  ...
  sp_rcontext *trigger_runtime_ctx =
      sp_rcontext::create(thd, m_root_parsing_ctx, nullptr, nullptr, nullptr);
...
bool sp_head::execute_function(THD *thd, Item **argp, uint argcount,
  ...
  */
  ...
  if (m_parent && m_parent->instantiate_if_needed(thd)) return true;

  const sp_pcontext *package_head_pct = nullptr;
  if (m_parent && m_parent->m_package_head)
    // package body's sp get package head's pctx
    package_head_pct =
        m_parent->m_package_head->get_root_parsing_context()->child_context(0);
  ...
  */
  ...
  sp_rcontext *func_runtime_ctx = sp_rcontext::create(
      thd, m_root_parsing_ctx, return_value_fld, package_head_pct,
      m_parent ? &m_parent->m_arena : nullptr);
  if (!func_runtime_ctx) {
    thd->swap_query_arena(backup_arena, &call_arena);
    err_status = true;
    goto err_with_cleanup;
...
bool sp_head::execute_procedure(THD *thd, mem_root_deque<Item *> *args) {
  ...
  */
  ...
  const sp_pcontext *package_head_pct = nullptr;
  if (pkg && pkg->m_package_head) {
    // package body get package head's pctx
    package_head_pct =
        pkg->m_package_head->get_root_parsing_context()->child_context(0);
  } else if (m_parent && m_parent->m_package_head)
    // package body's sp get package head's pctx
    package_head_pct =
        m_parent->m_package_head->get_root_parsing_context()->child_context(0);
  ...
  if (!parent_sp_runtime_ctx) {
    ...
    parent_sp_runtime_ctx = sp_rcontext::create(
        thd, m_root_parsing_ctx, nullptr, package_head_pct, nullptr);
  ...
  if (!pkg) {
    proc_runtime_ctx = sp_rcontext::create(thd, m_root_parsing_ctx, nullptr,
                                           package_head_pct, nullptr);

    if (!proc_runtime_ctx) {
      thd->sp_runtime_ctx = sp_runtime_ctx_saved;

      if (sp_runtime_ctx_saved) ::destroy_at(parent_sp_runtime_ctx);

      return true;
    }
  } else {
    if (pkg->m_rcontext) {
      proc_runtime_ctx = pkg->m_rcontext;
    } else {
      /*
        proc_runtime_ctx of a package should be kept until it is dropped
        from sp_cache. otherwise
       */
      saved_mem_root = thd->mem_root;
      saved_item_list = thd->item_list();
      thd->mem_root = pkg->get_persistent_mem_root();
      thd->reset_item_list();

      thd->swap_query_arena(pkg->m_arena, &backup_arena);

      proc_runtime_ctx = sp_rcontext::create(thd, m_root_parsing_ctx, nullptr,
                                             package_head_pct, &pkg->m_arena);

      thd->swap_query_arena(backup_arena, &pkg->m_arena);
      ...
      pkg->m_rcontext = proc_runtime_ctx;
    }
  }
  ...
  if (params > 0) {
    ...
    for (uint i = 0; i < params; ++i, ++it_args) {
      ...
      spvar->ref_actual_parameter = arg_item;
  ...
  */
  ...
  if (!pkg) ::destroy_at(proc_runtime_ctx);
  thd->sp_runtime_ctx = sp_runtime_ctx_saved;
...
bool sp_head::check_show_access(THD *thd, bool *full_access) {
  ...
  *full_access = has_full_view_routine_access(thd, m_db.str, m_definer_user.str,
  ...
  return *full_access ? false
                      : !has_partial_view_routine_access(thd, m_db.str,
                                                         m_name.str, m_type);
}
...
bool sp_head::set_security_ctx(THD *thd, Security_context **save_ctx) {
  ...
  */
  ...
  if (*save_ctx && !m_fake_synonym &&
      check_routine_access(thd, EXECUTE_ACL, m_db.str, m_name.str, m_type,
                           false)) {
    m_security_ctx.restore_security_context(thd, *save_ctx);
    *save_ctx = nullptr;
    return true;
...
bool sp_head::check_for_type_table() {
  if (nested_table_udt.str &&
      my_strcasecmp(table_alias_charset, m_db.str, udt_table_db.str)) {
    my_error(ER_WRONG_UDT_DATA_TYPE, MYF(0), m_db.str, nested_table_udt.str,
  ...
  ulong reclength = 0;
  sql_mode_t sql_mode;
  LEX_CSTRING nested_table_udt_tmp = NULL_CSTR;
  uint table_type = 255;
  ulonglong varray_limit = 0;
  bool ret = sp_find_ora_type_create_fields(
      current_thd, udt_table_db, nested_table_udt, false, &field_def_list,
      &reclength, &sql_mode, &nested_table_udt_tmp, &table_type, &varray_limit);
  if (ret || !reclength) {
    my_error(ER_SP_DOES_NOT_EXIST, MYF(0), "TYPE", nested_table_udt.str);
    return true;
  }
  if (table_type == 0 || table_type == 1) {
    my_error(ER_NOT_SUPPORTED_YET, MYF(0),
             "create type as table object of udt table object");
    return true;
  }
  return false;
}
...
bool sp_package::validate_after_parser(THD *thd) {
  if (m_type != enum_sp_type::PACKAGE_BODY) return false;
  sp_head *sp =
      sp_cache_lookup(&thd->sp_package_spec_cache, m_qname.str, m_qname.length);
  sp_package *spec = sp ? sp->get_package() : NULL;
  assert(spec);  // CREATE PACKAGE must already be cached
  return validate_public_routines(thd, spec) || validate_private_routines(thd);
}
  ...
  for (LEX *lex; (lex = it++);) {
    ...
    for (LEX *lex2; (lex2 = it2++);) {
      ...
      if (sp_eq_routine_name(lex2->sphead->m_name, lex->sphead->m_name) &&
        ...
        found = true;
        Create_field cf = lex->sphead->m_return_field_def;
        Create_field cf2 = lex2->sphead->m_return_field_def;
        if (lex2->sphead->m_type == enum_sp_type::FUNCTION &&
            lex->sphead->m_type == enum_sp_type::FUNCTION &&
            (cf.sql_type != cf2.sql_type ||
             cf.ora_record.is_ref_cursor != cf2.ora_record.is_ref_cursor ||
             (cf.udt_name.length && cf2.udt_name.length &&
              my_strcasecmp(system_charset_info, cf.udt_name.str,
                            cf2.udt_name.str)))) {
          my_error(ER_NOT_SUPPORTED_YET, MYF(0),
                   "different return type for package function");
          return true;
        }
        /*
...
bool sp_package::instantiate_if_needed(THD *thd) {
  mem_root_deque<Item *> args(thd->mem_root);
  if (m_is_instantiated) return false;
  /* package initialization may call routines in the same package.
     Set m_is_instantiated to true early to avoid recursion in such scenario.
  */
  ...
  do {
    ...
    */
    ...
    // 1.run package head to set args
    if (m_package_head &&
        m_package_head->get_root_parsing_context()->child_context(0)) {
      if (m_package_head->execute_procedure(thd, &args)) {
        thd->server_status &= ~bits_to_be_cleared;
        break;
      }
    }

    // 2.run package body to set args
    // args.elements= 0;
    bool result = execute_procedure(thd, &args);
...
sp_variable *sp_package::find_package_variable(const char *name,
  ...
  */
  sp_pcontext *ctx = m_root_parsing_ctx->child_context(0);
  return ctx ? ctx->find_variable(name, name_len, true) : NULL;
}

bool sp_package::find_package_cursor(LEX_STRING name, uint *poff) const {
  /*
    sp_head::m_pcont is a special level for routine parameters.
    Variables declared inside CREATE PACKAGE BODY reside in m_children.at(0).
  */
  sp_pcontext *ctx = m_root_parsing_ctx->child_context(0);
  return ctx ? ctx->find_cursor(name, poff, true) : false;
}
...
sp_package::~sp_package() {
  ...
  m_body = NULL_CSTR;
  if (m_current_routine) m_current_routine->sphead = nullptr;
  m_arena.free_items();
  if (m_rcontext) ::destroy_at(m_rcontext);
  m_is_instantiated = false;
  m_rcontext = nullptr;
}

void sp_package::savepoint(MDL_savepoint &savepoint) {
  // only PACKAGE_BODY got the chance to call lock_routine_name() indirectly.
  if (m_type != enum_sp_type::PACKAGE_BODY) return;
  mdl_savepoint = savepoint;
  m_savepoint_set = true;
}

void sp_package::rollback(THD *thd) {
  if (!m_savepoint_set) return;
  thd->mdl_context.rollback_to_savepoint(mdl_savepoint);
  m_savepoint_set = false;
}

sp_variable *sp_package::find_package_head_variable(LEX_CSTRING udt) const {
  sp_pcontext *pctx = nullptr;
  sp_variable *spvar = nullptr;
  if (m_type == enum_sp_type::PACKAGE_SPEC &&
      m_root_parsing_ctx->child_context(0)) {
    // package head find from package head
    pctx = m_root_parsing_ctx->child_context(0);
  } else if (m_type == enum_sp_type::PACKAGE_BODY) {
    // package body find from cache package head
    if (!m_package_head ||
        !m_package_head->get_root_parsing_context()->child_context(0))
      return nullptr;
    pctx = m_package_head->get_root_parsing_context()->child_context(0);
  }
  if (!pctx) return nullptr;
  spvar = pctx->find_variable(udt.str, udt.length, true);

  return spvar;
}

bool sp_package::find_package_head_cursor(LEX_STRING name, uint *poff) const {
  sp_pcontext *pctx = nullptr;
  if (m_type == enum_sp_type::PACKAGE_SPEC &&
      m_root_parsing_ctx->child_context(0)) {
    // package head find from package head
  ...
  return pctx->find_cursor(name, poff, true);
}

///////////////////////////////////////////////////////////////////////////
// sp_ora_type implementation.
///////////////////////////////////////////////////////////////////////////
sp_ora_type::~sp_ora_type() {
  LEX *lex;
   ...
   * need to figure out whether nullptr for m_root_parsing_ctx here is
   * reasonable
   */
  if (m_root_parsing_ctx) ::destroy_at(m_root_parsing_ctx);
  /*
    If we have non-empty LEX stack then we just came out of parser with
    error. Now we should delete all auxiliary LEXes and restore original
    THD::lex. It is safe to not update LEX::ptr because further query
