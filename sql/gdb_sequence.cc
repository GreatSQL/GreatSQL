/* Copyright (c) 2023, 2026, GreatDB Software Co., Ltd. All rights
   reserved.
...
bool evict_cached_sequences(const std::vector<std::string> *seq_keys,
                            bool need_lock) {
  if (need_lock) {
    mysql_rwlock_rdlock(&THR_LOCK_sequences);
  }

  const sequences_cache_map &seq_cache = *global_sequences_cache();
  if (seq_keys && seq_keys->size() > 0) {
    for (const auto &key : *seq_keys) {
      auto seq = seq_cache.find(key);
      if (seq != seq_cache.end()) {
        std::lock_guard<std::mutex> _lock(seq->second->m_mutex);
        seq->second->evict_cached_numbers();
      } else {
        std::stringstream ss_seq_not_found;
        ss_seq_not_found << "Sequence [" << key << "] not found, skipped.";
        ss_seq_not_found.flush();
        push_warning(thd_get_current_thd(), Sql_condition::SL_WARNING,
                     ER_GDB_READ_SEQUENCE, ss_seq_not_found.str().c_str());
        continue;
      }
    }
  } else {
    for (auto &seq : seq_cache) {
      std::lock_guard<std::mutex> _lock(seq.second->m_mutex);
      seq.second->evict_cached_numbers();
    }
  }
...
class Gdb_seq_persist_handler {
 public:
  Gdb_seq_persist_handler() = delete;
...
class Gdb_seq_persist_front_end : public Gdb_seq_persist_handler {
 public:
  Gdb_seq_persist_front_end() = delete;
...
#define POOL_MAX_CONN_CNT 30
class Gdb_seq_persist_pooled_front_end : public Gdb_seq_persist_handler {
 public:
  Gdb_seq_persist_pooled_front_end() = delete;
  Gdb_seq_persist_pooled_front_end(Gdb_sequence_entity *seq)
      : Gdb_seq_persist_handler(seq) {}
  ~Gdb_seq_persist_pooled_front_end() override;
  bool create_persist_record(const my_decimal &cur) override;
  bool delete_persist_record() override;
  bool delete_persist_record_without_commit() override {
    assert(0);
  ...
  bool release_rec_lock(bool must_commit) override;
  bool close() override;
  static void destroy_all_conns();
  static void destroy_redundant_conns(int reserves);

  static std::queue<std::unique_ptr<Gdb_cmd_service>> g_conns_pool;
  static int g_conns_cnt;
 ...
 private:
  std::unique_ptr<Gdb_cmd_service> m_h_cmd_service;
};

/* implement a simple connection pool using following three global objects:
       g_conns_pool, g_conns_cnt, g_conns_mutex  */
std::queue<std::unique_ptr<Gdb_cmd_service>>
    Gdb_seq_persist_pooled_front_end::g_conns_pool;
int Gdb_seq_persist_pooled_front_end::g_conns_cnt = 0;
std::mutex Gdb_seq_persist_pooled_front_end::g_conns_mutex;
bool Gdb_seq_persist_pooled_front_end::get_conn() {
  assert(m_h_cmd_service == nullptr);

  for (int i = 0; i < 2; i++) {
    {  // auto release mutex
      std::lock_guard<std::mutex> lock(g_conns_mutex);
      if (!g_conns_pool.empty()) {
        m_h_cmd_service = std::move(g_conns_pool.front());
        g_conns_pool.pop();
        return false;
      }
    }
    // sleep 2ms try again (one time allocate seq cache needs 4 RTTs by sql)
    if (i == 0 && g_conns_cnt >= POOL_MAX_CONN_CNT) {
      THD *thd = thd_get_current_thd();
      uint us_cnt = 1680;
      uint us_rand = (thd->thread_id() % 64) * 10;
      us_cnt += us_rand;  // Pseudo rand 1.68ms ~ 2.31ms
      usleep(us_cnt);
    } else
      break;
  }

  // create new cmd_service
  m_h_cmd_service = std::make_unique<Gdb_cmd_service>();
  if (!m_h_cmd_service) {
    m_errno = -1;
    m_errmsg = "create cmd_service handler fail!";
    return true;
  }
  if (m_h_cmd_service->open_session()) {
    m_errno = -1;
    m_errmsg = "open cmd_service session fail!";
    m_h_cmd_service.reset(nullptr);
    return true;
  }
  if (m_h_cmd_service->execute_sql("set session autocommit=1")) {
    auto &cb_data = m_h_cmd_service->get_cb_data();
...
void Gdb_seq_persist_pooled_front_end::release_conn() {
  if (m_h_cmd_service == nullptr) return;
  std::lock_guard<std::mutex> lock(g_conns_mutex);
  if (g_conns_cnt < POOL_MAX_CONN_CNT) {
    g_conns_pool.push(std::move(m_h_cmd_service));
    assert(m_h_cmd_service == nullptr);
  } else {
    m_h_cmd_service.reset(nullptr);  // close session inner
    g_conns_cnt--;
  }
}
void Gdb_seq_persist_pooled_front_end::destroy_conn() {
  if (m_h_cmd_service == nullptr) return;
  m_h_cmd_service.reset(nullptr);  // close session inner
  std::lock_guard<std::mutex> lock(g_conns_mutex);
  g_conns_cnt--;
}
void Gdb_seq_persist_pooled_front_end::destroy_all_conns() {
  std::lock_guard<std::mutex> lock(g_conns_mutex);
  assert(g_conns_cnt == (int)g_conns_pool.size());
  g_conns_pool = {};  // auto de-construct && close cmd_service inner
  g_conns_cnt = 0;
}

void Gdb_seq_persist_pooled_front_end::destroy_redundant_conns(int reserves) {
  std::lock_guard<std::mutex> lock(g_conns_mutex);
  if (reserves >= POOL_MAX_CONN_CNT || g_conns_cnt <= reserves) return;
  int destroy_cnt = g_conns_cnt - reserves;
  while (destroy_cnt) {
    g_conns_pool.pop();
    g_conns_cnt--;
    destroy_cnt--;
...
bool Gdb_sequence_entity::setup_persist_handler() {
  ...
  m_persist_handler = new Gdb_seq_persist_pooled_front_end(this);  // use pooled
...
bool Gdb_sequence_entity::read_val(bool read_next, my_decimal &val) {
  ...
  if (read_next && (read_only || super_read_only)) {
    val = m_cur;
...
void sequences_close() {
  ...
  mysql_rwlock_unlock(&THR_LOCK_sequences);
  Gdb_seq_persist_pooled_front_end::destroy_all_conns();
}
...
bool Sql_cmd_drop_sequence::execute(THD *thd) {
  ...
  // add for pooled_front_end
  Gdb_seq_persist_pooled_front_end::destroy_redundant_conns(
      (int)sequences_cache->size());
...
Sql_cmd_reload_sequences::Sql_cmd_reload_sequences(
    const Mem_root_array<Table_ident *> *seqs) {
  THD *thd = thd_get_current_thd();

  if (seqs != nullptr && seqs->size() > 0) {
    for (Table_ident *ident : *seqs) {
      LEX_STRING db = to_lex_string(ident->db);
      if (db.str == nullptr || db.length == 0 || strlen(db.str) == 0) {
        thd->copy_db_to(&db.str, &db.length);
      }
      this->m_seq_keys.push_back(calc_seq_key(db.str, ident->table.str));
    }
  }
}

bool Sql_cmd_reload_sequences::execute(THD *thd) {
  Disable_binlog_guard binlog_guard(thd);

  bool ret = evict_cached_sequences(&m_seq_keys);
  if (!ret) {
    my_ok(thd, 0);
  }
  std::vector<std::string>().swap(m_seq_keys);
  return ret;
}

/* return value:
     0(NO_SEQ_DEF)                is not a sequence
    >0                            is metadata_version of a sequence
    uint64_max(FIND_SEQ_FAIL)     error (lock time out) */
uint64_t has_sequence_def(THD *thd, const char *db, const char *seq_name) {
  ...
  uint64_t res = NO_SEQ_DEF;
  auto mdl_savepoint = thd->mdl_context.mdl_savepoint();
  if (lock_sequence(thd, db, seq_name, false)) {
    return FIND_SEQ_FAIL;  // lock failed
  }
  ...
  /* release MDL if not sequence name */
  if (!res) {
...
/* NOTE: seq_name mustn't be a table name.
     Mostly, this func is called after fix_field
  return value:
     nullptr    error
     ptr        ptr->Gdb_sequence_entity */
std::shared_ptr<Gdb_sequence_entity> find_sequence_def(THD *thd, const char *db,
  ...
  auto res = get_sequence_def(db, seq_name);
  /* release MDL if not sequence name */
  if (!res) {
    thd->mdl_context.rollback_to_savepoint(mdl_savepoint);
  }
...
Gdb_seq_persist_pooled_front_end::~Gdb_seq_persist_pooled_front_end() {
  release_conn();
}

// will destory or release conn inner
bool Gdb_seq_persist_pooled_front_end::create_persist_record(
    const my_decimal &cur) {
  bool ret = false;
  auto grd = create_scope_guard([ret, this]() {
    if (!ret)
      this->release_conn();
    else
      this->destroy_conn();
  ...
  auto sql = build_insert_sql(m_seq, cur);
  if (get_conn() || m_h_cmd_service->execute_sql(sql)) {
    auto &cb_data = m_h_cmd_service->get_cb_data();
    m_errno = cb_data.error_no();
    m_errmsg = cb_data.error_msg();
    return true;
...
// will destory or release conn inner
bool Gdb_seq_persist_pooled_front_end::delete_persist_record() {
  bool ret = false;
  auto grd = create_scope_guard([ret, this]() {
    if (!ret)
      release_conn();
...
// will destory or release if failure
bool Gdb_seq_persist_pooled_front_end::read_persist_record(bool lock,
                                                           my_decimal &val) {
  auto sql = build_query_sql(m_seq, lock);
  if (get_conn()) {
    return true;
  ...
  auto ret = ((lock && m_h_cmd_service->execute_sql(
                           STRING_WITH_LEN("START TRANSACTION"))) ||
              m_h_cmd_service->execute_sql(sql));
  auto &cb_data = m_h_cmd_service->get_cb_data();
  if (ret) {
    m_errno = cb_data.error_no();
    m_errmsg = cb_data.error_msg();
  ...
  if (cb_data.rows() == 0) {
    ...
    m_errmsg += m_seq->db() + "." + m_seq->name() + "] not found!";
    if (lock) {
      if (m_h_cmd_service->execute_sql(STRING_WITH_LEN("ROLLBACK")))
        goto err_exit;
    }
  } else {
    std::string val_str = cb_data.get_value(0, 0);
    ret = str2my_decimal(E_DEC_FATAL_ERROR, val_str.c_str(), val_str.size(),
                         &my_charset_bin, &val);
    if (ret) {
      m_errno = -2;
      m_errmsg = "read sequence decimal column error for sequence [";
  ...
  // keep conn only if lock==true and succed.
  if (ret || !lock) release_conn();
  return ret;
...
// will destory or release conn inner
bool Gdb_seq_persist_pooled_front_end::update_persist_record(
    const my_decimal &currval) {
  assert(m_h_cmd_service != nullptr);
  auto sql = build_update_sql(m_seq, currval);
  auto ret = m_h_cmd_service->execute_sql(sql);
  if (ret) {
    auto &cb_data = m_h_cmd_service->get_cb_data();
    m_errno = cb_data.error_no();
    m_errmsg = cb_data.error_msg();
  ...
  ret = release_rec_lock(true /*must commit*/);  // will destory or release conn
  return ret;
}

// will destory or release conn inner
bool Gdb_seq_persist_pooled_front_end::release_rec_lock(bool must_commit) {
  assert(m_h_cmd_service != nullptr);
  bool ret = false;
  if (must_commit) {
    ret = m_h_cmd_service->execute_sql("COMMIT");
    if (ret) {
      destroy_conn();
      return true;
    }
  } else {
    ret = m_h_cmd_service->execute_sql("ROLLBACK");
    if (ret) {
      destroy_conn();
      return false;  // release rec by close conn
    }
  }
  release_conn();
  return false;
}
bool Gdb_seq_persist_pooled_front_end::close() {
  release_conn();
  return false;
}
