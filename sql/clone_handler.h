...
enum clone_data_recovery_mode_t {
  CLONE_DATA_RECOVERY_GTID = 0,
  CLONE_DATA_RECOVERY_BINLOG_POSITION = 1,
};
...
class Clone_handler {
 public:
  ...
  struct XA_Block {
    ...
    @param[in]	thd	session thread */
    explicit XA_Block(THD *thd, long unsigned recovery_mode);
...
void set_clone_data_recovery_mode(ulong mode);
long unsigned get_clone_data_recovery_mode();
