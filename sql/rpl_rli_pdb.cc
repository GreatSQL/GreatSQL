    ...
    */
    ...
    if (unlikely(error)) {
      ...
      if (mts_submode->min_waited_timestamp != SEQ_UNINIT)
        mysql_cond_broadcast(&c_rli->logical_clock_cond);
...
int Slave_worker::slave_worker_exec_event(Log_event *ev) {
  ...
  if (!is_mts_db_partitioned(rli) && may_have_timestamp(ev) &&
      ...
      rli->current_mts_wait_mode != REPLICA_WAIT_ON_WORKER_THREAD) {
...
bool Slave_worker::retry_referenced_transaction(
    my_off_t start_relay_pos, const char *start_event_relay_log_name,
    my_off_t end_relay_pos, const char *end_event_relay_log_name) {
  THD *thd = info_thd;
...
bool Slave_worker::retry_transaction(my_off_t start_relay_pos,
  ...
  do {
    ...
    DBUG_EXECUTE_IF("error_on_xa_blocked_timeout", {
      char const act[] =
          "now SIGNAL xa_blocked_timeout_retry_start WAIT_FOR "
          "xa_blocked_timeout_retry_continue";
...
int slave_worker_exec_job_group(Slave_worker *worker, Relay_log_info *rli) {
  ...
  while (true) {
    ...
    */
    ...
    if (worker->m_next_worker_wait_last_committed != SEQ_UNINIT) {
      Mts_submode_logical_clock *rli_submode =
          static_cast<Mts_submode_logical_clock *>(rli->current_mts_submode);
      if (rli_submode->wait_for_last_committed_trx(
              thd, rli, worker->m_next_worker_wait_last_committed, true)) {
        error = true;
        goto err;
      }
      worker->m_next_worker_wait_last_committed = SEQ_UNINIT;
    }
