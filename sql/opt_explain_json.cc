...
bool table_base_ctx::format_body(Opt_trace_context *json,
  ...
  return format_gather(json) || format_derived(json) ||
         format_query_expression(json);
}
...
class join_tab_ctx : public joinable_ctx, public table_with_where_and_derived {
 ...
 public:
  ...
  int add_gather(join_ctx *ctx,
                 Query_expression *subquery MY_ATTRIBUTE((unused))) override {
    return gather.push_back(ctx);
  }
...
class join_ctx : public unit_ctx, virtual public qep_row {
 ...
 public:
  ...
  int add_where_subquery(subquery_ctx *ctx,
                         Query_expression *subquery) override;
  int add_gather(join_ctx *ctx, Query_expression *subquery) override;
  size_t join_tabs_count() override { return join_tabs.elements; }
  sort_ctx *get_sort() override { return sort; }
  window_ctx *get_window() override { return window; }
};
    ...
    while ((w = li++)) {
      ...
      if (w->needs_sorting()) {
        ...
        for (; ord != nullptr; ord = ord->next) {
          ...
          if (ord->nulls_pos == NULLS_POS_FIRST)
            str.append(STRING_WITH_LEN(" nulls first"));
          else if (ord->nulls_pos == NULLS_POS_LAST)
            str.append(STRING_WITH_LEN(" nulls last"));
