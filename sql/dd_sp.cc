...
static Field *make_field(const dd::Parameter &param, TABLE_SHARE *share,
                         ...
                         Table_ident *table_rowtype_ref = nullptr) {
  // Decimals
...
static bool prepare_type_string_from_dd_param(THD *thd,
  ...
  if (param_options.exists("udt_name")) {
    ...
    if (!param_options.exists("table_rowtype")) {
      ...
      is_rowtype_ref = true;
      table_ident = new (thd->mem_root)
          Table_ident(to_lex_cstring(udt_db_name), udt_name);
    }
  }
  ...
  unique_ptr_destroy_only<Field> field(
      make_field(*param, table.s, geom_type, interval,
                 ...
                 is_row_table, &udt_name, udt_db_name, table_ident));
...
inline void append_param_mode(dd::Stringstream_type *s,
                              dd::Parameter::enum_parameter_mode mode,
                              sql_mode_t sql_mode) {
  switch (mode) {
  ...
  *params_str = "";
  ...
  for (const dd::Parameter *param : routine->parameters()) {
    ...
    */
    ...
    // PARAMETER MODE while in MySQL mode
    if (routine->type() == dd::Routine::RT_PROCEDURE &&
        !(routine->sql_mode() & MODE_ORACLE)) {
      append_param_mode(&params_ss, param->mode(), routine->sql_mode());
    }
    ...
    */
    ...
    if ((routine->sql_mode() & MODE_ORACLE) &&
        param->options().exists("default_value")) {
      dd::String_type default_value;
      param->options().get("default_value", &default_value);
      params_ss << " DEFAULT ";
      params_ss << default_value;
    }
  }
