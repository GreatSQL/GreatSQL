...
Field *create_tmp_field(THD *thd, TABLE *table, Item *item, Item::Type type,
  ...
  if (type == Item::ORACLE_ROWTYPE_ITEM ||
    ...
    if (item->is_splocal()) {
      Item_splocal *splocal = down_cast<Item_splocal *>(item);
      item->this_item()->item_name.copy(
          item->item_name.is_set() ? item->item_name : splocal->m_name);
      item = item->this_item();
    }
  }
  ...
  switch (type) {
    ...
    case Item::TRIGGER_FIELD_ITEM: {
      ...
      CREATE or replace PROCEDURE p1() as
        ...
        rec1_row t2%ROWTYPE;
      begin
        select * from (SELECT rec.id,rec.name1) t1 inner join (SELECT
        rec1_row.id,rec1_row.id,rec1_row.name1) t2;
      end;
      for rec.id,rec.name1,the field's table doesn't have storage_handler,so it
      should run create_tmp_field_from_item().
      */
      if (!item_field->field->table->has_storage_handler()) {
        if (make_copy_field) {
        ...
        /*
          If item have to be able to store NULLs but underlaid field can't do
          it, create_tmp_field_from_field() can't be used for tmp field
          creation.
        */
        ...
        /*
          Fields that are used as arguments to the DEFAULT() function already
          have their data pointers set to the default value during name
          resolution. See Item_default_value::fix_fields.
        */
...
TABLE *create_tmp_table(THD *thd, Temp_table_param *param,
  ...
  */
  ...
  uint *udt_field = own_root.ArrayAlloc<uint>(field_count + 2);
  ...
  table->init_tmp_table(thd, share, &own_root, param->table_charset,
                        table_alias, reg_field, blob_field, false, udt_field);
    ...
    if (store_column && is_sum_func && group == nullptr &&
      ...
      for (uint i = 0; i < sum_item->argument_count(); i++) {
        ...
        if (!arg->const_item()) {
          ...
          if (new_field->udt_name().str) {
            *udt_field++ = new_field->field_index();
            share->udt_fields++;
          }
  ...
  */
  ...
  if (except_connect_by || except_connect_by_value) {
...
TABLE *create_duplicate_weedout_tmp_table(THD *thd, uint uniq_tuple_length_arg,
  ...
  if (!multi_alloc_root(
          ...
          sizeof(Field *) * (1 + 2), &blob_field, sizeof(uint) * 3, &udt_field,
          sizeof(uint) * 3, &keyinfo, sizeof(*keyinfo), &key_part_info,
          sizeof(*key_part_info) * 2, &group_buff,
  ...
  table->init_tmp_table(thd, share, &own_root, nullptr, "weedout-tmp",
                        reg_field, blob_field, false, udt_field);
  uint reclength = 0;
  uint null_count = 0;
...
TABLE *create_tmp_table_from_fields(THD *thd, List<Create_field> &field_list,
  ...
  */
  ...
  if (!multi_alloc_root(m_root, &table, sizeof(*table), &share, sizeof(*share),
                        ...
                        &udt_field, (field_count + 1) * sizeof(uint), &bitmaps,
  ...
  table->init_tmp_table(thd, share, m_root, nullptr, alias, reg_field,
                        blob_field, is_virtual, udt_field);
  ...
  while ((cdef = it++)) {
    *reg_field =
    ...
    (*reg_field)->set_rowtype_table_ident(cdef->ora_record.table_rowtype_ref());
    if (callers_arena && (*reg_field)->result_type() == ROW_RESULT)
      (*reg_field)->m_callers_arena = callers_arena;
    reg_field++;
  }
  *reg_field = nullptr;              /* mark the end of the list */
  share->blob_field[blob_count] = 0; /* mark the end of the list */
  share->blob_fields = blob_count;
  share->udt_field[udt_count] = 0; /* mark the end of the list */
  share->udt_fields = udt_count;
...
/*used for CTE to sync all tmp table's handler.
CTE's proccess:
1.create table A,clone table B => table->file=temptable_hton
2.create_tmp_table_with_fallback table A->file
  => A->file->create return HA_ERR_RECORD_FILE_FULL,so change to innodb_hton
    but table B doesn't do create_tmp_table_with_fallback,
    so tableA->file=innodb,tableB->file=temptable
3.open_tmp_table tableA,tableB => tableB error
it should change table B's handler here.
*/
...
static bool create_tmp_table_with_fallback(THD *thd, TABLE *table) {
  ...
  */
  ...
  if (error == HA_ERR_RECORD_FILE_FULL &&
    ...
    if (wtable_list && wtable_list->common_table_expr() &&
        resolve_cte_common_table_expr(table, wtable_list))
      return true;
    else {
      table->file = get_new_handler(
          table->s, false, share->alloc_for_tmp_file_handler, innodb_hton);
