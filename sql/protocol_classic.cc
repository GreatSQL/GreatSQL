...
#ifdef WITH_MTR_HINT
    // write mtr-hint
    if (m_thd->net.mtr_hint) {
      // write mtr-hint message string length
      size_t msg_size = strlen(m_thd->net.mtr_hint_msg) + 1;
      pos = net_store_length((uchar *)&tmp, msg_size);
      my_net_write(&m_thd->net, (uchar *)&tmp, (size_t)(pos - (uchar *)&tmp));
      // write mtr-hint message string
      my_net_write(&m_thd->net, (uchar *)m_thd->net.mtr_hint_msg, msg_size);
    } else {
      pos =
...
bool Protocol_classic::store_string(const char *from, size_t length,
  ...
  if ((current_thd->variables.sql_mode & MODE_ORACLE) && !send_metadata &&
...
bool Protocol_text::store_decimal(const my_decimal *d, uint prec, uint dec) {
  ...
  */
  if (current_thd->variables.sql_mode & MODE_ORACLE) {
    int intg = decimal_intg(d);
    int fdec = decimal_actual_fraction(d);
    string_length = d->sign() + (intg ? intg : 1) + fdec + (fdec ? 1 : 0);
  }
