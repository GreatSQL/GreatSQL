  ...
  if (!m_key_id.empty()) {
    ...
    if (error_and_key.first != Rpl_encryption::Keyring_status::SUCCESS) {
      ...
      if (my_aes_decrypt(m_encrypted_password.data(),
                         ...
                         error_and_key.second.length(), MY_DEFAULT_CBC_256_MODE,
...
bool Rpl_encryption_header_v1::encrypt_file_password(Key_string password_str) {
  ...
  /* Encrypt password */
  if (!error) {
    error = (my_aes_encrypt(
                 password_str.data(), password_str.length(), encrypted_password,
                 master_key.m_value.data(), master_key.m_value.length(),
                 MY_DEFAULT_CBC_256_MODE, iv, false) == MY_AES_BAD_DATA);
