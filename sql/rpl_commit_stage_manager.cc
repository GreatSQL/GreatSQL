...
void Commit_stage_manager::init(PSI_mutex_key key_LOCK_flush_queue,
  ...
  mysql_cond_init(key_COND_flush_queue, &m_stage_cond_large_trx_wait);
...
void Commit_stage_manager::deinit() {
  ...
  mysql_cond_destroy(&m_stage_cond_large_trx_wait);
...
bool Commit_stage_manager::append_to(StageID stage, THD *thd) {
  ...
  /*
    Make sure the large transaction committing by rotation
    is the leader of the binlog flush queue.
  */
  if (stage == Commit_stage_manager::BINLOG_FLUSH_STAGE &&
      binlog_should_commit_by_rotate(thd)) {
    thd->binlog_should_commit_by_rotate = true;
    if (!m_queue[stage].is_empty()) {
      large_trx_waiting_list.push_back(thd);
      while (thd != m_queue[stage].get_leader()) {
        mysql_cond_wait(&m_stage_cond_large_trx_wait, &m_queue_lock[stage]);
      }
      return true;
    }
  }
...
THD *Commit_stage_manager::Mutex_queue::fetch_and_empty() {
  ...
  /*
    Signal the waiting large transaction to awake
    and become the leader of binlog flush queue.
  */
  Commit_stage_manager &mngr = Commit_stage_manager::get_instance();
  if (!mngr.large_trx_waiting_list.empty() &&
      &mngr.m_queue[BINLOG_FLUSH_STAGE] == this) {
    bool leader = append(mngr.large_trx_waiting_list.front());
    assert(leader == true);
    mngr.large_trx_waiting_list.pop_front();
    mysql_cond_signal(&mngr.m_stage_cond_large_trx_wait);
  }
