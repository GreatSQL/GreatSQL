...
bool PT_connect_by::do_contextualize(Parse_context *pc) {
  if (super::do_contextualize(pc)) return true;
  if (m_start) {
...
bool PT_set_variable::do_contextualize(Parse_context *pc) {
  ...
  const bool is_1d_name = m_opt_prefix.str == nullptr && !m_def_list;
  ...
  */
  ...
  if (is_1d_name) {
    ...
    sp_variable *spv =
        lex->find_variable(m_name.str, m_name.length, &pctx, &rh);
    if (spv != nullptr) {
      if (m_opt_expr == nullptr) {
        error(pc,
      ...
      */
      ...
      auto *sp_instr = new (thd->mem_root) sp_instr_set(
          sp->instructions(), lex, spv->offset, m_opt_expr, expr_query, true,
          pctx, rh);  // The instruction owns its lex.
  ...
  // It's a()...d
  if (m_def_list) {
    sp_head *const sp_local = lex->sphead;
    ...
    if (spv && spv->field_def.ora_record.is_row_type()) {
      // type is record of
      ...
      sp_instr_set_row_field_by_name *is = new (thd->mem_root)
          sp_instr_set_row_field_by_name(sp->instructions(), lex, spv->offset,
                                         m_opt_expr, expr_query, true, pctx, rh,
                                         m_name);
...
bool PT_select_sp_var::do_contextualize(Parse_context *pc) {
  ...
  const Sp_rcontext_handler *rh = nullptr;
  if (!(spv = lex->find_variable(name.str, name.length, &ctx, &rh))) {
  ...
  m_select_into_spv_info.m_cursor_rowtype_offset = spv->offset;
  m_select_into_spv_info.m_ctx_cursor = ctx;
  m_select_into_spv_info.m_rh_cursor = rh;
  is_row_table = spv->field_def.ora_record.row_field_table_definitions();
  is_rowtype = !is_row_table && spv->field_def.ora_record.is_row_type();
  is_index_by_varchar =
...
bool PT_select_sp_var_field::do_contextualize(Parse_context *pc) {
  // Associative arrays and VARRAY variables are not supported in 'into list'
  if (!ora_var_ident || 0 == ora_var_ident->bipartite_name.name.length) {
    char buff[64] = {0};
    snprintf(buff, sizeof(buff), "variable '%s' used in 'into list'", name.str);
  ...
  if (!ora_var_ident->medium) {
    spvar_name_def = ora_simple_ident_def(name, nullptr);
    m_full_ident_list.push_back(&spvar_name_def);
  } else {
    for (auto ident = ora_var_ident->medium->begin();
         ident != ora_var_ident->medium->end(); ++ident) {
      m_full_ident_list.push_back(&(*ident));
    }
  }
  field_name_def = ora_simple_ident_def(
      to_lex_string(ora_var_ident->bipartite_name.name), nullptr);
  m_full_ident_list.push_back(&field_name_def);
  sp_variable *spv = nullptr;
  sp_pcontext *ctx = nullptr;
  const Sp_rcontext_handler *rh = nullptr;
  if (!(spv = pc->thd->lex->find_variable(name.str, name.length, &ctx, &rh))) {
    my_error(ER_SP_UNDECLARED_VAR, MYF(0), name.str);
    return true;
  }
  m_spv_offset = spv->offset;
  m_rh_cursor = rh;
  return false;
}
...
Sql_cmd *PT_update::make_cmd(THD *thd) {
  ...
  if (opt_row_value_list != nullptr) {
    select->parsing_place = CTX_INSERT_VALUES;
    if (opt_column_list->contextualize(&pc)) return nullptr;
    if (opt_row_value_list->contextualize(&pc)) return nullptr;

    assert(select->parsing_place == CTX_INSERT_VALUES);
    select->parsing_place = CTX_NONE;
  }
  if (opt_merge_update_where != nullptr) {
    if (opt_merge_update_where->itemize(&pc, &opt_merge_update_where))
      return nullptr;
    if (!opt_merge_update_where->is_bool_func()) {
      opt_merge_update_where = make_condition(&pc, opt_merge_update_where);
      if (opt_merge_update_where == nullptr) return nullptr;
    }
  }
  if (opt_merge_insert_where != nullptr) {
    if (opt_merge_insert_where->itemize(&pc, &opt_merge_insert_where))
      return nullptr;
    if (!opt_merge_insert_where->is_bool_func()) {
      opt_merge_insert_where = make_condition(&pc, opt_merge_insert_where);
      if (opt_merge_insert_where == nullptr) return nullptr;
    }
  }
  if (opt_merge_update_delete != nullptr) {
    if (opt_merge_update_delete->itemize(&pc, &opt_merge_update_delete))
      return nullptr;
    if (!opt_merge_update_delete->is_bool_func()) {
      opt_merge_update_delete = make_condition(&pc, opt_merge_update_delete);
      if (opt_merge_update_delete == nullptr) return nullptr;
    }
  }
  ...
  */
  ...
  // additional parameters for merge-into
  sql_cmd->merge_into_stmt = merge_into_stmt;
  if (opt_row_value_list != nullptr) {
    sql_cmd->merge_when_insert = true;
    sql_cmd->insert_field_list = opt_column_list->value;
    sql_cmd->insert_many_values = opt_row_value_list->get_many_values();
    sql_cmd->opt_merge_insert_where = opt_merge_insert_where;
  }
  sql_cmd->opt_merge_update_where = opt_merge_update_where;
  sql_cmd->opt_merge_update_delete = opt_merge_update_delete;
...
bool PT_insert_all_into::do_contextualize(Parse_context *pc) {
...
Sql_cmd *PT_call::make_cmd(THD *thd) {
  ...
  if (!pkg_name->m_name.length)
    sp_add_own_used_routine(lex, thd, Sroutine_hash_entry::PROCEDURE,
                            proc_name);
  else {
...
bool PT_table_record_function::do_contextualize(Parse_context *pc) {
  if (super::do_contextualize(pc)) return true;
  const Sp_rcontext_handler *rh = nullptr;
  sp_variable *spv = nullptr;
  ...
  if (!pc->thd->lex->parsing_options.allows_variable) {
    my_error(ER_VIEW_SELECT_VARIABLE, MYF(0));
    return true;
  }
  if (!m_record_index_i && !spv->field_def.ora_record.is_row_type()) {
    my_error(ER_SP_MISMATCH_RECORD_VAR, MYF(0), m_table_alias.str);
    return true;
  }
  if (m_record_index_i &&
  ...
  auto stf = new (pc->mem_root)
      Table_function_record(m_table_alias.str, vt_list, spv, m_record_index_i,
                            false, rh, nullptr, m_spv_i);
  if (stf == nullptr) return true;  // OOM
...
bool PT_table_udt_function::do_contextualize(Parse_context *pc) {
  if (super::do_contextualize(pc) || m_expr->itemize(pc, &m_expr)) return true;

  auto stf = new (pc->mem_root) Table_function_udt(m_table_alias.str, m_expr);
  if (stf == nullptr) return true;  // OOM
  ...
  auto ti = new (pc->mem_root) Table_ident(alias, stf);
  if (ti == nullptr) return true;
  ti->db = to_lex_cstring(m_expr->get_udt_db_name().str);
...
bool PT_create_external_dir_file_name::do_contextualize(
...
bool PT_column_def::do_contextualize(Table_ddl_parse_context *pc) {
  ...
  if (field_def->udt_ident) {
    ...
    // For create table db2.tabl1 (id int, u1 t_air),the db=db2.
    LEX_CSTRING nested_table_udt = NULL_CSTR;
    bool ret =
        sp_get_ora_type_reclength(pc->thd, field_def->udt_ident->table,
                                  to_lex_string(field_def->udt_ident->m_column),
                                  false, &reclength, &nested_table_udt);
    if (ret || !reclength) return true;
    if (nested_table_udt.str) {
      my_error(ER_NOT_SUPPORTED_YET, MYF(0),
               "create table with udt table column");
      return true;
    }
    field_def->length = pc->thd->strmake(std::to_string(reclength).c_str(),
                                         std::to_string(reclength).length());
  }
...
static bool gdb_check_obj_name(const char *name, size_t length) {
  switch (check_table_name(name, length)) {
    case Ident_name_check::WRONG:
      my_error(ER_WRONG_TABLE_NAME, MYF(0), name);
      return true;
...
Sql_cmd *PT_create_synonym_stmt::make_cmd(THD *thd) {
  ...
  if (gdb_check_obj_name(m_synonym_ident->table.str,
                         m_synonym_ident->table.length))
    return nullptr;
...
Sql_cmd *PT_create_table_stmt::make_cmd(THD *thd) {
  ...
  if (opt_like_clause != nullptr) {
    ...
    if (my_strcasecmp(table_alias_charset, table->db, m_udt_ident->table.str)) {
      my_error(ER_WRONG_UDT_DATA_TYPE, MYF(0), table->db,
               m_udt_ident->m_column.str, m_udt_ident->table.str,
               m_udt_ident->m_column.str);
      return nullptr;
    }
    if (opt_create_table_options) {
      uint64_t unsupport_fields =
          HA_CREATE_USED_SECONDARY_ENGINE | HA_CREATE_USED_AVG_ROW_LENGTH |
          HA_CREATE_USED_AUTO | HA_CREATE_USED_ROW_FORMAT |
          HA_CREATE_USED_UNION | HA_CREATE_USED_SECONDARY_ENGINE_ATTRIBUTE;
      for (auto option : *opt_create_table_options) {
        if (option->contextualize(&pc2)) return nullptr;
        if (pc2.create_info->used_fields & unsupport_fields) {
          my_error(ER_NOT_SUPPORTED_YET, MYF(0),
                   "create table of type with this table option");
          return nullptr;
        }
      }
    }
    pc2.create_info->options |= HA_LEX_CREATE_TABLE_OF;
    pc2.create_info->udt_db_name = m_udt_ident->table;
    pc2.create_info->udt_name = m_udt_ident->m_column;
  } else {
...
Table_ident *PT_create_mlog_stmt::make_mlog_name(THD *thd,
                                                 Table_ident *table_name) {
  auto log_name_len = table_name->table.length + sizeof(MLOG_PREFIX);

  auto log_name = (char *)thd->alloc(log_name_len);
  if (log_name == nullptr) {
    return nullptr;
  }
  snprintf(log_name, log_name_len, MLOG_PREFIX "%s", table_name->table.str);
  return new (thd->mem_root)
      Table_ident(table_name->db, LEX_CSTRING{log_name, log_name_len});
}

Sql_cmd *PT_create_mlog_stmt::make_cmd(THD *thd) {
  LEX *const lex = thd->lex;
  lex->sql_command = SQLCOM_CREATE_TABLE;

  Parse_context pc(thd, lex->current_query_block());

  /**  select id , ,src_table_obj.* from src_table_obj where false */
  auto log_ident = PT_create_mlog_stmt::make_mlog_name(thd, table_name);
  if (log_ident == nullptr) return nullptr;

  Table_ref *table = pc.select->add_table_to_list(
      thd, log_ident, nullptr, TL_OPTION_UPDATING, TL_WRITE, MDL_SHARED);
  if (table == nullptr) return nullptr;
  ...
  Table_ref *src_table = pc.select->add_table_to_list(
      thd, table_name, nullptr, 0, TL_READ, MDL_SHARED_READ);
  if (!src_table) return nullptr;
  /* CREATE TABLE ... LIKE is not allowed for views. */
  src_table->required_type = dd::enum_table_type::BASE_TABLE;
  Table_ref *qe_tables = *like_clause_table;
  ...
  lex->create_info->used_fields |= HA_CREATE_MATERIALIZED_VIEW_LOG;

  return new (thd->mem_root) Sql_cmd_create_table(&m_alter_info, qe_tables);
}

Sql_cmd *PT_create_materialized_view_stmt::make_cmd(THD *thd) {
  LEX *const lex = thd->lex;
  ...
  if (mview_opt->column_list) {
    //
    for (auto column_alias : *mview_opt->column_list) {
      if (check_column_name(column_alias)) {
        my_error(ER_WRONG_COLUMN_NAME, MYF(0), column_alias.str);
        return nullptr;
      }

      if (strncasecmp(column_alias.str, STRING_WITH_LEN(HIDDEN_IMV_PREFIX)) ==
          0) {
        my_error(ER_WRONG_COLUMN_NAME, MYF(0), column_alias.str);
        return nullptr;
      }
  ...
  if (mview_opt->qe->contextualize(&pc)) return nullptr;
  if (pc.finalize_query_expression()) return nullptr;
  ...
  */
  ...
  // for shared source table
  pc.select->set_lock_for_tables(TL_READ_WITH_SHARED_LOCKS);

  auto qe_tables = *query_expression_tables;
  ...
  // note:
  //  setup_base_ref_items need add count or sum func but int imv can't
  //  use having condition , select_n_having_items is big
  //  not need to add ref_item
  //    pc.select->n_sum_items +=2;
  //

  thd->lex->alter_info = &m_alter_info;
  thd->lex->set_is_materialized_view(true);
  lex->create_info->used_fields |= HA_CREATE_MATERIALIZED_VIEW;

  auto mv_info = new (thd->mem_root) materialized_view_info;
  mv_info->build_clasue = mview_opt->build_clause;
  mv_info->flush_mode = mview_opt->flush_mode;
  mv_info->with_row_id = mview_opt->with_rowid;

  if (thd->variables.sql_generate_invisible_primary_key) {
    mv_info->with_row_id = 1;
  }

  if (mview_opt->attrs) {
    if (mview_opt->attrs->length() > 0 || mview_opt->flush_mode == 0) {
      mv_info->table_ref = mview_opt->attrs->lex_string();

      lex->create_info->m_transactional_ddl = true;
      lex->create_info->used_fields |= HA_CREATE_USED_START_TRANSACTION;
    } else {
      thd->syntax_error_at(POS(), "Illegal attribute");
      return nullptr;
    }
  }
  ...
  uint dummy_err;
  if (query_view_str_utf8.copy(mview_opt->define.str, mview_opt->define.length,
  ...
  mv_info->create_view_define.str = utf8_view;
  mv_info->create_view_define.length = query_view_str_utf8.length();
  mv_info->create_view_query_block.str = utf8_view;
  mv_info->create_view_query_block.length = query_view_str_utf8.length();

  lex->create_info->m_mv_info = mv_info;
  mem_root_unordered_map<std::string_view, Item *> hidden_items(thd->mem_root);

  // agg item
  for (size_t i = 0; i < lex->query_block->fields.size(); i++) {
    auto it = lex->query_block->fields.at(i);
    if (it->has_aggregation()) {
      if (greatdb::check_fields_to_rewrite_aggr_func(thd, it, pc.select,
                                                     &hidden_items))
        return nullptr;
    }
  }

  if (lex->query_block->group_list.first != nullptr) {
    // add_hidden_count(0)
    auto count0 = greatdb::add_count0_func(thd);
    if (!count0) return nullptr;
    hidden_items.insert(std::make_pair(
        std::string_view(count0->item_name.ptr(), count0->item_name.length()),
        count0));
  }

  for (auto itr : hidden_items) {
    Item *it = itr.second;
    // if(it->itemize(&pc, &it)) return nullptr;

    lex->query_block->fields.push_back(it);

    if (mview_opt->column_list) {
      mview_opt->column_list->push_back(
          {it->item_name.ptr(), it->item_name.length()});
    }
  }

  if (mview_opt->column_list) {
    qe_tables->set_derived_column_names(mview_opt->column_list);
  }
...
Sql_cmd *PT_create_sequence_stmt::make_cmd(THD *thd) {
  ...
  if (gdb_check_obj_name(seq_ident->table.str, seq_ident->table.length))
    return nullptr;
...
Sql_cmd *PT_show_create_package::make_cmd(THD *thd) {
  LEX *lex = thd->lex;
  lex->sql_command = m_sql_command;
...
Sql_cmd *PT_show_create_package_body::make_cmd(THD *thd) {
  LEX *lex = thd->lex;
  lex->sql_command = m_sql_command;
...
Sql_cmd *PT_show_create_synonym::make_cmd(THD *thd) {
  ...
  auto *synonym_ident = m_sql_cmd.table_ident();
  if (gdb_check_obj_name(synonym_ident->table.str, synonym_ident->table.length))
    return nullptr;

  return &m_sql_cmd;
}  // Sql_cmd *PT_show_create_synonym::make_cmd(THD *thd)

Sql_cmd *PT_show_create_materialized_view::make_cmd(THD *thd) {
  LEX *lex = thd->lex;
  lex->sql_command = m_sql_command;
  return &m_sql_cmd;
}
...
Sql_cmd *PT_show_create_sequence::make_cmd(THD *thd) {
  ...
  auto *seq_ident = m_sql_cmd.table_ident();
  if (gdb_check_obj_name(seq_ident->table.str, seq_ident->table.length))
    return nullptr;
...
Sql_cmd *PT_show_status_package::make_cmd(THD *thd) {
  LEX *lex = thd->lex;
  lex->sql_command = m_sql_command;
...
Sql_cmd *PT_show_status_package_body::make_cmd(THD *thd) {
  LEX *lex = thd->lex;
  lex->sql_command = m_sql_command;
...
Sql_cmd *PT_show_status_type::make_cmd(THD *thd) {
  LEX *lex = thd->lex;
  lex->sql_command = m_sql_command;
...
/**
 * @brief
 *  change materialized view  has init or not
 *
 * @param thd
 *
 * @return Sql_cmd*
 */
Sql_cmd *PT_alter_materialized_view::make_cmd(THD *thd) {
  thd->lex->sql_command = SQLCOM_ALTER_TABLE;
  ...
  if (init_alter_table_stmt(
          &pc, mv_ident,
          Alter_info::enum_alter_table_algorithm::ALTER_TABLE_ALGORITHM_DEFAULT,
          Alter_info::enum_alter_table_lock::ALTER_TABLE_LOCK_DEFAULT,
          Alter_info::enum_with_validation::ALTER_VALIDATION_DEFAULT))

    return nullptr;

  m_alter_info.flags |= Alter_info::ALTER_OPTIONS;
  m_create_info.used_fields |= HA_CREATE_MATERIALIZED_VIEW;

  auto mv_info = new (thd->mem_root) materialized_view_info;
  mv_info->build_clasue = 1;
  m_create_info.m_mv_info = mv_info;
...
Sql_cmd *PT_load_table::make_cmd(THD *thd) {
  ...
  /* Add for GreatDB, for parse set_var(gdb_parallel_load[_xxx]=xxx) */
  if (m_opt_hints != nullptr) {
    if (m_opt_hints->contextualize(&pc)) return nullptr;
    const auto &hints = m_opt_hints->get_hints();
    for (const auto hint : hints) {
      const auto &hint_sys_var = dynamic_cast<PT_hint_sys_var *>(hint);
      if (hint_sys_var == nullptr ||
          (my_strcasecmp(system_charset_info,
                         hint_sys_var->get_sys_var_name().str,
                         "gdb_parallel_load") &&
           my_strcasecmp(system_charset_info,
                         hint_sys_var->get_sys_var_name().str,
        ...
        my_error(ER_GDB_PARALLEL_LOAD, MYF(0),
                 "only support hints: set_var(gdb_parallel_load[_xxx]=xx)");
        return nullptr;
      }
    }
  }
...
bool PT_limit_clause::do_contextualize(Parse_context *pc) {
  ...
  if (limit_options.is_offset_first != 0 &&
      limit_options.opt_offset != nullptr &&
      limit_options.opt_offset->itemize(pc, &limit_options.opt_offset))
    return true;

  if (limit_options.limit != nullptr) {
  ...
  pc->select->select_limit_percent = limit_options.percent;
  pc->select->select_limit_ties = limit_options.ties;

  if (limit_options.is_offset_first == 2) {
    pc->select->select_limit_fetch = true;
  }
...
Sql_cmd *PT_reload_sequences::make_cmd(THD *thd [[maybe_unused]]) {
  thd->lex->sql_command = SQLCOM_RELOAD_SEQUENCES;

  if (m_seqs) {
    for (uint i = 0; i < m_seqs->size(); i++) {
      auto *ident = m_seqs->at(i);
      if (gdb_check_obj_name(ident->table.str, ident->table.length))
        return nullptr;
    }
  }

  Sql_cmd *sql_com = new (thd->mem_root) Sql_cmd_reload_sequences(m_seqs);
...
Sql_cmd *PT_install_component::make_cmd(THD *thd) {
  ...
  DBUG_EXECUTE_IF("bug13497_install_component_set",
