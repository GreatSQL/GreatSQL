...
Item_splocal *create_item_for_sp_var(THD *thd, LEX_CSTRING name,
  ...
  spv = lex->find_variable(name.str, name.length, &ctx, &rh);
  ...
  Item_splocal *item = new (thd->mem_root) Item_splocal(
      rh, name, spv->offset, spv->type, spv_pos_in_query, spv_len_in_query);

  if (item) item->m_sp = spv->sph;

  return item;
}

Item_splocal_row_field_by_name *create_item_for_sp_var_row_field(
    THD *thd, LEX_CSTRING name, const char *field,
    const Sp_rcontext_handler *rh, sp_variable *spv,
  ...
  Item_splocal_row_field_by_name *item = new (thd->mem_root)
      Item_splocal_row_field_by_name(rh, name, to_lex_cstring(field),
                                     spv->offset, spv->type, spv_pos_in_query,
                                     spv_len_in_query);

  if (item) item->m_sp = sph;

  return item;
}

// a.b.c
Item_splocal_row_field_table_by_name *create_item_for_sp_var_row_field_table(
    THD *thd, const char *field, const Sp_rcontext_handler *rh,
    List<ora_simple_ident_def> *arg_list, sp_variable *spv, Item *number,
    sp_pcontext *pctx, const char *query_start_ptr, const char *start,
    const char *end, sp_head *sph) {
  LEX *lex = thd->lex;
  size_t spv_pos_in_query = 0;
  size_t spv_len_in_query = 0;
  ...
  Item_splocal_row_field_table_by_name *item = new (thd->mem_root)
      Item_splocal_row_field_table_by_name(rh, spv->name, to_lex_cstring(field),
                                           spv->offset, arg_list, number, pctx,
                                           spv_pos_in_query, spv_len_in_query);
...
Item_splocal_row_field_table_by_index *create_item_table_by_index_for_sp_var(
    ...
    Item *index, sp_head *sph, const char *query_start_ptr, const char *start,
    const char *end) {
  LEX *lex = thd->lex;
  ...
  Item_splocal_row_field_table_by_index *item = new (thd->mem_root)
      Item_splocal_row_field_table_by_index(rh, name, spv->offset, index,
                                            spv->type, spv_pos_in_query,
                                            spv_len_in_query);
