...
// for spv1 cursor%rowtype;cursor maybe package's head or body define.
struct Ora_cursor_rowtype {
 public:
  int m_cursor_rowtype_offset;
  sp_pcontext *m_ctx_cursor;
  const Sp_rcontext_handler *m_rh_cursor;
  Ora_cursor_rowtype(int m_offset, sp_pcontext *ctx,
                     const Sp_rcontext_handler *rh)
      : m_cursor_rowtype_offset(m_offset), m_ctx_cursor(ctx), m_rh_cursor(rh) {}
  void set_cursor(Ora_cursor_rowtype *from) {
    m_cursor_rowtype_offset = from->m_cursor_rowtype_offset;
    m_ctx_cursor = from->m_ctx_cursor;
    m_rh_cursor = from->m_rh_cursor;
  }
};
...
class Ora_record_refer {
 ...
 public:
  ...
  Ora_cursor_rowtype m_cursor_rowtype_info;   // for cursor%ROWTYPE
  ...
  */
  ...
  // for select/fetch into spv,save the info of spv's pctx and rh
  Ora_cursor_rowtype m_select_into_spv_info;
  bool is_single_type_define;  // for table of type
  ...
  Ora_record_refer()
      : m_row_field_definitions(nullptr),
        ...
        m_cursor_rowtype_info({-1, nullptr, nullptr}),
        ...
        m_cursor_offset(-1),
        m_is_sp_param(false),
        m_select_into_spv_info({-1, nullptr, nullptr}),
        is_single_type_define(false) {}
  void set_record(Ora_record_refer *from) {
    ...
    m_cursor_rowtype_info.set_cursor(&from->m_cursor_rowtype_info);
    ...
    m_select_into_spv_info.set_cursor(&from->m_cursor_rowtype_info);
    is_single_type_define = from->is_single_type_define;
  }
  ...
  int cursor_rowtype_offset() const {
    return m_cursor_rowtype_info.m_cursor_rowtype_offset;
  }

  void set_cursor_rowtype_ref(int offset, sp_pcontext *ctx,
                              const Sp_rcontext_handler *rh) {
    m_cursor_rowtype_ref = true;
    m_cursor_rowtype_info.m_cursor_rowtype_offset = offset;
    m_cursor_rowtype_info.m_ctx_cursor = ctx;
    m_cursor_rowtype_info.m_rh_cursor = rh;
  }
  ...
  sp_rcontext *get_spv_rcontext(sp_rcontext *ctx) const {
    return m_select_into_spv_info.m_rh_cursor
               ? m_select_into_spv_info.m_rh_cursor->get_rcontext(ctx)
               : nullptr;
  }
  bool is_type_define() const {
    return is_record_table_type_define || is_ref_cursor_define ||
           is_record_table_define || is_record_define;
  }
  bool is_row_type() const {
    return is_cursor_rowtype_ref() || table_rowtype_ref() ||
           row_field_definitions() || column_type_ref();
  }
};
...
class Create_field {
  ...
  */
  ...
  void set_sp_define_param(PT_row_type *row_type, bool is_sp_param);
