...
/* Instrumentation key for performance schema for mts_temp_table_LOCK */
 ...
 private:
  Gtid_set *gtid_set;
  std::vector<std::pair<rpl_sidno, rpl_gno>> batching_gtids;
  /*
 ...
 public:
  ...
  void add_logged_batching_gtids() {
    for (const auto &gtid : batching_gtids) {
      add_logged_gtid(gtid.first, gtid.second);
    }
  }

  void collect_batching_gtids(rpl_sidno sidno, rpl_gno gno) {
    batching_gtids.push_back({sidno, gno});
  }

  void clear_batching_gtids() { batching_gtids.clear(); }
...
*/
  ...
  */
  ...
  std::set<ulonglong> m_full_table_ids; /* RBR: table_ids before filter out */
  ...
  */
  ...
  /* logical_clock parallel wait mode */
  int current_mts_wait_mode;
  /* logocal_clock parallel, temp wait flag for next worker */
  longlong m_next_worker_wait_last_committed;
  /* MTS submode  */
