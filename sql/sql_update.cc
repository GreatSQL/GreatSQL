...
bool Sql_cmd_update::precheck(THD *thd) {
  ...
  if (!multitable) {
    ...
    /*
      if merge_when_insert is set, INSERT_ACL should be granted for the
      target table, i.e. insert_table_list.
      if merge_when_insert is not set, it means no insertion of record.
      i.e. not INSERT_ACL is necessary.
    */
    ...
    if (merge_into_stmt && merge_when_insert) {
      ...
      if (insert_table_list->is_view() || insert_table_list->is_derived())
        return true;
      merge_privilege |= INSERT_ACL;
    }
    if (merge_into_stmt && opt_merge_update_delete != nullptr)
      merge_privilege |= DELETE_ACL;
    ...
    */
    for (Table_ref *tr = lex->query_tables; tr; tr = tr->next_global) {
      ...
      */
      ...
      if (tr == insert_table_list) privilege |= merge_privilege;
...
bool Sql_cmd_update::check_privileges(THD *thd) {
  ...
  if (merge_into_stmt) {
    if (merge_when_insert) {
      mem_root_deque<Item *> *insert_values = insert_many_values[0];

      if (check_privileges_for_list(thd, *insert_values, SELECT_ACL))
        return true;
      if (check_privileges_for_list(thd, insert_field_list, INSERT_ACL))
        return true;
      if (opt_merge_insert_where) {
        thd->want_privilege = SELECT_ACL;
        if (opt_merge_insert_where->walk(&Item::check_column_privileges,
                                         enum_walk::PREFIX,
    ...
    if (opt_merge_update_where) {
      thd->want_privilege = SELECT_ACL;
      if (opt_merge_update_where->walk(&Item::check_column_privileges,
                                       enum_walk::PREFIX,
                                       pointer_cast<uchar *>(thd)))
        return true;
    }
    if (opt_merge_update_delete) {
      thd->want_privilege = SELECT_ACL;
      if (opt_merge_update_delete->walk(&Item::check_column_privileges,
                                        enum_walk::PREFIX,
                                        pointer_cast<uchar *>(thd)))
        return true;
    }
...
bool Sql_cmd_update::prepare_inner(THD *thd) {
  ...
  if (merge_into_stmt) {
    // for merge_into, insert/update table is the latest entry in table_list.
    for (insert_table_list = table_list; insert_table_list->next_local;
         insert_table_list = insert_table_list->next_local)
      ;
    lex->insert_table = insert_table_list;
    insert_into_view = insert_table_list->is_view();
    column_count = insert_field_list.size();
    if (insert_table_list->query_block) {
      insert_table_list->query_block->merge_target = insert_table_list;
    }
  }
  ...
  if (merge_into_stmt && merge_when_insert) {
    if (insert_into_view) {
      ...
      if (insert_table_list->resolve_derived(thd, false)) return true;
      // require proper privileges for all leaf tables of the view.
      // check for target table only.
      if (select->check_view_privileges(thd, INSERT_ACL, SELECT_ACL))
        return true;

      // On second preparation, we may need to resolve view condition
      // generated when mergin in the view.
      if (!select->first_execution && insert_table_list->is_merged() &&
          fix_join_cond_for_insert(thd, insert_table_list))
        return true;
    }

    if (!insert_table_list->is_insertable()) {
      my_error(ER_NON_INSERTABLE_TABLE, MYF(0), insert_table_list->alias,
               "MERGE INTO");
      return true;
    }

    if (insert_into_view && column_count == 0) {
      if (insert_table_list->is_multiple_tables()) {
        my_error(ER_VIEW_NO_INSERT_FIELD_LIST, MYF(0), insert_table_list->db,
                 insert_table_list->table_name);
        return true;
      }
    }

    // must be allocated before Item::fix_fields()
    if (insert_table_list->set_insert_values(thd->mem_root)) return true;
  ...
  */
  ...
  if (!multitable) {
    ...
    result = new (thd->mem_root) Query_result_update(
        &select->fields, update_value_list, insert_table_list,
        &insert_field_list, &insert_many_values);
  ...
  // setup_fields() should be called after select->setup_base_ref_items()
  // otherwise, derived table reference could not be resolved.
  if (merge_into_stmt && merge_when_insert) {
    TABLE *const insert_table = lex->insert_table_leaf->table;
    uint field_count = insert_field_list.size();
    // inesrt_many_values.size() == 1 which is limited by grammar.
    const List_item *values = insert_many_values[0];

    if (values->size() == 0) {
      my_error(ER_WRONG_VALUE_COUNT_ON_ROW, MYF(0), 1);
      return true;
    }
    if (field_count != 0 && values->size() != field_count) {
      my_error(ER_WRONG_VALUE_COUNT_ON_ROW, MYF(0), 1);
      return true;
    }
    value_count = values->size();

    // const_cast is fine, since we're sending split_sum_funcs false.
    ...
    // Table_ref::map() is available after setup_fields()
    table_map insert_map = insert_table_list->map();
    for (Item *item : *values) {
      if (!item->const_for_execution()) values_need_privilege_check = true;
      if (item->used_tables() & insert_map) {
        my_error(ER_NOT_SUPPORTED_YET, MYF(0),
                 "VALUES clause refers target table");
        return true;
      }
    }
    ...
    // check_insert_fields() will usually mark all inserted columns in
    // wriet_set, except when
    ...
    //    resolve_derived(), thus setup_fields() in check_insert_fields()
    //    will not process them), or
    // - no column where provided in field list
    //   (except when no value is given - this is a special case that
    //    implies that all columns are given default values, and default
    //    values are managed in a different manner, set COPY_INFO for
    //    details)
    if ((insert_into_view || column_count == 0) && value_count > 0)
      bitmap_set_all(insert_table->write_set);

#if 0 /* @@TODO@@ no support now */
    MY_BITMAP *function_default_columns = nullptr;
    if (get_default_columns(thd, insert_table, &function_columns))
      return true;
#endif
    ...
    // @@NOTE@@ insert_table->write_set is set in check_insert_fields()
    // or bitmap_set_all() above.
    // So insert_table->write_set should be cloned and cleared here.
    ...
    bitmap_init(&merge_insert_bitmap, bitmaps, n_bits);
  ...
  if (setup_fields(thd, /*want_privilege=*/UPDATE_ACL,
                   ...
                   &select->fields, Ref_item_array(), true))
    return true;
  ...
  if (merge_into_stmt) {
    Item *join_on = insert_table_list->join_cond();
    if (!join_on) return true;
...
#if 0  // keep MySQL behavior
    if (field_list.size() != select->fields.size()) {
      my_error(ER_NOT_SUPPORTED_YET, MYF(0),
               "duplicated column in 'UPDATE SET' fields");
      return true;
    }
#endif
    mem_root_deque<Item_field *> item_list(thd->mem_root);
    join_on->walk(&Item::collect_item_field_processor, enum_walk::POSTFIX,
                  (uchar *)&item_list);
    table_map insert_map = insert_table_list->map();
    for (Item_field *update_field : field_list) {
      if (update_field->used_tables() != insert_map) {
        my_error(ER_MERGE_UPDATE_SOURCE_TABLE, MYF(0), update_field->table_name,
                 update_field->field_name);
        return true;
      }
      for (Item_field *on_item : item_list) {
        if (update_field->eq(on_item, true)) {
          my_error(ER_NOT_SUPPORTED_YET, MYF(0), "update field in ON-clause");
          return true;
        }
      }
    }
  ...
  */
  ...
  // for merge-into without when-matched-clause
  assert(merge_when_insert || update_table_count_local > 0);
  ...
  */
  ...
  for (Table_ref *tl = select->leaf_tables; tl; tl = tl->next_leaf) {
    if (tl->is_updated() || tl->is_inserted()) {
  ...
  */
  ...
  // it is necessary to fill all necessary attributes
  // of Query_result_update object before prepare()
  if (merge_into_stmt) {
    Item *join_on = insert_table_list->join_cond();
    if (!join_on) return true;

    const_join_on = join_on->const_item();
    if (const_join_on) {
...
void UpdateRowsIterator::store_values(THD *thd,
                                      const mem_root_deque<Item *> &values) {
  // const mem_root_deque<Item *> *insert_values = (*insert_many_values)[0];
  const mem_root_deque<Item *> *insert_values = &values;

  if (CountVisibleFields(*insert_field_list) != 0) {
    restore_record(insert_table, s->default_values);
    if (!validate_default_values_of_unset_fields(thd, insert_table))
      fill_record_n_invoke_before_triggers(
          thd, insert, *insert_field_list, *insert_values, insert_table,
          TRG_EVENT_INSERT, insert_table->s->fields, true, nullptr);
  } else {
    fill_record_n_invoke_before_triggers(
        thd, insert_table->field, *insert_values, insert_table,
        TRG_EVENT_INSERT, insert_table->s->fields);
  }

  check_that_all_fields_are_given_values(thd, insert_table, insert_table_list);
}

bool UpdateRowsIterator::merge_insert_into_tmp(THD *thd) {
  DBUG_TRACE;

  if (opt_merge_insert_where && !opt_merge_insert_where->val_int())
    return false;

  // table->has_null_row() is always true for insert.
  // it is necessary to clear this for add_pke().
  insert_table_list->table->reset_null_row();

  thd->check_for_truncated_fields = CHECK_FIELD_WARN;

  if (tmp_insert_table->triggers) {
    for (Field **modified_fields = tmp_insert_table->visible_field_ptr();
  ...
  /* store regular insert fields in the row */
  fill_record(thd, tmp_insert_table, tmp_insert_table->visible_field_ptr(),
              *insert_values, nullptr, nullptr, false);
  ...
  auto error =
      tmp_insert_table->file->ha_write_row(tmp_insert_table->record[0]);
  if (error) {
    if (!tmp_insert_table->file->is_ignorable_error(error)) {
      bool is_duplicate = false;
      if (create_ondisk_from_heap(thd, tmp_insert_table, error, true, true,
                                  &is_duplicate)) {
        return true; /* purecov: inspected */
      }
      if (!is_duplicate) {
  ...
  for (Table_ref *cur_table = m_update_tables; cur_table != nullptr;
    ...
    */
    ...
    if (m_updated_rows != org_updated ||
        m_deleted_found_rows != m_deleted_rows) {
      if (!table->file->has_transactions()) {
...
int UpdateRowsIterator::Read() {
  ...
  if (!local_error ||
    ...
    if (DoDelayedUpdates(&trans_safe, &transactional_tables)) {
      local_error = true;
    } else if (do_inserts(thd(), &trans_safe, &transactional_tables)) {
      local_error = true;
    }
  }
...
bool UpdateRowsIterator::do_inserts(THD *thd, bool *trans_safe,
                                    bool *transactional_tables) {
  if (!merge_into_stmt || !merge_when_insert) return false;

  // dump all records in tmp_insert_table into insert_table
  // triggers are fired here also
  int local_error = 0;
  myf error_flags = MYF(0);
  ha_rows from_tmp_rows = 0;

  insert_completed = true;

  if (mysql_bin_log.is_open())
    *transactional_tables |= insert_table->file->has_transactions();
  ...
  // setup copy functions to copy fields from temporary table
  Field **field = tmp_insert_table->visible_field_ptr();
  Item_field *item;
  mem_root_deque<Item *> *item_list =
      new (thd->mem_root) List_item(thd->mem_root);
  for (; *field; field++) {
    item = new (thd->mem_root) Item_field(*field);
    item_list->push_back(item);
  }
  assert(item_list->size() == insert_field_list->size());

  // @@NOTE@@ there is no index created, should this be called ?
  // tmp_insert_table->file->ha_index_or_rnd_end();
  if ((local_error = tmp_insert_table->file->ha_rnd_init(true))) {
    if (insert_table->file->is_fatal_error(local_error))
      error_flags |= ME_FATALERROR;
    insert_table->file->print_error(local_error, error_flags);
    goto err;
  }
  ...
  for (;;) {
    ...
    if ((local_error = tmp_insert_table->file->ha_rnd_next(
             tmp_insert_table->record[0]))) {
      if (local_error == HA_ERR_END_OF_FILE) break;
      if (local_error == HA_ERR_RECORD_DELETED)
        continue;  // May happen on dup key, but how for this tmp_inesrt_table?
      if (insert_table->file->is_fatal_error(local_error))
        error_flags |= ME_FATALERROR;
      insert_table->file->print_error(local_error, error_flags);
      goto err;
    ...
    insert_table_list->set_inserted();
    // store_record(insert_table, record[1]);
    thd->check_for_truncated_fields = CHECK_FIELD_WARN;
    store_values(thd, *item_list);
    thd->check_for_truncated_fields = CHECK_FIELD_ERROR_FOR_NULL;
    ...
    if (write_record(thd, insert_table, insert, nullptr)) goto err;
    if (insert_table->triggers) {
      /*
    ...
    if (insert_table->next_number_field) {
      /*
      ...
      */
      insert_table->next_number_field->reset();
    }
    from_tmp_rows++;
  }

  if (from_tmp_rows != m_insert_found_rows) goto err;
  (void)tmp_insert_table->file->ha_rnd_end();
  return false;

err:
  if (tmp_insert_table->file->inited)
    (void)tmp_insert_table->file->ha_rnd_end();
  if (from_tmp_rows != m_insert_found_rows) {
    if (insert_table->file->has_transactions())
      *transactional_tables = true;
    else {
      *trans_safe = false;
      thd->get_transaction()->mark_modified_non_trans_table(
...
bool Query_result_update::send_eof(THD *thd) {
  ...
  const ha_rows deleted_rows = iterator->deleted_rows();

  ha_rows inserted_row_count;
  inserted_row_count = (merge_when_insert) ? insert.stats.records : 0;
  if (merge_into_stmt)
    snprintf(buff, sizeof(buff), ER_THD(thd, ER_MERGE_INFO), (long)found_rows,
             (long)updated_rows, (long)inserted_row_count, (long)deleted_rows,
             (long)thd->get_stmt_da()->current_statement_cond_count());
  else
    snprintf(buff, sizeof(buff), ER_THD(thd, ER_UPDATE_INFO), (long)found_rows,
             (long)updated_rows,
...
unique_ptr_destroy_only<RowIterator> Query_result_update::create_iterator(
  ...
  unique_ptr_destroy_only<RowIterator> p = NewIterator<UpdateRowsIterator>(
