...
/* only use for show MGR second_behind_group info */
struct current_exec_channel {
  std::atomic<Master_info *> m_mi{nullptr};
  unsigned long long get_second_behind_group() const {
    unsigned long long ret = 0;
    Master_info *mi = m_mi.load();
    if ((mi != nullptr) && (mi->rli != nullptr) && mi->rli->slave_running) {
      long long time_diff = static_cast<long long>(
          time(nullptr) - mi->rli->last_master_timestamp);
      ret = static_cast<unsigned long long>(
          mi->rli->last_master_timestamp != 0 ? std::max(0LL, time_diff) : 0LL);
    }
    return ret;
  }
  void set_as_applier() {
    channel_map.rdlock();
    m_mi.store(channel_map.get_mi("group_replication_applier"));
    channel_map.unlock();
  }
  void set_as_recover() {
    channel_map.rdlock();
    m_mi.store(channel_map.get_mi("group_replication_recovery"));
    channel_map.unlock();
  }
  void compare_and_reset(const Master_info *mi) {
    if (mi == m_mi.load()) m_mi.store(nullptr);
  }
};
extern struct current_exec_channel cur_exec_channel;
