...
bool PTI_comp_op_all_any::do_itemize(Parse_context *pc, Item **res) {
  if (super::do_itemize(pc, res) || left->itemize(pc, &left) ||
...
bool PTI_function_call_nonkeyword_trigger_event_is::do_itemize(
    Parse_context *pc, Item **res) {
  if ((is_NULL = !m_column)) m_column = new (pc->mem_root) Item_null();
  if (super::do_itemize(pc, res) || m_column->itemize(pc, &m_column))
    return true;
...
std::pair<bool, Item *> create_item_for_sp_var(THD *thd, LEX_STRING db,
                                               LEX_STRING name,
      ...
      if (!nested_table_udt.str) {
        if (enum_table_type != enum_udt_table_of_type::TYPE_INVALID)
          return std::make_pair(
              false, new (thd->mem_root) Item_func_udt_single_type_table(
                         POS(), db, name, enum_table_type, varray_limit,
                         item_list, def_lists, reclength, sql_mode));
        else {
          return std::make_pair(
              false, new (thd->mem_root) Item_func_udt_constructor(
                         POS(), db, name, use_explicit_name, item_list,
                         field_def_list, reclength, sql_mode));
        }
      } else
        return std::make_pair(
            false, new (thd->mem_root) Item_func_udt_table(
                       POS(), db, name, nested_table_udt, reclength,
                       enum_table_type, varray_limit, item_list, def_lists));
    }
    thd->mdl_context.rollback_to_savepoint(mdl_savepoint);
    if (!is_udt_type && db.str) {
    ...
    */
    ...
    if (spv) {
      if (spv->field_def.ora_record.is_record_define &&
        ...
        return std::make_pair(
            false,
            create_item_table_by_index_for_sp_var(
        ...
        if (spv->field_def.ora_record.is_record_table_type_define)
          return std::make_pair(
              false, new (thd->mem_root) Item_func_udt_single_type_table(
                         POS(), db, name, enum_table_type, /*varray_limit*/ 0,
                         item_list, field_def_list, /*reclength*/ 0, sql_mode,
                         spv->field_def.ora_record.index_length));
        else
          return std::make_pair(
              false, new (thd->mem_root) Item_func_udt_table(
                         POS(), db, name, spv->field_def.nested_table_udt,
                         reclength, enum_table_type,
    ...
    if (pctx && pctx->find_cursor(name, &offset, false)) {
      return std::make_pair(
          false, new (thd->mem_root) Item_func_cursor(POS(), name, item_list));
    }
  }
    ...
    *res = builder->create_func(thd, ident, opt_udf_expr_list);
    ...
    if (udf) {
      ...
      *res = Create_udf_func::s_singleton.create(thd, udf, opt_udf_expr_list);
      ...
      auto res_pair = create_item_for_sp_var(pc->thd, NULL_STR, ident, false,
                                             opt_udf_expr_list, raw);
      *res = res_pair.second;
      if (!(*res || res_pair.first)) {
...
bool PTI_simple_ident_ident::do_itemize(Parse_context *pc, Item **res) {
  ...
  /*for next case,the variable num is not sp_variable.
  WITH FUNCTION f1(num NUMBER) begin end ; SELECT num */
  bool is_with_func_ident =
      lex->m_with_func && pc->select->parsing_place == CTX_SELECT_LIST;
...
bool PTI_simple_ident_q_3d::do_itemize(Parse_context *pc, Item **res) {
  ...
  if (!(spv = thd->lex->find_variable(db, to_lex_cstring(db).length, &pctx,
    ...
    if (spv->field_def.ora_record.row_field_table_definitions() ||
        spv->field_def.ora_record.is_row_type()) {
...
bool PTI_simple_ident_q_2d::do_itemize(Parse_context *pc, Item **res) {
  ...
  */
  if (sp && sp->m_type == enum_sp_type::TRIGGER &&
    ...
    *res = trg_fld;
  } else if ((spv = lex->find_variable(table, table_length, &ctx, &rh)) &&
    ...
    if (spv->field_def.ora_record.row_field_table_definitions()) {
      if (my_strcasecmp(system_charset_info, field, "count") == 0) {
        *res =
            new (thd->mem_root) Item_func_table_count(POS(), spv->offset, rh);
      } else if (my_strcasecmp(system_charset_info, field, "first") == 0) {
        *res =
            new (thd->mem_root) Item_func_table_first(POS(), spv->offset, rh);
      } else if (my_strcasecmp(system_charset_info, field, "last") == 0) {
        *res = new (thd->mem_root) Item_func_table_last(POS(), spv->offset, rh);
      } else {
        my_error(ER_SP_MISMATCH_RECORD_VAR, MYF(0), spv->name.str);
        return true;
      }
    } else if (spv->field_def.ora_record.is_row_type()) {
...
// select a.b.c
bool PTI_simple_ident_q_nd::do_itemize(Parse_context *pc, Item **res) {
  THD *thd = pc->thd;
  LEX *lex = thd->lex;
  sp_head *sp = lex->sphead;
  const Sp_rcontext_handler *rh = nullptr;
...
bool PTI_start_with::do_itemize(Parse_context *pc, Item **res) {
  if (super::do_itemize(pc, res)) return true;
