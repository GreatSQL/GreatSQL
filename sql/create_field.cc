...
void Create_field::set_sp_define_param(PT_row_type *row_type,
                                       bool is_sp_param) {
  table_type = row_type->m_table_type;
  varray_limit = row_type->m_varray_limit;
  nested_table_udt = row_type->m_nested_table_udt_tmp;
  ora_record.set_record(&row_type->m_record);
  ora_record.set_row_field_definitions(row_type->m_row_def);
  ora_record.set_row_field_table_definitions(row_type->m_row_def_table);
  ora_record.m_is_sp_param = is_sp_param;
  ora_record.is_record_define = false;
}
...
Row_definition_list *Row_definition_table_list::find_row_fields_by_offset(
    uint offset) const {
  ...
  List<Row_definition_list> *sd =
      dynamic_cast<List<Row_definition_list> *>(rdl);
  List_iterator<Row_definition_list> it(*sd);
  Row_definition_list *def;
  for (uint i = 0; (def = it++); i++) {
    if (i == offset) return def;
  }
  return nullptr;
}
...
Create_field *Row_definition_table_list::find_row_field_by_name(
  ...
  Create_field *result;
  for (uint i = 0; (def = it++); i++) {
    result = def->find_row_field_by_name(name, offset);
...
Create_field *Row_definition_list::find_row_field_by_name(LEX_STRING name,
  ...
  Row_definition_list *rdl = const_cast<Row_definition_list *>(rr);
  List<Create_field> *sd = dynamic_cast<List<Create_field> *>(rdl);
...
Create_field *Row_definition_list::find_row_field_by_offset(uint offset) const {
  // Cast-off the "const" qualifier
  const Row_definition_list *rr = this;
  Row_definition_list *rdl = const_cast<Row_definition_list *>(rr);
  List<Create_field> *sd = dynamic_cast<List<Create_field> *>(rdl);
