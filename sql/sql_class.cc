...
THD::THD(bool enable_plugins)
    : Query_arena(&main_mem_root, STMT_REGULAR_EXECUTION),
      ...
      binlog_should_commit_by_rotate(false),
      ...
      is_wait_gtid_set_timeout(false),
  ...
  is_internal_con_no_result = false;
  main_lex->reset();
  set_psi(nullptr);
  ...
  if (level == Sql_condition::SL_ERROR) {
    ...
    */
    ...
    if (!da->is_error()) {
      ...
      da->set_error_status(sql_errno, msg, sqlstate, user_defined);
    }
  }
...
void THD::cleanup_connection(void) {
  ...
  sp_cache_clear(&sp_ora_type_cache);
...
bool THD::is_normal_slave() const {
  return rli_slave && !Multisource_info::is_group_replication_channel_name(
                          rli_slave->get_channel());
}

bool THD::is_mgr_slave() const {
  return rli_slave && Multisource_info::is_group_replication_channel_name(
                          rli_slave->get_channel());
}
...
void THD::change_item_tree(Item **place, Item *new_value) {
  ...
  if (!stmt_arena->is_regular() &&
      lex->is_exec_started() /*connect_by compile func_prior in preparation*/) {
...
void THD::end_statement() {
  ...
  */
  // clean current sp_rcontext for next sp
  sp_rcontext_layers.clear();
}
...
void THD::set_greatdb_plan_baseline_info(Query_expression *unit) {
#ifdef HAVE_PSI_THREAD_INTERFACE
  greatdb_plan_baseline::GDB_plan_baseline_get_explain(this, unit);
#endif
}
...
bool THD::sql_parser() {
  ...
  */
  ...
  extern int ora_sql_parser_parse(class THD * thd,
                                  class Parse_tree_root * *root);

  Parse_tree_root *root = nullptr;
  bool mysql_parse_status = ((this->variables.sql_mode & MODE_ORACLE)
                                 ? ora_sql_parser_parse(this, &root)
                                 : my_sql_parser_parse(this, &root)) != 0;

  if (mysql_parse_status || is_error()) {
    /*
...
void THD::cleanup_after_parse_error() {
  ...
  if (sp) {
    ...
    // sp_cache_reentrant() may call lock_routine_name() indirectly.
    // release the lock for such case.
    // ref. comment in sp_start_package_parsing()
    sp->rollback(this);

    //  Do not delete sp_head if is invoked in the context of sp execution.
    if (sp_runtime_ctx == nullptr) {
      sp_package *pkg;
      if ((pkg = lex->sphead->m_parent)) {
        /*
          If a syntax error happened inside a package routine definition,
          then lex points to the routine sublex. It is necessary to
          restore to the top level LEX.
        */
        assert(pkg->m_top_level_lex);
        assert(pkg == pkg->m_top_level_lex->sphead);
        pkg->m_parser_data.finish_parsing_sp_body(this);

        // ref. comment in sp_start_package_parsing()
        pkg->rollback(this);

        /*
          proc error before adding into package.
          destroy package will not release this proc.
          destroy this proc manually.
          lex may not be the top lex of the sp.
          so destroy the sp to let thd->lex point to the topest lex.
        */
        sp_head::destroy(sp);
        lex->sphead = nullptr;
        lex_end(lex);
  ...
  if (lex->m_with_func) lex->clear_with_fuction_flag();
}
...
void Query_dumpvar_param_rela::Field_attr::init() {
  is_lob = false;
  table_name.clear();
  field_name.clear();
  db_name.clear();
  item_type = Item::INVALID_ITEM;
}

Query_dumpvar_param_rela::Query_dumpvar_param_rela() {
  field.init();
  query_expr_cond.clear();
  is_multi_table = false;
  is_updating = false;
  is_rownum_or_limit = false;
  is_query_dumpvar_param_rela_null = false;
  origin_lob_content.clear();
  check_result_flag = true;
  table_miss_pk = false;
  table_pk_type_invalid = false;
  where_cond_miss_pk = false;
}

Sp_rcontext_layer::Item_expr_holder::Item_expr_holder() {
  item_name.clear();
  item_type = Sp_rcontext_layer::Item_expr_holder::INVALID;
  vals.llval = 0;
  vals.strval = nullptr;
}

Sp_rcontext_layer::Item_expr_holder::Item_type
Sp_rcontext_layer::Item_expr_holder::get_item_type(enum_field_types type) {
  Sp_rcontext_layer::Item_expr_holder::Item_type item_type =
      (is_integer_type(type) ? Sp_rcontext_layer::Item_expr_holder::INTEGER_TYPE
       : (is_string_type(type) || type == MYSQL_TYPE_DECIMAL ||
          type == MYSQL_TYPE_NEWDECIMAL)
           ? Sp_rcontext_layer::Item_expr_holder::STRING_TYPE
           : Sp_rcontext_layer::Item_expr_holder::INVALID);
  return item_type;
}

bool Sp_rcontext_layer::find_sp_rcontext(sp_rcontext *rctx) const {
  return this->get_sp_rcontext() == rctx;
}

Sp_rcontext_layer::Sp_rcontext_layer(sp_rcontext *rctx,
                                     enum_sp_rcontext_type sp_rcontext_type) {
  this->rctx = rctx;
  this->sp_rcontext_type = sp_rcontext_type;
  sp_variable_list.clear();
  query_dumpvar_map.clear();

  calc_var_level();
}

void Sp_rcontext_layer::calc_var_level() {
  if (rctx != nullptr) {
    List<sp_variable> *vars = rctx->get_sp_variable_list();
    List_iterator_fast<sp_variable> li(*vars);
    sp_variable *var;
    while ((var = li++)) {
      Item *ref_var = var->ref_actual_parameter;
      if (ref_var != nullptr) {
        std::string current_var_name(var->name.str, var->name.length);
        std::string parent_var_name(ref_var->item_name.ptr(),
                                    ref_var->item_name.length());
        sp_variable_list.push_back(Sp_rcontext_layer::Sp_variable_level{
            current_var_name, parent_var_name});
      }
    }
  }
}

Query_dumpvar_param_rela *Sp_rcontext_layer::find_ref_actual_variable(
    THD *thd, const std::string var_name) {
  Query_dumpvar_param_rela *param_rela = nullptr;
  std::string var(var_name);
  std::deque<Sp_rcontext_layer>::reverse_iterator riter;
  for (riter = thd->sp_rcontext_layers.rbegin();
       riter != thd->sp_rcontext_layers.rend(); riter++) {
    std::deque<Sp_rcontext_layer::Sp_variable_level> &sp_variable_list =
        *(*riter).get_sp_variable_list();

    if (!sp_variable_list.size()) continue;

    std::deque<Sp_rcontext_layer::Sp_variable_level>::iterator iter =
        std::find_if(sp_variable_list.begin(), sp_variable_list.end(),
                     Sp_rcontext_layer::Sp_variable_level(var));
    if (iter != sp_variable_list.end()) {
      var.assign(iter->parent_var_name);
    } else {
      continue;
    }

    param_rela = Sp_rcontext_layer::match_variable(thd, var);
...
void Sp_rcontext_layer::push_param(THD *thd, std::string var_key,
                                   Query_dumpvar_param_rela param) {
  std::deque<Sp_rcontext_layer>::iterator iter = std::find_if(
      thd->sp_rcontext_layers.begin(), thd->sp_rcontext_layers.end(),
      std::bind(std::function<bool(const Sp_rcontext_layer &, sp_rcontext *)>(
                    &Sp_rcontext_layer::find_sp_rcontext),
                std::placeholders::_1, thd->sp_runtime_ctx));

  if (iter != thd->sp_rcontext_layers.end()) {
    std::map<std::string, Query_dumpvar_param_rela> &query_dumpvar_map =
        *(iter->get_query_dumpvar_map());
    std::pair<std::map<std::string, Query_dumpvar_param_rela>::iterator, bool>
        ret = query_dumpvar_map.emplace(var_key, param);
    if (!ret.second) {
      query_dumpvar_map.erase(var_key);
      query_dumpvar_map.emplace(var_key, param);
    }
  }
}

Query_dumpvar_param_rela *Sp_rcontext_layer::match_variable(
    THD *thd, const std::string field_name) {
  // match select .. into var or select .. into @var
  Query_dumpvar_param_rela *param_rela = nullptr;
  if (thd->sp_rcontext_layers.size()) {
    std::deque<Sp_rcontext_layer>::reverse_iterator riter =
        thd->sp_rcontext_layers.rbegin() + 1;
    while (riter != thd->sp_rcontext_layers.rend()) {
      std::map<std::string, Query_dumpvar_param_rela> &query_dumpvar_map =
          *(riter->get_query_dumpvar_map());
      if (query_dumpvar_map.size()) {
        std::map<std::string, Query_dumpvar_param_rela>::iterator iter =
            query_dumpvar_map.find(field_name);
        if (iter != query_dumpvar_map.end()) {
          param_rela = &iter->second;
          break;
        }
      }
      riter++;
...
bool find_pk_item_type(const Sp_rcontext_layer::Item_expr_holder &holder) {
  return holder.item_type ==
             Sp_rcontext_layer::Item_expr_holder::INTEGER_TYPE ||
         holder.item_type == Sp_rcontext_layer::Item_expr_holder::STRING_TYPE;
}

std::string Sp_rcontext_layer::locate_rec(THD *thd, Table_ref *table_list,
                                          Item *current_where,
                                          Query_dumpvar_param_rela *param) {
  std::string query_expr_cond(""), field_name("");

  if (!current_where) {
    param->where_cond_miss_pk = true;
    return query_expr_cond;
  }

  std::ostringstream oss("");
  uint primary_key = 0;
  KEY *key_info = (table_list ? table_list->table->key_info : nullptr);
  TABLE_SHARE *share = (table_list ? table_list->table->s : nullptr);
  std::set<std::string> set_pk_field, table_pk_field, where_cond_field;
  std::deque<Sp_rcontext_layer::Item_expr_holder> pk_items;
  std::map<std::string, Item *> cond_item;
  bool found_primary = false;

  if (!key_info || !share) {
    param->table_miss_pk = true;
    return query_expr_cond;
  }
  ...
  for (uint i = 0; i < share->keys; i++, key_info++) {
    ...
    if (!found_primary) continue;
    for (uint j = 0; j < key_info->user_defined_key_parts; j++, key_part++) {
      if (key_part->field &&
          !key_part->field->is_field_for_functional_index()) {
        Sp_rcontext_layer::Item_expr_holder holder;
        field_name.assign(key_part->field->field_name);
        std::transform(field_name.begin(), field_name.end(), field_name.begin(),
                       ::tolower);
        holder.item_name.assign(field_name);
        table_pk_field.emplace(holder.item_name);
        holder.item_type =
            Item_expr_holder::get_item_type(key_part->field->type());
        pk_items.push_back(holder);
      }
    }
  }

  if (!table_pk_field.size()) {
    param->table_miss_pk = true;
    return query_expr_cond;
  }

  mem_root_deque<Item_field *> field_list(thd->mem_root);
  current_where->walk(&Item::collect_item_field_processor, enum_walk::POSTFIX,
                      (uchar *)&field_list);
  for (Item_field *cur_item : field_list) {
    if (!cur_item->fixed || cur_item->type() != Item::FIELD_ITEM) continue;
    field_name.assign(cur_item->field->field_name);
    std::transform(field_name.begin(), field_name.end(), field_name.begin(),
                   ::tolower);
    where_cond_field.emplace(field_name);
    cond_item.emplace(field_name, cur_item);
  }

  if (!where_cond_field.size()) {
    param->where_cond_miss_pk = true;
    return query_expr_cond;
  }

  std::set_intersection(table_pk_field.begin(), table_pk_field.end(),
                        where_cond_field.begin(), where_cond_field.end(),
                        std::insert_iterator<std::set<std::string>>(
                            set_pk_field, set_pk_field.begin()));

  if (!set_pk_field.size() || set_pk_field.size() != table_pk_field.size()) {
    param->where_cond_miss_pk = true;
    return query_expr_cond;
  }

  std::deque<Sp_rcontext_layer>::iterator rctx_iter = std::find_if(
      thd->sp_rcontext_layers.begin(), thd->sp_rcontext_layers.end(),
      std::bind(std::function<bool(const Sp_rcontext_layer &, sp_rcontext *)>(
                    &Sp_rcontext_layer::find_sp_rcontext),
                std::placeholders::_1, thd->sp_runtime_ctx));

  if (rctx_iter != thd->sp_rcontext_layers.end()) {
    std::deque<Sp_rcontext_layer::Item_expr_holder>::iterator item_iter =
        std::find_if(
            pk_items.begin(), pk_items.end(),
            std::not1(std::function<bool(
                          const Sp_rcontext_layer::Item_expr_holder &)>(
                find_pk_item_type)));

    if (item_iter != pk_items.end()) {
      param->table_pk_type_invalid = true;
      return query_expr_cond;
    }
  } else {
    param->check_result_flag = false;
    return query_expr_cond;
  }

  std::set<std::string>::iterator pk_iter = set_pk_field.begin();
  oss << Sp_rcontext_layer::Item_expr_holder::COND_WHERE
      << gen_field_expr(*pk_iter, &cond_item);
  for (++pk_iter; pk_iter != set_pk_field.end(); ++pk_iter) {
    oss << Sp_rcontext_layer::Item_expr_holder::COND_AND;
    oss << gen_field_expr(*pk_iter, &cond_item);
  }

  query_expr_cond.assign(oss.str());
  oss.clear();
  return query_expr_cond;
}

std::string Sp_rcontext_layer::gen_field_expr(
    const std::string field_name, std::map<std::string, Item *> *cond_item) {
  std::string field_expr("");
  std::ostringstream oss("");
  Item *pk_item = nullptr;
  Item_field *pk_item_field = nullptr;
  String temp;
  ...
  std::map<std::string, Item *>::iterator iter = cond_item->find(field_name);
  if (cond_item->size() && iter != cond_item->end()) {
    pk_item = iter->second;
    if (pk_item->type() != Item::FIELD_ITEM) return field_expr;
    pk_item_field = pointer_cast<Item_field *>(pk_item);
    Sp_rcontext_layer::Item_expr_holder::Item_type item_type =
        Item_expr_holder::get_item_type(pk_item_field->field->type());

    switch (item_type) {
      case Sp_rcontext_layer::Item_expr_holder::INTEGER_TYPE: {
        oss << pk_item_field->field_name
            << Sp_rcontext_layer::Item_expr_holder::COND_EQ
            << pk_item_field->val_int();
        break;
      }
