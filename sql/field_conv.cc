...
void Copy_field::invoke_do_copy(bool reverse) {
  ...
  Field_udt_type *udt_field_to = dynamic_cast<Field_udt_type *>(m_to_field);
  Field_udt_type *udt_field_from = dynamic_cast<Field_udt_type *>(m_from_field);
  if ((!udt_field_to && udt_field_from) || (udt_field_to && !udt_field_from)) {
    my_error(ER_NOT_SUPPORTED_YET, MYF(0), "conversion of udt/non-udt column");
    return;
  }
