...
class Item_func_vector_dim : public Item_int_func {
 ...
 public:
  ...
  longlong val_int() override;
  enum Functype functype() const override { return VECTOR_DIM; }
...
class Item_func_vec_distance : public Item_real_func {
 public:
  Item_func_vec_distance(const POS &pos, Item *ia, Item *ib)
      : Item_real_func(pos, ia, ib) {
    set_nullable(true);
  }

  Item_func_vec_distance(const POS &pos, Item *ia, Item *ib, Item *ic)
      : Item_real_func(pos, ia, ib, ic) {
    set_nullable(true);
  }
  ...
  bool resolve_type(THD *thd) override;
  double val_real() override;
  enum Functype functype() const override { return VECTOR_DISTANCE; }
};
...
class Item_func_table_count final : public Item_int_func {
 ...
 public:
  explicit Item_func_table_count(const POS &pos, uint spv_idx,
                                 const Sp_rcontext_handler *rh)
      : Item_int_func(pos), m_spv_idx(spv_idx), m_rcontext_handler(rh) {}
...
class Item_func_sp final : public Item_func {
  ...
  */
  ...
  Item **args_udt{nullptr};
  uint arg_udt_count{0};
  // used for cleanup()
  MEM_ROOT m_return_field_mem_root{key_memory_sp_head_execute_root,
                                   MEM_ROOT_BLOCK_SIZE};
  Query_arena m_return_field_arena{&m_return_field_mem_root,
                                   Query_arena::STMT_EXECUTED};
  // used for recall sp
  MEM_ROOT m_return_field_tmp_mem_root{key_memory_sp_head_execute_root,
                                       MEM_ROOT_BLOCK_SIZE};
  Query_arena m_return_field_tmp_arena{&m_return_field_tmp_mem_root,
                                       Query_arena::STMT_EXECUTED};
  List<Create_field> *m_rdl{nullptr};
  Row_definition_table_list *m_rdl_table{nullptr};
  bool execute();
  ...
  */
  ...
  void cleanup_for_refcursor() override {
    if (is_ora_udt_type() && sp_result_field) {
      sp_result_field->~Field();
  ...
  Item *element_index(uint i) override {
    return arg_udt_count ? args_udt[i] : this;
  }
  Item **addr(uint i) override { return arg_udt_count ? args_udt + i : NULL; }
  const CHARSET_INFO *charset_for_protocol() override {
  ...
  bool is_rowtype() override { return sp_result_field->is_rowtype_table(); }
  void compute_cost(CostOfItem *root_cost) const override {
    root_cost->MarkExpensive();
  }
};
...
class Item_func_cursor_rowcount : public Item_int_func, public Cursor_ref {
 public:
  Item_func_cursor_rowcount(const POS &pos, LEX_CSTRING name, uint offset,
                            ...
                            const Sp_rcontext_handler *rh)
      : Item_int_func(pos), Cursor_ref(name, offset, cursor_spv, rh) {}
