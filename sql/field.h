...
class Field {
 ...
 public:
  ...
  TABLE *m_udt_table{nullptr};
  // rcontext's callers_arena,it maybe package body's or head's callers_arena
  Query_arena *m_callers_arena{nullptr};
  /**
  ...
  */
  ...
  virtual type_conversion_status store_udt(TABLE *, Item *) {
    assert(false);
    return TYPE_ERR_BAD_VALUE;
  }
  virtual TABLE *val_udt() { return m_udt_table; }
  virtual void set_rowtype_table_ident(Table_ident *) {}
  virtual bool is_rowtype_table() { return false; }
  ...
  */
  virtual uint32 data_length(ptrdiff_t row_offset [[maybe_unused]] = 0) const {
    if (type() == MYSQL_TYPE_NULL) return 0;
    return pack_length();
  }
  ...
  */
  bool is_null_in_table(ptrdiff_t row_offset = 0) const {
    /*
  ...
  virtual bool is_null(ptrdiff_t row_offset = 0) const {
    return is_null_in_table(row_offset);
  }

  virtual bool is_empty_string(ptrdiff_t row_offset = 0) const {
    (void)row_offset;
    return false;
  }
  ...
  */
  ...
  virtual void set_null(ptrdiff_t row_offset = 0);

  virtual void set_notnull(ptrdiff_t row_offset = 0);
...
class Field_new_decimal : public Field_num {
  ...
  */
  ...
  bool get_keep_precision() { return m_keep_precision; }
};
...
class Field_row : public Field_null {
 protected:
  ...
  // for var table%rowtype,it doesn't have udt name
  Table_ident *m_rowtype_table_ident;

 public:
  Field_row(uchar *ptr_arg, const char *field_name_arg)
      : Field_null(ptr_arg, 0, Field::NONE, field_name_arg, &my_charset_bin),
        m_cdf_default_list(nullptr),
        arg_count(0),
        m_udt_name(NULL_STR),
        m_udt_db_name(NULL_STR),
        m_rowtype_table_ident(nullptr),
        m_nested_table_udt(NULL_CSTR),
  ...
  void set_udt_value_initialized(bool init) override {
    ...
    for (uint j = 0; j < m_udt_table->s->fields; j++) {
      m_udt_table->field[j]->set_udt_value_initialized(init);
    }
  }
  ...
  void sql_type(String &str) const override;
  type_conversion_status store_udt(TABLE *table, Item *item) override;
  void create_udt_table(THD *thd, List<Create_field> *fld_list);
  uint32 pack_length() const override {
    if (!m_udt_table || !m_udt_table->s) return 0;
    return m_udt_table->s->reclength;
  }
  void set_rowtype_table_ident(Table_ident *ident) override {
    m_rowtype_table_ident = ident;
  }
  bool is_rowtype_table() override {
    return m_rowtype_table_ident ? true : false;
  }
...
class Field_refcursor final : public Field_row {
 ...
 public:
  ...
  bool set_cursor_return_type_from_other_refcursor(
      Item_field_refcursor *item_field);
  TABLE *val_udt() override { return m_return_table->get_table(); }
...
class Field_row_table final : public Field_row {
 ...
 public:
  ...
  bool m_is_index_by;
  // for bulk collect into : a temp Table_function_record to save data from
  // value.
  Table_function_record *tmp_table_record;
  // forall insert : a temp Table_function_record to save data from forall.
  Table_function_record *forall_table_record;
 ...
 public:
  Field_row_table(uchar *ptr_arg, const char *field_name_arg, int index_length)
      : Field_row(ptr_arg, field_name_arg),
        ...
        m_is_index_by(false),
        tmp_table_record(nullptr),
        forall_table_record(nullptr) {}
  ...
  bool create_tmp_record_table_to_store_bulk_data(THD *thd, const char *alias,
                                                  List<Create_field> vt_list,
                                                  Item *forall_i,
                                                  bool is_str_index);
  bool fill_bulk_table_with_tmp_table();
  void set_udt_value_initialized(bool init) override;
  bool resolve_forall_table_record(THD *thd, Item *forall_i);
};
...
class Field_string : public Field_longstr {
 public:
  ...
  bool is_null(ptrdiff_t row_offset = 0) const override;
  bool is_empty_string(ptrdiff_t row_offset = 0) const override;
...
class Field_varstring : public Field_longstr {
 public:
  ...
  bool is_null(ptrdiff_t row_offset = 0) const override;
  bool is_empty_string(ptrdiff_t row_offset = 0) const override {
    return data_length(row_offset) == 0;
  }
...
class Field_udt_type : public Field_varstring {
  LEX_CSTRING m_ident;
 ...
 public:
  ...
  Field_udt_type *clone(MEM_ROOT *mem_root) const override {
    assert(type() == MYSQL_TYPE_VARCHAR);
    return new (mem_root) Field_udt_type(*this);
  }
  /*
    Field(const Item &) = delete;
    The original intention was seemingly for Field to be non-copyable,
    ...
    implicit copy constructor for 'Field_udt_type'
...
Field *make_field(
    MEM_ROOT *mem_root_arg, TABLE_SHARE *share, uchar *ptr, size_t field_length,
    ...
    std::optional<gis::srid_t> srid, bool is_array, bool is_rowtype_ref = false,
    bool is_row_table = false, LEX_CSTRING *udt_name = nullptr,
    LEX_STRING db_name = NULL_STR, int index_length = 0,
    dd::Column::enum_hidden_type hidden =
        dd::Column::enum_hidden_type::HT_VISIBLE,
    bool is_ref_cursor = false, Table_ident *table_rowtype_ref = nullptr,
