...
int group_concat_key_cmp_with_distinct(const void *arg, const void *key1,
...
#ifndef NDEBUG
  uint field_cnt = table->visible_field_count();
#endif
  ...
  for (uint i = 0; i < item_func->m_field_arg_count; i++) {
    ...
    */
    Field *field;
    if (!item_func->m_window)
      field = item->get_tmp_table_field();
    else {
      // @@NOTE@@ 'i >= field_cnt' should not happen
      assert(i < field_cnt);
      field = table->visible_field_ptr()[i];
    }
...
int dump_leaf_key(void *key_arg, element_count count [[maybe_unused]],
  ...
  /*
     ...
     the maximum size.
  */
  if (item->result_limitation_check(old_length)) return 1;
  return 0;
}
...
Item_func_group_concat::Item_func_group_concat(const POS &pos,
  ...
  orafun = from_orafun;
  if (w) {
    // in yacc file, error will be thrown if Window::m_order_by is set.
    if (orafun != 2)
      assert(!w->effective_order_by());
    else
...
bool Item_func_group_concat::add() {
  ...
  for (uint i = 0; i < m_field_arg_count; i++) {
    ...
    /*
      When orafun is true or emptystring_equal_null mode, if field is an empty
      string, return false.
    */
    if ((orafun ||
         (current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL))) {
      if ((field->is_null() || field->is_empty_string()))
        return false;  // Skip row if it contains empty string
    }
  }
...
bool Item_func_group_concat::fix_fields(THD *thd, Item **ref) {
  PT_window *saved_win = m_window;
  PT_order_list *ord = nullptr;
  if (saved_win) ord = saved_win->get_order_list();

  if (super::fix_fields(thd, ref)) return true;

  if (saved_win && m_window && m_window != saved_win) {
    if (m_window->effective_order_by() != nullptr) {
      my_error(ER_CLAUSE_IN_LISTAGG_NOT_ALLOWED, MYF(0), "ORDER BY",
               func_name());
      return true;
    }
    if (!m_window->frame()->m_originally_absent) {
      my_error(ER_CLAUSE_IN_LISTAGG_NOT_ALLOWED, MYF(0), "window frame",
               func_name());
      return true;
    }
...
bool Item_func_group_concat::open_temp_table(THD *thd) {
  assert(m_is_window_function);

  if (distinct && table == nullptr) {
    // if group_concat is used as window function,
    // it is necessary to create tmp table to handle distinct.
    // no record is stored in temp table. it is used to collect the meta info
    // and data of columns for group_concat(). unique_filter will use the meta
    // info to compare the data of columns.
    // ref. Item_func_group_concat::setup().
    // NOTE: in prepare statement, cleanup() is called after prepared.
    // i.e. table will be freed.
    // for aggregate function, setup() will be called to create temp table.
    // for analytic function, open_temp_table() is used in val_str().
    ...
    tmp_table_param->func_count = m_field_arg_count;
    ...
    // unique_filter will store all unique values.
    // if the number of the elements of unique_filter is the same after
    // inserting a new value, the new value is duplicated and is ignored
    // for the distinct output.
...
String *Item_func_group_concat::val_str(String *str) {
  assert(fixed);
  if (m_is_window_function) {
    // null_value is set on Item_func_group_concat::clear().
    if (m_null_resolved) return nullptr;

    if (wf_common_init()) {
      null_value = true;
      return &result;
    }
    if (open_temp_table(current_thd)) {
      null_value = true;
      return &result;
    }

    // length too long, it should be stopped concating data
    if (warning_for_row) return &result;

    // flds is used for distinct only.
    Field **flds = nullptr;
    if (distinct) flds = table->visible_field_ptr();

    size_t old_length = result.length();

    // if any arg is null, nothing append, previous result is used.
    for (uint i = 0; i < m_field_arg_count; i++) {
      Item *show_item = args[i];
      if (show_item->const_item()) continue;
      if (show_item->is_null()) return &result;
      /*
        When orafun is true or emptystring_equal_null mode，if field is an
        empty string return current result.
      */
      if ((orafun ||
           (current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL)) &&
          is_string_type(show_item->data_type())) {
        StringBuffer<MAX_FIELD_WIDTH> tmp;
        String *field_result = show_item->val_str(&tmp);
        if (field_result->is_empty())
          return &result;  // Skip row if it contains empty string
      }

      // save args[i] into the temp table record buffer, table->record[0].
      if (flds) show_item->save_in_field(flds[i], false);
    }

    if (distinct) {
      uint count = unique_filter->elements_in_tree();
      unique_filter->unique_add(table->record[0] + table->s->null_bytes);
      uint c2 = unique_filter->elements_in_tree();
      // if duplicate entry is found, nothing append, previous result is used.
      if (count == c2) return &result;
    }

    row_count++;

    // Window framework does the trick,
    // the result is appended for each iteration.
    for (uint i = 0; i < m_field_arg_count; i++) {
      Item *show_item = args[i];

      // how about to check the length of res ?
      String *res = show_item->val_str(str);
      if (!m_result_finalized)
        m_result_finalized = true;
      else if (separator != nullptr)
        result.append(*separator);
      result.append(*res);
    }

    // in clear(), null_value will be set to true as default value.
    null_value = false;

    result_limitation_check(old_length);
  ...
  /*
    When oracle fun and emptystring_equal_null mode, if result is empty string,
    return nullptr
  */
  if ((orafun ||
       (current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL)) &&
...
bool Item_func_group_concat::check_wf_semantics1(
    THD *thd, Query_block *select, Window_evaluation_requirements *r) {
  const PT_order_list *eord;
  PT_order_list *ord = m_window->get_order_list();
  ...
  return Item_sum::check_wf_semantics1(thd, select, r);
}
...
void Item_func_group_concat::print(const THD *thd, String *str,
  ...
  if (orafun == 1) {
    ...
    if (m_window) {
      ...
      PT_order_list *ord = m_window->get_order_list();
      if (ord) {
        str->append(STRING_WITH_LEN(" within group (order by "));
        Query_block::print_order(thd, str, ord->value.first, query_type);
        str->append(STRING_WITH_LEN(")"));
      }
      assert(m_order_arg_count == 0);
    }
    if (m_order_arg_count > 0) {
      str->append(STRING_WITH_LEN(" within group (order by "));
      for (uint i = 0; i < m_order_arg_count; i++) {
  ...
  if (m_window) {
    // In group_concat, Window::m_order_by is not set by window_spec, but it is
    // set by within-group-order-by clause.
    // m_window->set_order_list_print(false) was called before.
    str->append(" OVER ");
    m_window->print(thd, str, query_type, false);
  }
}

bool Item_func_group_concat::result_limitation_check(size_t old_length) {
  if (result.length() <= group_concat_max_len) return false;
  ...
  size_t add_length;
  /*
    It's ok to use result.length() as the fourth argument
    as this is never used to limit the length of the data.
    Cut is done with the third argument.
  */
  add_length = cs->cset->well_formed_len(cs, ptr + old_length,
                                         ptr + group_concat_max_len,
...
bool Item_func_grouping::fix_fields(THD *thd, Item **ref) {
  ...
  */
  if (!m_query_block->is_non_primitive_grouped() ||
      m_query_block->connect_by_cond() ||
      m_query_block->resolve_place == Query_block::RESOLVE_JOIN_NEST ||
      m_query_block->resolve_place == Query_block::RESOLVE_CONDITION ||
      m_query_block->resolve_place == Query_block::RESOLVE_START_WITH ||
      m_query_block->resolve_place == Query_block::RESOLVE_CONNECT_BY) {
    my_error(ER_INVALID_GROUP_FUNC_USE, MYF(0));
    return true;
  }
...
/***CONNECT BY BEGIN ***/
bool Item_connect_by_func::do_itemize(Parse_context *pc, Item **res) {
  if (super::do_itemize(pc, res)) return true;
      ...
      [thd, select](Item *it) -> Item * {
        ...
        switch (it->type()) {
          ...
          case Item::FIELD_ITEM: {
            ...
            if (find_item_in_list(thd, it, select->get_fields_list(),
...
bool Item_connect_by_func_value::do_itemize(Parse_context *pc, Item **res) {
  if (super::do_itemize(pc, res)) return true;
...
bool Item_connect_by_func_value::resolve_type_inner(THD *thd) {
  ...
  */
  if (find_item_in_list(thd, args[0], select->get_fields_list(), &select_ref,
                        &counter, &resolution))
    return true; /* Some error occurred. */

  if (select_ref == nullptr) {
...
bool Item_connect_by_func_root::do_itemize(Parse_context *pc, Item **res) {
  if (super::do_itemize(pc, res)) return true;
...
bool Item_connect_by_func_sys_path::do_itemize(Parse_context *pc, Item **res) {
  if (super::do_itemize(pc, res)) return true;
...
void Item_connect_by_func_sys_path::cleanup() {
  ...
  if (table) {
    if (table->blob_storage) ::destroy_at(table->blob_storage);
    close_tmp_table(table);
    free_tmp_table(table);
    table = nullptr;
  }
...
bool Item_connect_by_func_iscycle::do_itemize(Parse_context *pc, Item **res) {
  if (super::do_itemize(pc, res)) return true;
  if (pc->select->parsing_place == CTX_START_WITH ||
...
bool Item_connect_by_func_isleaf::do_itemize(Parse_context *pc, Item **res) {
  if (super::do_itemize(pc, res)) return true;
  if (pc->select->parsing_place == CTX_START_WITH ||
      pc->select->parsing_place == CTX_CONNECT_BY) {
    my_error(ER_PSEUDOCOLUMN_NOT_ALLOW, MYF(0), func_name(), pc->thd->where);
