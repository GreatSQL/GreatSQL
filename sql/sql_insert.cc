...
bool check_insert_fields(THD *thd, Table_ref *table_list,
...
bool check_valid_table_refs(const Table_ref *view,
  ...
  {  // Statement plan is available within these braces
    ...
    */
    ...
    if (query_block->sequences_counter) {
      query_block->sequences_counter->Reset();
    }
  ...
  */
  ...
  if (!select_insert) {
    ...
    */
    ...
    // For example:insert into table_of select
    // udt(),column_count=0,field_count != unit->num_visible_fields()
    if (!(!column_count && insert_table->s->udt_name.str &&
          unit->num_visible_fields() == 1) &&
        field_count != unit->num_visible_fields()) {
      my_error(ER_WRONG_VALUE_COUNT_ON_ROW, MYF(0), 1);
      return true;
    }

    // For example:INSERT INTO tab2(type) SELECT t_air1('a');
    ...
    if (field_count == 1 && column_count == 1 &&
      ...
      Item *tmp = unit->first_query_block()->fields.front()->this_item();
      bool is_udt = tmp->is_ora_type() &&
                    (tmp->result_type() == ROW_RESULT || tmp->get_udt_table());
      if (insert_field_list.front()->this_item()->is_ora_type() != is_udt) {
...
static TABLE *create_table_from_items(THD *thd, HA_CREATE_INFO *create_info,
  ...
  for (Item *item : VisibleFields(items)) {
    ...
    if (create_info->used_fields & HA_CREATE_MATERIALIZED_VIEW) {
      if (strncasecmp(create_field->field_name,
                      STRING_WITH_LEN(HIDDEN_IMV_PREFIX)) == 0) {
        create_field->hidden = dd::Column::enum_hidden_type::HT_HIDDEN_USER;
      }
    }
...
bool Sql_cmd_insert_all::precheck(THD *thd) {
  DBUG_TRACE;
  Table_ref *tables = lex->query_tables;
  for (uint i = 0; i < into_table_count; i++, tables = tables->next_global) {
    if (check_single_table_access(thd, INSERT_ACL, tables, false)) return true;
  }
...
bool Sql_cmd_insert_all::check_privileges(THD *thd) {
  DBUG_TRACE;
  if (check_all_table_privileges(thd)) return (true);

  for (uint i = 0; i < into_table_count; i++) {
    if (check_privileges_for_list(thd, *fields_for_table[i], INSERT_ACL))
      return true;
  }
...
bool Sql_cmd_insert_all::restore_cmd_properties(THD *thd) {
  lex->restore_properties_for_insert();
  return Sql_cmd_dml::restore_cmd_properties(thd);
}

// for rewrite plugins
bool Sql_cmd_insert_all::accept(THD *thd, Select_lex_visitor *visitor) {
  for (uint i = 0; i < into_table_count; i++) {
    for (auto field : *fields_for_table[i]) {
      if (walk_item(field, visitor)) return true;
    }
  }
...
bool Sql_cmd_insert_all::prepare_inner(THD *thd) {
  DBUG_TRACE;
  Query_expression *const unit = lex->unit;
  Query_block *const select = lex->query_block;
  ...
  Name_resolution_context *const context = &select->context;
  Name_resolution_context_state ctx_state;
  // firset into_table_count is insert target table
  Table_ref *const insert_table_first = lex->query_tables;
  lex->insert_table = insert_table_first;

  // not support view table in insert all
  auto insert_table_end_ptr = lex->query_tables;

  for (auto i = into_table_count; i > 1; i--) {
    if (insert_table_end_ptr->is_view()) {
      my_error(ER_INSERT_ALL_NOT_USE_VIEW, MYF(0));
      return true;
    }
    insert_table_end_ptr = insert_table_end_ptr->next_local;
  }
  ...
  ctx_state.save_state(context, insert_table_first);

  DBUG_PRINT("enter", ("table_list %p,  count %d", insert_table_first,
                       into_table_count));

  lex->in_update_value_clause = false;
  Table_ref *first_query_block_table = insert_table_end_ptr->next_local;
  insert_table_end_ptr->next_local = nullptr;

  for (auto tb = insert_table_first; tb != nullptr; tb = tb->next_local) {
    // setup fields need set insert table ref
    tb->next_name_resolution_table = nullptr;
  }

  // only set the insert tables
  if (select->setup_tables(thd, insert_table_first, true)) {
    return true;
  }

  // checkout insert table is insert
  for (auto tb = insert_table_first; tb != nullptr; tb = tb->next_local) {
    if (!tb->is_insertable()) {
      my_error(ER_NON_INSERTABLE_TABLE, MYF(0), tb->alias, "INSERT");
      return true;
    }
  }

  // Prepare the lists of columns and values in the statement.
  // if col is nullptr will fill
  auto insert_tb_idx = 0;
  for (auto tb = insert_table_first; tb != nullptr; tb = tb->next_local) {
  ...
  for (uint i = 0; i < into_table_count; i++) {
    for (auto itr : *fields_for_table[i]) {
      if (insert_field_list.push_back(itr)) {
        return true;
      }
    }
  }
  ...
  */
  ...
  // Remove the insert table from the first query block
  select->m_table_list.first = context->table_list =
      context->first_name_resolution_table = first_query_block_table;
  ...
  if (insert_field_list.size() != unit->num_visible_fields()) {
    my_error(ER_WRONG_VALUE_COUNT_ON_ROW, MYF(0), 1);
    return true;
  }
  uint offset_values = 0;
  List_item tmp_values(thd->mem_root);
  auto unit_column_types = unit->get_unit_column_types();

  for (auto tr = insert_table_first; tr != nullptr; tr = tr->next_local) {
    // get tmp value
    if ((tr->table->has_gcol() || tr->table->gen_def_fields_ptr)) {
      tmp_values.clear();
      for (uint col_i = 0; col_i < fields_for_table[tr->shared]->size();
           col_i++) {
        tmp_values.push_back(unit_column_types->at(offset_values++));
      }
      if (validate_gc_assignment(*fields_for_table[tr->shared], tmp_values,
                                 tr->table))
        return true;
    }
    /*
    ...
    fields as used.
    */
    if (tr->table->triggers) {
      if (tr->table->triggers->mark_fields(TRG_EVENT_INSERT)) return true;
    }
  }

  assert(offset_values != insert_field_list.size());
  // The insert table should be a separate name resolution context
  // assert(table_list->next_name_resolution_table == nullptr);
  ...
  // Restore the insert table and the name resolution context
  select->m_table_list.first = context->table_list = insert_table_first;
  insert_table_end_ptr->next_local = first_query_block_table;
  ctx_state.restore_state(context, insert_table_first);
...
bool Query_result_insert_all::prepare(THD *thd, const mem_root_deque<Item *> &,
                                      Query_expression *u) {
  DBUG_TRACE;

  unit = u;
  Query_block *const select = unit->first_query_block();
  insert_tables = select->parent_lex->insert_table_leaf;

  // note: need free
  insert_operations =
      (COPY_INFO **)thd->mem_calloc(sizeof(COPY_INFO *) * insert_table_count);
  if (insert_operations == nullptr) return true;

  bulk_insert_start =
      (bool *)thd->mem_calloc(sizeof(bool) * insert_table_count);
  if (bulk_insert_start == nullptr) return true;
  // values

  for (auto tb = insert_tables; tb != nullptr; tb = tb->next_leaf) {
    auto target_columns = fields_for_table[tb->shared];
    bulk_insert_start[tb->shared] = false;
    assert(target_columns != nullptr);

    COPY_INFO *insert_info = new (thd->mem_root) COPY_INFO(
        COPY_INFO::INSERT_OPERATION, target_columns,
        (target_columns == nullptr || !target_columns->empty()), DUP_ERROR);
    if (insert_info == nullptr) {
      return true;
    }
    if (insert_info->add_function_default_columns(tb->table,
                                                  tb->table->write_set))
      return true;
    insert_info->reset_counters();
    insert_operations[tb->shared] = insert_info;
  }

  return false;
}

void Query_result_insert_all::send_error(THD *, uint errcode, const char *err) {
  DBUG_TRACE;
  my_message(errcode, err, MYF(0));
}

bool Query_result_insert_all::send_eof(THD *thd) {
  ...
  for (auto tb = insert_tables; tb != nullptr; tb = tb->next_leaf) {
    ...
    if (bulk_insert_start)
      if (bulk_insert_start[tb->shared]) {
        error = table->file->ha_end_bulk_insert();
    ...
    DBUG_EXECUTE_IF("after_mysql_insert_all", {
      bool insert_status = (insert_operations[tb->shared]->stats.copied ||
                            insert_operations[tb->shared]->stats.deleted ||
                            insert_operations[tb->shared]->stats.updated);

      /*
        INSERT ... SELECT on non-transactional table which changes any rows
        must be marked as unsafe to rollback.
      */
      assert(table->file->has_transactions() || !insert_status ||
  ...
  */
  ...
  if (mysql_bin_log.is_open() &&
    ...
    if (thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str,
                          thd->query().length, is_trans, false, false,
                          errcode)) {
      return true;
    }
  }
  ...
  if (error) {
    ...
    for (auto tb = insert_tables; tb != nullptr; tb = tb->next_leaf) {
      error_flags = MYF(0);
      if (tb->table->file->is_fatal_error(my_errno()))
        error_flags |= ME_FATALERROR;
      tb->table->file->print_error(my_errno(), error_flags);
    }
    return true;
  }
  /*
  ...
  */
  ...
  for (uint i = 0; i < insert_table_count; i++) {
    records += (long)insert_operations[i]->stats.records;
    changed += (long)(insert_operations[i]->stats.records -
                      insert_operations[i]->stats.copied);
    row_count += insert_operations[i]->stats.copied +
                 insert_operations[i]->stats.deleted +
                 (thd->get_protocol()->has_client_capability(CLIENT_FOUND_ROWS)
                      ? insert_operations[i]->stats.touched
                      : insert_operations[i]->stats.updated);
  }
  snprintf(buff, sizeof(buff), ER_THD(thd, ER_INSERT_INFO), records, changed,
           (long)thd->get_stmt_da()->current_statement_cond_count());
  /*
  id = (thd->first_successful_insert_id_in_cur_stmt > 0)
           ? thd->first_successful_insert_id_in_cur_stmt
           : (thd->arg_of_last_insert_id_function
                  ? thd->first_successful_insert_id_in_prev_stmt
                  : (row_count > 0 ? autoinc_value_of_last_inserted_row : 0));
  */
  thd->first_successful_insert_id_in_prev_stmt = 0;
  thd->first_successful_insert_id_in_cur_stmt = 0;
  thd->first_successful_insert_id_in_prev_stmt_for_binlog = 0;

  my_ok(thd, row_count, 0, buff);
  thd->updated_row_count += row_count;
  return false;
}

void Query_result_insert_all::abort_result_set(THD *thd) {
  DBUG_TRACE;
  bool transactional_table = false;
  for (auto tb = insert_tables; tb != nullptr; tb = tb->next_leaf) {
    if (tb->table != nullptr) {
      if (bulk_insert_start && bulk_insert_start[tb->shared]) {
        tb->table->file->ha_end_bulk_insert();
        bulk_insert_start[tb->shared] = false;
      }

      transactional_table |= tb->table->file->has_transactions();
      if (thd->get_transaction()->cannot_safely_rollback(
              Transaction_ctx::STMT)) {
        if (mysql_bin_log.is_open()) {
          int errcode = query_error_code(thd, thd->killed == THD::NOT_KILLED);
          /* error of writing binary log is ignored for each table */
          (void)thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str,
                                  thd->query().length, transactional_table,
                                  false, false, errcode);
        }
      }
      tb->table->file->ha_release_auto_increment();
...
void Query_result_insert_all::cleanup() {
  DBUG_TRACE;
  // check insert_table->table=nullptr  if insert_table_leaf leave is table
  // nullptr ? insert_table_leaf
  auto tb = insert_tables;

  for (uint i = 0; i < insert_table_count; i++) {
    if (tb != nullptr && tb->table != nullptr) {
      tb->table->next_number_field = nullptr;
      tb->table->file->ha_reset();
      tb->table = nullptr;
      tb = tb->next_leaf;
    }

    if (insert_operations && insert_operations[i]) {
      insert_operations[i]->cleanup();
    }
  }
...
bool Query_result_insert_all::start_execution(THD *thd) {
  DBUG_TRACE;
  TABLE *table;
  for (auto tb = insert_tables; tb != nullptr; tb = tb->next_leaf) {
    table = tb->table;
    restore_record(table, s->default_values);
    table->next_number_field = table->found_next_number_field;
    // prepare has already set why set again ???
    if (insert_operations[tb->shared]->add_function_default_columns(
    ...
    */
    ...
    if (thd->locked_tables_mode <= LTM_LOCK_TABLES && !thd->lex->is_explain()) {
      table->file->ha_start_bulk_insert((ha_rows)0);
      if (bulk_insert_start) bulk_insert_start[tb->shared] = true;
    }
  }
 ...
 *
 * @param thd
 * @param  items is the same as vals list (not use)
 *
 * @return true
 * @return false
 */

bool Query_result_insert_all::send_data(
    THD *thd, const mem_root_deque<Item *> &values_list) {
  DBUG_TRACE;
  bool error = false;
  TABLE *cur_table;
  COPY_INFO *info;
  ...
  for (auto tb = insert_tables; tb != nullptr; tb = tb->next_leaf) {
    ...
    if (CountVisibleFields(*fields) != 0) {
      ...
      if (!validate_default_values_of_unset_fields(thd, cur_table)) {
        fill_record_n_invoke_before_triggers(
            thd, info, *fields, values, cur_table, TRG_EVENT_INSERT,
            cur_table->s->fields, true, nullptr);
      }
    } else {
      // into t select
      fill_record_n_invoke_before_triggers(thd, cur_table->field, values,
                                           cur_table, TRG_EVENT_INSERT,
                                           cur_table->s->fields);
    }
    check_that_all_fields_are_given_values(thd, cur_table, select_tables);
    ...
    if (invoke_table_check_constraints(thd, cur_table)) {
      return thd->is_error();
    }
    error = write_record(thd, cur_table, info, nullptr);

    DEBUG_SYNC(thd, "create_select_after_write_rows_event_into_single_table");

    if (!error && tb->table->triggers) {
      restore_record(cur_table, s->default_values);
    }
    if (!error && cur_table->next_number_field) {
      /*
        If no value has been autogenerated so far, we need to remember the
        value we just saw, we may need to send it to client in the end.
      */
      if (thd->first_successful_insert_id_in_cur_stmt == 0)  // optimization
        autoinc_value_of_last_inserted_row =
            cur_table->next_number_field->val_int();
      /*
        Clear auto-increment field for the next record, if triggers are used
        we will clear it twice, but this should be cheap.
      */
      cur_table->next_number_field->reset();
    }
    if (error) {
      continue;
    }
  }
  DEBUG_SYNC(thd, "create_select_after_write_rows_event_into_all");
  return error;
}
