...
Truncated_value_error_handler::Truncated_value_error_handler(THD *thd)
    : m_thd(thd), m_has_error(false) {
  thd->push_internal_handler(this);
}

Truncated_value_error_handler::~Truncated_value_error_handler() {
  m_thd->pop_internal_handler();
}
bool Truncated_value_error_handler::handle_condition(
    THD *, uint sql_errno, const char *, Sql_condition::enum_severity_level *,
    const char *) {
  switch (sql_errno) {
    case ER_TRUNCATED_WRONG_VALUE:
