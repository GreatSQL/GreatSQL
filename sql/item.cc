...
Item::Item(THD *thd, const Item *item)
    : super(POS()),
      ...
      m_is_udt_table_index(item->m_is_udt_table_index),
...
static bool resolve_pushed_cond(Item *item) {
  ...
  switch (item_type) {
    case Item::FUNC_ITEM: {
      ...
      switch (item_func->functype()) {
        ...
        case Item_func::EQUAL_FUNC: {
          auto fi = down_cast<Item_bool_func2 *>(item_func);
          if (fi == nullptr) return true;
          if (fi->set_cmp_func()) return true;
          break;
        }
        default:
          break;
    ...
    case Item::COND_ITEM: {
      ...
      Item *cond_item;
      while ((cond_item = li++)) {
        if (resolve_pushed_cond(cond_item)) return true;
      }
      break;
    }
    default:
...
Item *Item::clone_pushed_cond(THD *thd, TABLE *table,
  ...
  *all_item_fields =
  ...
  if (resolve_pushed_cond(new_cond)) return nullptr;
...
// collect_all_item_fields_processor() is modeled after
// collect_item_field_processor() but keep items reference the same Field.
bool Item_field::collect_all_item_fields_processor(uchar *arg) {
  DBUG_TRACE;
  DBUG_PRINT("info", ("%s", field_name ? field_name : "noname"));
  mem_root_deque<Item_field *> *item_list =
      reinterpret_cast<mem_root_deque<Item_field *> *>(arg);
...
bool Item_field::collect_item_field_or_view_ref_processor(uchar *arg) {
  ...
  while ((curr_item = item_list_it++)) {
    bool is_eq = curr_item->eq(this, true);
    if (is_eq) {
      ...
      if (info->check_joined_flag) {
        if (curr_item->outer_joined == this->outer_joined) {
          return false;
        }
      } else {
...
bool Item_string::eq(const Item *item, bool binary_cmp) const {
  if (type() == item->type() && item->basic_const_item()) {
    ...
    String str;
    /*
      Return false when arg '' val_str is nullptr in MODE_EMPTYSTRING_EQUAL_NULL
      mode
    */
    if ((current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
        item->type() == STRING_ITEM && !arg->val_str(&str)) {
      return false;
    }
...
bool Item_sp_variable::fix_fields_from_item(THD *thd, Item **, const Item *it) {
  ...
  if (const_cast<Item *>(it)->is_ora_udt_type() &&
      it->result_type() == ROW_RESULT && thd->lex->check_udt_type_field())
    return true;
...
bool Item_sp_variable::fix_fields(THD *thd, Item **ref) {
  Item *it = this_item();
  ...
  return fix_fields_from_item(thd, ref, it);
}
...
TABLE *Item_sp_variable::val_udt() {
  assert(fixed);
  Item *it = this_item();
  if (it == nullptr) {
    null_value = true;
  ...
  TABLE *ret = it->val_udt();
  null_value = it->null_value;
  return ret;
}

TABLE *Item_sp_variable::get_udt_table() {
  assert(fixed);
  Item *it = this_item();
  TABLE *ret = it->get_udt_table();
  return ret;
}
...
Item_splocal::Item_splocal(const Sp_rcontext_handler *rh,
                           const Name_string sp_var_name, uint sp_var_idx,
...
sp_rcontext *Item_splocal::get_rcontext(sp_rcontext *local_ctx) const {
  if (local_ctx->sp->m_type == m_sp->m_type)
    return local_ctx;
  else {
    switch (m_sp->m_type) {
      case enum_sp_type::PACKAGE_SPEC:
        if (local_ctx->sp->m_type == enum_sp_type::PACKAGE_BODY)
          return local_ctx->sp->get_package()->m_package_head->m_rcontext;
        else  // procedure or function
          return local_ctx->sp->m_parent->m_package_head->m_rcontext;
      case enum_sp_type::PACKAGE_BODY:
        // procedure or function
        return local_ctx->sp->m_parent->m_rcontext;
      default:
        assert(0);
    }
  }
...
bool Item_splocal::fix_fields(THD *thd, Item **ref) {
  Item *item = get_item(thd->sp_runtime_ctx);
  // set_handler(item->type_handler());        // @@NOTE@@ there is no such
  // thing in percona
  // For Item_field_row type,it sets result_type to correct type now.
  set_result_type(item->result_type());
  return fix_fields_from_item(thd, ref, item);
}

Item *Item_splocal::this_item() {
  return get_item(current_thd->sp_runtime_ctx);
}
...
Item **Item_splocal::this_item_addr(THD *thd, Item **) {
  return get_rcontext(thd->sp_runtime_ctx)->get_item_addr(m_var_idx);
}
...
void Item_splocal::print(const THD *thd, String *str, enum_query_type) const {
  ...
  str->append(prefix->str, prefix->length);
  str->append(m_name);
...
bool Item_splocal::set_value(THD *thd, sp_rcontext *ctx, Item **it) {
  return get_rcontext(ctx)->set_variable(thd, false, get_var_idx(), it);
}

bool Item_splocal::fix_index_and_check(THD *thd, Item **index) {
  auto is_udt_table_index = (*index)->m_is_udt_table_index;
  if ((*index)->fix_fields(thd, index)) {
    return true;
  }
  (*index)->m_is_udt_table_index = is_udt_table_index;
  if ((*index)->forbid_udt_table_index()) return true;
  return false;
}
...
bool Item_field_refcursor::set_cursor_rowtype_table(THD *thd, Item *item_to,
                                                    ...
                                                    Item_field_refcursor *spv,
                                                    sp_rcontext *ctx_item_to) {
  ...
  if (item_cursor) {
    ...
    if (has_return_type) {
      return item_cursor->get_refcursor_field()
          ->set_cursor_return_type_from_other_refcursor(spv);
    } else {
      my_error(ER_NOT_SUPPORTED_YET, MYF(0),
               "non structure variable used in %rowtype");
      return true;
    }
  } else if (item_row && get_refcursor_field()->val_udt()) {
    const Sp_udt_stmt_arena_holder sp_arena_holder(thd, field->m_callers_arena);
...
/**
  Allocate memory for arguments using thd->alloc().
  @retval false  - success
  @retval true   - error (arg_count is set to 0 for conveniece)
*/
bool Item_field_row::alloc_arguments(THD *thd, uint count) {
...
bool Item::find_duplicate_def_name(uint arg_count, Create_field *def,
                                   List<Create_field> *list) {
...
bool Item_field_row::row_create_items(THD *thd, List<Create_field> *list) {
  assert(list);
  // Field_udt_type has field->m_udt_table by create_udt_table_from_routine.
  if (!field->m_udt_table &&
      !(field->m_udt_table = create_tmp_table_from_fields(
            thd, *list, true, 0, field->field_name, field->m_callers_arena))) {
    my_error(ER_CANT_CREATE_TABLE, myf(0), "udt table", 1005,
             "create tmp table error");
    return true;
  }

  field->m_udt_table->copy_blobs = true;
  field->m_udt_table->alias = "";
  arg_count = list->elements;
  // init_var_items
  if (alloc_arguments(thd, list->elements)) return true;

  bool rec = row_create_items_from_cdf_list(
      thd, list, field->m_udt_table, &arg_count,
      const_cast<char *>(field->field_name), args);
  field->set_arg_count(arg_count);
  return rec;
}

bool Item::row_create_items_from_cdf_list(THD *thd, List<Create_field> *list,
                                          TABLE *table, uint *arg_count_out,
                                          char *row_table_name, Item **args) {
  List_iterator_fast<Create_field> it(*list);
  uint arg_count_tmp = *arg_count_out;
  Create_field *def;
  bool rec = false;
  for (uint i = 0; (def = it++); i++) {
    if (find_duplicate_def_name(arg_count_tmp, def, list)) return true;
    if (def->ora_record.row_field_definitions()) {
    ...
    } else if (def->ora_record.row_field_table_definitions()) {
...
bool Item_field_row::index_item_save_in_field_no_index(Field *to) {
  if (to->get_udt_db_name() && to->udt_name().str) {
    ...
    // e.g: to is table%rowtype / cursor%rowtype
    if (!to->m_udt_table) {
      my_error(ER_UDT_INCONS_DATATYPES, myf(0));
      return true;
    }
    if (to->m_udt_table->visible_field_count() !=
        field->m_udt_table->visible_field_count()) {
  ...
  for (uint i = 1; i < arg_count; i++) {
    if (sp_eval_expr(current_thd, true, to->m_udt_table->field[i - 1], &args[i]))
      return true;
  }
  to->set_notnull();
...
bool Item_field_row::udt_table_store_to_table(TABLE *table_to) {
  memcpy(table_to->record[0],
         const_cast<uchar *>(field->m_udt_table->record[0]),
         field->val_udt()->s->reclength);
  return false;
}
...
Item *Item_field_row_table::element_row_index(Item *item_index, uint i) {
  ...
  if (args[i]->result_type() == ROW_RESULT && !field_udt) {
    if (get_row_table_field()->m_row_table_items.elements) {
      result = field_table->get_table_element(item_index, i);
    } else {
      result = element_table_index(item_index);
      if (result) result = result->element_index(i);
    }
  } else if (!(result =
...
Item *Item_field_row_table::element_table_index(Item *item_index) {
  ...
  Item *result = nullptr;
  if (get_nested_table_udt().length == 0) {
...
bool Item_field_row_table::row_table_create_items(
  ...
  List<Row_definition_list> *sd =
      dynamic_cast<List<Row_definition_list> *>(list);
  if (!field_table || !sd) return true;
  ...
  for (uint offset = 0; (def_list = it_table++); offset++) {
    ...
    if (field_table->create_tmp_record_table_to_store_bulk_data(
            thd, row_table_name, *sd_list, nullptr,
    ...
    if (row_create_items_from_cdf_list(thd, sd_list,
                                       field_table->table_record->table,
                                       &arg_count, row_table_name, args)) {
      field->clear_table_all_field();
      return true;
    }
    // It's for dr_table(i).
    ...
    if (item_row_tmp->row_create_items_for_tmp_field(
            thd, arg_count, args, get_nested_table_udt(),
            field_table->table_record->table)) {
      field->clear_table_all_field();
      return true;
    }
  }
  field->m_udt_table = field_table->table_record->table;
  field->set_arg_count(arg_count);
  return false;
}
...
/*e.g:
    ...
    nm int,
    name_d t_air := t_air(1,'qq'),
    score float := 0.1
    );
    type tklist is table of stu_record INDEX BY pls_integer;
    the default value of tklist's name_d should reparse to a new Item.
*/
Item *Item_field_row_table::clone_default_value_from_record(THD *thd,
                                                            Item *arg) {
  if (!arg || arg->is_splocal()) return arg;
  String tmp;
  arg->print(thd, &tmp, QT_ORDINARY);
  tmp.append('\0');

  const Sp_udt_stmt_arena_holder sp_arena_holder(thd, field->m_callers_arena);
  Expression_parser_state parser_state;
  Item *result = string_to_expr(thd, tmp.c_ptr_quick(), tmp.length(),
                                static_cast<Parser_state *>(&parser_state))
                     ? nullptr
                     : parser_state.result;
  if (!result) return nullptr;
  if (arg->item_name.m_str) {
    result->item_name.copy(arg->item_name.m_str, arg->item_name.m_length);
    result->item_name.set_autogenerated(arg->item_name.is_autogenerated());
  }
...
bool Item_field_row_table::
  ...
  List_iterator_fast<Row_definition_list> it_table(*sd);
  Row_definition_list *def_list = it_table++;
  sp_variable *spv = nullptr;
  List_iterator_fast<Create_field> it(*def_list);
  Create_field *def;
  uint i;
  bool rec = false;
  const Sp_udt_stmt_arena_holder sp_arena_holder(thd, field->m_callers_arena);
  if (this_field->alloc_default_value_list(thd, get_arg_count())) {
    return true;
  }
  for (i = 0; (def = it++); i++) {
    if (def->ora_record.row_field_definitions()) {
      ...
      field_table_tmp->set_rowtype_table_ident(
          def->ora_record.table_rowtype_ref());
      field_table_tmp->m_callers_arena = field->m_callers_arena;
      if (def->udt_name.str) {
      ...
      if (def->ora_record.record_default_value) {
        Item *item_default = clone_default_value_from_record(
            thd, def->ora_record.record_default_value);
        rec = sp_eval_expr(thd, true, field_table_tmp,
...
bool Item_field_row_table::insert_into_table_default_value(THD *thd,
  ...
  auto strict_handler_guard = create_scope_guard([thd]() {
    if (thd->is_strict_mode() && !thd->lex->is_ignore())
...
bool Item_field_row_table::insert_into_table_all_value(
  ...
  if (CountVisibleFields(items) == 1 &&
      (item->type() == ORACLE_ROWTYPE_ITEM || item->type() == FUNC_ITEM ||
       item->type() == FIELD_ITEM || item->type() == SUBQUERY_ITEM)) {
    if (field_table->get_nested_table_udt().length != 0 &&
        item->get_udt_db_name().str && item->get_udt_name().str) {
      if (my_strcasecmp(system_charset_info, get_udt_db_name().str,
                        item->get_udt_db_name().str) != 0 ||
          my_strcasecmp(system_charset_info, get_nested_table_udt().str,
        ...
        my_error(ER_WRONG_UDT_DATA_TYPE, myf(0), item->get_udt_db_name().str,
                 ...
                 get_nested_table_udt().str);
        return true;
      }
    }
    // select val bulk collect into val1;val1 is type table,but val is nested
    // table.
    if (is_bulk_into && !field_table->get_nested_table_udt().length &&
        item->get_udt_name().str) {
      my_error(ER_UDT_INCONS_DATATYPES, myf(0));
      return true;
    }
  }
  Table_function_record *tmp_record =
      is_bulk_into ? field_table->tmp_table_record : field_table->table_record;
  return tmp_record->fill_result_table_all_col(thd, items, item_index,
                                               is_bulk_into);
}
  ...
  */
  ...
  if (field_idx == -1) {
    if (item->type() == Item::FUNC_ITEM ||
      ...
      if (!field_table->get_nested_table_udt().length &&
          item->get_udt_name().str) {
        my_error(ER_UDT_INCONS_DATATYPES, myf(0));
        return true;
      }
  ...
  if (row) {
    ret = sp_eval_expr(current_thd, true, row, value);
  } else
    ret = field_table->table_record->update_result_table_one_col(
...
bool Item_field_row_table::modify_item_field_about_cursor(
    THD *thd, List<Create_field> *defs, sp_cursor *, sp_rcontext *ctx) {
  Field_row_table *field_table = dynamic_cast<Field_row_table *>(field);
  int length = field_table->get_index_length();
  Row_definition_list *rdl = new (thd->mem_root) Row_definition_list();
  if (rdl->make_from_defs(thd->mem_root, defs)) return true;
  sp_variable *spv = ctx->search_variable(item_name.ptr(), item_name.length());
  // it must modify the sp_variable's row_field_table_definitions() to get this
  // item's column later.
  Row_definition_list *rdl_result =
      spv ? spv->field_def.ora_record.row_field_table_definitions()
                ->find_row_fields_by_offset(0)
          : nullptr;
  if (!rdl_result) return true;
  rdl_result->clear();
  return rdl->make_new_create_field_to_store_index(
      thd, length > 0 ? std::to_string(length).c_str() : nullptr,
      field_table->m_is_index_by, &rdl_result);
}

bool Item_splocal_row_field::fix_fields(THD *thd, Item **ref) {
  Item *item = get_item(thd->sp_runtime_ctx)->element_index(m_field_idx);
...
Item *Item_splocal_row_field::this_item() {
  Item_field *item =
      dynamic_cast<Item_field *>(get_item(current_thd->sp_runtime_ctx));
  Field_udt_type *udt = dynamic_cast<Field_udt_type *>(item->field);
  if (udt) {
    udt->copy_data_to_table();
  }
  return get_item(current_thd->sp_runtime_ctx)->element_index(m_field_idx);
}

const Item *Item_splocal_row_field::this_item() const {
  Item_field *item =
      dynamic_cast<Item_field *>(get_item(current_thd->sp_runtime_ctx));
  Field_udt_type *udt = dynamic_cast<Field_udt_type *>(item->field);
  if (udt) {
...
Item **Item_splocal_row_field::this_item_addr(THD *thd, Item **) {
  Item_field *item = dynamic_cast<Item_field *>(get_item(thd->sp_runtime_ctx));
  Field_udt_type *udt = dynamic_cast<Field_udt_type *>(item->field);
  if (udt) {
    udt->copy_data_to_table();
  }
  Item *item_result = nullptr;
  if (fix_fields(thd, &item_result)) return nullptr;
  return get_item(thd->sp_runtime_ctx)->addr(m_field_idx);
}
...
bool Item_splocal_row_field_by_name::fix_fields(THD *thd, Item **it) {
  if (get_rcontext(thd->sp_runtime_ctx)
          ->find_row_field_by_name_or_error(&m_field_idx, m_var_idx,
                                            m_field_name))
    return true;
  return Item_splocal_row_field::fix_fields(thd, it);
}

void Item_splocal_row_field_by_name::print(const THD *, String *str,
                                           enum_query_type) const {
  // +16 should be enough for .NNN@[""]
  if (str->reserve(m_name.length() + 2 * m_field_name.length())) return;
  str->append(m_name);
  str->append('.');
  str->append(m_field_name);
}

bool Item_splocal_row_field_by_name::set_value(THD *thd, sp_rcontext *ctx,
                                               Item **it) {
  assert(fixed);  // Make sure m_field_idx is already set
  return Item_splocal_row_field::set_value(thd, ctx, it);
}
/*****************************************************************************
  Item_splocal_row_field_table_by_name methods
*****************************************************************************/
bool Item_splocal_row_field_table_by_name::do_itemize(Parse_context *pc,
                                                      Item **res) {
  if (Item_splocal::do_itemize(pc, res)) return true;
  pc->thd->lex->is_include_udt_table_item = true;
  return false;
}

Item *Item_splocal_row_field_table_by_name::make_item(
    ora_simple_ident_def *def_last, LEX_STRING *def_name_before,
    Row_definition_list **cdf_list_before,
    Row_definition_table_list **cdf_table_list_before) {
  List_iterator_fast<ora_simple_ident_def> it(*m_arg_list);
  ora_simple_ident_def *def;
  Item *item_first = get_item(current_thd->sp_runtime_ctx);
  Item *item = item_first;
  String tmp_str;
  Item_field_row_table *item_field;
  Create_field *cdf_before;
  *def_last = *(m_arg_list->head());
  *def_name_before = def_last->ident;
  sp_variable *spv = m_pctx->find_variable(def_name_before->str,
                                           def_name_before->length, false);
  if (!spv) {
    my_error(ER_SP_UNDECLARED_VAR, MYF(0), def_name_before->str);
    return nullptr;
  }
  *cdf_list_before = spv->field_def.ora_record.row_field_definitions();
  *cdf_table_list_before =
      spv->field_def.ora_record.row_field_table_definitions();
  /*type rec_cjr is record
    (cjrid varchar2(30) := 'aaa',
    tk tklist);
    def_name_before is tklist.
  */
  for (uint offset = 0; (def = it++); offset++) {
    if (offset > 0) {
      // It must find the offset of def_last by name.Now it's def_last.def
      if (def_last->number == nullptr) {  // a.b
        if (!(*cdf_list_before) &&
          ...
          Item_field *item_cursor = dynamic_cast<Item_field *>(
              get_rcontext(current_thd->sp_runtime_ctx)->get_item(spv->offset));
          ...
          if (table_cursor->sp_find_field_by_name_or_error(
                  &m_field_idx, to_lex_cstring(item_cursor->field->field_name),
                  to_lex_cstring(def->ident)))
            return nullptr;
          *def_name_before = table_cursor->field[1]->udt_name();
        } else if (*cdf_list_before) {
          cdf_before = (*cdf_list_before)
        ...
        if (!(*cdf_table_list_before)) {
          my_error(ER_SP_MISMATCH_RECORD_TABLE_VAR, MYF(0),
                   def_last->ident.str);
          return nullptr;
        }
        cdf_before = (*cdf_table_list_before)
                         ->find_row_field_by_name(def->ident, &m_field_idx);
        if (!cdf_before) {
          my_error(ER_ROW_VARIABLE_DOES_NOT_HAVE_FIELD, MYF(0),
                   def_name_before->str, def->ident.str);
        ...
        *def_name_before = cdf_before->udt_name.str
        ...
        if (!def_last->number->fixed &&
            fix_index_and_check(current_thd, &def_last->number))
          return nullptr;
      ...
      *def_last = *def;
      if (cdf_before->ora_record.table_rowtype_ref()) {
        *cdf_list_before = cdf_before->ora_record.row_field_definitions();
        *cdf_table_list_before = nullptr;
      } else {
        spv = m_pctx->find_variable(def_name_before->str,
                                    def_name_before->length, false);
        if (!spv) {
          spv = get_rcontext(current_thd->sp_runtime_ctx)
                    ->search_udt_variable(def_name_before->str,
                                          def_name_before->length, nullptr, 0);
          if (!spv) {
            my_error(ER_SP_UNDECLARED_VAR, MYF(0), def_name_before->str);
            return nullptr;
          }
...
bool Item_splocal_row_field_table_by_name::pre_fix_fields(Item **result) {
  ora_simple_ident_def def_last(NULL_STR, nullptr);
  LEX_STRING def_name_before = NULL_STR;
  Create_field *cdf = nullptr;
  Row_definition_list *cdf_list_before = nullptr;
  Row_definition_table_list *cdf_table_list_before = nullptr;
  Item *item = make_item(&def_last, &def_name_before, &cdf_list_before,
                         &cdf_table_list_before);
  *result = item;
  if (!item) return true;
  if (def_last.number == nullptr) {
    ...
    cdf = cdf_list_before->find_row_field_by_name(
    ...
    cdf = cdf_table_list_before->find_row_field_by_name(
        to_lex_string(to_lex_cstring(m_field_name.ptr())), &m_field_idx);
    if (!cdf) {
      my_error(ER_ROW_VARIABLE_DOES_NOT_HAVE_FIELD, MYF(0), def_last.ident.str,
               m_field_name.ptr());
...
bool Item_splocal_row_field_table_by_name::fix_fields(THD *thd, Item **) {
  Item *item = nullptr;
  bool rc = pre_fix_fields(&item);
  if (rc) return true;
  set_result_type(item->result_type());
  return fix_fields_from_item(thd, &item, item);
}
...
const Item *Item_splocal_row_field_table_by_name::this_item() const {
  return const_cast<Item_splocal_row_field_table_by_name *>(this)->this_item();
}

void Item_splocal_row_field_table_by_name::print(const THD *, String *str,
                                                 enum_query_type) const {
  int length = m_name.length() + m_field_name.length() + 8;
  if (m_number) length = length + m_number->item_name.length();
  List_iterator_fast<ora_simple_ident_def> it_length(*m_arg_list);
  ora_simple_ident_def *def;
  for (uint offset = 0; (def = it_length++); offset++) {
    length = length + def->ident.length + 1;
    if (def->number != nullptr)
      length = length + def->number->item_name.length() + 2;
  }
  str->reserve(length);
  str->append(m_name);
  if (m_number) {
  ...
  for (uint offset = 0; (def = it++); offset++) {
    ...
    str->reserve(def->ident.length + 1);
...
bool Item_splocal_row_field_table_by_name::set_value(THD *thd, sp_rcontext *ctx,
                                                     Item **it) {
  return get_rcontext(ctx)->set_variable_row_table_by_name(
      thd, m_var_idx, m_field_name, m_arg_list, it);
}

Item **Item_splocal_row_field_table_by_name::this_item_addr(THD *,
                                                            Item **item) {
  *item = this_item();
  return item;
}
...
*****************************************************************************/
bool Item_splocal_row_field_table_by_index::do_itemize(Parse_context *pc,
                                                       Item **res) {
  if (Item_splocal::do_itemize(pc, res)) return true;
  if (m_index->itemize(pc, &m_index)) return true;
  m_index->m_is_udt_table_index = true;
  pc->thd->lex->is_include_udt_table_item = true;
  return false;
}

bool Item_splocal_row_field_table_by_index::fix_fields(THD *thd, Item **) {
  Item *item_first = get_item(thd->sp_runtime_ctx);
  if (!item_first) return true;
  if (!m_index->fixed && fix_index_and_check(thd, &m_index)) return true;
  Item_field_row_table *item_field =
...
Item *Item_splocal_row_field_table_by_index::this_item() {
  ...
  if (field_table->check_if_row_not_exist(m_index)) {
    my_error(ER_SP_NOT_EXIST_OF_RECORD_TABLE, MYF(0),
             item_first->item_name.ptr());
    return new (current_thd->mem_root) Item_null();
  }
  Item *item = item_field->element_table_index(m_index);
  return item;
}
...
bool Item_splocal_row_field_table_by_index::set_value(THD *thd,
                                                      sp_rcontext *ctx,
                                                      Item **it) {
  return get_rcontext(ctx)->set_variable_row_table_by_index(thd, m_var_idx,
                                                            m_index, it);
}
...
Item **Item_splocal_row_field_table_by_index::this_item_addr(THD *,
                                                             Item **item) {
  *item = this_item();
  return item;
}
...
void Item_field::set_field(Field *field_par) {
  ...
  /*
    Character type non-null field and set_nullable must be set to true after
    turning on the empty string and null equivalent switch
  */
  if ((current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
...
/* only for storage engine greatdb */
static void convert_and_print_strictly(const String *from_str, String *to_str,
  ...
  } else                      // need to convert
  {
    THD *thd = current_thd;
    LEX_STRING lex_str;
    thd->strictly_convert = 1;
    thd->convert_string(&lex_str, to_cs, from_str->ptr(), from_str->length(),
                        from_str->charset(), true);
    thd->strictly_convert = 0;
...
void Item_string::print(const THD *, String *str,
  ...
  if (query_type & QT_TO_SYSTEM_CHARSET) {
    ...
    if (print_introducer)
      ...
      */
      if (query_type & QT_PRINT_VALUE_STRICTLY) {
        convert_and_print_strictly(&str_value, str, str->charset());
      } else {
...
double Item_string::val_real() {
  ...
  if (str_value.is_empty() &&
    ...
    // the same as item_Null when empty '' string as null oracle compat
    null_value = true;
    return 0.0;
  }
...
String *Item_string::val_str(String *) {
  assert(fixed);
  if (str_value.is_empty() &&
      (current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL)) {
    // the same as item_Null when empty '' string as null oracle compat
...
int Item_field::fix_outer_field(THD *thd, Field **from_field,
  ...
  */
  ...
  if (ref == nullptr && *from_field == not_found_field) {
    bool sequence = false;
    if (try_fix_sequence_field(thd, reference, &sequence)) {
      return -1;
    }
    if (upward_lookup) {
      if (sequence) {
...
bool Item_field::try_fix_sequence_field(THD *thd, Item **ref,
                                        bool *is_sequence) {
  DBUG_TRACE;
  bool seq_need_get_next = false;
  ...
  auto seq_enty = find_sequence_def(thd, real_db_name, table_name);
  // not found
  if (seq_enty == nullptr) return false;

  auto seq_def = new (thd->mem_root) SequenceEntity(
      seq_enty.get(), real_db_name, table_name, seq_enty->metadata_version());
  if (seq_def == nullptr) return true;

  Item *it_seq =
      new (thd->mem_root) Item_func_sequence(POS(), seq_def, seq_need_get_next);

  if (it_seq == nullptr || it_seq->fix_fields(thd, &it_seq)) return true;

  it_seq->item_name.set(this->item_name.ptr(), this->item_name.length());
  *ref = it_seq;
  *is_sequence = true;
...
Field *Item::make_string_field(TABLE *table) const {
  ...
  else if (type() != Item::TYPE_HOLDER_ITEM ||
           data_type() != MYSQL_TYPE_STRING) {
    if (is_ora_table()) {
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "nested table as a column");
      return nullptr;
    }
    if (is_ora_type() && result_type() == ROW_RESULT) {
      LEX_CSTRING udt_name =
          current_thd->strmake(to_lex_cstring(get_udt_name()));
      char *udt_db_name =
          current_thd->strmake(get_udt_db_name().str, get_udt_db_name().length);
      field = new (*THR_MALLOC) Field_udt_type(
          max_length, is_nullable(), item_name.ptr(), table->s, &my_charset_bin,
          udt_name, *THR_MALLOC, LEX_STRING{udt_db_name, strlen(udt_db_name)});
    } else
      field = new (*THR_MALLOC)
...
type_conversion_status Item_field::save_in_field_inner(Field *to,
                                                       bool no_conversions) {
  DBUG_TRACE;
  if (field->udt_name().length != 0 || field->is_rowtype_table()) {
...
type_conversion_status Item::save_in_field(Field *field, bool no_conversions) {
   ...
   */
  if (!is_ora_refcursor() && type() != NULL_ITEM &&
      type() != DEFAULT_VALUE_ITEM) {
    bool is_udt = get_udt_table() || result_type() == ROW_RESULT;
    TABLE *tab = field->m_udt_table;
    if ((tab && !is_udt) || (!tab && is_udt)) {
      if (!(tab && type() == STRING_ITEM &&
            data_type() == MYSQL_TYPE_VARCHAR /*Item_string*/)) {
        my_error(ER_UDT_INCONS_DATATYPES, myf(0));
        return TYPE_ERR_BAD_VALUE;
      }
    }
  ...
  */
  ...
  /*
    If item is null value from dk, set field to null.
  */
  if (m_is_dk_null_value) {
    null_value = true;
    return set_field_to_null_with_conversions(field, no_conversions);
  }
...
type_conversion_status Item::save_in_field_inner(Field *field,
  ...
  /* this Item maybe
     Item_func_udt_constructor,Item_ref,Item_splocal,Item_func_udt_table,
     Item_func_udt_single_type_table.
     Only Item->is_ora_type() can save value to Field_udt_type and Field_row.
     When it's udt=udt,item->is_ora_type()==true but result_type() is not
     ROW_RESULT.
  */
  if ((get_udt_table() || result_type() == ROW_RESULT) &&
      (is_ora_type() || is_ora_table())) {
    /*
      'this' can be Item_cache_str.
    */
    if (!field->is_rowtype_table() &&
        (!field->get_udt_db_name() || !field->udt_name().str)) {
      my_error(ER_UDT_INCONS_DATATYPES, myf(0));
      return TYPE_ERR_BAD_VALUE;
    }

    if (!field->is_rowtype_table() &&
        (my_strcasecmp(system_charset_info, get_udt_db_name().str,
    ...
    type_conversion_status ret = TYPE_OK;
    if (null_value) {
      ret = set_field_to_null_with_conversions(field, no_conversions);
    } else {
      assert(table);
      field->set_notnull();
      ret = field->store_udt(table, this);
    }
...
type_conversion_status Item_string::save_in_field_inner(Field *field, bool) {
  String *result;
  result = val_str(&str_value);
  // If field is udt type,it needs to check the length and m_cs_specified=true.
  // Used for mysqldump for table with udt column.
  if (field->result_type() == ROW_RESULT &&
      (field->row_pack_length() != result->length() || !is_cs_specified())) {
    my_error(ER_UDT_INCONS_DATATYPES, myf(0));
    return TYPE_ERR_BAD_VALUE;
  }
...
TABLE *Item_ref::val_udt() {
  assert(fixed);
  TABLE *tab = ref_item()->val_udt();
  null_value = ref_item()->null_value;
  return tab;
}
...
bool Item_ref::collect_item_ref_processor(uchar *arg) {
  DBUG_TRACE;
  DBUG_PRINT("info", ("%s", result_field->field_name ? result_field->field_name
                                                     : "noname"));
  mem_root_deque<Item_ref *> *item_list =
      reinterpret_cast<mem_root_deque<Item_ref *> *>(arg);
  for (Item_ref *curr_item : *item_list) {
    if (curr_item->eq(this, true)) return false; /* Already in the set. */
  }
  item_list->push_back(this);
  return false;
...
Item_cache *Item_cache::get_cache(const Item *item, const Item_result type) {
  Item *item_tmp = const_cast<Item *>(item);
  switch (type) {
    case INT_RESULT:
      /*
...
bool Item_cache::collect_item_cache_processor(uchar *arg) {
  DBUG_TRACE;
  DBUG_PRINT("info", ("%s", (cached_field && cached_field->field_name)
                                ? cached_field->field_name
                                : "noname"));
  mem_root_deque<Item_cache *> *item_list =
      reinterpret_cast<mem_root_deque<Item_cache *> *>(arg);
  for (Item_cache *curr_item : *item_list) {
    if (curr_item->eq(this, true)) return false; /* Already in the set. */
  }
  item_list->push_back(this);
  return false;
...
bool Item_cache_str::cache_value() {
  ...
  //  update t1 set (u1)=(select name from tt_air),it must save as BINARY.
  ...
  if (is_ora_table()) example->val_udt();
...
TABLE *Item_cache_str::val_udt() {
  assert(fixed);
  if (!has_value()) return nullptr;
  if (m_udt_name.length == 0) return nullptr;

  // bad
  if (is_ora_type()) {
    TABLE *table = example->val_udt();
    if (example->null_value) return nullptr;
    memcpy(table->record[0], value->ptr(), table->s->reclength);
    return table;
  } else {
    return example->val_udt();
  }
}
...
List<Create_field> *Item_cache_str::get_field_create_field_list() {
  return example ? example->get_field_create_field_list() : nullptr;
}
...
Item *Item_field::element_index(uint i) {
  Field_udt_type *udt = dynamic_cast<Field_udt_type *>(field);
  if (udt && current_thd->sp_runtime_ctx) {
    TABLE *tab = field->val_udt();
    if (tab == nullptr) return nullptr;
    assert(tab->s->fields > i);
    const Sp_udt_stmt_arena_holder sp_arena_holder(current_thd,
                                                   field->m_callers_arena);
    Item *item = new (current_thd->mem_root) Item_field(tab->field[i]);
    return item;
  }
  return Item::element_index(i);
}
...
/**
  Perform validity check for named parameters and reorder them if necessary.

 @param thd          IN    current thread.
 @param type         IN    type of routine, like: FUNCTION or PROCEDURE
 @param params       INOUT array of input parameter's pointers
 @param params_count IN    number of input parameters
 @param sp           IN    sp_head represents a stored program
 @param is_ordered   OUT   is params already ordered

 @note primary rule of check:

 - params_count is same as sp definition.
 - no positional parameter after first named parameter.
 - all item_name in params can find a match in sp_variables of sp definition
 - all sp_variable names can find a match in item_name of params

 @note item_name.is_autogenerated()==false means: arg is named parameteters
 may check: PTI_udf_expr::itemize for more info.

 @note If is_ordered = false, caller may need to copy the ordered params
 back to somewhere else; or if is_ordered=true, no further processing is
 necessary.

 @note "After first named param, unnamed param is not allowed", during
 mixed-notation, assume: p1(a int, b int, c int):

 - Ok: call p1(1, 2, c=>3);
 - Ok: call p1(1, c=>3, b=>2);
 - Ng: call p1(a=>1, 2, 3);

 @note: case like: p1(a=>1, a=>2, c=>1) and p1( 1, a=>2, c=>1) will be report
 as 'param b is missed' even root cause is: duplicate of param 'a'.
*/
bool reorder_named_parameters(THD *thd, const char *type, Item **params,
                              uint params_count, sp_head *sp,
                              bool *is_ordered) {
  bool has_named_parameters = false;
  *is_ordered = false;

  assert((thd->variables.sql_mode & MODE_ORACLE) == MODE_ORACLE);
  ...
  // confirm: params count is right
  if (params_count != sp->get_root_parsing_context()->context_var_count()) {
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), type, sp->m_name.str,
             sp->get_root_parsing_context()->context_var_count(), params_count);
  ...
  for (uint i = 0; i < params_count; ++i) {
    ...
    // after first named param, unnamed param is not allowed
    if (has_named_parameters && param->item_name.is_autogenerated()) {
      my_error(ER_SP_NAMED_PARAM_BEFORE_POS_PARAM, MYF(0), type,
               sp->m_name.str);

      return true;
    }

    // mark for existance of named param
    if (has_named_parameters == false &&
        param->item_name.is_autogenerated() == false) {
      has_named_parameters = true;
    }

    if (param->item_name.is_autogenerated()) continue;

    // check for invalid name by comparing with sp definition
    bool found = false;
    for (uint j = 0; j < params_count; ++j) {
      sp_variable *var = sp->get_root_parsing_context()->find_variable(j);

      if (param->item_name.eq_safe(var->name.str)) {
        found = true;
        break;
      }
    }

    // no matching in defined param names
    if (found == false) {
      my_error(ER_SP_INVALID_NAMED_PARAM, MYF(0), type, sp->m_name.str,
               param->item_name.ptr());
  ...
  // no named parameter, should not get here
  if (has_named_parameters == false) {
    *is_ordered = true;
    return false;
  } else {
    ...
    for (uint i = 0; i < params_count; ++i) {
      sp_variable *var = sp->get_root_parsing_context()->find_variable(i);
      Item *param = params[i];

      if (param->item_name.is_autogenerated()) continue;

      // named_parameter not yet ordered
      if (param->item_name.eq_safe(var->name.str) == false) {
        ordered = false;
  ...
  assert(has_named_parameters == true);
  assert(*is_ordered == false);

  auto ordered_params = thd->mem_root->ArrayAlloc<Item *>(params_count);

  if (ordered_params == nullptr) return true;

  for (uint i = 0; i < params_count; ++i) {
    // maybe mixed-notation
    if (params[i]->item_name.is_autogenerated()) {
      ordered_params[i] = params[i];
      continue;
    }

    assert(params[i]->item_name.is_autogenerated() == false);
    ...
    for (uint j = 0; j < params_count; ++j) {
      ...
      // if not named param, move to next
      if (param->item_name.is_autogenerated()) continue;

      if (param->item_name.eq_safe(var->name.str)) {
        ordered_params[i] = param;
        found = true;
    ...
    // no matching in input for defined param name
    if (found == false) {
      my_error(ER_SP_MISSING_NAMED_PARAM, MYF(0), type, sp->m_name.str,
               var->name.str);
  ...
  // output ordered params
  for (uint i = 0; i < params_count; ++i) {
    assert(ordered_params[i] != nullptr);
    params[i] = ordered_params[i];
  }
...
/**
  Check routine default value 's validity.

 @param item         IN    default value item.
 @param field_type   IN    default value related field type
 @param type         IN    default value charset info
 @param name         IN    default value charset info

 @note primary rule of check:

 - number, string don't use NOW() as default
 - no unsupported field type, like: BLOB, JSON
 - db, connection, field share same charset

 @note valid data for ENUM, SET can work now, but the validity
 check don't:

 p1( a SET('a', 'b', 'c') DEFAULT 'a,d') can pass current check
 and get a result of: 'a'
*/

bool check_for_routine_default_parameters(THD *thd, Item *item,
                                          enum_field_types field_type,
                                          PT_type *type, char *name) {
  assert(type != nullptr);
  assert(item != nullptr);
  ...
  switch (field_type) {
    ...
    // unsupported data type: SET, ENUM, BLOB, JSON, GEOMETRY
    default:

      my_error(ER_SP_INVALID_DEFAULT_PARAM, MYF(0), name, "unsupported type");

      return true;
  }

  // check for unsupported charset environment
  PT_char_type *char_type = dynamic_cast<PT_char_type *>(type);
  if (char_type != nullptr) {
    // connection and db charset same
    if (!my_charset_same(thd->variables.collation_connection,
                         thd->variables.collation_database)) {
      my_error(ER_SP_INVALID_DEFAULT_PARAM, MYF(0), name,
               "mismatched connection and db charset");
      return true;
    }

    // char charset is not null or binary and not same as DB, error
    if (char_type->get_charset() != nullptr &&
        strcmp(char_type->get_charset()->csname, "binary") != 0 &&
        !my_charset_same(char_type->get_charset(),
                         thd->variables.collation_database)) {
      my_error(ER_SP_INVALID_DEFAULT_PARAM, MYF(0), name,
               "mismatched char and db charset");
      return true;
    }
  }
...
/**
  Append default parameters's value for input

 @param thd          IN    current thread.
 @param params       INOUT array of input parameter's pointers
 @param sp           IN    sp_head represents a stored program
 @param input_count  INPUT input parameter's size
 @param is_inserted  OUT   is default param inserted

 @note primary steps:

 - copy default params with nullptr as no default placeholder
 - merge input 's positional param
 - merge input 's named param (may use sp_var's name if default is nullptr)
 - if nullptr in params, then error
*/

bool append_default_parameters(THD *thd, mem_root_deque<Item *> *params,
                               sp_head *sp, uint *input_count) {
  assert(sp != nullptr);
  assert(params != nullptr);

  sp_pcontext *root_pc = sp->get_root_parsing_context();
  uint define_count = root_pc->context_var_count();

  assert(thd->variables.sql_mode & MODE_ORACLE && *input_count < define_count);
  mem_root_deque<Item *> default_args(thd->mem_root);

  if (root_pc->copy_default_params(&default_args)) return true;

  // if input is not empty, check valid and merge
  if (params->size() > 0) {
    // check duplicated named params
    for (size_t i = 0; i < params->size(); ++i) {
      auto left = (*params)[i];

      if (left->item_name.is_autogenerated() == true) continue;

      // check for invalid name by comparing with sp definition
      bool found = false;
      for (uint j = 0; j < define_count; ++j) {
        sp_variable *var = sp->get_root_parsing_context()->find_variable(j);

        if (left->item_name.eq_safe(var->name.str)) {
          found = true;
          break;
        }
      }
      ...
      // no matching in defined param names
      if (found == false) {
        my_error(ER_SP_INVALID_DEFAULT_PARAM, MYF(0), left->item_name.ptr(),
                 "wrong name");
        return true;
      }
      ...
      for (size_t j = i + 1; j < params->size(); ++j) {
        ...
        if (right->item_name.is_autogenerated() == true) continue;

        if (left->item_name.eq_safe(right->item_name)) {
          my_error(ER_SP_INVALID_DEFAULT_PARAM, MYF(0), left->item_name.ptr(),
                   "duplicating");
          return true;
        }
    ...
    // merge input and default params
    for (size_t i = 0; i < params->size(); ++i) {
      auto arg = (*params)[i];

      if (arg->item_name.is_autogenerated() == true) {
        // use input pos param to replace default;
        default_args[i] = (*params)[i];
      } else {
        // use matched input named param to replace default;
        for (size_t j = 0; j < define_count; ++j) {
          auto d = default_args[j];
          sp_variable *var = root_pc->find_variable(j);

          // default not null
          if (d != nullptr) {
            if (d->item_name.eq_safe(arg->item_name)) {
              default_args[j] = arg;
              break;
            }
            // use var definition to decide proper replace location
          } else if (d == nullptr && arg->item_name.eq(var->name)) {
            default_args[j] = arg;
            break;
          }
        }
  ...
  if (!default_args.empty()) {
    ...
    // check for nullptr (missing param)
    for (size_t i = 0; i < default_args.size(); ++i) {
      if (default_args[i] == nullptr) {
        sp_variable *var = root_pc->find_variable(i);
        my_error(ER_SP_INVALID_DEFAULT_PARAM, MYF(0), var->name.str, "missing");
        return true;
      }
      params->push_back(default_args[i]);
    }
