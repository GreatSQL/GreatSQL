...
Item *Item_field::replace_with_derived_expr_ref(uchar *arg) {
  ...
  if (select_item) {
    ...
    if (find_item_in_list(current_thd, select_item,
      ...
      Item_ref *rr = new Item_ref(&query_block->context, replace_item, nullptr,
...
sp_rcontext *Item_splocal::get_rcontext(sp_rcontext *local_ctx) const {
  assert(m_rcontext_handler->get_rcontext(local_ctx));
...
bool Item_field_row::row_create_items(THD *thd, List<Create_field> *list) {
  ...
  if (!field->m_udt_table &&
      !(field->m_udt_table = create_tmp_table_from_fields(thd, *list)))
    return true;
  ...
  bool rec =
      row_create_items_from_cdf_list(thd, list, field->m_udt_table, &arg_count,
...
bool Item_field_row::index_item_save_in_field_no_index(Field *to) {
  ...
  for (uint i = 1; i < arg_count; i++) {
    if (sp_eval_expr(current_thd, to->m_udt_table->field[i - 1], &args[i]))
      return true;
  }
  to->set_notnull();
...
bool Item_field_row_table::row_table_create_items(
  ...
  for (uint offset = 0; (def_list = it_table++); offset++) {
    ...
    if (field_table->create_tmp_record_table_to_store_bulk_data(
            thd, row_table_name, *sd_list, 0, 0,
...
bool Item_field_row_table::insert_into_table_all_value(
  ...
  if (CountVisibleFields(items) == 1 &&
      (item->type() == ORACLE_ROWTYPE_ITEM || item->type() == FUNC_ITEM ||
       item->type() == FIELD_ITEM || item->type() == SUBSELECT_ITEM)) {
...
bool Item_field_row_table::modify_row_field_table_definitions(
  ...
  sp_variable *spv =
      thd->sp_runtime_ctx->search_variable(item_name.ptr(), item_name.length());
...
Item *Item_splocal_row_field::this_item() {
  assert(m_sp == current_thd->sp_runtime_ctx->sp);
  Item_field *item = dynamic_cast<Item_field *>(
...
void Item_splocal_row_field::print(const THD *, String *str,
...
bool Item_splocal_row_field_by_name::fix_fields(THD *thd, Item **it) {
  if (thd->sp_runtime_ctx->find_row_field_by_name_or_error(
          &m_field_idx, m_var_idx, m_field_name))
    return true;
...
*****************************************************************************/
bool Item_splocal_row_field_table_by_name::itemize(Parse_context *pc,
                                                   Item **res) {
...
Item *Item_splocal_row_field_table_by_name::make_item(
  ...
  */
  for (uint offset = 0; (def = it++); offset++) {
    if (offset == 0) {
      *def_last = *def;
      *def_name_before = def->ident;
      item = item_first;
      continue;
    }
    if (offset > 0) {
      // It must find the offset of def_last by name.Now it's def_last.def
      spv = m_pctx->find_variable(def_name_before->str, def_name_before->length,
                                  false);
      if (!spv) {
...
bool Item_splocal_row_field_table_by_name::set_value(THD *thd, sp_rcontext *ctx,
                                                     Item **it) {
  return ctx->set_variable_row_table_by_name(
      thd, thd->lex->get_sp_current_parsing_ctx(), m_var_idx, m_field_name,
      m_arg_list, it);
}
...
*****************************************************************************/
bool Item_splocal_row_field_table_by_ident::itemize(Parse_context *pc,
                                                    Item **res) {
  if (Item_splocal::itemize(pc, res)) return true;
  pc->thd->lex->is_include_udt_table_item = true;
  return false;
...
const Item *Item_splocal_row_field_table_by_ident::this_item() const {
  return const_cast<Item_splocal_row_field_table_by_ident *>(this)->this_item();
}

void Item_splocal_row_field_table_by_ident::print(const THD *, String *str,
                                                  enum_query_type) const {
  Item *item_number = current_thd->sp_runtime_ctx->get_item(m_number_idx);
  LEX_CSTRING number_cstr = to_lex_cstring(item_number->item_name.ptr());
  str->reserve(m_name.length() + m_field_name.length() + number_cstr.length);
  str->append(m_name);
  str->append('(');
  str->append(number_cstr);
  str->append(')');
  str->append('.');
  str->append(m_field_name);
}
...
/**
 * connect by  only support single table, having
 */
bool Item_ref::strip_db_table_name_processor(uchar *) {
  db_name = nullptr;
  table_name = nullptr;
  return false;
}
...
Item *Item_field::element_index(uint i) {
  ...
  if (udt && current_thd->sp_runtime_ctx) {
    ...
    Query_arena backup_arena;
    current_thd->swap_query_arena(*current_thd->sp_runtime_ctx->callers_arena,
                                  &backup_arena);
    Item *item = new (current_thd->mem_root) Item_field(tab->field[i]);
    current_thd->swap_query_arena(backup_arena,
                                  current_thd->sp_runtime_ctx->callers_arena);
    return item;
  }
