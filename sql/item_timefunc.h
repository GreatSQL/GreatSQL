...
class Item_func_to_char final : public Item_str_func {
  ...
  */
  uint16 *datetime_format_array;  // [MAX_DATETIME_FORMAT_MODEL_LEN +1];
  ...
  bool make_date_str_oracle(const MYSQL_TIME *l_time, String *str);

  bool make_num_str_oracle(my_decimal *num, String *str);
 ...
 public:
  Item_func_to_char(const POS &pos, Item *a, Item *b)
      : Item_str_func(pos, a, b),
        ...
        locale(0),
        is_date(false),
        datetime_format_array(nullptr),
        num_format(nullptr) {}

  Item_func_to_char(const POS &pos, Item *a, Item *b, Item *c)
...
class Item_func_to_date final : public Item_temporal_hybrid_func {
  enum_mysql_timestamp_type cached_timestamp_type;
  const MY_LOCALE *locale;
 ...
 public:
  Item_func_to_date(const POS &pos, Item *a, Item *b)
      : Item_temporal_hybrid_func(pos, a, b),
...
class Item_func_to_timestamp final : public Item_temporal_hybrid_func {
  enum_mysql_timestamp_type cached_timestamp_type;
  const MY_LOCALE *locale;
  uint16 decimal_digits = DATETIME_MAX_DECIMALS;  // decimal digits to keep.

  void set_decimal_digits(uint16 *fmt_array);

 protected:
  bool val_datetime(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date) override;

 public:
  Item_func_to_timestamp(const POS &pos, Item *a, Item *b)
      : Item_temporal_hybrid_func(pos, a, b),
        cached_timestamp_type(MYSQL_TIMESTAMP_NONE),
        locale(nullptr) {
    set_has_notsupported_func_true();
...
class Item_func_add_months final : public Item_temporal_hybrid_func {
  bool get_month_interval(Item *intvl_args, Interval *interval);
  bool add_months_internal(MYSQL_TIME *ltime, Interval interval, int *warnings);
