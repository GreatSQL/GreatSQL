    ...
    *error_generated = false;
    ...
    if (my_aes_needs_iv(aes_opmode)) {
      ...
      if (arg_count > 2) {
        ...
        if (!my_des_is(aes_opmode) && (!iv || iv->length() < MY_AES_IV_SIZE)) {
          my_error(ER_AES_INVALID_IV, MYF(0), func_name,
                   (long long)MY_AES_IV_SIZE);
          *error_generated = true;
          return nullptr;
        } else if (my_des_is(aes_opmode) &&
                   (!iv ||
                    (iv->length() != MY_DES_IV_SIZE && (iv->length() != 0)))) {
          my_error(ER_AES_INVALID_IV, MYF(0), func_name,
                   (long long)MY_DES_IV_SIZE);
          *error_generated = true;
          return nullptr;
        }
...
#ifdef SSL_GM
bool Item_func_sm4_encrypt::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
  /* Unsafe for SBR since result depends on a session variable */
  pc->thd->lex->set_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_SYSTEM_FUNCTION);
...
String *Item_func_sm4_encrypt::val_str(String *str) {
  char key_buff[80];
  String tmp_key_value(key_buff, sizeof(key_buff), system_charset_info);
  String *sptr, *key;
  int sm4_length;
  ...
  sm4_opmode = thd->variables.my_sm4_mode;

  if (sptr && key)  // we need both arguments to be not NULL
  {
    const unsigned char *iv_str =
        iv_arg.retrieve_iv_ptr((enum my_aes_opmode)sm4_opmode, arg_count, args,
                               func_name(), thd, &null_value);
    if (null_value) return nullptr;

    // Calculate result length
    sm4_length =
        my_aes_get_size(sptr->length(), (enum my_aes_opmode)sm4_opmode);

    tmp_value.set_charset(&my_charset_bin);
    if (!tmp_value.alloc(sm4_length))  // Ensure that memory is free
    {
      // finally encrypt directly to allocated buffer.
      if (my_aes_encrypt((unsigned char *)sptr->ptr(), sptr->length(),
                         (unsigned char *)tmp_value.ptr(),
                         (unsigned char *)key->ptr(), key->length(),
                         (enum my_aes_opmode)sm4_opmode,
                         iv_str) == sm4_length) {
        // We got the expected result length
        tmp_value.length(static_cast<size_t>(sm4_length));
        return &tmp_value;
      }
    }
  }
...
bool Item_func_sm4_encrypt::resolve_type(THD *thd) {
  ulong sm4_opmode = thd->variables.my_sm4_mode;
...
bool Item_func_sm4_decrypt::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
  /* Unsafe for SBR since result depends on a session variable */
  pc->thd->lex->set_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_SYSTEM_FUNCTION);
...
String *Item_func_sm4_decrypt::val_str(String *str) {
  assert(fixed);
  char key_buff[80];
  String tmp_key_value(key_buff, sizeof(key_buff), system_charset_info);
  String *sptr, *key;
  ...
  if (sptr && key)  // Need to have both arguments not NULL
  {
    ...
    if (!str_value.alloc(sptr->length()))  // Ensure that memory is free
    {
      ...
      length = my_aes_decrypt((unsigned char *)sptr->ptr(), sptr->length(),
                              ...
                              (unsigned char *)key->ptr(), key->length(),
                              (enum my_aes_opmode)sm4_opmode, iv_str);
      if (length >= 0)  // if we got correct data data
      {
        str_value.length((uint)length);
        return &str_value;
...
bool Item_func_sm4_decrypt::resolve_type(THD *thd) {
  if (Item_str_func::resolve_type(thd)) return true;
  set_data_type_string(args[0]->max_char_length());
  set_nullable(true);
  return false;
...
bool Item_func_sm3::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
  /* Unsafe for SBR since result depends on a session variable */
  pc->thd->lex->set_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_SYSTEM_FUNCTION);
...
String *Item_func_sm3::val_str(String *str) {
  String *msg;
  DBUG_TRACE;
  msg = args[0]->val_str(str);  // String to sm3
  if (msg) {
    str_value.set_charset(&my_charset_bin);
    if (!str_value.alloc(SM3_DIGEST_LENGTH))  // Ensure that memory is free
    {
      if (sm3((unsigned char *)msg->ptr(), msg->length(),
              (unsigned char *)str_value.ptr()) == 0) {
        str_value.length(SM3_DIGEST_LENGTH);
        return &str_value;
      }
    }
  }
...
bool Item_func_sm3::resolve_type(THD *) {
  set_data_type_string(SM3_DIGEST_LENGTH, &my_charset_bin);
  return false;
}

bool Item_func_hmac_sm3::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
  /* Unsafe for SBR since result depends on a session variable */
  pc->thd->lex->set_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_SYSTEM_FUNCTION);
...
String *Item_func_hmac_sm3::val_str(String *str) {
  String *msg;
  msg = args[0]->val_str(str);  // String to hmac_sm3
  String hmac_key;
  String *hmac_key_str = args[1]->val_str(&hmac_key);
  if (msg && hmac_key_str)  // we need both arguments to be not NULL
  {
    str_value.set_charset(&my_charset_bin);
    if (!str_value.alloc(SM3_DIGEST_LENGTH))  // Ensure that memory is free
    {
      if (hmac_sm3((unsigned char *)msg->ptr(), msg->length(),
                   (unsigned char *)str_value.ptr(),
                   (unsigned char *)hmac_key_str->ptr(),
                   hmac_key_str->length()) == 0) {
        str_value.length(SM3_DIGEST_LENGTH);
        return &str_value;
      }
    }
...
bool Item_func_hmac_sm3::resolve_type(THD *) {
  set_data_type_string(SM3_DIGEST_LENGTH, &my_charset_bin);
  return false;
}

bool Item_func_sm2_encrypt::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
  /* Unsafe for SBR since result depends on a session variable */
  pc->thd->lex->set_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_SYSTEM_FUNCTION);
...
String *Item_func_sm2_encrypt::val_str(String *str) {
  String *msg;
  msg = args[0]->val_str(str);  // String to sm2 encrypt
  String tmp;
  String *key_file_or_data = args[1]->val_str(&tmp);
  if (msg && key_file_or_data)  // we need both arguments to be not NULL
  {
    EVP_PKEY *pkey = NULL;
    FILE *fp = fopen(key_file_or_data->ptr(), "r");
    if (!fp) {
      my_error(ER_ERROR_WHEN_EXECUTING_COMMAND, MYF(0), func_name(),
               "please check whether sm2 public key file exists");
      null_value = true;
      return nullptr;
    }
    pkey = PEM_read_PUBKEY(fp, nullptr, nullptr, nullptr);
    fclose(fp);
    if (!pkey || EVP_PKEY_id(pkey) != EVP_PKEY_SM2) {
      if (pkey) EVP_PKEY_free(pkey);
      my_error(ER_ERROR_WHEN_EXECUTING_COMMAND, MYF(0), func_name(),
               "please check whether sm2 key file is public key and pem file");
      ERR_clear_error();
      null_value = true;
      return nullptr;
    }
    str_value.set_charset(&my_charset_bin);
    int cal_size = msg->length() + 1024;
    if (!str_value.alloc(cal_size))  // Ensure that memory is free
    {
      size_t dest_len = 0;
      // finally encrypt directly to allocated buffer.
      if (sm2_encrypt((unsigned char *)msg->ptr(), msg->length(),
                      (unsigned char *)str_value.ptr(), dest_len, pkey) == 0) {
        // We got the expected result length
        str_value.length(static_cast<size_t>(dest_len));
        return &str_value;
      }
    }
  }
...
bool Item_func_sm2_encrypt::resolve_type(THD *) {
  set_data_type_string(args[0]->max_length + 32, &my_charset_bin);
  return false;
}

bool Item_func_sm2_decrypt::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
  /* Unsafe for SBR since result depends on a session variable */
  pc->thd->lex->set_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_SYSTEM_FUNCTION);
...
String *Item_func_sm2_decrypt::val_str(String *str) {
  String *msg;
  msg = args[0]->val_str(str);
  String tmp;
  String *key_file_or_data = args[1]->val_str(&tmp);
  if (msg && key_file_or_data)  // we need both arguments to be not NULL
  {
    ...
    if (!fp) {
      my_error(ER_ERROR_WHEN_EXECUTING_COMMAND, MYF(0), func_name(),
               "please check whether sm2 private key file exists");
      null_value = true;
      return nullptr;
    }
    ...
    if (!pkey || EVP_PKEY_id(pkey) != EVP_PKEY_SM2) {
      if (pkey) EVP_PKEY_free(pkey);
      my_error(ER_ERROR_WHEN_EXECUTING_COMMAND, MYF(0), func_name(),
               "please check whether sm2 key file is private_key and pem file");
      ERR_clear_error();
      null_value = true;
      return nullptr;
    }
    str_value.set_charset(&my_charset_bin);
    if (!str_value.alloc(msg->length()))  // Ensure that memory is free
    {
      size_t dest_len = 0;
      if (sm2_decrypt((unsigned char *)msg->ptr(), msg->length(),
                      (unsigned char *)str_value.ptr(), dest_len, pkey) == 0) {
        // We got the expected result length
        str_value.length(static_cast<size_t>(dest_len));
        return &str_value;
...
bool Item_func_sm2_decrypt::resolve_type(THD *) {
  set_data_type_string(args[0]->max_char_length());
  set_nullable(true);
  return false;
}
...
String *Item_func_concat::val_str(String *str) {
  ...
  for (uint i = 0; i < arg_count; ++i) {
    ...
    if (m_orafun && current_thd->variables.sql_mode & MODE_ORACLE &&
        args[i]->null_value) {
...
String *Item_func_ora_oror::val_str(String *str) {
  assert(fixed);
...
bool Item_func_ora_oror::resolve_type(THD *thd) {
  if (param_type_is_default(thd, 0, -1)) return true;
...
/**
  here use map<key,value> to translate string.
**/
String *Item_func_translate::val_str(String *str) {
  assert(fixed);

  String *resString = args[0]->val_str(str);
  if ((null_value = args[0]->null_value)) return nullptr;

  resString->set_charset(collation.collation);

  tmp_value_res.length(0);
  tmp_value_res.set_charset(collation.collation);
  String *result = &tmp_value_res;

  StringBuffer<STRING_BUFFER_USUAL_SIZE> fromString_converted(nullptr);
  StringBuffer<STRING_BUFFER_USUAL_SIZE> toString_converted(nullptr);

  String *fromString =
      eval_string_arg(collation.collation, args[1], &fromString_converted);

  if (fromString == nullptr) return error_str();

  String *toString =
      eval_string_arg(collation.collation, args[2], &toString_converted);

  if (toString == nullptr) return error_str();
  if (resString->length() == 0 || fromString->length() == 0) {
    null_value = true;
    return nullptr;
  }

  THD *thd = current_thd;
  const ulong max_size = thd->variables.max_allowed_packet;

  const char *search = fromString->ptr();
  const char *searchend = fromString->ptr() + fromString->length();
  const char *to = toString->ptr();
  const char *toend = toString->ptr() + toString->length();
  const char *ptr = resString->ptr();
  const char *strend = resString->ptr() + resString->length();
  std::map<std::string, std::string> mymap;
  /*make map[fromString]=toString*/

  /**
    At first compares the fromstring and tostring,if they're the same,it
  directly returns the input string.
  **/
  if (stringcmp(fromString, toString) == 0) {
    result->copy(*resString);
    return result;
  }

  while (search < searchend) {
    uint32 start = use_mb(fromString->charset())
                       ? my_ismbchar(fromString->charset(), search, searchend)
                       : 0;
    String fromTmp;
    if (start != 0) {
      while (start-- > 0) {
        fromTmp.append(*search++);
      }
    } else {
      fromTmp.append(*search++);
    }
    ...
    if (to >= toend) {
      /**If len(fromstring)>len(totring),then it must delete the extra char.**/
      mymap.insert(std::pair<std::string, std::string>(to_string(fromTmp), ""));
      continue;
    } else {
      uint32 Sto = use_mb(toString->charset())
                       ? my_ismbchar(toString->charset(), to, toend)
                       : 0;
      String toTmp;
      if (Sto != 0) {
        while (Sto-- > 0) {
  ...
  while (ptr < strend) {
    bool err = false;
    String resTmp;
    uint32 l = use_mb(resString->charset())
                   ? my_ismbchar(resString->charset(), ptr, strend)
                   : 0;
    if (l != 0) {
      while (l-- > 0) {
        resTmp.append(*ptr++);
    ...
    if (result->length() + resTmp.length() > max_size)
      return push_packet_overflow_warning(thd, func_name());

    if (mymap.find(to_string(resTmp)) != mymap.end()) {
      std::string value = mymap.find(to_string(resTmp))->second;

      err |= result->append(value.data(), value.length());
    } else
      err |= result->append(resTmp.ptr(), resTmp.length());

    if (err) return error_str();
  }
  return result->length() ? result : (null_value = true) ? nullptr : result;
}
...
static char *str2hex(const CHARSET_INFO *cs, char *to, int to_base,
  ...
  for (; str != str_end; ++str) {
    ...
    if (to_base > 0) {
      tmp[0] = dig_vec_lower[((uchar)*str) >> 4];
      tmp[1] = dig_vec_lower[((uchar)*str) & 0x0F];

      if (to_base != 16) {
        dec = (longlong)my_strntoull(cs, tmp, 2, 16, &endptr, &err);
        char ans[MAX_FIELD_WIDTH];
        memset(ans, 0, sizeof(ans));
        char *ptr = longlong2str(dec, ans, to_base);
        memcpy(to + tolen, ans, ptr - ans);
        tolen += ptr - ans;
      } else {
        to[tolen] = tmp[0];
        to[tolen + 1] = tmp[1];
      ...
      tmp[0] = to[tolen] = (uchar)*str;

      if (!my_isascii(tmp[0])) {
        to[tolen] = dig_vec_lower[((uchar)*str) >> 4];
        to[tolen + 1] = dig_vec_lower[((uchar)*str) & 0x0F];
        tolen += 1;
      }
      tolen += 1;
    }
...
String *Item_func_dump::val_str(String *str) {
  assert(fixed);
  uchar *storebuff = nullptr;
  char *outbuff = nullptr;
  int flag_charset = 0;
  longlong tmp_length = 0, storelen = 0, length = 0, data_type = 0, start = 0,
           to_base = 0;
  ...
  if ((null_value =
           (args[0]->null_value || (arg_count > 2 && args[2]->null_value) ||
            (arg_count == 4 && args[3]->null_value)))) {
    null_value = true;
    return nullptr; /* purecov: inspected */
  }

  if (args[0]->is_null() || res->is_empty()) {
    null_value = true;
    return nullptr;
  }
  ...
  /* must be longlong to avoid truncation */
  start = arg_count <= 2 ? 0 : args[2]->val_int();

  /* if "unsigned_flag" is set, we have a *huge* positive number. */
  /* Assumes that the maximum length of a String is < INT_MAX32. */
  if ((arg_count > 2) &&
      ((!args[2]->unsigned_flag && (start < INT_MIN32 || start > INT_MAX32)) ||
       (args[2]->unsigned_flag && ((ulonglong)start > INT_MAX32)))) {
    null_value = true;
    return nullptr; /* purecov: inspected */
  }
  ...
  /* Assumes that the maximum length of a String is < INT_MAX32. */
  /* Limit so that code sees out-of-bound value properly. */
  length = arg_count == 4 ? args[3]->val_int() : INT_MAX32;

  if (length < 0) length = abs(length);

  if ((length == 0) || (length > INT_MAX32)) length = INT_MAX32;

  to_base = arg_count >= 2 ? (int)args[1]->val_int() : 10;

  if (to_base != 2 && to_base != 8 && to_base != 16 && to_base < 16)
    to_base = 10;

  if (to_base > 1000) {
    if ((to_base - 1000 == 2) || (to_base - 1000 == 8) ||
        (to_base - 1000 == 10) || (to_base - 1000 == 16))
      to_base = to_base - 1000;
    if (data_type == MYSQL_TYPE_VARCHAR || data_type == MYSQL_TYPE_STRING ||
        data_type == MYSQL_TYPE_VAR_STRING)
      flag_charset = 1;
  }

  if (to_base > 16)
    (data_type == MYSQL_TYPE_VARCHAR || data_type == MYSQL_TYPE_STRING ||
     data_type == MYSQL_TYPE_VAR_STRING)
        ? to_base = 0
        : to_base = 16;
  ...
  if (!to_base) {
    storelen = res->length();
    storebuff = static_cast<uchar *>(my_alloca(storelen + 1));
    memset(storebuff, 0, storelen + 1);
    memcpy(storebuff, res->ptr(), res->length());
  } else if (Item_field *item_field = dynamic_cast<Item_field *>(args[0])) {
    ...
    storelen = item_field->field->pack_length();
    ...
    if (Field_varstring *varstr =
      ...
      storelen = content_length + length_bytes;
    ...
    storebuff = static_cast<uchar *>(my_alloca(storelen + 1));
    memset(storebuff, 0, storelen + 1);
    memcpy(storebuff, field_ptr, storelen);
  } else {
    storelen = calc_key_length(args[0]->data_type(), res->length(),
                               args[0]->decimals, args[0]->unsigned_flag, 0);

    storebuff = static_cast<uchar *>(
        my_alloca(storelen + 2 + 1));  // +2 for may by need add length
    memset(storebuff, 0, storelen + 2 + 1);
    const char *endptr = res->ptr() + res->length();
    ...
    switch (data_type) {
      ...
      case MYSQL_TYPE_VARCHAR: {
        int2store(storebuff, storelen);
        storelen += 2;
        memcpy(storebuff + 2, res->ptr(), res->length());
        break;
      }
      ...
      case MYSQL_TYPE_TINY: {
        int err;
        longlong data = my_strtoll10(res->ptr(), &endptr, &err);
        *storebuff = (uchar)data;
        break;
      }

      case MYSQL_TYPE_YEAR: {
        longlong i32 = args[0]->val_int() - 1900;
        shortstore(storebuff + 3, (short)i32);  // YEAR store length 4
        storelen += 3;                          // unsigned_flag =[1].
        break;
      }
      ...
      case MYSQL_TYPE_SHORT: {
        int err;
        longlong data = my_strtoll10(res->ptr(), &endptr, &err);
        shortstore(storebuff, (short)data);
        break;
      }
      ...
      case MYSQL_TYPE_LONG: {
        int err;
        longlong data = my_strtoll10(res->ptr(), &endptr, &err);
        longstore(storebuff, (int32)data);
        break;
      }
      ...
      case MYSQL_TYPE_LONGLONG: {
        ...
        if (dynamic_cast<Item_func_false *>(args[0]) ||
          ...
          // SELECT VSIZE(false);  // SELECT VSIZE(true);
          storelen = 1;
          data_type = MYSQL_TYPE_BOOL;
          storebuff[0] = (uint)data;
        } else
          longlongstore(storebuff, data);
      ...
      case MYSQL_TYPE_FLOAT: {
        int err;
        double data = my_strntod(&my_charset_latin1, res->ptr(), res->length(),
                                 &endptr, &err);
        float fdata = (float)data;
        floatstore(storebuff, fdata);
        break;
      }
      ...
      case MYSQL_TYPE_NEWDECIMAL: {
        my_decimal m;
        my_decimal *r = args[0]->val_decimal(&m);
        my_decimal2binary(E_DEC_ERROR, r, storebuff, r->precision(), r->frac);
      ...
      case MYSQL_TYPE_DATE: {
        MYSQL_TIME ltime;
        args[0]->get_date(&ltime, TIME_NO_ZERO_DATE);
        my_date_to_binary(&ltime, storebuff);
        storelen -= 1;  // 3
        break;
      }
      ...
      case MYSQL_TYPE_TIMESTAMP2: {
        my_timeval tm;
        int warnings = 0;
        args[0]->get_timeval(&tm, &warnings);
        my_timestamp_to_binary(&tm, storebuff, 6);
        break;
      }
      ...
      case MYSQL_TYPE_DATETIME2: {
        MYSQL_TIME ltime;
        args[0]->get_time(&ltime);
        longlong templong = TIME_to_longlong_packed(ltime);
        my_datetime_packed_to_binary(templong, storebuff, 6);
        break;
      }
      ...
      case MYSQL_TYPE_TIME2: {
        ...
        args[0]->get_time(&ltime);
        longlong templong = TIME_to_longlong_packed(ltime);
        my_time_packed_to_binary(templong, storebuff, 6);
        break;
      }
  ...
  start = ((start < 0) ? storelen + start : start == 0 ? 0 : start - 1);

  if ((start < 0) || (start + 1 > storelen)) {
    null_value = true;
    return nullptr; /* purecov: inspected */
  }

  tmp_length = storelen - start;
  length = min(length, tmp_length);

  if (to_base == 2) {
    outbuff = static_cast<char *>(my_alloca(storelen * 9 + 150));
    memset(outbuff, 0, storelen * 9 + 150);
  } else {
    outbuff = static_cast<char *>(my_alloca(storelen * 4 + 150));
    memset(outbuff, 0, storelen * 4 + 150);
  }

  if (flag_charset)
    sprintf(outbuff, "Type=%lld Len=%lld CharacterSet=%s:", data_type, storelen,
            (collation.collation)->csname);
  else
    sprintf(outbuff, "Type=%lld Len=%lld:", data_type, storelen);

  str2hex(res->charset(), outbuff + strlen(outbuff), to_base,
          (char *)storebuff + start, length);
  tmp_value.length(0);
  tmp_value.append(outbuff, (uint32)strlen(outbuff));
  return &tmp_value;
...
bool Item_func_dump::resolve_type(THD *thd) {
  if (arg_count > 4) my_error(ER_INVALID_PARAMETER_USE, MYF(0), func_name());

  if (param_type_is_default(thd, 0, 1)) return true;

  if (param_type_is_default(thd, 1, 3, MYSQL_TYPE_LONGLONG)) return true;
  uint32 max_head = 20;
  uint32 max_charset = 30;
  uint multiply = collation.collation->caseup_multiply;
  uint32 max_char_length =
      max_head + max_charset + args[0]->max_char_length() * multiply * 4;
  ...
  if (arg_count == 4 && args[3]->const_item()) {
    longlong length = args[3]->val_int() + max_head + max_charset;

    if (args[3]->null_value) goto end;

    Integer_value length_val(length, args[3]->unsigned_flag);
  ...
  collation.set(system_charset_info, DERIVATION_IMPLICIT);  // add
  set_data_type_string(max_char_length);
  return false;
}
...
String *Item_func_substrb::val_str(String *str) {
  ...
  /* Assumes that the maximum length of a String is < INT_MAX32. */
  /* Set here so that rest of code sees out-of-bound value as such. */
  if ((lengthb <= 0) || (lengthb > INT_MAX32)) lengthb = INT_MAX32;
  ...
  tmp_value.set(*res, (uint32)startb, (uint32)lengthb);

  /* now fill 0x20 if start or end is not in char boundary */
  ub = startb;
  lb = startb / res->charset()->mbmaxlen;
  for (i = lb; i <= ub; i++) {
    charb = res->charpos(i);
    if (charb == startb) break;
    if (charb > startb) {
      char *c_ptr;
      if (!tmp_value.is_alloced()) tmp_value.copy();
      c_ptr = tmp_value.c_ptr();
      endb = charb - startb;
      if (endb > lengthb) endb = lengthb;
      for (i = 0; i < endb; i++) c_ptr[i] = 0x20;
      break;
    }
  }
  ...
  for (i = lb; i <= ub; i++) {
    ...
    if (charb > endb) {
      ...
      charb = res->charpos(i);
      if (charb < startb) break;
      for (i = charb - startb; i < endb; i++) c_ptr[i] = 0x20;
      break;
    }
  }
  return &tmp_value;
...
inline void append_char(String *str, int32 num, const CHARSET_INFO *cs) {
  char tmp[4];
  if (num & 0xFF000000L) {
  ...
  if (oracle_mode_flag) {
    ...
    */
    ...
    /*
      ...
      at this stage.
    */
    res = alloc_buffer(res, str, &tmp_value,
                       static_cast<size_t>(target_byte_size));
    if (res == nullptr) return error_str();
...
bool Item_func_sys_guid::resolve_type(THD *) {
  collation.set(system_charset_info, DERIVATION_COERCIBLE, MY_REPERTOIRE_ASCII);
  set_data_type_string(uint32(UUID_LENGTH - 4));
  return false;
}

bool Item_func_sys_guid::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
  pc->thd->lex->set_stmt_unsafe(LEX::BINLOG_STMT_UNSAFE_SYSTEM_FUNCTION);
  pc->thd->lex->safe_to_cache_query = false;
...
String *mysql_generate_sys_guid(String *str) {
  char *s;
  THD *thd = current_thd;
  ...
  str->mem_realloc(UUID_LENGTH + 1 - 4);
  str->length(UUID_LENGTH - 4);
  str->set_charset(system_charset_info);
  s = str->ptr();
  // s[8] = s[13] = '-';
  tohex(s, time_low, 8);
  tohex(s + 8, time_mid, 4);
  tohex(s + 12, time_hi_and_version, 4);
  int j, k = 16;
  for (j = 0; clock_seq_and_node_str[j] != '\0'; j++)
    if (clock_seq_and_node_str[j] != '-') s[k++] = clock_seq_and_node_str[j];

  DBUG_EXECUTE_IF("force_fake_guid",
                  my_stpcpy(s, "a2d00942b69c11e4a6960020ff6fcbe6"););
  return str;
}

String *Item_func_sys_guid::val_str(String *str) {
  assert(fixed);
  return mysql_generate_sys_guid(str);
}

bool Item_func_start_secondary_engine_increment_load_task::resolve_type(
    THD *thd) {
  DBUG_TRACE;
  if (param_type_is_default(thd, 0, 3)) return true;
  if (param_type_is_default(thd, 3, 4, MYSQL_TYPE_LONGLONG)) return true;

  collation.set(default_charset(), DERIVATION_COERCIBLE, MY_REPERTOIRE_ASCII);

  if (!auto_position) {
    set_data_type_string(args[2]->max_length + 64 * 2 + 16);
  } else {
    set_data_type_string((uint32_t)64 * 2 + 16);
  }
  return false;
}

String *Item_func_start_secondary_engine_increment_load_task::val_str_ascii(
    String *str) {
  DBUG_TRACE;
  ...
  const char *charp3 = nullptr;
  if (arg_count >= 3) {
    String *str3 = args[2]->val_str_ascii(&buf3);
    if (str3 == nullptr) {
      return error_str();
    }
    charp3 = str3->c_ptr_safe();
    if (str3->length() == 0) auto_position = true;
    assert(charp3 != nullptr);
  }

  if (arg_count >= 4) {
    strict_mode = args[3]->val_int();
  }

  size_t length = 256;
  if (!str->mem_realloc(length)) {
    str->length(0);
    Secondary_engine_increment_load_task_manager &manager =
        Secondary_engine_increment_load_task_manager::instance();
    (void)manager.start_inc_load_task(charp1, charp2, charp3, str,
                                      auto_position, strict_mode);
    return str;
  }
  return error_str();
}

bool Item_func_stop_secondary_engine_increment_load_task::resolve_type(
    THD *thd) {
  if (param_type_is_default(thd, 0, -1)) return true;

  collation.set(default_charset(), DERIVATION_COERCIBLE, MY_REPERTOIRE_ASCII);
  set_data_type_string((uint32_t)(64 * 2));
  return false;
}

String *Item_func_stop_secondary_engine_increment_load_task::val_str_ascii(
    String *str) {
  DBUG_TRACE;
  ...
  if (!str->mem_realloc(length)) {
    ...
    Secondary_engine_increment_load_task_manager &manager =
        Secondary_engine_increment_load_task_manager::instance();
    (void)manager.stop_inc_load_task(charp1, charp2, str);
    return str;
  }
  return error_str();
}

bool Item_func_read_secondary_engine_load_gtid::resolve_type(THD *thd) {
  if (param_type_is_default(thd, 0, -1)) return true;
...
String *Item_func_read_secondary_engine_load_gtid::val_str_ascii(String *str) {
  DBUG_TRACE;
  ...
  if (!str->mem_realloc(length)) {
    ...
    Secondary_engine_increment_load_task_manager &manager =
        Secondary_engine_increment_load_task_manager::instance();
    (void)manager.read_load_gtid(charp1, charp2, str);
    return str;
  }
  return error_str();
}
...
String *Item_func_get_dd_tablespace_private_data::val_str(String *str) {
  ...
  if (p->exists("secondary_load")) {
    dd::String_type tmp_opt_value;
    p->get("secondary_load", &tmp_opt_value);
    if (!tmp_opt_value.empty()) {
      ptr = my_stpcpy(ptr, " SECONDARY_LOAD=\"");
      ptr = my_stpcpy(ptr, tmp_opt_value.c_str());
      ptr = my_stpcpy(ptr, "\"");
    }
  }
  ...
  while (src < srcend) {
    ...
    if (1 == mblen) {
      ...
      */
      ...
      was_alnum =
          ((ctype & MY_CHAR_NMR) || stringcmp(to_upper_ptr, to_lower_ptr) != 0);
    }
    src += mblen;
  }
...
bool Item_func_sqlerrm::resolve_type(THD *thd) {
  if (!(thd->variables.sql_mode & MODE_ORACLE)) {
    my_error(ER_SYNTAX_ERROR, MYF(0));
    return true;
  }
...
bool Item_func_table_first::resolve_type(THD *thd) {
  ...
  auto row_table_item = dynamic_cast<Item_field_row_table *>(
      m_rcontext_handler->get_rcontext(thd->sp_runtime_ctx)
          ->get_item(m_spv_idx));
  if (!row_table_item) {
...
bool Item_func_maskall::resolve_type(THD *thd) {
  // 0 1
  if (param_type_is_default(thd, 0, 2)) return true;
  ...
  if (arg_count == 2) {
    if (args[1]->type() == STRING_ITEM) {
      ...
      auto s = args[1]->val_str(&tmpstr);
      if (s && !args[1]->null_value && s->numchars() != 0) {
        auto len = s->charpos(1);
        mask_char.copy(s->ptr(), len, s->charset());
      }
      fix_mask = true;
    }
  } else {
...
String *Item_func_maskall::val_str(String *str) {
  null_value = true;

  auto res = args[0]->val_str(str);
  ...
  if (!fix_mask) {
    StringBuffer<STRING_BUFFER_USUAL_SIZE> tmpstr;
    auto mask_c = args[1]->val_str(&tmpstr);
    if (mask_c && !args[1]->null_value && mask_c->numchars() != 0) {
      auto mask_len = mask_c->charpos(1);
      mask_char.copy(mask_c->ptr(), mask_len, mask_c->charset());
    }
  }

  if (mask_char.length() == 1) {
    if (mask_char.ptr()[0] == '\0') {
      mask_char.ptr()[0] = 'x';
    }
  }

  result_buffer.length(0);
  result_buffer.set_charset(collation.collation);
  auto nchar_len = res->numchars();
  while (nchar_len--) {
    result_buffer.append(mask_char);
  }
...
bool Item_func_mask_inside::resolve_type(THD *thd) {
  if (arg_count < 3 && !end_pos_fix && !start_pos_fix) {
    my_error(ER_INVALID_PARAMETER_USE, MYF(0), func_name());
    return true;
  }
  // str min max mask_char
  if (param_type_is_default(thd, 0, 1)) return true;
  // start end
  if (param_type_is_default(thd, 1, 3, MYSQL_TYPE_LONGLONG)) return true;

  if (arg_count > 3) {
    if (args[3]->propagate_type(thd, MYSQL_TYPE_VARCHAR)) return true;
    if (!fix_mask)
      if (args[3]->type() == STRING_ITEM) {
        StringBuffer<STRING_BUFFER_USUAL_SIZE> tmpstr;

        auto s = args[3]->val_str(&tmpstr);
        if (s && !args[3]->null_value && s->numchars() != 0) {
          auto len = s->charpos(1);
          mask_char.copy(s->ptr(), len, s->charset());
        }
        fix_mask = true;
  ...
  if (arg_count > 1)
    if (args[1]->const_item() && args[1]->may_eval_const_item(thd)) {
      auto start = args[1]->val_int();
      if (args[1]->null_value) {
        start = 0;
      }
      if (!args[1]->unsigned_flag && start < 0) {
        start = 0;
      }
      start_pos = start;
      start_pos_fix = true;
  ...
  if (arg_count > 2)
    if (args[2]->const_item() && args[2]->may_eval_const_item(thd)) {
      auto end = args[2]->val_int();
      if (args[2]->null_value) {
        end = INT_MAX32;
      }
      if (!args[2]->unsigned_flag && end < 0) {
        end = INT_MAX32;
      }
      end_pos = end;
      end_pos_fix = true;
...
String *Item_func_mask_inside::val_str(String *str) {
  ...
  if (!start_pos_fix) {
    auto start_tmp = args[1]->val_int();
    if (args[1]->null_value) {
      start_tmp = 0;
    }
    if (!args[1]->unsigned_flag && start_tmp < 0) {
      start_tmp = 0;
    }
    start = start_tmp;
  }
  ...
  if (!end_pos_fix) {
    auto tmp_end = args[2]->val_int();
    if (args[2]->null_value) {
      tmp_end = res->numchars();
    }
    if (!args[2]->unsigned_flag && tmp_end < 0) {
      tmp_end = res->numchars();
    }
    end = tmp_end;
  }
  ...
  if (!fix_mask) {
    StringBuffer<STRING_BUFFER_USUAL_SIZE> tmpstr;
    auto mask_c = args[3]->val_str(&tmpstr);
    if (mask_c && !args[3]->null_value && mask_c->numchars() != 0) {
      auto mask_len = mask_c->charpos(1);
      mask_char.copy(mask_c->ptr(), mask_len, mask_c->charset());
    }
  }
  ...
  result_buffer.length(0);
  result_buffer.set_charset(collation.collation);
  if (res->length() != 0 && start < res->length() && start < end) {
    auto nchar_len = res->numchars();
    end = end > nchar_len ? nchar_len : end;

    if (nchar_len != 0) {
      auto start_offset_len = res->charpos(start);
      if (start_offset_len != 0) {
        result_buffer.copy(res->ptr(), start_offset_len, res->charset());
      }
      ...
      if (end < nchar_len) {
        auto end_offset = res->charpos(end);
        if (res->length() - end_offset != 0)
          result_buffer.append(res->ptr() + end_offset,
                               res->length() - end_offset, res->charset());
      }
    }
  } else {
    result_buffer.copy(res->ptr(), res->length(), res->charset());
  }
  null_value = false;
  return &result_buffer;
}

String *Item_func_alter_all_event::val_str(String *str) {
  DBUG_TRACE;
  std::string action, where_schema, where_name, where_status;
  std::ostringstream oss("");

  String *string1 = args[0]->val_str(&buf1);
  String *string2 = args[1]->val_str(&buf2);
  String *string3 = args[2]->val_str(&buf3);
  if (string1 == nullptr || string2 == nullptr || string3 == nullptr) {
    oss << func_name() << " . Varibales is NULL";
    my_error(ER_INVALID_PARAMETER_USE, MYF(0), oss.str().c_str());
    return error_str();
  }
  action = string1->c_ptr();
  std::transform(action.begin(), action.end(), action.begin(), ::toupper);
  where_schema = string2->c_ptr();
  where_name = string3->c_ptr();
  // Evaluate variables
  if (action == "ENABLE") {
    where_status = "DISABLED";
  } else if (action == "DISABLE") {
    where_status = "ENABLED";
  } else {
    oss << func_name()
        << ". Invalid alter_action parameter. Must be enable or disable";
    my_error(ER_INVALID_PARAMETER_USE, MYF(0), oss.str().c_str());
    return error_str();
  }
  if (where_schema.find(";") != std::string::npos ||
      where_name.find(";") != std::string::npos) {
    oss << func_name()
        << ". db_pattern or name_pattern contains invalid characters ';'";
    my_error(ER_INVALID_PARAMETER_USE, MYF(0), oss.str().c_str());
    return error_str();
  }
  if (where_schema.empty()) where_schema = "%";
  if (where_name.empty()) where_name = "%";

  Gdb_cmd_service cmd_service;
  if (cmd_service.open_cur_session()) {
    oss << "Init_st_session failed";
    my_error(ER_CMD_SERVICE_FAIL, MYF(0), func_name(), ER_CMD_SERVICE_FAIL,
             oss.str().c_str());
    return error_str();
  }
  ...
  String select_sql;
  select_sql.set_charset(system_charset_info);
  select_sql.append(
      STRING_WITH_LEN("SELECT EVENT_SCHEMA, EVENT_NAME, STATUS FROM "
                      "information_schema.events WHERE EVENT_SCHEMA LIKE "));
  append_ident(&select_sql, where_schema.c_str(), where_schema.length(),
               VALUE_QUOTE_CHAR);
  select_sql.append(STRING_WITH_LEN(" AND EVENT_NAME LIKE "));
  append_ident(&select_sql, where_name.c_str(), where_name.length(),
               VALUE_QUOTE_CHAR);
  select_sql.append(STRING_WITH_LEN(" AND STATUS LIKE "));
  append_ident(&select_sql, where_status.c_str(), where_status.length(),
               VALUE_QUOTE_CHAR);
  auto &cb_data = cmd_service.get_cb_data();
  if (cmd_service.execute_sql(select_sql.c_ptr()) || cb_data.is_error()) {
    oss << "Execute [" << select_sql.c_ptr() << "] failed due to ["
        << cb_data.error_msg() << "]";
    my_error(ER_CMD_SERVICE_FAIL, MYF(0), func_name(), cb_data.error_no(),
             oss.str().c_str());
    return error_str();
  }

  std::vector<std::string> event_name_vec;
  for (uint i = 0; i < cb_data.rows(); i++) {
    String full_event_name;
    full_event_name.set_charset(system_charset_info);
    append_ident(&full_event_name, cb_data.get_value(i, 0).c_str(),
                 cb_data.get_value(i, 0).length(), IDENT_QUOTE_CHAR);
    full_event_name.append(STRING_WITH_LEN("."));
    append_ident(&full_event_name, cb_data.get_value(i, 1).c_str(),
                 cb_data.get_value(i, 1).length(), IDENT_QUOTE_CHAR);
    event_name_vec.push_back(full_event_name.c_ptr());
  }
  for (auto it : event_name_vec) {
    std::string alter_sql = "ALTER EVENT " + it + " " + action;
    if (cmd_service.execute_sql(alter_sql) || cb_data.is_error()) {
      oss << "Already alter " << aft_rows << " events. Then alter "
          << it.c_str() << " failed due to " << cb_data.error_msg();
      my_error(ER_CMD_SERVICE_FAIL, MYF(0), func_name(), cb_data.error_no(),
               oss.str().c_str());
      return error_str();
    }
  ...
  null_value = false;
  oss << "Alter success " << aft_rows << " events.";
  str->copy(oss.str().c_str(), oss.str().length(), system_charset_info);
  return str;
}
