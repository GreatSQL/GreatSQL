...
#define SLAVE_REPORT_ERR_STR "replica_report%s.error"
...
/**
   ...
   @param report_file the extra info of reported may need to write to file
...
static void inline slave_rows_error_report(enum loglevel level, int ha_error,
                                           ...
                                           const char *report_file = nullptr) {
    ...
    if (ha_error != 0) {
      ...
      if (report_file) {
        ...
        rli->report(level,
                    thd->is_error() ? thd->get_stmt_da()->mysql_errno()
                                    : ER_UNKNOWN_ERROR,
                    "Related event data has been written to file: %s",
                    report_file);
      }
    } else {
      rli->report(level,
  ...
  */
  DBUG_PRINT(
      ...
      ("ev->server_id=%lu, ::server_id=%lu,"
       ...
       (ulong)server_id, (ulong)::server_id, rli->replicate_same_server_id,
       rli->replicate_server_id_mode, rli->slave_skip_counter.load()));
...
void Rows_log_event::print_verbose(IO_CACHE *file,
  ...
  my_b_printf(file, "### affected rows: %lu\n", affected_row);
...
void Log_event::print_base64(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info,
  ...
  if (print_event_info->row_events_base64_max_size) {
    if (get_type_code() == mysql::binlog::event::TABLE_MAP_EVENT) {
      print_event_info->last_table_map_base64.push_back(tmp_str);
      print_event_info->last_table_map_size += size;
      if (print_event_info->verbose) {
        // prepare table_map for print verbose
        ...
        Table_map_log_event *map =
            new Table_map_log_event((const char *)ptr, &fd_evt);
  ...
  print_event_info->need_print_end = false;
  if (print_event_info->base64_output_mode != BASE64_OUTPUT_DECODE_ROWS) {
    if (my_b_tell(file) == 0 || print_event_info->row_events_base64_max_size) {
      if (print_event_info->row_events_base64_max_size) {
        // calc_size : current event + table_map size
        if (!print_event_info->last_table_map_base64.empty()) {
          Log_event_type type_code = get_type_code();
          if (type_code == mysql::binlog::event::UPDATE_ROWS_EVENT ||
              type_code == mysql::binlog::event::WRITE_ROWS_EVENT ||
              type_code == mysql::binlog::event::DELETE_ROWS_EVENT ||
              type_code ==
                  mysql::binlog::event::OBSOLETE_UPDATE_ROWS_EVENT_V1 ||
              type_code == mysql::binlog::event::OBSOLETE_WRITE_ROWS_EVENT_V1 ||
              type_code ==
                  mysql::binlog::event::OBSOLETE_DELETE_ROWS_EVENT_V1 ||
              type_code == mysql::binlog::event::PARTIAL_UPDATE_ROWS_EVENT) {
            // print table_map
            if (print_event_info->split_events_when_print == 0) {
              my_b_printf(file, "\nBINLOG '\n");
              print_event_info->print_last_table_map_base64(file);
              if (!more) {
                print_event_info->clear_last_table_map_base64();
              }
            }
            ulong calc_size = size + print_event_info->last_table_map_size;
            if (!more ||
                print_event_info->row_events_base64_size_calc + calc_size >=
                    print_event_info->row_events_base64_max_size) {
              print_event_info->need_print_end = true;
              print_event_info->split_events_when_print = 0;
              print_event_info->row_events_base64_size_calc = 0;
            } else {
              print_event_info->row_events_base64_size_calc += calc_size;
              print_event_info->split_events_when_print += 1;
            }
          } else {
            // print table map
            my_b_printf(file, "\nBINLOG '\n");
    ...
    if (!more || (print_event_info->need_print_end)) {
  ...
  if (print_event_info->verbose) {
    ...
    switch (et) {
      // if row-events-base64-max-size > 0, 'et' will not be TABLE_MAP_EVENT .
...
int Log_event::apply_event(Relay_log_info *rli) {
  ...
  if (worker && rli->m_next_worker_wait_last_committed != SEQ_UNINIT) {
    worker->m_next_worker_wait_last_committed =
        rli->m_next_worker_wait_last_committed;
    rli->m_next_worker_wait_last_committed = SEQ_UNINIT;
  }
...
int Xid_apply_log_event::do_apply_event_worker(Slave_worker *w) {
  ...
  error = do_commit(thd);
  if (error) {
    thd->rli_slave->report(WARNING_LEVEL, thd->get_stmt_da()->mysql_errno(),
  ...
  if (error) {
    ...
    if (idempotent_error || ignored_error) {
      ...
      slave_rows_error_report(
          ...
          (ulong)common_header->log_pos, nullptr);
  ...
  if (error && error != HA_ERR_RECORD_DELETED) {
    if (error != HA_ERR_KEY_NOT_FOUND) {
      ...
      if (m_table->s->primary_key != MAX_KEY)
        print_key_contains_error(m_table, m_key_info, MYF(0));
    }
  } else {
    ...
    switch (error) {
      case 0: {
        ...
        /**
          ...
          We should also take care of the scenario of collisions occurs between
          the AI and another row's BI ​within a single Update_rows_log_event.
          Ex:
            UPDATE  A1 -> A2
            UPDATE  B1 -> B2
          A2 and B1 have the same hash value after the make_hash_key() function
          call.​ Then within the do..while loop, it attempts to update B1 to
          B2. However, without the row positioning capability for B1 provided by
          next_record_scan(), the update targets an incorrect row, leading to
          data inconsistency.​

          Therefore, If m_hash.get() retrieves the next entry, then in the next
          loop, after record_compare():
            1. entry is not nullptr (found the entry we needed)
                 Ex:
                   UPDATE  A1 -> A2
                   UPDATE  A2 -> A3
                 In this case, we can update A2 to A3.
            2. entry is nullptr (not found the entry we needed)
                 Ex:
                   UPDATE  A1 -> A2
                   UPDATE  B1 -> B2
                 In this case, we can't update B1 to B2 but break the loop
                 instead.
        */
        do {
          store_record(table, record[1]);
          ...
          if (entry) {
            ...
            */
            ...
            // We have not found the entry we needed.
            break;
          }
...
close_table:
  ...
  if (error) {
    if (replication_error_write_file &&
        (error == HA_ERR_END_OF_FILE || error == HA_ERR_KEY_NOT_FOUND)) {
      store_record(table, record[1]);
      init_report_file(rli);
      print_row_data_to_report_file(rli, table);
    }
    table->file->print_error(error, MYF(0));
...
void Rows_log_event::init_report_file(const Relay_log_info *rli) {
  char filename[FN_REFLEN] = {0};
  char *cur_channel = rli->get_channel();
  std::string tmp_channel;
  if (cur_channel && strcmp(cur_channel, "")) {
    tmp_channel.append("_");
    tmp_channel.append(cur_channel);
  }
  snprintf(filename, sizeof(filename), SLAVE_REPORT_ERR_STR,
           tmp_channel.c_str());
  report_file = filename;
}
int Rows_log_event::print_hash_info_to_cache(IO_CACHE *cache,
                                             Relay_log_info const *rli,
                                             TABLE *table) {
  int error = 0;
  HASH_ROW_ENTRY *entry = nullptr;
  size_t m_hash_size = m_hash.size();
  my_b_printf(cache,
              "# Event scan and not apply rows: %lu\n# Event scan and not "
              "apply data:\n",
              m_hash_size);
  bool is_beyond_limit = false;
  if (m_hash_size > replication_error_write_file) {
    is_beyond_limit = true;
    m_hash_size = replication_error_write_file;
  }
  auto write_fields_to_cache = [&cache, &table]() {
    for (uint f = 0; f < table->s->fields; f++) {
      Field *cur_field = table->field[f];
      char field_buff[MAX_FIELD_WIDTH];
      String tmp_buf(field_buff, sizeof(field_buff), system_charset_info);
      tmp_buf.length(0);
      if (!(is_blob(cur_field->type()))) {
        // when type is BIT or NULL, need to do field_unpack
        field_unpack(&tmp_buf, cur_field, 0, false);
      } else {
        // when type is blob, need val_str to get_blob_data
        cur_field->val_str(&tmp_buf);
      }
      const char *format_str =
          cur_field->is_null() ? "# %s=%s\n" : "# %s='%s'\n";
      my_b_printf(cache, format_str, cur_field->field_name, tmp_buf.c_ptr());
    }
  };

  // m_distinct_keys_original_order[m_distinct_key_idx]
  if (m_key) {
    // store the m_key back to table
    key_restore(table->record[0], m_key, m_key_info, 0);
    char key_buff[MAX_KEY_LENGTH];
    String str(key_buff, sizeof(key_buff), system_charset_info);
    str.length(0);
    key_unpack(&str, table, m_key_info);
    my_b_printf(cache, "# lost data key: %s=%s\n", m_key_info->name, str.ptr());
    return error;
  }
  size_t row_i = 0;
  while (!rli->abort_slave && row_i < m_hash_size) {
    entry = m_hash.get_one_entry();
    ++row_i;
    if (entry) {
      m_curr_row = entry->positions->bi_start;
      m_curr_row_end = entry->positions->bi_ends;
      ...
      my_b_printf(cache, "## row [%lu] :\n", row_i);
      // print
      write_fields_to_cache();
      if ((error = m_hash.del(entry))) return error;
    } else {
      break;
    }
  }
  ...
  if (is_beyond_limit)
    my_b_printf(
        cache,
        "## Event has too many conflict rows, will only output [%u] rows.\n",
        replication_error_write_file);
  return error;
}
/**
  Write row data to a outfile for error diagnostics.
  This is used when a table has no primary key and replication
  occurs, to help identify the problematic row.

  @param rli         pointer to the active Relay_log_info instance
  @param table       pointer to the event's table object
  @return            0 on success, non-zero on error
*/
int Rows_log_event::print_row_data_to_report_file(Relay_log_info const *rli,
                                                  TABLE *table) {
  if (!rli || !table) return 1;
  int error = 0;
  // 1. init
  File fd;
  IO_CACHE io_cache;
  if ((fd = my_open(report_file.c_str(), O_WRONLY | O_CREAT | O_TRUNC,
                    MYF(MY_WME))) < 0) {
    LogErr(ERROR_LEVEL, ER_RPL_REPLICA_ERROR_OPEN_FILE_FAILED, "file",
           report_file.c_str(), my_errno());
    return 1;
  }
  if (init_io_cache(&io_cache, fd, IO_SIZE * 2, WRITE_CACHE, 0L, false,
                    MYF(MY_WME))) {
    LogErr(ERROR_LEVEL, ER_RPL_REPLICA_ERROR_OPEN_FILE_FAILED, "a cache on ",
           report_file.c_str(), my_errno());
  ...
  time_t ev_tm = get_time();
  char tm_buf[256] = {0};
  get_date(tm_buf, GETDATE_SHORT_DATE | GETDATE_DATE_TIME, ev_tm);

  const char *ev_type = get_type_str();
  const char *ev_log_name =
      const_cast<Relay_log_info *>(rli)->get_rpl_log_name();
  ulong ev_pos = (ulong)common_header->log_pos;

  const char *format_str =
      "# Event data for error diagnostics, Current Time: %s"
      "# Event table: %s\n"
      "# Event timestamp: %s\n"
      "# Event type: %s\n"
      "# Event Source log: %s\n"
      "# Event end_log_pos: %lu\n";
  my_b_printf(&io_cache, format_str, ctime(&now), table->s->table_name.str,
              tm_buf, ev_type, ev_log_name, ev_pos);
  flush_io_cache(&io_cache);

  // 3. write main content
  error = print_hash_info_to_cache(&io_cache, rli, table);
...
int Rows_log_event::do_apply_event(Relay_log_info const *rli) {
  ...
  */
  if (!thd->lock) {
    ...
    */
    ...
    if (rli->m_full_table_ids.count(m_table_id.id()) == 0) {
      const char *buf = "No mapping table_map_event for rows_event";
      if (thd->slave_thread)
        rli->report(ERROR_LEVEL, ER_REPLICA_FATAL_ERROR,
  ...
  */
  ...
  if (error) {
    bool row_data_written = !report_file.empty();
    slave_rows_error_report(
        ERROR_LEVEL, error, rli, thd, table, get_type_str(),
        const_cast<Relay_log_info *>(rli)->get_rpl_log_name(),
        (ulong)common_header->log_pos,
        row_data_written ? report_file.c_str() : nullptr);
    /*
...
int Table_map_log_event::do_apply_event(Relay_log_info const *rli) {
  ...
  const_cast<Relay_log_info *>(rli)->m_full_table_ids.insert(m_table_id.id());
...
int Write_rows_log_event::write_row(const Relay_log_info *const rli,
  ...
  DBUG_EXECUTE_IF("replica_insert_io_full", {
    const char act[] =
        "now signal reach_insert "
        "wait_for continue_insert";
    assert(!debug_sync_set_action(current_thd, STRING_WITH_LEN(act)));
  });
...
/* Fill Ignorable_log_event body with zero */
bool Ignorable_log_event::write_zero(Basic_ostream *ostream,
                                     size_t event_data_len) {
  DBUG_TRACE;
  char pad_buf[IO_SIZE];
  bzero(pad_buf, IO_SIZE);
  while (event_data_len) {
    uint64 size = event_data_len >= IO_SIZE ? IO_SIZE : event_data_len;
    if (ostream->write((uchar *)pad_buf, size)) return true;
    event_data_len -= size;
  }
  return false;
}
...
Transaction_context_log_event::Transaction_context_log_event(
    ...
    bool is_gtid_specified_arg, bool is_empty_transaction_arg)
    : mysql::binlog::event::Transaction_context_event(
          thread_id_arg, is_gtid_specified_arg, is_empty_transaction_arg),
      Log_event(header(), footer(),
                using_trans ? Log_event::EVENT_TRANSACTIONAL_CACHE
                            : Log_event::EVENT_STMT_CACHE,
                Log_event::EVENT_NORMAL_LOGGING) {
...
bool Transaction_context_log_event::write_data_header(Basic_ostream *ostream) {
  ...
  uint8_t gtid_flag = 0;
  if (gtid_specified) gtid_flag |= 0x01;
  if (gtid_empty_transaction) gtid_flag |= 0x02;
  buf[ENCODED_GTID_SPECIFIED_OFFSET] = gtid_flag;
...
size_t Transaction_payload_log_event::binlog_cache_write_len() {
  auto codec =
  ...
  if (result.second) return (size_t)-1;
  return LOG_EVENT_HEADER_LEN + result.first + m_payload_size;
}
...
PRINT_EVENT_INFO::PRINT_EVENT_INFO()
    : flags2_inited(false),
      ...
      only_unflushed_rows_query_log_event(false),
  ...
  */
  row_events_base64_max_size = 0;
  row_events_base64_size_calc = 0;
  need_print_end = false;
  last_table_map_size = 0;
  clear_last_table_map_base64();
  split_events_when_print = 0;
