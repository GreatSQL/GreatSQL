...
class Sql_cmd_load_table final : public Sql_cmd {
 public:
  Sql_cmd_load_table(
      ...
      : m_exchange(filename.str, false, filetype),
        ...
        m_concurrency(concurrency),  // MERGE_PS8404_CHECK_TAG: two parallel?
  ...
  */
  ...
  bool m_parallel_load;      // ture if current thd is master session
  uint m_master_session_id;  // master session id, used for find worker_handler
  uint m_worker_id;          // serial number of worker, for find worker_handler
