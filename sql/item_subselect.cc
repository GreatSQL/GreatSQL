...
bool Item_singlerow_subselect::fix_fields(THD *thd, Item **ref) {
  ...
  if (!query_expr()->is_set_operation() && !inner->has_tables() &&
      ...
      !is_maxmin() && !inner->connect_by_cond()) {
  ...
  */
  ...
  if (join->connect_by_cond && join->after_connect_by_cond) {
    join->after_connect_by_cond =
        remove_in2exists_conds(join->after_connect_by_cond);
  }
...
bool Item_subselect::fix_fields(THD *thd, Item **) {
  ...
  accumulate_properties();
    ...
    /* MERGE_PS8404_CHECK_TAG: @linlj, substitution removed from item subselect
    if (substitution) {
      .....
      if (!ref) {
        assert(0);
...
enum Item_result Item_singlerow_subselect::result_type() const {
  if (cols() > 1 || is_ora_type() || is_ora_table()) return ROW_RESULT;
...
bool Item_singlerow_subselect::resolve_type(THD *thd) {
  ...
  if (m_max_columns == 1) {
    if (m_value->is_ora_type())
      set_ora_type();
    else if (m_value->is_ora_table())
      set_ora_table();
  } else
    m_value = *m_row;
  ...
  */
  ...
  /*
    ...
    semi-join (which is done in flatten_subqueries()). The requirements are:
      ...
      16. Connect by not allow semijoin
      17. rownum not allow semijoin
      18. vpd not allow semijoin
  */
  ...
  if (inner->semijoin_enabled(thd) &&                              // 1
      ...
      !(outer->has_connect_by || inner->has_connect_by) &&                 // 16
      !(outer->has_rownum || inner->has_rownum)) {                         // 17
    return true;
  }
  return false;
}
...
LEX_STRING Item_singlerow_subselect::get_udt_name() const {
  if (m_value == nullptr) return NULL_STR;
  return m_value->get_udt_name();
}
LEX_STRING Item_singlerow_subselect::get_udt_db_name() const {
  if (m_value == nullptr) return NULL_STR;
  return m_value->get_udt_db_name();
}

List<Create_field> *Item_singlerow_subselect::get_field_create_field_list() {
  if (m_value == nullptr) return nullptr;
  return m_value->get_field_create_field_list();
}
...
bool Query_expression::replace_items(Item_transformer t, uchar *arg) {
  ...
  for (Query_block *qb = first_query_block(); qb != nullptr;
    ...
    if (qb->connect_by_cond() != nullptr &&
        replace_and_update(qb->connect_by_cond(), qb->connect_by_cond_ref()))
      return true;
