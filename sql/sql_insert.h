...
class Query_result_insert_all : public Query_result_interceptor {
 public:
  ...
  void send_error(THD *thd, uint errcode, const char *err);
  bool send_eof(THD *thd) override;
  void abort_result_set(THD *thd) override;
  void cleanup() override;

 private:
  uint insert_table_count;
 ...
 protected:
  bool *bulk_insert_start;
  ulonglong autoinc_value_of_last_inserted_row;  // array autogenerated or not
};

/*
  like Sql_cmd_insert_base

  insert all
     into t1 (col) values( vals)
     into t1 (col) values( vals)
     into t1 (col) values( vals)
  select xxx  ;
        ||
  ...
  insert into t2(col)
  insert into t3(col)
  select t1.vals, t2.vals , t3.vals from (xxx) ""

  not support  insert all when/else  and insert first into

*/
class Sql_cmd_insert_all : public Sql_cmd_dml {
 protected:
  bool precheck(THD *thd) override;
  bool check_privileges(THD *thd) override;
  bool prepare_inner(THD *thd) override;
 ...
 public:
  ...
  enum_sql_command sql_command_code() const override {
    return SQLCOM_INSERT_ALL_SELECT;
  }
  void cleanup(THD *) override { insert_field_list.clear(); }
