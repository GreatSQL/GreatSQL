...
class Commit_stage_manager {
  ...
  */
  ...
  /**
    ...
    m_stage_cond_leader, m_stage_cond_large_trx_wait condition variables
    and m_lock_done mutex.
    ...
    The large transaction committing by rotation should serve as the leader
    of the binlog flush queue. Then it blocks on m_stage_cond_large_trx_wait
    till fetch_and_empty() signals it to awake and become leader of empty queue.
  ...
  */
  ...
  /**
     The large transaction committing by rotation should serve as the
     leader of the binlog flush queue. Then the large transaction waits
     on this condition variable while appending to the binlog flush queue
     and get signalled to wake up when the binlog flush queue is empty.
  */
  mysql_cond_t m_stage_cond_large_trx_wait;

  /**
     The list of large transactions waiting for binlog flush to be empty.
     Ensure thread safety of the list via m_queue_lock[BINLOG_FLUSH_STAGE].
  */
  std::list<THD *> large_trx_waiting_list;
