...
enum GtidCheckType { NOT_SUBSET, IS_SUBSET };
...
class MYSQL_BIN_LOG : public TC_LOG {
  ...
  */
  ...
  /**
    Given gtid_set, search for binlog files

    @param[out] binlog_file_name the file name of binary log found
    ...
    @param[out] errmsg the error message outputted, which is left untouched
                if the function returns false
    @param[in]  check_type the value is IS_SUBSET will be called from
                find_first_log_not_in_gtid_set.
                if NOT_SUBSET will be called from find_log_name_by_gtid_set
    @param[in]  is_relay_log whether or not relay log file
    @return false on success, true on error.
  */
  bool find_binlog_file_helper(char *binlog_file_name, const Gtid_set *gtid_set,
                               Gtid *first_gtid, std::string &errmsg,
                               GtidCheckType check_type, bool is_relay_log);
  ...
  */
  ...
  /**
    Find the binary log where the given gtid set.
    The gtid set only contains one gtid.

    @param[out] binlog_file_name the file name of binary log found,
                if more_info is true, contains position and transaction length
    @param[in] gtid_set the given gtid set
    @param[in] more_info whether search for more info

    @return false on success, true on error.
  */
  bool find_log_name_by_gtid_set(char *binlog_file_name, Gtid_set *gtid_set);

  /**
    Find the more information from binary log base on the given gtid set

    @param[out] pos  position of gtid
    @param[out] trx_length  transaction length of gtid
    @param filename  The binary log
    @param more_info  whether search for more info
    @param gset  The given gtid set

    @return false on success, true on error.
  */
  bool find_pos_from_binary_log(my_off_t &pos, ulonglong &trx_length,
  ...
  */
  bool open_binlog(const char *log_name, const char *new_name,
                   ...
                   Format_description_log_event *extra_description_event,
                   uint32 new_index_number = 0, bool commit_by_rotate = false);
  ...
  */
  ...
  bool write_gtid_log_event(THD *thd, binlog_cache_data *cache_data,
                            Binlog_event_writer *writer,
                            bool parallelization_barrier, bool commit_by_rotate,
...
class Binlog_commit_by_rotate {
  ...
  */
  bool should_commit_by_rotate(THD *thd) const;
  ...
  /**
    ...
    @retval true    Fails if error happens.
    @retval false   Succeeds to rename binlog cache and commit the transaction.
                    Or the cache cannot be renamed, back to the origin logic.
  */
  bool commit(THD *thd);
  ...
  /**
    ...
    @retval true    It only return true if some error happened
                    after the new binlog file is deleted.
    @retval false   Succeeds to replace the binlog file.
                    Or failed to replace the binlog file but
                    the new binlog file is not deleted.
  */
  bool replace_binlog_file(THD *thd);

  /**
    Set the flag indicating whether the current active binlog is
    rotated by a large transaction commit.

    @param commit_by_rotate  Committing large transaction by rotate.
  */
  void set_binlog_rotated(bool commit_by_rotate) {
    m_binlog_rotated = commit_by_rotate;
  }

  bool get_binlog_rotated() { return m_binlog_rotated; }
  ...
  */
  ...
  /**
    ...
    And reserving space is a transaction level action, so there always
    are some transactions reserving space with the old value.
  */
  uint32 get_reserved_size() { return m_reserved_bytes; }
  ...
  */
  ...
  /**
    If true, the current binlog was rotated by a large transaction commit.
    All events written to it must skip checksum.
  */
  bool m_binlog_rotated{false};

  uint32 m_reserved_bytes{IO_SIZE};

 public:
  /**
    It will be set to true if Gtid_log_event has been
    written in replace_binlog_file().
  */
  bool m_has_gtid_written{false};
};
