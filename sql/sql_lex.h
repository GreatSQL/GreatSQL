...
*/
enum class enum_sp_type : int {
  FUNCTION = 1,
  PROCEDURE,
  PACKAGE_SPEC,  // PACKAGE is defined in my_config.h. don't use PACKAGE here.
  PACKAGE_BODY,
  TYPE,
  TRIGGER,
  EVENT,
...
enum class enum_sp_create_mode { SP_CREATE_NEW, SP_CREATE_OR_REPLACE };

enum class enum_udt_table_of_type : int {
  TYPE_VARRAY = 0,
  TYPE_TABLE,
  TYPE_INVALID = 255
};
...
#define TYPE_ENUM_PACKAGE_SPEC 3
#define TYPE_ENUM_PACKAGE_BODY 4
#define TYPE_ENUM_TYPE 5
#define TYPE_ENUM_TRIGGER 6
#define TYPE_ENUM_PROXY 7

enum class Acl_type {
  TABLE = 0,
  FUNCTION = TYPE_ENUM_FUNCTION,
  PROCEDURE = TYPE_ENUM_PROCEDURE,
  PACKAGE_SPEC = TYPE_ENUM_PACKAGE_SPEC,
  PACKAGE_BODY = TYPE_ENUM_PACKAGE_BODY,
  TYPE = TYPE_ENUM_TYPE,
};
...
class Table_ident {
  ...
  */
  ...
  bool resolve_table_rowtype_ref(THD *thd, List<Create_field> &defs);
};
...
class Qualified_column_ident : public Table_ident {
 ...
 public:
  Qualified_column_ident(const LEX_CSTRING &column)
      : Table_ident(NULL_CSTR), m_column(column) {}
  Qualified_column_ident(const LEX_CSTRING &table_arg,
                         const LEX_CSTRING &column)
      : Table_ident(table_arg), m_column(column) {}
  Qualified_column_ident(const LEX_CSTRING &db_arg,
                         const LEX_CSTRING &table_arg,
                         const LEX_CSTRING &column)
...
class Query_expression {
  ...
  */
  ...
  /* offset fetch oracle typle limit clause */
  bool select_limit_ties, select_limit_percent;
  double select_limit_percent_value;
...
class Query_block : public Query_term {
  ...
  */
  ha_rows get_offset(const THD *thd, bool *isNull) const;
  /**
   Get limit.
  ...
  */
  ...
  /**
    ...
    @param      query_type   Options to print out string output
  */
  void print_insert_all(const THD *thd, String *str,
                        enum_query_type query_type);
  ...
  */
  ...
  /*
    try to push limit of this query block to materialized derived tables.
  */
  bool push_limit_to_derived_tables();
  ...
  */
  ...
  bool select_limit_percent{false};
  double select_limit_percent_value{0.0};
  bool select_limit_ties{false};
  bool select_limit_fetch{false};
  /**
...
class Lex_input_stream {
  ...
  */
  const char *get_cpp_tok_end_rtrim() {
    const char *p;
    for (p = m_cpp_tok_end;
         p > m_cpp_buf && my_isspace(system_charset_info, p[-1]); p--) {
    }
    return p;
  }
  ...
  */
  const char *get_cpp_ptr_rtrim() {
    const char *p;
    for (p = m_cpp_ptr; p > m_cpp_buf && my_isspace(system_charset_info, p[-1]);
         p--) {
    }
    return p;
  }
...
struct LEX : public Query_tables_list {
  ...
  */
  ...
  void set_is_materialized_view_log(bool mlog_param) { m_mlog = mlog_param; }
  bool is_materialized_view_log() const { return m_mlog; }
  ...
  */
  ...
  void clear_with_fuction_flag() { m_with_func = false; }
  /**
  ...
  */
  ...
  sp_pcontext *get_sp_current_parsing_ctx() const {
 ...
 public:
  ...
  sp_variable *find_variable(const char *name, size_t name_len,
                             sp_pcontext **ctx, const Sp_rcontext_handler **rh,
                             bool also_find_udt = true) const;
  sp_variable *find_variable(const char *name, size_t name_len,
                             const Sp_rcontext_handler **rh) const {
    sp_pcontext *not_used_ctx;
    return find_variable(name, name_len, &not_used_ctx, rh);
  }
  ...
  /**oracle sp start **/
  bool sp_block_begin_label(THD *thd, LEX_CSTRING lab_name = EMPTY_CSTR);
  bool sp_block_end_label(THD *thd, LEX_CSTRING lab_name = EMPTY_CSTR);

  /**
  The operation is called from the parser in order to
  set parsing context whenever an sp_block start.

  @param thd        Thread context.

  */
  bool sp_block_init(THD *thd);

  /**
  The operation is called from the parser at the end of 'sp_block's in order to
  1. backpatch instructions with label.
  2. popup parsing context to get out of current block.

  @param thd Thread context.
  @param spblock Struct to store information of declaration sections.
  @return false if successful, true if error
  */
  bool sp_block_finalize(THD *thd, const spblock_st spblock) {
    return sp_block_finalize(thd, spblock.hndlrs, spblock.curs);
  }

  bool sp_block_finalize(THD *thd, int hndlrs = 0, int curs = 0);

  bool sp_exception_declarations(THD *thd);

  bool sp_exception_section(THD *thd, int pos);

  bool sp_exception_finalize(THD *thd, int pos_begin, int exception_num);

  bool sp_variable_declarations_record_percent_type(THD *thd, LEX_STRING ident,
  ...
  bool sp_variable_declarations_column_type_finalize(
  ...
  bool sp_for_loop_bounds_set_cursor(THD *thd, const LEX_STRING name,
                                     ...
                                     LEX *lex_expr, Item **args, uint arg_count,
  ...
  bool sp_variable_declarations_add_udt_variable(
  ...
  bool sp_variable_declarations_record_finalize(THD *thd, int nvars,
                                                LEX_STRING record,
                                                Item *default_value,
                                                LEX_CSTRING dflt_value_query);

  bool sp_variable_declarations_type_record_set_finalize(
      THD *thd, int nvars, LEX_STRING record, Item *default_value,
      LEX_CSTRING dflt_value_query);

  bool sp_variable_declarations_udt_table_set_finalize(
      THD *thd, int nvars, List<Create_field> *field_def_list,
      LEX_CSTRING nested_table_udt, uint table_type, ulonglong varray_limit,
      LEX_STRING record, Item *default_value, LEX_CSTRING dflt_value_query);

  bool sp_variable_declarations_udt_set_finalize(
      THD *thd, int nvars, List<Create_field> *field_def_list,
      LEX_STRING record, Item *default_value, LEX_CSTRING dflt_value_query);

  Create_field *sp_variable_declarations_definition_in_record(
      THD *thd, LEX_STRING ident, LEX_STRING record, Item *default_value);

  bool sp_variable_declarations_type_record_table_set_finalize(
      THD *thd, int nvars, LEX_STRING record, Item *default_value);
  ...
  bool close_ref_cursor(THD *thd, uint cursor_offset, sp_pcontext *ctx_cursor,
                        const Sp_rcontext_handler *rh_cursor);
  ...
  or: insert into t1 values stu_record_val
  */
  bool insert_into_table_record(
      THD *thd, LEX_STRING name, Item *name_i, const POS &loc_start,
      Mem_root_array_YY<PT_table_reference *> *table_reference_list);
  bool check_forall_i_ident(THD *thd, PT_query_expression_body *insert_query);
  bool check_forloop_set_var(THD *thd, const LEX_CSTRING &opt_prefix,
                             const LEX_CSTRING &name);
  /**oracle sp end **/
  PT_set *dbmsotpt_set_serveroutput(THD *thd, bool enabled,
                                    MY_SQL_PARSER_LTYPE pos_set,
                                    MY_SQL_PARSER_LTYPE pos_val);
...
class sp_lex_local : public st_lex_local {
 public:
  sp_lex_local(THD *thd_arg, const LEX *oldlex) {
    /* Reset most stuff. */
    start(thd_arg);
    /* Keep the parent SP stuff */
    sphead = oldlex->sphead;
    set_sp_current_parsing_ctx(oldlex->get_sp_current_parsing_ctx());
...
extern int ora_sql_parser_lex(MY_SQL_PARSER_STYPE *, POS *, class THD *);
