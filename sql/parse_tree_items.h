...
class PTI_simple_ident_ident : public Parse_tree_item {
 ...
 public:
  ...
  bool itemize(Parse_context *pc, Item **res) override;
  LEX_CSTRING get_ident() { return ident; }
};
...
/**
  Parse tree Item wrapper for ident(i).var
*/
class PTI_simple_ident_q_for_loop : public Parse_tree_item {
  typedef Parse_tree_item super;

 protected:
  Symbol_location raw;
  sp_variable *m_sp_ident;
  sp_variable *m_sp_for;
  const char *field;

 public:
  PTI_simple_ident_q_for_loop(const POS &pos, sp_variable *sp_ident,
                              sp_variable *sp_for, const char *field_arg)
      : super(pos),
        raw(pos.raw),
        m_sp_ident(sp_ident),
        m_sp_for(sp_for),
...
/**
  Parse tree Item wrapper for n-dimentional simple_ident-s
*/
class PTI_simple_ident_q_nd : public Parse_tree_item {
  typedef Parse_tree_item super;

 protected:
  Symbol_location raw;
  List<ora_simple_ident_def> *list_arg;
  const char *field;

 public:
  PTI_simple_ident_q_nd(const POS &pos, List<ora_simple_ident_def> *arg,
                        const char *field_arg)
      : super(pos), raw(pos.raw), list_arg(arg), field(field_arg) {}
...
class PTI_text_literal : public Item_string {
 ...
 public:
  bool is_empty() { return (literal.length == 0); }
};
