...
static int lex_token(MY_SQL_PARSER_STYPE *yacc_yylval, POS *yylloc, THD *thd);
...
void LEX::reset() {
  ...
  set_is_materialized_view_log(false);
    ...
    for (Window &w : current_query_block()->m_windows) {
      auto check_udt_type = [](const PT_order_list *order_list) -> bool {
        for (ORDER *o = order_list->value.first; o; o = o->next) {
          if ((*o->item)->type() == Item::FIELD_ITEM) {
            Item_field *item_field = down_cast<Item_field *>(*o->item);
            if (!item_field->field) continue;
          }
          if ((*o->item)->result_type() == ROW_RESULT) {
            my_error(ER_TYPE_IN_ORDER_BY, MYF(0));
            return true;
          }
        }
...
int ora_sql_parser_lex(MY_SQL_PARSER_STYPE *yacc_yylval, POS *yylloc,
                       THD *thd) {
  return lex_token(yacc_yylval, yylloc, thd);
}
...
bool Query_expression::is_mergeable() const {
  ...
  return !select->is_grouped() && select->having_cond() == nullptr &&
         ...
         !select->has_limit() && !select->has_wfs() && !has_rownum &&
         !has_any_connect_by() && !has_lnnvl_func && !has_foj;
}
...
sp_variable *LEX::find_variable(const char *name, size_t name_len,
  ...
  // package body find from package body and head
  sp_package *pkg = sphead ? sphead->get_package() : nullptr;
  if (pkg) {
    if (pkg->m_type == enum_sp_type::PACKAGE_BODY &&
        (spv = pkg->find_package_variable(name, name_len))) {
      *ctx = pkg->get_root_parsing_context()->child_context(0);
      *rh = &sp_rcontext_handler_package_body;
      spv->sph = pkg;
      return spv;
    }
    // find from cache package head
    if ((spv = pkg->find_package_head_variable(LEX_CSTRING{name, name_len}))) {
      *ctx = pkg->m_package_head->get_root_parsing_context()->child_context(0);
      *rh = &sp_rcontext_handler_package_head;
      spv->sph = pkg->m_package_head;
      return spv;
    }
  }
  // package body sp find from package body and head
  sp_package *pkg_parent = sphead ? sphead->m_parent : NULL;
  if (pkg_parent) {
    if (pkg_parent->m_type == enum_sp_type::PACKAGE_BODY &&
        (spv = pkg_parent->find_package_variable(name, name_len))) {
      *ctx = pkg_parent->get_root_parsing_context()->child_context(0);
      *rh = &sp_rcontext_handler_package_body;
      spv->sph = pkg_parent;
      return spv;
    }
    // find from cache package head
    if (pkg_parent->m_package_head &&
        (spv = pkg_parent->find_package_head_variable(
             LEX_CSTRING{name, name_len}))) {
      *ctx =
          pkg_parent->m_package_head->get_root_parsing_context()->child_context(
              0);
      *rh = &sp_rcontext_handler_package_head;
      spv->sph = pkg_parent->m_package_head;
      return spv;
    }
  }
  *ctx = NULL;
...
bool LEX::sp_variable_declarations_record_percent_type(
    THD *thd, LEX_STRING ident, Qualified_column_ident *ref,
    Item *dflt_value_item, Create_field *cdf) {
  LEX *lex = thd->lex;
  if (ref->db.length == 0 && ref->table.length == 0) {
    // It's variable%type
    sp_variable *spv;
    sp_pcontext *ctx = nullptr;
    const Sp_rcontext_handler *rh = nullptr;
    if (!(spv = lex->find_variable(ref->m_column.str, ref->m_column.length,
                                   &ctx, &rh))) {
      my_error(ER_SP_UNDECLARED_VAR, MYF(0), ref->m_column.str);
      return true;
    }
    if (spv->field_def.ora_record.is_type_define() ||
        spv->field_def.ora_record.is_ref_cursor) {
      my_error(ER_NOT_SUPPORTED_YET, MYF(0),
               "the use likes ref_cursor%TYPE or type%TYPE");
      return true;
    }
    *cdf = *spv->field_def.clone(thd->mem_root);
    if (!dflt_value_item) {
      sp_pcontext *orig_type_spvar_pctx = nullptr;
      const Sp_rcontext_handler *orig_type_spvar_rh = nullptr;
      sp_variable *orig_type_spvar = lex->find_variable(
          spv->field_def.udt_name.str, spv->field_def.udt_name.length,
          &orig_type_spvar_pctx, &orig_type_spvar_rh, false);
      if (!orig_type_spvar) {
        dflt_value_item = new (thd->mem_root) Item_null();
      } else {
        dflt_value_item = new (thd->mem_root)
            Item_splocal(orig_type_spvar_rh, orig_type_spvar->name,
                         orig_type_spvar->offset, orig_type_spvar->type);
        if (dflt_value_item)
          static_cast<Item_splocal *>(dflt_value_item)->m_sp =
              orig_type_spvar->sph;
      }
    }
  } else {  // It's db.table.column%type
    cdf->ora_record.set_column_type_ref(ref);
  }
  if (!dflt_value_item) {
    dflt_value_item = new (thd->mem_root) Item_null();

    if (!dflt_value_item) return true;
  }
  cdf->field_name = ident.str;
...
bool LEX::sp_variable_declarations_vartype_finalize(THD *thd, int nvars,
  ...
  sp_pcontext *ctx = nullptr;
  const Sp_rcontext_handler *rh = nullptr;
  if (!(spv = lex->find_variable(ref.str, ref.length, &ctx, &rh))) {
  ...
  if (spv->field_def.ora_record.is_cursor_rowtype_ref()) {
    int offset = spv->field_def.ora_record.cursor_rowtype_offset();
    return sp_variable_declarations_cursor_rowtype_finalize(
        thd, nvars, offset, nullptr, nullptr,
        spv->field_def.ora_record.m_cursor_rowtype_info.m_ctx_cursor,
        spv->field_def.ora_record.m_cursor_rowtype_info.m_rh_cursor);
  }
  ...
  return sp_variable_declarations_copy_type_finalize(thd, nvars, spv, ref,
                                                     spv->sph);
}
...
bool LEX::sp_variable_declarations_copy_type_finalize(THD *thd, int nvars,
  ...
  for (int i = 0; i < nvars; i++) {
    ...
    spvar->field_def = spvar_ident->field_def;
    spvar->default_value = spvar_ident->default_value;
    spvar->field_def.field_name = spvar->name.str;
    spvar->field_def.is_nullable = true;
    if (spvar_ident->field_def.ora_record.row_field_definitions() ||
        spvar_ident->field_def.ora_record.row_field_table_definitions()) {
      sp_instr_setup_row_field *is =
          new (thd->mem_root) sp_instr_setup_row_field(
              sp->instructions(), pctx, spvar, &sp_rcontext_handler_local);

      if (!is || sp->add_instr(thd, is)) return true;
    }

    Item *default_tmp = nullptr;
    sp_pcontext *orig_type_spvar_pctx = nullptr;
    const Sp_rcontext_handler *orig_type_spvar_rh = nullptr;
    sp_variable *orig_type_spvar = lex->find_variable(
        spvar->field_def.udt_name.str, spvar->field_def.udt_name.length,
        &orig_type_spvar_pctx, &orig_type_spvar_rh);
    if (!orig_type_spvar) {
      default_tmp = new (thd->mem_root) Item_null();
    } else {
      default_tmp = new (thd->mem_root)
          Item_splocal(orig_type_spvar_rh, orig_type_spvar->name,
                       orig_type_spvar->offset, orig_type_spvar->type);
      if (default_tmp) static_cast<Item_splocal *>(default_tmp)->m_sp = sph;
    }
...
bool LEX::sp_variable_declarations_rowtype_finalize(
  ...
  sp_pcontext *ctx_cursor = nullptr;
  const Sp_rcontext_handler *rh_cursor = nullptr;
  int rc = check_percent_rowtype(thd, ref, &table_ref, &rdl, &offset,
                                 &ctx_cursor, &rh_cursor);
  if (rc == -1) return true;
  /*
  ...
  */
  return sp_variable_declarations_cursor_rowtype_finalize(
      thd, nvars, offset, table_ref, rdl, ctx_cursor, rh_cursor);
}

bool LEX::sp_variable_declarations_cursor_rowtype_finalize(
    THD *thd, int nvars, int offset, Table_ident *table_ref,
    Row_definition_list *rdl, sp_pcontext *ctx_cursor,
    const Sp_rcontext_handler *rh_cursor) {
  LEX *lex = thd->lex;
  sp_head *sp = lex->sphead;
  sp_pcontext *pctx = lex->get_sp_current_parsing_ctx();
  ...
  for (int i = 0; i < nvars; i++) {
    ...
    if (offset != -1) {  // it has cursor_rowtype_ref().
      spvar->field_def.ora_record.set_cursor_rowtype_ref(offset, ctx_cursor,
                                                         rh_cursor);
      sp_instr_cursor_copy_struct *copy_struct =
          new (thd->mem_root) sp_instr_cursor_copy_struct(
              sp->instructions(), pctx, offset, spvar->offset, false,
              &sp_rcontext_handler_local, ctx_cursor, rh_cursor);
...
bool LEX::sp_variable_declarations_table_rowtype_finalize(
  ...
  for (int i = 0; i < nvars; i++) {
    ...
    if (sp_variable_declarations_table_rowtype_make_field_def(thd, spvar, db,
      ...
      return true;
    spvar->field_def.ora_record.set_row_field_definitions(rdl);
    sp_instr_setup_row_field *is_row =
        new (thd->mem_root) sp_instr_setup_row_field(
            sp->instructions(), pctx, spvar, &sp_rcontext_handler_local);
...
bool LEX::sp_for_loop_bounds_set_cursor(THD *thd, const LEX_STRING name,
  ...
  sp_variable *spvar_cursor =
      lex->find_variable(name.str, name.length, &ctx, &rh);
  ...
  if (!pctx->find_cursor(name, offset, false, lex, ctx_cursor, rh_cursor)) {
    my_error(ER_SP_CURSOR_MISMATCH, MYF(0), name.str);
    return true;
  }
  if (!(pcursor = (*ctx_cursor)->find_cursor_parameters(*offset)) ||
...
bool LEX::sp_open_cursor(THD *thd, const LEX_STRING name,
  ...
  if (sp_for_loop_bounds_set_cursor(thd, name, parameters, &offset, lex,
                                    nullptr, 0, &ctx_cursor, &rh_cursor))
    return true;

  sp_instr_copen *i = new (thd->mem_root) sp_instr_copen(
      sp->instructions(), ctx_cursor, offset, &sp_rcontext_handler_local);
...
bool LEX::sp_open_cursor_for(THD *thd, LEX_STRING sql_ident,
  ...
  sp_pcontext *ctx = nullptr;
  const Sp_rcontext_handler *rh = nullptr;
  sp_variable *spvar_cursor = cursor_lex->find_variable(
      cursor_ident.str, cursor_ident.length, &ctx, &rh);
  if (!spvar_cursor || !spvar_cursor->field_def.ora_record.is_ref_cursor) {
  ...
  if (!pctx->find_cursor(cursor_ident, &offp, false, cursor_lex, &ctx, &rh)) {
    my_error(ER_SP_CURSOR_MISMATCH, MYF(0), cursor_ident.str);
    return true;
  }
  ...
  if (!sql_ident.str) { /*open c for sql*/
    ...
    sp_instr_copen_for_sql *i_open = new (thd->mem_root)
        sp_instr_copen_for_sql(sp->instructions(), ctx, cursor_lex,
                               cursor_query, spvar_cursor->offset, rh);

    if (!i_open || sp->add_instr(thd, i_open)) return true;

  } else { /*open c for ident*/
    LEX *lex = thd->lex;
    sp_pcontext *ctx_ident = nullptr;
    const Sp_rcontext_handler *rh_ident = nullptr;
    sp_variable *spvar_sql = lex->find_variable(sql_ident.str, sql_ident.length,
                                                &ctx_ident, &rh_ident);
    if (!spvar_sql) {
    ...
    sp_instr_copen_for_ident *i_open = new (thd->mem_root)
        sp_instr_copen_for_ident(sp->instructions(), ctx, lex,
                                 spvar_cursor->offset, spvar_sql->offset, rh,
                                 ctx_ident, rh_ident);
  ...
  */
  if (pctx->find_variable(ident.str, ident.length, false) ||
      (sp->m_type == enum_sp_type::PACKAGE_BODY &&
       lex->find_variable(ident.str, ident.length, &ctx, &rh))) {
  ...
  sp_instr_setup_row_field *is_set =
      new (thd->mem_root) sp_instr_setup_row_field(
          sp->instructions(), pctx, spvar, &sp_rcontext_handler_local);
...
bool LEX::sp_variable_declarations_add_udt_variable(
    THD *thd, LEX_STRING record, LEX_STRING udt_db_name, ulong *reclength,
  ...
  if (!sp_find_ora_type_create_fields(
          thd, udt_db_name, record, false, field_def_list, reclength, &sql_mode,
          nested_table_udt, table_type, varray_limit)) {
...
bool LEX::sp_variable_declarations_record_finalize(
  ...
  if (spvar_udt) {
    ...
    if (sp_variable_declarations_add_udt_variable(
            thd, record, lex->sphead->m_db, &reclength, &rdl_result, &rdl_table,
  ...
  sp_variable *spvar_record =
      lex->find_variable(record.str, record.length, &ctx, &rh);
  if (!spvar_record) {
    my_error(ER_SP_UNDECLARED_VAR, MYF(0), record.str);
    return true;
  }
  ...
  */
  for (uint i = 0; i < (uint)nvars; i++) {
    ...
    if (spvar_record->field_def.ora_record.is_index_by && !dflt_value &&
      ...
      dflt_value = new (thd->mem_root) Item_splocal(
          rh, spvar_record->name, spvar_record->offset, spvar_record->type);
      Item_splocal *splocal = dynamic_cast<Item_splocal *>(dflt_value);
      if (splocal) splocal->m_sp = spvar_record->sph;
    }
...
Create_field *LEX::sp_variable_declarations_definition_in_record(
  ...
  */
  ...
  if (spvar_udt) {
    ...
    ulonglong varray_limit = 0;
    if (sp_variable_declarations_add_udt_variable(
            thd, record, thd->lex->sphead->m_db, &reclength, &rdl_result,
            &rdl_table, &nested_table_udt, &table_type, &varray_limit,
            &field_def_list))
      return nullptr;
    if (rdl_result || rdl_table) {
      if (rdl_result) {
  ...
  const Sp_rcontext_handler *rh = nullptr;
  sp_variable *spvar =
      thd->lex->find_variable(record.str, record.length, &ctx, &rh);
  if (!spvar) {
    my_error(ER_SP_MISMATCH_RECORD_VAR, MYF(0), record.str);
    return nullptr;
  }
  ...
  if (!default_value) {
    default_value = new (thd->mem_root)
        Item_splocal(rh, spvar->name, spvar->offset, spvar->type);
    Item_splocal *item_sp = dynamic_cast<Item_splocal *>(default_value);
    if (item_sp) item_sp->m_sp = spvar->sph;
  }
  cdf->ora_record.record_default_value = default_value;
  return cdf;
}
...
bool LEX::sp_variable_declarations_type_record_set_finalize(
  ...
  for (uint i = 0; i < (uint)nvars; i++) {
    ...
    if (default_tmp) {
      ...
      default_tmp = new (thd->mem_root) Item_splocal(
          rh, spvar_record->name, spvar_record->offset, spvar_record->type);
...
bool LEX::sp_variable_declarations_type_table_finalize(THD *thd,
  ...
  */
  ...
  sp_instr_setup_row_field *is = new (thd->mem_root) sp_instr_setup_row_field(
      sp->instructions(), pctx, spvar, &sp_rcontext_handler_local);
...
bool LEX::sp_variable_declarations_type_table_type_finalize(THD *thd,
  ...
  */
  ...
  spvar->field_def.ora_record.is_single_type_define = true;
  spvar->field_def.ora_record.is_index_by = is_index_by;
  spvar->field_def.udt_name = spvar->name;
  spvar->field_def.udt_db_name = lex->sphead->m_db;
  if (length) {
...
bool LEX::sp_variable_declarations_type_refcursor_finalize(
  ...
  sp_pcontext *ctx;
  /*
    type mycur is ref cursor [return xx];
  */
  ...
  if (set_static_cursor_with_return_type(thd, return_type, offp, ident))
    return true;
  if (pcursor->get_cursor_rowtype().m_cursor_rowtype_offset != -1) {
    sp_instr_cursor_copy_struct *copy_struct =
        new (thd->mem_root) sp_instr_cursor_copy_struct(
            sp->instructions(), pctx,
            pcursor->get_cursor_rowtype().m_cursor_rowtype_offset,
            spvar->offset, false, &sp_rcontext_handler_local,
            pcursor->get_cursor_rowtype().m_ctx_cursor,
            pcursor->get_cursor_rowtype().m_rh_cursor);
    if (!copy_struct || sp->add_instr(thd, copy_struct)) return true;
  }
  spvar->field_def.ora_record.set_row_field_definitions(
...
bool LEX::sp_variable_declarations_define_cursor_return_type(
    ...
    Row_definition_list **rdl, int *offset, sp_pcontext **ctx_cursor,
    const Sp_rcontext_handler **rh_cursor) {
  ...
  *offset = -1;
  ...
  if (return_type->ident.length) {
    // get record define.
    sp_variable *spv_type = lex->find_variable(
        return_type->ident.str, return_type->ident.length, &ctx, &rh);
    if (!spv_type) {
    ...
    int rc = check_percent_rowtype(thd, return_type->ref, table_ref, rdl,
                                   offset, ctx_cursor, rh_cursor);
    if (rc == -1) return true;
  } else if (return_type->type ==
...
int LEX::check_percent_rowtype(THD *thd, Qualified_column_ident *ref,
  ...
  bool cursor_rowtype =
      ref->table.str && ref->db.str
          ? false
          : pctx->find_cursor(to_lex_string(ref->m_column), (uint *)offset,
                              false, lex, ctx, rh);
  sp_pcursor *pcursor = nullptr;
  // cursor return cursor%rowtype
  if (cursor_rowtype) {
    pcursor = (*ctx)->find_cursor_parameters(*offset);
    if (!pcursor) {
      my_error(ER_SP_CURSOR_MISMATCH, MYF(0), ref->m_column.str);
      return -1;
    }
    const Sp_rcontext_handler *rh_var;
    sp_pcontext *ctx_var;
    sp_variable *spv_cursor = lex->find_variable(
        ref->m_column.str, ref->m_column.length, &ctx_var, &rh_var);
...
bool LEX::check_cursor_type(THD *thd, Qualified_column_ident *ref,
  ...
  sp_variable *record_type =
      ref->table.str && ref->db.str
          ? nullptr
          : lex->find_variable(ref->m_column.str, ref->m_column.length, &ctx,
                               &rh);
...
bool LEX::set_static_cursor_with_return_type(THD *thd,
  ...
  if (sp_variable_declarations_define_cursor_return_type(
          thd, return_type, &table_ref, &rdl, &offset, &ctx_cursor, &rh_cursor))
    return true;
  ...
  pcursor->set_cursor_rowtype_offset(offset, ctx_cursor, rh_cursor);
  return false;
}
...
bool LEX::sp_variable_declarations_define_ref_cursor_finalize(
  ...
  if (sp->m_type == enum_sp_type::PACKAGE_BODY ||
      sp->m_type == enum_sp_type::PACKAGE_SPEC) {
    my_error(ER_NOT_SUPPORTED_YET, MYF(0),
             "REF_CURSOR used in package body or head");
    return true;
  }
  uint offp_record = 0;
  sp_pcontext *ctx_cursor = nullptr;
  const Sp_rcontext_handler *rh_cursor = nullptr;
  if (!pctx->find_cursor(spvar_record->name, &offp_record, false, lex,
                         &ctx_cursor, &rh_cursor)) {
    my_error(ER_SP_CURSOR_MISMATCH, MYF(0), spvar_record->name.str);
    return true;
  }
  sp_pcursor *pcursor_record = ctx_cursor->find_cursor_parameters(offp_record);
  if (!pcursor_record) {
    my_error(ER_SP_CURSOR_MISMATCH, MYF(0), spvar_record->name.str);
    return true;
  }
  ...
  for (uint i = 0; i < (uint)nvars; i++) {
    ...
    if (pcursor_record->has_return_type()) {
      sp_pcontext *ctx = nullptr;
      const Sp_rcontext_handler *rh = nullptr;
      if (!lex->find_variable(spvar->name.str, spvar->name.length, &ctx, &rh)) {
        my_error(ER_SP_UNDECLARED_VAR, MYF(0), spvar->name.str);
        return true;
      }
      sp_instr_cursor_copy_struct *copy_struct = new (thd->mem_root)
          sp_instr_cursor_copy_struct(sp->instructions(), pctx, offp_record,
                                      spvar->offset, false, rh, ctx_cursor,
                                      rh_cursor);
      if (!copy_struct || sp->add_instr(thd, copy_struct)) return true;
    }
...
bool LEX::sp_variable_declarations_type_table_rowtype_finalize(
  ...
  */
  if (pctx->find_variable(ident.str, ident.length, false) ||
      (sp->m_type == enum_sp_type::PACKAGE_BODY &&
       lex->find_variable(ident.str, ident.length, &ctx_cursor, &rh_cursor))) {
    my_error(ER_SP_DUP_PARAM, MYF(0), ident.str);
    return true;
  }
...
bool LEX::ora_sp_forall_loop_insert_value(THD *thd, LEX_CSTRING query,
  ...
  sp_head *sp = lex->sphead;
  sp_pcontext *pctx = lex->get_sp_current_parsing_ctx();
  lex_save->sp_lex_in_use = true;
  const Table_ref *tl = lex->query_tables;
  Table_function *tf = nullptr;
  for (; tl != nullptr; tl = tl->next_global) {
    if (tl->is_table_function()) tf = tl->table_function;
  }
  Table_function_record *record = down_cast<Table_function_record *>(tf);
  sp_pcontext *ctx = nullptr;
  const Sp_rcontext_handler *rh = nullptr;
  sp_variable *spv = lex->find_variable(record->m_spv->name.str,
                                        record->m_spv->name.length, &ctx, &rh);
  if (!spv) {
    my_error(ER_SP_UNDECLARED_VAR, MYF(0), record->m_spv->name.str);
    return true;
  }
  sp_instr_forall_fill_table *i =
      new (thd->mem_root) sp_instr_forall_fill_table(
          sp->instructions(), ctx, spv->offset, rh, record->m_forall_i);
  ...
  if (sp->restore_lex(thd)) return true;
  sp_instr_stmt *i_stmt =
      new (thd->mem_root) sp_instr_stmt(sp->instructions(), lex_save, query);

  if (!i_stmt || sp->add_instr(thd, i_stmt)) return true;
  return false;
}
  ...
  if (sp_for_loop_bounds->is_cursor) {
    ...
    *var_offset = spvar->offset;
    ...
    if (!pcursor->get_row_definition_list() && !pcursor->get_table_ref()) {
      spvar->field_def.ora_record.set_cursor_rowtype_ref(
          offset, sp_for_loop_bounds->ctx_cursor,
          sp_for_loop_bounds->rh_cursor);

      sp_instr_cursor_copy_struct *copy_struct =
          new (thd->mem_root) sp_instr_cursor_copy_struct(
              sp->instructions(), pctx, offset, spvar->offset, true,
              &sp_rcontext_handler_local, sp_for_loop_bounds->ctx_cursor,
              sp_for_loop_bounds->rh_cursor);
      if (!copy_struct || sp->add_instr(thd, copy_struct)) return true;
    } else {
      if (!spvar->field_def.ora_record.row_field_definitions())
    ...
    // OPEN Cursor;
    sp_instr_copen *iOpen = new (thd->mem_root)
        sp_instr_copen(sp->instructions(), sp_for_loop_bounds->ctx_cursor,
                       offset, &sp_rcontext_handler_local);

    if (!iOpen || sp->add_instr(thd, iOpen)) return true;

    // FETCH Cursor
    sp_instr_cfetch *i = new (thd->mem_root)
        sp_instr_cfetch(sp->instructions(), sp_for_loop_bounds->ctx_cursor,
                        offset, &sp_rcontext_handler_local);
    ...
    sp_pcontext *ctx = nullptr;
    const Sp_rcontext_handler *rh = nullptr;
    sp_variable *spv = lex->find_variable(ident.str, ident.length, &ctx, &rh);
    if (!spv) {
      my_error(ER_SP_UNDECLARED_VAR, MYF(0), ident.str);
      return true;
    }
    spv->field_def.ora_record.m_select_into_spv_info.m_ctx_cursor = ctx;
    spv->field_def.ora_record.m_select_into_spv_info.m_rh_cursor = rh;
...
bool LEX::close_ref_cursor(THD *thd, uint cursor_offset,
  ...
  LEX *lex = thd->lex;
  sp_head *sp = lex->sphead;
  const LEX_STRING *cursor_name = ctx_cursor->find_cursor(cursor_offset);
  sp_variable *spvar = cursor_name
                           ? ctx_cursor->find_variable(
                                 cursor_name->str, cursor_name->length, false)
                           : nullptr;
  sp_instr_cclose *iclose = new (thd->mem_root)
      sp_instr_cclose(sp->instructions(), ctx_cursor, cursor_offset, true,
                      &sp_rcontext_handler_local);
  if (!iclose || sp->add_instr(thd, iclose)) return true;
  if (spvar && spvar->field_def.ora_record.is_ref_cursor) {
    spvar->field_def.ora_record.m_select_into_spv_info.m_ctx_cursor =
        ctx_cursor;
    spvar->field_def.ora_record.m_select_into_spv_info.m_rh_cursor = rh_cursor;
...
bool LEX::sp_continue_loop(THD *thd, uint *offset, sp_label *lab,
  ...
  if (for_loop) {
    ...
    if (for_loop->is_cursor) {
      // cfetch once more
      if (!pctx->find_cursor(for_loop->cursor_name, offset, false, lex,
                             ctx_cursor, rh_cursor)) {
        my_error(ER_SP_CURSOR_MISMATCH, MYF(0), for_loop->cursor_name.str);
        return true;
      }
      sp_instr_cfetch *i_fetch = new (thd->mem_root) sp_instr_cfetch(
          sp->instructions(), pctx, *offset, &sp_rcontext_handler_local);
      if (!i_fetch || sp->add_instr(thd, i_fetch)) return true;

      spv->field_def.ora_record.m_select_into_spv_info.m_ctx_cursor = pctx;
      spv->field_def.ora_record.m_select_into_spv_info.m_rh_cursor =
          &sp_rcontext_handler_local;
  ...
  for (sp_pcontext *pctx_tmp = pctx;
    ...
    if (lab_tmp->m_for_loop && lab_tmp->m_for_loop->is_cursor &&
      ...
      if (!pctx->find_cursor(lab_tmp->m_for_loop->cursor_name, &cursor_offset,
                             false, lex, &ctx_cursor, &rh_cursor)) {
        my_error(ER_SP_CURSOR_MISMATCH, MYF(0),
                 lab_tmp->m_for_loop->cursor_name.str);
        return true;
      }
      sp_instr_cclose *iclose = new (thd->mem_root)
          sp_instr_cclose(sp->instructions(), ctx_cursor, cursor_offset, false,
                          &sp_rcontext_handler_local);
      if (!iclose || sp->add_instr(thd, iclose)) return true;
    }
  }
...
bool LEX::sp_continue_statement(THD *thd, LEX_CSTRING name) {
  ...
  sp_pcontext *ctx_cursor = nullptr;
  const Sp_rcontext_handler *rh_cursor = nullptr;
  if (sp_continue_loop(thd, &offset, lab, &ctx_cursor, &rh_cursor) ||
      sp_change_context(thd, lab))
    return true;
...
bool LEX::sp_continue_when_statement(THD *thd, LEX_CSTRING name, Item *expr,
  ...
  const Sp_rcontext_handler *rh_cursor = nullptr;
  if (sp_continue_loop(thd, &offset, lab, &ctx_cursor, &rh_cursor) ||
      sp_change_context(thd, lab, false))
    return true;
...
bool LEX::insert_into_table_record(
    THD *thd, LEX_STRING name, Item *name_i, const POS &loc_start,
    Mem_root_array_YY<PT_table_reference *> *table_reference_list) {
  LEX *lex = thd->lex;
  sp_head *sp = lex->sphead;
  sp_pcontext *pctx = lex->get_sp_current_parsing_ctx();
  if (!sp ||
      (sp->m_type != enum_sp_type::PROCEDURE &&
       sp->m_type != enum_sp_type::FUNCTION) ||
      !pctx) {
    thd->syntax_error_at(loc_start);
    return true;
  }

  sp_pcontext *ctx = nullptr;
  const Sp_rcontext_handler *rh = nullptr;
  sp_variable *spvar_table =
      lex->find_variable(name.str, name.length, &ctx, &rh);
  sp_variable *spvar_i = nullptr;
  if (name_i) {
    if (!spvar_table ||
        !spvar_table->field_def.ora_record.row_field_table_definitions() ||
        spvar_table->field_def.ora_record.is_record_table_define) {
      my_error(ER_SP_MISMATCH_RECORD_VAR, MYF(0), name.str);
      return true;
    }

    if (spvar_table->field_def.ora_record.is_single_type_define) {
      char buff[500] = {0};
      snprintf(
          buff, sizeof(buff),
          "single type define %s used in 'into list' without "
          "parentheses",
          std::string(loc_start.cpp.start, loc_start.cpp.length()).c_str());
    ...
    if (name_i->is_splocal() || name_i->type() == Item::FIELD_ITEM) {
      const char *name_tmp =
          name_i->is_splocal() ? down_cast<Item_splocal *>(name_i)->m_name.ptr()
                               : name_i->item_name.ptr();
      size_t length = name_i->is_splocal()
                          ? down_cast<Item_splocal *>(name_i)->m_name.length()
                          : name_i->item_name.length();
      spvar_i = lex->find_variable(name_tmp, length, &ctx, &rh);
      if (!spvar_i) {
        my_error(ER_SP_UNDECLARED_VAR, MYF(0), name_tmp);
        return true;
      }
    }
  } else {
    if (!spvar_table || !spvar_table->field_def.ora_record.is_row_type() ||
        spvar_table->field_def.ora_record.is_record_define ||
        spvar_table->field_def.ora_record.row_field_table_definitions()) {
      my_error(ER_SP_MISMATCH_RECORD_VAR, MYF(0), name.str);
      return true;
    }
  ...
  PT_table_record_function *record = new (thd->mem_root)
      PT_table_record_function(loc_start, name, name_i, spvar_i);
  if (table_reference_list->push_back(record)) return true;  // OOM
  return false;
}

bool LEX::check_forall_i_ident(THD *thd,
                               PT_query_expression_body *insert_query) {
  LEX *lex = thd->lex;
  sp_pcontext *pctx = lex->get_sp_current_parsing_ctx();
  PT_query_specification *query =
      down_cast<PT_query_specification *>(insert_query);
  if (!query) return true;
  Mem_root_array_YY<PT_table_reference *> from = query->get_from_clause();
  PT_table_record_function *table_ref =
      down_cast<PT_table_record_function *>(from[0]);
  if (!table_ref) return true;
  const Item *item_i = table_ref->get_forall_i();
  if (!item_i) {
    my_error(ER_WRONG_PARAMCOUNT_TO_TYPE_TABLE, MYF(0), "index parameter");
    return true;
  }
  Item *item_tmp = const_cast<Item *>(item_i);
  const char *name = item_i->is_splocal()
                         ? down_cast<Item_splocal *>(item_tmp)->m_name.ptr()
                         : item_tmp->item_name.ptr();
  size_t length = item_i->is_splocal()
                      ? down_cast<Item_splocal *>(item_tmp)->m_name.length()
                      : item_tmp->item_name.length();
  sp_variable *spvar = pctx->find_variable(name, length, true);
  if (!spvar || !spvar->field_def.ora_record.is_forall_loop_var) {
    my_error(ER_WRONG_PARAMCOUNT_TO_TYPE_TABLE, MYF(0), name);
    return true;
  }
  return false;
}
...
bool LEX::check_forloop_set_var(THD *thd, const LEX_CSTRING &opt_prefix,
                                const LEX_CSTRING &name) {
  LEX *lex = thd->lex;
  sp_pcontext *pctx = lex->get_sp_current_parsing_ctx();
  if (opt_prefix.str) return false;
  sp_variable *spvar = pctx->find_variable(name.str, name.length, false);
  if (spvar && spvar->field_def.ora_record.is_for_loop_var) {
    my_error(ER_NOT_SUPPORTED_YET, MYF(0),
             "set loop variable in for loop stmt");
    return true;
  }
  return false;
}
...
PT_set *LEX::dbmsotpt_set_serveroutput(THD *thd, bool enabled, POS pos_set,
                                       POS pos_val) {
  ...
  // MERGE_PS8404_CHECK_TAG: @fangnw, following PT_XX() add param "POS()" OK?

  PT_option_value_no_option_type *option_value =
    new (thd->mem_root) PT_set_system_variable(POS(),
          OPT_DEFAULT, pos_val, val_name.prefix, val_name.name, val_enabled);

  PT_start_option_value_list *value_list = new (thd->mem_root)
    PT_start_option_value_list_no_type(POS(),option_value, pos_val, nullptr);

  PT_set *ret = new (thd->mem_root) PT_set(POS(), pos_set, value_list);
