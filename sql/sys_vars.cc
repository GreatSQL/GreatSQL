...
static Sys_var_bool Sys_sched_affinity_enabled(
    "sched_affinity_enabled", "Sched affinity enabled or not",
    READ_ONLY NON_PERSIST GLOBAL_VAR(sched_affinity::sched_affinity_enabled),
    CMD_LINE(OPT_ARG), DEFAULT(true));
...
static const char *udt_format_result_names[] = {"BINARY", "DBA", nullptr};

static Sys_var_enum Sys_udt_format_result(
    "udt_format_result",
...
static Sys_var_flagset Sys_optimizer_switch(
    ...
    " derived_condition_pushdown, hash_set_operations, favor_range_scan,"
    " remove_useless_outerjoin, derived_limit_pushdown} and val is one of "
    "{on, off, default}",
...
static bool check_sql_mode(sys_var *s_var, THD *thd, set_var *var) {
  return check_candidate_mode(s_var, thd, var, true);
}

static bool check_shrink_sql_mode(sys_var *s_var, THD *thd, set_var *var) {
  return check_candidate_mode(s_var, thd, var, false);
}
...
static Sys_var_set Sys_shrink_sql_mode(
    "shrink_sql_mode",
    "Syntax: shrink_sql_mode=mode[,mode[,mode...]]."
    "shrink sql_mode when sql_mode is oracle, ansi, traditional",
    HINT_UPDATEABLE SESSION_VAR(shrink_sql_mode), CMD_LINE(REQUIRED_ARG),
    sql_mode_names, DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(check_shrink_sql_mode), ON_UPDATE(fix_shrink_sql_mode));
...
// start add for greatdb
static Sys_var_bool Sys_gdb_parallel_load(
    "gdb_parallel_load", "enable load data in parallel",
    HINT_UPDATEABLE SESSION_ONLY(gdb_parallel_load), NO_CMD_LINE,
    DEFAULT(false));
...
static Sys_var_uint Sys_replication_error_write_file(
    "replication_error_write_file",
    "When replication occurs error, the variable is the max number of rows "
    "that one event is written to file.",
    GLOBAL_VAR(replication_error_write_file), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, UINT_MAX), DEFAULT(500), BLOCK_SIZE(1), NO_MUTEX_GUARD,
    NOT_IN_BINLOG, ON_CHECK(nullptr), ON_UPDATE(nullptr));
...
static const char *replica_parallel_wait_mode_names[] = {
    "WAIT_ON_SQL_THREAD", "WAIT_ON_WORKER_THREAD", NullS};
static Sys_var_enum Sys_replica_parallel_wait_mode(
    "replica_parallel_wait_mode",
    "when enable parallel replay for replica, whether use more effective way",
    GLOBAL_VAR(opt_replica_parallel_wait_mode), CMD_LINE(OPT_ARG),
    replica_parallel_wait_mode_names, DEFAULT(REPLICA_WAIT_ON_SQL_THREAD));
...
static bool fix_errorlog(sys_var *, THD *, enum_var_type) {
  if (DBUG_EVALUATE_IF("test_errorlog_support_chinese", true, false) &&
      !strcmp("zh_CN", errorlog_lc_messages->name)) {
    LogErr(WARNING_LEVEL, ER_LOG_PRINTF_MSG,
           "debug模式,开始测试errorlog支持中文,清忽略以下输出");
    ...
    LogErr(WARNING_LEVEL, ER_DD_CANT_GET_OBJECT_KEY);
    LogErr(WARNING_LEVEL, ER_DD_CANT_CREATE_OBJECT_KEY);
    LogErr(WARNING_LEVEL, ER_LOG_PRINTF_MSG,
           "debug模式,结束测试errorlog支持中文,清忽略以上输出");
  }
  return false;
}

static Sys_var_struct<MY_LOCALE, Get_locale_name> Sys_errorlog_messages(
    "errorlog_messages_language",
    "Set the language used for the errorlog messages",
    GLOBAL_VAR(errorlog_lc_messages), NO_CMD_LINE,
    DEFAULT(&my_default_errorlog_lc_messages), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(check_locale), ON_UPDATE(fix_errorlog));
...
static Sys_var_bool Sys_enable_gm_ssl(
    "enable_gm_ssl", "whether enable gm ssl connect",
    READ_ONLY NON_PERSIST GLOBAL_VAR(enable_gm_ssl), CMD_LINE(OPT_ARG),
    DEFAULT(false), NO_MUTEX_GUARD, NOT_IN_BINLOG);
#endif
...
#define MIN_BINLOG_LARGE_COMMIT_THRESHOLD (10 * 1024 * 1024)
extern ulonglong opt_binlog_commit_by_rotate_threshold;
static bool check_binlog_large_commit_threshold(sys_var *, THD *thd,
                                                set_var *var) {
  ulonglong value = var->save_result.ulonglong_value;
  if (value == 0) {
    return false;
  }
  if (value < MIN_BINLOG_LARGE_COMMIT_THRESHOLD) {
    var->save_result.ulonglong_value = MIN_BINLOG_LARGE_COMMIT_THRESHOLD;
    push_warning(thd, Sql_condition::SL_WARNING, ER_WRONG_VALUE_FOR_VAR,
                 "The given value is too small and "
                 "has been adjusted to 10485760.");
  }
  return false;
}
static Sys_var_ulonglong Sys_binlog_large_commit_threshold(
    ...
    "binlogging",
    GLOBAL_VAR(opt_binlog_commit_by_rotate_threshold), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, ULLONG_MAX), DEFAULT(128 * 1024 * 1024), BLOCK_SIZE(1),
    NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(check_binlog_large_commit_threshold), ON_UPDATE(nullptr));
