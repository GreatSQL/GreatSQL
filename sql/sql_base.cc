...
static bool check_if_table_exists(THD *thd, Table_ref *table, bool *exists) {
  ...
  *exists = true;
  ...
  ret = has_sequence_def(thd, table->db, table->table_name);
...
static bool ora_create_and_open_temporary(THD *thd, Table_ref *tl) {
  ...
  */
  ...
  thd->is_internal_con_no_result = true;
  {
    ...
    case Sroutine_hash_entry::PROCEDURE: {
      ...
      */
      if (rt != prelocking_ctx->sroutines_list.first ||
          rt->type() == Sroutine_hash_entry::PACKAGE_BODY ||
        ...
        */
        ...
        // only take MDL for package body.
        // all other tasks are done when its routines is called.
        if (rt->type() == Sroutine_hash_entry::PACKAGE_BODY) break;
          ...
          if (!has_prelocking_list) {
            ...
            if (sp->m_parent) {
              ...
              *routine_modifies_data |= sp->m_parent->modifies_data();
              prelocking_strategy->handle_routine(
                  thd, prelocking_ctx, rt, sp->m_parent, need_prelocking);
            }
          }
        }
      } else {
...
static bool open_and_process_table(THD *thd, LEX *lex, Table_ref *const tables,
  ...
  // insert into t2 values dr_table(i); dr_table needs to be reprepare to get
  // new m_vt_list.
  if (tables->is_table_function() &&
      strcmp(tables->table_function->func_name(), "record_table") == 0) {
    error = ask_to_reprepare(thd);
    goto end;
  }
  /*
  ...
  */
  /* MERGE tables need to access parent and child Table_refs. */
...
bool open_tables_only_view_structure(THD *thd, Table_ref *table_list,
                                     bool can_deadlock) {
  /*
...
bool get_table_share_only_view_structure(THD *thd, Table_ref *table_list,
                                         List<Create_field> *cdf_list) {
  /*
    Let us set fake sql_command so views won't try to merge
    themselves into main statement. If we don't do this,
    SELECT * from information_schema.xxxx will cause problems.
  ...
  */
  auto sql_command =
      create_scope_guard([thd, save_sql_command = thd->lex->sql_command]() {
        thd->lex->sql_command = save_sql_command;
      });
  thd->lex->sql_command = SQLCOM_SHOW_FIELDS;
  bool rc = open_temporary_tables(thd, table_list);
  if (rc) return true;
  if (!is_temporary_table_being_opened(table_list))
    rc = open_tables_for_rowtype(thd, table_list, cdf_list);
  else {
    /*
      Sp is synchronization between nodes in great cluster,but tmp table is not.
      Forbid use tmp table to compat with great cluster.
      In future if need,use thd->lex->query_tables->table->export_structure(thd,
      cdf_list); to recover.
    */
    my_error(ER_NOT_SUPPORTED_YET, MYF(0),
             "tmp table used with %type or %rowtype");
    rc = true;
  }
  return rc;
}

bool open_tables_for_rowtype(THD *thd, Table_ref *tables,
                             List<Create_field> *cdf_list) {
  DBUG_TRACE;
  TABLE_SHARE share;
  ...
  DBUG_EXECUTE_IF("rowtype_with_null_mdl_ticket", { goto func_exit; });
  if (dd::acquire_shared_table_mdl(thd, tables->db, tables->table_name, false,
                                   &mdl_ticket))
    goto func_exit;
  if (thd->dd_client()->acquire(tables->db, tables->table_name, &dd_table) ||
      dd_table == nullptr) {
    if (thd->dd_client()->acquire(tables->db, tables->table_name, &view) ||
        !view) {
      my_error(ER_NO_SUCH_TABLE, MYF(0), tables->db, tables->table_name);
      goto func_exit;
    }
    my_error(ER_TYPE_CANT_OPEN_TABLE, MYF(0), tables->table_name);
    goto func_exit;
  }

  init_tmp_table_share(thd, &share, tables->db, strlen(tables->db),
                       tables->table_name, "" /* file name */, nullptr);

  error = open_table_def_suppress_invalid_meta_data(thd, &share, *dd_table);
  if (!error) share.export_structure(thd, cdf_list, true);
  free_table_share(&share);

func_exit:
  if (mdl_ticket) dd::release_mdl(thd, mdl_ticket);
  return error;
}
    ...
    if (tl->dblink_name) {
      ...
      */
              ...
              Transaction_ctx::STMT);
      {
        ...
        thd->is_internal_con_no_result = true;
        LEX_STRING str;
        lex_string_strmake(thd->mem_root, &str, create_dblink_trigger_table,
  ...
  for (; !field_it.end_of_fields(); field_it.next()) {
    if (!my_strcasecmp(system_charset_info, field_it.name(), name)) {
      ...
      */
      ...
      /**
       * select * from t1, (select * from t2) t where t1.ida = t.ida(+). when t2
       *is merged to parent_query, item <t.ida(+)> will be replace by t2.ida, so
       *must set
       * (+) info.
       **/
      if (*ref) {
        item->outer_joined = (*ref)->outer_joined;
      }
      *ref = item;
...
Field *find_field_in_tables(THD *thd, Item_ident *item, Table_ref *first_table,
  ...
  /*
   For merge-into statement, it is necessary to search for target table
   first in its query block.
   Query_block.merge_target is set in Sql_cmd_update::prepare_inner()
   for merge-into statement only.
   @@NOTE@@ to comply the oracle behavior, only column without table name or
   alias name should be searched in target table first.
   */
  if (first_table && first_table->query_block &&
      first_table->query_block->merge_target && !table_name) {
    Table_ref *cur_table = first_table->query_block->merge_target;
    ...
    if (cur_field) {
      item->cached_table =
          (!actual_table->cacheable_table) ? nullptr : actual_table;
      if (item->type() == Item::FIELD_ITEM)
        down_cast<Item_field *>(item)->field_index = field_index;
...
bool find_item_in_list(THD *thd, Item *find, mem_root_deque<Item *> *items,
  ...
  */
  ...
  for (auto it = VisibleFields(*items).begin();
    ...
    if (thd->get_query_plan_plugin_runtime() && item->origin_item)
      item = item->origin_item;
#endif
...
/*
  ...
  IMPLEMENTATION
    Notice that the table references in the list nested_join->m_tables
  ...
  */
  ...
  for (auto it = fields->begin(); it != fields->end(); ++it) {
    ...
    if ((!item->fixed && item->fix_fields(thd, item_pos)) ||
        (item = *item_pos)->check_cols(cols_expected)) {
      DBUG_PRINT("info",
...
/**
  ...
  order and store leaf Table_refs in that order in a vector.
  ...
  if (table->s->udt_name.str && thd->lex->sql_command != SQLCOM_UPDATE &&
    ...
    if ((fields.size() > 1 || (table->s->fields == 1 && fields.size() == 1)) &&
      ...
      if (table->has_gcol() && update_generated_write_fields(
                                   bitmap ? bitmap : table->write_set, table))
        return true;
      return thd->is_error();
    }
    if (!is_udt_insert && fields.size() != values.size()) {
      my_error(ER_WRONG_VALUE_COUNT_ON_ROW, MYF(0), 1);
      return true;
    }
  }
  ...
  for (Item *fld : VisibleFields(fields)) {
    ...
    if (item_row) {
      ...
      for (uint ii = 0; ii < item_row->cols(); ii++) {
        if (item_subselect && !item_subselect->is_value_assigned()) {
...
bool fill_record(THD *thd, TABLE *table, Field **ptr,
  ...
  */
  ...
  if (CountVisibleFields(values) == 1 &&
      values.at(0)->type() == Item::SUBQUERY_ITEM &&
      values.at(0)->cols() > 1) {
    ...
    null_value = !item_subselect->is_value_assigned();
    is_ora_update = true;
  }
  while ((field = *ptr++) && !thd->is_error()) {
