...
static bool switch_user(MYSQL_SESSION session, const char *user,
                        const char *host, const char *ip, const char *db) {
  DBUG_TRACE;
  MYSQL_SECURITY_CONTEXT sc;
  thd_get_security_context(srv_session_info_get_thd(session), &sc);
  return security_context_lookup(sc, user, host, ip, db) != 0;
}

bool Gdb_cmd_service::open_cur_session() {
  DBUG_TRACE;
  m_st_session = srv_session_open(nullptr, nullptr);
  if (m_st_session == nullptr) {
#ifndef NDEBUG
    LogErr(ERROR_LEVEL, ER_LOG_PRINTF_MSG, "srv_session_open failed.");
#endif
    return true;
  }
  THD *thd = thd_get_current_thd();
  Security_context *security_ctx = thd->security_context();
  if (nullptr == security_ctx ||
      switch_user(m_st_session, security_ctx->user().str,
                  security_ctx->host().str, security_ctx->ip().str,
                  thd->db().str)) {
#ifndef NDEBUG
    LogErr(ERROR_LEVEL, ER_LOG_PRINTF_MSG,
           "open_cur_session switch_user failed.");
#endif
    return true;
  }
  return false;
