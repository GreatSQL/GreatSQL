...
bool Sql_cmd_show_create_package::check_privileges(THD *) { return false; }

bool Sql_cmd_show_create_package::execute_inner(THD *thd) {
  return sp_show_create_routine(thd, enum_sp_type::PACKAGE_SPEC, lex->spname);
}

bool Sql_cmd_show_create_package_body::check_privileges(THD *) { return false; }
...
bool Sql_cmd_show_create_type::execute_inner(THD *thd) {
...
bool Sql_cmd_show_routine_code::execute_inner(THD *thd) {
#ifndef NDEBUG
  assert(m_sql_command == SQLCOM_SHOW_PROC_CODE ||
         m_sql_command == SQLCOM_SHOW_FUNC_CODE ||
         m_sql_command == SQLCOM_SHOW_PACKAGE_BODY_CODE);

  enum_sp_type sp_type;
  if (m_sql_command == SQLCOM_SHOW_FUNC_CODE)
    sp_type = enum_sp_type::FUNCTION;
  else if (m_sql_command == SQLCOM_SHOW_PROC_CODE)
  ...
  sp_head *sp = nullptr;
  sp_name name(m_routine_name->m_db, m_routine_name->m_name,
               m_routine_name->m_explicit_name);
  ...
  my_error(ER_FEATURE_DISABLED, MYF(0),
           !(thd->variables.sql_mode & MODE_ORACLE)
               ...
               : "SHOW PROCEDURE|FUNCTION|PACKAGE BODY CODE",
           "--with-debug");
  return true;
#endif  // ifndef NDEBUG
}
...
bool mysqld_show_create(THD *thd, Table_ref *table_list) {
  ...
  */
  ...
  if ((thd->lex->only_view && !table_list->is_view()) ||
      (table_list && table_list->table && table_list->table->s &&
       (table_list->table->s->is_mlog || table_list->table->s->mview_info))) {
...
bool Sql_cmd_show_create_materialized_view::check_privileges(THD *) {
  // Privilege check for this command is placed in execute_inner() function
  return false;
}

bool Sql_cmd_show_create_materialized_view::execute_inner(THD *thd) {
  LEX *old_lex = thd->lex;
  LEX local_lex;
  ...
  {
    Show_create_error_handler view_error_suppressor(thd, tbl);
    thd->push_internal_handler(&view_error_suppressor);

    const Prepared_stmt_arena_holder ps_arena_holder(thd);
    uint counter;
    bool open_error =
        open_tables(thd, &tbl, &counter, MYSQL_OPEN_FORCE_SHARED_HIGH_PRIO_MDL);

    thd->pop_internal_handler();
    if (open_error && (thd->killed || thd->is_error())) goto exit;

    if (!(tbl->table && tbl->table->s && tbl->table->s->mview_info)) {
      my_error(ER_WRONG_OBJECT, MYF(0), tbl->db, tbl->table_name,
               "MATERIALIZED VIEW");
      goto exit;
    }
  }
  ...
  field_list.push_back(new Item_empty_string(
      "Create Materialized View", max<size_t>(buffer.length(), 1024U)));
  ...
  if (tbl->schema_table)
    protocol->store(tbl->schema_table->table_name, system_charset_info);
  else
    protocol->store(tbl->table->alias, system_charset_info);

  buffer.length(0);
  if (greatdb::store_materialized_view_create_info(thd, tbl, &buffer, nullptr,
                                                   false, true)) {
    return true;
  }
...
bool append_identifier(const THD *thd, String *packet, const char *name,
  ...
  if (q == EOF) {
    return packet->append(to_name, to_length, packet->charset());
  }
  ...
  */
  ...
  (void)packet->reserve(to_length * 2 + 2);
  quote_char = (char)q;
  if (packet->append(&quote_char, 1, system_charset_info)) return true;
...
bool append_identifier(const THD *thd, String *packet, const char *name,
                       size_t length) {
  if (thd == nullptr)
    return append_identifier(packet, name, length);
  else
    return append_identifier(thd, packet, name, length, nullptr, nullptr);
}
...
void store_create_index_info(THD *thd, Table_ref *table_list, String *packet) {
  const char *alias;
  KEY *key_info;
  TABLE *table = table_list->table;
  TABLE_SHARE *share = table->s;
  ...
  for (uint i = 0; i < share->keys; i++, key_info++) {
    ...
    if ((i == share->primary_key &&
         !strcmp(key_info->name, primary_key_name)) ||
        (key_info->flags & HA_GENERATED_KEY)) {
      /* Skip primary and foreign keys*/
      continue;
    } else if (key_info->flags & HA_NOSAME)
      packet->append(STRING_WITH_LEN("CREATE UNIQUE INDEX "));
    else if (key_info->flags & HA_FULLTEXT)
      packet->append(STRING_WITH_LEN("CREATE FULLTEXT INDEX "));
    else if (key_info->flags & HA_SPATIAL)
      packet->append(STRING_WITH_LEN("CREATE SPATIAL INDEX "));
    else if (key_info->flags & HA_CLUSTERING)
      packet->append(STRING_WITH_LEN("CREATE CLUSTERING INDEX "));
    else
      packet->append(STRING_WITH_LEN("CREATE INDEX "));
    ...
    if (i < (share->keys - 1)) packet->append(STRING_WITH_LEN("\n  "));
  }
}
...
static bool append_at_host(const THD *thd, String *buffer,
                           const LEX_CSTRING &host) {
  if (!host.str) return false;
...
bool append_definer(const THD *thd, String *buffer,
                    const LEX_CSTRING &definer_user,
                    const LEX_CSTRING &definer_host) {
  return buffer->append(STRING_WITH_LEN("DEFINER=")) ||
         append_identifier(thd, buffer, definer_user.str,
                           definer_user.length) ||
         append_at_host(thd, buffer, definer_host) || buffer->append(' ');
}

void view_store_create_info(const THD *thd, Table_ref *table, String *buff) {
  const bool foreign_db_mode = (thd->variables.sql_mode & MODE_ANSI) != 0;
