  ...
  if (item_func && item_func->fixed &&
      (item_func->functype() == Item_func::UDT_TABLE_FUNC ||
       ...
       (item_func->functype() == Item_func::FUNC_SP &&
        item_func->is_ora_udt_type()))) {
    if (item_func->refix_fields()) return nullptr;
  }
  /*When it's Item_splocal_row_field_table_by_name, it doesn't have
    this_item_addr(),so return this_item().*/
...
sp_package *sp_start_package_parsing(THD *thd, enum_sp_type sp_type,
  ...
  // 5. set package head
  sp_head *sp_head = sp_cache_lookup(&thd->sp_package_spec_cache,
                                     pkg->m_qname.str, pkg->m_qname.length);
  pkg->m_package_head = sp_head ? sp_head->get_package() : NULL;
...
bool sp_resolve_package_routine_explicit(THD *thd, enum_sp_type type,
  ...
  */
  ...
  // for package function in view,it's name style like 'db.package.fn',but
  // name->m_explicit_name is true.
  int count = std::count(name->m_qname.str,
                         name->m_qname.str + name->m_qname.length, '.');
  if (count == 2) {
    char *qname = thd->strmake(name->m_name.str, name->m_name.length);
    name->m_qname = LEX_STRING{qname, name->m_name.length};
    std::string str(name->m_name.str, name->m_name.length);
    size_t start = 0, end = str.find('.');
    while (end != std::string::npos) {
      std::string word = str.substr(start, end - start);
      char *result = thd->strmake(word.c_str(), word.length());
      name->m_db = LEX_CSTRING{result, word.length()};
      start = end + 1;
      end = str.find('.', start);
    }
    std::string word = str.substr(start, end);
    char *result = thd->strmake(word.c_str(), word.length());
    name->m_name = LEX_STRING{result, word.length()};
  }
...
bool sp_validate_into_variable_hierarchy(THD *thd,
                                         Bipartite_name_ora *ora_ident) {
  assert(thd && ora_ident);

  enum Ident_type { VARIABLE, FIELD, TYPE };
  auto find_variable_and_check =
      [&thd](LEX_CSTRING name, Ident_type ident_type) -> sp_variable * {
    sp_variable *spv = nullptr;
    sp_pcontext *ctx = nullptr;
    const Sp_rcontext_handler *rh = nullptr;
    if (!(spv = thd->lex->find_variable(name.str, name.length, &ctx, &rh))) {
      if (ident_type != FIELD) {
      ...
      switch (ident_type) {
        case VARIABLE:
          error = spv->field_def.ora_record.is_type_define() ||
        ...
        case FIELD:
          ...
          break;
        case TYPE:
          error = spv->field_def.ora_record.row_field_table_definitions();
          break;
        default:
          assert(0);
          break;
  ...
  sp_variable *spv = nullptr;
  if (ora_ident->bipartite_name.prefix.length) {
    if (!(spv = find_variable_and_check(ora_ident->bipartite_name.prefix,
                                        VARIABLE))) {
      return true;
    }
  }
  ...
  if (ora_ident->medium) {
    for (auto it = ora_ident->medium->begin(); it != ora_ident->medium->end();
         ++it) {
      if (spv) {
        auto *spv_cfd = &spv->field_def;
        auto row_fields = spv_cfd->ora_record.row_field_definitions();
        if (!row_fields) {
          my_error(ER_SP_UNDECLARED_VAR, MYF(0), spv->name.str);
          return true;
        }
        uint offset = 0;
        auto *cfd = row_fields->find_row_field_by_name(it->ident, &offset);
        if (!cfd) {
          my_error(ER_ROW_VARIABLE_DOES_NOT_HAVE_FIELD, MYF(0), spv->name.str,
                   it->ident.str);
          return true;
        }
        if ((!(spv = find_variable_and_check(to_lex_cstring(cfd->field_name),
                                             FIELD)) &&
             thd->is_error()) ||
            (!(spv = find_variable_and_check(cfd->udt_name.str
                                                 ? to_lex_cstring(cfd->udt_name)
                                                 : cfd->nested_table_udt,
                                             TYPE)))) {
          return true;
        }
      } else {
        if (!(spv = find_variable_and_check(to_lex_cstring(it->ident),
                                            VARIABLE))) {
          return true;
        }
      }
  ...
  if (row_fields->size() > 0) {
    uint offset = 0;
    auto *cfd = row_fields->find_row_field_by_name(
        to_lex_string(ora_ident->bipartite_name.name), &offset);
    if (!cfd) {
      my_error(ER_ROW_VARIABLE_DOES_NOT_HAVE_FIELD, MYF(0), spv->name.str,
               ora_ident->bipartite_name.name.str);
      return true;
    }
    if (!(spv = find_variable_and_check(to_lex_cstring(cfd->field_name),
                                        FIELD)) &&
