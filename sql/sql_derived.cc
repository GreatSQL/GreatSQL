...
TABLE *Common_table_expr::clone_tmp_table(THD *thd, Table_ref *tl) {
  ...
  */
  ...
  if (first->set_counter()) {
    ...
    t->set_set_op((Field_longlong *)couter_field, first->is_except(),
                  first->is_distinct());
...

{
  ...
  if (is_inner_table_of_outer_join() || foj_inner || foj_outer)
    table->set_nullable();
...
bool Table_ref::can_push_condition_to_derived(THD *thd) {
  ...
  Query_block const *slave = unit->first_query_block();
...
/**
  check wheather we can push a limit to the derived table.

  @param  thd   THD pointer

  @return false if successful, true if error
*/
bool Table_ref::can_push_limit_to_derived() const {
  Query_expression const *unit = derived_query_expression();

  auto *slave = unit->query_term()->query_block();
  auto *master = unit->outer_query_block();

  bool has_rownum =
      (slave->has_rownum) || ((master != nullptr) && master->has_rownum);
  bool has_lnnvl_func =
      slave->has_lnnvl_func || ((master != nullptr) && master->has_lnnvl_func);

  /*
    1. master only contanin limit, can not exist where condition, agg, sort,
    group by...
  */
  auto master_ok = !master->is_grouped() &&                 // no group
                   master->having_cond() == nullptr &&      // no having
                   !master->is_distinct() &&                // no distinct
                   !master->has_windows() &&                // no windows
                   !has_rownum &&                           // no rownum
                   !has_lnnvl_func &&                       // no lnnvl_func
                   master->connect_by_cond() == nullptr &&  // no connect
                   master->where_cond() == nullptr &&       // no where
                   !master->is_ordered() &&                 // no order
                   (master->has_limit() ||
                    master->select_limit_fetch);  // only limit or fetch rows

  /*
    2. slave can not contain limit
    for setop, we check for "(select * from t1 limit 1) union all select *
     from t2" not children query block.
    agg:
      select * from ((select * from t1 limit 1) union select * from t2)
    limit 2; is allowed.
  */
  auto slave_ok = !(slave->has_limit() || slave->select_limit_fetch);
