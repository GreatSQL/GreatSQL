  ...
  */
  ...
  if (!drop_temporary) {
    ...
    */
    ...
    if (rm_table_lock_mlogs(thd, tables)) return true;
  }
...
static void append_table_ident_by_mlog(const THD *thd, String *to,
                                       const Table_ref *table, bool force_db) {
  //  Don't write the database name if it is the current one.
  if (thd->db().str == nullptr || strcmp(table->db, thd->db().str) != 0 ||
      force_db) {
  ...
  greatdb::append_table_name_by_mlog(thd, to, table);
}
/**
...
class Drop_tables_query_builder {
  ...
  Drop_tables_query_builder(THD *thd, bool temporary, bool if_exists,
    ...
    if (m_bin_log_is_open) {
      ...
      if (thd->lex->is_materialized_view()) {
        m_built_query.append("MATERIALIZED VIEW ");
      } else if (thd->lex->is_materialized_view_log()) {
        m_built_query.append("MATERIALIZED VIEW LOG ");
        is_mlog = true;
      } else {
 ...
 private:
  void add_table_impl(const Table_ref *table) {
    if (is_mlog) {
      append_table_ident_by_mlog(m_thd, &m_built_query, table, m_no_db);
    } else {
...
static bool drop_base_table(THD *thd, const Drop_tables_ctx &drop_ctx,
  ...
  */
  ...
  if (!drop_ctx.drop_database) {
    ...
    if (thd->lex->is_materialized_view_log() != table_def->is_mlog()) {
      String tbl_name(table->db, system_charset_info);
      tbl_name.append('.');
      tbl_name.append(String(table->table_name, system_charset_info));
      my_error(ER_BAD_TABLE_ERROR, MYF(0), tbl_name.c_ptr());
    ...
    // drop mlog when drop base table and base table has mlog

    if (table_def->has_mlog() && !thd->lex->is_materialized_view_log() &&
        !thd->lex->is_materialized_view()) {
      // drop mlog first

      auto mlog_ref = new (thd->mem_root) Table_ref(
          table_def->mlog_db_name().c_str(), table_def->mlog_db_name().length(),
          table_def->mlog_name().c_str(), table_def->mlog_name().length(),
          TL_UNLOCK, MDL_EXCLUSIVE);
      if (!mlog_ref) return true;

      thd->lex->set_is_materialized_view_log(true);
      // @see
      if (drop_base_table(thd, drop_ctx, mlog_ref, atomic, post_ddl_htons,
                          fk_invalidator, safe_to_release_mdl,
                          foreach_table_root)) {
        thd->lex->set_is_materialized_view_log(false);
        return true;
      }
      thd->lex->set_is_materialized_view_log(false);
    }
  ...
  */
  ...
  // drop mlog and drop trigger on base table
  std::unique_ptr<invalid_mlog_trigger_type> mlog_ptr(nullptr);
  invalid_imv_ref_mlog_type mlog_ref_imvs;
  // drop mview (faster mode) update mlog ref value
  greatdb::Mview_mlogs imv_ref_mlogs(thd);

  if (table_def->is_mlog()) {
    auto minfo = table_def->get_mlog_info();
    assert(minfo);
    if (minfo) {
      if (!drop_ctx.drop_database) {
        mlog_ptr.reset(new invalid_mlog_trigger_type(
            std::make_pair(std::make_pair(minfo->referenced_table_schema_name(),
                                          minfo->referenced_table_name()),
                           minfo->referenced_trigger_name())));
      }
    }

    // find all mv depends
    if (prepare_materialized_view_tables_list(
            thd, table->db, table->table_name,
            (drop_ctx.drop_database && atomic), &mlog_ref_imvs))
      return true;
  }
  ...
  if (!drop_ctx.drop_database) {
    ...
    if (table_def->is_materialized_view()) {
      auto mv_info = table_def->get_mv_info();
      if (mv_info->flush_mode() != 0) {
        if (greatdb::string_to_mlog_status(mv_info->table_ref(),
                                           greatdb::Mview_mlogs::add_mlog,
                                           &imv_ref_mlogs))
          return true;

        if (imv_ref_mlogs.invalidator_remove(thd)) return true;

        if (imv_ref_mlogs.add_tables_ref(thd)) return true;

        if (imv_ref_mlogs.lock_mlog_tables(thd, nullptr)) return true;
      }
    }
  }
  ...
  */
  ...
  if (!drop_ctx.drop_database) {
    if (mlog_ptr) {
      result |= invalid_mlog_trigger(thd, mlog_ptr.get());
    }

    if (imv_ref_mlogs.get_mlogs_size() != 0) {
      {
        const Disable_binlog_guard binlog_guard(thd);
        result |= imv_ref_mlogs.updateRefValue(thd, false);
      }
      imv_ref_mlogs.unlock_mlog_tables(thd);

      result |= imv_ref_mlogs.updateMetaDataRefValue(thd, false);
    }
  }
  if (result) return result;
  if (!mlog_ref_imvs.empty()) {
    result |= mark_referencing_materialized_views_invalid(thd, mlog_ref_imvs);
  }
...
TYPELIB *create_typelib_from_field(MEM_ROOT *mem_root, Field_enum *field) {
  ...
  result->count = field->typelib->count;
  ...
  */
  ...
  if (key->type == KEYTYPE_FULLTEXT) {
    ...
    */
    ...
    // udt column can't be used in index.
    if (sql_field->udt_name.str) {
      my_error(ER_UDF_TYPE_USED_AS_KEY, MYF(0), column->get_field_name());
      return true;
    }
...
/*
  Create a table identical to the specified udt type

  SYNOPSIS
    mysql_create_of_table()
    thd    Thread object
    table       Table list element for target table
    create_info Create info
...
bool mysql_create_of_table(THD *thd, Table_ref *table,
                           HA_CREATE_INFO *create_info) {
  Alter_info local_alter_info(thd->mem_root);
  bool is_trans = false;
  uint not_used;
  ...
  TABLE *m_type_table = nullptr;
  ulong length = dd::get_udt_size_and_supporting_table(
      thd, thd->mem_root, create_info->udt_db_name.str, create_info->udt_name,
      &m_type_table, table->table_name);
  if (!length) {
    my_error(ER_SP_DOES_NOT_EXIST, MYF(0), sp_type_str(enum_sp_type::TYPE),
             create_info->udt_name.str);
    return true;
  }

  DEBUG_SYNC(thd, "create_table_of_after_open");

  HA_CREATE_INFO local_create_info;
  create_info->db_type =
      get_viable_handlerton_for_create(thd, table->table_name, *create_info);
  if (create_info->db_type == nullptr) return true;

  local_create_info.row_type = m_type_table->s->row_type;

  ulong reclength;
  List<Create_field> *field_def_lst = new (thd->mem_root) List<Create_field>;
  bool ret = sp_find_ora_type_create_fields(
      thd, to_lex_string(create_info->udt_db_name),
      to_lex_string(create_info->udt_name), false, &field_def_lst, &reclength,
      nullptr);
  if (ret) {
    return true;
  }
  local_alter_info.create_list = *field_def_lst;

  local_create_info = *create_info;

  local_create_info.table_options |= HA_OPTION_PACK_RECORD;
  ...
  */
  ...
  DEBUG_SYNC(thd, "create_table_of_before_binlog");
  ...
  */
  ...
  if (!(create_info->options & HA_LEX_CREATE_TMP_TABLE)) {
    /*
      Increase the udt type's ref_count.
    */
    if (!(table->table || table->is_view()) &&
        !dd::get_dictionary()->is_dd_table_name(table->db, table->table_name) &&
        (local_create_info.db_type->flags & HTON_SUPPORTS_FOREIGN_KEYS)) {
  ...
  */
...

  {
    ...
    */
    ...
    // copy mv or mlog
    altered_table_def->copy_mv_info(table_def);
    ...
    // down the ref_count of udt columns used by this table.
    if (dd::alter_type_columns_ref_count(thd, table_list->db, table_def, false))
      goto cleanup2;
...
bool mysql_alter_table(THD *thd, const char *new_db, const char *new_name,
  ...
  */
  ...
  // mlog not allow alter
  // flag 0 alter table use for flush
  if (table_list->table->s->is_mlog && alter_info->flags != 0) {
    my_error(ER_CANNOT_ALTER_MLOG, MYF(0));
    return true;
  }

  if (table_list->table->s->has_mlog && alter_info->flags != 0 &&
      (alter_info->flags & Alter_info::ALTER_ADD_COLUMN ||
       alter_info->flags & Alter_info::ALTER_DROP_COLUMN ||
       alter_info->flags & Alter_info::ALTER_CHANGE_COLUMN ||
       alter_info->flags & Alter_info::ANY_ENGINE_ATTRIBUTE ||
       alter_info->flags & Alter_info::ALTER_RENAME)) {
    my_error(ER_CANNOT_ALTER_WITH_MLOG, MYF(0), "alter");
    return true;
  }

  // only allow create index on mv
  if (table_list->table->s->mview_info && alter_info->flags != 0) {
    if (!((alter_info->flags & Alter_info::ALTER_ADD_INDEX) ||
          (alter_info->flags & Alter_info::ALTER_DROP_INDEX))) {
      if (alter_info->flags & Alter_info::ALTER_OPTIONS) {
        if (create_info->used_fields & HA_CREATE_USED_SECONDARY_ENGINE) {
          my_error(ER_INVALID_MATERIALIZED_VIEW, MYF(0), "alter",
                   table_list->table_name);
          return true;
        }
      } else {
  ...
  */
  ...
  if (old_table_def != nullptr && old_table_def->options().exists("udt_name")) {
    if (check_unsupport_alter_with_udt_table(alter_info->flags)) {
      my_error(ER_ALTER_WITH_OF_TYPE_TABLE, MYF(0));
      return true;
    } else {
      dd::String_type udt_name;
      old_table_def->options().get("udt_name", &udt_name);
      create_info->udt_db_name = to_lex_cstring(
          thd->strmake(alter_ctx.new_db, strlen(alter_ctx.new_db)));
      create_info->udt_name =
          to_lex_cstring(thd->strmake(udt_name.c_str(), udt_name.length()));
    }
  }
  ...
  uint64_t ret = has_sequence_def(thd, alter_ctx.new_db, alter_ctx.new_name);
  if (ret == FIND_SEQ_FAIL) {
    return true;  // lock failed
  } else if (ret) {
  ...
  */
  ...
  // Handle trigger name, check constraint names and histograms statistics.
  {
    ...
    */
    ...
    if (backup_table->is_mlog() || backup_table->is_materialized_view()) {
      new_dd_table->copy_mv_info(backup_table);
      backup_table->drop_all_mv();
      update = true;
    }
...
bool check_unsupport_alter_with_udt_table(ulonglong check_flags) {
  // Alter operations which affects udt table metadata.
  const ulonglong unsupport_alter_with_udt_table =
      (Alter_info::ALTER_ADD_COLUMN | Alter_info::ALTER_DROP_COLUMN |
       Alter_info::ALTER_CHANGE_COLUMN | Alter_info::ALTER_ORDER |
       Alter_info::ALTER_CHANGE_COLUMN_DEFAULT | Alter_info::ALTER_KEYS_ONOFF |
       Alter_info::ALTER_RECREATE | Alter_info::ALTER_ADD_PARTITION |
       Alter_info::ALTER_DROP_PARTITION | Alter_info::ALTER_COALESCE_PARTITION |
       ...
       Alter_info::ALTER_REMOVE_PARTITIONING | Alter_info::ADD_FOREIGN_KEY |
       Alter_info::DROP_FOREIGN_KEY | Alter_info::ALTER_EXCHANGE_PARTITION |
       Alter_info::ALTER_TRUNCATE_PARTITION | Alter_info::ALTER_COLUMN_ORDER |
       Alter_info::ALTER_DISCARD_TABLESPACE |
       Alter_info::ALTER_IMPORT_TABLESPACE | Alter_info::ALTER_SECONDARY_LOAD |
       Alter_info::ALTER_SECONDARY_UNLOAD | Alter_info::ADD_CHECK_CONSTRAINT |
       Alter_info::DROP_CHECK_CONSTRAINT |
       Alter_info::ENFORCE_CHECK_CONSTRAINT |
       Alter_info::SUSPEND_CHECK_CONSTRAINT | Alter_info::DROP_ANY_CONSTRAINT |
       Alter_info::ENFORCE_ANY_CONSTRAINT | Alter_info::SUSPEND_ANY_CONSTRAINT |
       Alter_info::ANY_ENGINE_ATTRIBUTE | Alter_info::ALTER_COLUMN_VISIBILITY);
  if (check_flags & unsupport_alter_with_udt_table) {
    return true;
  }
  return false;
}
...
/*
  Create a  mlog table identical to the specified table

  SYNOPSIS
    mysql_create_mlog_table()
    thd    Thread object
    table       Table list element for target table
    src_table   Table list element for source table
    create_info Create info
...
bool mysql_create_mlog_table(THD *thd, Table_ref *table, Table_ref *src_table,
                             HA_CREATE_INFO *create_info) {
  Alter_info local_alter_info(thd->mem_root);
  Alter_table_ctx local_alter_ctx;  // Not used
  bool is_trans = false;
  ...
  if ((!src_table->is_base_table()) ||
      src_table->table->s->tmp_table != NO_TMP_TABLE) {
    my_error(ER_BAD_TABLE_ERROR, MYF(0), src_table->table_name);
    return true;
  }

  StringBuffer<256> trigger_name(STRING_WITH_LEN("imv_mlog_trigger_"),
                                 &my_charset_bin);
  {
    StringBuffer<MAX_KEY_LENGTH> primary_key;
    ...
    // Should not happen, we know the table exists and can be opened.
    assert(src_table_obj != nullptr);
    DEBUG_SYNC(thd, "create_mlog_table_after_open");

    if (src_table_obj == nullptr) {
      my_error(ER_BAD_TABLE_ERROR, MYF(0), src_table->table_name);
      return true;
    }
    if (src_table_obj->is_materialized_view()) {
      my_error(ER_CANNOT_CREATE_MLOG, MYF(0), src_table->table_name,
               "materialized view");
      return true;
    }

    if (src_table_obj->is_mlog()) {
      my_error(ER_CANNOT_CREATE_MLOG, MYF(0), src_table->table_name,
               "materialized view log");
      return true;
    }

    if (src_table_obj->has_mlog()) {
      my_error(ER_CANNOT_CREATE_MLOG, MYF(0), src_table->table_name,
               "has a materialized view log");
      return true;
    }

    for (const dd::Foreign_key *fk : src_table_obj->foreign_keys()) {
      if (fk->delete_rule() == dd::Foreign_key::RULE_CASCADE ||
          fk->update_rule() == dd::Foreign_key::RULE_CASCADE) {
        my_error(ER_CANNOT_CREATE_MLOG, MYF(0), src_table->table_name,
                 "foreign key cascade");
        return true;
      }
    ...
    bool first_pk = true;
    for (auto col : src_table_obj->columns()) {
    ...
    trigger_name.append(src_table_obj->name().c_str(),
                        src_table_obj->name().length());
    if (trigger_name.length() >= NAME_CHAR_LEN) {
      my_error(ER_TOO_LONG_IDENT, MYF(0), src_table->table_name);
      return true;
    }

    if (src_table_obj->get_trigger(trigger_name.c_ptr_quick())) {
      my_error(ER_TRG_INVALID_CREATION_CTX, MYF(0), src_table->db,
               src_table->table_name);
      return true;
    }

    auto trigger_name_utf8 =
        thd->strmake(trigger_name.c_ptr_quick(), trigger_name.length());

    create_info->m_mlog_info = new (thd->mem_root) mlog_info{
      ref_db : {src_table->table->s->db.str, src_table->table->s->db.length},
      ref_table_name : src_table_obj->name(),
      trigger_name : {trigger_name_utf8, trigger_name.length()}
    };
    if (create_info->m_mlog_info == nullptr) {
      return true;
    }
  }

  // use default storage innodb or greatdb
  handlerton *actual_hton =
      get_viable_handlerton_for_create(thd, table->table_name, *create_info);
  if (actual_hton == nullptr) return true;

  create_info->db_type = actual_hton;

  Create_field *cid_field = new (thd->mem_root) Create_field();
  if (cid_field == nullptr) return true;  // OOM

  if (cid_field->init(thd, mlog_id_column_name, MYSQL_TYPE_LONGLONG, nullptr,
                      nullptr, (UNSIGNED_FLAG | NOT_NULL_FLAG), nullptr,
                      nullptr, &EMPTY_CSTR, nullptr, nullptr, nullptr, false, 0,
                      nullptr, nullptr, nullptr, {},
                      dd::Column::enum_hidden_type::HT_VISIBLE, false))
    return true;
  if (local_alter_info.create_list.push_back(cid_field)) return true;

  cid_field = new (thd->mem_root) Create_field();
  if (cid_field == nullptr) return true;  // OOM

  if (cid_field->init(thd, mlog_op_column_name, MYSQL_TYPE_SHORT, nullptr,
                      nullptr, NOT_NULL_FLAG, nullptr, nullptr, &EMPTY_CSTR,
  ...
  if (cid_field->init(thd, mlog_ref_column_name, MYSQL_TYPE_LONGLONG, nullptr,
                      nullptr, UNSIGNED_FLAG, nullptr, nullptr, &EMPTY_CSTR,
                      nullptr, nullptr, nullptr, false, 0, nullptr, nullptr,
                      nullptr, {}, dd::Column::enum_hidden_type::HT_VISIBLE,
                      false))
    return true;
  ...
  Key_part_spec *key_part_spec = new (thd->mem_root) Key_part_spec(
      {mlog_id_column_name, strlen(mlog_id_column_name)}, 0, ORDER_ASC);
  ...
  if (key == nullptr || local_alter_info.key_list.push_back(key))
    return true;  // OOM

  /* Fill HA_CREATE_INFO and Alter_info with description of source table. */

  Create_field *def = nullptr;
  for (Field **field = src_table->table->field; *field != nullptr; ++field) {
    if ((*field)->is_gcol()) {
      // generated columns reject literal INSERTs from the trigger body
      my_error(ER_CANNOT_CREATE_MLOG, MYF(0), src_table->table_name,
               "generated column");
      return true;
    }
    ...
    def->m_default_val_expr = nullptr;
    def->constant_default = nullptr;
    local_alter_info.create_list.push_back(def);
  }

  if (mysql_create_table_no_lock(
          thd, table->db, table->table_name, create_info, &local_alter_info, 0,
          false,  // No FKs, no need to lookup parent keys
          &is_trans, &post_ddl_ht))
    goto err;
  ...
  */
  ...
  DEBUG_SYNC(thd, "create_mlog_table_before_binlog");

  // update src_table has mlog
  // add trigger to src_table
  {
    // mdl lock trigger
    ...
    if (push_mlog_src_table_and_trigger_mdl_request_to_list(
            thd, src_table->db, src_table->table_name,
            trigger_name.c_ptr_quick(), &mdl_requests))
      goto err;
    ...
    if (thd->dd_client()->acquire_for_modification(
            src_table->db, src_table->table_name, &new_table)) {
      goto err;
    }

    auto dd_trigger = new_table->add_trigger(
        dd::Trigger::enum_action_timing::AT_AFTER,
        dd::Trigger::enum_event_type::ET_INSERT_UPDATE_DELETE);
    if (dd_trigger == nullptr) {
      goto err;
    }
    // see fill_in_dd_trigger_object

    if (dd::fill_create_mlog_trigger(thd, table->table_name, new_table,
                                     dd_trigger)) {
      goto err;
    }
    ...
    close_all_tables_for_name(thd, src_table->table->s, false, nullptr);
    // update all sp
    sp_cache_invalidate();
  }

  query.length(0);
  if (greatdb::store_materialized_view_log_create_info(thd, src_table,
                                                       &query)) {
    goto err;
  }
