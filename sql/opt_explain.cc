...
bool Explain_join::shallow_explain() {
  ...
  if (query_lex->insert_table_leaf &&
    ...
    */
    tab = nullptr;
    // insert all print all table
    if (query_lex->sql_command == SQLCOM_INSERT_ALL_SELECT) {
      for (auto tb = query_lex->insert_table_leaf; tb != nullptr;
           tb = tb->next_leaf) {
        table = tb->table;
      ...
      if (query_plan->get_lex()->insert_table_leaf &&
        ...
        ret =
            Explain_table(
                ...
                /*quick=*/nullptr, /*condition=*/nullptr, MAX_KEY, HA_POS_ERROR,
                false, false,
                (query_plan->get_lex()->sql_command == SQLCOM_INSERT_ALL_SELECT
                     ? MT_INSERT
...
void print_query_for_explain(const THD *query_thd, Query_expression *unit,
  ...
  if (query_thd->query_plan.get_command() == SQLCOM_SELECT ||
      ...
      query_thd->query_plan.get_command() == SQLCOM_INSERT_ALL_SELECT ||
      ...
      query_thd->query_plan.get_command() == SQLCOM_INSERT_ALL_SELECT)  // (2)
  {
    /*
...
bool explain_query(THD *explain_thd, const THD *query_thd,
  ...
  explain_thd->lex->unit->select_limit_percent = false;
  explain_thd->lex->unit->select_limit_percent_value = 0;
  explain_thd->lex->unit->select_limit_ties = false;
