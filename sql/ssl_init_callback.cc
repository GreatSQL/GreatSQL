...
ssl_artifacts_status Ssl_init_callback_server_main::auto_detect_ssl() {
  MY_STAT cert_stat, key_stat, ca_stat;
#ifdef SSL_GM
  MY_STAT enc_cert_stat, enc_key_stat;
#endif /* SSL_GM */
  uint result = 1;
  ...
  */
  if ((!opt_ssl_cert || !opt_ssl_cert[0]) &&
    ...
    if (enable_gm_ssl) {
      result =
          result
          << ((my_stat(DEFAULT_SSL_SERVER_SIGN_CERT, &cert_stat, MYF(0)) &&
               my_stat(DEFAULT_SSL_SERVER_ENC_CERT, &enc_cert_stat, MYF(0)))
                  ? 1
                  : 0)
          << ((my_stat(DEFAULT_SSL_SERVER_SIGN_KEY, &key_stat, MYF(0)) &&
               my_stat(DEFAULT_SSL_SERVER_ENC_KEY, &enc_key_stat, MYF(0)))
                  ? 1
                  : 0);
    } else {
      result = result
               << (my_stat(DEFAULT_SSL_SERVER_CERT, &cert_stat, MYF(0)) ? 1 : 0)
               << (my_stat(DEFAULT_SSL_SERVER_KEY, &key_stat, MYF(0)) ? 1 : 0);
    }
#else  /* SSL_GM */
    result =
        result << (my_stat(DEFAULT_SSL_SERVER_CERT, &cert_stat, MYF(0)) ? 1 : 0)
               << (my_stat(DEFAULT_SSL_SERVER_KEY, &key_stat, MYF(0)) ? 1 : 0);
#endif /* SSL_GM */
    result = result << (my_stat(DEFAULT_SSL_CA_CERT, &ca_stat, MYF(0)) ? 1 : 0);
...
#ifdef SSL_GM
        if (enable_gm_ssl) {
          opt_ssl_cert = DEFAULT_GM_SSL_SERVER_CERT;
          opt_ssl_key = DEFAULT_GM_SSL_SERVER_KEY;
        } else {
