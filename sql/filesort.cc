...
uint Sort_param::make_sortkey(Bounds_checked_array<uchar> dst,
  ...
  for (sort_field = local_sortorder.begin();
    ...
    */
    ...
    if (maybe_null) {
      assert(*to == 0 || *to == 1);
      /* maybe_null: the first byte of 'to' as following at this point */
      /* if is_null, *to = 0, otherwise, *to = 1 */
      switch (sort_field->nulls_pos) {
        case NULLS_POS_MYSQL:
          /* asc:  null 0,    non-null 1 */
          /* desc: null 0xff, non-null 1 */
          if (sort_field->reverse && is_null) *to = 0xff;
          break;
        case NULLS_POS_ORACLE:
          /* asc:  null 0xff, non-null 1 */
          /* desc: null 0,    non-null 1 */
          if (!sort_field->reverse && is_null) *to = 0xff;
          break;
        case NULLS_POS_LAST:
          /* null 0xff, non-null 1 */
          if (is_null) *to = 0xff;
          break;
        case NULLS_POS_FIRST:
          /* null 0, non-null 1 */
          break;
      }
      ++to;
