...
static bool fill_dd_view_routines(View *view_obj,
  ...
  for (Sroutine_hash_entry *rt = routines_ctx->sroutines_list.first;
    ...
    if (rt->type() == Sroutine_hash_entry::PACKAGE_BODY) {
      my_error(ER_NOT_SUPPORTED_YET, MYF(0), "package function in VIEW");
      return true;
    }
    /*
...
static bool fill_dd_view_definition(THD *thd, View *view_obj, Table_ref *view) {
  ...
  */
  return fill_dd_view_routines(view_obj, thd->lex);
}
