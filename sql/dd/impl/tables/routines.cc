...
Routines::Routines() {
  ...
  m_target_def.add_field(FIELD_TYPE, "FIELD_TYPE",
                         ...
                         "'PACKAGE', 'PACKAGE BODY', 'TYPE') NOT NULL");
...
Routine *Routines::create_entity_object(const Raw_record &r) const {
  ...
  else if (routine_type == Routine::RT_PROCEDURE)
    return dd::create_object<Procedure>();
  else if (routine_type == Routine::RT_PACKAGE_SPEC)
    return dd::create_object<Package_spec>();
  else if (routine_type == Routine::RT_PACKAGE_BODY)
    return dd::create_object<Package_body>();
  else if (routine_type == Routine::RT_TYPE)
    return dd::create_object<Oracle_type>();
  else
    return nullptr;
}
