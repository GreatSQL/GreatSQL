...
static bool check_events(THD *thd, std::unique_ptr<Schema> &schema,
  ...
  auto process_event = [&](std::unique_ptr<dd::Event> &event) {
    ...
    // Create event creation context
    Stored_program_creation_ctx *creation_ctx = nullptr;
    Event_creation_ctx::create_event_creation_ctx(*event, &creation_ctx);
    if (creation_ctx) {
    ...
    thd->variables.sql_mode = event->sql_mode();
    dd::String_type sql;
...
static bool check_routines(THD *thd, std::unique_ptr<Schema> &schema,
  ...
  auto process_routine = [&](std::unique_ptr<dd::Routine> &routine) {
    if (routine->sql_mode() & MODE_ORACLE) {
      Routine_in_sql_mode_error_handler sql_mode_error_handler;
      thd->push_internal_handler(&sql_mode_error_handler);
      if (invalid_routine(thd, *schema, *routine)) {
        LogErr(ERROR_LEVEL, ER_UPGRADE_PARSE_ERROR, "Routine",
               ...
               sql_mode_error_handler.error_message());
      }
      ...
      if (invalid_routine(thd, *schema, *routine)) {
        ...
        // TODO: As upgrades to 9.0 only come from 8.4+, this code can be
        // removed in 9.0
...
bool invalid_routine(THD *thd, const dd::Schema &schema,
  ...
  if (prepare_return_type_string_from_dd_routine(thd, &routine,
                                                 &return_type_str)) {
    // Only if a UDT depended upon by a stored procedure is missing.
    my_error(ER_UDT_REFFER, MYF(0), "not found.");
    return true;
  }
  ...
  else if (routine.type() == dd::Routine::RT_PROCEDURE)
...
bool build_event_sp(const THD *thd, const char *name, size_t name_len,
  ...
  String temp(STATIC_SQL_LENGTH + name_len + body_len +
              (thd->variables.sql_mode & MODE_ORACLE ? oracle_style_len : 0));
  ...
  if (thd->variables.sql_mode & MODE_ORACLE) {
    temp.append(STRING_WITH_LEN("AS BEGIN "));
  }
  temp.append(body, body_len);
  if (thd->variables.sql_mode & MODE_ORACLE) {
    temp.append(STRING_WITH_LEN(" END"));
  }
