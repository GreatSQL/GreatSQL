...
/**
  This class is used to capture and store errors that occur during the parsing
  of stored programs during the upgrade process.
*/

class Routine_in_sql_mode_error_handler : public Internal_error_handler {
  dd::String_type reason{""};
 ...
 public:
  ...
  const char *error_message() const { return reason.c_str(); }
};
