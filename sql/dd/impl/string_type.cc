...
size_t hash<std::pair<dd::String_type, dd::String_type>>::operator()(
    const std::pair<dd::String_type, dd::String_type> &s) const {
  auto s1 = murmur3_32(reinterpret_cast<const uchar *>(s.first.c_str()),
                       s.first.size(), 0);
  auto s2 = murmur3_32(reinterpret_cast<const uchar *>(s.second.c_str()),
                       s.second.size(), 0);
  my_hash_combine(s1, s2);
  return s1;
}
}  // namespace std
