  ...
  */
  ...
  if (!opt_initialize) {
    ...
    */
    ...
    if (dd::tables::DD_properties::instance().get(
            thd, "GREATDB_DD_VERSION", &actual_greatdb_dd_version, &exists) ||
        !exists)
      actual_greatdb_dd_version = GREATDB_INITIAL_DD_VERSION_ID;

    if (actual_greatdb_dd_version != GREATDB_DD_VERSION_ID) {
      /* MERGE_PS8404_CHECK_TAG: @luhao, "opt_no_dd_upograde" not used but have no deleted.
      if (actual_greatdb_dd_version < GREATDB_DD_VERSION_ID &&
          opt_no_dd_upgrade) {
  ...
  */
  if (opt_initialize) {
    if (setprop(thd, "DD_VERSION", dd::DD_VERSION) ||
        ...
        setprop(thd, "GREATDB_DD_VERSION", GREATDB_DD_VERSION_ID))
      return dd::end_transaction(thd, true);

    if (setprop(thd, "MYSQLD_VERSION_UPGRADED", MYSQL_VERSION_ID) ||
        setprop(thd, "GREATDB_DD_VERSION_UPGRADED", GREATDB_DD_VERSION_ID))
      return true;
    bootstrap::DD_bootstrap_ctx::instance().set_upgraded_server_version(
        MYSQL_VERSION_ID);
    bootstrap::DD_bootstrap_ctx::instance().set_upgraded_greatdb_dd_version(
    ...
    if ((mysqld_version_lo > MYSQL_VERSION_ID &&
        ...
        (mysqld_version != MYSQL_VERSION_ID &&
         (setprop(thd, "MYSQLD_VERSION", MYSQL_VERSION_ID) ||
          ...
          setprop(thd, "SERVER_DOWNGRADE_THRESHOLD",
                  SERVER_DOWNGRADE_THRESHOLD) ||
          setprop(thd, "SERVER_UPGRADE_THRESHOLD",
                  SERVER_UPGRADE_THRESHOLD))) ||
        ((!exists_greatdb_dd_version ||
