...
#include "sql/dd/impl/tables/mlog.h"               // dd::materialized_view_logs
...
#include "sql/dd/types/materialized_view.h"  // Materialized_view
#include "sql/dd/types/oracle_type.h"        // Oracle_type
#include "sql/dd/types/package_body.h"       // Package_body
#include "sql/dd/types/package_spec.h"       // Package_spec
...
template <>
bool Dictionary_client::fetch_schema_component_names<Package_spec>(
    const Schema *schema, std::vector<String_type> *names) const {
  auto fetch_criteria = [&](Raw_record *r) {
    return static_cast<dd::Routine::enum_routine_type>(
               r->read_int(dd::tables::Routines::FIELD_TYPE)) ==
           dd::Routine::RT_PACKAGE_SPEC;  // Select only Pacakge_bodys.
  };
  return fetch_schema_component_names_by_criteria<Routine>(m_thd, schema, names,
                                                           fetch_criteria);
}
...
template <>
bool Dictionary_client::fetch_schema_component_names<Package_body>(
    const Schema *schema, std::vector<String_type> *names) const {
  auto fetch_criteria = [&](Raw_record *r) {
    return static_cast<dd::Routine::enum_routine_type>(
               r->read_int(dd::tables::Routines::FIELD_TYPE)) ==
           dd::Routine::RT_PACKAGE_BODY;  // Select only Pacakge_bodys.
  };
  return fetch_schema_component_names_by_criteria<Routine>(m_thd, schema, names,
                                                           fetch_criteria);
}
...
template <>
bool Dictionary_client::fetch_schema_component_names<Oracle_type>(
    const Schema *schema, std::vector<String_type> *names) const {
  auto fetch_criteria = [&](Raw_record *r) {
    return static_cast<dd::Routine::enum_routine_type>(
               r->read_int(dd::tables::Routines::FIELD_TYPE)) ==
           dd::Routine::RT_TYPE;  // Select only Pacakge_bodys.
  };
  return fetch_schema_component_names_by_criteria<Routine>(m_thd, schema, names,
                                                           fetch_criteria);
}
...
bool Dictionary_client::find_all_mlog(
    std::vector<String_type> *parent_schema,
    std::vector<String_type> *parent_name,
    std::vector<String_type> *children_schemas,
    std::vector<String_type> *children_names,
    std::vector<uint64_t> *children_ref) {
  dd::Transaction_ro trx(m_thd, ISO_READ_COMMITTED);

  trx.otx.register_tables<Materialized_view_log>();
  Raw_table *mlog_table = trx.otx.get_table<Materialized_view_log>();
  assert(mlog_table);
  ...
  auto tables_size = parent_schema->size();
  std::unique_ptr<Object_key> object_key(nullptr);
  for (size_t i = 0; i < tables_size; i++) {
    object_key.reset(tables::Materialized_view_logs::create_key_by_name(
        String_type(Dictionary_impl::default_catalog_name()),
        parent_schema->at(i), parent_name->at(i)));

    std::unique_ptr<Raw_record_set> mlog_rs;
    if (mlog_table->open_record_set(object_key.get(), mlog_rs)) {
      return true;
    }
    // has result
    dd::Raw_record *r = mlog_rs->current_record();
    if (r) {
      auto mlog_name = r->read_str(tables::Materialized_view_logs::FIELD_NAME);
      ulonglong ref_value = 0;
      if (!r->is_null(tables::Materialized_view_logs::FIELD_REF)) {
        ref_value = r->read_uint(tables::Materialized_view_logs::FIELD_REF);
      }
      children_schemas->push_back(parent_schema->at(i));
      children_names->push_back(mlog_name);
      children_ref->push_back(ref_value);
    } else {
      // unknow error
      return true;
    }
...
bool Dictionary_client::find_all_imv(
    std::function<bool(const String_type &table_ref)> func,
    std::vector<std::pair<String_type, String_type>> *ref_imvs) {
  std::vector<Object_id> table_id_list;
  auto processor = [&](Raw_record *r) -> bool {
    auto flush = r->read_int(tables::Materialized_views::FIELD_FLUSH_MODE);
    if (flush != 0) {
      auto table_ref = r->read_str(tables::Materialized_views::FIELD_TABLE_REF);
      // read
      if (func(table_ref)) {
        Object_id table_id =
            r->read_int(tables::Materialized_views::FIELD_TABLE_ID);
  ...
  if (fetch_raw_record<Materialized_view>(m_thd, processor)) {
    return true;
  }
  ...
  Raw_table *tables = trx.otx.get_table<dd::Table>();
  if (!tables) {
    return true;
  }

  Raw_table *schema_table = trx.otx.get_table<dd::Schema>();
  if (!schema_table) return true;

  if (trx.otx.open_tables()) {
    return true;
  }
  std::unique_ptr<Raw_record_set> tables_rs;
  std::unique_ptr<Raw_record_set> schema_rs;
  Primary_id_key schema_pk;
  Primary_id_key table_pk;
  Raw_record *rec = nullptr;
  Raw_record *schema_rec = nullptr;

  for (auto &id : table_id_list) {
    table_pk.update(id);
    tables_rs.reset();

    if (tables->open_record_set(&table_pk, tables_rs)) return true;
    rec = tables_rs->current_record();
    if (rec == nullptr) return true;
    auto table_name = rec->read_str(tables::Tables::FIELD_NAME);
    Object_id schema_id = rec->read_int(tables::Tables::FIELD_SCHEMA_ID);
    schema_pk.update(schema_id);
    schema_rs.reset();
    ...
    auto schema_name = schema_rec->read_str(tables::Schemata::FIELD_NAME);
    ref_imvs->push_back(std::make_pair(schema_name, table_name));
  }
...
template bool Dictionary_client::acquire_uncached(Object_id, Package_spec **);
template bool Dictionary_client::acquire(Object_id, const Package_spec **);
template bool Dictionary_client::acquire_for_modification(Object_id,
                                                          Package_spec **);
template void Dictionary_client::remove_uncommitted_objects<Package_spec>(bool);
template bool Dictionary_client::acquire(const String_type &,
                                         const String_type &,
                                         const Package_spec **);
template bool Dictionary_client::acquire_for_modification(const String_type &,
                                                          const String_type &,
                                                          Package_spec **);
template bool Dictionary_client::drop(const Package_spec *);
template bool Dictionary_client::store(Package_spec *);
template bool Dictionary_client::update(Package_spec *);

template bool Dictionary_client::acquire_uncached(Object_id, Package_body **);
template bool Dictionary_client::acquire(Object_id, const Package_body **);
template bool Dictionary_client::acquire_for_modification(Object_id,
                                                          Package_body **);
template void Dictionary_client::remove_uncommitted_objects<Package_body>(bool);
template bool Dictionary_client::acquire(const String_type &,
                                         const String_type &,
                                         const Package_body **);
template bool Dictionary_client::acquire_for_modification(const String_type &,
                                                          const String_type &,
                                                          Package_body **);
template bool Dictionary_client::drop(const Package_body *);
template bool Dictionary_client::store(Package_body *);
template bool Dictionary_client::update(Package_body *);

template bool Dictionary_client::acquire_uncached(Object_id, Oracle_type **);
template bool Dictionary_client::acquire(Object_id, const Oracle_type **);
template bool Dictionary_client::acquire_for_modification(Object_id,
                                                          Oracle_type **);
template void Dictionary_client::remove_uncommitted_objects<Oracle_type>(bool);
template bool Dictionary_client::acquire(const String_type &,
                                         const String_type &,
                                         const Oracle_type **);
template bool Dictionary_client::acquire_for_modification(const String_type &,
                                                          const String_type &,
                                                          Oracle_type **);
template bool Dictionary_client::drop(const Oracle_type *);
template bool Dictionary_client::store(Oracle_type *);
template bool Dictionary_client::update(Oracle_type *);
...
template bool Dictionary_client::acquire<Package_spec>(
    const String_type &, const String_type &,
    const Package_spec::Cache_partition **);
template bool Dictionary_client::acquire<Package_body>(
    const String_type &, const String_type &,
    const Package_body::Cache_partition **);
template bool Dictionary_client::acquire<Oracle_type>(
    const String_type &, const String_type &,
    const Oracle_type::Cache_partition **);
...
template bool Dictionary_client::acquire_for_modification<Package_spec>(
    const String_type &, const String_type &, Package_spec::Cache_partition **);
template bool Dictionary_client::acquire_for_modification<Package_body>(
    const String_type &, const String_type &, Package_body::Cache_partition **);
template bool Dictionary_client::acquire_for_modification<Oracle_type>(
    const String_type &, const String_type &, Oracle_type::Cache_partition **);
