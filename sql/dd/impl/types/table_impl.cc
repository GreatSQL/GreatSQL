...
#include "sql/dd/impl/tables/materialized_view.h"  // Materialized_views
#include "sql/dd/impl/tables/mlog.h"               // Materialized_view_logs
...
#include "sql/dd/impl/types/materialized_view_impl.h"  // Materialized_view_impl
#include "sql/dd/impl/types/mlog_impl.h"       // Materialized_view_log_impl
...
using dd::tables::Materialized_view_logs;
using dd::tables::Materialized_views;
...
bool Table_impl::restore_children(Open_dictionary_tables_ctx *otx) {
  ...
  */
  ...
  bool skip_materialized_view =
      (bootstrap::DD_bootstrap_ctx::instance()
           .is_greatdb_dd_upgrade_from_before(GREATDB_DD_VERSION_700));
  ...
  return (
      ...
      (!skip_materialized_view &&
       (m_materialized_view.restore_items(
            this, otx, otx->get_table<Materialized_view>(),
            Materialized_view::DD_table::create_key_by_table_id(this->id())) ||
        reload_check_has_mlog(otx) ||
        m_mlog.restore_items(
            this, otx, otx->get_table<Materialized_view_log>(),
            Materialized_view_log::DD_table::create_key_by_table_id(
                this->id())))));
}
...
bool Table_impl::store_children(Open_dictionary_tables_ctx *otx) {
  ...
  */
  ...
  return Abstract_table_impl::store_children(otx) ||
         ...
         (!skip_materialized_view &&
          (m_mlog.store_items(otx) || m_materialized_view.store_items(otx)));
}
...
bool Table_impl::drop_children(Open_dictionary_tables_ctx *otx) const {
  ...
  return m_materialized_view.drop_items(
             otx, otx->get_table<Materialized_view>(),
             Materialized_views::create_key_by_table_id(this->id())) ||
         m_mlog.drop_items(
             otx, otx->get_table<Materialized_view_log>(),
             Materialized_view_logs::create_key_by_table_id(this->id())) ||
         m_check_constraints.drop_items(
...
bool Table_impl::restore_attributes(const Raw_record &r) {
  bool materialized_view = false;
  {
    ...
    // keep flag , use for show table status table_type  materialized view and
    // mlog
    if (table_type == enum_table_type::MATERIALIZED_VIEW) {
      materialized_view = true;
      table_type = enum_table_type::BASE_TABLE;
    }
    if (table_type == enum_table_type::MATERIALIZED_VIEW_LOG) {
      table_type = enum_table_type::BASE_TABLE;
    }
  ...
  // compat upgrage for 6.0.1 old
  if (bootstrap::DD_bootstrap_ctx::instance().is_greatdb_dd_upgrade_from_before(
          GREATDB_DD_VERSION_700)) {
    if (materialized_view) {
      if (!r.is_null(Tables::FIELD_VIEW_DEFINITION_UTF8)) {
        auto define = r.read_str(Tables::FIELD_VIEW_DEFINITION);
        // 'select 1'
        if (define.length() > 7) {
          auto mv = add_mv();
          if (mv) {
...
bool Table_impl::store_attributes(Raw_record *r) {
  ...
  auto table_type = !m_mlog.empty() ? enum_table_type::MATERIALIZED_VIEW_LOG
                    : !m_materialized_view.empty()
                        ? enum_table_type::MATERIALIZED_VIEW
                        : type();

  // Store field values
  return Abstract_table_impl::store_attributes(r) ||
         r->store(Tables::FIELD_TYPE, static_cast<int>(table_type)) ||
...
void Table_impl::serialize(Sdi_wcontext *wctx, Sdi_writer *w) const {
  ...
  serialize_each(wctx, w, m_mlog, STRING_WITH_LEN("mlog"));
  serialize_each(wctx, w, m_materialized_view,
                 STRING_WITH_LEN("materialized_view"));
...
bool Table_impl::deserialize(Sdi_rcontext *rctx, const RJ_Value &val) {
  ...
  deserialize_each(
      rctx, [this]() { return add_mlog(); }, val, "mlog");

  deserialize_each(
      rctx, [this]() { return add_mv(); }, val, "materialized_view");
...
////////////////////////////////////////////////////////////////////////////
void Table_impl::drop_all_mv() {
  m_materialized_view.remove_all();
  m_mlog.remove_all();
}

void Table_impl::copy_mv_info(const Table *tab_obj) {
  if (tab_obj) {
    auto mv_info = tab_obj->get_mv_info();
    if (mv_info) {
      // has ready has new info no not copy new
      if (m_materialized_view.empty()) {
        auto new_mv = new (std::nothrow) Materialized_view_impl(
            *dynamic_cast<const Materialized_view_impl *>(mv_info), this);
        if (new_mv) {
          m_materialized_view.push_back(new_mv);
        }
      }
    }
    auto mlog = tab_obj->get_mlog_info();
    if (mlog) {
      if (m_mlog.empty()) {
        auto new_mlog = new (std::nothrow) Materialized_view_log_impl(
            *dynamic_cast<const Materialized_view_log_impl *>(mlog), this);
        if (new_mlog) {
          m_mlog.push_back(new_mlog);
        }
      }
    }
    m_has_mlog = tab_obj->has_mlog();
  }
}

Materialized_view_log *Table_impl::add_mlog() {
  Materialized_view_log_impl *mlog =
      new (std::nothrow) Materialized_view_log_impl(this);
  if (mlog != nullptr) m_mlog.push_back(mlog);
  return mlog;
}

Materialized_view_log *Table_impl::get_mlog_info() {
  return m_mlog.empty() ? nullptr : m_mlog.at(0);
}

const Materialized_view_log *Table_impl::get_mlog_info() const {
  return m_mlog.empty() ? nullptr : m_mlog.at(0);
}

bool Table_impl::is_mlog() const { return !m_mlog.empty(); }

Materialized_view *Table_impl::add_mv() {
  Materialized_view_impl *mv = new (std::nothrow) Materialized_view_impl(this);
  if (mv != nullptr) {
    m_materialized_view.push_back(mv);
  }
  return mv;
}

Materialized_view *Table_impl::get_mv_info() {
  return m_materialized_view.empty() ? nullptr : m_materialized_view.at(0);
}

const Materialized_view *Table_impl::get_mv_info() const {
  return m_materialized_view.empty() ? nullptr : m_materialized_view.at(0);
}

bool Table_impl::reload_check_has_mlog(Open_dictionary_tables_ctx *otx) {
  // 1. Read the parent's schema name based on schema_id.
  Raw_table *schema_table = otx->get_table<dd::Schema>();
  assert(schema_table);
  Primary_id_key schema_pk(schema_id());
  ...
  Table_reference_range_key parent_ref_key(
      tables::Materialized_view_logs::INDEX_UK_NAME,
      tables::Materialized_view_logs::FIELD_REFERENCED_TABLE_CATALOG,
      String_type(Dictionary_impl::default_catalog_name()),
      tables::Materialized_view_logs::FIELD_REFERENCED_TABLE_SCHEMA,
      schema_rec->read_str(tables::Schemata::FIELD_NAME),
      tables::Materialized_view_logs::FIELD_REFERENCED_TABLE_NAME, name());

  Raw_table *mlog_table = otx->get_table<dd::Materialized_view_log>();
  assert(mlog_table);

  std::unique_ptr<Raw_record_set> mlog_rs;
  if (mlog_table->open_record_set(&parent_ref_key, mlog_rs)) {
    return true;
  }
  // has result
  Raw_record *mlog_rec = mlog_rs->current_record();

  if (mlog_rec) {
    m_has_mlog = true;

    m_mlog_name =
        mlog_rec->read_str(tables::Materialized_view_logs::FIELD_NAME);

    // mlog db -> table_id->schema_id
    auto mlog_db_id =
        mlog_rec->read_int(tables::Materialized_view_logs::FIELD_SCHEMA_ID);
    schema_pk.update(mlog_db_id);
    schema_rs.reset();

    if (schema_table->open_record_set(&schema_pk, schema_rs)) return true;

    Raw_record *mlog_schema_rec = schema_rs->current_record();
    assert(mlog_schema_rec);

    if (mlog_schema_rec) {
      m_mlog_db = mlog_schema_rec->read_str(tables::Schemata::FIELD_NAME);
    } else {
      return true;
    }
...
void Table_impl::register_tables(Open_dictionary_tables_ctx *otx) {
  ...
  */
  ...
  if (!bootstrap::DD_bootstrap_ctx::instance()
           .is_greatdb_dd_upgrade_from_before(GREATDB_DD_VERSION_700)) {
    otx->register_tables<Materialized_view_log>();
    otx->register_tables<Materialized_view>();
  }
}
...
Table_impl::Table_impl(const Table_impl &src)
    : Weak_object(src),
      ...
      m_has_mlog(src.m_has_mlog),
      m_mlog_db(src.m_mlog_db),
      m_mlog_name(src.m_mlog_name),
  ...
  m_mlog.deep_copy(src.m_mlog, this);
  m_materialized_view.deep_copy(src.m_materialized_view, this);
}
}  // namespace dd
