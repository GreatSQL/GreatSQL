...
class Column_impl : public Entity_object_impl, public Column {
 public:
  ...
  // only used for udt.
  Column_impl(const Parameter_impl &src);
 ...
 public:
  ...
  bool is_udt() const override { return options().exists("udt_name"); }

  dd::String_type udt_name() const override {
    dd::String_type ident{""};
    if (options().exists("udt_name")) {
      options().get("udt_name", &ident);
    }
