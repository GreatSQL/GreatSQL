...
Package_spec *Schema_impl::create_package_spec(THD *thd) const {
  std::unique_ptr<Package_spec> p(dd::create_object<Package_spec>());
  p->set_schema_id(this->id());
...
Package_body *Schema_impl::create_package_body(THD *thd) const {
  std::unique_ptr<Package_body> p(dd::create_object<Package_body>());
  p->set_schema_id(this->id());
...
Oracle_type *Schema_impl::create_ora_type(THD *thd) const {
  std::unique_ptr<Oracle_type> type(dd::create_object<Oracle_type>());
  type->set_schema_id(this->id());

  // Get statement start time.
  ulonglong ull_curtime =
      dd::my_time_t_to_ull_datetime(thd->query_start_in_secs());

  type->set_created(ull_curtime);
  type->set_last_altered(ull_curtime);
