...
static const std::set<String_type> default_valid_option_keys = {
    "geom_type", "default_value", "sys_refcursor",
    "udt_name",  "udt_db_name",   "table_rowtype"};
...
bool Parameter_impl::store_attributes(Raw_record *r) {
  if (r->store(Parameters::FIELD_OPTIONS, m_options)) {
    my_error(ER_SP_INVALID_DEFAULT_PARAM, MYF(0), name().c_str(),
             "unsupport character");
    return true;
  }
