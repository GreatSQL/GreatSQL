...
class Table_impl : public Abstract_table_impl, virtual public Table {
 ...
 public:
  ...
  void copy_mv_info(const Table *tab_obj) override;
  ...
  Materialized_view_log *add_mlog() override;
  Materialized_view_log *get_mlog_info() override;
  const Materialized_view_log *get_mlog_info() const override;
  bool is_mlog() const override;

  bool has_mlog() const override { return m_has_mlog; }

  const String_type &mlog_name() const override { return m_mlog_name; }

  const String_type &mlog_db_name() const override { return m_mlog_db; }

  bool reload_check_has_mlog(Open_dictionary_tables_ctx *otx);
 ...
 public:
  ...
  // materialized_view
  /////////////////////////////////////////////////////////////////////////
  bool is_materialized_view() const override {
    return !m_materialized_view.empty();
  }

  Materialized_view *add_mv() override;
  Materialized_view *get_mv_info() override;
  const Materialized_view *get_mv_info() const override;
  void drop_all_mv() override;
 ...
 private:
  ...
  bool m_is_temporary;
  bool m_has_mlog;
  String_type m_mlog_db;
  String_type m_mlog_name;
  ...
  // refrence to  materialized_view_log
  // use collection is easy to copy
  // record mlog , and this collection size always <= 1
  Mlog_collection m_mlog;
  Materialized_view_collection m_materialized_view;
