...
Column_impl::Column_impl(const Parameter_impl &src)
    : Weak_object(src),
      Entity_object_impl(src),
      m_type(src.data_type()),
      m_is_nullable(true),
      m_is_zerofill(src.is_zerofill()),
      m_is_unsigned(src.is_unsigned()),
      m_is_auto_increment(false),
      m_is_virtual(false),
      m_hidden(enum_hidden_type::HT_VISIBLE),
      m_ordinal_position(src.ordinal_position()),
      m_char_length(src.char_length()),
      m_numeric_precision(src.numeric_precision()),
      m_numeric_scale(src.numeric_scale()),
      m_numeric_scale_null(src.is_numeric_scale_null()),
      m_datetime_precision(src.datetime_precision()),
      m_datetime_precision_null(src.is_datetime_precision_null()),
      m_has_no_default(false),
      m_default_value_null(true),
      m_options(default_valid_option_keys),
      m_se_private_data(),
      m_table(nullptr),
      m_elements(),
      m_column_type_utf8(src.data_type_utf8()),
      m_collation_id(src.collation_id()),
      m_is_explicit_collation(false),
      m_column_key(CK_NONE) {
  if (src.data_type() == enum_column_types::ENUM ||
      src.data_type() == enum_column_types::SET) {
    for (const Parameter_type_element *e : src.elements()) {
      Column_type_element_impl *col_impl = new Column_type_element_impl(*e);
      m_elements.push_back(col_impl);
    }
  }
  if (src.data_type() == enum_column_types::BIT) {
    if (!m_options.exists("treat_bit_as_char")) {
      set_options("treat_bit_as_char=1");
    }
  }
}
