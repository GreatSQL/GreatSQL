...
static const std::set<String_type> default_valid_option_keys = {
    "ref_count", "udt_size", "nested_table_udt", "table_type", "varray_limit"};
...
void Routine::create_mdl_key(enum_routine_type type,
  ...
  */
  ...
  assert(type == RT_FUNCTION || type == RT_PROCEDURE ||
         type == RT_PACKAGE_SPEC || type == RT_PACKAGE_BODY || type == RT_TYPE);
  // PACKAGE is treated as a whole object.
  // so they should share the same MDL key.
  mdl_key->mdl_key_init(
      type == RT_FUNCTION
          ? MDL_key::FUNCTION
          : (type == RT_PROCEDURE
                 ? MDL_key::PROCEDURE
                 : (type == RT_TYPE ? MDL_key::TYPE : MDL_key::PACKAGE_SPEC)),
