...
Query_block *build_show_procedures_query(const POS &pos, THD *thd, String *wild,
  ...
  assert(current_cmd == SQLCOM_SHOW_STATUS_FUNC ||
         current_cmd == SQLCOM_SHOW_STATUS_PROC ||
         current_cmd == SQLCOM_SHOW_STATUS_PACKAGE ||
         current_cmd == SQLCOM_SHOW_STATUS_PACKAGE_BODY ||
         current_cmd == SQLCOM_SHOW_STATUS_TYPE);
  ...
  */
  ...
  if (thd->lex->sql_command == SQLCOM_SHOW_STATUS_FUNC)
    current_type = "FUNCTION";
  else if (thd->lex->sql_command == SQLCOM_SHOW_STATUS_PROC)
    current_type = "PROCEDURE";
  else if (thd->lex->sql_command == SQLCOM_SHOW_STATUS_PACKAGE)
    current_type = "PACKAGE";
  else if (thd->lex->sql_command == SQLCOM_SHOW_STATUS_PACKAGE_BODY)
    current_type = "PACKAGE BODY";
  else if (thd->lex->sql_command == SQLCOM_SHOW_STATUS_TYPE)
    current_type = "TYPE";
  else
    return nullptr;
