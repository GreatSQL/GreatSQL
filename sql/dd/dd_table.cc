...
bool fill_dd_columns_from_create_fields(THD *thd, dd::Abstract_table *tab_obj,
  ...
  for (const Create_field &field : create_fields) {
    ...
    */
    ...
    // udt column
    if (field.udt_name.str) {
      // increase ref_count of the type referenced by the column.
      dd::Table *tab_obj_tmp = dynamic_cast<dd::Table *>(tab_obj);
      if (tab_obj_tmp != nullptr) {
        if (tab_obj_tmp->is_temporary()) {
...
static bool fill_dd_table_from_create_info(
  ...
  // View definition.
  if (create_info->m_mv_info) {
    auto m_info = create_info->m_mv_info;
    auto mv = tab_obj->get_mv_info();
    if (mv == nullptr) {
      mv = tab_obj->add_mv();
      if (mv == nullptr) return true;
      mv->set_status(0);
      mv->set_flush_mode(m_info->flush_mode);
      mv->set_build_clause(m_info->build_clasue);
      mv->set_definition(dd::String_type(m_info->create_view_define.str,
                                         m_info->create_view_define.length));
      mv->set_definition_view(
          dd::String_type(m_info->create_view_query_block.str,
                          m_info->create_view_query_block.length));
      mv->set_table_ref(
          dd::String_type(m_info->table_ref.str, m_info->table_ref.length));

      mv->set_columns(dd::String_type(m_info->derived_columns.str,
                                      m_info->derived_columns.length));

      mv->set_last_updated(thd->query_start_timeval_trunc(2));
    } else {
      if (m_info->build_clasue != 0) {
  ...
  if (create_info->m_mlog_info) {
    auto mlog = tab_obj->get_mlog_info();
    if (mlog == nullptr) {
      mlog = tab_obj->add_mlog();
      if (mlog == nullptr) return true;
      auto tb = create_info->m_mlog_info;
      mlog->set_referenced_table_schema_name(
          dd::String_type(tb->ref_db.str, tb->ref_db.length));
      mlog->set_referenced_table_name(tb->ref_table_name);
      mlog->set_referenced_table_catalog_name(
          Dictionary_impl::instance()->default_catalog_name());
      mlog->set_referenced_trigger_name(
          dd::String_type(tb->trigger_name.str, tb->trigger_name.length));
      mlog->set_status(0);
      mlog->set_ref(0);
      mlog->set_last_purged(thd->query_start_timeval_trunc(2));
    }
  }
  ...
  if (create_info->udt_name.str) {
    table_options->set("udt_name", (char const *)create_info->udt_name.str);
...
bool drop_table(THD *thd, const char *schema_name, const char *name,
  ...
  // down ref_count of udt columns used in this table
  if (alter_type_columns_ref_count(thd, schema_name, &table_def, false))
    return true;
...
/**
  increate/decrease ref_count of udt columns used in this table
*/
bool alter_type_columns_ref_count(THD *thd, const char *db,
                                  const dd::Table *obj, bool increase) {
  for (const dd::Column *col_obj : obj->columns()) {
    if (col_obj->options().exists("udt_name")) {
      dd::String_type ident;
      col_obj->options().get("udt_name", &ident);
      if (alter_type_ref_count(thd, db, ident.c_str(), increase)) return true;
    }
  }
  if (obj->options().exists("udt_name")) {
    dd::String_type udt_name;
    obj->options().get("udt_name", &udt_name);
    if (dd::alter_type_ref_count(thd, db, udt_name.c_str(), increase))
      return true;
  }
  return false;
}
