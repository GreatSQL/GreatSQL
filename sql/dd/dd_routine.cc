...
static void fill_parameter_info_from_field(THD *thd, Create_field *field,
  ...
  Table_ident *table_ident = field->ora_record.table_rowtype_ref();
  if (!table_ident) {
    if (field->udt_name.length != 0) {
      Properties *param_options = &param->options();
      String_type udt_name(field->udt_name.str, field->udt_name.length);
      String_type udt_db_name(field->udt_db_name.str,
                              field->udt_db_name.length);
      param_options->set("udt_name", udt_name);
      param_options->set("udt_db_name", udt_db_name);
    }
  } else {
    Properties *param_options = &param->options();
    String_type table_name(table_ident->table.str, table_ident->table.length);
    String_type table_db_name(table_ident->db.str, table_ident->db.length);
    param_options->set("udt_name", table_name);
    param_options->set("udt_db_name", table_db_name);
    param_options->set("table_rowtype", 1);
  }
}
...
static bool fill_dd_routine_info(THD *thd, const dd::Schema &schema,
  ...
  if (sp->get_ora_type()) {
    ...
    sp_root_parsing_ctx->retrieve_field_definitions(
        &field_def_lst, &vars_list, nullptr, &udt_var_count_diff, nullptr);
