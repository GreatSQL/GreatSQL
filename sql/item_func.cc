...
bool Item_func::do_itemize(Parse_context *pc, Item **res) {
  ...
  for (size_t i = 0; i < arg_count; i++) {
    ...
    /* MERGE_PS8404_CHECK_TAG: @fnw, double check*/
    add_accum_properties(args[i]);
    if ((pc->thd->variables.sql_mode & MODE_ORACLE) && functype() == FUNC_SP) {
      // allow named parameters in FUNC_SP for sql_mode = oracle
    } else if (no_named_params && !args[i]->item_name.is_autogenerated()) {
...
bool Item_func::fix_func_arg(THD *thd, Item **arg) {
  ...
  if (m_is_udt_table_index && (*arg)->type() == SUBQUERY_ITEM) {
  ...
  // do constant folding only for select stmt
  if (thd->lex->sql_command == SQLCOM_SELECT &&
      (*arg)->type() == Item::FUNC_ITEM &&
      static_cast<Item_func *>(*arg)->functype() == Item_func::TO_VECTOR &&
      (*arg)->const_item()) {
    char buff[MAX_FIELD_WIDTH];
    String tmp(buff, sizeof(buff), &my_charset_bin);
    String *result = (*arg)->val_str(&tmp);
    if (thd->is_error()) return true;
    ...
    if ((*arg)->null_value) {
      ...
      *arg =
          new (thd->mem_root) Item_string(tmp_str, length, result->charset());
      (*arg)->set_data_type_vector(length);
    }
  }
...
bool Item_func::collect_item_func_processor(uchar *arg) {
  DBUG_TRACE;
  DBUG_PRINT("info", ("%s", (result_field && result_field->field_name)
                                ? result_field->field_name
                                : "noname"));
  mem_root_deque<Item_func *> *item_list =
      reinterpret_cast<mem_root_deque<Item_func *> *>(arg);
  for (Item_func *curr_item : *item_list) {
    if (this == curr_item) return false; /* Already in the set. */
  }
...
// check * / %  datetype forbid in oracle mode
bool Item_num_op::resolve_type_inner(THD *thd) {
  if (thd->variables.sql_mode & MODE_ORACLE) {
    assert(args[0]->data_type() != MYSQL_TYPE_INVALID);
    assert(arg_count > 0 && args[1]->data_type() != MYSQL_TYPE_INVALID);
    for (uint i = 0; i < arg_count; i++) {
      if (args[i]->is_temporal()) {
        my_error(ER_ORACMP_DATE_OP, MYF(0),
                 fieldtype2str(real_type_to_type(args[0]->data_type())),
                 func_name(),
                 fieldtype2str(real_type_to_type(args[1]->data_type())));
        return true;
      }
    }
  }
  return Item_func_numhybrid::resolve_type_inner(thd);
}
...
int Item_typeto_number::check_input_str_precision(String *res) {
  int64_t length = res->length();
  int64_t precision = 0;
  String tmp_res;
  tmp_res.copy(res->ptr(), res->length(), res->charset());
  /*
    Processing ' xxx ' input ,trim left and right spaces
  */
  tmp_res.rtrim();
  tmp_res.ltrim();
  ...
  if (!format_flag) {
    for (i = length; i > 0; i--) {
      if (length > 1 && tmp_res.ptr()[i - 1] == '0')
        lastzerosum++;
      else
        break;
    }

    precision = length - (strchr(tmp_res.ptr(), '.')
                              ? (strchr(tmp_res.ptr(), '.') - tmp_res.ptr()) +
                                    1 + lastzerosum
                              : length);
    set_data_type_decimal(length, precision);
  }
  return false;
}
...
bool Item_typeto_format_number::generate_number_format_array(
    const String *format, uint *fmt_len, uint *dec) {
  const char *ptr, *end;
  ...
  for (; ptr < end; ptr++) {
    ...
    switch (cfmt) {
      ...
      case 'D':  // like .
        if (num_format->proint) goto error;
        for (uint16 *start_pos = num_format->fmt_inter;
             start_pos != tmp_fmt_int; start_pos++) {
          if (*start_pos == NUM_COMMA) goto error;
        }
      ...
      case 'V':
        ...
        if (!num_format->fillzero) num_format->fillzero = 1;
        break;
  ...
  *tmp_fmt_dec = 0;
  ...
  switch (num_format->is_EEEE) {
    ...
    case NUM_E:
      ...
      {
        Item *arg0 = args[0]->this_item();
        num_format->format_length =
            min(min(arg0->max_char_length(), uint32(MAX_BLOB_WIDTH)) * 10,
  ...
  *dec = num_format->fmt_decimal_len;
  return true;
...
bool Item_typeto_format_number::make_format_number_oracle(my_decimal *num,
                                                          String *str) {
  my_decimal rnd_dec;
  double rnd;
  int tmp_len = 0;
  ...
  // support to_number('0','99.99')
  if (num->intg == 1 && tmp_len == 0) num_format->fixed_prec = 1;
    ...
    switch (num_format->is_EEEE) {
      ...
      case NUM_E:  //
        if (!is_zero) {
          ...
          // remove final zero 1.230 e+02  => 1.23 e+02
          if (num_format->FM)
      ...
      default:
        if (!is_zero) {
          ...
          */
          ...
          const int actual_frac = decimal_actual_fraction(num);
          // keep decimal_len
          if ((fixed_length - actual_frac) != 0) {
            // FM: fmt 0.090999  0.123 => 0.123  0.1234 =>0.1234 0.1=>0.10
            if (num_format->fmt_decimal_len > actual_frac) {
              fixed_length = actual_frac;
              if (num_format->FM)
  ...
  if (rc != E_DEC_OK) {
    if (rc == E_DEC_OVERFLOW) {
      str->fill(num_format->format_length, '#');
      return false;
    } else {
      return true;
    }
...
bool delchar(char x) { return x == ','; }

my_decimal *Item_typeto_format_number::format_arg_string_to_number(
    String *format_string, my_decimal *dec) {
  int rec;
  std::string str(format_string->ptr(), format_string->length());
  str.erase(std::remove_if(str.begin(), str.end(), delchar), str.end());
  str.erase(std::remove_if(str.begin(), str.end(), ::isspace), str.end());
  if (!num_format->is_EEEE) {
    // str.erase(std::remove_if(str.begin(), str.end(), ::isalpha), str.end());
    rec = str2my_decimal(E_DEC_FATAL_ERROR, str.c_str(), str.length(),
                         collation.collation, dec);
    if (rec != E_DEC_OK) {
      my_error(ER_STD_INVALID_ARGUMENT, MYF(0), "args 0 is invald",
               func_name());
    }
  } else if (num_format->is_EEEE == NUM_E) {
    int error;
    uint length_c_str = str.length();
    const char *end_not_used;
    double value = my_strntod(collation.collation, str.c_str(), length_c_str,
                              &end_not_used, &error);
    if (error) {
      char tmp[NAME_LEN + 1];
      snprintf(tmp, sizeof(tmp), "%.*s", length_c_str, str.c_str());
    ...
    const char *endptr;
    int err;
    tmpdec = (longlong)my_strntoull(format_string->charset(), str.c_str(),
                                    str.length(), 16, &endptr, &err);
    if (err) {
      /*
      ...
      str2my_decimal(E_DEC_OK, str.c_str(), str.length(),
                     format_string->charset(), &res_as_dec);
      if (res_as_dec.sign()) dec = nullptr;
      ErrConvString err_str(format_string);
      push_warning_printf(current_thd, Sql_condition::SL_WARNING,
...
bool Item_typeto_format_number::check_format(String *format_string) {
  String *format;
  StringBuffer<DECIMAL_MAX_FIELD_SIZE> format_buffer;
  uint ulen = 0;
  uint deci = 0;
  int number_actual_frac = 0;
  int number_actual_intg = 0;
  actual_precision = 0;
  int lastzerosum = 0;
  std::string str(format_string->ptr(), format_string->length());
  str.erase(std::remove_if(str.begin(), str.end(), delchar), str.end());
  {
    if (!fixed_length) {
      ...
      assert(num_format);
      memset(num_format, 0, sizeof(NUMDesc));
      if (!generate_number_format_array(format, &ulen, &deci)) {
        if (ulen > DECIMAL_MAX_FIELD_SIZE || deci > DECIMAL_MAX_SCALE) {
      ...
      set_data_type_decimal(ulen, deci);
    }
    // not support 'B' FMT
    if (num_format->zero_return_empty) goto error;
    // not support 'S' FMT
    if (num_format->is_minus == NUM_S) goto error;
    // not support 'PR' FMT
    if (num_format->is_minus == NUM_PR) goto error;
    // not support 'MI' FMT
    if (num_format->is_minus == NUM_MI) goto error;
    // not support 'FM' FMT
    if (num_format->FM) goto error;
    // not support 'V' FMT
    if (num_format->fillzero) goto error;

    if (strchr(format_string->ptr(), '.')) {
      number_actual_frac =
          format_string->length() -
          (strchr(format_string->ptr(), '.') - format_string->ptr()) - 1;
      number_actual_intg = format_string->length() - number_actual_frac - 1;
    } else {
      number_actual_intg = format_string->length() - number_actual_frac;
    }
    ...
    switch (args[0]->data_type()) {
      ...
      case MYSQL_TYPE_NEWDECIMAL: {
        int i = 0;
        if (strchr(format_string->ptr(), '.')) {
          for (i = format_string->length(); i > 0; i--) {
            if (format_string->length() > 1 &&
                format_string->ptr()[i - 1] == '0')
              lastzerosum++;
            else
              break;
          }
          number_actual_frac =
              format_string->length() -
              (strchr(format_string->ptr(), '.') - format_string->ptr()) - 1;
          number_actual_frac = number_actual_frac - lastzerosum;
        }
        set_data_type_decimal(num_format->format_length, number_actual_frac);

        // support to_number(1.12,'09.99') error
        if (strchr(format_string->ptr(), '.'))
          number_actual_intg = format_string->length() - number_actual_frac - 1;
        else
          number_actual_intg = format_string->length() - number_actual_frac;
        if (number_actual_intg - sign < num_format->fixed_prec) goto error;
        // support to_number(-0.12,'0.99') error,to_number(-0.12,'9.99') not
        // error
        if ((number_actual_intg - sign) == 1 &&
            format_string->ptr()[number_actual_intg - 1] == '0' &&
            num_format->fmt_inter[num_format->intg - 1] == NUM_0)
          goto error;

        if ((number_actual_frac == 0) && strchr(format_string->ptr(), '.')) {
        ...
        if (number_actual_frac > num_format->fmt_decimal_len) goto error;
      } break;
    ...
    to_number('12','999,999') not error
    support to_number('1234','999,999') to error
    */
    if (num_format->fmt_inter_len !=
            (num_format->intg + num_format->fillzero) &&
        !num_format->is_EEEE) {
      auto format_string_pos = std::find(ptr, end, '.');
      int format_string_int = format_string_pos - ptr;
      int j = 0;
      int k = format_string_int;
      // support to_number('3,,2,,1','9,9,9.99') to_number('1,2,34','9,999')
      // The integer part is from right to left. The first time there are
      // commas, it can be ignored; the second time there are multiple
      // commas, an error should be reported,for example
      // to_number('3,,2,,1','9,9,9.99')
      bool fisrt_comma = false;
      ...
      if (k > j) goto error;
    } else if (!num_format->is_EEEE) {
      if (sign == 1) {
        if ((str.length() - 1 - lastzerosum) > num_format->format_length) {
          return true;
        }
      } else {
        if ((str.length() - lastzerosum) > num_format->format_length) {
          return true;
        }
      }
    }

    // to_number('01','9099') to error
    if ((int)(str.length() - sign) < num_format->fixed_prec) goto error;
    // support to_number('1.12','09.99') error
    if (number_actual_intg < num_format->fixed_prec) goto error;
    ...
    if (num_format->is_EEEE) {
          ...
          if ((proint_pos != num_E_end)) {
            ...
            if (!lastzerosum) {
              ...
              for (; E_decimal_len > 0; E_decimal_len--) {
                if (*(proint_pos + E_decimal_len) == '0')
                  befor_E_pos_zero++;
                else
                  break;
              }
              num_format->fmt_decimal_len =
                  E_pos - proint_pos - 1 - befor_E_pos_zero;
              fixed_length = false;
            }
          } else {
      ...
      // to_number('10.001','9.99') ,to_number('-1.0','9999') to error
      // FMT E ,to_number('1.6E+02','9.9EEEE') is not error
      auto format_string_priont_pos = std::find(ptr, end, '.');
      int actual_frac = end - format_string_priont_pos - lastzerosum - 1;
    ...
    if (lastzerosum != 0) {
      num_format->fmt_decimal_len = number_actual_frac;
      fixed_length = false;
    }
    return false;
  }
error:
  my_error(ER_DATA_OUT_OF_RANGE, MYF(0), "NUMBER", func_name());
  push_warning_printf(
      current_thd, Sql_condition::SL_WARNING, ER_WARN_DATA_OUT_OF_RANGE,
      ER_THD(current_thd, ER_WARN_DATA_OUT_OF_RANGE), item_name.ptr(), 1L);
  null_value = true;
...
String *Item_typeto_format_number::val_str(String *str) {
  assert(nullptr != str);
  null_value = false;
  String *str_tmp = args[0]->val_str(str);
  if (args[0]->is_null()) {
    null_value = true;
    return nullptr;
  }
  /*
     Processing '' input returns null
   */
  if (str_tmp != nullptr && str_tmp->length() == 0) {
    null_value = true;
    return nullptr;
  }
    if (args[1]->null_value || args[1]->is_null()) {
      null_value = true;
      return nullptr;
    }
    my_decimal tmp_buf, *tmp = val_decimal(&tmp_buf);
    ...
    if (num_format->is_EEEE == NUM_E) {
      double value;
      my_decimal2double(E_DEC_FATAL_ERROR, tmp, &value);
      str->set_real(value, DECIMAL_NOT_SPECIFIED, collation.collation);
      return str;
    } else {
      my_decimal2string(E_DEC_FATAL_ERROR, tmp, str);
      return str;
...
double Item_typeto_format_number::val_real() {
  my_decimal tmp_buf, *tmp = val_decimal(&tmp_buf);
  double res;
...
longlong Item_typeto_format_number::val_int() {
  my_decimal tmp_buf, *tmp = val_decimal(&tmp_buf);
  longlong res;
...
my_decimal *Item_typeto_format_number::val_decimal(my_decimal *dec) {
  my_decimal *num;
  char str_buffer[64];
  null_value = false;
  ...
  String *tmp = args[0]->val_str(&str);
  {
    ...
    if (!make_format_number_oracle(num, &str)) {
      str.ltrim();
      str.rtrim();
      str.rtrim_zero();
      if (!str.length()) goto err;
      int error = str2my_decimal(E_DEC_FATAL_ERROR, str.ptr(), str.length(),
                                 str.charset(), dec);
      if (error != E_DEC_OK) goto err;
      return dec;
    }
  }
err:
...
String *Item_typeto_number::val_str(String *str) {
  assert(nullptr != str);
  if (!args[0]->is_null()) {
    ...
    /*
      Processing '' input returns null
    */
    if (res == nullptr || !res->length()) {
      null_value = true;
      return nullptr;
    }
    if (check_input_str_precision(res)) goto err;
    from_str = true;
    my_decimal tmp_buf, *tmp = val_decimal(&tmp_buf);
...
double Item_typeto_number::val_real() {
  my_decimal tmp_buf, *tmp = val_decimal(&tmp_buf);
  double res;
...
longlong Item_typeto_number::val_int() {
  my_decimal tmp_buf, *tmp = val_decimal(&tmp_buf);
  longlong res;
...
my_decimal *Item_typeto_number::val_decimal(my_decimal *dec) {
  my_decimal tmp_buf, *tmp = args[0]->val_decimal(&tmp_buf);
  bool sign;
  uint precision;
  String tmp_value;
  ...
  if ((null_value = args[0]->null_value)) return nullptr;
  /*
    Processing where expr to_number('xxx') > < = xxx
  */
  ...
  my_decimal_round(E_DEC_FATAL_ERROR, tmp, (from_str ? decimals : tmp->frac),
                   true, dec);
  from_str = false;
  sign = dec->sign();
...
void Item_typeto_number::print(const THD *thd, String *str,
                               enum_query_type query_type) const {
  str->append(STRING_WITH_LEN("to_number("));
  args[0]->print(thd, str, query_type);
  str->append(')');
}
...
/**
  Calculate plus  Datetime  with number
  ...
  @retval
    0  Value was NULL;  In this case null_value is set
*/
bool Item_func_plus::date_op(MYSQL_TIME *ltime, my_time_flags_t) {
  assert(datetype_offset);
  assert(current_thd->variables.sql_mode & MODE_ORACLE);
  double num;
  auto offset = datetype_offset == 1 ? 1 : 0;
  null_value = args[datetype_offset - 1]->get_date(ltime, TIME_NO_ZERO_IN_DATE);
  if (null_value) return null_value;
  num = args[offset]->val_real();
  if (args[offset]->null_value || !std::isfinite(num)) {
    null_value = true;
    return true;
  }
  ...
  if ((null_value = date_add_double_with_warn(current_thd, ltime, num, false)))
    return true;
  return false;
}

bool Item_func_plus::resolve_type_inner(THD *thd) {
  if (thd->variables.sql_mode & MODE_ORACLE) {
    bool r1 = args[0]->is_temporal();
    bool r2 = args[1]->is_temporal();
    Item *date_item = nullptr;
    if (r1) {
      if (r2) {
        my_error(ER_ORACMP_DATE_OP, MYF(0),
                 fieldtype2str(real_type_to_type(args[0]->data_type())),
    ...
    switch (real_type_to_type(date_item->data_type())) {
      case MYSQL_TYPE_DATETIME:
        set_data_type_datetime(date_item->datetime_precision());
        break;
      case MYSQL_TYPE_TIMESTAMP:
        set_data_type_timestamp(date_item->datetime_precision());
        break;
      case MYSQL_TYPE_TIME:
        set_data_type_datetime(date_item->time_precision());
        break;
      default:
...
my_decimal *Item_func_minus::decimal_op(my_decimal *decimal_value) {
  if (datetype_count == 2) {
    ...
    long microseconds;
    int neg = 1;
    my_decimal second_24hour, val;
    int err;

    null_value = args[0]->get_date(&t0, TIME_NO_ZERO_IN_DATE);
    if (null_value) return nullptr;

    null_value = args[1]->get_date(&t1, TIME_NO_ZERO_IN_DATE);
    if (null_value) return nullptr;

    if (calc_time_diff(t0, t1, 1, &seconds, &microseconds)) neg = -1;

    int2my_decimal(E_DEC_FATAL_ERROR, SECONDS_IN_24H * 1000000L, false,
                   &second_24hour);

    if (check_decimal_overflow(int2my_decimal(
            E_DEC_FATAL_ERROR, ((seconds * 1000000L + microseconds) * neg),
            false, &val)) > 3) {
      return error_decimal(decimal_value);
    }
    my_decimal_set_zero(decimal_value);
    if ((err = check_decimal_overflow(my_decimal_div(
             E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW & ~E_DEC_DIV_ZERO,
             decimal_value, &val, &second_24hour, decimals))) > 3) {
...
bool Item_func_minus::resolve_type_inner(THD *thd) {
  if (thd->variables.sql_mode & MODE_ORACLE) {
    bool r1 = args[0]->is_temporal();
    bool r2 = args[1]->is_temporal();
    if (r1) {
      if (r2) {
        ...
        by mysql have not  interval return so use decimal the same

        datatype:
          Item_num_op::set_numeric_type
        */
        datetype_count = 2;
        hybrid_type = DECIMAL_RESULT;
        set_data_type(MYSQL_TYPE_NEWDECIMAL);
        ...
        // 2 + 2 + 2 = h + m + s
        // not use in var_str
        decimals += 6 + thd->variables.div_precincrement;
      } else {
        // date - number
        hybrid_type = STRING_RESULT;
        datetype_count = 1;
        switch (real_type_to_type(args[0]->data_type())) {
          case MYSQL_TYPE_DATETIME:
            set_data_type_datetime(args[0]->datetime_precision());
            break;
          case MYSQL_TYPE_TIMESTAMP:
            set_data_type_timestamp(args[0]->datetime_precision());
            break;
          case MYSQL_TYPE_TIME:
            set_data_type_datetime(args[0]->time_precision());
            break;
          default:  // MYSQL_TYPE_DATE
            set_data_type_datetime(0);
            break;
        }
      }
...
bool Item_func_minus::date_op(MYSQL_TIME *ltime, my_time_flags_t) {
  assert(current_thd->variables.sql_mode & MODE_ORACLE);
  assert(datetype_count == 1);
  double num;
  null_value = args[0]->get_date(ltime, TIME_NO_ZERO_IN_DATE);
  if (null_value) return null_value;

  num = args[1]->val_real();
  if (args[1]->null_value || !std::isfinite(num)) {
    null_value = true;
    return true;
  }
  ...
  if ((null_value = date_add_double_with_warn(current_thd, ltime, num, true)))
    return true;

  return false;
}

String *Item_func_minus::val_str(String *str) {
  if (datetype_count == 2) {
    assert(current_thd->variables.sql_mode & MODE_ORACLE);
    assert(hybrid_type == DECIMAL_RESULT);
    my_decimal decimal_value, *val;
    if (!(val = decimal_op(&decimal_value))) return nullptr;  // null is set
    auto dec = decimal_actual_fraction(val);
    my_decimal_round(E_DEC_FATAL_ERROR, val, dec, false, val);

    str->set_charset(collation.collation);
    my_decimal2string(E_DEC_FATAL_ERROR, val, str);
    return str;
  }
  return Item_func_numhybrid::val_str(str);
}
...
longlong Item_func_round::int_op() {
  const longlong value = args[0]->val_int();
  const longlong dec = get_decimal_place();
  decimals = 0;
  ulonglong abs_dec;
  if ((null_value = args[0]->null_value || args[1]->null_value)) return 0;
...
bool Item_func_ora_round::resolve_type(THD *thd) {
  if ((is_temporal_type(args[0]->data_type()) &&
       args[0]->data_type() != MYSQL_TYPE_YEAR) ||
...
longlong Item_func_lengthb::val_int() {
  assert(fixed);
...
longlong Item_func_length::val_int() {
  assert(fixed);
  String *res = args[0]->val_str(&value);
  if (!res || (0 == res->length() && oracle_mode_flag)) {
    null_value = true;
    return 0; /* purecov: inspected */
  }
  null_value = false;
  return oracle_mode_flag ? (longlong)res->numchars() : (longlong)res->length();
}
bool Item_func_length::resolve_type(THD *thd) {
...
bool Item_func_vec_distance::resolve_type(THD *thd) {
  // check 1st and 2ed arg
  if (param_type_is_default(thd, 0, 2, MYSQL_TYPE_VECTOR)) {
    return true;
  }

  // collation is bin
  auto valid_type = [](const Item *item) {
    return (item->data_type() == MYSQL_TYPE_VECTOR) ||
           (item->result_type() == STRING_RESULT &&
            item->collation.collation == &my_charset_bin);
  };

  bool valid_arg3 = true;
  if (arg_count == 3) {
    if (param_type_is_default(thd, 2, 3)) {
      return true;
    }
    valid_arg3 = args[2]->result_type() == STRING_RESULT;
  }

  if (!valid_type(args[0]) || !valid_type(args[1]) || !valid_arg3) {
    my_error(ER_WRONG_ARGUMENTS, MYF(0), func_name());
    return true;
  }
...
double Item_func_vec_distance::val_real() {
  String tmp_value1;
  String tmp_value2;
  String tmp_value_type;

  null_value = false;

  String *vec1 = args[0]->val_str(&tmp_value1);

  if (vec1 == nullptr || vec1->ptr() == nullptr) return error_real();

  String *vec2 = args[1]->val_str(&tmp_value2);

  if (vec2 == nullptr || vec2->ptr() == nullptr) return error_real();

  String *type_str = nullptr;
  if (arg_count == 3) type_str = args[2]->val_str(&tmp_value_type);

  uint32 dimensions1 = get_dimensions(vec1->length(), Field_vector::precision);
  if (dimensions1 == UINT32_MAX) {
    my_error(ER_TO_VECTOR_CONVERSION, MYF(0), 32, vec1->ptr());
    return error_real();
  }

  uint32 dimensions2 = get_dimensions(vec2->length(), Field_vector::precision);
  if (dimensions2 == UINT32_MAX) {
    my_error(ER_TO_VECTOR_CONVERSION, MYF(0), 32, vec2->ptr());
    return error_real();
  }

  if (dimensions1 != dimensions2) {
    my_error(ER_WRONG_ARGUMENTS, MYF(0), "vector dimensions must be match");
    return error_real();
  }

  float *v1 = (float *)vec1->ptr();
  float *v2 = (float *)vec2->ptr();

  if (type_str == nullptr || type_str->ptr() == nullptr ||
      my_strcasecmp(system_charset_info, type_str->c_ptr(), "dot") == 0)
    return vector_distance::dot_product(v1, v2, dimensions1);

  if (my_strcasecmp(system_charset_info, type_str->c_ptr(), "euclidean") == 0)
    return vector_distance::euclidean_distance(v1, v2, dimensions1);
  if (my_strcasecmp(system_charset_info, type_str->c_ptr(), "cosine") == 0)
    return vector_distance::cosine_distance(v1, v2, dimensions1);

  my_error(ER_WRONG_ARGUMENTS, MYF(0),
           "Invalid distance type. Must be one of: Euclidean, Cosine, Dot");
  return error_real();
}
...
longlong Item_func_ora_instr::val_int() {
  assert(fixed);
  int default_pos = 0;
  ...
  char *str_stop = a->ptr() + a->length();

  boost::bimaps::bimap<int, long> index_map;

  int count = 0;
  longlong len = 0;

  for (str_start = a->ptr(); str_start < str_stop; count++) {
    uint flag = use_mb(a->charset())
                    ? my_ismbchar(a->charset(), str_start, str_stop)
                    : 0;

    String temp("", a->charset());

    if (flag) {
      while (flag--) {
        temp.append(*str_start++);
      }
    } else {
      temp.append(*str_start++);
    }

    int index = (count == 0 ? 0 : len);
    len += temp.length();
    index_map.insert(boost::bimaps::bimap<int, long>::value_type(count, index));
  }

  std::string str(a->ptr(), a->length());
  std::string sub_str(b->ptr(), b->length());

  size_t num = a->numchars();
  ...
  if (arg_count == 4) {
    ...
    if ((args[3]->null_value) || (null_value = args[2]->null_value)) {
      null_value = true;
      return default_pos; /* purecov: inspected */
    }
    /*Decimal to integer,removing rounding; udt function is rounding*/
    ...
    if (positive) {
      start = tmp - 1;
      if (start > static_cast<longlong>(a->length())) return default_pos;
      if (start + b->length() > a->length()) return default_pos;
      start_position = start;
    } else {
      start_position = num + tmp;
      if (start_position < 0) return default_pos;
    }

    boost::bimaps::bimap<int, long>::left_iterator iter =
        index_map.left.find(start_position);

    std::size_t position = 0;
    if (iter != index_map.left.end()) {
    ...
    while ((position = (positive ? str.find(sub_str, position)
                                 : str.rfind(sub_str, position))) !=
           std::string::npos) {
      boost::bimaps::bimap<int, long>::right_iterator right_iter =
          index_map.right.find(position);
      if (right_iter != index_map.right.end()) {
        match_pos = right_iter->second;
      } else {
        break;
      }

      if (i >= occurrence) {
        match_occurrence = true;
        break;
      }

      boost::bimaps::bimap<int, long>::left_iterator left_iter =
          index_map.left.find((positive ? ++match_pos : --match_pos));
      if (left_iter != index_map.left.end()) {
        position = left_iter->second;
      } else {
        break;
      }
    ...
    return match_occurrence ? match_pos + 1 : default_pos;
  }

  std::size_t position = str.find(sub_str);
  if (position != std::string::npos) {
    boost::bimaps::bimap<int, long>::right_iterator right_iter =
        index_map.right.find(position);
    if (right_iter != index_map.right.end()) {
      return right_iter->second + 1;
    } else {
      return default_pos;
    }
  } else {
...
void Item_func_ora_instr::print(const THD *thd, String *str,
                                enum_query_type query_type) const {
  str->append(STRING_WITH_LEN("instr("));
  args[0]->print(thd, str, query_type);
  str->append(',');
  args[1]->print(thd, str, query_type);
  if (arg_count == 4) {
...
String *user_var_entry::val_str(bool *null_value, String *str,
  ...
  switch (m_type) {
    ...
    case STRING_RESULT:
      ...
      /*
        When emptystring_equal_null mode，if m_ptr is an empty string return
        nullptr
      */
...
void Item_func_get_system_var::print_var_name(const THD *, String *str,
                                              enum_query_type) {
  str->append(var_tracker.get_var_name());
}
...
longlong Item_func_table_count::val_int() {
  assert(fixed && current_thd->sp_runtime_ctx);

  Item_field_row_table *row_table_item = dynamic_cast<Item_field_row_table *>(
      m_rcontext_handler->get_rcontext(current_thd->sp_runtime_ctx)
          ->get_item(m_spv_idx));
  if (!row_table_item) {
...
Item_func_udt_constructor::Item_func_udt_constructor(
    ...
    List<Create_field> *field_def, ulong reclength, sql_mode_t sql_mode)
    : Item_func(pos, item_list) {
  m_db_name = db_name;
  m_udt_name = udt_name;
  m_use_explicit_name = use_explicit_name;
  ...
  /* In Item_func(pos, item_list), arg_count and args are initialized
     using item_list.
   */
}

bool Item_func_udt_constructor::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
  ...
  /* args are initialized in super::do_itemize() */
  uint flds_num = m_field_def_list->size();
  if (flds_num > arg_count) {
    Create_field *fld = m_field_def_list[0][arg_count];
    my_error(ER_SP_INVALID_DEFAULT_PARAM, MYF(0), fld->field_name,
             m_udt_name.str);
    return true;
  } else if (flds_num < arg_count) {
    String buff(m_db_name.length + 1 + m_udt_name.length + 1);
    if (m_db_name.length > 0) {
      buff.append(m_db_name.str);
      buff.append('.');
    }
    buff.append(m_udt_name.str);
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), "TYPE", buff.c_ptr(), flds_num,
             arg_count);
    return true;
  }
  return false;
...
bool Item_func_udt_constructor::fix_fields(THD *thd, Item **ref) {
...
bool Item_func_udt_constructor::refix_fields() {
  for (uint i = 0; i < arg_count; i++) {
    Item_func *arg_func = dynamic_cast<Item_func *>(args[i]);
    if (arg_func && arg_func->refix_fields()) {
      return true;
    }
  }
  return false;
...
TABLE *Item_func_udt_constructor::val_udt() {
  ...
  for (uint i = 0; i < arg_count; i++) {
    ...
    if (sp_eval_expr(current_thd, true, m_table->field[i], &args[i])) {
      return nullptr;
    }
  }
...
void Item_func_udt_constructor::make_field(Send_field *tmp_field) {
  tmp_field->db_name = m_db_name.str;
  tmp_field->org_table_name = "";
  tmp_field->table_name = "";
  tmp_field->org_col_name = full_name();
  tmp_field->col_name = full_name();
  tmp_field->charsetnr = charset_for_protocol()->number;
  tmp_field->length = m_reclength;
  tmp_field->type = MYSQL_TYPE_VARCHAR;
  if (current_thd->variables.udt_format_result == UDT_FORMAT_RESULT_BINARY) {
    tmp_field->flags = BINARY_FLAG;
  } else {
    tmp_field->flags = 0;
  }
...
bool Item_func_udt_constructor::udt_table_store_to_table(TABLE *table_to) {
  ...
  if (table_to->s->db.length && table_to->s->udt_name.length &&
      (my_strcasecmp(table_alias_charset, table_to->s->db.str,
                     get_udt_db_name().str) ||
       my_strcasecmp(table_alias_charset, table_to->s->udt_name.str,
                     get_udt_name().str))) {
    my_error(ER_WRONG_UDT_DATA_TYPE, MYF(0), table_to->s->db.str,
             table_to->s->udt_name.str, get_udt_db_name().str,
             get_udt_name().str);
    return true;
  }
  memcpy(table_to->record[0], const_cast<uchar *>(m_table->record[0]),
         m_table->s->reclength);
  return false;
}
...
type_conversion_status Item_func_udt_constructor::udt_table_store_to_field(
  ...
  for (uint i = 0; i < arg_count; i++) {
    if (sp_eval_expr(current_thd, true, to->val_udt()->field[i], &args[i])) {
      return TYPE_ERR_BAD_VALUE;
    }
  }
...
void Item_func_udt_constructor::update_used_tables() {
  Item_func::update_used_tables();
  set_ora_type();
}
...
TABLE *Item_func_udt_collection::val_udt() {
  ...
  for (uint i = m_has_assignment_list; i < arg_count;
    ...
    if (sp_eval_expr(current_thd, true, m_table->field[0], &item_index))
      return nullptr;

    if (m_nested_table_udt.length == 0) {
      // single type
      if (sp_eval_expr(current_thd, true, m_table->field[1], &args[i]))
        return nullptr;
    } else {
      // object
      ...
      if (args[i]->null_value) {
        ...
        for (uint j = 1; j < m_table->s->fields; j++) {
          ...
          if (sp_eval_expr(current_thd, true, m_table->field[j], &item_field))
            return nullptr;
        }
      }
    }
...
void Item_func_udt_collection::update_used_tables() {
  Item_func::update_used_tables();
  set_ora_table();
}
...
****************************************************************************/
Item_func_udt_table::Item_func_udt_table(
    const POS &pos, const LEX_STRING &db_name, const LEX_STRING &udt_name,
...
bool Item_func_udt_table::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (Item_func::do_itemize(pc, res)) return true;
...
****************************************************************************/
Item_func_udt_single_type_table::Item_func_udt_single_type_table(
    const POS &pos, const LEX_STRING &db_name, const LEX_STRING &udt_name,
    enum_udt_table_of_type table_type, ulonglong varray_limit,
    PT_item_list *item_list, List<Create_field> *field_def, ulong reclength,
...
bool Item_func_udt_single_type_table::do_itemize(Parse_context *pc,
                                                 Item **res) {
  if (skip_itemize(res)) return false;
  if (Item_func::do_itemize(pc, res)) return true;
...
bool Item_func_sp::do_itemize(Parse_context *pc, Item **res) {
  ...
  /*
    all of following conditions are matched, the with-func should be used.
    1. this sp function is shown up in with-func statement
    2. there is no database name in function name
    3. sp func name is the same as with-func name.
...
void Item_func_sp::cleanup() {
  ...
  m_return_field_arena.free_items();
  m_return_field_mem_root.ClearForReuse();
  Field_row_table *field_table =
      dynamic_cast<Field_row_table *>(sp_result_field);
  if (field_table) {
    // the elements in table_record->m_first_last_str_list not need to free_mem
    // again
    field_table->table_record->cleanup();
  }
  m_sp = nullptr;
  Item_func::cleanup();
}
...
static void my_missing_function_error(const LEX_STRING &token,
  ...
  if (token.length && is_lex_native_function(&token))
    ...
    my_error(ER_SP_DOES_NOT_EXIST, MYF(0), type_name, func_name);
}
...
bool Item_func_sp::init_result_field(THD *thd) {
  ...
  const Sp_udt_stmt_arena_holder sp_arena_holder(thd,
                                                 &m_return_field_tmp_arena);
  if (dynamic_cast<Field_refcursor *>(sp_result_field))
    m_is_refcursor = true;
  else if (dynamic_cast<Field_udt_type *>(sp_result_field))
    set_ora_type();
  else if (m_sp->m_return_field_def.ora_record.row_field_definitions()) {
    sp_result_field->m_callers_arena = &m_return_field_arena;
    if (row_create_items(
            thd, m_sp->m_return_field_def.ora_record.row_field_definitions()))
      return true;
  } else if (m_sp->m_return_field_def.ora_record
                 .row_field_table_definitions()) {
    sp_result_field->m_callers_arena = &m_return_field_arena;
    if (row_table_create_items(
            thd,
            m_sp->m_return_field_def.ora_record.row_field_table_definitions()))
      return true;
  }
  return sp_result_field == nullptr;
}

bool Item_func_sp::row_create_items(THD *thd, List<Create_field> *list) {
  assert(list);
  // Field_udt_type has field->m_udt_table by create_udt_table_from_routine.
  if (!sp_result_field->m_udt_table &&
      !(sp_result_field->m_udt_table = create_tmp_table_from_fields(
            thd, *list, true, 0, "sp_table",
            thd->sp_runtime_ctx ? thd->sp_runtime_ctx->callers_arena
                                : nullptr)))
    return true;

  sp_result_field->m_udt_table->copy_blobs = true;
  sp_result_field->m_udt_table->alias = "";
  arg_udt_count = list->elements;
  // init_var_items
  if ((args_udt = (Item **)thd->alloc(sizeof(Item *) * list->elements)) ==
      NULL) {
    arg_udt_count = 0;
    return true;
  }
  bool rec = row_create_items_from_cdf_list(
      thd, list, sp_result_field->m_udt_table, &arg_udt_count,
      const_cast<char *>(sp_result_field->field_name), args_udt);
  sp_result_field->set_arg_count(arg_udt_count);
  set_ora_type();
  sp_result_field->set_udt_value_initialized(true);
  m_rdl = list;
  return rec;
}

bool Item_func_sp::row_table_create_items(THD *thd,
  ...
  while ((def_list = it_table++)) {
    List<Create_field> *sd_list = dynamic_cast<List<Create_field> *>(def_list);
    arg_udt_count = sd_list->elements;
    if (field_table->create_tmp_record_table_to_store_bulk_data(
            thd, field_table->field_name, *sd_list, nullptr,
            field_table->get_index_length() > 0 ? true : false)) {
      sp_result_field->clear_table_all_field();
      return true;
    }

    if ((args_udt = (Item **)thd->alloc(sizeof(Item *) * sd->elements *
                                        arg_udt_count)) == NULL) {
      arg_udt_count = 0;
      sp_result_field->clear_table_all_field();
      return true;
    }
    if (row_create_items_from_cdf_list(
            thd, sd_list, field_table->table_record->table, &arg_udt_count,
            const_cast<char *>(field_table->field_name), args_udt)) {
      sp_result_field->clear_table_all_field();
      return true;
    }
  }
  sp_result_field->m_udt_table = field_table->table_record->table;
  sp_result_field->set_arg_count(arg_udt_count);
  set_ora_table();
  m_rdl_table = list;
  return false;
...
// used for sp.cc : sp_prepare_func_item when recall udt := function()
bool Item_func_sp::refix_fields() {
  assert(is_ora_udt_type());
  /* The item->args will do bind_fields when call
  sp_lex_instr::reset_lex_and_exec_core next time, so should delete it from
  m_return_field_tmp_arena */
  sp_result_field->clear_table_all_field();
  m_return_field_tmp_arena.free_items();
  m_return_field_tmp_mem_root.ClearForReuse();
  const Sp_udt_stmt_arena_holder sp_arena_holder(current_thd,
                                                 &m_return_field_tmp_arena);
  if (m_rdl) {
    if (row_create_items(current_thd, m_rdl)) return true;
  } else if (m_rdl_table) {
    if (row_table_create_items(current_thd, m_rdl_table)) return true;
  }
  return false;
}
...
bool Item_func_sp::sp_check_access(THD *thd) {
  ...
  char *routine_name = (sp_type == enum_sp_type::FUNCTION)
                           ? m_sp->m_name.str
                           : m_pkg_name->m_name.str;
  if (!m_sp->m_fake_synonym &&
      check_routine_access(thd, EXECUTE_ACL, m_sp->m_db.str, routine_name,
                           sp_type, false))
    return true;
...
bool Item_func_sp::fix_fields(THD *thd, Item **ref) {
  ...
  if (!thd->lex->is_view_context_analysis() ||
    ...
    char *routine_name = sp_type == enum_sp_type::FUNCTION
                             ? m_name->m_name.str
                             : m_pkg_name->m_name.str;
    bool res = !m_name->m_fake_synonym &&
               check_routine_access(thd, EXECUTE_ACL, m_name->m_db.str,
                                    routine_name, sp_type, false);
    thd->set_security_context(save_security_ctx);
  ...
  */
  ...
  uint define_count = sp_ctx->context_var_count();

  if ((thd->variables.sql_mode & MODE_ORACLE) && arg_count < define_count) {
    mem_root_deque<Item *> params(thd->mem_root);

    for (uint i = 0; i < arg_count; ++i) params.push_back(args[i]);

    if (append_default_parameters(thd, &params, m_sp, &arg_count))
      return true;
    else
      set_arguments(&params, true);
  }
  ...
  if ((thd->variables.sql_mode & MODE_ORACLE) && arg_count > 0) {
    ...
    if (has_named_parameter) {
      ...
      if (reorder_named_parameters(thd, "FUNCTION", args, arg_count, m_sp,
                                   &is_ordered))
        return true;

      // note: for function, not copy back from args operation is needed
    }
  }
...
bool Item_func_sp::resolve_udt_type(THD *thd) {
  if (is_ora_udt_type() && get_udt_name().length == 0 &&
      !sp_result_field->is_rowtype_table()) {
    reset_ora_type();
    reset_ora_table();
  }
...
void Item_func_sp::update_used_tables() {
  ...
  if (get_udt_name().length) set_ora_type();
...
void Item_func_sp::set_pkg_db(LEX_CSTRING &db_name) {
  if (m_name != nullptr) {
    m_name->m_pkg_db = db_name;
  }
}
...
longlong Item_func_can_access_routine::val_int() {
  ...
  char *type_charp = type_ptr->ptr();
  enum_sp_type sp_type;
  if (strcmp(type_charp, "PROCEDURE") == 0)
    sp_type = enum_sp_type::PROCEDURE;
  else if (strcmp(type_charp, "FUNCTION") == 0)
    sp_type = enum_sp_type::FUNCTION;
  else if (strcmp(type_charp, "PACKAGE") == 0)
    sp_type = enum_sp_type::PACKAGE_SPEC;
  else if (strcmp(type_charp, "PACKAGE BODY") == 0)
    sp_type = enum_sp_type::PACKAGE_BODY;
  else if (strcmp(type_charp, "TYPE") == 0)
...
static ulonglong get_table_statistics(
  ...
  *null_value = false;
  ...
  // function INTERNAL_TABLE_ROWS INTERNAL_AVG_ROW_LENGTH ..... is
  // IF(ISNULL(tbl.partition_type), tbl.engine, ''), engine_name_ptr restore
  // empty string
  if ((current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
      engine_name_ptr == nullptr) {
    engine_name_ptr = &engine_name;
    engine_name.set("", 0, system_charset_info);
  }
...
longlong Item_func_internal_index_column_cardinality::val_int() {
  ...
  // function INTERNAL_INDEX_COLUMN_CARDINALITY args[6] is
  // IF(ISNULL(tbl.partition_type), tbl.engine, ''), engine_name_ptr restore
  // empty string
  if ((current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
      engine_name_ptr == nullptr) {
    engine_name_ptr = make_empty_result();
  }
...
bool Item_func::ensure_multi_equality_fields_are_available_walker(uchar *arg) {
  ...
  for (uint i = 0; i < arg_count; ++i) {
    if (args[i]->type() == FIELD_ITEM) {
      args[i] = FindEqualField(down_cast<Item_field *>(args[i]),
                               reachable_tables, false, &found);
    }
  }
  return false;
}
...
bool Item_func_rownum::do_itemize(Parse_context *pc, Item **res) {
  if (skip_itemize(res)) return false;
  if (super::do_itemize(pc, res)) return true;
...
bool Item_func_rownum::fix_fields(THD *thd, Item **res) {
  ...
  if (query_block->having_fix_field) {
    ...
    if (find_item_in_list(thd, this, query_block->get_fields_list(),
...
bool Item_func_lnnvl::do_itemize(Parse_context *pc, Item **res) {
  if (!pc->select->has_lnnvl_func) pc->select->has_lnnvl_func = true;
  return Item_int_func::do_itemize(pc, res);
}
