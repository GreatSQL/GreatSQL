...
class UpdateRowsIterator final : public RowIterator {
 ...
 public:
  bool do_inserts(THD *thd, bool *trans_safe, bool *transactional_tables);
  bool merge_insert_into_tmp(THD *thd);
  void store_values(THD *thd, const mem_root_deque<Item *> &values);
  ha_rows deleted_rows() const { return m_deleted_rows; }

  TABLE *tmp_insert_table{nullptr};
  Temp_table_param *tmp_insert_table_param{nullptr};
  /// Number of rows found that should be inserted
  ha_rows m_insert_found_rows{0};
  ...
  /// Number of rows found that should be deleted after update
  ha_rows m_deleted_found_rows{0};
  /// Number of rows is deleted in do_updates().
  ...
  void set_merge_params(bool merge_when_insert_arg,
                        ...
                        MY_BITMAP *delete_write_set_arg, COPY_INFO *insert_arg,
