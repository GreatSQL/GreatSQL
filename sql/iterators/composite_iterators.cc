...
bool LimitOffsetIterator::Init() {
  ...
  if (m_with_ties) {
    if (m_join->order_fields.is_empty()) {
      assert(0);
      m_with_ties = false;
    }
  }
  ...
  if (percent_value > 0) {
    if (m_limit != HA_POS_ERROR) m_need_count_percent = true;
  } else {
    m_need_count_percent = false;
  }
...
int LimitOffsetIterator::Read() {
  if (m_need_count_percent) {
    ha_rows count_size = 0;
    int err = 0;
    if (m_needs_offset) {
    ...
    auto val = count_size * percent_value / 100;
    m_limit = static_cast<ha_rows>(val);
    if ((val - (double)m_limit) > 0) m_limit++;
    if (m_offset > 0) {
      m_limit += m_offset;
      m_seen_rows = m_limit;
    } else {
    ...
    /**
     *  if m_is_percent set m_source to MaterializeIterator ?
     */
    if (m_source->Init()) return 1;
    m_need_count_percent = false;
  }
  ...
  if (m_seen_rows >= m_limit) {
    ...
    if (m_seen_rows >= m_limit) {
      if (m_with_ties) {
        ...
        if (result == 0) {
          if (update_item_cache_if_changed(m_join->order_fields) == -1) {
            ++m_seen_rows;
            return result;
          } else {
            if (m_count_all_rows) {
  ...
  ++m_seen_rows;
  if (m_with_ties && m_seen_rows == m_limit) {
    update_item_cache_if_changed(m_join->order_fields);
  }
  ...
  while (true) {
    ...
    if (is_union_or_table) {
      ...
      if (t->is_except()) {
        ...
        //                       row's counter contains the number of
        //                       duplicates seen of that row.
      ...
      if (t->is_except()) {
        ...
        if (t->is_distinct()) {
          ...
          //
          if (check_unique_constraint(t)) {
            // row doesn't have a counter-part in left side, so we can ignore
            // it
            continue;
          }
