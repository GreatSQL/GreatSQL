...
bool TableScanIterator::Init() {
  ...
  */
  ...
  DBUG_EXECUTE_IF("oracmp_offset_fetch_init_failed", {
    if (!first_init) {
      PrintError(HA_ERR_NO_PARTITION_FOUND);
      return true;
    }
  });
...
int TableScanIterator::Read() {
  ...
  DBUG_EXECUTE_IF("oracmp_offset_fetch_read_failed",
                  { return HandleError(HA_ERR_RECORD_DELETED); });
