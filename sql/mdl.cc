...
PSI_stage_info MDL_key::m_namespace_to_wait_state_name[NAMESPACE_END] = {
    ...
    {0, "Waiting for stored package metadata lock", 0, PSI_DOCUMENT_ME},
    {0, "Waiting for stored type metadata lock", 0, PSI_DOCUMENT_ME},
...
void MDL_key::init_psi_keys() {
  ...
  assert(count == NAMESPACE_END);
  for (i = 0; i < count; i++) {
    assert(MDL_key::m_namespace_to_wait_state_name[i].m_name != nullptr);
