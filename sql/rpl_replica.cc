...
std::atomic<uint64_t> opt_rpl_read_binlog_speed_limit = 0;
...
int init_recovery(Master_info *mi) {
  ...
  */
  ...
  bool is_gtid_with_autopos_on = (global_gtid_mode.get() == Gtid_mode::ON) &&
                                 rli->mi && mi->is_auto_position();
    ...
    if (mi == channel_map.get_default_channel_mi()) {
      for (auto it : channel_map) {
        ...
        if (tmp_mi != nullptr && tmp_mi != mi) {
          ...
          // MERGE_PS8404_CHECK_TAG: @wenfengz, the second param "true" was deleted for compile OK
  ...
  if (ev) {
    ...
    */
    if ((!rli->is_parallel_exec() || rli->last_master_timestamp == 0) &&
      ...
      bool is_mgr_channel = is_group_replication_running() &&
                            Multisource_info::is_group_replication_channel_name(
                                rli->get_channel());
      // MGR change GTID common_header->when after flush to relay log
      // so use original_commit_timestamp
      if (is_mgr_channel && ev->get_type_code() == binary_log::GTID_LOG_EVENT) {
        Gtid_log_event *gtid =
            const_cast<Gtid_log_event *>(down_cast<const Gtid_log_event *>(ev));
        rli->last_master_timestamp = gtid->original_commit_timestamp / 1000000;
      }
  ...
  my_thread_init();
  {
    ...
    if (rli->channel_mts_submode != MTS_PARALLEL_TYPE_DB_NAME)
      rli->current_mts_submode =
          new Mts_submode_logical_clock(rli->current_mts_wait_mode);
    else
      rli->current_mts_submode = new Mts_submode_database();
    rli->m_next_worker_wait_last_committed = 0;

    // Only use replica preserve commit order if more than 1 worker exists
    // If replication is mgr, keep commit order when
    // replica_preserve_commit_order =on or =off.

    bool is_mgr_s =
        Multisource_info::is_group_replication_channel_name(rli->get_channel());
    if ((opt_replica_preserve_commit_order || is_mgr_s) &&
        !rli->is_parallel_exec() && rli->opt_replica_parallel_workers > 1)
      commit_order_mngr =
...
int heartbeat_queue_event(bool is_valid, Master_info *&mi,
  ...
  DBUG_SIGNAL_WAIT_FOR(current_thd, "wait_rotate", "write_rotate_event_puase",
                       "write_rotate_event_continue");
...
int connect_to_master(THD *thd, MYSQL *mysql, Master_info *mi, bool reconnect,
  ...
  if (is_io_thread) {
    mysql_options(mysql, MYSQL_OPT_CONNECT_TIMEOUT,
                  (char *)&replica_net_timeout);
  } else {
    // rpl_io_monitor thread
    uint monitor_net_timeout = 3;
    mysql_options(mysql, MYSQL_OPT_CONNECT_TIMEOUT,
                  (char *)&monitor_net_timeout);
  }
  ...
  */
  ...
  ulong retry_count = mi->retry_count;
  // rpl_io_monitor thread has its own outside retry loop
  if (!is_io_thread) {
    retry_count = 1;
  }
  while (!connected) {
        ...
        */
        if (set_mts_settings) {
          ...
          if (mts_parallel_option == MTS_PARALLEL_TYPE_DB_NAME) {
            ...
            mi->rli->channel_mts_submode = MTS_PARALLEL_TYPE_LOGICAL_CLOCK;
            mi->rli->current_mts_wait_mode = opt_replica_parallel_wait_mode;
          }
