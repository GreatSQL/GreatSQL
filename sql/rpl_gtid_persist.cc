...
int Gtid_table_persistor::save(const Gtid_set *gtid_set, bool compress) {
  ...
  /*
    When rotating binlog for a large trx,
    don't use current_thd to save gtids.
  */
  if (current_thd && current_thd->binlog_should_commit_by_rotate) {
    thd = nullptr;
    old_thd = current_thd;
  }
  ...
  if (old_thd) current_thd = old_thd;
