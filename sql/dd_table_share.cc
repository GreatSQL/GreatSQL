...
static bool prepare_share(THD *thd, TABLE_SHARE *share,
  ...
  if (share->udt_fields) {
    Field **ptr_udt = nullptr;
    uint k, *save_udt = nullptr;

    /* Store offsets to udt fields to find them fast */
    if (!(share->udt_field = save_udt = (uint *)share->mem_root.Alloc(
              (uint)(share->udt_fields * sizeof(uint)))))
      return true;  // OOM error message already reported
    for (k = 0, ptr_udt = share->field; *ptr_udt; ptr_udt++, k++) {
      if ((*ptr_udt)->udt_name().str) (*save_udt++) = k;
    }
  }
...
static bool fill_share_from_dd(THD *thd, TABLE_SHARE *share,
  ...
  // Udt name
  if (table_options.exists("udt_name")) {
    dd::String_type udt_name;
    table_options.get("udt_name", &udt_name);
    share->udt_name = to_lex_cstring(strmake_root(
...
static Field *make_field(const dd::Column &col_obj, const CHARSET_INFO *charset,
  ...
  bool is_udt_column = column_options.exists("udt_name") ? true : false;
  MYSQL_LEX_CSTRING ident = NULL_CSTR;
  if (is_udt_column) {
    column_options.get("udt_name", &ident, &share->mem_root);
  }
...
static bool fill_column_from_dd(THD *thd, TABLE_SHARE *share,
  ...
  // udt column
  if (column_options->exists("udt_name")) {
    TABLE *m_type_table;
    MYSQL_LEX_CSTRING ident = NULL_CSTR;
...
// see fill_dd_table_from_create_info
static bool fill_materialized_view_from_dd(TABLE_SHARE *share,
                                           const dd::Table *tab_obj) {
  share->has_mlog = tab_obj->has_mlog();

  share->is_mlog = tab_obj->is_mlog();

  // Save the materialized view flag bit
  if (tab_obj->is_materialized_view()) {
    auto info = tab_obj->get_mv_info();
    auto mview_info = new (&share->mem_root) materialized_view_info;
    if (mview_info == nullptr) return true;
    mview_info->build_clasue = info->build_clause();
    mview_info->flush_mode = info->flush_mode();
    mview_info->create_view_define.str =
        strmake_root(&share->mem_root, info->definition().c_str(),
                     info->definition().length());
    mview_info->create_view_define.length = info->definition().length();
    mview_info->create_view_query_block.str =
        strmake_root(&share->mem_root, info->definition_view().c_str(),
                     info->definition_view().length());
    mview_info->create_view_query_block.length =
        info->definition_view().length();

    mview_info->table_ref.str =
        strmake_root(&share->mem_root, info->table_ref().c_str(),
                     info->table_ref().length());
    mview_info->table_ref.length = info->table_ref().length();
    mview_info->derived_columns.str = strmake_root(
        &share->mem_root, info->columns().c_str(), info->columns().length());
    mview_info->derived_columns.length = info->columns().length();

    share->mview_info = mview_info;
  } else {
    share->mview_info = nullptr;
  }
  return false;
}
...
bool open_table_def(THD *thd, TABLE_SHARE *share, const dd::Table &table_def) {
  ...
  bool error = (fill_share_from_dd(thd, share, &table_def) ||
                ...
                fill_materialized_view_from_dd(share, &table_def) ||
...
bool create_cfield_from_dd_columns(THD *thd, const char *db,
                                   const dd::Routine *routine,
                                   List<Create_field> *field_def_lst,
                                   ulong *reclength) {
  dd::Table_impl *table = new dd::Table_impl();
  for (const dd::Parameter *param : routine->parameters()) {
    dd::Parameter *param_con = const_cast<dd::Parameter *>(param);
    dd::Parameter_impl *param_impl =
        dynamic_cast<dd::Parameter_impl *>(param_con);
    dd::Column_impl *col = new (std::nothrow) dd::Column_impl(*param_impl);
    table->columns()->push_back(col);
  }

  table->set_engine(default_storage_engine);
  table->set_collation_id(routine->schema_collation_id());
  dd::Properties *table_options = &table->options();
  table_options->set("pack_record", true);
  table_options->set("key_block_size", 0);
  TABLE_SHARE share;
  bool rc = true;
  init_tmp_table_share(thd, &share, db, 0, "", "", nullptr);
  if (fill_share_from_dd(thd, &share, table) ||
      fill_columns_from_dd(thd, &share, table))
    goto error;
  if (reclength != nullptr) *reclength = share.reclength;
  rc = share.export_structure(thd, field_def_lst);
