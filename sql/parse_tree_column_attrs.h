...
/**
  ...
  @ingroup ptn_column_types
*/
class PT_udt_type : public PT_char_type {
  Qualified_column_ident *m_udt_ident;

 public:
  PT_udt_type(const POS &pos, Char_type char_type,
              Qualified_column_ident *type_ident, const CHARSET_INFO *charset)
      : PT_char_type(pos, char_type, charset, false), m_udt_ident(type_ident) {}
...
class PT_row_type : public PT_type {
 ...
 public:
  ...
  // use for package's global define
  Ora_record_refer m_record;
  bool m_is_rowtype{false};
  bool m_is_package_define{false};

 public:
  PT_row_type(const POS &pos, Qualified_column_ident *type_ident,
              bool is_rowtype = false)
      : PT_type(pos, MYSQL_TYPE_NULL),
        m_udt_ident(type_ident),
        m_is_rowtype(is_rowtype) {}
  ...
  bool do_contextualize(Parse_context *pc) override {
    if (supper::do_contextualize(pc)) return true;

    List<Create_field> *field_def_list = nullptr;
    ulong reclength = 0;

    Row_definition_list *rdl = nullptr;
    Row_definition_table_list *rdtl = nullptr;
    bool ret = pc->thd->lex->sp_variable_declarations_add_udt_variable(
        pc->thd, to_lex_string(m_udt_ident->m_column),
        to_lex_string(m_udt_ident->db), &reclength, &rdl, &rdtl,
        &m_nested_table_udt_tmp, &m_table_type, &m_varray_limit,
        &field_def_list);
    if (ret || !reclength) {
      if (m_is_rowtype) {  // If it's %rowtype
        Table_ident *table_ref = nullptr;
        int offset = -1;
        sp_pcontext *ctx_cursor = nullptr;
        const Sp_rcontext_handler *rh_cursor = nullptr;
        int rc = pc->thd->lex->check_percent_rowtype(
            pc->thd, m_udt_ident, &table_ref, &m_row_def, &offset, &ctx_cursor,
            &rh_cursor);
        if (rc == -1) return true;
        if (offset == -1) {  // It's table%rowtype
          m_row_def = new (pc->thd->mem_root) Row_definition_list();
          m_record.set_row_field_definitions(m_row_def);
          m_record.set_record_table_rowtype_ref(table_ref);
          reclength = 1;
        } else {
          // TBD: it's cursor%rowtype
        }
      } else if (pc->thd->lex->sphead && pc->thd->lex->sphead->m_parent &&
                 pc->thd->lex->sphead->m_parent->get_package()) {
        // check if it's package's define
        m_is_package_define = true;
        sp_package *pkg = pc->thd->lex->sphead->m_parent->get_package();
        sp_variable *spvar =
            pkg->find_package_head_variable(m_udt_ident->m_column);
        if (!spvar) {
          my_error(ER_SP_DOES_NOT_EXIST, MYF(0), "TYPE",
                   m_udt_ident->m_column.str);
          return true;
        }
        if (!spvar->field_def.ora_record.is_record_define &&
            !spvar->field_def.ora_record.is_record_table_define) {
          my_error(ER_SP_MISMATCH_RECORD_VAR, MYF(0),
                   m_udt_ident->m_column.str);
          return true;
        }
        m_record.set_record(&spvar->field_def.ora_record);
        m_row_def = m_record.row_field_definitions();
        m_row_def_table = m_record.row_field_table_definitions();
        m_record.is_record_define = false;
        m_record.is_record_table_define = false;
        m_record.is_record_table_type_define = false;
        m_nested_table_udt_tmp = spvar->field_def.nested_table_udt;
        if (spvar->field_def.ora_record.row_field_table_definitions()) {
          // type tklist is table of xx
          m_table_type = (uint)enum_udt_table_of_type::TYPE_TABLE;
        }
        // It will be fixed later in sp_head::execute_procedure /
        // execute_function
        reclength = 1;
      } else {
        my_error(ER_SP_DOES_NOT_EXIST, MYF(0), "TYPE",
    ...
    if (m_is_package_define || m_is_rowtype) return false;
...
class PT_sys_refcursor_type : public PT_type {
 public:
  PT_sys_refcursor_type(const POS &pos) : PT_type(pos, MYSQL_TYPE_NULL) {}
...
class PT_field_def_base : public Parse_tree_node {
  ...
  */
  ...
  Qualified_column_ident *udt_ident;

 protected:
  PT_type *type_node;

  explicit PT_field_def_base(const POS &pos, PT_type *type_node)
      : super(pos), udt_ident(nullptr), type_node(type_node) {}
 ...
 public:
  bool do_contextualize(Parse_context *pc) override {
    ...
    udt_ident = type_node->get_udt_name();
