...
class IO_CACHE_binlog_cache_storage : public Truncatable_ostream {
  ...
  */
  ...
  inline IO_CACHE *get_io_cache() { return &m_io_cache; }

  void set_file_reserved_bytes(uint32 bytes) { m_file_reserved_bytes = bytes; }
  uint32 file_reserved_bytes() const { return m_file_reserved_bytes; }
...
class Binlog_cache_storage : public Basic_ostream {
  ...
  */
  bool truncate(my_off_t offset) {
    return m_pipeline_head->truncate(offset + m_file.file_reserved_bytes());
  }
  ...
  */
  my_off_t length() const {
    return m_file.length() - m_file.file_reserved_bytes();
  }
  /**
  ...
  */
  ...
  inline IO_CACHE *get_io_cache() { return m_file.get_io_cache(); }

  void set_file_reserved_bytes(uint32 bytes) {
    m_file.set_file_reserved_bytes(bytes);
  }
