  ...
  */
  ...
  for (Window &w : *windows) {
    ...
    if (w.m_partition_by == nullptr && w.get_keep_dir() != KEEP_DIR_NONE &&
      ...
      w.m_partition_by = new (thd->mem_root) PT_order_list(POS());
      for (ORDER *o = select->group_list.first; o != nullptr; o = o->next) {
        PT_order_expr *pt_order = new (thd->mem_root)
            PT_order_expr(POS(), o->item_initial, o->direction);
...
void Window::print(const THD *thd, String *str, enum_query_type qt,
  ...
  if (m_name != nullptr && !expand_definition) {
    ...
    if (m_order_by != nullptr && m_order_by_print) {
      str->append("ORDER BY ");
