...
void Field_row::create_udt_table(THD *thd, List<Create_field> *fld_list) {
  assert(!m_udt_table);
  m_udt_table = create_tmp_table_from_fields(thd, *fld_list, true, 0, nullptr,
                                             m_callers_arena);
}
...
bool Field_row_table::create_tmp_record_table_to_store_bulk_data(
    THD *thd, const char *alias, List<Create_field> vt_list, uint spv_offset,
    Item *forall_i, bool is_str_index) {
  table_record = new (thd->mem_root)
      Table_function_record(alias, vt_list, spv_offset, forall_i, is_str_index,
                            nullptr, m_callers_arena);
