...
static enum_osp_base_types field_types_osp_base_types[FIELDTYPE_NUM] = {
    ...
    // MYSQL_TYPE_NEWDATE      MYSQL_TYPE_VARCHAR
    OSP_DATE, OSP_STRING,
    // MYSQL_TYPE_BIT          MYSQL_TYPE_VECTOR
...
bool Field::sp_prepare_and_store_item(THD *thd, Item **value) {
  ...
  if (!(expr_item = sp_prepare_func_item(thd, true, value))) goto error;
  ...
  */
  ...
  if (likely(!thd->is_error())) DBUG_RETURN(false);
...
void Field_row::clear_table_all_field() {
  if (!m_udt_table) return;
  free_udt_tmp_table(m_udt_table);
  m_udt_table = nullptr;
}

bool Field_row::alloc_default_value_list(THD *thd, uint count) {
  if ((m_cdf_default_list = (Item **)thd->alloc(sizeof(Item *) * count)) ==
      NULL) {
    return true;
  }
  return false;
...
String *Field_row::val_str(String *str, String *) const {
  ...
  if (current_thd->variables.udt_format_result == UDT_FORMAT_RESULT_BINARY) {
    str->set(pointer_cast<const char *>(m_udt_table->record[0]),
             m_udt_table->s->reclength, field_charset);
  } else {
    return show_udt_values(m_udt_table, str);
  }
  return str;
}

void Field_row::make_send_field(Send_field *field) const {
  Field_str::make_send_field(field);
  field->type = MYSQL_TYPE_VARCHAR;
}

const CHARSET_INFO *Field_row::charset_for_protocol() const {
  if (current_thd->variables.udt_format_result == UDT_FORMAT_RESULT_BINARY)
...
void Field_row::set_null(ptrdiff_t row_offset) {
  /*
    In
    Item_field_row_table::create_item_for_store_default_value_and_insert_into_table
    defines tmp_field,it doesn't in table,so no need to set null or notnull.
  */
  if (!is_tmp_field) {
    Field::set_null(row_offset);
  }
  for (uint j = 0; j < m_udt_table->s->fields; j++) {
    Field_row_table *field_table =
        dynamic_cast<Field_row_table *>(m_udt_table->field[j]);
    if (!field_table) m_udt_table->field[j]->set_null(row_offset);
  }
}

void Field_row::set_notnull(ptrdiff_t row_offset) {
  /*
    In
    Item_field_row_table::create_item_for_store_default_value_and_insert_into_table
    defines tmp_field,it doesn't in table,so no need to set null or notnull.
...
void Field_row::sql_type(String &str) const {
  if (!m_rowtype_table_ident) {
    str.set(m_udt_name.str, m_udt_name.length, str.charset());
  } else {
    str.set(m_rowtype_table_ident->db.str, m_rowtype_table_ident->db.length,
            str.charset());
    str.append('.');
    str.append(m_rowtype_table_ident->table);
    str.append("%rowtype");
  }
}
...
void Field_row::create_udt_table(THD *thd, List<Create_field> *fld_list) {
  assert(!m_udt_table);
  m_udt_table = create_tmp_table_from_fields(thd, *fld_list, true, 0,
                                             field_name, m_callers_arena);
}

type_conversion_status Field_row::store_udt(TABLE *table,
                                            Item *item [[maybe_unused]]) {
  if (table->visible_field_count() != m_udt_table->visible_field_count() &&
      table->s->fields != m_udt_table->s->fields) {
    my_error(ER_UDT_INCONS_DATATYPES, myf(0));
    return TYPE_ERR_BAD_VALUE;
  }
  int off = table->s->fields - m_udt_table->s->fields;
  for (uint i = 0; i < m_udt_table->s->fields; i++) {
    Item *item_filed =
        new (current_thd->mem_root) Item_field(table->field[i + off]);
    if (item_filed == nullptr) return TYPE_ERR_BAD_VALUE;
    if (sp_eval_expr(current_thd, true, m_udt_table->field[i], &item_filed))
      return TYPE_ERR_BAD_VALUE;
  }
...
void Cursor_return_table::cleanup() {
  ...
  if (m_table->blob_storage) ::destroy_at(m_table->blob_storage);
...
bool Field_refcursor::set_cursor_return_type_from_other_refcursor(
    Item_field_refcursor *item_field) {
...
TABLE *Field_row_table::val_udt() { return table_record->table; }
...
void Field_row_table::clear_table() {
  ...
  if (tmp_table_record && tmp_table_record->table->is_created())
    tmp_table_record->init_table();
}

bool Field_row_table::create_tmp_record_table_to_store_bulk_data(
    THD *thd, const char *alias, List<Create_field> vt_list, Item *forall_i,
    bool is_str_index) {
  table_record = new (thd->mem_root)
      Table_function_record(alias, vt_list, nullptr, forall_i, is_str_index,
                            nullptr, m_callers_arena, nullptr);
  // is_virtual=false for it needs handler
  if (!table_record || table_record->create_result_table(thd, 0, alias) ||
      instantiate_tmp_table(thd, table_record->table))
    return true;
  tmp_table_record =
      new (thd->mem_root) Table_function_record(table_record, m_callers_arena);
  if (!tmp_table_record) return true;
  if (tmp_table_record->create_result_table(thd, 0, "")) return true;
  return instantiate_tmp_table(thd, tmp_table_record->table);
}

bool Field_row_table::fill_bulk_table_with_tmp_table() {
  return table_record->fill_result_table_with_table(tmp_table_record->table);
}

bool Field_row_table::resolve_forall_table_record(THD *thd, Item *forall_i) {
  const Sp_udt_stmt_arena_holder sp_arena_holder(thd, m_callers_arena);
  if (!forall_table_record) {
    forall_table_record = new (thd->mem_root)
        Table_function_record(table_record, m_callers_arena);
    if (!forall_table_record) return true;
    if (forall_table_record->create_result_table(thd, 0, "") ||
        instantiate_tmp_table(thd, forall_table_record->table))
      return true;
  }
  forall_table_record->m_forall_i = forall_i;
  if (forall_i->this_item()->null_value) return false;
  return forall_table_record->fill_forall_table_with_udt_table(thd,
                                                               table_record);
}
...
void Field_row_table::clear_table_all_field() {
  ...
  clear_table();
  free_udt_tmp_table(table_record->table);
  table_record->table = nullptr;
  m_udt_table = nullptr;
  if (!tmp_table_record || !tmp_table_record->table) return;
  free_udt_tmp_table(tmp_table_record->table);
  tmp_table_record->table = nullptr;
  if (!forall_table_record || !forall_table_record->table) return;
  free_udt_tmp_table(forall_table_record->table);
  forall_table_record->table = nullptr;
}

bool Field_row_table::fill_result_table_all_default_col(Item *index_item) {
  if (sp_eval_expr(current_thd, true, table_record->get_field(0), &index_item))
    return true;
  const Sp_udt_stmt_arena_holder sp_arena_holder(current_thd, m_callers_arena);
  for (uint offset = 1; offset < arg_count; offset++) {
    ...
    if (field_row) {
      ...
      */
      ...
      if (sp_eval_expr(current_thd, true, table_record->get_field(offset), &value))
        return true;
...
void Field_row_table::set_null(ptrdiff_t row_offset) {
  /*
    In
    Item_field_row_table::create_item_for_store_default_value_and_insert_into_table
    defines tmp_field,it doesn't in table,so no need to set null or notnull.
...
void Field_row_table::set_notnull(ptrdiff_t row_offset) {
  /*
    In
    Item_field_row_table::create_item_for_store_default_value_and_insert_into_table
    defines tmp_field,it doesn't in table,so no need to set null or notnull.
...
String *Field_row_table::val_str(String *str, String *) const {
  ...
  str = get_udt_table_val_str(str, table_record->table, m_udt_name,
                              m_nested_table_udt);
  return str;
}

type_conversion_status Field_row_table::store_udt(TABLE *table, Item *item) {
  if (table->visible_field_count() !=
          tmp_table_record->table->visible_field_count() &&
      table->s->fields != tmp_table_record->table->s->fields) {
    my_error(ER_UDT_INCONS_DATATYPES, myf(0));
    return TYPE_ERR_BAD_VALUE;
  }
  Item_field *item_field = dynamic_cast<Item_field *>(item->this_item());

  if (item_field != nullptr) {
    Field_row_table *fld = dynamic_cast<Field_row_table *>(item_field->field);
    if (fld == nullptr) return TYPE_ERR_BAD_VALUE;

    // this scope code is copyed from prior code
    // one test cant passed without this scope code
    // optimize in the future when better understaning and designing
    THD *thd = current_thd;
    const Sp_udt_stmt_arena_holder sp_arena_holder(thd, m_callers_arena);

    m_row_table_items.clear();
    List_iterator_fast<Item> it_table(fld->m_row_table_items);
    Item *it;
    for (uint i = 0; (it = it_table++); i++) {
      ...
      if (item_table_from_tmp) {
        ...
        if (sp_eval_expr(thd, true, field_table_tmp, &value))
          return TYPE_ERR_BAD_VALUE;
        ...
        if (item_row_tmp->row_create_items(
          ...
          return TYPE_ERR_BAD_VALUE;
        Item *value = item_tmp;
        if (sp_eval_expr(thd, true, field_tmp, &value)) return TYPE_ERR_BAD_VALUE;
        result = item_row_tmp;
      }
  ...
  if (table_record->fill_result_table_with_table(table)) {
    return TYPE_ERR_BAD_VALUE;
  }
...
void Field_row_table::set_udt_value_initialized(bool init) {
  is_udt_value_initialized = init;
  if (!table_record || !table_record->table) return;
  for (uint j = 0; j < table_record->table->s->fields; j++) {
    table_record->table->field[j]->set_udt_value_initialized(init);
  }
}
/****************************************************************************
...
bool Field_string::is_null(ptrdiff_t row_offset) const {
  if (is_null_in_table(row_offset)) return true;

  if ((current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
      is_empty_string(row_offset))
    return true;

  return false;
}

bool Field_string::is_empty_string(ptrdiff_t) const {
  if ((current_thd->variables.sql_mode & MODE_PAD_CHAR_TO_FULL_LENGTH))
    return false;

  return field_charset->cset->lengthsp(field_charset, (const char *)ptr,
                                       field_length) == 0;
}
...
bool Field_varstring::is_null(ptrdiff_t row_offset) const {
  if (is_null_in_table(row_offset)) return true;

  if ((current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
      data_length(row_offset) == 0)
    return true;
  return false;
}
...
bool Field_blob::is_null(ptrdiff_t row_offset) const {
  if (is_null_in_table(row_offset)) return true;
...
** packlength slot and may be from 1-4.
****************************************************************************/
Field_udt_type::Field_udt_type(uchar *ptr_arg, uint32 len_arg,
...
void Field_udt_type::clear_table_all_field() {
  if (!m_udt_table) return;
  free_blobs(m_udt_table);
}

type_conversion_status Field_udt_type::store(const Field *from) {
  Field *from_tmp = const_cast<Field *>(from);
  if (!from_tmp->val_udt()) {
    my_error(ER_UDT_INCONS_DATATYPES, myf(0));
    return TYPE_ERR_BAD_VALUE;
  }
  if (my_strcasecmp(system_charset_info, from_tmp->udt_name().str,
                    udt_name().str) == 0 &&
      my_strcasecmp(system_charset_info, m_udt_table->s->db.str,
                    from_tmp->table->s->db.str) == 0) {
    Field_udt_type *from_udt = dynamic_cast<Field_udt_type *>(from_tmp);
    return store((const char *)from_udt->data_ptr(), m_udt_table->s->reclength,
                 charset());
  } else {
    my_error(ER_TRUNCATED_WRONG_VALUE, myf(0), from_tmp->field_name,
             field_name);
    return TYPE_ERR_BAD_VALUE;
  }
}

type_conversion_status Field_udt_type::store_udt(TABLE *table,
                                                 Item *item [[maybe_unused]]) {
  if (m_udt_table != table) {
    int off = table->s->fields - m_udt_table->s->fields;
    for (uint i = 0; i < m_udt_table->s->fields; i++) {
      Item *item_filed =
          new (current_thd->mem_root) Item_field(table->field[i + off]);
      m_udt_table->field[i]->reset();
      if (sp_eval_expr(current_thd, true, m_udt_table->field[i], &item_filed))
        return TYPE_ERR_BAD_VALUE;
    }
  }

  return store((const char *)m_udt_table->record[0], m_udt_table->s->reclength,
               &my_charset_bin);
}

TABLE *Field_udt_type::val_udt() {
  if (m_udt_table == nullptr) return nullptr;
  copy_data_to_table();
  return m_udt_table;
}
...
String *Field_udt_type::val_str(String *, String *val_ptr) const {
  ...
  if (current_thd->variables.udt_format_result == UDT_FORMAT_RESULT_BINARY)
    ...
    return show_udt_values(m_udt_table, val_ptr);
  }
  return val_ptr;
}

void Field_udt_type::copy_data_to_table() const {
  memcpy(m_udt_table->record[0], const_cast<uchar *>(data_ptr()),
         m_udt_table->s->reclength);
}

bool Field_udt_type::update_field_of_table(THD *thd, uint idx, Item **value) {
  memcpy(m_udt_table->record[0], data_ptr(), (size_t)m_udt_table->s->reclength);
  if (sp_eval_expr(thd, true, m_udt_table->field[idx], value)) return true;
  return false;
}

bool Field_udt_type::create_udt_table_from_routine(THD *thd) {
  ulong length_table = dd::get_udt_size_and_supporting_table(
      thd, m_share_mem_root, m_udt_db_name, m_ident, &m_udt_table, field_name);
  if (length_table == 0) return true;
  return false;
}

void Field_udt_type::sql_type(String &res) const {
  res.set(m_ident.str, m_ident.length, res.charset());
}

const CHARSET_INFO *Field_udt_type::charset_for_protocol() const {
  if (current_thd->variables.udt_format_result == UDT_FORMAT_RESULT_BINARY)
    return &my_charset_bin;
  else
    return system_charset_info;
...
bool Field_udt_type::field_udt_table_store_to_table(TABLE *table_to) {
  if (!m_udt_table) return true;
  if (table_to->s->db.length && table_to->s->udt_name.length &&
    ...
    my_error(ER_WRONG_UDT_DATA_TYPE, MYF(0), table_to->s->db.str,
             table_to->s->udt_name.str, get_udt_db_name(), udt_name().str);
    return true;
  }

  if (is_null()) {
    my_error(ER_UDT_INSERT_NULL_OBJECT, MYF(0));
    return true;
  } else
    memcpy(table_to->record[0], const_cast<uchar *>(data_ptr()),
           m_udt_table->s->reclength);
  return false;
}
...
bool Field_set::is_null(ptrdiff_t row_offset) const {
  if (is_null_in_table(row_offset)) return true;

  if ((current_thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL) &&
      Field_enum::val_int() == 0)
    return true;

  return false;
}

bool Field_set::is_empty_string(ptrdiff_t) const {
  return Field_enum::val_int() == 0;
}
...
Field *make_field(
    MEM_ROOT *mem_root, TABLE_SHARE *share, uchar *ptr, size_t field_length,
    uchar *null_pos, uchar null_bit, enum_field_types field_type,
    const CHARSET_INFO *field_charset, Field::geometry_type geom_type,
    uchar auto_flags, TYPELIB *interval, const char *field_name,
    bool is_nullable, bool is_zerofill, bool is_unsigned, uint decimals,
    bool treat_bit_as_char, uint pack_length_override,
    std::optional<gis::srid_t> srid, bool is_array, bool is_rowtype_ref,
    bool is_row_table, LEX_CSTRING *udt_name, LEX_STRING db_name,
    int index_length, dd::Column::enum_hidden_type hidden, bool is_ref_cursor,
    Table_ident *table_rowtype_ref, LEX_CSTRING nested_table_udt) {
  ...
  if (is_rowtype_ref && field_type == MYSQL_TYPE_NULL && !is_row_table) {
    ...
    field_row->set_null_ptr(null_pos, null_bit);
    if (udt_name) field_row->set_udt_name(*udt_name);
    if (db_name.str)
      field_row->set_udt_db_name(
          strmake_root(mem_root, db_name.str, db_name.length));
    field_row->set_rowtype_table_ident(table_rowtype_ref);
    return field_row;
  }
  if (is_ref_cursor) {
    ...
    field_refcursor->set_null_ptr(null_pos, null_bit);
  ...
  if (is_row_table) {
    ...
    field_row_table->set_null_ptr(null_pos, null_bit);
    if (udt_name) field_row_table->set_udt_name(*udt_name);
    if (db_name.str)
      field_row_table->set_udt_db_name(
          strmake_root(mem_root, db_name.str, db_name.length));
    if (nested_table_udt.str)
      field_row_table->set_nested_table_udt(nested_table_udt);
...
Field *make_field(const Create_field &create_field, TABLE_SHARE *share,
  ...
  bool rowtype_ref = !create_field.ora_record.row_field_table_definitions() &&
                     !create_field.ora_record.is_ref_cursor &&
                     create_field.ora_record.is_row_type();
  ...
  return make_field(
      *THR_MALLOC, share, ptr, field_length, null_pos, null_bit,
      ...
      create_field.ora_record.row_field_table_definitions(), &cstr,
      create_field.udt_db_name, create_field.ora_record.index_length,
      create_field.hidden, create_field.ora_record.is_ref_cursor,
      create_field.ora_record.table_rowtype_ref(),
      create_field.nested_table_udt);
}
...
bool Field::set_warning(Sql_condition::enum_severity_level level, uint code,
  ...
  */
  ...
  if (current_warning_mask) {
    ...
    push_warning_printf(thd, level, code, ER_THD_NONCONST(thd, code), col_name,
                        thd->get_stmt_da()->current_row_for_condition());
  }
...
String *get_udt_table_val_str(String *str, TABLE *table, LEX_STRING m_udt_name,
  ...
  uint is_cmp_udt_table = current_thd->lex->is_cmp_udt_table;
  bool empty = true;
  do {
    ...
    if (res == 0) {
      ...
      if (m_nested_table_udt.length == 0) {
        for (uint i = 1 - is_cmp_udt_table; i < table->s->fields; i++) {
...
void free_udt_tmp_table(TABLE *table) {
  if (table == nullptr) return;
  for (uint i = 0; i < table->s->fields; i++) {
    if (table->field[i]->udt_name().length != 0) {
      table->field[i]->~Field();
    }
  }
  free_blobs(table);
