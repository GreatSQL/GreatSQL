...
bool Query_block::prepare(THD *thd, mem_root_deque<Item *> *insert_field_list) {
  ...
  */
  ...
  if (has_joined_item) {
    for (Table_ref *table : m_table_nest) {
  ...
  */
  ...
  if (has_joined_item) {
    Outerjoin_transformer oj_transformer(this);
    if (oj_transformer.setup_outer_join(thd, m_where_cond)) {
      return true;
    }
  }
...
/*
  Push limit if possible to all the materialized derived tables.

  @param thd      thread handler

  @returns false if success, true if error

  call this func while apply local transforms,
*/
bool Query_block::push_limit_to_derived_tables() {
  if (materialized_derived_table_count > 0) {
    for (Table_ref *tl = leaf_tables; tl != nullptr; tl = tl->next_leaf) {
      // select from a single subquery
      if (tl->next_leaf != nullptr) return false;

      /*
        push limit down except limit percent
        At now, limit percent retrieves the row count by execute sub iterator,
        and may perform better if there has a materialization
      */
      if (tl->is_view_or_derived() && tl->uses_materialization() &&
          ((has_limit() || select_limit_fetch) && !select_limit_percent) &&
          tl->can_push_limit_to_derived()) {
        auto *qb = tl->derived_query_expression()->query_term()->query_block();
        assert(qb->select_limit == nullptr);
        assert(qb->offset_limit == nullptr);

        // move limit to child
        qb->select_limit = select_limit;
        qb->offset_limit = offset_limit;
        qb->select_limit_percent = select_limit_percent;
        qb->select_limit_percent_value = select_limit_percent_value;
        qb->select_limit_ties = select_limit_ties;
        qb->select_limit_fetch = select_limit_fetch;
        select_limit = nullptr;
        offset_limit = nullptr;
        select_limit_percent = false;
        select_limit_percent_value = 0.0;
        select_limit_ties = false;
        select_limit_fetch = false;
      }
    }
  }
...
bool Query_block::apply_local_transforms(THD *thd, bool prune) {
  ...
  */
  ...
  if (thd->optimizer_switch_flag(OPTIMIZER_SWITCH_REMOVE_USELESS_OUTERJOIN)) {
    ...
    if (need_to_remap) {
      ...
      need_to_remap = false;
      /**
       * Update all items occurred in this query_block, such as
       * where_cond, join_cond, etc.
       */
      update_used_tables();
    }
  }
  ...
  */
  ...
  if (thd->optimizer_switch_flag(OPTIMIZER_SWITCH_DERIVED_LIMIT_PUSHDOWN) &&
      push_limit_to_derived_tables())
    return true;
...
bool Query_block::resolve_limits(THD *thd) {
  if (offset_limit != nullptr) {
    ...
    // oracle support like '1+1' const expr
    if (!offset_limit->const_for_execution()) {
      my_error(ER_INVALID_FETCH_LIMIT_EXPR, MYF(0));
      return true;
    }
  }
  ...
  if (select_limit != nullptr) {
    ...
    // oracle support like '1+1' const expr
    if (!select_limit->const_for_execution()) {
      my_error(ER_INVALID_FETCH_LIMIT_EXPR, MYF(0));
      return true;
    }
  }

  // not need ties if order not set
  if (order_list.elements == 0 && select_limit_ties) {
    select_limit_ties = false;
  }
  return false;
}
  ...
  for (Table_ref *table : *join_list) {
    ...
    if (nested_join != nullptr) {
      ...
      */
      ...
      if (*cond != nullptr && (!table->foj_inner && !table->foj_outer))
    ...
    if ((!table->outer_join || (used_tables & not_null_tables)) &&
      ...
      */
      ...
      if (table->join_cond() != nullptr && !table->foj_nest) {
  ...
  */
  ...
  if (derived_query_block->is_ordered()) {
    ...
    */
    ...
    bool is_insert_select = (lex->sql_command == SQLCOM_INSERT_ALL_SELECT ||
                             lex->sql_command == SQLCOM_INSERT_SELECT)
                                ? true
                                : false;
    if ((lex->sql_command == SQLCOM_SELECT ||
         lex->sql_command == SQLCOM_UPDATE ||
         (is_insert_select && has_limit()) ||
...
bool find_order_in_list(THD *thd, Ref_item_array ref_item_array,
  ...
  uint counter = 0;
  enum_resolution_type resolution(NOT_RESOLVED);
...
class AccessOrFilter {
 ...
 public:
  ...
  bool isFilter(Item *it) {
    ...
    if (it->type() == Item::SUBQUERY_ITEM) return true;
  ...
  bool transformItem_in(THD *thd, Item_func_in *it, Item **join_cond,
    ...
    if (argsused != 0) {
      ...
      PT_item_list pt_item_where{POS()};
      PT_item_list pt_item_join{POS()};
      for (uint i = 1; i < it->argument_count(); i++) {
