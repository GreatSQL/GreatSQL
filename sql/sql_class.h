...
struct Query_dumpvar_param_rela {
  struct Field_attr {
    std::string db_name;
  ...
  bool is_multi_table;
  bool is_updating;
  bool is_rownum_or_limit;
  std::string query_expr_cond;
  bool is_query_dumpvar_param_rela_null;
  std::string origin_lob_content;
  bool check_result_flag;
  // check table pk:
  bool table_miss_pk;
  bool table_pk_type_invalid;
  bool where_cond_miss_pk;

  Query_dumpvar_param_rela();
};

class Sp_rcontext_layer : public std::unary_function<Sp_rcontext_layer, bool> {
 public:
  enum enum_sp_rcontext_type { FUNCTION_RUNTIME_CTX, SP_RUNTIME_CTX };
  struct Sp_variable_level
      : public std::unary_function<Sp_variable_level, bool> {
    std::string current_var_name;
    std::string parent_var_name;

    Sp_variable_level(std::string current_var_name,
                      std::string parent_var_name = "") {
      this->current_var_name = current_var_name;
      this->parent_var_name = parent_var_name;
    }

    bool operator()(const Sp_variable_level &vl) const {
      return !this->current_var_name.compare(vl.current_var_name);
    }
  };

  struct Item_expr_holder {
    enum Item_type { INVALID, INTEGER_TYPE, STRING_TYPE };
    static constexpr const char *COND_AND = " AND ";
    static constexpr const char *COND_WHERE = " WHERE ";
    static constexpr const char *COND_EQ = " = ";
    ...
    static Sp_rcontext_layer::Item_expr_holder::Item_type get_item_type(
 ...
 private:
  ...
  std::deque<Sp_variable_level> sp_variable_list;
  std::map<std::string, Query_dumpvar_param_rela> query_dumpvar_map;

 public:
  Sp_rcontext_layer(sp_rcontext *rctx, enum_sp_rcontext_type sp_rcontext_type =
                                           Sp_rcontext_layer::SP_RUNTIME_CTX);

  bool find_sp_rcontext(sp_rcontext *rctx) const;

  std::deque<Sp_variable_level> *get_sp_variable_list() {
    return &sp_variable_list;
  }

  std::map<std::string, Query_dumpvar_param_rela> *get_query_dumpvar_map() {
    return &query_dumpvar_map;
  }

  sp_rcontext *get_sp_rcontext() const { return rctx; }

  static bool filter_rcontext(THD *thd);
  static Query_dumpvar_param_rela *match_variable(THD *thd,
                                                  const std::string field_name);
  static void push_param(THD *thd, std::string var_key,
                         Query_dumpvar_param_rela param);
  static Query_dumpvar_param_rela *find_ref_actual_variable(
      THD *thd, const std::string var_name);

  static std::string locate_rec(THD *thd, Table_ref *table_list,
                                Item *current_where,
                                Query_dumpvar_param_rela *param);
  static std::string gen_field_expr(const std::string field_name,
                                    std::map<std::string, Item *> *cond_item);

  friend bool find_pk_item_type(
      const Sp_rcontext_layer::Item_expr_holder &holder);
...
class THD : public MDL_context_owner,
  ...
  */
  ...
  /**
    The function checks whether the thread is a slave thd, but not mgr slave
    thd.

    @return true  when the thread is a normal slave
  */
  bool is_normal_slave() const;
  bool is_mgr_slave() const;
 ...
 public:
  ...
  void set_binlog_table_maps(int i) { binlog_table_maps = i; }
 ...
 public:
  ...
  void set_greatdb_plan_baseline_info(Query_expression *unit);
  ...
  */
  ...
  /* Whether rename to binlog should be executed on the current cache_data. */
  bool binlog_should_commit_by_rotate;
  ...
  */
  ...
  sp_cache *sp_ora_type_cache;

  // sp_rcontext layer use in dbms_lob
  std::deque<Sp_rcontext_layer> sp_rcontext_layers;
  ...
  */
  ...
  /// Sets Diagnostics Area for parsing.
  void set_parser_da(Diagnostics_area *da) { m_parser_da = da; }
  ...
  */
  ...
  bool is_parallel_load_executor() {
    return variables.gdb_parallel_load_executor;
  }
  ...
  */
  ...
  Diagnostics_area parser_da; /**< cf. get_parser_da() */
 ...
 public:
  bool is_skip_audit_srv() { return skip_audit_srv; }
  void set_skip_audit_srv(bool b) { skip_audit_srv = b; }
  bool is_system_user();
 ...
 public:
  ...
  // set
  void set_query_plan_plugin_interface(ST_QUERY_PLAN *st_interface) {
    mysql_mutex_lock(&LOCK_thd_query_plan_plugin);
    query_plan_plugin_interface = st_interface;
    mysql_mutex_unlock(&LOCK_thd_query_plan_plugin);
  }
 ...
 public:
  ...
  /// Internal call to execute_direct has no result
  bool is_internal_con_no_result;
};
/**
