...
class PT_order_list : public Parse_tree_node {
 ...
 public:
  ...
  PT_order_list *pq_clone(THD *thd, Query_block *select);
};
...
class PT_table_record_function : public PT_table_reference {
  typedef PT_table_reference super;

 public:
  PT_table_record_function(const POS &pos, const LEX_STRING &table_alias,
                           Item *forall_i, sp_variable *spv_i)
      : super(pos),
        m_table_alias(table_alias),
        m_record_index_i(forall_i),
        m_spv_i(spv_i) {}

  bool do_contextualize(Parse_context *pc) override;
  PT_table_reference_type type() const override { return PT_RECORD; }
  Item *get_forall_i() { return m_record_index_i; }

 private:
  LEX_STRING m_table_alias;
  Item *m_record_index_i;
  sp_variable *m_spv_i;
};

class PT_table_udt_function : public PT_table_reference {
  typedef PT_table_reference super;

 public:
  PT_table_udt_function(const POS &pos, Item *expr,
...
class PT_connect_by : public Parse_tree_node {
 ...
 public:
  ...
  PT_connect_by(const POS &pos, Item *start_arg, Item *expr_arg, int nocycle)
      : super(pos), m_start(start_arg), m_expr(expr_arg), m_nocycle(nocycle) {}
...
class PT_column_locking_clause : public PT_locking_clause {
 public:
  ...
  PT_column_locking_clause(const POS &pos, Lock_strength strength,
                           Mem_root_array_YY<Qualified_column_ident *> columns,
                           Locked_row_action_ora *action)
      : PT_locking_clause(pos, strength, action->locked_row_action),
        m_columns(columns),
        m_action(action) {}
...
class PT_query_expression_body : public Parse_tree_node {
  ...
  */
  ...
  virtual PT_qe_body_type get_qe_body_type() const = 0;
};
...
class PT_set_variable : public PT_option_value_no_option_type {
 ...
 public:
  PT_set_variable(const POS &pos, const POS &varpos,
                  ...
                  List<ora_simple_ident_def> *def_list, const POS &expr_pos,
                  Item *opt_expr, bool ora_set_var = false)
      : super{pos},
        m_varpos{varpos},
        m_opt_prefix{opt_prefix},
        m_name{name},
...
class PT_into_destination : public Parse_tree_node {
 ...
 protected:
  explicit PT_into_destination(const POS &pos)
      : super(pos), is_bulk_into(false) {}
...
class PT_select_var : public Parse_tree_node {
 public:
  ...
  virtual sp_rcontext *get_rh(sp_rcontext *) const { return nullptr; }
  virtual bool var_is_rowtype() { return false; }
...
class PT_select_sp_var : public PT_select_var {
 ...
 protected:
  Ora_cursor_rowtype m_select_into_spv_info;
  bool is_rowtype;
  bool is_row_table;
 ...
 public:
  PT_select_sp_var(const POS &pos, const LEX_STRING &name_arg)
      : super(pos, name_arg), m_select_into_spv_info({-1, nullptr, nullptr}) {}

  bool is_local() const override { return true; }
  uint get_offset() const override {
    return m_select_into_spv_info.m_cursor_rowtype_offset;
  }
  sp_rcontext *get_rh(sp_rcontext *ctx) const override {
    sp_rcontext *result = m_select_into_spv_info.m_rh_cursor->get_rcontext(ctx);
  ...
  bool var_is_rowtype() override { return is_rowtype; }
...
class PT_select_sp_var_field : public PT_select_var {
  typedef PT_select_var super;

  Bipartite_name_ora *ora_var_ident{nullptr};
  ora_simple_ident_def spvar_name_def{NULL_STR, nullptr};
  ora_simple_ident_def field_name_def{NULL_STR, nullptr};
  List<ora_simple_ident_def> m_full_ident_list;
  int m_spv_offset;
  const Sp_rcontext_handler *m_rh_cursor;

 public:
  PT_select_sp_var_field(const POS &pos, const LEX_STRING &var_name_arg,
                         Bipartite_name_ora *ident)
      : super(pos, var_name_arg), ora_var_ident(ident) {}

  bool do_contextualize(Parse_context *pc) override;
  Bipartite_name_ora *get_var_ident() const { return ora_var_ident; }
  List<ora_simple_ident_def> get_full_ident_list() const {
    return m_full_ident_list;
  }
  List<ora_simple_ident_def> *get_full_ident_list_ptr() {
    return &m_full_ident_list;
  }
  PT_select_sp_var_field *get_my_var_sp() override { return this; }
  bool is_local() const override { return true; }
  uint get_offset() const override { return m_spv_offset; }
  sp_rcontext *get_rh(sp_rcontext *ctx) const override {
    sp_rcontext *result = m_rh_cursor->get_rcontext(ctx);
    if (!result) result = ctx;
    return result;
  }
};
...
class PT_query_specification : public PT_query_primary {
 ...
 public:
  PT_query_specification(
      ...
      PT_window_list *opt_window_clause_arg, Item *opt_qualify_clause_arg,
  ...
  PT_query_specification(
      ...
      : PT_query_specification(pos, options_arg, item_list_arg, from_clause_arg,
  ...
  PT_qe_body_type get_qe_body_type() const override { return PT_QUERY_SPEC; }
...
class PT_table_value_constructor : public PT_query_primary {
 ...
 public:
  ...
  PT_qe_body_type get_qe_body_type() const override { return PT_TABLE_VALUE; }
};
...
class PT_query_expression final : public PT_query_expression_body {
 public:
  ...
  PT_qe_body_type get_qe_body_type() const override {
    return PT_QUERY_EXPRESSION;
  }
...
class PT_locking final : public PT_query_expression_body {
 ...
 public:
  ...
  PT_qe_body_type get_qe_body_type() const override { return PT_LOCKING; }
...
class PT_subquery : public Parse_tree_node {
 ...
 public:
  ...
  PT_query_expression_body *query_primary() { return qe; }
};
...
class PT_set_operation : public PT_query_expression_body {
 ...
 public:
  ...
  PT_qe_body_type get_qe_body_type() const override { return PT_SET_OP; }
...
/*
 * MERGE_PS8404_CHECK_TAG: why not derived from  Parse_tree_root?
 */
class PT_merge_when_list final {
 public:
  PT_merge_when_list(PT_item_list *column_list_arg,
                     PT_item_list *value_list_arg, Item *where_arg,
                     Item *delete_arg)
      : update_column_list(column_list_arg),
        update_value_list(value_list_arg),
        update_where(where_arg),
        update_delete(delete_arg),
        insert_column_list(nullptr),
        insert_row_value_list(nullptr),
        insert_where(nullptr) {}

  PT_merge_when_list(PT_item_list *column_list_arg,
                     PT_insert_values_list *row_value_list_arg, Item *where_arg)
      : update_column_list(nullptr),
        update_value_list(nullptr),
        update_where(nullptr),
        update_delete(nullptr),
        insert_column_list(column_list_arg),
        insert_row_value_list(row_value_list_arg),
        insert_where(where_arg) {}

  PT_item_list *update_column_list;
  PT_item_list *update_value_list;
  Item *update_where;
  Item *update_delete;
  PT_item_list *insert_column_list;
  PT_insert_values_list *insert_row_value_list;
  Item *insert_where;
};
...
class PT_update : public Parse_tree_root {
  ...
  PT_item_list *opt_column_list{nullptr};
  PT_insert_values_list *opt_row_value_list{nullptr};
 ...
 public:
  PT_update(const POS &pos, PT_with_clause *with_clause_arg,
      ...
      : super(pos),
        ...
        opt_column_list(nullptr),
        opt_row_value_list(nullptr),
        opt_merge_update_where(nullptr),
        opt_merge_insert_where(nullptr) {}

  void set_merge_params(PT_item_list *column_list_arg,
                        PT_insert_values_list *row_value_list_arg,
                        Item *merge_update_where_arg,
                        Item *merge_insert_where_arg,
                        Item *merge_update_delete_arg) {
    opt_column_list = column_list_arg;
    opt_row_value_list = row_value_list_arg;
    opt_merge_update_where = merge_update_where_arg;
    opt_merge_insert_where = merge_insert_where_arg;
    opt_merge_update_delete = merge_update_delete_arg;
 ...
 public:
  PT_insert(const POS &pos, bool is_replace_arg, PT_hint_list *opt_hints_arg,
      ...
      : super(pos),
        ...
        is_forall_insert(m_is_forall_insert) {
...
class PT_insert_all_into : public Parse_tree_node {
 ...
 public:
  ...
  PT_insert_all_into(const POS &pos,
                     Table_ident *table_ident_arg,
                     List<String> *opt_use_partition_arg,
                     PT_item_list *column_list_arg,
...
class PT_insert_all : public Parse_tree_root {
 ...
 public:
  PT_insert_all(const POS &pos, PT_hint_list *opt_hints_arg,
...
class PT_execute final : public Parse_tree_root {
 ...
 public:
  PT_execute(const POS &pos, Item *query_arg, PT_into_destination *into_arg,
             int mode_arg, PT_item_list *param_arg)
      : Parse_tree_root(pos),
        m_query(query_arg),
        m_into(into_arg),
        mode(mode_arg),
...
class PT_create_external_dir_file_name : public PT_create_table_option {
 ...
 public:
  explicit PT_create_external_dir_file_name(const POS &pos, LEX_STRING &dir,
                                            LEX_STRING &file)
      : super(pos), dir_name(dir), file_name(file) {}
...
class PT_create_synonym_stmt final : public Parse_tree_root {
 ...
 public:
  PT_create_synonym_stmt(const POS &pos, Table_ident *synonym_ident,
                         bool or_replace, bool is_public,
                         Table_ident *target_ident)
      : Parse_tree_root(pos),
...
class PT_drop_synonym_stmt final : public Parse_tree_root {
 ...
 public:
  PT_drop_synonym_stmt(const POS &pos, Table_ident *synonym_ident,
                       bool is_public)
      : Parse_tree_root(pos),
        m_synonym_ident(synonym_ident),
        m_is_public(is_public) {}
...
class PT_alter_synonym_stmt final : public Parse_tree_root {
 ...
 public:
  PT_alter_synonym_stmt(const POS &pos, Table_ident *synonym_ident,
                        bool is_public)
      : Parse_tree_root(pos),
        m_synonym_ident(synonym_ident),
        m_is_public(is_public) {}
...
class PT_create_table_stmt final : public PT_table_ddl_stmt_base {
  ...
  */
  PT_create_table_stmt(
      ...
      bool is_temporary, bool only_if_not_exists, Table_ident *table_name,
      Table_ident *opt_like_clause, Qualified_column_ident *udt_ident = nullptr,
      const Mem_root_array<PT_create_table_option *> *opt_create_table_options =
          nullptr)
      : PT_table_ddl_stmt_base(pos, mem_root),
        m_opt_hints(opt_hints),
...
/**
   Top-level node for the CREATE %mlog statement
*/

class PT_create_mlog_stmt final : public PT_table_ddl_stmt_base {
  bool only_if_not_exists;
  Table_ident *table_name;
 ...
 public:
  static Table_ident *make_mlog_name(THD *thd, Table_ident *table_name);

  PT_create_mlog_stmt(const POS &pos, MEM_ROOT *mem_root, bool not_exists,
                      Table_ident *table_name)
      : PT_table_ddl_stmt_base(pos, mem_root),
        only_if_not_exists(not_exists),
        table_name(table_name) {}
  Sql_cmd *make_cmd(THD *thd) override;
};

class PT_create_materialized_view_stmt final : public PT_table_ddl_stmt_base {
  bool only_if_not_exists;
  Table_ident *table_name;

  Materialized_view_opt *mview_opt;
  HA_CREATE_INFO m_create_info;

 public:
  PT_create_materialized_view_stmt(const POS &pos, MEM_ROOT *mem_root,
                                   bool not_exists, Table_ident *table_name,
                                   Materialized_view_opt *opt)
      : PT_table_ddl_stmt_base(pos, mem_root),
        only_if_not_exists(not_exists),
        table_name(table_name),
        mview_opt(opt) {}
...
class PT_alter_materialized_view final : public PT_table_ddl_stmt_base {
  Table_ident *mv_ident;
  int build_clause;
  HA_CREATE_INFO m_create_info;

 public:
  PT_alter_materialized_view(const POS &pos, MEM_ROOT *mem_root,
                             Table_ident *mv_ident, int build_clause_arg)
      : PT_table_ddl_stmt_base(pos, mem_root),
        mv_ident(mv_ident),
        build_clause(build_clause_arg) {}
...
class PT_create_sequence_stmt final : public Parse_tree_root {
 ...
 public:
  PT_create_sequence_stmt(const POS &pos, bool if_not_exists,
                          Table_ident *seq_ident)
      : Parse_tree_root(pos),
...
class PT_drop_sequence_stmt final : public Parse_tree_root {
 ...
 public:
  PT_drop_sequence_stmt(const POS &pos, Table_ident *seq_ident)
      : Parse_tree_root(pos), seq_ident(seq_ident) {}
  Sql_cmd *make_cmd(THD *thd) override;
};
...
class PT_alter_sequence_stmt final : public Parse_tree_root {
 ...
 public:
  PT_alter_sequence_stmt(const POS &pos, Table_ident *seq_ident)
      : Parse_tree_root(pos), seq_ident(seq_ident) {}
...
/// Parse tree node for SHOW CREATE PACKAGE statement

class PT_show_create_package final : public PT_show_base {
 public:
  PT_show_create_package(const POS &pos, sp_name *package_name)
      : PT_show_base(pos, SQLCOM_SHOW_CREATE_PACKAGE), m_spname(package_name) {}
 ...
 private:
  ...
  Sql_cmd_show_create_package m_sql_cmd;
};

/// Parse tree node for SHOW CREATE PACKAGE BODY statement

class PT_show_create_package_body final : public PT_show_base {
 public:
  PT_show_create_package_body(const POS &pos, sp_name *package_name)
      : PT_show_base(pos, SQLCOM_SHOW_CREATE_PACKAGE_BODY),
        m_spname(package_name) {}
...
class PT_show_create_materialized_view final : public PT_show_base {
 public:
  PT_show_create_materialized_view(const POS &pos, Table_ident *table_ident)
 ...
 private:
  Sql_cmd_show_create_materialized_view m_sql_cmd;
};

/// Parse tree node for SHOW CREATE TYPE statement

class PT_show_create_type final : public PT_show_base {
 public:
  PT_show_create_type(const POS &pos, sp_name *type_name)
      : PT_show_base(pos, SQLCOM_SHOW_CREATE_TYPE), m_spname(type_name) {}
...
/// Parse tree node for SHOW PACKAGE BODY CODE statement.

class PT_show_package_body_code final : public PT_show_routine_code {
 public:
  PT_show_package_body_code(const POS &pos, const sp_name *package_name)
      : PT_show_routine_code(pos, SQLCOM_SHOW_PACKAGE_BODY_CODE, package_name) {
  }
};
...
/// Parse tree node for SHOW STATUS PACKAGE statement

class PT_show_status_package final : public PT_show_filter_base {
 public:
  PT_show_status_package(const POS &pos, const LEX_STRING &wild, Item *where)
      : PT_show_filter_base(pos, SQLCOM_SHOW_STATUS_PACKAGE, wild, where) {}

  Sql_cmd *make_cmd(THD *thd) override;

 private:
  Sql_cmd_show_status_package m_sql_cmd;
};

/// Parse tree node for SHOW STATUS PACKAGE BODY statement

class PT_show_status_package_body final : public PT_show_filter_base {
 public:
  PT_show_status_package_body(const POS &pos, const LEX_STRING &wild,
                              Item *where)
      : PT_show_filter_base(pos, SQLCOM_SHOW_STATUS_PACKAGE_BODY, wild, where) {
  }

  Sql_cmd *make_cmd(THD *thd) override;

 private:
  Sql_cmd_show_status_package_body m_sql_cmd;
};

/// Parse tree node for SHOW STATUS TYPE statement

class PT_show_status_type final : public PT_show_filter_base {
 public:
  PT_show_status_type(const POS &pos, const LEX_STRING &wild, Item *where)
      : PT_show_filter_base(pos, SQLCOM_SHOW_STATUS_TYPE, wild, where) {}

  Sql_cmd *make_cmd(THD *thd) override;

 private:
  Sql_cmd_show_status_type m_sql_cmd;
};
...
class PT_reload_sequences : public Parse_tree_root {
 public:
  typedef Parse_tree_root super;

  explicit PT_reload_sequences(const POS &pos,
                               Mem_root_array<Table_ident *> *table_list)
      : m_pos(pos), m_seqs(table_list) {}

  Sql_cmd *make_cmd(THD *thd) override;

 private:
  POS m_pos;
  const Mem_root_array<Table_ident *> *m_seqs;
};
