...
bool init_binlog_cache_dir() {
  /* If opt_bin_log is false, do nothing. */
  if (!opt_bin_log) {
    return false;
  }
  ...
  /* If opt_bin_log is true, log_bin_basename is valid and usable. */
...
class MYSQL_BIN_LOG::Binlog_ofile : public Basic_ostream {
  ...
  */
  ...
  Truncatable_ostream *get_pipeline_head_ptr() { return m_pipeline_head.get(); }
  /**
...
bool binlog_rotated_by_large_trx() {
  return binlog_commit_by_rotate.get_binlog_rotated();
}
uint32 binlog_cache_reserved_size() {
  return binlog_commit_by_rotate.get_reserved_size();
}
bool binlog_should_commit_by_rotate(THD *thd) {
  return binlog_commit_by_rotate.should_commit_by_rotate(thd);
}
...
class binlog_cache_data {
 public:
  binlog_cache_data(class binlog_cache_mngr &cache_mngr, bool trx_cache_arg,
    ...
    m_cache.set_file_reserved_bytes(0);
    m_log_pos_offset = 0;
  }
  ...
  virtual ~binlog_cache_data() {
    ...
    IO_CACHE *cache_log = m_cache.get_io_cache();
    if (cache_log->file != -1) unlink(my_filename(cache_log->file));
    m_cache.close();
  }
  ...
  */
  bool write_prepare(size_t write_length) {
    ...
    if (unlikely(cache_log->write_pos + write_length > cache_log->write_end &&
      ...
      if (!std::strcmp(cache_log->dir, binlog_cache_dir))
  ...
  */
  ...
  uint32 log_pos_offset() { return m_log_pos_offset; }
  ...
  */
  const char *temp_file_name() { return m_cache.tmp_file_name(); }
  ...
  */
  ...
  struct Flags {
    ...
    */
    ...
    /* This indicates that the cache contain an external user xid, like
     * XA PREPARE 'xxx', XA COMMIT 'xxx', or XA ROLLBACK 'xxx'
     */
    bool with_external_xid : 1;
  ...
  */
  ...
  /*
    Stores the offset used to calculate the end_log_pos
    of events are being written into binlog cache.
  */
  uint32 m_log_pos_offset{0};
...
class binlog_cache_mngr {
  ...
  */
  void reset() {
    m_parallelization_barrier = false;
  ...
  int flush(THD *thd, my_off_t *bytes_written, bool *wrote_xid) {
    ...
    m_parallelization_barrier = parallelization_barrier;
  ...
  */
  ...
  bool m_parallelization_barrier{false};
...
class Binlog_event_writer : public Basic_ostream {
  ...
  */
  Binlog_event_writer(MYSQL_BIN_LOG::Binlog_ofile *binlog_file)
    ...
    if (binlog_rotated_by_large_trx()) {
      have_checksum = false;
    }
  }
...
inline bool Binlog_commit_by_rotate::should_commit_by_rotate(THD *thd) const {
  ulonglong threshold = opt_binlog_commit_by_rotate_threshold;

  /* Don't do rename if binlog_commit_by_rotate_threshold is 0. */
  if (threshold == 0) return false;

  /* Don't do rename if binlog encryption or compression is enabled. */
  if (rpl_encryption.get_enabled_var() || thd->variables.binlog_trx_compression)
    return false;

  binlog_cache_mngr *cache_mngr = thd_get_cache_mngr(thd);
  binlog_cache_data *trx_cache = cache_mngr->get_binlog_cache_data(true);
  binlog_cache_data *stmt_cache = cache_mngr->get_binlog_cache_data(false);

  /* Don't do rename if transaction size less than threshold. */
  if (likely(trx_cache->get_byte_position() <= threshold &&
             stmt_cache->get_byte_position() <= threshold))
    return false;

  binlog_cache_data *cache_data = trx_cache;
  if (unlikely(stmt_cache->is_finalized() && !stmt_cache->is_binlog_empty()))
  ...
  if (cache_data->file_reserved_bytes() == 0 ||
      cache_data->get_cache()->disk_writes() == 0)
    return false;

  /*
    Don't do rename if the offset used for calculating end_log_pos
    is not same to the actual reserved space size.
   */
  if (unlikely(cache_data->log_pos_offset() !=
               cache_data->file_reserved_bytes()))
    return false;
   ...
   */
  if (stmt_cache->is_finalized() && trx_cache->is_finalized() &&
      !stmt_cache->is_binlog_empty() && !trx_cache->is_binlog_empty())
    return false;
...
bool Binlog_commit_by_rotate::commit(THD *thd) {
  ...
  if (unlikely(!cache_data->is_finalized() || cache_data->is_binlog_empty()))
    cache_data = cache_mngr->get_binlog_cache_data(false);
  IO_CACHE *cache_log = cache_data->get_cache()->get_io_cache();

  if (cache_data->sync_temp_file()) return true;
  ...
  */
  my_chsize(cache_log->file, cache_data->temp_file_length(), 0, MYF(0));

  if (!mysql_bin_log.is_open()) {
    // It cannot do rename, back to the origin logic
    return false;
  }
  ...
  */
  ...
  if (mysql_bin_log.rotate(true, &dont_care, true /* commit_by_rotate */)) {
    assert(!mysql_bin_log.is_open());
    return true;
  }
  ...
  mysql_bin_log.get_binlog_file()->seek(cache_data->temp_file_length());
...
bool Binlog_commit_by_rotate::replace_binlog_file(THD *thd) {
  ...
  Binlog_event_writer writer(binlog_file);
  IO_CACHE_ostream *io_cache =
      dynamic_cast<IO_CACHE_ostream *>(binlog_file->get_pipeline_head_ptr());
  if (!io_cache) return false;

  size_t reserved_bytes = m_cache_data->file_reserved_bytes();
  size_t remaining_size = reserved_bytes - binlog_file->get_real_file_size();

  // Write ignorable_log_event and gtid_log_event.
  if (mysql_bin_log.write_gtid_log_event(thd, m_cache_data, &writer,
                                         cache_mngr->m_parallelization_barrier,
                                         true, remaining_size)) {
    return false;
  }
  m_has_gtid_written = true;

  // Flush all binlog events.
  if (binlog_file->flush()) return false;
  ...
  if (new_log_fd == -1) return false;

  /* Copy the part which has been flushed to binlog file to binlog cache */
  if (binlog_file->position() > 0) {
    size_t copy_len = 0;
    uchar buf[IO_SIZE];

    int read_fd = mysql_file_open(
        key_file_binlog, mysql_bin_log.get_log_fname(), O_RDONLY, MYF(MY_WME));
    if (read_fd == -1) goto err;

    while (copy_len < binlog_file->position()) {
  ...
  mysql_file_close(io_cache->get_io_cache()->file, MYF(0));
  io_cache->get_io_cache()->file = new_log_fd;
  new_log_fd = -1;
  ...
  DBUG_EXECUTE_IF("simulate_rename_binlog_cache_to_binlog_error", goto err;);

  if (my_rename(m_cache_data->temp_file_name(), mysql_bin_log.get_log_fname(),
                MYF(MY_WME)))
    goto err;
...
err:
  if (new_log_fd != -1) mysql_file_close(new_log_fd, MYF(0));
  return ret;
}
...
int binlog_cache_data::write_event(Log_event *ev) {
  ...
  if (ev != nullptr) {
    /*
      Initialize the offset used to calculate the end_log_pos
      of events are being written into binlog cache.
    */
    if (m_cache.is_empty()) {
      m_log_pos_offset = binlog_cache_reserved_size();
    }
    /* Calculate the current log_pos. */
    ev->common_header->log_pos = m_cache.length() + log_pos_offset();

    size_t write_len = ev->binlog_cache_write_len();
    if (write_len == (size_t)-1 || write_prepare(write_len)) {
      return 1;
    }
    ...
    if (ev->is_external_xid_end_event()) flags.with_external_xid = true;
...
/**
  Write the Gtid_log_event to the binary log.
  Write the Ignorable_log_event to the binary log when committing large
  transaction by rotate if nessary.
  ...
  it.
  @param commit_by_rotate Committing large transaction by rotate.
  @param remaining_size Remaining space for writing the ignorable_log_event
  and gtid_log_event. Effective only when commit_by_rotate is true.

  @retval false Success.
  @retval true Error.
*/
bool MYSQL_BIN_LOG::write_gtid_log_event(THD *thd,
  ...
  */
  ...
  if (commit_by_rotate) {
    size_t gtid_event_len = gtid_event.get_data_size() + LOG_EVENT_HEADER_LEN;
    size_t igno_event_len = remaining_size - gtid_event_len;
    size_t igno_event_data_len = igno_event_len - LOG_EVENT_HEADER_LEN;
    DBUG_EXECUTE_IF("simulate_remaining_size_too_small", remaining_size = 0;);
    if (unlikely(remaining_size < gtid_event_len)) {
      sql_print_information(
          "Could not rename binlog cache to binlog. The reserved "
          "header space is not enough for writing the gtid_log_event.");
      return true;
    }
    if (unlikely(igno_event_len > 0 && igno_event_len < LOG_EVENT_HEADER_LEN)) {
      sql_print_information(
          "Could not rename binlog cache to binlog. The reserved "
          "header space is not enough for writing the ignorable_log_event.");
      return true;
    } else if (igno_event_len >= LOG_EVENT_HEADER_LEN) {
      Ignorable_log_event igno_event(thd);
      igno_event.common_header->log_pos =
          mysql_bin_log.get_binlog_file()->get_real_file_size();
      if (igno_event.write_with_data(writer, igno_event_data_len)) return true;
    }
    gtid_event.common_header->log_pos =
        mysql_bin_log.get_binlog_file()->get_real_file_size();
  }
...
bool MYSQL_BIN_LOG::write_transaction(THD *thd, binlog_cache_data *cache_data,
  ...
  binlog_commit_by_rotate.m_has_gtid_written = false;
  if (unlikely(thd->binlog_should_commit_by_rotate)) {
    ret = binlog_commit_by_rotate.commit(thd);
    if (ret) goto end;
  }
  if (likely(!binlog_commit_by_rotate.m_has_gtid_written)) {
    ret = write_gtid_log_event(thd, cache_data, writer, parallelization_barrier,
                               false, 0);
    if (ret) goto end;
  }
  ...
  if (!ret) {
    ...
    binlog::global_context.monitoring_context()
        ...
        .update(binlog::monitoring::log_type::BINARY,
                ...
                my_micro_time(), cache_data->get_compressed_size(),
                cache_data->get_decompressed_size());
  }

end:
  thd->binlog_should_commit_by_rotate = false;
  return ret;
}
...
int binlog_cache_data::finalize(THD *thd, Log_event *end_event) {
  ...
  if (!is_binlog_empty()) {
    ...
    } else if (flags.finalized) {
      // XA_ROLLBACK will always do rollback, and if error happen,
      // if will still invoke trans_rollback_stmt and then rollback twice
      return 1;
    }
  ...
  */
  ...
  if (flags.finalized) {
    ...
    */
    ...
    if ((flags.with_xid || flags.with_external_xid) && error == 0)
      *wrote_xid = true;
...
bool binlog_cache_data::init_file_reserved_bytes() {
  ...
  DBUG_EXECUTE_IF("simulate_reserved_8KB", m_file_reserved_bytes = 8192;);
  m_cache.set_file_reserved_bytes(m_file_reserved_bytes);
...
int MYSQL_BIN_LOG::rollback(THD *thd, bool all) {
  ...
  */
  ...
  } else if (thd->lex->sql_command == SQLCOM_XA_COMMIT &&
             (get_xa_opt(thd) != XA_ONE_PHASE)) {
    goto end;
  ...
  if ((error == 0 && stuff_logged) || cache_mngr->has_incident()) {
    ...
    if (RUN_HOOK(
            ...
            (thd, all, thd_get_cache_mngr(thd)->get_trx_cache(),
             ...
             false, thd_get_cache_mngr(thd)->has_empty_transaction()))) {
...
bool show_binlog_events(THD *thd, MYSQL_BIN_LOG *binary_log) {
  ...
  if (binary_log->is_open()) {
    ...
    assert(!(unit->select_limit_ties || unit->select_limit_percent));
...
bool MYSQL_BIN_LOG::find_binlog_file_helper(
    char *binlog_file_name, const Gtid_set *gtid_set, Gtid *first_gtid,
    std::string &errmsg, GtidCheckType check_type, bool is_relay_log) {
  DBUG_TRACE;
  auto log_index = this->get_log_index();
  std::list<std::string> filename_list = log_index.second;
  ...
  while (rit != filename_list.rend()) {
    ...
    switch (read_gtids_from_binlog(filename, nullptr, &binlog_previous_gtid_set,
      ...
      case GOT_PREVIOUS_GTIDS: {
        if (check_type == GtidCheckType::IS_SUBSET) {
        ...
        } else if (check_type == GtidCheckType::NOT_SUBSET) {
          if (!gtid_set->is_subset(&binlog_previous_gtid_set)) {
            strcpy(binlog_file_name, filename);
            /*
              Verify that the selected binlog is not the first binlog,
            */
...
bool MYSQL_BIN_LOG::find_first_log_not_in_gtid_set(char *binlog_file_name,
                                                   ...
                                                   std::string &errmsg) {
  DBUG_TRACE;
  return find_binlog_file_helper(binlog_file_name, gtid_set, first_gtid, errmsg,
                                 GtidCheckType::IS_SUBSET, this->is_relay_log);
}

bool MYSQL_BIN_LOG::find_pos_from_binary_log(my_off_t &pos,
                                             ulonglong &trx_length,
                                             char *filename, Gtid_set *gset) {
  DBUG_TRACE;
  DBUG_PRINT("info", ("Opening file %s", filename));
  bool ret = true;
  ...
  while (!done && (ev = binlog_file_reader.read_event_object()) != nullptr) {
    ...
    switch (ev->get_type_code()) {
      case mysql::binlog::event::ROTATE_EVENT:
        // File read completed, not found given gtid
        done = true;
        break;
      case mysql::binlog::event::GTID_LOG_EVENT: {
        Gtid_log_event *gtid_ev = (Gtid_log_event *)ev;
        rpl_sidno sidno = gtid_ev->get_sidno(gset->get_tsid_map());
        rpl_gno gno = gtid_ev->get_gno();
        if (gset->contains_gtid(sidno, gno)) {
          pos = binlog_file_reader.position();
          trx_length = gtid_ev->transaction_length;
          done = true, ret = false;
        }
        break;
      }
...
bool MYSQL_BIN_LOG::find_log_name_by_gtid_set(char *binlog_file_name,
                                              Gtid_set *gtid_set) {
  DBUG_TRACE;
  Gtid first_gtid = {0, 0};
  std::string errmsg;
  return find_binlog_file_helper(binlog_file_name, gtid_set, &first_gtid,
                                 errmsg, GtidCheckType::NOT_SUBSET, false);
}
...
bool MYSQL_BIN_LOG::open_binlog(
    ...
    uint32 new_index_number, bool commit_by_rotate) {
  ...
  my_off_t offset = 0;
  binlog_commit_by_rotate.set_binlog_rotated(commit_by_rotate);
  ...
  */
  ...
  if (!is_relay_log) {
    binlog_commit_by_rotate.set_reserved_bytes(offset);
  }
  ...
  */
  ...
  if (!error) {
    ...
    error = open_binlog(
        ...
        false /*need_lock_index=false*/, true /*need_tsid_lock=true*/,
        extra_description_event, 0, commit_by_rotate);
  }
    ...
    if (can_rotate) {
      ...
      */
      ...
      if (!last_gtid_queued->is_empty()) {
        ...
        // add current batch gtids to gtid_set
        mi->rli->add_logged_batching_gtids();
        mi->rli->clear_batching_gtids();
...
bool MYSQL_BIN_LOG::write_buffer(const char *buf, uint len, Master_info *mi) {
  ...
  DBUG_EXECUTE_IF("force_buffer_relay_log", { mi->io_buffered = true; });
  ...
  if (m_binlog_file->write(pointer_cast<const uchar *>(buf), len) == 0) {
    ...
    if (!mi->io_buffered) {
      ...
      if (mi->io_buffered_cnt >= MAX_EVENT_BUFFER_NUM ||
        ...
        if (mi->transaction_parser.is_not_inside_transaction()) {
          mysql_mutex_lock(&mi->data_lock);
          const Gtid *last_gtid_queued = mi->get_queueing_trx_gtid();
          if (!last_gtid_queued->is_empty()) {
            mi->rli->collect_batching_gtids(last_gtid_queued->sidno,
                                            last_gtid_queued->gno);
          }
          mysql_mutex_unlock(&mi->data_lock);
        }
...
int MYSQL_BIN_LOG::rotate(bool force_rotate, bool *check_purge,
  ...
  *check_purge = false;
  ...
  if (DBUG_EVALUATE_IF("force_rotate", 1, 0) || force_rotate ||
      ...
      DBUG_EVALUATE_IF("simulate_max_binlog_size", true, false)) {
    error = new_file_without_locking(nullptr, commit_by_rotate);
    *check_purge = true;
  ...
  if (stmt_stuff_logged || trx_stuff_logged) {
    ...
    if (RUN_HOOK(
            ...
            (thd, all, thd_get_cache_mngr(thd)->get_trx_cache(),
             ...
             max<my_off_t>(max_binlog_cache_size, max_binlog_stmt_cache_size),
             is_atomic_ddl,
             thd_get_cache_mngr(thd)->has_empty_transaction())) ||
