...
%else
%define api.prefix {ora_sql_parser_}
%endif
/*
...
%token <lexer.keyword> ROWID_SYM 1465
...
%left  INTERVAL_SYM
%left PREFER_PARENTHESES PIVOT_SYM
%left '(' ')'
...
/* MERGE_PS8404_CHECK_TAG: start, note: oracle compatibility */
%ifndef SQL_MODE_ORACLE
        routine_string
%endif
/* MERGE_PS8404_CHECK_TAG: end */
        opt_explain_into
...
%type <bipartite_name_ora> ora_internal_variable_name ora_internal_variable_qname
...
%type <top_level_node>
        ...
        opt_mv_or_mlog_stmt
        create_materialized_view_or_log_stmt
        show_create_materialized_view_stmt
...
%type <table_ident> drop_mview_or_mlog
%type <opt_materialized_view> opt_materialized_view
%type <string> opt_imv_attrs
%endif
%%
...
%ifdef SQL_MODE_ORACLE
        | drop_mview_or_mlog_stmt       { $$= nullptr; }
%endif
...
change:
          CHANGE change_replication_stmt TO_SYM
          {
            LEX *lex = Lex;
...
source_log_pos:
          MASTER_LOG_POS_SYM
          {
            push_deprecated_warn(YYTHD, "SOURCE_LOG_POS",
                                        "SOURCE_LOG_POS");
          }
        | SOURCE_LOG_POS_SYM
        ;
        ...
        | source_log_pos EQ ulonglong_num
          {
            ...
            /*
               ...
               (SOURCE_LOG_POS explicitely specified as 0) and 0 (unspecified),
...
create_materialized_view_or_log_stmt:
          CREATE definer_opt MATERIALIZED_SYM VIEW_SYM opt_mv_or_mlog_stmt
          {
            if($1 != nullptr)
            {
              my_error(ER_NOT_SUPPORTED_YET, MYF(0),"create materialized view with hint.");
              MYSQL_YYABORT;
            }
            $$= $5;
          }
...
opt_mv_or_mlog_stmt:
          LOG_SYM IF not EXISTS ON_SYM table_ident
          {
            $$= NEW_PTN PT_create_mlog_stmt(@$, YYMEM_ROOT, true, $6);
          }
        | LOG_SYM ON_SYM table_ident
          {

            $$= NEW_PTN PT_create_mlog_stmt(@$, YYMEM_ROOT, false, $3);
          }
        | IF not EXISTS table_ident opt_materialized_view
          {
              $$= NEW_PTN PT_create_materialized_view_stmt(@$,YYMEM_ROOT,
              true,
              $4, /* view_name */
              $5
              );
          }
        | table_ident opt_materialized_view
          {
              $$= NEW_PTN PT_create_materialized_view_stmt(@$,YYMEM_ROOT,
              false,
              $1,
...
opt_materialized_view:
          ...
          AS query_expression_with_opt_locking_clauses
            {
              auto opt = NEW_PTN Materialized_view_opt;
              if(!opt) {
                 MYSQL_YYABORT; // OOM
              }
              ...
              opt->attrs = $6;
              opt->qe = $8;
              opt->define = make_string(YYTHD,   @7.raw.end ,   @8.raw.end);

              if($1.size() != 0) {
                void *column_list_raw_mem= YYTHD->memdup(&($1), sizeof($1));
                        if (!column_list_raw_mem)
                          MYSQL_YYABORT; // OOM
                opt->column_list =static_cast<Create_col_name_list *>(column_list_raw_mem);
              } else {
                opt->column_list = nullptr;
              }
              $$ = opt;
            }
        ;
...
opt_build_mv:
          %empty { $$= 1; }
        | BUILD_SYM IMMEDIATE_SYM { $$ = 1;  }
        | BUILD_SYM DEFERRED_SYM  { $$ = 0;  }
        ;

opt_flush_mode_mv:
          %empty { $$= 0; }
        | REFRESH_SYM FAST_SYM
          {
            $$= 1;
          }
        | REFRESH_SYM COMPLETE_SYM
          {
            $$= 0;
          }
        ;
...
opt_imv_attrs:
      ...
      | ATTRIBUTE_SYM opt_equal text_string
       {
          $$= $3;
       }
      ;
...
%endif
          {
            ...
            if (row_type && !row_type->m_is_rowtype) {
...
%ifdef SQL_MODE_ORACLE
            ...
            if (row_type) {
              spvar->field_def.set_sp_define_param(row_type, true);
            }
%endif
            ...
            */
            ...
            $$=
              create_item_for_sp_var(
                thd, to_lex_cstring($1), nullptr, nullptr,
                sp->m_parser_data.get_current_stmt_start_ptr(),
                @1.raw.start,
                @1.raw.end);
...
%endif
          {
            ...
            if (pctx->find_variable($1.str, $1.length, true) ||
            (sp->m_type == enum_sp_type::PACKAGE_BODY && lex->find_variable($1.str, $1.length, &ctx, &rh)))
            {
        ...
        | sp_decl_idents ',' ident
          {
            ...
            (sp->m_type == enum_sp_type::PACKAGE_BODY && lex->find_variable($3.str, $3.length, &ctx, &rh)))
            {
...
sp_proc_stmt_close:
          CLOSE_SYM ident
          {
            ...
            if (! pctx->find_cursor($2, &offset, false, lex, &ctx_cursor, &rh_cursor))
            {
              my_error(ER_SP_CURSOR_MISMATCH, MYF(0), $2.str);
              MYSQL_YYABORT;
            }
            ...
            sp_variable *spvar= lex->find_variable($2.str, $2.length, &ctx, &rh);
            sp_instr_cclose *i=
              NEW_PTN sp_instr_cclose(sp->instructions(), ctx_cursor, offset, false, &sp_rcontext_handler_local);
            ...
            {
              spvar->field_def.ora_record.m_select_into_spv_info.m_ctx_cursor = ctx;
              spvar->field_def.ora_record.m_select_into_spv_info.m_rh_cursor = rh;
...
sp_fetch_list:
          ident
          {
            ...
            sp_pcontext *ctx = nullptr;
            const Sp_rcontext_handler *rh = nullptr;
            if (!(spv= lex->find_variable($1.str, $1.length, &ctx, &rh)))
            {
              my_error(ER_SP_UNDECLARED_VAR, MYF(0), $1.str);
              MYSQL_YYABORT;
            }
        ...
        | sp_fetch_list ',' ident
          {
            ...
            sp_pcontext *ctx = nullptr;
            const Sp_rcontext_handler *rh = nullptr;
            if (!(spv= lex->find_variable($3.str, $3.length, &ctx, &rh)))
            {
        ...
        | ora_internal_variable_qname
          {
            ...
            LEX_STRING spvar_name = NULL_STR;
            if ($1->bipartite_name.prefix.length != 0) {
              spvar_name = to_lex_string($1->bipartite_name.prefix);
            } else {
              spvar_name = $1->medium->head()->ident;
            }
            /*
              Does not support: ora_value_ident '(' udt_index_expr ')'
            */
            if (0 == $1->bipartite_name.name.length) {
            ...
            */
            if (check_reserved_words(spvar_name.str)) {
              YYTHD->syntax_error_at(@1);
              MYSQL_YYABORT;
            }

            /*
              Validates whether the given variable path is legal.
            */
            if (sp_validate_into_variable_hierarchy(YYTHD, $1)) {
              MYSQL_YYABORT;
            }

            const Sp_rcontext_handler *rh = nullptr;
            sp_pcontext *pctx = nullptr;
            sp_variable *spv = lex->find_variable(spvar_name.str, spvar_name.length, &pctx, &rh);
            if (!spv) {
              my_error(ER_SP_UNDECLARED_VAR, MYF(0), spvar_name.str);
              MYSQL_YYABORT;
            }
            spv->field_def.ora_record.m_select_into_spv_info.m_ctx_cursor = pctx;
            spv->field_def.ora_record.m_select_into_spv_info.m_rh_cursor = rh;
            sp_instr_cfetch *i= (sp_instr_cfetch *)sp->last_instruction();
            i->add_to_varlist(spv, $1);
          }
        | sp_fetch_list ',' ora_internal_variable_qname
          {
            LEX *lex= Lex;
            sp_head *sp= lex->sphead;
            if (!sp) {
            ...
            LEX_STRING spvar_name = NULL_STR;
            if ($3->bipartite_name.prefix.length != 0) {
              spvar_name = to_lex_string($3->bipartite_name.prefix);
            } else {
              spvar_name = $3->medium->head()->ident;
            }
            /*
              Does not support: ora_value_ident '(' udt_index_expr ')'
            */
            if (0 == $3->bipartite_name.name.length) {
              YYTHD->syntax_error_at(@1);
              MYSQL_YYABORT;
            }
            /*
            ...
            */
            ...
            /*
              Validates whether the given variable path is legal.
            */
            if (sp_validate_into_variable_hierarchy(YYTHD, $3)) {
              MYSQL_YYABORT;
            }
...
evict_sequences_stmt:
          RELOAD SEQUENCES opt_table_list
          {
            $$ = NEW_PTN PT_reload_sequences(@$, $3);
          }
        ;
...
%ifdef SQL_MODE_ORACLE
alter_materialized_view_stmt:
          ALTER MATERIALIZED_SYM VIEW_SYM table_ident BUILD_SYM IMMEDIATE_SYM
          {
            $$= NEW_PTN PT_alter_materialized_view(@$, YYMEM_ROOT, $4, 1);
          }
        | ALTER MATERIALIZED_SYM VIEW_SYM table_ident BUILD_SYM DEFERRED_SYM
          {
            $$= NEW_PTN PT_alter_materialized_view(@$, YYMEM_ROOT, $4, 0);
          }
        ;
%endif
...
%ifdef SQL_MODE_ORACLE
        | bit_expr OR_OR_SYM bit_expr  %prec OR_OR_SYM
          {
            $$= NEW_PTN Item_func_ora_oror(@$, $1, $3);
          }
        ...
        | IDENT_sys '(' opt_udf_expr_list ')' '.' ident
          {
            ...
            sp_pcontext *ctx = nullptr;
            const Sp_rcontext_handler *rh = nullptr;
            sp_variable *spvar= lex->find_variable($1.str, $1.length, &ctx, &rh);
            if (!spvar)
            {
              my_error(ER_SP_UNDECLARED_VAR, MYF(0), $1.str);
              MYSQL_YYABORT;
        ...
        | COUNT_SYM '(' DISTINCT expr_list ')' opt_keep_clause opt_windowing_clause
          {
            ...
            $$= NEW_PTN Item_sum_count(@$, $4, window);
          }
        ...
        | ora_internal_variable_qname
          {
            ...
            */
            ...
            $$= NEW_PTN PT_select_sp_var_field(@$, spvar_name, $1);
          }
%endif
        ;
...
%ifdef SQL_MODE_ORACLE
drop_mview_or_mlog_stmt:
          DROP MATERIALIZED_SYM VIEW_SYM drop_mview_or_mlog
          {
            if(!$4) {
              MYSQL_YYABORT;
            }
            ...
            if (!Select->add_table_to_list(YYTHD, $4, nullptr, TL_OPTION_UPDATING,
                                   YYPS->m_lock_type, YYPS->m_mdl_type))
              MYSQL_YYABORT;
            Lex->m_sql_cmd= NEW_PTN Sql_cmd_drop_table();
            if (!Lex->m_sql_cmd)
...
drop_mview_or_mlog:
          LOG_SYM IF EXISTS ON_SYM table_ident
          {
            Lex->set_is_materialized_view_log(true);
            Lex->drop_if_exists= 1;
            $$= PT_create_mlog_stmt::make_mlog_name(YYTHD, $5);
          }
        | LOG_SYM  ON_SYM table_ident
          {
            Lex->set_is_materialized_view_log(true);
            Lex->drop_if_exists= 0;
            $$= PT_create_mlog_stmt::make_mlog_name(YYTHD, $3);
          }
        | IF EXISTS  table_ident
           {
...
insert_record_query_expression:
          ...
          {                             /*$3*/
            ...
            if (!sp || (sp->m_type != enum_sp_type::PROCEDURE &&
              ...
              thd->syntax_error_at(@1);
              MYSQL_YYABORT;
            }
            sp->reset_lex(thd);
          }
          signal_allowed_expr ')'   /* #4 */
          {
            ...
            option.query_spec_options= 0;
            Mem_root_array_YY<PT_table_reference *> table_reference_list;
            if(Lex->insert_into_table_record(YYTHD, $1, $4, @1, &table_reference_list))
              MYSQL_YYABORT; // OOM
            // select * from table;
        ...
        | ident /* #1 */
          {
            ...
            option.query_spec_options= 0;
            Mem_root_array_YY<PT_table_reference *> table_reference_list;
            if(lex->insert_into_table_record(thd, $1, nullptr, @1, &table_reference_list))
              MYSQL_YYABORT; // OOM
            // select * from table;
            PT_select_item_list *itemLs= NEW_PTN PT_select_item_list(@$);
            Item_asterisk *as = NEW_PTN Item_asterisk(@$, nullptr, nullptr);
...
show_create_materialized_view_stmt:
          ...
          {
            auto *tmp = NEW_PTN PT_show_create_materialized_view(@$, $5);
            MAKE_CMD(tmp);
          }
        ;
%endif
        ...
        | describe_command opt_explain_options  PLAN_SYM FOR_SYM explain_oracle_stmt
          {
            $$= NEW_PTN PT_explain(@$, $2.explain_format_type, $2.is_analyze,
                                   ...
                                   std::optional<std::string_view>(
                                     to_string_view($2.explain_into_variable_name)) :
                                   std::optional<std::string_view>(std::nullopt),
                                   NULL_CSTR); // MERGE_PS8404_CHECK_TAG: need to recompatible?
          }
        ;
...
explainable_stmt:
        ...
        | opt_explain_for_schema insert_all_stmt
          {
            $$.statement = $2;
            $$.schema_name_for_explain = $1;
          }
        ...
        | opt_explain_for_schema merge_stmt
          {
            $$.statement = $2;
            $$.schema_name_for_explain = $1;
          }
...
%endif
        ...
        | LOGFILE_SYM
        | LOGS_SYM
        | LOG_SYM %prec KEYWORD_USED_AS_IDENT
        | MASTER_AUTO_POSITION_SYM
        | MASTER_COMPRESSION_ALGORITHM_SYM
        | MASTER_CONNECT_RETRY_SYM
        | MASTER_DELAY_SYM
...
sp_for_loop_bounds:
          ...
          for_loop_bound_expr
          {
            ...
            uint offset = 0;
            sp_pcontext *ctx_cursor = Lex->get_sp_current_parsing_ctx();
            const Sp_rcontext_handler *rh_cursor = &sp_rcontext_handler_local;
            ...
            if(function || var) {  // for var in cursor
              ...
              if (unlikely(Lex->sp_for_loop_bounds_set_cursor(YYTHD, ident, nullptr,
                ...
                item_func_tmp ? item_func_tmp->argument_count() : 0, &ctx_cursor, &rh_cursor)))
                MYSQL_YYABORT;
              name_result= ident;
            ...
            $$= NEW_PTN Oracle_sp_for_loop_bounds(offset,name_result,true,0,$2,nullptr,subselect ? true:false, ctx_cursor, rh_cursor);
            if (!$$)
        ...
        | IN_SYM               /*$1*/
          ...
          for_loop_bound_expr  /*$4*/
          {
            ...
            $$= NEW_PTN Oracle_sp_for_loop_bounds(0,empty_str,false,1,$2,$4, false, nullptr, nullptr);
            if (!$$)
              MYSQL_YYABORT_ERROR(ER_DA_OOM, MYF(0)); /* purecov: inspected */
          }
        | IN_REVERSE_SYM        /*$1*/
        ...
        | FOR_SYM                       /*$1*/
          ...
          sp_for_loop_index_and_bounds      /*$3*/
          {
            ...
            if($3->is_cursor){
              ...
              sp_pcontext *ctx_cursor = nullptr;
              const Sp_rcontext_handler *rh_cursor = nullptr;
              if (! pctx->find_cursor($3->cursor_name, &offset, false, lex, &ctx_cursor, &rh_cursor))
              {
                my_error(ER_SP_CURSOR_MISMATCH, MYF(0), $3->cursor_name.str);
                MYSQL_YYABORT;
              }
              if ( ! (expr = ctx_cursor->make_item_plsql_cursor_attr(thd, to_lex_cstring($3->cursor_name.str), offset, rh_cursor)))
                MYSQL_YYABORT;
            } else {
              Item *tmp = NEW_PTN Item_func_uuid_short(@$);
              if (unlikely(tmp == NULL))
          ...
          for_loop_statements  /*$5*/ //loop..end loop
          {
            ...
            sp_pcontext *ctx_cursor = nullptr;
            const Sp_rcontext_handler *rh_cursor = nullptr;
            if(lex->sp_continue_loop(thd, &offset, pctx->find_label_current_loop_start(), &ctx_cursor, &rh_cursor))
              MYSQL_YYABORT;
...
raise_stmt:
        ...
        | RAISE_APPLICATION_ERROR '(' ulong_num ',' expr ')'
          {
            THD *thd= YYTHD;
            LEX *lex= thd->lex;
            if(!lex->sphead) {
...
sf_tail:
          ...
          {                                  /* $8 */
            ...
            if (row_type) {
              if(!row_type->m_is_rowtype)
                udt_ident = row_type->get_udt_name();
              if ((!sp->m_parent || !sp->m_parent->get_package() ||
              !row_type->m_is_package_define) &&
              (row_type->m_table_type == 0 || row_type->m_table_type == 1)) {
                my_error(ER_NOT_SUPPORTED_YET, MYF(0), "return udt of collation type");
                MYSQL_YYABORT;
              }
              field_type= MYSQL_TYPE_VARCHAR;
              if(row_type->m_is_rowtype || (sp->m_parent && sp->m_parent->get_package()))
            ...
            if (row_type) {
              sp->m_return_field_def.set_sp_define_param(row_type, false);
              if(row_type->m_is_rowtype) {
                Table_ident *table_ref = new (YYTHD->mem_root) Table_ident(
                  !row_type->get_udt_name()->table.str ? to_lex_cstring(sp->m_db) : row_type->get_udt_name()->table, 
                  row_type->get_udt_name()->m_column);
                sp->m_return_field_def.ora_record.set_table_rowtype_ref(table_ref);
              }
            }
            if (prepare_sp_create_field(YYTHD,
                                        &sp->m_return_field_def))
          ...
          select_stmt   /*$9*/
          {
            ...
            sp_instr_cpush_rowtype *i=
              NEW_PTN sp_instr_cpush_rowtype(sp->instructions(), pctx,
                                     cursor_lex, cursor_query,
                                     pctx_new->current_cursor_count(), &sp_rcontext_handler_local);
        ...
        | sp_decl_ident SYS_REFCURSOR_SYM
          {
            ...
            if(sp->m_type == enum_sp_type::PACKAGE_BODY ||
              ...
              my_error(ER_NOT_SUPPORTED_YET, MYF(0), "SYS_REFCURSOR used in package body or head");
              MYSQL_YYABORT;
            }
...
row_field_definition:
        ...
        | IDENT_sys opt_qualified_column_ident PERCENT_ORACLE_SYM TYPE_SYM sp_opt_default
          {
            Item *dflt_value_item= $5.expr;
            Create_field *cdf = NEW_PTN Create_field();
            if (Lex->sp_variable_declarations_record_percent_type(YYTHD, $1, $2, dflt_value_item, cdf))
              MYSQL_YYABORT;
            $$= cdf;
          }
        ;
        ...
        | IDENT_sys '(' opt_expr_list ')' '.'
          {
            ...
            sp_head *sp= lex->sphead;
            if(!sp) {
              my_error(ER_WRONG_USAGE, MYF(0), $1.str, ".");
              MYSQL_YYABORT;
            }
            sp->reset_lex(thd);
...
ora_value:
          ...
          set_expr_or_default
          {
            if (Lex->check_forloop_set_var(YYTHD, $1->bipartite_name.prefix,
              $1->bipartite_name.name))
            {
                MYSQL_YYABORT;
            }
...
explicit_cursor_attr:
          ident PERCENT_ORACLE_SYM plsql_cursor_attr
          {
            ...
            sp_pcontext *ctx_cursor = nullptr;
            const Sp_rcontext_handler *rh_cursor = nullptr;
            if (!sp || !pctx || !pctx->find_cursor($1, &offset, false, lex, &ctx_cursor, &rh_cursor))
            {
              my_error(ER_SP_CURSOR_MISMATCH, MYF(0), $1.str);
              MYSQL_YYABORT;
            }
            sp_pcursor *pc = ctx_cursor->find_cursor_parameters(offset);
            if(!pc)
            {
              my_error(ER_SP_CURSOR_MISMATCH, MYF(0), $1.str);
              MYSQL_YYABORT;
            ...
            switch ($3) {
              case plsql_cursor_attr_type::PLSQL_CURSOR_ATTR_ISOPEN:
                $$= NEW_PTN Item_func_cursor_isopen(to_lex_cstring($1.str), offset, pc->get_cursor_spv(), rh_cursor);
                break;
              case plsql_cursor_attr_type::PLSQL_CURSOR_ATTR_FOUND:
                $$= NEW_PTN Item_func_cursor_found(to_lex_cstring($1.str), offset, pc->get_cursor_spv(), rh_cursor);
                break;
              case plsql_cursor_attr_type::PLSQL_CURSOR_ATTR_NOTFOUND:
                $$= NEW_PTN Item_func_cursor_notfound(to_lex_cstring($1.str), offset, pc->get_cursor_spv(), rh_cursor);
                break;
              case plsql_cursor_attr_type::PLSQL_CURSOR_ATTR_ROWCOUNT:
                $$= NEW_PTN Item_func_cursor_rowcount(@$, to_lex_cstring($1.str), offset, pc->get_cursor_spv(), rh_cursor);
                break;              
              default:
                YYTHD->syntax_error_at(@3);
                MYSQL_YYABORT;
...
sp_labeled_control:
          ...
          sp_unlabeled_control_ora sp_opt_label
          {
            ...
            if($3.curs == 1)
            {
              if(lex->close_ref_cursor(YYTHD, $3.vars, $3.ctx_cursor, $3.rh_cursor))
                MYSQL_YYABORT;
            }
          }
        ;
...
sp_proc_stmt_unlabeled:
          ...
          sp_unlabeled_control_ora
          {
            ...
            if($2.curs == 1)
            {
              if(lex->close_ref_cursor(YYTHD, $2.vars, $2.ctx_cursor, $2.rh_cursor))
                MYSQL_YYABORT;
            }
          }
        ;
...
sp_proc_stmt_fetch_ora:
          ...
          {          /*$4*/
            ...
            sp_instr_cfetch *i= NEW_PTN sp_instr_cfetch(sp->instructions(),
                                                        ctx_cursor, offset, &sp_rcontext_handler_local);
        ...
        | FETCH_SYM  /*$1*/
          ...
          opt_bulk_limit_ora /*$7*/
          {
            ...
            sp_pcontext *ctx = nullptr;
            const Sp_rcontext_handler *rh = nullptr;
            if (!(spv= lex->find_variable($6.str, $6.length, &ctx, &rh)))
            {
...
ora_sp_forall_loop:
          ...
          sp_for_loop_index_and_bounds
          {
            ...
            sp_variable *spvar_ident_i =
                pctx->find_variable($3->cursor_var.str, $3->cursor_var.length, true);
            if (!spvar_ident_i)
            {
          ...
          insert_stmt  /*$5*/
          {
            ...
            MAKE_CMD($5);
            if(Lex->check_forall_i_ident(YYTHD, insert->get_insert_query()))
              MYSQL_YYABORT;
