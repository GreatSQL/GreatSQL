...
#include "sql/sql_delta.h"  // Item_dbms_mview_delta_imv
  ...
  Item *instantiate(THD *thd, PT_item_list *args) {
    switch (args->elements()) {
      case 1: {
        ...
        if (cs == NULL) return nullptr;

        Cast_type type{ITEM_CAST_CHAR, cs, NULL, NULL};
        int64_t length = 0;
        unsigned precision = 0;
        if (validate_cast_type_and_extract_length(thd, (*args)[0], type, false,
                                                  &length, &precision)) {
          return nullptr;
        }
        return new (thd->mem_root)
            Item_typecast_char(POS(), (*args)[0], length, cs);
      }
      case 2: {
        return new (thd->mem_root)
            Item_func_to_char(POS(), (*args)[0], (*args)[1]);
      }
      case 3: {
        return new (thd->mem_root)
            Item_func_to_char(POS(), (*args)[0], (*args)[1], (*args)[2]);
      }
      default:
        assert(false);
        return nullptr;
  ...
  Item *instantiate(THD *thd, PT_item_list *args) {
    switch (args->elements()) {
      case 1: {
        ...
        return new (thd->mem_root) Item_func_trunc(POS(), (*args)[0], i0, true);
      }
      case 2: {
        return new (thd->mem_root)
            Item_func_trunc(POS(), (*args)[0], (*args)[1], false);
      }
      default:
        assert(false);
        return nullptr;
...
class Instr_instantiator {
 public:
  static const uint Min_argcount = 2;
  static const uint Max_argcount = 4;

  Item *instantiate(THD *thd, PT_item_list *args) {
    sql_mode_t mode_oracle = thd->variables.sql_mode & MODE_ORACLE;
    if (mode_oracle) {
      switch (args->elements()) {
        case 2:
          /* Yes, parameters in that order : 1, 2 */
          return new (thd->mem_root)
              Item_func_ora_instr(POS(), (*args)[0], (*args)[1]);
        case 3: {
          Item_int *nth_appearance = new (thd->mem_root) Item_int(1);
          return new (thd->mem_root) Item_func_ora_instr(
      ...
      switch (args->elements()) {
        case 2:
          /* Yes, parameters in that order : 1, 2 */
          return new (thd->mem_root)
              Item_func_instr(POS(), (*args)[0], (*args)[1]);
        default:
          my_error(ER_WRONG_PARAMCOUNT_TO_NATIVE_FCT, MYF(0), "instr");
          return nullptr;
      }
    }
  }
...
Item *Create_sp_func::create(THD *thd, LEX_STRING db, LEX_STRING name,
  ...
  /*
    is it a udt ? if so, get its definitions.
    if the names of record and udt are the same, it is resolved as udt.
  */
  ...
  if ((thd->variables.sql_mode & MODE_ORACLE)) {
    ...
    sp_pcontext *ctx;
    sp_variable *spv =
        thd->lex->find_variable(name.str, name.length, &ctx, &rh);
    /*e.g:
    ...
    */
    if (!spv && thd->sp_runtime_ctx) {
      spv = thd->sp_runtime_ctx->search_variable(name.str, name.length);
    }
    if (spv) {
      if (spv->field_def.ora_record.is_record_define &&
        ...
        return create_item_table_by_index_for_sp_var(
            ...
            spv->sph,
            thd->lex->sphead->m_parser_data.get_current_stmt_start_ptr(),
            POS().raw.start, POS().raw.end);
...
static const std::pair<const char *, Create_func *> func_array[] = {
    ...
    {"ALTER_ALL_EVENT", SQL_FN(Item_func_alter_all_event, 3)},
#ifdef SSL_GM
    ...
    {"GTID_COUNT", SQL_FN(Item_func_gtid_count, 1)},
    {"GTID_POSITION_IN_BINLOG", SQL_FN(Item_func_gtid_pos_in_binlog, 2)},
    {"GTID_SUB", SQL_FN(Item_func_gtid_sub, 2)},
    ...
    {"LENGTHB", SQL_FN(Item_func_lengthb, 1)},
#ifndef NDEBUG
    ...
    {"ORA_OROR", SQL_FN(Item_func_ora_oror, 2)},
    ...
    {"SNOWFLOW_ID", SQL_FN(Item_func_snowflow_id, 0)},
    ...
    {"TO_TIMESTAMP", SQL_FN(Item_func_to_timestamp, 2)},
    {"TRUNC", SQL_FACTORY(Trunc_instantiator)},
    ...
    {"DISTANCE", SQL_FN_V(Item_func_vec_distance, 2, 3)},
    {"VECTOR_DISTANCE", SQL_FN_V(Item_func_vec_distance, 2, 3)},
...
*/
Item *create_func_to_number(THD *thd, const POS &pos, Item *arg) {
  int64_t length = 0;
  unsigned precision = 0;
  char str_precision[2 + 1];
  char str_dec[2 + 1];
  Cast_type type;

  memset(str_precision, 0, sizeof(str_precision));
  memset(str_dec, 0, sizeof(str_dec));
  type.target = ITEM_CAST_DECIMAL;
  type.charset = nullptr;

  /*
    Remove zero padding after decimal type display in oracle mode
    Set the default maximum number of decimal places for decimal
  */
  if (current_thd->variables.sql_mode & MODE_ORACLE) {
    type.length = ullstr(DECIMAL_MAX_PRECISION, str_precision);
    type.dec = ullstr(DECIMAL_MAX_SCALE, str_dec);
  } else {
    type.length = nullptr;
    type.dec = nullptr;
  }

  if (validate_cast_type_and_extract_length(thd, arg, type, false, &length,
                                            &precision))
    return nullptr;

  return new (thd->mem_root) Item_typeto_number(pos, arg, length, precision);
}
