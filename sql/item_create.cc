  ...
  Item *instantiate(THD *thd, PT_item_list *args) {
    switch (args->elements()) {
      case 1: {
        ...
        if (validate_cast_type_and_extract_length(thd, POS(), (*args)[0], type,
                                                  false, &length, &precision)) {
          return nullptr;
        }
        return new (thd->mem_root)
...
Item *create_func_to_number(THD *thd, const POS &pos, Item *arg) {
  ...
  */
  ...
  if (validate_cast_type_and_extract_length(thd, pos, arg, type, false, &length,
                                            &precision))
    return nullptr;
