...
class Sp_rcontext_handler {
  ...
  */
  ...
  /**
    At execution time THD->sp_runtime_ctx points to the run-time context
    (sp_rcontext) of the currently executed routine. Local variables store their
    data in the sp_rcontext pointed by thd->sp_runtime_ctx. Package body
    variables store data in separate sp_rcontext that belongs to the package.
...
class Sp_rcontext_handler_package_head : public Sp_rcontext_handler_local {
 public:
  const LEX_CSTRING *get_name_prefix() const override;
  sp_rcontext *get_rcontext(sp_rcontext *ctx) const override;
};
...
extern MYSQL_PLUGIN_IMPORT Sp_rcontext_handler_local sp_rcontext_handler_local;

extern MYSQL_PLUGIN_IMPORT Sp_rcontext_handler_package_head
    sp_rcontext_handler_package_head;

extern MYSQL_PLUGIN_IMPORT Sp_rcontext_handler_package_body
    sp_rcontext_handler_package_body;
...
class Item : public Parse_tree_node {
 ...
 public:
  /**
    ...
    example convert_constant_item()/resolve_const_item(), this point to old
    item for new item.
  */
  ...
  */
  ...
  virtual bool result_type_table() const { return false; }
  virtual void clear_args_table() {}
  /**
  ...
  */
  ...
  virtual bool collect_session_vars(uchar *) { return false; }  // parallel load
  ...
  */
  ...
  // row table
  virtual bool row_table_create_items(THD *, Row_definition_table_list *) {
    return false;
  }
  ...
  virtual bool modify_item_field_about_cursor(THD *, List<Create_field> *,
                                              sp_cursor *, sp_rcontext *) {
    return false;
  }
  virtual bool is_rowtype() { return false; }
  ...
  */
  ...
  bool has_rownum_expr() const { return m_accum_properties & PROP_ROWNUM_FUNC; }

  void set_rownum() { m_accum_properties |= PROP_ROWNUM_FUNC; }
  ...
  */
  ...
  /**
   * Oracle outer join syntax, such as t1.a=t2.b(+);
   * outer_joined is true for t2.b.
   */
  ...
  */
  static constexpr uint32 PROP_HAS_GROUPING_SET_DEP = 0x10;
  /**
    Set if the item or one or more of the underlying items is a GROUPING
    function.
  */

  static constexpr uint32 PROP_ROLLUP_EXPR = 0x001000;

  static constexpr uint32 PROP_GROUPING_FUNC = 0x002000;

  static constexpr uint32 PROP_ROWNUM_FUNC = 0x004000;

  /*
    Set if the item or one or more of the underlying items is a connect by
    function.
  */
  static constexpr uint32 PROP_CONNECT_BY_FUNC = 0x008000;

  /**
    Set if the item or one or more of the underlying items is oracle
    udt type.
  */
  static constexpr uint32 PROP_ORA_TYPE = 0x010000;
  /**
    Set if the item or one or more of the underlying items is oracle
    udt table.
  */
  static constexpr uint32 PROP_ORA_TABLE = 0x020000;

  /**
    Set if the item or one or more of the underlying items is a connect by
    function. syc_connect_by_path,isleaf,iscycle
  */
  static constexpr uint32 PROP_CONNECT_BY_VALUE = 0x040000;

  /**
    Set if the item or one or more of the underlying items is a sequence
  */
  static constexpr uint32 PROP_SEQUENCE = 0x080000;
...
class Item_splocal : public Item_sp_variable,
  ...
  */
  ...
  Item_splocal(const Sp_rcontext_handler *rh, const Name_string sp_var_name,
               uint sp_var_idx, enum_field_types sp_var_type, uint pos_in_q = 0,
               uint len_in_q = 0);
 ...
 public:
  ...
  Item_result result_type() const override {
    if (m_result_type == ROW_RESULT) return m_result_type;
  ...
  void set_result_type(Item_result arg_type) { m_result_type = arg_type; }
};
...
class Item_splocal_row_field : public Item_splocal {
 protected:
  Name_string m_field_name;
  uint m_field_idx;
  bool set_value(THD *thd, sp_rcontext *ctx, Item **it) override;

 public:
  Item_splocal_row_field(const Sp_rcontext_handler *rh,
                         const Name_string sp_var_name,
                         const Name_string sp_field_name, uint sp_var_idx,
                         uint sp_field_idx, enum_field_types sp_var_type,
  ...
  void print(const THD *, String *str,
             enum_query_type query_type) const override;
  inline enum Type type() const override { return Item::ORACLE_ROWTYPE_ITEM; }
...
class Item_splocal_row_field_by_name : public Item_splocal_row_field {
 public:
  Item_splocal_row_field_by_name(const Sp_rcontext_handler *rh,
                                 const Name_string sp_var_name,
                                 const Name_string sp_field_name,
                                 uint sp_var_idx, enum_field_types sp_var_type,
                                 uint pos_in_q = 0, uint len_in_q = 0)
      : Item_splocal_row_field(
            rh, sp_var_name, sp_field_name, sp_var_idx,
            0 /* field index will be set at TABLE::sp_find_field_by_name */,
...
// a.b.c
class Item_splocal_row_field_table_by_name
    : public Item_splocal_row_field_by_name {
  List<ora_simple_ident_def> *m_arg_list;
  Item *m_number;
  sp_pcontext *m_pctx;

 public:
  Item_splocal_row_field_table_by_name(const Sp_rcontext_handler *rh,
                                       const Name_string sp_var_name,
                                       const Name_string sp_field_name,
                                       uint sp_var_idx,
                                       List<ora_simple_ident_def> *arg_list,
                                       Item *number, sp_pcontext *pctx,
                                       uint pos_in_q = 0, uint len_in_q = 0)
      : Item_splocal_row_field_by_name(rh, sp_var_name, sp_field_name,
                                       sp_var_idx, MYSQL_TYPE_NULL, pos_in_q,
                                       len_in_q),
        m_arg_list(arg_list),
        m_number(number),
        m_pctx(pctx) {}
  bool do_itemize(Parse_context *pc, Item **res) override;
  bool pre_fix_fields(Item **result);
  bool fix_fields(THD *thd, Item **) override;
  Item *this_item() override;
  const Item *this_item() const override;
  void print(const THD *, String *str,
  ...
  Item *make_item(ora_simple_ident_def *def_last, LEX_STRING *def_name_before,
                  Row_definition_list **cdf_list_before,
                  Row_definition_table_list **cdf_table_list_before);
  Item **this_item_addr(THD *thd, Item **item) override;
...
class Item_ident : public Item {
  ...
  */
  ...
  void set_field_name(const char *name_arg) { field_name = name_arg; }
...
class Item_ident_for_show final : public Item {
 public:
  ...
  String *val_str(String *str) override {
    String *result = field->val_str(str);
...
class Item_field_refcursor : public Item_field {
 ...
 public:
  ...
  ~Item_field_refcursor() override { field->~Field(); }
  ...
  bool modify_item_field_about_cursor(THD *thd, List<Create_field> *defs,
                                      sp_cursor *c, sp_rcontext *ctx) override;
  double val_real() override {
    assert(0);
    return 0;
  }
...
class Item_field_row : public Item_field {
 ...
 public:
  Item_field_row(Field *field_row)
      : Item_field(field_row),
        ...
        m_is_rowtype_item(field_row->is_rowtype_table()) {
    set_ora_type();
    reset_ora_table();
  }
  ~Item_field_row() override {
    if (row) row->mem_free();
    if (!m_is_refer_from_field_table) field->~Field();
  }
  Item_result result_type() const override { return ROW_RESULT; }
  ...
  bool modify_item_field_about_cursor(THD *thd, List<Create_field> *defs,
                                      sp_cursor *c, sp_rcontext *ctx) override;
  bool is_rowtype() override { return m_is_rowtype_item; }
 ...
 protected:
  ...
  bool m_is_rowtype_item;
};

/**
  Item_field for the type of table data type
*/
class Item_field_row_table : public Item_field_row {
 ...
 public:
  ...
  bool row_table_create_items(THD *thd,
                              Row_definition_table_list *list) override;
  ...
  Item *clone_default_value_from_record(THD *thd, Item *arg);
...
class Item_ref : public Item_ident {
  ...
  */
  ...
  bool pq_copy_from(THD *thd, Query_block *select, Item *item) override {
    ...
    assert(iref);
    m_ref_item = iref->m_ref_item;
    return Item_ident::pq_copy_from(thd, select, item);
  }
};
...
class Cached_item_str : public Cached_item {
 ...
 public:
  ...
  void set_tmp_value(String val) { tmp_value = val; }
  String get_tmp_value() { return tmp_value; }
};
...
class Cached_item_int : public Cached_item {
 ...
 public:
  ...
  bool cmp() override;
  void set_value(longlong val) { value = val; }
...
class Cached_item_decimal : public Cached_item {
 ...
 public:
  ...
  bool cmp() override;
  void set_value(my_decimal val) { value = val; }
...
class Item_cache_int : public Item_cache {
  ...
  */
  ...
  void set_value(longlong val_arg) {
    value = val_arg;
    value_cached = true;
  }
  double val_real() override;
...
class Item_cache_decimal final : public Item_cache {
 ...
 public:
  ...
  Item *pq_clone(THD *thd, Query_block *select) override;
  void set_value(my_decimal *value_arg) {
    decimal_value = *value_arg;
    value_cached = true;
  }
};
...
class Item_cache_str final : public Item_cache {
 ...
 public:
  Item_cache_str(const Item *item)
    ...
    char *str =
        current_thd->strmake(item->this_item()->get_udt_db_name().str,
                             item->this_item()->get_udt_db_name().length);
    m_db_name = LEX_STRING{str, strlen(str)};
    collation.set(item->collation);
  ...
  Item *pq_clone(THD *thd, Query_block *select) override;
  void set_value(String *arg_value) {
    value = &value_buff;
    value_cached = true;
    value_buff = *arg_value;
  }
...
class Item_cache_datetime : public Item_cache {
  ...
  */
  ...
  Item *pq_clone(THD *thd, Query_block *select) override;
  void set_value(longlong i_value, String *s_value, bool vcached,
                 bool svcached) {
    value_cached = vcached;
    str_value_cached = svcached;
    int_value = i_value;
    cached_string = *s_value;
  }
};
...
bool reorder_named_parameters(THD *thd, const char *type, Item **params,
                              uint params_count, sp_head *sp, bool *is_ordered);
