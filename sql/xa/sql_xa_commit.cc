...
#ifndef NDEBUG
  DBUG_EXECUTE_IF("wait_before_xa_guard", {
    if (m_xid->get_format_id() != 1) {
      const char act[] = "now SIGNAL reach_xa_guard WAIT_FOR end_xa_guard";
...
bool Sql_cmd_xa_commit::process_attached_xa_commit(THD *thd) const {
  ...
  if (xa_guard.failed()) {
    my_error(ER_XA_BLOCK_TIMEOUT_CLONE, MYF(0), "COMMIT");
    return true;
  }

  bool is_trans_finish = false;
  if (xid_state->xa_trans_rolled_back()) {
    /*
    ...
    */
...
#ifndef NDEBUG
    DBUG_SIGNAL_WAIT_FOR(thd, "xa_commit_wait", "reach_wait_sync",
                         "end_wait_sync");
    ...
    if (gtid_error) {
      ...
      */
      ...
      DBUG_EXECUTE_IF("simulate_xa_commit_log_failure", {
        res = true;
        is_trans_finish = true;
      });
  ...
  if (res == 0 || (m_xa_opt == XA_ONE_PHASE) || is_trans_finish) {
