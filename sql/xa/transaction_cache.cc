...
std::shared_ptr<Transaction_ctx> xa::Transaction_cache::find(
  ...
  auto &instance = xa::Transaction_cache::instance();
  MUTEX_LOCK(mutex_guard, &instance.m_LOCK_transaction_cache);
  return find_with_no_lock(xid, filter);
}

std::shared_ptr<Transaction_ctx> xa::Transaction_cache::find_with_no_lock(
    XID *xid, filter_predicate_t filter) {
  auto &instance = xa::Transaction_cache::instance();
...
void xa::Transaction_cache::lock_cache() {
  auto &instance = xa::Transaction_cache::instance();
  mysql_mutex_lock(&instance.m_LOCK_transaction_cache);
}

void xa::Transaction_cache::unlock_cache() {
  auto &instance = xa::Transaction_cache::instance();
  mysql_mutex_unlock(&instance.m_LOCK_transaction_cache);
}
