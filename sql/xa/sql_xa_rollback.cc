...
bool Sql_cmd_xa_rollback::process_attached_xa_rollback(THD *thd) const {
  ...
  */
...
#ifndef NDEBUG
  DBUG_SIGNAL_WAIT_FOR(thd, "xa_rollback_wait", "reach_wait_sync",
                       "end_wait_sync");
  if (DBUG_EVALUATE_IF("xa_rollback_sleep", true, false)) {
    sleep(5);
  }
#endif

  /* Inform clone handler of XA operation. */
  bool two_phase_xa = xid_state->has_state(XID_STATE::XA_PREPARED);
  Clone_handler::XA_Operation xa_guard(thd, two_phase_xa);
  if (xa_guard.failed()) {
    my_error(ER_XA_BLOCK_TIMEOUT_CLONE, MYF(0), "ROLLBACK");
    return true;
  }
  ...
  DBUG_SIGNAL_WAIT_FOR(thd, "test_xa_rollback", "reach_xa_rollback",
                       "end_xa_rollback");
  ...
  if (res == 0 || !xid_state->has_state(XID_STATE::XA_PREPARED)) {
    cleanup_trans_state(thd);

    xid_state->set_state(XID_STATE::XA_NOTR);
    xid_state->unset_binlogged();
  } else {
    xid_state->get_xid()->set_format_id(m_xid->get_format_id());
  }
...
bool Sql_cmd_xa_rollback::process_detached_xa_rollback(THD *thd) {
  ...
  /* Inform clone handler of XA operation. */
  Clone_handler::XA_Operation xa_guard(thd, true);
  if (xa_guard.failed()) {
    my_error(ER_XA_BLOCK_TIMEOUT_CLONE, MYF(0), "ROLLBACK");
  ...
  if (this->m_result == 0 || tc_log != &mysql_bin_log) {
