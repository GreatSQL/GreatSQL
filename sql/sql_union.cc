  ...
  for (Query_term *term : m_children) {
    ...
    /*
      ...
      for (Query_block *select = first_query_block(); select != nullptr;
           select =
               select
                   ->next_query_block()) {  // Termination condition at end of
      loop. JOIN *join = select->join; MaterializePathParameters::QueryBlock
      query_block; assert(join && join->is_optimized());
