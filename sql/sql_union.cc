  ...
  switch (qt->term_type()) {
    ...
    case QT_QUERY_BLOCK: {
      ...
      for (auto field : qt->query_block()->visible_fields()) {
        // a invalid charactor string would be cast to a empty string with
        // nullable is false, if MODE_EMPTYSTRING_EQUAL_NULL is on, empty string
        // will treated as NULL, however nullable is still false, this is a
        // incorrect statue and may cause some unexpected questions set nullable
        // to true if string get NULL result during MODE_EMPTYSTRING_EQUAL_NULL
        // is on
        if (thd->variables.sql_mode & MODE_EMPTYSTRING_EQUAL_NULL &&
            dynamic_cast<Item_string *>(field) != nullptr) {
          String str;
          auto *v = field->check_well_formed_result(&str, false, true);
          nullable[idx++] = (v == nullptr || v->is_empty());
        } else {
...
AccessPath *make_set_op_access_path(
    ...
    bool calc_found_rows, bool is_ties, bool is_percent) {
    ...
    case QT_EXCEPT: {
      ...
      if (!qts->m_is_materialized) {
        ...
        const ha_rows max_rows =
            push_limit_down ? qts->query_block()->get_limit(thd) +
                                  qts->query_block()->get_offset(thd, &isNull)
                            : HA_POS_ERROR;
      ...
      if (union_all_subpaths != nullptr) {
        ...
        for (size_t i = start_idx; i < qts->m_children.size(); ++i) {
          ...
          if (term->term_type() != QT_QUERY_BLOCK) {
            child_path =
                make_set_op_access_path(thd, nullptr, term, nullptr,
                                        calc_found_rows, is_ties, is_percent);
          }
...
void Query_expression::create_access_paths(THD *thd) {
  ...
  bool isNull{false};
  ha_rows offset = global_parameters()->get_offset(thd, &isNull);
  ...
  bool is_ties = global_parameters()->select_limit_ties;
  bool is_percent = global_parameters()->select_limit_percent;
  double limit_percent_value = 0.0;
  if (is_percent && !isNull) {
    limit_percent_value = global_parameters()->select_limit_percent_value;
  }
  ...
  if (streaming_allowed && (limit != HA_POS_ERROR || offset != 0) &&
    ...
    m_root_access_path = NewLimitOffsetAccessPath(
        ...
        /*reject_multiple_rows=*/false, &send_records, limit_percent_value,
        is_ties);
  }
}
...
bool Query_expression::ExecuteIteratorQuery(THD *thd) {
  ...
  if (thd->is_classic_protocol()) {
    ...
    if (net->mtr_hint && !thd->running_explain_analyze) {
      bool is_first = true;
      String str;

      str.append(STRING_WITH_LEN("--MTR-hint: order "));

      auto qb = first_query_block();
      auto order_list = qb->order_list;
      if (qb->master_query_expression()->is_set_operation())
        order_list = qb->master_query_expression()
                         ->query_term()
                         ->query_block()
                         ->order_list;

      for (ORDER *ord = order_list.first; ord; ord = ord->next) {
        Item *order_item = *ord->item;
        Item **select_item = nullptr;
        uint counter = 0;
        enum_resolution_type resolution(NOT_RESOLVED);

        if (order_item->hidden) continue;

        if (find_item_in_list(thd, order_item, fields, &select_item, &counter,
                              &resolution))
          continue; /* The item is not unique, or some other error occurred. */
        ...
        if (select_item) {
          ...
          str.append(std::to_string(counter + 1));
        }
      }

      if (!is_first) {
        // sql_print_information("Query_expression::ExecuteIteratorQuery(): %s",
        //                       str.c_ptr());
        assert(str.length() <= sizeof(net->mtr_hint_msg));
        my_stpncpy(net->mtr_hint_msg, (const char *)str.c_ptr(),
                   sizeof(net->mtr_hint_msg));
      } else {
        my_stpncpy(net->mtr_hint_msg, "--MTR-hint: order none",
                   sizeof(net->mtr_hint_msg));
      }
    }
  }
