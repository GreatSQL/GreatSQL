  ...
  for (Item_splocal **splocal = sp_vars_uses.begin();
    ...
    */
    ...
    if ((*splocal)->type() == Item::ORACLE_ROWTYPE_TABLE_ITEM ||
      ...
      String str{};
      (*splocal)->print(thd, &str, /*unused*/ QT_ORDINARY);
      res |= qbuf.append(str);
    } else
  ...
  */
  ...
  if (!parsing_failed) {
    ...
    if (sp->m_type == enum_sp_type::TRIGGER) {
      ...
      */
      /*
        we do not need to find trigger again, we can set trigger subject when
        sp_head been inited.
      */
      // Trigger *t = sp->m_trg_list->find_trigger(thd->lex->sphead->m_name);
      Trigger *t = sp->run_trigger;
      assert(t);
...
bool sp_lex_instr::validate_lex_and_execute_core(THD *thd, uint *nextp,
  ...
  while (true) {
    ...
    if (is_invalid() ||
        (!m_first_execution &&
         (m_lex->has_udf() || m_lex->has_udt_table() ||
      ...
      m_arena.set_state(Query_arena::STMT_INITIALIZED_FOR_SP);
...
///////////////////////////////////////////////////////////////////////////
// sp_instr_forall_fill_table implementation.
///////////////////////////////////////////////////////////////////////////
#ifdef HAVE_PSI_INTERFACE
PSI_statement_info sp_instr_forall_fill_table::psi_info = {
    0, "forall_fill_table", 0, PSI_DOCUMENT_ME};
#endif

bool sp_instr_forall_fill_table::execute(THD *thd, uint *nextp) {
  *nextp = get_ip() + 1;
  Item_field_row_table *record =
      (Item_field_row_table *)get_rcontext(thd)->get_item(m_offset);
  return record->get_row_table_field()->resolve_forall_table_record(thd,
                                                                    m_forall_i);
}

void sp_instr_forall_fill_table::print(const THD *, String *str) {
  /* set name@offset ... */
  size_t rsrv = SP_INSTR_UINT_MAXLEN + 22;
  ...
  qs_append(STRING_WITH_LEN("forall_fill_table "), str);
...
bool sp_instr_set::exec_core(THD *thd, uint *nextp) {
  ...
  *nextp = get_ip() + 1;
  ...
  if (!ctx->set_variable(thd, true, m_offset, &m_value_item)) return false;

  /* Failed to evaluate the value. Reset the variable to NULL. */

  if (ctx->set_variable(thd, true, m_offset, nullptr)) {
...
bool sp_instr_setup_row_field::execute(THD *thd, uint *nextp) {
  sp_rcontext *ctx = get_rcontext(thd);
  *nextp = get_ip() + 1;
  const Sp_udt_stmt_arena_holder sp_arena_holder(thd, ctx->callers_arena);
  ...
  if (item_table) {
    rc = item_table->row_table_create_items(
        thd, m_spv->field_def.ora_record.row_field_table_definitions());
  } else {
    rc = item->row_create_items(
        thd, m_spv->field_def.ora_record.row_field_definitions());
  }
  return rc;
}
...
bool sp_instr_set_row_field_table_by_name::exec_core(THD *thd, uint *nextp) {
  sp_rcontext *ctx = get_rcontext(thd);
  *nextp = get_ip() + 1;
  if (ctx->set_variable_row_table_by_name(thd, m_offset, m_field_name,
...
bool sp_instr_cpush::execute(THD *thd, uint *nextp) {
  *nextp = get_ip() + 1;
  ...
  sp_cursor *c = get_rcontext(thd)->get_cursor(m_cursor_idx);
  if (c) return false;

  bool rc = get_rcontext(thd)->push_cursor(this, m_cursor_idx);
  if (rc) return true;
  c = get_rcontext(thd)->get_cursor(m_cursor_idx);
  if (!c) {
...
bool sp_instr_cpush_rowtype::exec_core(THD *thd, uint *) {
  ...
  const Sp_udt_stmt_arena_holder sp_arena_holder(
      thd, get_rcontext(thd)->callers_arena);
  if (!c || c->open(thd)) {
    my_error(ER_SP_CURSOR_FAILED, MYF(0));
    goto finish;
  }
  ...
  if (c->export_structure(thd, &defs) ||
      get_rcontext(thd)->add_udt_to_sp_var_list(thd, defs))
    goto finish;
...
bool sp_instr_cpush_rowtype::copy_structure_from_return(
  ...
  if (!(pcursor->get_table_ref() || pcursor->get_row_definition_list() ||
        pcursor->get_cursor_rowtype().m_cursor_rowtype_offset != -1)) {
    ...
    if (pcursor->get_table_ref()) {
    ...
    } else if (pcursor->get_cursor_rowtype().m_cursor_rowtype_offset != -1) {
      sp_cursor *c_return = get_rcontext(thd)->get_cursor(
      ...
      if (c_return) {
        ...
        sp_pcursor *pcursor_def =
            pcursor->get_cursor_rowtype().m_ctx_cursor->find_cursor_parameters(
                pcursor->get_cursor_rowtype().m_cursor_rowtype_offset);
        if (c->set_return_table_from_pcursor(
                thd, pcursor_def,
                pcursor->get_cursor_rowtype().m_rh_cursor->get_rcontext(
                    thd->sp_runtime_ctx)))
          return true;
      }
    }
...
void sp_instr_cpush_rowtype::cleanup_mem_root() {
  Item *item = get_rcontext(current_thd)->get_item(m_for_var);
...
bool sp_instr_copen_for_sql::exec_core(THD *thd, uint *) {
  Item *item = get_rcontext(thd)->get_item(m_spv_idx);
...
bool sp_instr_copen_for_ident::exec_core(THD *thd, uint *nextp) {
  *nextp = get_ip() + 1;
  ...
  field_ref->set_cursor_return_type();
  bool rc = field_ref->m_cursor->open_for_ident(
      thd,
      m_rh_ident->get_rcontext(thd->sp_runtime_ctx)->get_item(m_sql_spv_idx));
  if (rc) {
    return rc;
  }
...
void sp_instr_copen_for_ident::print(const THD *, String *str) {
  ...
  sp_variable *var_sql = m_pctx_ident->find_variable(m_sql_spv_idx);
...
bool sp_instr_cpop::execute(THD *thd, uint *nextp) {
  get_rcontext(thd)->pop_cursors(m_count);
  *nextp = get_ip() + 1;
...
bool sp_instr_cursor_copy_struct::execute(THD *thd, uint *nextp) {
  ...
  *nextp = get_ip() + 1;
  ...
  // Get the cursor pointer.
  sp_pcursor *pcursor = m_ctx_cursor->find_cursor_parameters(m_cursor_idx);
  if (!pcursor) {
    my_error(ER_SP_CURSOR_MISMATCH, MYF(0), "cursor");
    return true;
  }
  ...
  if (pcursor->get_cursor_spv()) {
    Item_field_refcursor *item_cursor = dynamic_cast<Item_field_refcursor *>(
        m_rh_cursor->get_rcontext(thd->sp_runtime_ctx)
            ->get_item(pcursor->get_cursor_spv()->offset));
    return item_cursor->set_cursor_rowtype_table(
        thd, get_rcontext(thd)->get_item(m_var), pcursor->has_return_type(),
        item_cursor, get_rcontext(thd));
  }
  ...
  ret = item->modify_item_field_about_cursor(thd, &defs, c, get_rcontext(thd));
  c->m_is_cursor = m_is_cursor;
  return ret;
}

void sp_instr_cursor_copy_struct::print(const THD *, String *str) {
  const LEX_STRING *cursor_name = m_ctx_cursor->find_cursor(m_cursor_idx);
  ...
  *nextp = get_ip() + 1;
  ...
  if (m_cursor_spv) {
    Item_field_refcursor *item = dynamic_cast<Item_field_refcursor *>(
        m_cursor_spv->field_def.ora_record
            .get_spv_rcontext(thd->sp_runtime_ctx)
...
bool sp_instr_cfetch::execute(THD *thd, uint *nextp) {
  ...
  *nextp = get_ip() + 1;
  if (m_cursor_spv) {
    ...
    field->m_cursor->m_result.set_spvar_idents(m_ora_var_idents);
  ...
  c->m_result.set_spvar_idents(m_ora_var_idents);
...
#ifdef HAVE_PSI_INTERFACE
PSI_statement_info sp_instr_cfetch_bulk::psi_info = {
    0, "cfetch", PSI_FLAG_DISABLED | PSI_FLAG_UNTIMED, PSI_DOCUMENT_ME};
#endif
...
bool sp_instr_cfetch_bulk::execute(THD *thd, uint *nextp) {
  ...
  */
  ...
  if (m_cursor_spv) {
    ...
    m_varlist.head()
        ->field_def.ora_record.get_spv_rcontext(thd->sp_runtime_ctx)
        ->cleanup_record_variable_row_table(m_varlist.head()->offset);
    field->set_cursor_return_type();
