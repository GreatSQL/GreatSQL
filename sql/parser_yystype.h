...
struct Limit_options {
  ...
  Item *opt_offset;
  /*
    1 for "LIMIT offset,limit" and 0 for "LIMIT limit OFFSET offset"
    2 for "OFFSET FETCH"
  */
  int is_offset_first;
  bool percent;
...
struct spblock_handlers_st {
 public:
  int hndlrs;
  void init(int count) { hndlrs = count; }
};

struct spblock_st : public spblock_handlers_st {
 public:
  uint vars;
  uint conds;
  uint curs;
  ...
  void join(const spblock_st &b1, const spblock_st &b2) {
...
struct Oracle_sp_for_loop_bounds {
  uint offset;
  ...
  Oracle_sp_for_loop_bounds(uint m_offset, LEX_STRING m_cursor_name,
                            ...
                            bool m_is_select_loop, sp_pcontext *m_ctx,
...
struct Oracle_sp_for_loop_index_and_bounds {
  ...
  Oracle_sp_for_loop_index_and_bounds(
      ...
      : offset(oracle_sp_for_loop_bounds.offset),
        ...
        is_cursor(oracle_sp_for_loop_bounds.is_cursor),
        direction(oracle_sp_for_loop_bounds.direction),
...
struct Udt_table_type {
  ...
  Udt_table_type(enum_udt_table_of_type type_arg, ulonglong size_limit_arg)
      : type(type_arg), size_limit(size_limit_arg) {}
};

enum class plsql_cursor_attr_type {
...
struct Bipartite_name_ora {
  Bipartite_name bipartite_name;
  List<ora_simple_ident_def> *medium;
  Bipartite_name_ora(Bipartite_name m_bipartite_name,
                     List<ora_simple_ident_def> *m_medium)
      : bipartite_name(m_bipartite_name), medium(m_medium) {}
};
...
union MY_SQL_PARSER_STYPE {
  ...
  */
  ...
  struct spblock_handlers_st spblock_handlers;
  struct spblock_st spblock;
  sp_name *spname;
  LEX *lex;
  sp_head *sphead;
  ...
  Bipartite_name bipartite_name;
  Bipartite_name_ora *bipartite_name_ora;
  Oracle_sp_for_loop_bounds *oracle_sp_for_loop_bounds;
  Oracle_sp_for_loop_index_and_bounds *oracle_sp_for_loop_index_and_bounds;
  plsql_cursor_attr_type plsql_cursor_attr;
  Qualified_column_ident *qualified_column_ident;
  Row_definition_list *oracle_Row_definition_list;
  Create_field *oracle_create_field_decl;
  ...
  Materialized_view_opt *opt_materialized_view;
};

using ORA_SQL_PARSER_STYPE = MY_SQL_PARSER_STYPE;
