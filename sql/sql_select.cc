...
/**
  As qep_tab0 and qep_tab1 share the same table info, we only free these tables
  once and the free procedure is finished in qep_tab0.
*/
...
bool JOIN::make_leader_tables_info() {
  ...
  */
  ...
  /*
    ...
    workers's sum data
  */
  TABLE *table = create_tmp_table(thd, tmp_param, tmplist, nullptr, false, true,
                                  query_block->active_options(), HA_POS_ERROR,
                                  "", true, true);
  if (table == nullptr) {
    DBUG_RETURN(true);
  ...
  */
  ...
  if (change_to_use_tmp_fields(curr_fields, thd, ref_items[REF_SLICE_PQ_TMP],
...
bool JOIN::make_tmp_tables_info() {
  ...
  */
    ...
    */
    ...
    if (qep_tab &&
      ...
      /*
        ...
        with_ties or is_percent need (we need a full table scan)
      */
      pq_last_sort_idx = curr_tmp_table;
      if (sort_tab->filesort)
        sort_tab->filesort->limit =
  ...
  if (query_block->select_limit_ties) {
    for (auto ord = order_initial; ord; ord = ord->next) {
      auto *order_item = ord->item[0];
      Cached_item *tmp = new_Cached_item(thd, order_item->real_item());
      if (!tmp || order_fields.push_front(tmp)) return true;
    }
  }
