...
then
    go_out="$(getopt --options=iqdvj:m:t: \
        --longoptions=i686,quiet,debug,valgrind,with-jemalloc:,with-mecab:,with-ssl:,with-no-git:,local-boost:,tag: \
        --name="$(basename "$0")" -- "$@")"
    test $? -eq 0 || exit 1
    eval set -- $go_out
fi
...
(
    ...
    then
      ...
      mv boost_1_59_0 libboost
      rm -f boost_1_59_0.tar.gz
      cd "$WORKDIR_ABS/bld"
    else
      cd "$WORKDIR_ABS"
      tar -xzvf $WORKDIR_ABS/../../boost_1_59_0.tar.gz
      mv boost_1_59_0 libboost
      cd "$WORKDIR_ABS/bld"
    fi
