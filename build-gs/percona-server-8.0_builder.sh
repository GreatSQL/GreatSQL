...
get_sources(){
    ...
    #fi
    env
    cmake3 ${CURDIR}/..  -DWITH_SSL=system -DWITH_ZLIB=bundled -DWITH_ROCKSDB=OFF -DGROUP_REPLICATION_WITH_ROCKSDB=OFF -DALLOW_NO_SSE42=ON -DFORCE_INSOURCE_BUILD=1 -DWITH_AUTHENTICATION_LDAP=OFF
    make dist
    #
...
build_rpm(){
    ...
    if [ "${RHEL}" -lt 8 ]; then
      echo "build and install custom openssl"
      (
        if [ ! -d openssl ]; then
          git clone http://gitee.com/mirrors/openssl.git
          cd openssl
          git checkout 3bd75cfa  # OpenSSL_1_1_1-stable
          ./config --prefix=${WORKDIR}/ssl/
          make -j10
          make install
       else
          cd openssl
    ...
    if [ ${ARCH} = x86_64 ]; then
      if [ "${RHEL}" -lt 8 ]; then
        rpmbuild --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .el${RHEL}" --define "with_rocksdb 0"  --define "with_ssl ${WORKDIR}/ssl/" --define "with_mecab ${MECAB_INSTALL_DIR}/usr" --define 'with_debuginfo 1' --rebuild rpmbuild/SRPMS/${SRCRPM}
      else
        rpmbuild --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .el${RHEL}" --define "with_rocksdb 0"  --define "with_mecab ${MECAB_INSTALL_DIR}/usr" --define 'with_debuginfo 1' --rebuild rpmbuild/SRPMS/${SRCRPM}
      fi
    else
      if [ "${RHEL}" -lt 8 ]; then
        rpmbuild --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .el${RHEL}" --define "with_rocksdb 0"  --define "with_ssl ${WORKDIR}/ssl/" --define "with_mecab ${MECAB_INSTALL_DIR}/usr" --define 'with_debuginfo 1' --rebuild rpmbuild/SRPMS/${SRCRPM}
...
build_tarball(){
    ...
    if [ "x$WITH_SSL" = "x1" ]; then
        CMAKE_OPTS="-DWITH_ROCKSDB=0 -DGROUP_REPLICATION_WITH_ROCKSDB=OFF -DALLOW_NO_SSE42=ON -DINSTALL_LAYOUT=STANDALONE -DWITH_AUTHENTICATION_LDAP=OFF -DWITH_SSL=${WORKDIR}/ssl " bash -xe ${SOURCEDIR}/build-gs/build-binary.sh --with-mecab="${MECAB_INSTALL_DIR}/usr" --with-jemalloc="${JEMALLOC_DIR}" --with-no-git=$NO_GIT_INFO ${BUILD_OPT} ${EXTRA_BUILD_OPTION} TARGET
        DIRNAME="yassl"
    else
        CMAKE_OPTS="-DWITH_ROCKSDB=0 -DGROUP_REPLICATION_WITH_ROCKSDB=OFF -DALLOW_NO_SSE42=ON -DWITH_AUTHENTICATION_LDAP=OFF" bash -xe ${SOURCEDIR}/build-gs/build-binary.sh --with-mecab="${MECAB_INSTALL_DIR}/usr" --with-jemalloc="${JEMALLOC_DIR}" --with-no-git=$NO_GIT_INFO ${BUILD_OPT} ${EXTRA_BUILD_OPTION} TARGET
        DIRNAME="tarball"
    fi
