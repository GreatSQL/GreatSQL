...
get_sources(){
    ...
    if [ $LOCAL_BOOST = 0 ]
    then
      cmake3 ${CURDIR}/../  -DWITH_SSL=system -DWITH_ZLIB=bundled -DWITH_TOKUDB=0 -DFORCE_INSOURCE_BUILD=1 -DDOWNLOAD_BOOST=1 -DWITH_BOOST=${CURDIR}/boost_1_73_0.tar.gz
    else
      cmake3 ${CURDIR}/../  -DWITH_SSL=system -DWITH_ZLIB=bundled -DWITH_TOKUDB=0 -DFORCE_INSOURCE_BUILD=1 -DDOWNLOAD_BOOST=0 -DWITH_BOOST=${CURDIR}/boost_1_73_0.tar.gz
    fi
    make dist
    #
...
build_tarball(){
    ...
    if [ "x$WITH_SSL" = "x1" ]; then
        CMAKE_OPTS="-DWITH_ROCKSDB=1 -DGROUP_REPLICATION_WITH_ROCKSDB=ON -DALLOW_NO_SSE42=ON -DINSTALL_LAYOUT=STANDALONE -DWITH_SSL=${WORKDIR}/ssl/ " bash -xe ${SOURCEDIR}/build-gs/build-binary.sh --with-mecab="${MECAB_INSTALL_DIR}/usr" --with-jemalloc="${JEMALLOC_DIR}" --with-no-git=$NO_GIT_INFO --local-boost=$LOCAL_BOOST ${BUILD_OPT} TARGET
        DIRNAME="yassl"
    else
        CMAKE_OPTS="-DWITH_ROCKSDB=1 -DGROUP_REPLICATION_WITH_ROCKSDB=ON -DALLOW_NO_SSE42=ON" bash -xe ${SOURCEDIR}/build-gs/build-binary.sh --with-mecab="${MECAB_INSTALL_DIR}/usr" --with-jemalloc="${JEMALLOC_DIR}" --with-no-git=$NO_GIT_INFO --local-boost=$LOCAL_BOOST ${BUILD_OPT} TARGET
        DIRNAME="tarball"
    fi
