...
add_percona_yum_repo(){
    ...
    then
        curl -o /etc/yum.repos.d/percona-dev.repo https://jenkins.percona.com/yum-repo/percona-dev.repo -k
...
get_sources(){
    ...
    cat MYSQL_VERSION > ${WORKDIR}/percona-server-5.7.properties
    echo "REVISION=${REVISION}" >> ${WORKDIR}/percona-server-5.7.properties
    BRANCH_NAME="${BRANCH}"
    echo "BRANCH_NAME=${BRANCH_NAME}" >> ${WORKDIR}/percona-server-5.7.properties
    export PRODUCT=Percona-Server-${MYSQL_VERSION_MAJOR}.${MYSQL_VERSION_MINOR}
    echo "PRODUCT=Percona-Server-${MYSQL_VERSION_MAJOR}.${MYSQL_VERSION_MINOR}" >> ${WORKDIR}/percona-server-5.7.properties
    export PRODUCT_FULL=${PRODUCT}.${MYSQL_VERSION_PATCH}${MYSQL_VERSION_EXTRA}
    echo "PRODUCT_FULL=${PRODUCT}.${MYSQL_VERSION_PATCH}${MYSQL_VERSION_EXTRA}" >> ${WORKDIR}/percona-server-5.7.properties
    echo "BUILD_NUMBER=${BUILD_NUMBER}" >> ${WORKDIR}/percona-server-5.7.properties
    echo "BUILD_ID=${BUILD_ID}" >> ${WORKDIR}/percona-server-5.7.properties
    #echo "PERCONAFT_REPO=${PERCONAFT_REPO}" >> ${WORKDIR}/percona-server-5.7.properties
    #echo "PERCONAFT_BRANCH=${PERCONAFT_BRANCH}" >> ${WORKDIR}/percona-server-5.7.properties
    #echo "TOKUBACKUP_REPO=${TOKUBACKUP_REPO}" >> ${WORKDIR}/percona-server-5.7.properties
    #echo "TOKUBACKUP_BRANCH=${TOKUBACKUP_BRANCH}" >> ${WORKDIR}/percona-server-5.7.properties
    #echo "TOKUDB_VERSION=${MYSQL_VERSION_MAJOR}.${MYSQL_VERSION_MINOR}.${MYSQL_VERSION_PATCH}${MYSQL_VERSION_EXTRA}" >> ${WORKDIR}/percona-server-5.7.properties
    BOOST_PACKAGE_NAME=$(cat cmake/boost.cmake|grep "SET(BOOST_PACKAGE_NAME"|awk -F '"' '{print $2}')
    echo "BOOST_PACKAGE_NAME=${BOOST_PACKAGE_NAME}" >> ${WORKDIR}/percona-server-5.7.properties
    echo "RPM_RELEASE=${RPM_RELEASE}" >> ${WORKDIR}/percona-server-5.7.properties
    echo "DEB_RELEASE=${DEB_RELEASE}" >> ${WORKDIR}/percona-server-5.7.properties

    if [ -z "${DESTINATION:-}" ]; then
        export DESTINATION=experimental
    fi
    TIMESTAMP=$(date "+%Y%m%d-%H%M%S")
    echo "DESTINATION=${DESTINATION}" >> ${WORKDIR}/percona-server-5.7.properties
    echo "UPLOAD=UPLOAD/${DESTINATION}/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${BRANCH_NAME}/${REVISION}/${TIMESTAMP}" >> ${WORKDIR}/percona-server-5.7.properties
    ...
    if [ $LOCAL_BOOST = 0 ]
    then
      cmake3 ${CURDIR}/../  -DWITH_SSL=system -DWITH_ZLIB=bundled -DWITH_TOKUDB=0 -DFORCE_INSOURCE_BUILD=1 -DDOWNLOAD_BOOST=1 -DWITH_BOOST=${CURDIR}/boost_1_59_0.tar.gz
    else
      cmake3 ${CURDIR}/../  -DWITH_SSL=system -DWITH_ZLIB=bundled -DWITH_TOKUDB=0 -DFORCE_INSOURCE_BUILD=1 -DDOWNLOAD_BOOST=0 -DWITH_BOOST=${CURDIR}/boost_1_59_0.tar.gz
    fi
    make dist
    #
    ...
    rsync -av ${CURDIR}/../storage/rocksdb/third_party/zstd/ ${PSDIR}/storage/rocksdb/third_party/zstd --exclude .git
    ...
    cd ${WORKDIR}/mybuild
    if [ $SRPM = 0 ]; then
      tar --owner=0 --group=0 --exclude=.bzr --exclude=.git --exclude=.gitconfig --exclude=.gitlab-ci.yml --exclude=.gitmodules -czf ${PSDIR}.tar.gz ${PSDIR}
    else
      tar --owner=0 --group=0 --exclude=.bzr --exclude=.git --exclude=.gitconfig --exclude=.gitlab-ci.yml --exclude=.gitmodules  -czf ${PSDIR}.tar.gz ${PSDIR}
    fi
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
        ...
        if [ "${RHEL}" -lt 8 ]; then
          yum -y install libzstd-devel zstd-devel
        else
...
build_srpm(){
    ...
    if [ $LOCAL_BOOST = 0 ]
    then
      wget https://jenkins.percona.com/downloads/boost/boost_1_59_0.tar.gz
    else
      cp ${CURDIR}/boost_1_59_0.tar.gz .
    fi
    ...
    tar vxzf ${WORKDIR}/${TARFILE} --wildcards '*/build-gs/rpm/my_config.h' --strip=3
    #
    cd ${WORKDIR}
    #
...
build_tarball(){
    ...
    if [ "x$WITH_SSL" = "x1" ]; then
        CMAKE_OPTS="-DWITH_ROCKSDB=0 -DGROUP_REPLICATION_WITH_ROCKSDB=OFF -DALLOW_NO_SSE42=ON -DINSTALL_LAYOUT=STANDALONE -DWITH_SSL=${WORKDIR}/ssl/ " bash -xe ${SOURCEDIR}/build-gs/build-binary.sh --with-mecab="${MECAB_INSTALL_DIR}/usr" --with-jemalloc="${JEMALLOC_DIR}" --with-no-git=$NO_GIT_INFO --local-boost=$LOCAL_BOOST ${BUILD_OPT} TARGET
        DIRNAME="yassl"
    else
        CMAKE_OPTS="-DWITH_ROCKSDB=0 -DGROUP_REPLICATION_WITH_ROCKSDB=OFF -DALLOW_NO_SSE42=ON" bash -xe ${SOURCEDIR}/build-gs/build-binary.sh --with-mecab="${MECAB_INSTALL_DIR}/usr" --with-jemalloc="${JEMALLOC_DIR}" --with-no-git=$NO_GIT_INFO --local-boost=$LOCAL_BOOST ${BUILD_OPT} TARGET
        DIRNAME="tarball"
    fi
...
CURDIR=$(pwd)
VERSION_FILE=$CURD/..//percona-server-5.7.properties
args=
WORKDIR=
SRPM=0
SDEB=0
