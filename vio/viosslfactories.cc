...
#ifdef SSL_GM
/************************ VioSSLFd **********************************/
static bool split_string_by_space(const std::string &str, std::string &str1,
                                  std::string &str2) {
  auto pos = str.find(' ');
  if (pos == str.npos || pos == 0 || pos == str.size()) return true;
  str1 = str.substr(0, pos);
  str2 = str.substr(pos + 1);
  if (str2.find(' ') != str2.npos) return true;
  return false;
}

static int set_sign_cert_key_stuff(SSL_CTX *ctx, const char *cert,
                                   const char *key) {
  if (!SSL_CTX_use_sign_certificate_file(ctx, cert, SSL_FILETYPE_PEM)) {
    DBUG_PRINT("error", ("error setting sign certificate\n"));
    return 0;
  }

  if (!SSL_CTX_use_sign_PrivateKey_file(ctx, key, SSL_FILETYPE_PEM)) {
    DBUG_PRINT("error", ("error setting sign private key\n"));
    return 0;
  }
...
static int set_enc_cert_key_stuff(SSL_CTX *ctx, const char *cert,
                                  const char *key) {
  if (!SSL_CTX_use_enc_certificate_file(ctx, cert, SSL_FILETYPE_PEM)) {
    DBUG_PRINT("error", ("error setting enc certificate\n"));
    return 0;
  }

  if (!SSL_CTX_use_enc_PrivateKey_file(ctx, key, SSL_FILETYPE_PEM)) {
    DBUG_PRINT("error", ("error setting enc private key\n"));
    return 0;
  }
...
static struct st_VioSSLFd *new_VioSSLFd(
  ...
  std::string sign_cert_file, enc_cert_file, sign_key_file, enc_key_file;
  ...
  if (!(ssl_fd = ((struct st_VioSSLFd *)my_malloc(
    ...
    return nullptr;
#ifdef SSL_GM
  if (enable_gm_ssl || gm_ssl) {
    ssl_fd->ssl_context =
        SSL_CTX_new(is_client ? NTLS_client_method() : NTLS_server_method());
  } else {
    ssl_fd->ssl_context = SSL_CTX_new(is_client ?
#ifdef HAVE_TLSv13
                                                TLS_client_method()
                                                : TLS_server_method()
#else /* HAVE_TLSv13 */
  ...
  */
  ...
  if (enable_gm_ssl || gm_ssl) {
    if (cipher == nullptr) {
      tls12_cipher_list = "ALL";
    } else
      tls12_cipher_list = cipher;
  } else {
    if (cipher == nullptr) {
...
#ifdef SSL_GM
  if (enable_gm_ssl || gm_ssl) SSL_CTX_enable_ntls(ssl_fd->ssl_context);
#endif /* SSL_GM */
...
#ifdef SSL_GM
  if (!(enable_gm_ssl || gm_ssl) || !cert_file || !key_file ||
      split_string_by_space(cert_file, sign_cert_file, enc_cert_file) ||
      split_string_by_space(key_file, sign_key_file, enc_key_file)) {
    if (vio_set_cert_stuff(ssl_fd->ssl_context, cert_file, key_file, error)) {
      DBUG_PRINT("warning", ("vio_set_cert_stuff failed"));
      goto error;
    }
  } else {
    if (!set_sign_cert_key_stuff(ssl_fd->ssl_context, sign_cert_file.c_str(),
                                 sign_key_file.c_str())) {
      DBUG_PRINT("warning", ("vio_set_sign_cert_stuff failed"));
      *error = SSL_INITERR_CERT;
      goto error;
    }
    if (!set_enc_cert_key_stuff(ssl_fd->ssl_context, enc_cert_file.c_str(),
                                enc_key_file.c_str())) {
      DBUG_PRINT("warning", ("vio_set_enc_cert_stuff failed"));
      *error = SSL_INITERR_CERT;
      goto error;
    }
  }
