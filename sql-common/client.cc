...
// cppcheck-suppress-macro nullPointer
#define STATE_DATA(M) \
      ...
      if (option[0][0] == '-' && option[0][1] == '-') {
        ...
        switch (find_type(*option + 2, &option_types, FIND_TYPE_BASIC)) {
          ...
          case OPT_socket:
            ...
            break;
          case OPT_mtr_hint:
            options->mtr_hint = true;
#ifdef WITH_MTR_HINT
            options->client_flag |= CLIENT_MTR_HINT;
#endif
            break;
          case OPT_compress:
...
#ifdef WITH_MTR_HINT
  /* read mtr_hint_msg */
  if (packet_error == cli_safe_read(mysql, nullptr)) return 1;
  pos = (uchar *)mysql->net.read_pos;
  auto mtr_hint_msg_size = (size_t)net_field_length(&pos);
  if (DISABLED_MTR_HINT_MSG_LENGTH != mtr_hint_msg_size) {
    if (packet_error == cli_safe_read(mysql, nullptr)) return 1;
    pos = (uchar *)mysql->net.read_pos;
    assert(mtr_hint_msg_size <= sizeof(mysql->net.mtr_hint_msg));
    memcpy(mysql->net.mtr_hint_msg, pos, mtr_hint_msg_size);
  }
#endif
...
#ifdef WITH_MTR_HINT
/* read mtr_hint_msg info in async way */
static net_async_status read_mtr_hint_nonblocking(MYSQL *mysql, int *res) {
  MYSQL_ASYNC *async_data = ASYNC_DATA(mysql);
  ulong pkt_len;
  uchar *pos;

  if (async_data->async_mtr_hint_state == MTR_HINT_IDLE) {
    if (cli_safe_read_nonblocking(mysql, nullptr, &pkt_len) ==
        NET_ASYNC_NOT_READY) {
      return NET_ASYNC_NOT_READY;
    }
    ...
    pos = (uchar *)mysql->net.read_pos;
    async_data->async_mtr_hint_length = (size_t)net_field_length(&pos);
    async_data->async_mtr_hint_state = MTR_HINT_READ_MSG;

    if (DISABLED_MTR_HINT_MSG_LENGTH == async_data->async_mtr_hint_length) {
      async_data->async_mtr_hint_state = MTR_HINT_COMPLETE;
      *res = 0;
      return NET_ASYNC_COMPLETE;
    };
  }

  if (async_data->async_mtr_hint_state == MTR_HINT_READ_MSG) {
    if (cli_safe_read_nonblocking(mysql, nullptr, &pkt_len) ==
        NET_ASYNC_NOT_READY) {
      return NET_ASYNC_NOT_READY;
    }
    ...
    pos = (uchar *)mysql->net.read_pos;
    assert(async_data->async_mtr_hint_length <=
           sizeof(mysql->net.mtr_hint_msg));
    memcpy(mysql->net.mtr_hint_msg, pos, async_data->async_mtr_hint_length);
    async_data->async_mtr_hint_state = MTR_HINT_COMPLETE;
    *res = 0;
    return NET_ASYNC_COMPLETE;
  }
...
#endif
    {"gb18030-2022", "gb18030-2022", my_cs_exact},
...
void st_mysql_load_balance_options::cleanup_hosts() {
  if (load_balance_hosts) {
    my_free(load_balance_hosts);
    load_balance_hosts = nullptr;
  }
  available_hosts.clear();
  block_hosts.clear();
  if (hosts) {
    for (size_t i = 0; i < hosts_cnt; ++i) {
      if (hosts[i].host) {
        my_free(hosts[i].host);
        hosts[i].host = nullptr;
      }
    }
    my_free(hosts);
    hosts = nullptr;
  }
  if (load_balance_status) {
    my_free(load_balance_status);
    load_balance_status = nullptr;
    load_balance_status_len = 0;
  }
  hosts_cnt = 0;
  next_conn_host_idx = 0;
}

int st_mysql_load_balance_options::set_hosts(const char *arg) {
  int ret = 0;
  auto generate_ip_port = [](const std::string &host, char **ip,
                             unsigned int &port) {
    int result = 0;
    do {
      size_t offset = host.find_last_of(":");
      if (offset == std::string::npos) {
        result = CR_LOAD_BALANCE_HOSTS_INVALID;
        break;
      }
      *ip = my_strdup(key_memory_mysql_options, host.substr(0, offset).c_str(),
                      MYF(MY_WME));
      if (++offset == host.size()) {
        result = CR_LOAD_BALANCE_HOSTS_INVALID;
        break;
      }
      /* check ip, ipv4/ipv6/hostname is valid. */
      struct addrinfo hints;
      struct addrinfo *host_addrinfo = nullptr;
      ...
      int ip_check_result = getaddrinfo(*ip, nullptr, &hints, &host_addrinfo);
      if (host_addrinfo) freeaddrinfo(host_addrinfo);
      if (0 != ip_check_result) {
        result = CR_LOAD_BALANCE_HOSTS_INVALID;
        break;
      }
      /* check port */
      char *p_start = const_cast<char *>(host.substr(offset).c_str());
      char *p_end = p_start;
      port = strtoull(p_start, &p_end, 10);
      if (p_end != p_start + host.size() - offset || errno == ERANGE ||
          port > 65535) {
        result = CR_LOAD_BALANCE_HOSTS_INVALID;
        break;
      }
  ...
  std::string opt_str(arg);
  // remove all the space
  opt_str.erase(std::remove_if(opt_str.begin(), opt_str.end(), isspace),
                opt_str.end());
  std::vector<std::string> host_vec;
  size_t pervious = 0;
  size_t current = opt_str.find_first_of(",");
  while (current != std::string::npos) {
    if (current > pervious) {
      host_vec.emplace_back(opt_str.substr(pervious, current - pervious));
    }
    pervious = current + 1;
    current = opt_str.find_first_of(",", pervious);
  }
  if (pervious != opt_str.size()) {
    host_vec.emplace_back(opt_str.substr(pervious));
  }

  next_conn_host_idx = 0;
  hosts_cnt = host_vec.size();
  available_hosts.reserve(hosts_cnt);
  std::set<std::string> host_set;

  do {
    load_balance_hosts = my_strdup(key_memory_mysql_options, arg, MYF(MY_WME));
    if (nullptr == load_balance_hosts) {
      ret = CR_LOAD_BALANCE_ALLOC_MEM_FAILED;
      break;
    }
    void *rawmem =
        my_malloc(key_memory_mysql_options,
                  sizeof(load_balance_host) * hosts_cnt, MYF(MY_WME));
    if (nullptr == rawmem) {
      ret = CR_LOAD_BALANCE_ALLOC_MEM_FAILED;
      break;
    }
    hosts = new (rawmem) load_balance_host[hosts_cnt];
    for (size_t i = 0; i < hosts_cnt; ++i) {
      if ((ret =
               generate_ip_port(host_vec[i], &(hosts[i].host), hosts[i].port)))
        break;
      // remove duplicate host
      if (host_set.find(host_vec[i]) == host_set.end()) {
        available_hosts.emplace_back(&(hosts[i]));
        host_set.insert(host_vec[i]);
      }
    }
  } while (false);
  if (ret) {
  ...
  /* compute random seed */
  unsigned int pid = static_cast<unsigned int>(syscall(SYS_gettid));
  // random_seed = ((time(NULL) & 0x00000FFF) << 20) | (pid & 0x000FFFFF);
  random_seed = time(NULL) ^ pid;
  srand(random_seed);
  return ret;
}

static int set_load_balance_option_hosts(MYSQL *mysql, const char *arg) {
  int ret = 0;
  do {
    if (mysql->options.load_balance) {
      mysql->options.load_balance->cleanup_hosts();
    } else {
      ALLOCATE_LOAD_BALANCE(&mysql->options);
      if (nullptr == mysql->options.load_balance) {
        ret = CR_LOAD_BALANCE_ALLOC_MEM_FAILED;
        break;
      }
    }
    ...
    if (nullptr == arg) {
      ret = CR_LOAD_BALANCE_HOSTS_INVALID;
      break;
    }
    ret = mysql->options.load_balance->set_hosts(arg);

  } while (false);

  if (ret) set_mysql_error(mysql, ret, unknown_sqlstate);
  return ret;
}

static void move_timeout_block_host_to_available_list(MYSQL *mysql) {
  unsigned long long begin = my_micro_time();
  unsigned long long now = my_milli_time();
  auto &block_hosts = mysql->options.load_balance->block_hosts;
  std::list<load_balance_host *>::iterator block_it = block_hosts.begin();
  for (; block_it != block_hosts.end();) {
    if (now - (*block_it)->invalid_time >=
        mysql->options.load_balance->blocklist_timeout) {
      mysql->options.load_balance->available_hosts.emplace_back(*block_it);
      block_it = block_hosts.erase(block_it);
    } else {
      ++block_it;
    }
  }
...
/* if blocklist_timeout != 0, move host to block_host_list */
static void move_host_to_block_list(MYSQL *mysql) {
  auto &host = mysql->options.load_balance->current_conn_host;
  if (nullptr == host) return;
  unsigned long long begin = my_micro_time();
  if (mysql->options.load_balance->blocklist_timeout != 0) {
    unsigned long long now = my_milli_time();
    auto &avliable_hosts = mysql->options.load_balance->available_hosts;
    std::vector<load_balance_host *>::iterator it = avliable_hosts.begin();
    for (; it != avliable_hosts.end();) {
      if (0 == strcmp((*it)->host, host->host) && (*it)->port == host->port) {
        (*it)->invalid_time = now;
        mysql->options.load_balance->block_hosts.insert(
            mysql->options.load_balance->block_hosts.begin(), *it);
        it = avliable_hosts.erase(it);
        break;
      } else {
        ++it;
      }
  ...
  ++host->connect_fail_cnt;
  mysql->options.load_balance->current_conn_host = nullptr;
  mysql->options.load_balance->mark_host_block_used_time +=
      my_micro_time() - begin;
}

static load_balance_host *mysql_load_balance_pick_host_random(MYSQL *mysql,
                                                              bool &last_one) {
  if (mysql->options.load_balance->available_hosts.empty()) {
    last_one = true;
    return nullptr;
  }
  int host_index = rand() % mysql->options.load_balance->available_hosts.size();
  last_one = mysql->options.load_balance->available_hosts.size() == 1;
  return mysql->options.load_balance->available_hosts[host_index];
}

static load_balance_host *mysql_load_balance_pick_host_round_robin(
    MYSQL *mysql, bool &last_one) {
  if (mysql->options.load_balance->available_hosts.empty()) {
    last_one = true;
    return nullptr;
  }
  last_one = mysql->options.load_balance->available_hosts.size() == 1;
  int host_index = (mysql->options.load_balance->next_conn_host_idx++) %
                   mysql->options.load_balance->available_hosts.size();
  return mysql->options.load_balance->available_hosts[host_index];
}

static load_balance_host *mysql_load_balance_pick_host(MYSQL *mysql,
                                                       bool check_block_host,
                                                       bool &last_one) {
  load_balance_host *host = nullptr;
  last_one = false;
  if (check_block_host) move_timeout_block_host_to_available_list(mysql);
  switch (mysql->options.load_balance->strategy) {
    case LOAD_BALANCE_RANDOM:
      host = mysql_load_balance_pick_host_random(mysql, last_one);
      break;
    case LOAD_BALANCE_ROUND_ROBIN:
      host = mysql_load_balance_pick_host_round_robin(mysql, last_one);
      break;
    default:
      assert(0);
  }
  if (nullptr != host) ++host->connect_cnt;
  mysql->options.load_balance->current_conn_host = host;
  return host;
}

static void reset_mysql_for_load_balance(MYSQL *mysql) {
  bool old_free_me = mysql->free_me;
  bool old_free_options = mysql->free_options;
  mysql->free_me = false;
  mysql->free_options = false;
  mysql_close(mysql);
  /* free old connection ssl_session_data */
  if (mysql->options.extension) {
    my_free(mysql->options.extension->ssl_session_data);
    mysql->options.extension->ssl_session_data = nullptr;
  ...
  memset(&mysql->net, 0, sizeof(mysql->net));
  mysql->free_me = old_free_me;
  mysql->free_options = old_free_options;
  mysql->client_flag = 0;
  mysql->port = 0;
  mysql->host = nullptr;
  mysql->server_version = nullptr;
  mysql->server_capabilities = 0;
  mysql->protocol_version = 0;
  mysql->server_status = 0;
  mysql->server_language = 0;
  mysql->warning_count = 0;
  memset(mysql->scramble, 0, sizeof(mysql->scramble));
}

static MYSQL *mysql_connect_load_balance(MYSQL *mysql, const char *user,
                                         const char *passwd, const char *db,
                                         ulong client_flag) {
  DBUG_TRACE;
  MYSQL *mysql_ret = nullptr;
  load_balance_host *host = nullptr;
  bool last_avliable_host = false;
  mysql_async_connect ctx;
  memset(&ctx, 0, sizeof(ctx));
  move_timeout_block_host_to_available_list(mysql);
  if (!mysql->options.load_balance->reconnecting)
    ++mysql->options.load_balance->external_connect_cnt;
  size_t retry_cnt = mysql->options.load_balance->hosts_cnt;

  while (retry_cnt--) {
    if (mysql->options.load_balance->reconnecting) {
      host = mysql->options.load_balance->current_conn_host;
      ++host->reconnect_cnt;
    } else {
      host = mysql_load_balance_pick_host(mysql, false, last_avliable_host);
      if (nullptr != host) reset_mysql_for_load_balance(mysql);
    }
    if (nullptr == host) {
      break;
    }
    ...
    ctx.client_flag =
        mysql->options.client_flag | client_flag | CLIENT_REMEMBER_OPTIONS;
    ctx.ssl_state = SSL_NONE;
    mysql_ret = (*mysql->methods->connect_method)(&ctx);
    if (nullptr == mysql_ret && !mysql->options.load_balance->reconnecting)
      move_host_to_block_list(mysql);
    if (nullptr != mysql_ret || last_avliable_host ||
        mysql->options.load_balance->reconnecting)
      break;
  }

  if (mysql->options.load_balance->reconnecting)
    mysql->options.load_balance->reconnecting = false;
  return mysql_ret;
}

static net_async_status STDCALL mysql_connect_nonblocking_load_balance(
    MYSQL *mysql, const char *user, const char *passwd, const char *db,
    ulong client_flag) {
  DBUG_TRACE;

  mysql_state_machine_status status;
  mysql_async_connect *ctx = nullptr;
  load_balance_host *host = nullptr;
  bool last_host = false;
  bool first_pick_host = true;
  size_t retry_cnt = mysql->options.load_balance->hosts_cnt;

  while (retry_cnt--) {
    ctx = ASYNC_DATA(mysql)->connect_context;
    if (!ctx) {
      if (first_pick_host)
        ++mysql->options.load_balance->external_connect_nonblocking_cnt;

      host = mysql_load_balance_pick_host(mysql, first_pick_host, last_host);
      first_pick_host = false;
      if (nullptr == host) {
        return NET_ASYNC_ERROR;
      }
      reset_mysql_for_load_balance(mysql);
    ...
    /*
      Continue to loop When different state returns STATE_MACHINE_CONTINUE,
      which means more work has to be done immediately and should not return to
      the caller.
    */
    do {
      status = ctx->state_function(ctx);
    ...
    if (status == STATE_MACHINE_FAILED) {
      ...
      end_server(mysql);
      /* connect_context will be freed in mysql_close_free */
      mysql_close_free(mysql);
      /* attempt to connect other host */
      continue;
    }
    /*
...
static std::string load_balance_strategy_to_str(
    mysql_load_balance_strategy strategy) {
  std::string ret;
  switch (strategy) {
    case LOAD_BALANCE_RANDOM:
      ret = "RANDOM";
...
#define LOAD_BALANCE_STATUS_SUMMARY                                          \
  "load balance status\nhosts: %s\nload balance strategy: %s, random seed: " \
  "%u\n"

#define LOAD_BALANCE_STATUS_CONNECTED "current connection ip: %s, port: %u\n"

#define LOAD_BALANCE_STATUS_NOT_CONNECTED "current no connection\n"

#define LOAD_BALANCE_STATUS_CONN_SUMMARY                                 \
  "external blocking connect count: %llu, external nonblocking connect " \
  "count: %llu\n"

#define LOAD_BALANCE_STATUS_AVLIABLE_HOST_TITLE "avliable host info\n"

#define LOAD_BALANCE_STATUS_AVLIABLE_HOST_DETAIL                              \
  "avliable host %s:%u internal connect count: %llu, connect success count: " \
  "%llu, connect fail count: %llu, reconnect count: %llu\n"

#define LOAD_BALANCE_STATUS_BLOCK_HOST_TITLE                              \
  "block host info\nblock host timeout: %llu milliseconds, current time " \
  "epoch: %llu\n"

#define LOAD_BALANCE_STATUS_BLOCK_HOST_DETAIL                                 \
  "block host %s:%u internal connect count: %llu, connect success count: "    \
  "%llu, connect fail count: %llu, reconnect count: %llu, block time epoch: " \
  "%llu\n"

#define LOAD_BALANCE_STATUS_TIME_CONSUME                                    \
  "mark hosts available consume: %llu microseconds, mark hosts unavliable " \
  "consume: %llu microseconds."

/* print status info, if status buffer not enough, expand status buffer and
 * reprint status info. */
static int print_load_balance_status_info(MYSQL *mysql, char **status,
                                          size_t &write_size_sum,
                                          size_t &buf_len, const string format,
                                          ...) {
  int result = 0;
  bool reprint = false;
  size_t buf_remain_len = 0;
  int str_len = 0;

  auto check_print_result_and_status_buff = [&mysql, &status, &write_size_sum,
                                             &buf_len, &reprint,
                                             &buf_remain_len, &str_len]() {
    reprint = false;
    if (str_len < 0) {
      return 0 != errno ? errno : 1;
    }
    if (static_cast<size_t>(str_len) > buf_remain_len) {
      /* status buffer not enough, expand status buffer and set reprint
       * flag.
       */
      size_t tmp_status_buf_len = 2 * buf_len + str_len;
      char *tmp_status_buf = static_cast<char *>(
          my_malloc(key_memory_mysql_options, tmp_status_buf_len, MYF(MY_WME)));
      if (nullptr == tmp_status_buf) {
        return CR_LOAD_BALANCE_ALLOC_MEM_FAILED;
      }
      memcpy(tmp_status_buf, *status, buf_len);
      memset(tmp_status_buf + buf_len, 0, tmp_status_buf_len - buf_len);
      my_free(*status);
      *status = tmp_status_buf;
      buf_len = tmp_status_buf_len;
      mysql->options.load_balance->load_balance_status = tmp_status_buf;
      mysql->options.load_balance->load_balance_status_len = tmp_status_buf_len;
      reprint = true;
    } else {
      write_size_sum += str_len;
    }
    return 0;
  };

  do {
    buf_remain_len = buf_len - write_size_sum - 1; /* reserve 1 byte for \0. */
    va_list args;
    va_start(args, format);
    /* str_len not include \0. */
    str_len = vsnprintf(*status + write_size_sum, buf_remain_len,
                        format.c_str(), args);
    va_end(args);
    result = check_print_result_and_status_buff();
  } while (0 == result && reprint);
  return result;
}
...
int mysql_get_loadbalance_status(MYSQL *mysql, char **status) {
  ...
  /* Expand the format length by 2 times to ensure sufficient length. */
  size_t tmp_buf_len =
      (mysql->options.load_balance->load_balance_hosts
           ? strlen(mysql->options.load_balance->load_balance_hosts)
           : 0) +
      2 * (strlen(LOAD_BALANCE_STATUS_SUMMARY) +
           std::max(strlen(LOAD_BALANCE_STATUS_CONNECTED),
                    strlen(LOAD_BALANCE_STATUS_NOT_CONNECTED)) +
           strlen(LOAD_BALANCE_STATUS_CONN_SUMMARY) +
           strlen(LOAD_BALANCE_STATUS_AVLIABLE_HOST_TITLE) +
           strlen(LOAD_BALANCE_STATUS_AVLIABLE_HOST_DETAIL) *
               mysql->options.load_balance->available_hosts.size() +
           strlen(LOAD_BALANCE_STATUS_BLOCK_HOST_TITLE) +
           strlen(LOAD_BALANCE_STATUS_BLOCK_HOST_DETAIL) *
               mysql->options.load_balance->block_hosts.size() +
           strlen(LOAD_BALANCE_STATUS_TIME_CONSUME));

  if (tmp_buf_len > mysql->options.load_balance->load_balance_status_len) {
    if (nullptr != mysql->options.load_balance->load_balance_status) {
      my_free(mysql->options.load_balance->load_balance_status);
    }
    mysql->options.load_balance->load_balance_status = static_cast<char *>(
        my_malloc(key_memory_mysql_options, tmp_buf_len, MYF(MY_WME)));
    if (nullptr == mysql->options.load_balance->load_balance_status) {
      ret = CR_LOAD_BALANCE_ALLOC_MEM_FAILED;
      return ret;
    }
    mysql->options.load_balance->load_balance_status_len = tmp_buf_len;
  }
  *status = mysql->options.load_balance->load_balance_status;
  size_t buf_len = mysql->options.load_balance->load_balance_status_len;
  memset(*status, 0, buf_len);

  size_t write_size_sum = 0;
  ret = print_load_balance_status_info(
      mysql, status, write_size_sum, buf_len, LOAD_BALANCE_STATUS_SUMMARY,
      mysql->options.load_balance->load_balance_hosts,
      load_balance_strategy_to_str(mysql->options.load_balance->strategy)
          .c_str(),
      mysql->options.load_balance->random_seed);
  if (ret) return ret;

  if (mysql->options.load_balance->current_conn_host) {
    ret = print_load_balance_status_info(
        mysql, status, write_size_sum, buf_len, LOAD_BALANCE_STATUS_CONNECTED,
        mysql->options.load_balance->current_conn_host->host,
        mysql->options.load_balance->current_conn_host->port);
  } else {
    ret = print_load_balance_status_info(mysql, status, write_size_sum, buf_len,
                                         LOAD_BALANCE_STATUS_NOT_CONNECTED);
  }
  if (ret) return ret;

  ret = print_load_balance_status_info(
      mysql, status, write_size_sum, buf_len, LOAD_BALANCE_STATUS_CONN_SUMMARY,
      mysql->options.load_balance->external_connect_cnt,
      mysql->options.load_balance->external_connect_nonblocking_cnt);
  if (ret) return ret;
  ...
  if (!mysql->options.load_balance->available_hosts.empty()) {
    ret =
        print_load_balance_status_info(mysql, status, write_size_sum, buf_len,
                                       LOAD_BALANCE_STATUS_AVLIABLE_HOST_TITLE);
    if (ret) return ret;

    for (auto &item : mysql->options.load_balance->available_hosts) {
      ret = print_load_balance_status_info(
          mysql, status, write_size_sum, buf_len,
          LOAD_BALANCE_STATUS_AVLIABLE_HOST_DETAIL, item->host, item->port,
          item->connect_cnt, item->connect_cnt - item->connect_fail_cnt,
          item->connect_fail_cnt, item->reconnect_cnt);
      if (ret) return ret;
    }
  }
  ...
  ret = print_load_balance_status_info(
      mysql, status, write_size_sum, buf_len,
      LOAD_BALANCE_STATUS_BLOCK_HOST_TITLE,
      mysql->options.load_balance->blocklist_timeout, my_milli_time());
  if (ret) return ret;

  for (auto &item : mysql->options.load_balance->block_hosts) {
    ret = print_load_balance_status_info(
        mysql, status, write_size_sum, buf_len,
        LOAD_BALANCE_STATUS_BLOCK_HOST_DETAIL, item->host, item->port,
        item->connect_cnt, item->connect_cnt - item->connect_fail_cnt,
        item->connect_fail_cnt, item->reconnect_cnt, item->invalid_time);
    if (ret) return ret;
  }

  ret = print_load_balance_status_info(
      mysql, status, write_size_sum, buf_len, LOAD_BALANCE_STATUS_TIME_CONSUME,
      mysql->options.load_balance->mark_host_available_used_time,
      mysql->options.load_balance->mark_host_block_used_time);
  return ret;
}

int mysql_get_current_connection_host(MYSQL *mysql, char **ip,
                                      unsigned int *port) {
  int ret = 0;
  if (mysql->options.load_balance->current_conn_host) {
    *ip = mysql->options.load_balance->current_conn_host->host;
...
MYSQL *STDCALL mysql_real_connect(MYSQL *mysql, const char *host,
  ...
  DBUG_TRACE;
  if (mysql->options.load_balance && mysql->options.load_balance->hosts) {
    return mysql_connect_load_balance(mysql, user, passwd, db, client_flag);
  }
...
net_async_status STDCALL mysql_real_connect_nonblocking(
  ...
  DBUG_TRACE;
  if (mysql->options.load_balance && mysql->options.load_balance->hosts) {
    return mysql_connect_nonblocking_load_balance(mysql, user, passwd, db,
                                                  client_flag);
  }
...
#ifdef WITH_MTR_HINT
  if (mysql->client_flag & CLIENT_MTR_HINT) {
    net->mtr_hint = true;
  }
#endif
...
bool mysql_reconnect(MYSQL *mysql) {
  ...
  if (tmp_mysql.options.load_balance) {
    tmp_mysql.options.load_balance->reconnecting = true;
  }
  ...
  tmp_mysql.free_options = mysql->free_options;
...
void mysql_close_free_options(MYSQL *mysql) {
  ...
  if (mysql->options.load_balance) {
    mysql->options.load_balance->cleanup_hosts();
    FREE_LOAD_BALANCE(&mysql->options);
  }
  memset(&mysql->options, 0, sizeof(mysql->options));
}
...
#ifdef WITH_MTR_HINT
    async_context->async_mtr_hint_state = MTR_HINT_IDLE;
#endif
  }
...
#ifdef WITH_MTR_HINT
    status = read_mtr_hint_nonblocking(mysql, &res);
    if (status == NET_ASYNC_NOT_READY) {
      return NET_ASYNC_NOT_READY;
    }
    if (res) {
...
#endif
    status = read_com_query_metadata_nonblocking(mysql, pos, mysql->field_count,
                                                 &res);
    if (status == NET_ASYNC_NOT_READY) {
      return NET_ASYNC_NOT_READY;
    }
...
int STDCALL mysql_options(MYSQL *mysql, enum mysql_option option,
  ...
  switch (option) {
    ...
    case MYSQL_OPT_WRITE_TIMEOUT:
      ...
      break;
    case MYSQL_OPT_MTR_HINT:
      mysql->options.mtr_hint = true; /* Remember for connect */
#ifdef WITH_MTR_HINT
      mysql->options.client_flag |= CLIENT_MTR_HINT;
#endif
      break;
    ...
    case MYSQL_DEFAULT_AUTH:
      ...
      break;
#ifdef SSL_GM
    case MYSQL_OPT_GM_SSL:
      mysql->options.use_gm_ssl = true;
      break;
#endif /* SSL_GM */
    ...
    case MYSQL_OPT_TLS_SNI_SERVERNAME:
      EXTENSION_SET_STRING(&mysql->options, tls_sni_servername,
                           static_cast<const char *>(arg));
      break;
    case MYSQL_LOAD_BALANCE_HOSTS:
      if (set_load_balance_option_hosts(mysql, static_cast<const char *>(arg)))
        return 1;
      break;
    case MYSQL_LOAD_BALANCE_STRATEGY:
      if (!mysql->options.load_balance) ALLOCATE_LOAD_BALANCE(&mysql->options);
      if (!mysql->options.load_balance) return 1;
      mysql->options.load_balance->strategy =
          *static_cast<const mysql_load_balance_strategy *>(arg);
      break;
    case MYSQL_LOAD_BALANCE_BLOCKLIST_TIMEOUT:
      if (!mysql->options.load_balance) ALLOCATE_LOAD_BALANCE(&mysql->options);
      if (!mysql->options.load_balance) return 1;
      mysql->options.load_balance->blocklist_timeout =
          *static_cast<const unsigned int *>(arg);
...
int STDCALL mysql_get_option(MYSQL *mysql, enum mysql_option option,
  ...
  switch (option) {
    ...
    case MYSQL_LOAD_BALANCE_HOSTS:
      *(static_cast<char **>(const_cast<void *>(arg))) =
          mysql->options.load_balance
              ? static_cast<char *>(
                    mysql->options.load_balance->load_balance_hosts)
              : nullptr;
      break;
    case MYSQL_LOAD_BALANCE_STRATEGY:
      *(const_cast<mysql_load_balance_strategy *>(
          static_cast<const mysql_load_balance_strategy *>(arg))) =
          mysql->options.load_balance ? mysql->options.load_balance->strategy
                                      : LOAD_BALANCE_RANDOM;
      break;
    case MYSQL_LOAD_BALANCE_BLOCKLIST_TIMEOUT:
      *(const_cast<ulong *>(static_cast<const ulong *>(arg))) =
