...
#define ALLOCATE_LOAD_BALANCE(OPTS) \
  (OPTS)->load_balance = new (std::nothrow) st_mysql_load_balance_options;

#define FREE_LOAD_BALANCE(OPTS)       \
  do {                                \
    if ((OPTS)->load_balance) {       \
      delete (OPTS)->load_balance;    \
      (OPTS)->load_balance = nullptr; \
    }                                 \
  } while (0)
#endif
