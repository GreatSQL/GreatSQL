...
class Transaction_payload_event : public Binary_log_event {
 ...
 public:
  ...
  static const int MAX_DATA_LENGTH =
      ...
      UNCOMPRESSED_SIZE_MAX_LENGTH + COMMIT_GROUP_TICKET_LENGTH;
  /**
...
class Transaction_context_event : public Binary_log_event {
  ...
  */
  ...
  Transaction_context_event(unsigned int thread_id_arg,
      ...
      : Binary_log_event(TRANSACTION_CONTEXT_EVENT),
        ...
        gtid_empty_transaction(is_empty_transaction_arg) {}
 ...
 protected:
  ...
  uint32_t thread_id;
  bool gtid_specified;
  // if gtid not specified, gtid_empty_transaction will always be false,
  // so we will use one byte for gtid_specified and empty_transaction
  bool gtid_empty_transaction;
