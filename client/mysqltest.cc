...
static bool opt_mtr_hint = false;
static long print_mtr_hint_msg_lenth_total = 0;

#ifdef WITH_MTR_HINT
static size_t mtr_hint_pos_chg = 0;
#endif

static FILE *exp_res_file = nullptr;
...
static bool display_result_vertically = false, display_result_lower = false,
            display_metadata = false, display_result_sorted0 = false,
            display_result_sorted = false, display_session_track_info = false;
static bool skip_if_hypergraph = false;
static std::vector<int> order_columns0,
    order_columns; /* var0 is setting from user input, var is setting for
                      mtr-hint */
...
static std::string mtr_hint_str;
static size_t fields_print_len, sorted_postfix_len;
...
#ifdef WITH_MTR_HINT
  if (opt_mtr_hint) mysql_options(&con_slot->mysql, MYSQL_OPT_MTR_HINT, NullS);
#endif
...
#ifdef WITH_MTR_HINT
    {"mtr-hint", 0, "Enable test with mtr hint feature.", &opt_mtr_hint,
     &opt_mtr_hint, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, nullptr, 0, nullptr},
#endif
...
/*
  Sort the rows in dynamic string `ds` lexciographically from start_offset to
  end_offset.
*/
static void sorting_dyn_str(DYNAMIC_STRING *ds, size_t start_offset,
                            size_t end_offset) {
  std::vector<std::string> rows;
  const char *start = ds->str + start_offset;
  const char *end = ds->str + end_offset;
  const char *line_start = start;

  if (ds->length == 0) return;
  if (start_offset >= end_offset || end_offset > ds->length) {
    die("Invalid offsets for sorting the dynamic string.");
  }

  for (const char *p = start; p < end; p++) {
    if (*p == '\n') {
      rows.emplace_back(line_start, p - line_start);
      line_start = p + 1;
    }
  }
  ...
  for (const auto &row : rows) {
    sorted_length += row.length() + 1;  // +1 for `\n`
  }
  if (sorted_length > (end_offset - start_offset)) {
    die("Sorted rows exceed the pre-sort length.");
  }

  char *write_ptr = ds->str + start_offset;
  for (const auto &row : rows) {
    memcpy(write_ptr, row.c_str(), row.length());
    write_ptr += row.length();
    *write_ptr++ = '\n';
  }
}
/*
...
#ifdef WITH_MTR_HINT
static void parse_mtr_hint() {
  MYSQL *mysql = &cur_con->mysql;
  const char *start, *pos, *end;

  // skip if already set by "--sorted_result" or "--partially_sorted_result"
  // before
  if (display_result_sorted0) return;

  if (!mysql->net.mtr_hint_msg) goto LABEL_CLEAR;
  start = strstr(mysql->net.mtr_hint_msg, "--MTR-hint:");
  if (!start) goto LABEL_CLEAR;
  start += strlen("--MTR-hint:");
  // Skip spaces
  while (*start && isspace(*start)) start++;
  // processing "order none" or "order 1,4,2"
  pos = strstr(start, "order");
  if (!pos) goto LABEL_CLEAR;
  pos += strlen("order");
  while (*pos && isspace(*pos)) pos++;
  // Check for "none"
  if (!strncmp(pos, "none", 4)) {
    display_result_sorted = true;
    return;
  }
  // Find end of order list (comma or end of string)
  end = pos;

  while (*end && *end != '\n' && *end != '\r') end++;
  {
    std::string order_str(pos, end - pos);
    // Remove spaces
    order_str.erase(remove_if(order_str.begin(), order_str.end(), ::isspace),
                    order_str.end());
    // Split by comma
    std::stringstream ss(order_str);
    std::string token;
    while (std::getline(ss, token, ',')) {
      int col_idx = get_int_val(token.c_str());
      if (col_idx != -1) {
        order_columns.push_back(col_idx);
      }
    }
  }
  if (order_columns.empty()) goto LABEL_CLEAR;
...
static void append_table_headings(DYNAMIC_STRING *ds, MYSQL_FIELD *field,
  ...
  if (opt_mtr_hint && mysql->net.mtr_hint_msg &&
      strlen(mysql->net.mtr_hint_msg)) {
    // dynstr_append(ds, mysql->net.mtr_hint_msg);
    // dynstr_append(ds, "\n");
    mtr_hint_pos_chg = strlen(mysql->net.mtr_hint_msg) + 1;
  } else {
    mtr_hint_pos_chg = 0;
  }
  parse_mtr_hint();
...
static int append_warnings(DYNAMIC_STRING *ds, MYSQL *mysql) {
  ...
  /* Sort only if the user specified sort commmand, not sort for mtr-hint
   * processing */
  if (display_result_sorted0)
    sorting_dyn_str(&ds_warnings, 0, ds_warnings.length);
...
static void print_table_data_with_border(DYNAMIC_STRING *ds,
  ...
  if (opt_mtr_hint && mysql->net.mtr_hint_msg &&
      strlen(mysql->net.mtr_hint_msg)) {
    // dynstr_append(not_sort_ds, mysql->net.mtr_hint_msg);
    // dynstr_append(not_sort_ds, "\n");
    mtr_hint_pos_chg = strlen(mysql->net.mtr_hint_msg) + 1;
  } else {
    mtr_hint_pos_chg = 0;
  }
  ...
  separator.append("\n");
  dynstr_append(not_sort_ds, separator.c_str());
  ...
  for (uint off = 0; (field = mysql_fetch_field(result)); off++) {
    ...
    dynstr_append_mem(not_sort_ds, field_str, str_len);
    delete[] field_str;
  }
  dynstr_append(not_sort_ds, "\n");
  dynstr_append(not_sort_ds, separator.c_str());
...
static void process_display_result_sort(DYNAMIC_STRING *save_ds,
                                        DYNAMIC_STRING *pds_sorted) {
  auto ds_sorted = *pds_sorted;

  if ((display_result_sorted0 || display_result_sorted) &&
      pds_sorted->length > 0) {
    DYNAMIC_STRING ds_exp, ds_exp_sorted;
    bool need_cmp = !record;
    init_dynamic_string(&ds_exp, "", save_ds->length + 1);
    init_dynamic_string(&ds_exp_sorted, "", ds_sorted.length + 1);
    /* Read expected result file at same position with test result rows which
     * need sort.*/
    if (!exp_res_file)
      if (!(exp_res_file = fopen(result_file_name, "r"))) {
        need_cmp = false;
        verbose_msg("Could not open '%s' for reading, errno: %d",
                    result_file_name, errno);
      }

    if (need_cmp && fseek(exp_res_file,
                          log_file.bytes_written() -
                              print_mtr_hint_msg_lenth_total + save_ds->length,
                          SEEK_SET) == -1) {
      need_cmp = false;
      verbose_msg("Failed to seek in file: '%s', errno: %d", result_file_name,
                  errno);
    }

    if (need_cmp) {
      if (fread(ds_exp_sorted.str, 1, ds_sorted.length, exp_res_file) !=
          ds_sorted.length) {
        need_cmp = false;
        verbose_msg(
            "Failed to read from file: '%s' at position %ld with length %ld",
            result_file_name,
            log_file.bytes_written() - print_mtr_hint_msg_lenth_total +
                save_ds->length,
            ds_sorted.length);
      } else {
        /* Read data successfully, do as append data to ds_exp_sorted */
        ds_exp_sorted.length += ds_sorted.length;
        ds_exp_sorted.str[ds_exp_sorted.length] = 0;
      }
    }
    ...
    /*
       Compare expected result at same position after sort with test result
       after sort
    */
    if (need_cmp) {
      dynstr_append_sorted(&ds_exp, &ds_exp_sorted);
      if (memcmp(ds_exp.str, save_ds->str + save_ds->length - ds_sorted.length,
                 ds_sorted.length) == 0) {
        /*
          When equal, print out the result set before sort in the expected
          output. so here replace ds_sorted with ds_exp_sorted
        */
        memcpy(save_ds->str + save_ds->length - ds_exp_sorted.length,
               ds_exp_sorted.str, ds_exp_sorted.length);
      } else {
        /* When not equal, print MTR-hint in output if not just record */
        if (!record && opt_mtr_hint && mtr_hint_str.length() > 0) {
          mtr_hint_str.push_back('\n');
          print_mtr_hint_msg_lenth_total += mtr_hint_str.length();
          if (fields_print_len > 0) {
            /* in scenario like: n statements in on stored procedure */
            mtr_hint_str.append(save_ds->str + save_ds->length -
                                    fields_print_len - ds_sorted.length,
                                fields_print_len + ds_sorted.length);
            dynstr_trunc(save_ds, fields_print_len + ds_sorted.length);
            dynstr_append(save_ds, mtr_hint_str.c_str());
          } else
            dynstr_prepend(save_ds, mtr_hint_str.c_str());
        }
      }
    }
    dynstr_free(&ds_exp);
    dynstr_free(&ds_exp_sorted);
  } else {
    dynstr_append_mem(save_ds, ds_sorted.str, ds_sorted.length);
  }
  dynstr_trunc(pds_sorted, ds_sorted.length);
}
...
static void run_query_normal(struct st_connection *cn,
                             ...
                             DYNAMIC_STRING *ds, DYNAMIC_STRING *not_sort_ds,
                             DYNAMIC_STRING *ds_warnings) {
  int error = 0;
  ...
  if (flags & QUERY_SEND_FLAG) {
    ...
    if (mysql_send_query_wrapper(&cn->mysql, query,
      ...
      handle_error(command, mysql_errno(mysql), mysql_error(mysql),
                   mysql_sqlstate(mysql), not_sort_ds);
      goto end;
    }
  }
  ...
  do {
    if (counter != 0 && !display_result_sorted0) {
      process_display_result_sort(not_sort_ds, ds);
    }
    /*
      When  on first result set, call mysql_read_query_result_wrapper to
      retrieve answer to the query sent earlier
    */
    else if ((counter == 0) && mysql_read_query_result_wrapper(&cn->mysql)) {
    ...
    */
    ...
    if (!disable_result_log) {
      if (res) {
        ...
        fields_print_len = not_sort_ds->length;

        if (display_metadata) append_metadata(not_sort_ds, fields, num_fields);

        if (opt_format_result) {
#ifdef _WIN32
          die("do not support --format-result on Windows");
#endif
          print_table_data_with_border(ds, not_sort_ds, res);
        } else {
          if (!display_result_vertically) {
            append_table_headings(not_sort_ds, fields, num_fields);
          }

          append_result(ds, res);
        }
        fields_print_len = not_sort_ds->length - fields_print_len;
      }

      sorted_postfix_len = ds->length;
      ...
      sorted_postfix_len = ds->length - sorted_postfix_len;
    }

#ifdef WITH_MTR_HINT
    mtr_hint_str.clear();
    mtr_hint_str.append(mysql->net.mtr_hint_msg,
                        strlen(mysql->net.mtr_hint_msg));
#endif
  ...
  // one statement in one query, clear it to indicate print mtr_hint before
  // query instead of before printed fields
  fields_print_len = 0;
...
static void run_query_stmt(MYSQL *mysql, struct st_command *command,
  ...
  if (mysql_stmt_prepare(stmt, query, static_cast<ulong>(query_len))) {
    handle_error(command, mysql_stmt_errno(stmt), mysql_stmt_error(stmt),
                 mysql_stmt_sqlstate(stmt), not_sort_ds);
    goto end;
  }
...
static void run_query(struct st_connection *cn, struct st_command *command,
  ...
  */
  ...
  /*
     ...
     Even if !display_result_sorted, because this
     flag maybe set when executing query with mtr-hint.
  */
  ...
  */
  if (ps_protocol_enabled && complete_query &&
    ...
    run_query_stmt(mysql, command, query, query_len, ds, save_ds, &ds_warnings);
  else
    run_query_normal(cn, command, flags, query, query_len, ds, save_ds,
                     &ds_warnings);
...
int main(int argc, char **argv) {
  ...
  if (opt_connect_timeout)
    mysql_options(&con->mysql, MYSQL_OPT_CONNECT_TIMEOUT,
                  (void *)&opt_connect_timeout);
#ifdef WITH_MTR_HINT
  if (opt_mtr_hint) mysql_options(&con->mysql, MYSQL_OPT_MTR_HINT, NullS);
#endif
      ...
      switch (command->type) {
        ...
        case Q_SORTED_RESULT:
          /*
            ...
            command
          */
          display_result_sorted0 = true;
          break;
        case Q_PARTIALLY_SORTED_RESULT: {
          /*
            Turn on partially sorted result set, will be reset after next
            command. Accepts a comma-separated list of column indices to sort
            by.
          */
          const char *arg = command->first_argument;

          while (*arg) {
            // Skip whitespace
            while (*arg && my_isspace(charset_info, *arg)) arg++;
            if (!*arg) break;
            int col = atoi(arg);
            if (col > 0) order_columns0.push_back(col);
...
// fetch column start position and length for each column in
// order_columns, and store into column_start and column_length
static void get_columns_begin_and_length_of_order_columns(
    std::string str, std::vector<size_t> *column_start,
    std::vector<size_t> *column_length, char delim) {
  std::stringstream columns(str);
  std::string temp;
  size_t size_of_columns = 0;
  size_t i = 0, idx = 0;

  auto order_cols = display_result_sorted0 ? order_columns0 : order_columns;
  // here sort it for speed up
  std::sort(order_cols.begin(), order_cols.end());
  while (idx < order_cols.size() && getline(columns, temp, delim))
    if ((int)i < order_cols[idx] - 1) {
      size_of_columns = size_of_columns + temp.length() + 1;
      i++;
    } else if ((int)i == order_cols[idx] - 1) {
      column_start->push_back(size_of_columns);
      column_length->push_back(temp.length() + 1);
      size_of_columns = size_of_columns + temp.length() + 1;
      i++;
      idx++;
    }
...
[[maybe_unused]] static size_t length_of_n_first_columns(
    std::string str, int start_sort_column) {
  std::stringstream columns(str);
  std::string temp;
  size_t size_of_columns = 0;
...
void dynstr_append_sorted(DYNAMIC_STRING *ds, DYNAMIC_STRING *ds_input) {
  ...
  char *end = ds_input->str + ds_input->length - sorted_postfix_len;
  std::vector<std::string> sorted;
  DBUG_TRACE;

  if (ds_input->length == 0) return;
  ...
  */
  ...
  while (start < end) {
    ...
    if (!sorted.empty() &&
        (!order_columns0.empty() || !order_columns.empty())) {
      /*
       If doing partial sorting, and the prefix(i.e. those columns in
       order_columns) is different from that of the previous line, the group is
       done. Sort it and start another one.
      */
      std::vector<size_t> column_start, column_length;
      get_columns_begin_and_length_of_order_columns(
          sorted.back(), &column_start, &column_length,
          (opt_format_result) ? '|' : '\t');
      bool is_prefix_same = true;
      for (size_t idx = 0; idx < column_start.size(); ++idx) {
        /* compare part include line end */
        if (result_row.length() + 1 < column_start[idx] + column_length[idx] ||
            sorted.back().compare(column_start[idx], column_length[idx],
                                  result_row, column_start[idx],
                                  column_length[idx]) != 0) {
  ...
  if (sorted_postfix_len > 0) {
    dynstr_append_mem(ds, ds_input->str + ds_input->length - sorted_postfix_len,
                      sorted_postfix_len);
  }
}
