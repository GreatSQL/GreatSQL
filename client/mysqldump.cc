...
static bool verbose = false, opt_no_create_info = false, opt_no_data = false,
            ...
            opt_materialized_view = false, opt_materialized_view_log = false,
...
#include "client/include/multi_factor_passwordopt-longopts.h"
    ...
    {"materialized_views", OPT_MATERIALIZED_VIEWS,
     "Disable dump materiazlied views", &opt_materialized_view,
     &opt_materialized_view, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, nullptr, 0,
     nullptr},
    {"materialized_view_logs", OPT_MATERIALIZED_VIEW_LOGS,
     "Disable dump materiazlied view logs", &opt_materialized_view_log,
     &opt_materialized_view_log, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, nullptr, 0,
     nullptr},
...
bool check_is_materialized_viwe_or_mlog(const char *table_name, const char *db);
...
static bool get_materialized_view_log_structure(char *mlog, char *db);
static bool get_materialized_view_structure(char *table, char *db);
static bool dump_materialized_view_for_db(char *db);
static bool dump_materialized_view_log_for_db(char *db);
...
static int connect_to_db(char *host, char *user) {
  ...
  /* The server may open EMPTYSTTRIING EQUAL NULL mode
     leading to set sql mode= '' failure, so default ANSI setting
 */
  if (mysql_query_with_error_report(mysql, nullptr,
                                    "/*!40100 SET @@SQL_MODE='ANSI' */"))
    return 1;
 ...
 *
 */
[[maybe_unused]] static void fprintf_string(char *row, ulong row_len, char quote,
                           bool needs_newline) {
...
static uint get_table_structure(const char *table, char *db, char *table_type,
  ...
  *ignore_flag = check_if_ignore_table(table, table_type);
  ...
  /** mlog and materialized view shoud dump after all table check skip */
  if (is_view && check_is_materialized_viwe_or_mlog(db, table)) {
    DBUG_PRINT("exit", ("Dumping materialized view and mlog ignored."));
    *ignore_flag = IGNORE_DATA;
    return 0;
  }
...
static int dump_triggers_for_table(char *table_name, char *db_name) {
  ...
  while ((row = mysql_fetch_row(show_triggers_rs))) {
    // imv binlog ignore
    if (strncasecmp(row[0], STRING_WITH_LEN("imv_mlog_trigger_")) == 0) {
      continue;
    }
...
static int dump_all_tables_in_db(char *database) {
  ...
  *afterdot++ = '.';
  ...
  if ((!opt_materialized_view_log) &&
      mysql_get_server_version(mysql) >= 80404) {
    DBUG_PRINT("info",
               ("Dumping materialized view log for database %s", database));
    dump_materialized_view_log_for_db(database);
  }
  if ((!opt_materialized_view) && mysql_get_server_version(mysql) >= 80032) {
    DBUG_PRINT("info", ("Dumping materialized view for database %s", database));
    dump_materialized_view_for_db(database);
  }
...
static int dump_selected_tables(char *db, char **table_names, int tables) {
  ...
  if (opt_materialized_view_log && mysql_get_server_version(mysql) >= 80404) {
    for (pos = dump_tables; pos < end; pos++)
      get_materialized_view_log_structure(*pos, db);
  }

  if (opt_materialized_view && mysql_get_server_version(mysql) >= 80032) {
    for (pos = dump_tables; pos < end; pos++)
      get_materialized_view_structure(*pos, db);
  }
...
bool check_is_materialized_viwe_or_mlog(const char *db,
                                        const char *table_name) {
  ...
  bool ret = false;
  if (row[0]) {
    if ((my_strcasecmp(&my_charset_latin1, row[0], "MATERIALIZED VIEW LOG") ==
         0) ||
        (my_strcasecmp(&my_charset_latin1, row[0], "MATERIALIZED VIEW") == 0))
      ret = true;
  }
  mysql_free_result(res);
  return ret;
...
static bool get_view_structure(char *table, char *db) {
  ...
  /** mlog and materialized view shoud dump after all table check skip */
  if (check_is_materialized_viwe_or_mlog(db, table)) {
    DBUG_PRINT("exit", ("Dumping materialized view and mlog ignored."));
    return false;
  }
...
bool dump_materialized_view_log(char *table, char *db) {
  FILE *sql_file = md_result_file;
  switch_sql_mode_with_version(sql_file, "80032", ";", "ORACLE");
  ...
  if (db) {
    dump_fprintf(
        sql_file,
        "/*!80404 DROP MATERIALIZED VIEW LOG IF EXISTS ON \"%s\".\"%s\" */;\n"
        "/*!80404 CREATE MATERIALIZED VIEW LOG ON \"%s\".\"%s\" */ ;\n",
        db, table, db, table);
  } else {
    dump_fprintf(sql_file,
                 "/*!80404 DROP MATERIALIZED VIEW LOG IF EXISTS ON \"%s\" */;\n"
                 "/*!80404 CREATE MATERIALIZED VIEW LOG ON \"%s\" */;\n",
                 table, table);
  }
  dump_fprintf(
      sql_file,
...
bool dump_materialized_view(char *table, char *db) {
  FILE *sql_file = md_result_file;
  char query_buff[QUERY_LENGTH] = {0};
  MYSQL_RES *res;

  if (switch_character_set_results(mysql, "binary")) return true;
  if (mysql_query_with_error_report(mysql, nullptr, "SET SQL_MODE=ORACLE"))
    return true;

  snprintf(query_buff, sizeof(query_buff),
           "SHOW CREATE MATERIALIZED VIEW \"%s\".\"%s\"", db, table);

  if (mysql_query_with_error_report(mysql, &res, query_buff)) {
    switch_character_set_results(mysql, default_charset);
    return false;
  }
  MYSQL_ROW mv_row = mysql_fetch_row(res);
  ...
  if (mv_row && mv_row[1]) {
    ...
    dump_fprintf(
        ...
        "/*!50001 SET @saved_col_connection     = @@collation_connection */;\n"
        "/*!50503 SET character_set_client = utf8mb4 */;\n"
        "/*!80404 DROP MATERIALIZED VIEW IF EXISTS \"%s\" */;\n"
        "/*!80404 %s */;\n"
        "/*!50001 SET character_set_client      = @saved_cs_client */;\n"
        "/*!50001 SET character_set_results     = @saved_cs_results */;\n"
        "/*!50001 SET collation_connection      = @saved_col_connection */;\n",
  ...
  mysql_free_result(res);
  if (mysql_query_with_error_report(mysql, nullptr, "SET SQL_MODE=DEFAULT")) {
    return true;
  }
  if (switch_character_set_results(mysql, default_charset)) return true;
  return false;
...
/**
 * @brief Get the materialized view log structure object
 *
 * @param table  [base table] not mlog name
 * @param db
 *
 * @return true
 * @return false
...
static bool get_materialized_view_log_structure(char *mlog, char *db) {
  ...
  snprintf(query_buff, sizeof(query_buff),
           "select TABLE_SCHEMA, TABLE_NAME MATERIALIZED_VIEW_LOGS where \
   MLOG_NAME = '%s' and MLOG_DB = '%s' ",
           mlog, db);
  ...
  // if db is different
  bool need_db = false;
  if (strcmp(row[0], db) != 0) {
    need_db = true;
  }

  if (dump_materialized_view_log(row[1], need_db ? row[0] : nullptr))
    return true;
...
static bool get_materialized_view_structure(char *table, char *db) {
  ...
  snprintf(query_buff, sizeof(query_buff),
           "select count(*) from INFORMATION_SCHEMA_DB_NAME.MATERIALIZED_VIEWS"
           "where  VIEW_NAME = '%s' and VIEW_SCHEMA = '%s'  ",
           table, db);
  ...
  if (switch_character_set_results(mysql, default_charset)) return true;
  if (has_table) {
    if (dump_materialized_view(table, db)) return true;
  }
...
static bool dump_materialized_view_log_for_db(char *db) {
  ...
  {
    snprintf(query_buff, sizeof(query_buff),
             "SELECT TABLE_NAME from " INFORMATION_SCHEMA_DB_NAME
             ".MATERIALIZED_VIEW_LOGS WHERE TABLE_SCHEMA = '%s'",
             db_name_buff_ident);
    if (mysql_query_with_error_report(mysql, &list_res, query_buff)) {
      return 1;
    }

    if (mysql_num_rows(list_res)) {
      /* nice comments */
      bool freemem = false;
      char const *materialized_view_log_text =
          fix_identifier_with_newline(db, &freemem);
      print_comment(
          sql_file, false,
          "\n--\n-- Dumping materialized view log for database '%s'\n--\n",
          materialized_view_log_text);
      if (freemem) my_free(const_cast<char *>(materialized_view_log_text));

      while ((list_row = mysql_fetch_row(list_res))) {
        if (dump_materialized_view_log(list_row[0], nullptr)) return true;
      }
    }
...
static bool dump_materialized_view_for_db(char *db) {
  ...
  {
    snprintf(query_buff, sizeof(query_buff),
             "SELECT VIEW_NAME from information_schema.MATERIALIZED_VIEWS "
             "WHERE VIEW_SCHEMA = '%s'",
             db_name_buff_ident);
    if (mysql_query_with_error_report(mysql, &list_res, query_buff)) {
      return 1;
    ...
    if (mysql_num_rows(list_res)) {
      /* nice comments */
      bool freemem = false;
      char const *materialized_view_text =
          fix_identifier_with_newline(db, &freemem);
      print_comment(
          sql_file, false,
          "\n--\n-- Dumping materialized view for database '%s'\n--\n",
          materialized_view_text);
      if (freemem) my_free(const_cast<char *>(materialized_view_text));

      while ((list_row = mysql_fetch_row(list_res))) {
        if (dump_materialized_view(list_row[0], db)) return true;
      }
    }
    mysql_free_result(list_res);
  }
