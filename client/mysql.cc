...
TYPELIB set_decrypt_mode_typelib = {MY_AES_END + 1, "", my_aes_opmode_names,
                                    nullptr};
...
static int com_quit(String *buffer, char *), com_go(String *buffer, char *),
    ...
    com_source_decrypt(String *buffer, char *);
...
static COMMANDS commands[] = {
    ...
    {"source_decrypt", 0, com_source_decrypt, true,
     "Execute an encrypted script file. Takes file name, "
     "decrypt key as arguments."},
...
static struct my_option my_long_options[] = {
    ...
    {"comments", 'c',
     ...
     nullptr, 0, nullptr},
#ifdef WITH_MTR_HINT
    {"mtr-hint", 0, "Print MTR hint in server/client protocol.", &opt_mtr_hint,
    ...
    {"compress", 'C', "Use compression in server/client protocol.",
     ...
     0, nullptr},
#ifdef SSL_GM
    {"gm-ssl", OPT_GM_SSL, "Use gm to connect server.", &opt_use_gm,
     &opt_use_gm, nullptr, GET_BOOL, NO_ARG, 0, 0, 0, nullptr, 0, nullptr},
#endif
#ifdef NDEBUG
...
#ifdef SSL_GM
    case OPT_GM_SSL:
      opt_use_gm = (argument != disabled_my_option);
      break;
#endif /* SSL_GM */
...
#ifdef WITH_MTR_HINT
  if (opt_mtr_hint && mysql_handle.net.mtr_hint_msg &&
      strlen(mysql_handle.net.mtr_hint_msg))
    tee_puts(mysql_handle.net.mtr_hint_msg, PAGER);
#endif
...
static int com_source_decrypt(String *buffer [[maybe_unused]],
                              char *line) {
  char buff[1024], *param, name[1024];
  char key[MAX_ENCRYPT_KEY_SIZE + 1];
  char iv[MY_AES_IV_SIZE + 1];
  char source_name[FN_REFLEN];
  int mode = 0;
  char key_file[FN_REFLEN];
  int error;
  LINE_BUFFER *line_buff;
  STATUS old_status;
  FILE *sql_file;
  ...
  do {
    ...
    if (!param || !*param) {
      return put_info(
          "Usage: source_decrypt decrypt_file <file> "
          "decrypt_mode <mode> "
          "decrypt_key(_file) <key(_file)> "
          "decrypt_iv <iv>",
          INFO_ERROR, 0);
    }

    if (!native_strcasecmp("decrypt_key", name) ||
        !native_strcasecmp("decrypt-key", name)) {
      strncpy(key, param, sizeof(key) - 1);
    } else if (!native_strcasecmp("decrypt_key_file", name) ||
               !native_strcasecmp("decrypt-key-file", name)) {
      strncpy(key_file, param, sizeof(key_file) - 1);
    } else if (!native_strcasecmp("decrypt_iv", name) ||
               !native_strcasecmp("decrypt-iv", name)) {
      strncpy(iv, param, sizeof(iv) - 1);
    } else if (!native_strcasecmp("decrypt_mode", name) ||
               !native_strcasecmp("decrypt-mode", name)) {
      if ((mode = find_type(param, &set_decrypt_mode_typelib,
                            FIND_TYPE_BASIC)) <= 0)
        return put_info("Invalid decrypt mode.", INFO_ERROR, 0);
      --mode;
    } else if (!native_strcasecmp("decrypt_file", name) ||
               !native_strcasecmp("decrypt-file", name)) {
      strncpy(source_name, param, sizeof(source_name) - 1);
    } else {
      return put_info(
          "Usage: source_decrypt decrypt_file <file> "
          "decrypt_mode <mode> "
  ...
  if (!key[0]) {
    if (key_file[0]) {
      if (!(read_dump_encrypt_key(key_file, key, MAX_ENCRYPT_KEY_SIZE))) {
        return put_info("Invalid decrypt key file.", INFO_ERROR, 0);
      }
    } else {
      return put_info("No decrypt key specified.", INFO_ERROR, 0);
    }
  } else if (key_file[0]) {
    return put_info("Duplicate key and file specified.", INFO_ERROR, 0);
  }
  if (iv[0] && strlen(iv) < 16) {
    return put_info("Invalid decrypt iv.", INFO_ERROR, 0);
  }
  if (!source_name[0])
    return put_info("No decrypt file specified.", INFO_ERROR, 0);

  unpack_filename(source_name, source_name);

  /* open file name */
  if (!(sql_file = my_fopen(source_name, O_RDONLY | MY_FOPEN_BINARY, MYF(0)))) {
    char msg_buff[FN_REFLEN + 60];
    sprintf(msg_buff, "Failed to open file '%s', error: %d", source_name,
            errno);
    return put_info(msg_buff, INFO_ERROR, 0);
  }

  if (!(line_buff = batch_readline_init(MAX_BATCH_BUFFER_SIZE, sql_file, mode,
                                        key, iv))) {
...
#ifdef WITH_MTR_HINT
  if (opt_mtr_hint) mysql_options(mysql, MYSQL_OPT_MTR_HINT, NullS);
#endif

#ifdef SSL_GM
  if (opt_use_gm) mysql_options(mysql, MYSQL_OPT_GM_SSL, NullS);
#endif /* SSL_GM */
