...
#define MIN_NOT_ZERO_ROW_EVENTS_BASE64 8192  // 8K
/**
...
static uint row_events_base64_max_size = 0;
    ...
    switch (ev_type) {
      ...
      case mysql::binlog::event::PARTIAL_UPDATE_ROWS_EVENT: {
        ...
        if (stmt_end) {
          ...
          */
          if (skip_event) {
            ...
            if (print_event_info->only_unflushed_rows_query_log_event) {
              /*
                If reaching the end of a statement causes us to skip all events
                after the rows query log event,  then also clear the rows query
                log event that exists in the io_cache.
              */
              if (reinit_io_cache(&print_event_info->head_cache, WRITE_CACHE, 0,
                                  false, true) ||
                  reinit_io_cache(&print_event_info->body_cache, WRITE_CACHE, 0,
                                  false, true))
                goto err;
              print_event_info->only_unflushed_rows_query_log_event = false;
              print_event_info->skipped_event_in_transaction = true;
              goto end;
            }
        ...
        */
        ...
        if (ev_type == mysql::binlog::event::ROWS_QUERY_LOG_EVENT) {
          print_event_info->only_unflushed_rows_query_log_event = true;
          goto end;
        } else {
          print_event_info->only_unflushed_rows_query_log_event = false;
        }
        /* Flush head,body and footer cache to result_file */
        if (stmt_end || print_event_info->need_print_end) {
          print_event_info->have_unflushed_events = false;
          if (copy_event_cache_to_file_and_reinit(
                  &print_event_info->head_cache, result_file,
...
static struct my_option my_long_options[] = {
    ...
    */
    ...
    {"row-events-base64-max-size", OPT_ROW_EVENTS_BASE64_MAX_SIZE,
     "Try to split update/insert/delete events about one sql with the param."
     "If set the param to 0, we not split events."
     "If the param is more than 0 and less than 8192, will be set 8192."
     "If there still is an event bigger than the limit finally, will output it "
     "directly without split.",
     &row_events_base64_max_size, &row_events_base64_max_size, nullptr,
     GET_UINT, REQUIRED_ARG,
     /* def_value 0 */ 0, /* min_value */ 0,
     /* max_value */ 1024 * 1024 * 1024, /* arg_source */ nullptr,
     /* block_size */ 0, /* app_type */ nullptr},
...
static Exit_status dump_multiple_logs(int argc, char **argv) {
  ...
  */
  ...
  print_event_info.row_events_base64_max_size = row_events_base64_max_size;
...
int main(int argc, char **argv) {
  ...
  */
  ...
  if (row_events_base64_max_size &&
      row_events_base64_max_size < MIN_NOT_ZERO_ROW_EVENTS_BASE64) {
    row_events_base64_max_size = MIN_NOT_ZERO_ROW_EVENTS_BASE64;
  }
