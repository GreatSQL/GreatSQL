...
static bool init_line_buffer(LINE_BUFFER *buffer, File file, ulong size,
                             ulong max_size, int encrypt, char *key, char *iv);
static bool init_line_buffer_from_string(LINE_BUFFER *buffer, char *str);
static size_t fill_buffer(LINE_BUFFER *buffer);
static char *intern_read_line(LINE_BUFFER *buffer, ulong *out_length);

LINE_BUFFER *batch_readline_init(ulong max_size, FILE *file, int encrypt,
                                 char *key, char *iv) {
...
void batch_readline_end(LINE_BUFFER *line_buff) {
  if (line_buff) {
    my_free(line_buff->cipher_buff);
    my_free(line_buff->clear_buff);
...
static bool init_line_buffer(LINE_BUFFER *buffer, File file, ulong size,
                             ulong max_buffer, int encrypt, char *key,
                             char *iv) {
  buffer->file = file;
  buffer->bufread = size;
  ...
  if (!(buffer->cipher_buff =
            (char *)my_malloc(PSI_NOT_INSTRUMENTED, CIPHER_BUFFER_SIZE + 1,
                              MYF(MY_WME | MY_FAE))))
    return true;
  if (!(buffer->clear_buff =
            (char *)my_malloc(PSI_NOT_INSTRUMENTED, CIPHER_BUFFER_SIZE + 1,
                              MYF(MY_WME | MY_FAE))))
    return true;
  buffer->clear_pos = buffer->clear_buff;
  buffer->clear_size = 0;
  buffer->encrypt = encrypt;
  buffer->key = key;
  buffer->iv = (iv && iv[0]) ? iv : nullptr;
  return false;
}
...
static size_t read_buffer(LINE_BUFFER *buffer, size_t count) {
  if (!buffer->key) {
    if ((count = my_read(buffer->file, (uchar *)buffer->end, count,
  ...
  char *end = buffer->end;
  while (count > 0) {
    if (buffer->clear_size > 0) {
      size_t cp_len = count < buffer->clear_size ? count : buffer->clear_size;
      memcpy(end, buffer->clear_pos, cp_len);
      end += cp_len;
      buffer->clear_size -= cp_len;
      count -= cp_len;
      buffer->clear_pos += cp_len;
      ret_count += cp_len;
      continue;
    } else if (buffer->eof) {
      break;
    ...
    size_t read_count = my_read(buffer->file, (uchar *)buffer->cipher_buff,
                                CIPHER_BUFFER_SIZE, MYF(MY_WME));
    if (read_count == MY_FILE_ERROR) {
      return (size_t)-1;
    }

    if (read_count != CIPHER_BUFFER_SIZE) buffer->eof = 1;

    if (read_count > 0 &&
        (read_count = decrypt_buffer(
             buffer->cipher_buff, read_count, buffer->clear_buff,
             (my_aes_opmode)(buffer->encrypt), buffer->key, buffer->iv)) <= 0) {
      return (size_t)-1;
    }

    buffer->clear_size = read_count;
    buffer->clear_pos = buffer->clear_buff;
  }
...
static size_t fill_buffer(LINE_BUFFER *buffer) {
  ...
  /* Read in new stuff. */
  if ((read_count = read_buffer(buffer, read_count)) == MY_FILE_ERROR) {
    buffer->error = my_errno();
    return (size_t)-1;
  }
