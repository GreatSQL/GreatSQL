...
Gcs_message_data::Gcs_message_data(const uint32_t header_capacity,
    ...
    : m_header(NULL), m_header_slider(NULL), m_header_len(0),
      m_header_capacity(header_capacity), m_payload(NULL),
      m_payload_slider(NULL), m_payload_len(0),
      m_payload_capacity(payload_capacity), m_buffer(NULL), m_buffer_len(0),
      m_owner(true) {
...
Gcs_message_data::Gcs_message_data(const uint64_t data_len)
    : m_header(NULL), m_header_slider(NULL), m_header_len(0),
      m_header_capacity(0), m_payload(NULL), m_payload_slider(NULL),
      m_payload_len(0), m_payload_capacity(0), m_buffer(NULL),
      m_buffer_len(data_len), m_owner(true) {
  m_buffer = static_cast<uchar *>(malloc(sizeof(uchar) * m_buffer_len));
  if (!m_buffer) {
...
bool Gcs_message_data::encode(uchar *buffer, uint64_t *buffer_len)
{
  ...
  *buffer_len = encoded_size;
  ...
  MYSQL_GCS_DEBUG_EXECUTE(
      ...
      MYSQL_GCS_LOG_TRACE("Encoded message: (header)="
                          << encoded_header_size
...
bool Gcs_message_data::decode(const uchar *data, uint64_t data_len)
{
  ...
  MYSQL_GCS_LOG_TRACE("Decoded message: (header)="
