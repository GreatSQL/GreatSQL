...
#define LARGE_TRAN_THRESHOLD_FOR_RETRY 16777216
...
static double wakeup_delay(site_def *site, double old);
...
static void synode_set_to_event_horizon(synode_no *s) {
...
static inline node_no max_check(site_def *site) {
#ifdef MAXACCEPT
...
/* Definition of majority */
static inline int majority(bit_set const *nodeset, site_def *s, int all,
...
static int prep_majority(site_def *site, pax_machine *p) {
...
static int prop_majority(site_def *site, pax_machine *p) {
  int ok = 0;
...
site_def *get_executor_site() { return executor_site; }

static site_def *proposer_site = 0;

site_def *get_proposer_site() { return proposer_site; }
...
static pax_msg *create_read(site_def *site, pax_msg *p) {
  p->msg_type = normal;
...
/* purecov: begin deadcode */
static int learn_msg(site_def *site, pax_msg *p) {
  set_learn_type(p);
  p->reply_to = p->proposal;
  brand_app_data(p);
...
/* purecov: end */
static int tiny_learn_msg(site_def *site, pax_msg *p) {
  int retval;
...
static void prepare_push_3p(site_def *site, pax_machine *p, pax_msg *msg,
                            synode_no msgno) {
...
static void push_msg_2p(site_def *site, pax_machine *p) {
  assert(p->proposer.msg);
...
static void push_msg_3p(site_def *site, pax_machine *p, pax_msg *msg,
...
/* purecov: begin deadcode */
static int reject_send(site_def *site, recover_action r) {
...
static int	generator_task(task_arg arg MY_ATTRIBUTE((unused)))
{
  ...
  for (;;) {
    ...
    while (a) {
      ...
      MAY_DBG(FN; COPY_AND_FREE_GOUT(dbg_app_data(a)););
      if (a->body.c_t == exit_type) {
        bury_site(get_group_id(get_site_def()));
        copy_app_data(&a, NULL);
        task_terminate_all(); /* Kill, kill, kill, kill, kill, kill. This is the
                                 end. */
        ...
        xcom_shutdown = 1;
        TERMINATE;
      } else if (a->body.c_t == reset_type ||
                 a->body.c_t == remove_reset_type) {
...
/* purecov: begin deadcode */
static void learn_accepted_value(site_def *site, pax_msg *p, synode_no synode) {
...
#if TASK_DBUG_ON
static void dbg_reply_set(site_def *site, const char *s, bit_set *bs) {
  unsigned int i = 0;
...
static int	proposer_task(task_arg arg)
{
  ...
  while (!xcom_shutdown) { /* Loop until no more work to do */
    ...
    if (ep->client_msg->p->a->chosen) {
      DBGOUT(FN; PTREXP(ep->client_msg->p->a); STRLIT("pushing old ");
             SYCEXP(ep->client_msg->p->a->app_key));
      MAY_DBG(FN; COPY_AND_FREE_GOUT(dbg_pax_msg(ep->client_msg->p)););
    ...
    retry_old:
      ...
      while (!finished(ep->p)) {
        ...
        if (!synode_eq(ep->msgno, ep->p->synode)) {
          DBGOUT(FN;
                 STRLIT("proposer_task detected stolen state machine, retry"););
      ...
      while (!finished(ep->p)) { /* Try to get a value accepted */
        ...
        if (!synode_eq(ep->msgno, ep->p->synode) ||
          ...
          GOTO(retry_new); /* Need to break out of both loops,
                              and we have no "exit named loop"
                              construction */
        }
          ...
          break;
        {
          ...
          if (ep->size > LARGE_TRAN_THRESHOLD_FOR_RETRY) {
            max_wait_time = 30;
          }

          if ((ep->start_push + max_wait_time) <= now) {
            PAX_MSG_SANITY_CHECK(ep->p->proposer.msg);
            DBGOUT(FN; STRLIT("retry pushing "); SYCEXP(ep->msgno));
            G_MESSAGE("retry pushing, timeout:%f, size:%lu", max_wait_time,
                      ep->size);
            MAY_DBG(FN; COPY_AND_FREE_GOUT(dbg_app_data(ep->prepare_msg->a)););
...
if (ep->client_msg) { /* If we get here with a client message, we have failed to
                         deliver */
...
int iamthegreatest(site_def *s) { return leader(s) == s->nodeno; }
...
#ifdef USE_EXIT_TYPE
    case exit_type:
      g_critical("Unable to get message, process will now exit. Please ensure "
...
static void find_value(site_def *site, unsigned int *wait, int n) {
...
int get_xcom_message(pax_machine **p, synode_no msgno, int n) {
  ...
  *p = get_cache(msgno);
  ...
  while (!finished(*p)) {
    site_def *site = find_site_def(msgno);
...
synode_no set_executed_msg(synode_no msgno) {
  DBGOUT(FN; STRLIT("changing executed_msg from "); SYCEXP(executed_msg);
...
static void handle_learn(site_def *site, pax_machine *p, pax_msg *m);
...
static inline int LOSER(synode_no x, site_def *site) {
  /*  node_no i = 0;
          node_no n = 0;
...
#define DBGFIX2(x)
static void send_value(site_def *site, node_no to, synode_no synode) {
...
static inline int is_empty_site(site_def *s) {
...
static inline int is_member(site_def *site) {
...
/*
   ...
   Beware of the exit logic in this task, which is both simple and not so
   simple. Consider three configs C1 and C2. C1 has two nodes, A and B. C2 has
   only node B. C3 is empty. A config with message number N will be activated
   after a delay of (at least) alpha messages, where alpha is the size of the
   pipeline (or the event horizon).

   So, C1.start = C1+alpha,
   and C2.start = C2+alpha. A, which is removed from C1, cannot exit until a
   majority of nodes in the new config C2 (in this case B) has learned all the
   messages from config C1, which means all messages less than C2.start. How can
   A know that a majority of C2 has learned those messages?
   ...
   This leads to the requirement that a node which is removed (A) needs to wait
   until it knows the value of C2.start+alpha, since by then it knows that a
   majority of the nodes in C2 are ready to execute C2.start, which in turn
   implies that a majority of nodes in C2 knows all the values from config C1.
   Note that the last message that should be delivered to the application by a
   node that is leaving C1 is C2.start-1, which is the last message of C1.
   ...
   inform_removed() does this. We take care to handle the case where configs are
   close enough that C0 < C1 <= C0+alpha by tracking the oldest config that
   contains nodes that are leaving.

   This takes care of nodes leaving C1. What about nodes that leave C2? C3 is
   empty, so B, which is leaving C2, cannot wait for messages from C3. But since
   C3 is empty, there is no need to wait. It can exit immediately after having
   executed C3.start-1, the last message of C2. What if C3.start-1 <
   C2.start+alpha? This can happen if C2 and C3 are close. In that case, B will
   exit before A gets the chance to learn C2.start+alpha, which will leave A
   hanging forever. Clearly, we need to impose an additional constraint, that
   C3.start must be greater than C2.start+alpha. This is taken care of by the
   special test for an empty config.

   Complicated and confusing? Not really, but there is a clean and simple
   solution which has not been implemented yet, since it requires more changes
   to the consensus logic. If we require that for the messages C2..C2.start-1 we
   have a majority from both the nodes in C1 and the nodes in C2, the nodes not
   in C2 can exit when they have executed message C2.start-1, since we then know
   that a majority of the nodes of C2 has agreed on those messages as well, so
   they do not depend on the nodes not in C2 any more. This holds even if C2 is
   empty. Note that requiring a majority from both C1 and C2 is different from
   requiring a majority from C1+C2, which means that the proposer logic needs to
   consider answers from two different sets of acceptors for those messages.
   Since acceptors are identified by their node number, and the node numbers
   need not be the same for both configs, we need to maintain a mapping between
   the nodes numbers of any two consecutive configs. Alternatively, we could
   remove the node numbers altogether, and always use a unique, unchanging ID
   for a node, like IP address + port.

 */

/* FIFO which tracks the message numbers where we should deliver queued messages
   or inform the removed nodes */
      ...
      if (ep->state == FETCH) {
        if (!LOSER(executed_msg, executor_site)) {
              ...
              synode_gt(executed_msg, get_site_def()->boot_key)) /* Redo test */
          {
            ...
            DBGFIX2(
                FN; STRLIT("new config "); SYCEXP(
                    site->boot_key);); /*SYCEXP(site->start);
                                          NEXP(get_nodeno(site),d);
                                          NEXP(ARBITRATOR_HACK,d);
        ...
        site_def *x_site = find_site_def(delivered_msg);
...
static void propose_noop(synode_no find, pax_machine *p) {
  ...
  site_def *site = find_site_def(find);
  pax_msg *clone = NULL;
  assert(!too_far(find));
...
/* {{{ Message handlers */
#if 0
void	reply_msg(site_def * site, pax_msg *m)
{
		MAY_DBG(FN; );
...
static void teach_ignorant_node(site_def *site, pax_machine *p, pax_msg *pm,
                                synode_no synode, linkage *reply_queue) {
  CREATE_REPLY(pm);
...
static void handle_read(site_def *site, pax_machine *p, linkage *reply_queue,
                        pax_msg *pm) {
...
#ifdef USE_EXIT_TYPE
static void miss_prepare(site_def *site, pax_msg *pm, linkage *reply_queue) {
  CREATE_REPLY(pm);
...
static void miss_accept(site_def *site, pax_msg *pm, linkage *reply_queue) {
  CREATE_REPLY(pm);
  DBGOUT(FN; SYCEXP(pm->synode));
  ref_msg(reply);
  reply->msg_type = normal;
...
static void handle_simple_prepare(site_def *site, pax_machine *p, pax_msg *pm,
                                  synode_no synode, linkage *reply_queue) {
  if (finished(p)) { /* We have learned a value */
    ...
    if (greater || noop_match(p, pm)) {
      ...
      if (accepted(p)) {                    /* We have accepted a value */
        ...
        if (reply == NULL)
          return; // Failed to allocate memory for the copy.
      } else {
...
static void handle_prepare(site_def *site, pax_machine *p, linkage *reply_queue,
                           pax_msg *pm) {
...
static void check_propose(site_def *site, pax_machine *p) {
  MAY_DBG(FN; SYCEXP(p->synode);
...
static void check_learn(site_def *site, pax_machine *p) {
  MAY_DBG(FN; SYCEXP(p->synode);
          COPY_AND_FREE_GOUT(dbg_machine_nodeset(p, get_maxnodes(site))););
  PAX_MSG_SANITY_CHECK(p->proposer.msg);
...
static void do_learn(site_def *site MY_ATTRIBUTE((unused)), pax_machine *p,
                     pax_msg *m) {
...
static void handle_simple_ack_prepare(site_def *site MY_ATTRIBUTE((unused)),
...
static void handle_ack_prepare(site_def *site, pax_machine *p, pax_msg *m) {
  ADD_EVENTS(add_synode_event(p->synode); add_event(string_arg("m->from"));
             add_event(int_arg(m->from));
             add_event(string_arg(pax_op_to_str(m->op))););
#if 0
...
static void handle_ack_prepare_empty(site_def *site, pax_machine *p,
                                     pax_msg *m) {
  ADD_EVENTS(add_synode_event(p->synode); add_event(string_arg("m->from"));
             add_event(int_arg(m->from));
             add_event(string_arg(pax_op_to_str(m->op))););
...
static void handle_simple_accept(site_def *site, pax_machine *p, pax_msg *m,
                                 synode_no synode, linkage *reply_queue) {
  if (finished(p)) { /* We have learned a value */
...
static void handle_accept(site_def *site, pax_machine *p, linkage *reply_queue,
                          pax_msg *m) {
...
static void handle_ack_accept(site_def *site, pax_machine *p, pax_msg *m) {
  ADD_EVENTS(add_synode_event(p->synode); add_event(string_arg("m->from"));
             add_event(int_arg(m->from));
             add_event(string_arg(pax_op_to_str(m->op))););
...
/* Skip this value */
static void handle_skip(site_def *site, pax_machine *p, pax_msg *m) {
  /*   MAY_DBG(FN;); */
...
#define MAX_PINGS_GATHERED_BEFORE_CONNECTION_SHUTDOWN 15
int pre_process_incoming_ping(site_def *site, pax_msg const *pm,
  ...
  int did_shutdown = 0;
  int max_number_of_pings_received = PINGS_GATHERED_BEFORE_CONNECTION_SHUTDOWN;

  if (site && site->large_content_detected) {
    max_number_of_pings_received =
        MAX_PINGS_GATHERED_BEFORE_CONNECTION_SHUTDOWN;
  }
...
static inline void handle_alive(site_def *site, linkage *reply_queue,
                                pax_msg *pm) {
  int not_to_oneself;
  ...
  not_to_oneself = (pm->from != get_nodeno(site) && pm->from != pm->to);
...
static u_int is_reincarnation_adding(app_data_ptr a)
{
  ...
  site_def *new_site_def = get_site_def();
  site_def *valid_site_def = find_site_def(executed_msg);
  ...
  if (a && a->group_id != 0 && a->group_id != executed_msg.group_id) {
    ...
    case add_node_type:
      log_cfgchange_wrong_group(
          a, "The request to add %s to the group has been rejected because it "
             "is aimed at another group");
      break;
    case remove_node_type:
      log_cfgchange_wrong_group(
          a, "The request to remove %s from the group has been rejected "
             "because it is aimed at another group");
      break;
    case force_config_type:
      G_WARNING("The request to force the group membership has been rejected "
                "because it is aimed at another group");
      break;
    default:
      assert(0 &&
...
int	acceptor_learner_task(task_arg arg)
{
  ...
  while (!xcom_shutdown) {
    ...
    /*
       Getting a pointer to the server needs to be done after we
       have received a message, since without having received a
       message, we cannot know who it is from. We could peek at the
       message and de‐ serialize the message number and from field,
       but since the server does not change, it should be sufficient
       to cache the server in the acceptor_learner task. A cleaner
       solution would have been to move the timestamps out of the
       server object, and have a map in‐ dexed by IP/port or UUID to
       track the timestamps, since this is common to both the
       sender_task, reply_handler_task,  and the ac‐
       ceptor_learner_task.
     */
    ...
    receive_bytes[ep->p->op] += (uint64_t)n + MSG_HDR_SIZE;
    {
      ...
      if (ep->p->msg_type == normal ||
          ...
          (!behind)) {                /* Guard against cache pollution from
                                                                     other nodes */
...
static void server_handle_need_snapshot(server *srv, site_def *s, node_no node);
...
int	reply_handler_task(task_arg arg)
{
  ...
  MAY_DBG(FN; STRLIT(" shutdown "); NDBG(ep->s->con.fd, d);
          NDBG(task_now(), f));
  srv_unref(ep->s);
...
static void server_send_snapshot(server *srv, site_def *s,
...
/* purecov: begin deadcode */
static void send_snapshot(site_def *s, gcs_snapshot *gcs_snap, node_no node) {
...
/* purecov: begin deadcode */
static void handle_need_snapshot(site_def *s, node_no node) {
...
static void server_handle_need_snapshot(server *srv, site_def *s,
                                        node_no node) {
  gcs_snapshot *gs = export_config();
  synode_no app_lsn = get_app_snap(&gs->app_snap);
  if (!synode_eq(null_synode, app_lsn) && synode_lt(app_lsn, gs->log_start)) {
...
static inline result xcom_shut_close_socket(int *sock)
{
  ...
  if (*sock >= 0) {
    ...
    if (DisconnectEx != NULL) {
      (DisconnectEx(*sock, (LPOVERLAPPED)NULL, (DWORD)0, (DWORD)0) == TRUE)
          ? 0
          : -1;
    } else
#endif
...
static int timed_connect(int fd, sockaddr *sock_addr, socklen_t sock_size)
{
  ...
  if (is_socket_error(sysret)) {
    ...
    case SOCK_EALREADY:
      break;
    default:
      G_DEBUG("connect - Error connecting (socket=%d, error=%d).", fd, syserr);
      CONNECT_FAIL;
    }
    ...
    if (is_socket_error(sysret)) {
      G_DEBUG("poll - Error while connecting! (socket= %d, error=%d)", fd,
              syserr);
      CONNECT_FAIL;
    }
...
static connection_descriptor*	connect_xcom(char *server, xcom_port port)
{
...

  {
    ...
    if (peer >= 0) {
      if (check_tcp_connection_valid(fd.val, &same_ip) == FALSE) {
      ...
      if (ret.val < 0) {
        ...
        G_DEBUG(
            ...
            server, ret.funerr);
#else
        G_DEBUG("Setting node delay failed while connecting to %s with error "
                "%d - %s.",
      ...
      xcom_shut_close_socket(&fd.val);
#if defined (WIN32) || defined (WIN64)
      G_DEBUG("Getting the peer name failed while connecting to server %s with "
              "error %d.",
              server, ret.funerr);
#else
      G_DEBUG("Getting the peer name failed while connecting to server %s with "
...
int xcom_send_app_wait(connection_descriptor *fd, app_data *a, int force) {
  ...
  G_MESSAGE("Request failed: maximum number of retries (1) has been "
            "exhausted.");
  return 0;
}
