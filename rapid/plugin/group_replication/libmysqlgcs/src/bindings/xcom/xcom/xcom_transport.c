...
static bool_t
dummyfunc2 (XDR *xdr MY_ATTRIBUTE((unused)),
#ifdef OLD_XDR
...
static bool_t
dummyfunc3 (XDR *xdr MY_ATTRIBUTE((unused)),
...
static double getusec() {
  ...
  sec = tp.tv_sec * 1000000 + (1.0 * tp.tv_usec);
  return sec;
}
...
bool_t check_tcp_connection_valid(int fd, int *same_ip) {
  ...
  *same_ip = 0;
  /* Get client address */
  if (retrieve_addr_from_fd(fd, TRUE, client_addr, &client_port) == FALSE) {
    result = FALSE;
  } else {
    /* Get server address */
...
static int dial(server *s) {
  ...
  } else if (check_tcp_connection_valid(s->con.fd, &ep->same_ip) == FALSE) {
...
int	tcp_reaper_task(task_arg arg MY_ATTRIBUTE((unused)))
{
  ...
  while (!xcom_shutdown) {
    ...
    for (i = 0; i < maxservers; i++) {
      server *s = all_servers[i];
      if (s && s->con.fd != -1 && (s->active + 3600.0) < now) {
...
/*
   ...
   convenient way of sending something to a specific address/port without
   blocking the task system. Error handling is very rudimentary.
 */
/* purecov: begin deadcode */
/* Try to connect to another node */
      ...
      TASK_DELAY(1.000);
		...
		if (xcom_use_ssl()) {
...
#endif
                {
                  ...
                  /* Send protocol negotiation request */
                  DBGOUT(FN);
                  TASK_CALL(send_proto(&ep->c_descriptor, my_xcom_version,
                                       x_version_req, TAG_START, &sent));
                  if (sent < 0) {
                    TERMINATE_CLIENT(ep);
                  }
                  ...
                  TASK_CALL(recv_proto(&ep->c_descriptor, &ep->x_proto,
                                       &ep->x_type, &ep->tag, &n));
                  if (n < 0) {
                    TERMINATE_CLIENT(ep);
                  }
                  ...
                  if (ep->tag == TAG_START && ep->x_type == x_version_reply) {
                    DBGOUT(STRLIT("client task will use protcol version ");
                           NDBG(ep->x_proto, u);
                           STRLIT(xcom_proto_to_str(ep->x_proto)));
                    ...
                    /* Send message */
                    serialize_msg(ep->s->p, ep->c_descriptor.x_proto,
                                  &ep->buflen, &ep->buf);
                    if (ep->buflen) {
                      DBGOUT(FN);
                      TASK_CALL(task_write(&ep->c_descriptor, ep->buf,
                                           ep->buflen, &sent));
                      if (ep->buflen != sent) {
                        DBGOUT(FN; STRLIT("write failed "); STRLIT(ep->s->srv);
                               NDBG(ep->s->port, d); NDBG(ep->buflen, d);
                               NDBG(sent, d));
                        TERMINATE_CLIENT(ep);
                      }
                    }
...
/*
   ...
   copied) xdr function here is OK.
 */
/* purecov: begin deadcode */
...
bool_t xdr_node_list_1_1(XDR *xdrs, node_list_1_1 *objp)
{
  ...
  case x_1_0:
    return xdr_array(
        xdrs, (char **)&objp->node_list_val, (u_int *)&objp->node_list_len,
        NSERVERS, sizeof(node_address), (xdrproc_t)xdr_node_address_with_1_0);
  case x_1_1:
  case x_1_2:
    return xdr_array(xdrs, (char **)&objp->node_list_val,
                     (u_int *)&objp->node_list_len, NSERVERS,
