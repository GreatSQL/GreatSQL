...
static void task_init(task_env *t)
{
  ...
  /* #ifdef __sun */
  /*	 mem_watch(&ash_nazg_gimbatul,sizeof(&ash_nazg_gimbatul),
   * WA_WRITE); */
  /* #endif */
  assert(ash_nazg_gimbatul.type == type_hash("task_env"));
  /* assert(ash_nazg_gimbatul.suc > (linkage*)0x8000000); */
...
static int delayed_tasks() { return !task_queue_empty(&task_time_q); }
     ...
     */
    {
      ...
      if (delayed_tasks()) {
        ...
        if (ms > 0) {
          ...
          poll_wait(ms); /* Wait at most ms milliseconds and poll
                                            for IO */
...
int	connect_tcp(char *server, xcom_port port, int *ret)
{
  ...
  retry:
    ...
    TASK_YIELD;
    /* See if we timed out here. If we did, connect may or may not
       be active. If closing fails with EINPROGRESS, we need to
       retry the select. If close does not fail, we know that
       connect has indeed failed, and we exit from here and return
       -1 as socket fd */
    if (stack->interrupt) {
      result shut = {0, 0};
      stack->interrupt = 0;
...
bool_t retrieve_addr_from_fd(int fd, bool_t client, char *ip, int *port) {
  struct sockaddr_storage server;
  socklen_t len = (socklen_t)sizeof(struct sockaddr_storage);
  memset(&server, 0, sizeof(struct sockaddr_storage));
  if (client == TRUE) {
    if (getpeername(fd, (struct sockaddr *)&server, &len)) {
      return FALSE;
    }
  } else {
...
static void dump_range(double t MY_ATTRIBUTE((unused)), int start, int end) {
  int i;
  for (i = start; i < end; i++) {
...
{
  double t = 0.0;
  G_DEBUG("cur_task_event %d max_task_event %d", cur_task_event,
          max_task_event);
  add_event(end_arg());
