...
#define MAX_GTID_UNCHANGED_TIME 600
...
Certifier_broadcast_thread::Certifier_broadcast_thread()
  ...
  last_gtid_executed_string = NULL;
  gtid_executed_same = false;
}
...
Certifier_broadcast_thread::~Certifier_broadcast_thread()
{
  ...
  if (last_gtid_executed_string) {
    my_free(last_gtid_executed_string);
    last_gtid_executed_string = NULL;
  }
}
...
void Certifier_broadcast_thread::dispatcher()
{
  ...
  size_t last_record_gtid_changed_base = 0;
  ulonglong last_positive_cert = 0;
  ulonglong curr_positive_cert = 0;
  ...
  while (!aborted) {
    ...
    certify->garbage_collect();

    applier_module->get_flow_control_module()->flow_control_step();

    {
      /* Check if slave is dead locking */
      size_t checked_counter = gc_counter / 10;
      /* Defend slave dead lock */
      curr_positive_cert = certify->get_positive_certified();
      if (curr_positive_cert == last_positive_cert) {
        last_record_gtid_changed_base = checked_counter;
      } else {
        if (gtid_executed_same && last_record_gtid_changed_base > 0) {
          size_t diff = checked_counter - last_record_gtid_changed_base;
          if (diff >= MAX_GTID_UNCHANGED_TIME) {
            aborted = true;
            log_message(MY_ERROR_LEVEL,
                        "Wait gtid changed for too long and check slave "
                        "deadlock, timediff:%lu",
                        diff);
      ...
      last_positive_cert = curr_positive_cert;
    }
...
int Certifier_broadcast_thread::broadcast_gtid_executed()
{
  ...
  if (send_err == GCS_MESSAGE_TOO_BIG) {
    log_message(MY_ERROR_LEVEL,
                "Broadcast of committed transactions message "
                "failed. Message is too big."); /* purecov: inspected */
    error = 1;                                  /* purecov: inspected */
  } else if (send_err == GCS_NOK) {
    log_message(
        MY_INFORMATION_LEVEL,
        "Broadcast of committed transactions message failed."); /* purecov:
                                                                   inspected */
    error = 1; /* purecov: inspected */
  }
  ...
  if (last_gtid_executed_string &&
      strcmp(encoded_gtid_executed_string, last_gtid_executed_string) == 0) {
    gtid_executed_same = true;
    my_free(encoded_gtid_executed_string);
  } else {
    if (last_gtid_executed_string) {
      my_free(last_gtid_executed_string);
    }
    last_gtid_executed_string = encoded_gtid_executed_string;
    gtid_executed_same = false;
  }
  ...
  if (get_server_gtid_retrieved) {
    ...
    if (applier_channel.get_retrieved_gtid_set(applier_retrieved_gtids)) {
      log_message(
          ...
          " Certification module can't be properly initialized"); /* purecov:
                                                                     inspected
                                                                   */
      error = 1; /* purecov: inspected */
      goto end;  /* purecov: inspected */
...
rpl_gno Certifier::certify(Gtid_set *snapshot_version,
  ...
  if (generate_group_id) {
    ...
    if (group_gtid_executed->contains_gtid(sidno_for_group_gtid_sid_map,
      ...
      log_message(MY_ERROR_LEVEL,
                  "The requested GTID '%s:%lld' was already used, the "
    ...
    if (last_conflict_free_transaction_sidno < 1) {
      log_message(MY_WARNING_LEVEL,
                  ...
                  "performance_schema.replication_group_member_stats.last_"
                  "conflict_free_transaction"); /* purecov: inspected */
    } else {
...
bool Certifier::add_item(const char *item, Gtid_set_ref *snapshot_version,
  ...
  if (it == certification_info.end()) {
    ...
    *item_previous_sequence_number =
    ...
    Certification_index::iterator search =
        certification_index.find(orig_timestamp);
    if (search == certification_index.end()) {
      log_message(MY_WARNING_LEVEL, "not found, orig_timestamp:%lu",
                  orig_timestamp);
    } else {
...
Gtid_set *Certifier::get_certified_write_set_snapshot_version(const char* item)
{
  ...
  if (it == certification_info.end())
    ...
    DBUG_RETURN(it->second.gtid_ref);
}
...
bool Certifier::set_group_stable_transactions_set(Gtid_set* executed_gtid_set)
{
  ...
  if (executed_gtid_set == NULL) {
    log_message(MY_ERROR_LEVEL,
                "Invalid stable transactions set"); /* purecov: inspected */
    DBUG_RETURN(true);                              /* purecov: inspected */
  }
  ...
  if (stable_gtid_set->add_gtid_set(executed_gtid_set) != RETURN_STATUS_OK) {
    ...
    log_message(
        MY_ERROR_LEVEL,
        "Error updating stable transactions set"); /* purecov: inspected */
    DBUG_RETURN(true);                             /* purecov: inspected */
  }
  ...
  if (channel_add_executed_gtids_to_received_gtids(
    ...
    log_message(MY_WARNING_LEVEL, "There was an error when filling the missing "
                                  "GTIDs on "
                                  "the applier channel received set. Despite "
                                  "not critical, "
                                  "on the long run this may cause performance "
                                  "issues"); /* purecov:
                                                                                                        inspected */
  }
...
void Certifier::garbage_collect()
{
  ...
  Certification_index::iterator current = certification_index.begin();
  Certification_index::iterator end = certification_index.end();
  stable_gtid_set_lock->wrlock();

  while (current != end && count < cert_size) {
    const std::string &value = current->second;
    Certification_info::iterator it = certification_info.find(value);
    ...
    if (it != certification_info.end()) {
      ...
      log_message(MY_WARNING_LEVEL, "Certification index is corrupted");
      break;
    }
...
int Certifier::handle_certifier_data(
  ...
  if (this->get_members_size() != plugin_get_group_members_number()) {
    ...
    log_message(MY_WARNING_LEVEL,
                "Skipping this round of stable set "
...
int Certifier::stable_set_handle()
{
  ...
  while (!error && !this->incoming->empty()) {
    ...
    if (packet == NULL) {
      log_message(MY_ERROR_LEVEL,
                  "Null packet on certifier's queue"); /* purecov: inspected */
      error = 1;                                       /* purecov: inspected */
    ...
    if (member_set.add_gtid_encoding(payload, packet->len) !=
      ...
      log_message(
          MY_ERROR_LEVEL,
          "Error reading GTIDs from the message"); /* purecov: inspected */
      error = 1;                                   /* purecov: inspected */
    } else {
      /*
      ...
      if (executed_set.is_empty()) {
        if (executed_set.add_gtid_set(&member_set)) {
          log_message(
              MY_ERROR_LEVEL,
              "Error processing stable transactions set"); /* purecov: inspected
                                                            */
          error = 1; /* purecov: inspected */
        }
      } else {
...
set that value to executed_set to be used on the next intersection.
        ...
        if (member_set.intersection(&executed_set, &intersection_result) !=
          ...
          log_message(
              MY_ERROR_LEVEL,
              "Error processing intersection of stable transactions set"); /* purecov:
                                                                              inspected
                                                                            */
          error = 1; /* purecov: inspected */
        } else {
          ...
          if (executed_set.add_gtid_set(&intersection_result) !=
            ...
            log_message(
                MY_ERROR_LEVEL,
                "Error processing stable transactions set"); /* purecov:
                                                                inspected */
            error = 1; /* purecov: inspected */
          }
        }
  ...
  if (!error && set_group_stable_transactions_set(&executed_set)) {
    log_message(
        MY_ERROR_LEVEL,
        "Error setting stable transactions set"); /* purecov: inspected */
    error = 1;                                    /* purecov: inspected */
  }
  ...
  for (std::map<std::string, std::string>::iterator it = cert_info->begin();
    ...
    if (it->first.compare(GTID_EXTRACTED_NAME) == 0) {
      if (group_gtid_extracted->add_gtid_encoding(
        ...
        log_message(MY_ERROR_LEVEL,
                    "Error reading group_gtid_extracted from the "
                    "View_change_log_event");         /* purecov: inspected */
        mysql_mutex_unlock(&LOCK_certification_info); /* purecov: inspected */
        DBUG_RETURN(1);                               /* purecov: inspected */
      }
      continue;
    ...
    if (value->add_gtid_encoding(
            ...
            it->second.length()) != RETURN_STATUS_OK) {
      log_message(MY_ERROR_LEVEL,
                  "Error reading the write set item '%s' from the "
