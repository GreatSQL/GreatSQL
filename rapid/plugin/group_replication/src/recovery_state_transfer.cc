...
Recovery_state_transfer::Recovery_state_transfer(
    ...
    : selected_donor(NULL), group_members(NULL),
      donor_connection_retry_count(0), recovery_aborted(false),
      donor_transfer_finished(false), connected_to_donor(false),
      on_failover(false), donor_connection_interface(recovery_channel_name),
      channel_observation_manager(channel_obsr_mngr),
      recovery_channel_observer(NULL), recovery_use_ssl(false),
  ...
  if (donor_left) {
    ...
    if (connected_to_donor) {
      /*
         ...
         2) The flag is false and remains false so we restart the connection,
         and that new connection will deliver the rest of the data 3) The flag
         turns true while we are restarting the connection. In this case we will
         probably create a new connection that won't be needed and will be
         terminated the instant the lock is freed.
       */
  ...
  while (error != 0 && !recovery_aborted) {
    ...
    DBUG_EXECUTE_IF("gr_reset_max_connection_attempts_to_donors", {
      if (donor_connection_retry_count == 3) {
        const char act[] = "now signal signal.connection_attempt_3 wait_for "
    ...
    if (donor_connection_retry_count >= max_connection_attempts_to_donors) {
      log_message(MY_ERROR_LEVEL, "Maximum number of retries when trying to "
...
int Recovery_state_transfer::initialize_donor_connection()
{
  ...
  error = donor_connection_interface.initialize_channel(
      ...
      recovery_ssl_verify_server_cert, DEFAULT_THREAD_PRIORITY, 1, false, true,
      true);
...
int Recovery_state_transfer::purge_recovery_slave_threads_repos()
{
  ...
  error = donor_connection_interface.initialize_channel(
      ...
      NULL, NULL, NULL, NULL, DEFAULT_THREAD_PRIORITY, 1, false, true, true);
