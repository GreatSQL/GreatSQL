...
ulong request_time_threshold_var = 0;
...
ulong flow_control_replay_lag_behind_var = 600;
ulong flow_control_max_wait_time_var = 3600;
...
static void option_deprecation_warning(MYSQL_THD thd, const char* name)
{
  ...
  log_message(MY_WARNING_LEVEL,
              ER_DEFAULT(ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT), name);
}
...
int plugin_group_replication_start()
{
  ...
  if (allow_local_disjoint_gtids_join_var) {
    option_deprecation_warning(
        current_thd, "group_replication_allow_local_disjoint_gtids_join");
  }
...
int initialize_plugin_and_join(
  ...
  if (sql_command_interface->establish_session_connection(sql_api_isolation,
                                                          plugin_info_ptr)) {
  ...
  DBUG_EXECUTE_IF("group_replication_compatibility_rule_error_major", {
    ...
    compatibility_mgr->add_incompatibility(current_version, other_version);
  };);
  ...
  if ((error = start_group_communication())) {
    log_message(MY_ERROR_LEVEL, "Error on group communication engine start");
    goto err;
  }
...
int configure_group_member_manager(char *hostname, char *uuid, uint port,
  ...
  if (!strcmp(uuid, group_name_var)) {
    log_message(MY_ERROR_LEVEL,
                ...
                "Server_uuid %s matches group_name %s.",
                uuid, group_name_var);
    DBUG_RETURN(GROUP_REPLICATION_CONFIGURATION_ERROR);
  }
  ...
  if (gcs_module->belongs_to_group()) {
    ...
    case Gcs_operations::ERROR_WHEN_LEAVING:
      ...
      ss << "Unable to confirm whether the server has left the group or not. "
            "Check performance_schema.replication_group_members to check group "
            "membership information.";
      log_severity = MY_ERROR_LEVEL;
      break;
      /* purecov: end */
    case Gcs_operations::ALREADY_LEAVING:
      ss << "Skipping leave operation: concurrent attempt to leave the group "
            "is on-going.";
      break;
...
int plugin_group_replication_stop()
{
  ...
  DBUG_EXECUTE_IF("group_replication_wait_on_stop", {
    const char act[] =
        "now signal signal.stop_waiting wait_for signal.stop_continue";
    assert(!debug_sync_set_action(current_thd, STRING_WITH_LEN(act)));
  });
  ...
  log_message(MY_INFORMATION_LEVEL, "Plugin 'group_replication' is stopping.");

  plugin_is_waiting_to_set_server_read_mode = true;

  DBUG_EXECUTE_IF("group_replication_hold_stop_before_leave_the_group", {
    const char act[] = "now signal signal.stopping_before_leave_the_group "
...
int terminate_plugin_modules(bool flag_stop_async_channel)
{
  ...
  if (terminate_recovery_module()) {
    ...
    log_message(MY_WARNING_LEVEL, "On shutdown there was a timeout on the "
                                  "Group Replication "
                                  "recovery module termination. Check the log "
                                  "for more details"); /* purecov:
                                                          inspected
                                                        */
  }
  ...
  if (flag_stop_async_channel) {
    ...
    if (channel_err) {
      log_message(MY_ERROR_LEVEL,
                  ...
                  "additional details. Got error: %d",
                  channel_err);
      if (!error) {
...
int plugin_group_replication_init(MYSQL_PLUGIN plugin_info)
{
  ...
  if (start_group_replication_at_boot_var && plugin_group_replication_start()) {
    log_message(MY_ERROR_LEVEL, "Unable to start Group Replication on boot");
  }
...
int plugin_group_replication_deinit(void *p)
{
  ...
  if (observer_unregister_error == 0)
    log_message(MY_INFORMATION_LEVEL, "All Group Replication server observers"
...
int initialize_recovery_module()
{
  recovery_module = new Recovery_module(
      applier_module, channel_observation_manager, components_stop_timeout_var);
...
ulong get_transaction_size_limit()
{
  ...
  ulong limit =
      static_cast<ulong>(my_atomic_load64(&transaction_size_limit_var));
  DBUG_RETURN(limit);
}

ulong get_request_time_threshold() { return request_time_threshold_var; }
...
static int check_if_server_properly_configured()
{
  ...
  if (startup_pre_reqs.binlog_checksum_options !=
    ...
    log_message(MY_ERROR_LEVEL,
                "binlog_checksum should be NONE for Group Replication");
    DBUG_RETURN(1);
  }
  ...
  {
    log_message(MY_ERROR_LEVEL,
                "Relay log info repository must be set to TABLE");
    DBUG_RETURN(1);
  }
...
static int check_group_name_string(const char *str, bool is_var_update)
{
  ...
  if (strlen(str) > UUID_LENGTH) {
    if (!is_var_update)
      log_message(MY_ERROR_LEVEL,
                  "The group name '%s' is not a valid UUID, its"
                  " length is too big",
                  str);
    else
  ...
  if (!Uuid::is_valid(str)) {
    if (!is_var_update)
      log_message(MY_ERROR_LEVEL, "The group name '%s' is not a valid UUID",
...
static void update_ssl_use(MYSQL_THD thd, SYS_VAR *var, void *var_ptr,
                           const void *save) {
...
static void update_ssl_server_cert_verification(MYSQL_THD thd, SYS_VAR *var,
                                                void *var_ptr,
                                                const void *save) {
...
static void update_recovery_completion_policy(MYSQL_THD thd, SYS_VAR *var,
                                              void *var_ptr, const void *save) {
...
static void update_component_timeout(MYSQL_THD thd, SYS_VAR *var, void *var_ptr,
                                     const void *save) {
...
static int check_compression_threshold(MYSQL_THD thd, SYS_VAR *var, void *save,
                                       struct st_mysql_value *value) {
...
static int check_force_members(MYSQL_THD thd, SYS_VAR *var, void *save,
                               struct st_mysql_value *value) {
  ...
  if (force_members_running) {
    log_message(MY_ERROR_LEVEL, "There is one group_replication_force_members "
...
static int check_single_primary_mode(MYSQL_THD thd, SYS_VAR *var, void *save,
                                     struct st_mysql_value *value) {
...
static int
check_enforce_update_everywhere_checks(MYSQL_THD thd, SYS_VAR *var, void *save,
                                       struct st_mysql_value *value) {
...
static void update_allow_local_disjoint_gtids_join(MYSQL_THD thd, SYS_VAR *var,
                                                   void *var_ptr,
                                                   const void *save) {
  ...
  option_deprecation_warning(
      thd, "group_replication_allow_local_disjoint_gtids_join");
...
static MYSQL_SYSVAR_ULONG(recovery_retry_count,     /* name */
                          recovery_retry_count_var, /* var */
                          PLUGIN_VAR_OPCMDARG,      /* optional var */
                          "The number of times that the joiner tries to "
...
static MYSQL_SYSVAR_ULONG(recovery_reconnect_interval,     /* name */
                          recovery_reconnect_interval_var, /* var */
                          PLUGIN_VAR_OPCMDARG,             /* optional var */
                          "The sleep time between reconnection attempts when "
...
static MYSQL_SYSVAR_ULONG(components_stop_timeout,     /* name */
...
static MYSQL_SYSVAR_BOOL(
    ...
    PLUGIN_VAR_OPCMDARG,                 /* optional var */
    "Allow this server to join the group even if it has transactions not "
...
static MYSQL_SYSVAR_ULONG(
    ...
    PLUGIN_VAR_OPCMDARG |
        PLUGIN_VAR_NODEFAULT, /* optional var | no set default */
...
static MYSQL_SYSVAR_ULONG(
    ...
    flow_control_replay_lag_behind_var, /* var */
    PLUGIN_VAR_OPCMDARG,                /* optional var */
...
static MYSQL_SYSVAR_ULONG(flow_control_max_wait_time,     /* name */
                          flow_control_max_wait_time_var, /* var */
                          PLUGIN_VAR_OPCMDARG,            /* optional var */
...
static MYSQL_SYSVAR_ULONG(transaction_size_limit,          /* name */
                          transaction_size_limit_base_var, /* var */
                          PLUGIN_VAR_OPCMDARG,             /* optional var */
                          "Specifies the limit of transaction size that can be "
...
static MYSQL_SYSVAR_ULONG(unreachable_majority_timeout, /* name */
                          ...
                          PLUGIN_VAR_OPCMDARG,          /* optional var */
                          "The number of seconds before going into error when "
                          "a majority of members is unreachable."
...
static int show_primary_member(MYSQL_THD thd, SHOW_VAR *var, char *buff) {
  var->type = SHOW_CHAR;
  var->value = NULL;
