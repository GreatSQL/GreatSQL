...
Sql_service_command_interface::Sql_service_command_interface()
    : connection_thread_isolation(PSESSION_USE_THREAD),
      m_server_interface(NULL), m_plugin_session_thread(NULL) {}
...
int Sql_service_command_interface::establish_session_connection(
    enum_plugin_con_isolation isolation_param, void *plugin_pointer) {
...
long Sql_service_command_interface::set_read_only()
{
  ...
  if (connection_thread_isolation != PSESSION_DEDICATED_THREAD) {
    ...
    m_plugin_session_thread->queue_new_method_for_application(
        &Sql_service_commands::internal_set_read_only);
    error = m_plugin_session_thread->wait_for_method_execution();
  }
...
internal_set_read_only(Sql_service_interface *sql_interface)
{
  ...
  if (srv_err == 0) {
    ...
    log_message(MY_ERROR_LEVEL,
                "'SET read_only= 1' query execution"
                " resulted in failure. errno: %d",
...
Session_plugin_thread::Session_plugin_thread(
    ...
    : command_interface(command_interface), m_server_interface(NULL),
      ...
      m_session_thread_terminate(false), m_session_thread_error(0) {
...
int Session_plugin_thread::launch_session_thread(void *plugin_pointer_var) {
  ...
  if ((mysql_thread_create(key_GR_THD_plugin_session, &m_plugin_session_pthd,
    ...
    log_message(MY_ERROR_LEVEL, "Increase the number of threads that could be "
...
int Session_plugin_thread::terminate_session_thread() {
  ...
  while (m_session_thread_running || m_session_thread_starting) {
    ...
    } else if (m_session_thread_running ||
               m_session_thread_starting) // quit waiting
    {
      mysql_mutex_unlock(&m_run_lock);
      DBUG_RETURN(1);
    }
...
int Session_plugin_thread::session_thread_handler() {
  ...
  while (!m_session_thread_terminate) {
    ...
    m_method_execution_return_value =
        (command_interface->*method_to_execute)(m_server_interface);
