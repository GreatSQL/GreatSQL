  ...
  if (!view_change_packet->group_executed_set.empty()) {
    ...
    if (intersect_group_executed_sets(view_change_packet->group_executed_set,
      ...
      log_message(
          ...
          "some recovery operations may face future issues"); /* purecov:
                                                                 inspected */
...
int Applier_module::applier_thread_handle() {
  ...
  if (!cache || (!my_b_inited(cache) &&
    ...
    log_message(
        MY_ERROR_LEVEL, "Failed to create group replication pipeline applier "
                        "cache!"); /* purecov:
                                                                                                      inspected
                                                                                                    */
  ...
  if (pipeline != NULL) {
    if ((error = pipeline->terminate_pipeline())) {
      log_message(
          ...
          " disposed. Check the error log for further info."); /* purecov:
                                                                  inspected */
    }
...
void Applier_module::inform_of_applier_stop(char *channel_name, bool aborted) {
  ...
  if (!strcmp(channel_name, applier_module_channel_name) && aborted &&
      applier_running) {
    log_message(MY_ERROR_LEVEL, "The applier thread execution was aborted."
...
void Applier_module::leave_group_on_failure()
{
  ...
  case Gcs_operations::ALREADY_LEAVING:
    ss << "Skipping leave operation: concurrent attempt to leave the group is "
          "on-going."; /* purecov: inspected */
    break;             /* purecov: inspected */
  case Gcs_operations::ALREADY_LEFT:
    ss << "Skipping leave operation: member already left the group."; /* purecov:
                                                                         inspected
                                                                       */
    break; /* purecov: inspected */
  case Gcs_operations::NOW_LEAVING:
    set_read_mode = true;
    ss << "The server was automatically set into read only mode after an error "
          "was detected.";
    log_severity = MY_ERROR_LEVEL;
    break;
  }
