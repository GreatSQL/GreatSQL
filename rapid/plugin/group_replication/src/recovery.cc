...
int Recovery_module::start_recovery(const string &group_name,
  ...
  if (recovery_state_transfer.check_recovery_thread_status()) {
    ...
    log_message(MY_ERROR_LEVEL, "A previous recovery session is still running. "
...
void Recovery_module::leave_group_on_recovery_failure()
{
  log_message(MY_ERROR_LEVEL,
              "Fatal error during the Recovery process of "
...
cleanup:
  ...
  /*
     If recovery failed, it's no use to continue in the group as the member
     cannot take an active part in it, so it must leave.
   */
  if (error) {
...
void Recovery_module::notify_group_recovery_end()
{
  ...
  if (msg_error != GCS_OK) {
    log_message(
        MY_ERROR_LEVEL,
        "Error while sending message for group replication recovery"); /* purecov:
                                                                          inspected
                                                                        */
  }
