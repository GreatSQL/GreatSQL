...
int add_write_set(Transaction_context_log_event *tcle,
  ...
  for (int i = 0; i < iterator; i++) {
    ...
    char *write_set_value = (char *)my_malloc(
        PSI_NOT_INSTRUMENTED, static_cast<size_t>(tmp_str_sz), MYF(MY_WME));
    if (!write_set_value) {
      /* purecov: begin inspected */
      log_message(MY_ERROR_LEVEL,
                  "No memory to generate write identification hash");
      DBUG_RETURN(1);
      /* purecov: end */
    }
...
int group_replication_trans_before_dml(Trans_param *param, int& out)
{
  ...
  for (uint table = 0; out == 0 && table < param->number_of_tables; table++) {
    if (param->tables_info[table].db_type != DB_TYPE_INNODB) {
      log_message(MY_ERROR_LEVEL,
                  "Table %s does not use the InnoDB storage "
    ...
    if (param->tables_info[table].number_of_primary_keys == 0) {
      log_message(MY_ERROR_LEVEL,
                  "Table %s does not have any PRIMARY KEY. This is not "
    ...
    if (local_member_info->has_enforces_update_everywhere_checks() &&
      ...
      log_message(MY_ERROR_LEVEL,
                  "Table %s has a foreign key with"
...
int group_replication_trans_before_commit(Trans_param *param)
{
  ...
  if (member_status == Group_member_info::MEMBER_ERROR) {
    log_message(MY_ERROR_LEVEL, "Transaction cannot be executed while Group "
                                "Replication is on ERROR state."
  ...
  if (trx_cache_log_position > 0 && stmt_cache_log_position == 0) {
    ...
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "We can only use one cache type at a "
                "time on session %u",
                param->thread_id);
    shared_plugin_stop_lock->release_read_lock();
    DBUG_RETURN(1);
  ...
  if (reinit_cache(cache_log, READ_CACHE, 0)) {
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Failed to reinit binlog cache log for read "
                "on session %u",
                param->thread_id);
    error = pre_wait_error;
    goto err;
  ...
  tcle = new Transaction_context_log_event(param->server_uuid, is_dml,
                                           param->thread_id, is_gtid_specified);
  if (!tcle->is_valid()) {
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Failed to create the context of the current "
                "transaction on session %u",
                param->thread_id);
    error = pre_wait_error;
    goto err;
    /* purecov: end */
  ...
  if (is_dml) {
    ...
    if ((write_set == NULL) && (!is_gtid_specified)) {
      log_message(MY_ERROR_LEVEL,
                  "Failed to extract the set of items written "
    ...
    if (write_set != NULL) {
      if (add_write_set(tcle, write_set)) {
        ...
        log_message(MY_ERROR_LEVEL,
                    "Failed to gather the set of items written "
                    "during the execution of the current "
                    "transaction on session %u",
                    param->thread_id);
        error = pre_wait_error;
  ...
  gle = new Gtid_log_event(param->server_id, is_dml, 0, 1, may_have_sbr_stmts,
                           gtid_specification);
  gle->write(cache);
  ...
  if (reinit_cache(cache, READ_CACHE, 0)) {
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Error while re-initializing an internal "
  ...
  if (transaction_msg.append_cache(cache)) {
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Error while appending data to an internal "
                "cache on session %u",
                param->thread_id);
    error = pre_wait_error;
    goto err;
  ...
  if (transaction_msg.append_cache(cache_log)) {
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Error while writing binary log cache on "
                "session %u",
                param->thread_id);
    error = pre_wait_error;
    goto err;
  ...
  if (certification_latch->registerTicket(param->thread_id)) {
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Unable to register for getting notifications "
  ...
  if (send_error == GCS_MESSAGE_TOO_BIG) {
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Error broadcasting transaction to the group "
    ...
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Error while broadcasting the transaction to "
                "the group on session %u",
                param->thread_id);
    error = pre_wait_error;
    goto err;
    /* purecov: end */
  ...
  if (certification_latch->waitTicket(param->thread_id)) {
    /* purecov: begin inspected */
    log_message(MY_ERROR_LEVEL,
                "Error while waiting for conflict detection "
