...
Certification_handler::Certification_handler()
    : cert_module(NULL), applier_module_thd(NULL), group_sidno(0),
      transaction_context_packet(NULL), transaction_context_pevent(NULL),
...
int Certification_handler::handle_transaction_id(Pipeline_event *pevent,
  ...
  if (local_transaction) {
    ...
    if (set_transaction_ctx(transaction_termination_ctx)) {
      /* purecov: begin inspected */
      log_message(MY_ERROR_LEVEL,
                  "Unable to update certification result on server side, "
                  "thread_id: %lu",
                  tcle->get_thread_id());
      cont->signal(1, true);
      error = 1;
    ...
    if (seq_number > 0) {
      if (tcle->is_gtid_specified()) {
        ...
        if (error) {
          log_message(MY_ERROR_LEVEL,
                      "Unable to add gtid information to the "
    ...
    if (seq_number > 0) {
      if (!tcle->is_gtid_specified()) {
        ...
        Gtid_log_event *gle_generated = new Gtid_log_event(
            ...
            gle->sequence_number, gle->may_have_sbr_stmts, gtid_specification);
  ...
  if (error) {
    if (LOCAL_WAIT_TIMEOUT_ERROR == error) {
      ...
      log_message(MY_WARNING_LEVEL,
                  "Unable to log the group change View log event in its "
                  "exaction position in the log."
                  " This will not however affect the group replication "
                  "recovery process or the overall plugin process.");
      if (error)
        cont->signal(1, false);
      else
...
int Certification_handler::inject_transactional_events(Pipeline_event *pevent,
  ...
  Gtid_log_event *gtid_log_event = new Gtid_log_event(
      event->server_id, true, 0, 0, true, gtid_specification);
  ...
  /*
     ...
     If that is no longer true, another Continuation object must be created
     here.
   */
  next(pevent, cont);
  error = cont->wait();
...
int Certification_handler::log_view_change_event_in_order(
  ...
  /*
     A -1 view id means this event was queued to make the applier pipeline retry
     the logging of a view change log event that was not successful in the past.
