...
Plugin_gcs_events_handler::handle_certifier_message(const Gcs_message& message) const
{
  if (this->applier_module == NULL) {
    log_message(
        MY_ERROR_LEVEL,
        "Message received without a proper group replication applier"); /* purecov:
                                                                           inspected
                                                                         */
...
void Plugin_gcs_events_handler::on_suspicions(
  ...
  log_message(MY_INFORMATION_LEVEL, "on_suspicions is activated");
  ...
  if (!members.empty()) {
    for (mit = members.begin(); mit != members.end(); mit++) {
      ...
      if (uit != tmp_unreachable.end()) {
        ...
        if (member_info->is_unreachable())
          log_message(
              MY_WARNING_LEVEL, "Member with address %s:%u is reachable again.",
              member_info->get_hostname().c_str(), member_info->get_port());
  ...
  if ((members.size() - unreachable.size()) <= (members.size() / 2)) {
    if (!left_members.empty()) {
      fprintf(stderr, "left is not empty here\n");
    ...
    if (!left_members.empty()) {
      ...
      for (it = left_members.begin(); it != left_members.end(); it++) {
        ...
        if (member_info != NULL) {
          log_message(
              MY_INFORMATION_LEVEL, "host:%s, port:%d has left the group",
              member_info->get_hostname().c_str(), member_info->get_port());
        }
        delete member_info;
      }
    ...
    /*
       This code is present on on_view_changed and on_suspicions as no
       assumption can be made about the order in which these methods are
       invoked.
     */
  ...
  log_message(MY_INFORMATION_LEVEL, "on_suspicions is called over");
}
...
Plugin_gcs_events_handler::log_members_leaving_message(const Gcs_view& new_view) const
{
  ...
  log_message(MY_WARNING_LEVEL, "Members removed from the group: %s",
...
Plugin_gcs_events_handler::log_members_joining_message(const Gcs_view& new_view) const
{
  ...
  log_message(MY_INFORMATION_LEVEL, "Members joined the group: %s",
...
void Plugin_gcs_events_handler::on_view_changed(
  ...
  log_message(MY_INFORMATION_LEVEL, "on_view_changed is called");
  ...
  if (!is_leaving &&
      group_partition_handler->is_partition_handling_terminated()) {
    log_message(MY_INFORMATION_LEVEL,
                "on_view_changed breaks the view install");
    return;
  }
  ...
  if (update_group_info_manager(new_view, exchanged_data, is_joining,
    ...
    view_change_notifier->cancel_view_modification();
    log_message(MY_INFORMATION_LEVEL,
                "on_view_changed calls cancel_view_modification");
    return;
  }
  ...
  if (!is_leaving) {
    log_message(
        MY_INFORMATION_LEVEL, "Group membership changed to %s on view %s.",
  ...
  log_message(MY_INFORMATION_LEVEL, "on_view_changed is called over");
}
...
void Plugin_gcs_events_handler::handle_leader_election_if_needed() const
{
  ...
  /*  Sort lower version members based on member weight if member version
          is greater than equal to PRIMARY_ELECTION_MEMBER_WEIGHT_VERSION or
     uuid.
   */
  ...
  if (!am_i_leaving) {
    ...
    if (sql_command_interface == NULL ||
        sql_command_interface->establish_session_connection(
            PSESSION_INIT_THREAD, get_plugin_pointer()) ||
        sql_command_interface->set_interface_user(GROUPREPL_USER)) {
      log_message(
          MY_WARNING_LEVEL,
          "Unable to open session to (re)set read only mode. Skipping."); /* purecov:
                                                                             inspected
                                                                           */
      /*
    ...
    if (the_primary == NULL) {
      for (it = all_members_info->begin();
        ...
        assert(mi);
        if (mi && mi->get_recovery_status() == Group_member_info::MEMBER_ONLINE)
          the_primary = mi;
      }
    }
    ...
    if (the_primary != NULL) {
      ...
      if (has_primary_changed) {
        ...
        group_member_mgr->update_member_role(
            primary_uuid, Group_member_info::MEMBER_ROLE_PRIMARY);

        log_message(MY_INFORMATION_LEVEL,
                    "A new primary with address %s:%u "
      ...
      /*
         If there is only one server in the group, no need to pollute the error
         log with an entry about no suitable candidate while (quick) recovery is
         running for the first member.
       */
      if (all_members_info->size() != 1) {
        // There are no servers in the group or they are all
        // recoverying WARN to the user
        log_message(MY_WARNING_LEVEL,
                    "Unable to set any member as primary. No suitable "
                    "candidate."); /* purecov: inspected */
      }
...
void Plugin_gcs_events_handler::handle_joining_members(const Gcs_view &new_view,
   ...
   */
    ...
    /**
      ...
      Associated to this process, we collect and intersect the executed GTID
      sets of all ONLINE members so we can cut the certification info to gather
      and transmit to the minimum.
     */

    std::string view_id = new_view.get_view_id().get_representation();
    View_change_packet *view_change_packet = new View_change_packet(view_id);
    collect_members_executed_sets(new_view.get_joined_members(),
                                  view_change_packet);
    applier_module->add_view_change_packet(view_change_packet);
  }
}
...
int Plugin_gcs_events_handler::process_local_exchanged_data(
  ...
  for (Exchanged_data::const_iterator exchanged_data_it =
    ...
    if (data == NULL) {
      ...
      if (member_info != NULL) {
        log_message(MY_ERROR_LEVEL,
                    "Member with address '%s:%u' didn't provide any data"
...
Plugin_gcs_events_handler::get_exchangeable_data() const
{
  ...
  if (sql_command_interface->establish_session_connection(
    ...
    log_message(
        ...
        "Operations and checks made to group joiners may be incomplete"); /* purecov:
                                                                             inspected
                                                                           */
...
Plugin_gcs_events_handler::check_group_compatibility(size_t number_of_members) const
{
  ...
  /*
     ...
     It can be incompatible because its major version is lower or a rule says
     it. If incompatible notify whoever is waiting for the view with an error,
     so the plugin exits the group.
   */
...
Plugin_gcs_events_handler::check_version_compatibility_with_group() const
{
  ...
  for (all_members_it = all_members->begin();
    ...
    compatibility_type = compatibility_manager->check_local_incompatibility(
        member_version, member_version == lowest_version);
...
void Plugin_gcs_events_handler::collect_members_executed_sets(
    const vector<Gcs_member_identifier> &joining_members,
...
Plugin_gcs_events_handler::leave_group_on_error() const
{
  ...
  case Gcs_operations::ALREADY_LEAVING:
    /* purecov: begin inspected */
    ss << "Skipping leave operation: concurrent attempt to leave the group is "
          "on-going.";
    break;
    /* purecov: end */
