...
static bool is_valid_password_by_changed_user_name(void *thd,
                                                   const char *buffer,
                                                   int length) {
  MYSQL_LEX_CSTRING user = {nullptr, 0};
  /*get changed user*/
  if (mysql_service_mysql_thd_gdb_current_user->get(thd, &user)) return false;
  /*compare user info*/
  if (!user.str || user.length != (size_t)length || user.length == 0)
    return true;

  return memcmp(buffer, user.str, user.length) &&
         !my_memcmp_reverse(user.str, user.length, buffer, length);
}
/**
...
static bool is_valid_password_by_user_name(void *thd, my_h_string password) {
  ...
  return is_valid_user(ctx, buffer, length, "user") &&
         ...
         is_valid_password_by_changed_user_name(thd, buffer, length);
}
/**
...
REQUIRES_SERVICE_PLACEHOLDER(mysql_thd_gdb_current_user);
...
REQUIRES_SERVICE(registry), REQUIRES_SERVICE(log_builtins),
    ...
    REQUIRES_SERVICE(mysql_thd_gdb_current_user), REQUIRES_PSI_MEMORY_SERVICE,
