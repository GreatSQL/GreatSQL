...
const char *common_insert_bug13509(char * /* out */, size_t num_tables,
                                   TA_lock_type lock_type, size_t ticket_fuzz,
                                   int commit_action) {
  const int ID_COL = 0;
  const int NAME_COL = 1;
  const int LOWERNAME_COL = 2;
  static const TA_table_field_def columns[] = {
      {ID_COL, "id", 2, TA_TYPE_INTEGER, true, 0},
      {NAME_COL, "name", 4, TA_TYPE_VARCHAR, true, 100},
      {LOWERNAME_COL, "lowername", 9, TA_TYPE_VARCHAR, true, 100}};
  ...
  ticket = ta_srv->add(access, "shop", 4, "bug13509", 11, lock_type);
  ...
  if (fa_varchar_srv->set(access, table, LOWERNAME_COL, name_value)) {
    result = "set(lowername) failed";
    goto cleanup;
  }
...
const char *test_insert_bug13509(char *out) {
  // Nominal path
  return common_insert_bug13509(out, 1, TA_WRITE, 0, 1);
}
...
static test_driver_t driver[] = {
    ...
    {"INSERT-BUG13509-TRIGGER", test_insert_bug13509},
